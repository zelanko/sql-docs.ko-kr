...
Data Migration Assistant의 v 4.0 릴리스에는 사용자가을 호스트 하는 컴퓨터에서 수집 된 성능 카운터를 기반으로 권장 되는 최소 Azure SQL Database SKU를 식별할 수 있도록 하는 Azure SQL Database SKU 권장 사항 기능이 도입 되었습니다. 데이터베이스인. 이 기능은 가격 책정 계층, 계산 수준 및 최대 데이터 크기와 관련 된 권장 사항 뿐만 아니라 월별 예상 비용을 제공 합니다. 또한 Azure에 모든 데이터베이스를 대량으로 프로 비전 하는 기능을 제공 합니다.
