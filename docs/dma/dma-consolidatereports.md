...
- 다운로드 및 설치:
  ...
  - [Power BI desktop](https://docs.microsoft.com/power-bi/desktop-get-the-desktop)합니다.
...
2. 엽니다는 [PowerShell 모듈](https://techcommunity.microsoft.com/gxcuf89792/attachments/gxcuf89792/MicrosoftDataMigration/56/4/PowerShell-Modules2.zip), 사용자가 만든 폴더에 저장 합니다.
...
필터에 다음 항목 중 하나에 현재 적용 되는지 여부를 표시 하도록 보고서 페이지의 왼쪽 아래에 메시지가 같습니다.
...
이 시각적 개체에서 선택 항목 컨텍스트 내에서 현재 발생 하는 문제를 보여 줍니다. (모든 인스턴스, 데이터베이스 [의 배수로]). 클수록 단어 표시 화면에서 해당 범주에는 문제가 많을 수록 됩니다. 단어 위에 마우스 포인터를 가져가면 해당 범주에서 발생 한 문제 수를 보여 줍니다.
