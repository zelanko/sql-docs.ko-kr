...
보기로 열기/확장의 예제에서 Azure Data Studio, PowerShell 명령 프롬프트에서 다음을 실행 합니다.
...
다음 예에서는 작업을 수행 합니다는 `dir` (별칭 `Get-ChildItem`)를 등록 된 서버 파일에 나열 된 모든 SQL Server 인스턴스 목록을 가져오고 사용 하 여는 `Get-SqlDatabase` 해당 인스턴스의 각 데이터베이스의 목록을 확인 하는 cmdlet입니다.
...
이 예에서는 `Get-SqlDatabase` ServerB 인스턴스에서 모든 데이터베이스 목록을 검색 하는 cmdlet은 다음 표/테이블을 제공 (사용 하 여는 `Out-GridView` cmdlet) 백업할 데이터베이스 선택 합니다.  사용자가 "확인" 단추를 클릭 한 후 강조 표시 된 데이터베이스만 백업 됩니다.
...
이 예제에서는 마찬가지로: 등록 된 서버 파일에 나열 된 모든 SQL Server 인스턴스 목록을 가져옵니다 호출을 `Get-SqlAgentJobHistory` 나열 된 각 SQL Server 인스턴스에 대 한 자정 이후의 모든 실패 한 SQL 에이전트 작업을 보고 하는 합니다.
