...
3. **커널** 및 대상 컨텍스트(**연결 대상**)가 채워질 때까지 기다립니다. **커널**이 **SQL**로 설정되어 있는지 확인하고, SQL Server의 **연결 대상**(여기서는 *localhost*)을 설정합니다.

   ![커널 및 연결 대상 설정](media/notebooks-sql-kernel/set-kernel-and-attach-to.png)
...
1. Notebook 도구 모음에서 **연결 대상** 메뉴를 선택하고 **연결 변경**을 선택합니다.
   ...
   ![Notebook 도구 모음에서 연결 대상 메뉴 선택](./media/notebooks-sql-kernel/select-attach-to-1.png)
...
2. 다음 예를 복사하여 셀에 붙여넣은 다음 **셀 실행**을 클릭합니다. 다음 예제에서는 새 데이터베이스를 만듭니다.
   ...
   ![셀 실행](media/notebooks-sql-kernel/run-notebook-cell.png)
...
![결과 저장](media/notebooks-sql-kernel/run-notebook-cell-2.png)
