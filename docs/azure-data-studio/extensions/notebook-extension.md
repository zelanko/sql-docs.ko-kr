...
description: 확장 생성기를 사용하여 확장에 Notebook을 패키지하는 방법을 알아봅니다.
...
ms.locfileid: "89288155"
---
# <a name="create-a-jupyter-notebook-extension"></a>Jupyter Notebook 확장 만들기

이 자습서에서는 새 Notebooks Azure Data Studio 확장을 만드는 방법을 보여 줍니다. 확장에는 Azure Data Studio에서 열고 실행할 수 있는 샘플 Jupyter Notebook이 제공됩니다.
...
확장을 만드는 과정을 간소화하기 위해 Yeoman을 사용하여 [확장 생성기](https://www.npmjs.com/package/generator-azuredatastudio)를 빌드했습니다. 설치하려면 명령 프롬프트에서 다음을 실행합니다.
...
2. 확장 유형 목록에서 **새 Notebooks(개별)** 를 선택합니다.

   :::image type="content" source="media/notebook-extension/notebook-extension-generator.png" alt-text="Notebook 확장 생성기":::

3. 단계를 따라 확장 이름(이 자습서에서는 **Test Notebook** 사용), 게시자 이름(이 자습서에서는 **Microsoft** 사용)을 입력하고 설명을 추가합니다.

이제 몇 가지 분기가 존재합니다. 이미 만든 Jupyter Notebook을 추가하거나, 생성기를 통해 제공되는 샘플 Notebook을 사용할 수 있습니다.
...
전달하려는 Notebook이 있는 경우 전달하려는 기존 Notebook이 있다고 답한 다음, 모든 Notebook 또는 markdown 파일이 있는 절대 파일 경로를 입력합니다.

이전 단계를 완료하면 샘플 Notebook이 포함된 새 폴더가 만들어집니다. Visual Studio Code에서 폴더를 열면 새 Notebook 확장을 전달할 준비가 된 것입니다.
...
`vsc-extension-quickstart.md`는 중요 파일에 대한 참조를 제공합니다. `README.md`에서 새 확장에 대한 문서를 제공할 수 있습니다. `package.json`, `notebook.ts` 및 `pySample.ipynb` 파일이 있습니다.
...
이는 `notebook.ts`의 기본 함수로, **Notebook 시작: Notebook 테스트** 명령을 통해 확장을 실행할 때마다 호출됩니다. `vscode.commands.registerCommand` API를 사용하여 새 명령을 만들며, 중괄호 안에 있는 다음 정의는 명령을 호출할 때마다 실행되는 코드입니다. `processNotebooks` 함수에서 발견되는 각 Notebook의 경우 `azdata.nb.showNotebookDocument`를 사용하여 Azure Data Studio에서 엽니다. 

또한 `package.json` 파일은 **Notebook 시작: Notebook 테스트** 명령을 등록하는 중요한 역할을 합니다.
...
명령에 대한 활성화 이벤트가 있으며 특정 기여 지점도 추가했습니다. 해당 활성화 이벤트와 기여 지점은 사용자가 확장을 볼 때 확장이 게시된 확장 Marketplace에 표시됩니다. 명령을 추가하려는 경우 이를 `activationEvents` 필드에 추가해야 합니다. 더 많은 옵션은 [활성화 이벤트](https://code.visualstudio.com/api/references/activation-events)를 참조하세요.
...
이 작업이 완료된 후에 my test-notebook-0.0.1.vsix 파일이 생성되었고, 이 파일을 설치하고 전 세계 사용자와 공유할 준비가 되었습니다.

## <a name="run-your-extension"></a>확장 실행

확장을 실행 및 테스트하려면 Azure Data Studio를 열고 명령 팔레트(`Ctrl + Shift + P`)를 엽니다. **확장: VSIX에서 설치** 명령을 찾은 다음, 새 확장이 포함된 폴더로 이동합니다.

   :::image type="content" source="media/notebook-extension/install-vsix.png" alt-text="VSIX 설치":::

이제 Azure Data Studio의 확장 패널에 표시될 것입니다. 명령 팔레트를 다시 열면 확장을 사용하여 만든 새 명령인 **Book 시작: Book 테스트**가 있을 것입니다. 실행 시 확장을 사용하여 패키지한 Jupyter Book을 열어야 합니다.
...
> [!div class="checklist"]
> - 확장 프로젝트 만들기
> - 확장 생성기 설치 - Notebook 확장 만들기
> - 확장 만들기
> - 확장 패키지
> - Marketplace에 확장 게시

이 내용을 읽으면 Azure Data Studio에 대한 고유한 확장을 빌드하는 데 도움이 될 것입니다.
