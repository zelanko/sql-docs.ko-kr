...
### <a name="to-create-a-table-report"></a>테이블 보고서를 만들려면  
...
8.  `CountryRegionID.Value`바로 뒤에 커서를 놓고 **,Fields!ID.value, Fields!CountryRegion.value, "CountryRegion")** 을 입력합니다.  
    ...
    **Lookup** 함수의 구문에서는 식 데이터 세트의 CountryRegionID와 CountryRegion 데이터 세트에서 CountryRegion 값을 반환하는 CountryRegion 데이터 세트의 ID 사이에 조회를 지정합니다.  
