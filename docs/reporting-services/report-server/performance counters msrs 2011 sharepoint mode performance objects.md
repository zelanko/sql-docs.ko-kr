---
title: "MSRS 2011 웹 서비스 SharePoint 모드 및 MSRS 2011 Windows 서비스 SharePoint 모드 성능 개체에 대한 성능 카운터(SharePoint 모드) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "성능 카운터 [Reporting Services]"
  - "카운터 [Reporting Services]"
  - "보고서 서버 Windows 서비스, 성능 카운터"
  - "RS Windows Service 성능 개체"
  - "일정 예약 및 배달 프로세서 성능 개체 [Reporting Services]"
...
manager: "erikre"
---
# MSRS 2011 웹 서비스 SharePoint 모드 및 MSRS 2011 Windows 서비스 SharePoint 모드 성능 개체에 대한 성능 카운터(SharePoint 모드)
  이 항목에서는 [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]SharePoint 모드 배포의 일부인 **MSRS 2011 웹 서비스 SharePoint 모드**및 **MSRS 2011 Windows 서비스 SharePoint 모드** 성능 개체에 대한 성능 카운터를 설명합니다.  
  
> [!NOTE]  
>  이 성능 개체는 로컬 보고서 서버의 이벤트를 모니터링합니다. 확장 배포에서 보고서 서버를 실행 중이면 카운터는 현재 서버에만 적용되고 확장 배포 전체에는 적용되지 않습니다.  
...
-   [MSRS 2011 웹 서비스 SharePoint 모드 성능 카운터](#bkmk_webservice)  
  
-   [MSRS 2011 Windows 서비스 SharePoint 모드 성능 카운터](#bkmk_windowsservice)  
...
|**경고: 이벤트 큐 길이**||  
|**경고: 이벤트 처리됨 - CreateSchedule**||  
|**경고: 이벤트 처리됨 - Delete Schedule**||  
|**경고: 이벤트 처리됨 - DeliverAlert**||  
|**경고: 이벤트 처리됨 - FireAlert**||  
|**경고: 이벤트 처리됨 - FireSchedule**||  
|**경고: 이벤트 처리됨 - GenerateAlert**||  
|**경고: 이벤트 처리됨 - UpdateSchedule**||  
...
##  <a name="bkmk_powershell"></a> PowerShell Cmdlet을 사용하여 목록 반환  
 ![PowerShell 관련 내용](../../analysis-services/instances/install-windows/media/rs-powershellicon.png "PowerShell 관련 내용")다음 Windows PowerShell 스크립트는 CounterSetName이 "msr"로 시작하는 카운터 집합을 반환합니다.  
...
```  
 ...
 다음 Windows PowerShell 스크립트는 CounterSetName "MSRS 2011 Windows 서비스 SharePoint 모드"의 성능 카운터 목록을 반환합니다.  
