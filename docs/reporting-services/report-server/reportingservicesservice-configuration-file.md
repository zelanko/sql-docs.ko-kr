...
helpviewer_keywords: 
  ...
  - "보고서 서버 Windows 서비스, ReportingServicesService 구성 파일"
...
|**접두사**|로그 인스턴스를 구분하는 생성 값을 지정합니다. 기본적으로 타임스탬프 값이 추적 로그 파일 이름에 추가됩니다. 이 값은 " tid, time"으로 설정됩니다. 이 설정은 수정하지 마세요.|  
