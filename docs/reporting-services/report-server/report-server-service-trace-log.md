...
|**Prefix**|로그 인스턴스를 구분하는 생성 값을 지정합니다.|기본적으로 타임스탬프 값이 추적 로그 파일 이름에 추가됩니다. 이 값은 "appdomain, tid, time"으로 설정됩니다. 이 설정은 수정하지 마세요.|  
|**TraceListeners**|추적 로그 내용을 출력할 대상을 지정합니다. 대상이 여러 개일 경우 쉼표로 구분하여 지정할 수 있습니다.|유효한 값은<br /><br /> <br /><br /> DebugWindow<br /><br /> File(기본값)<br /><br /> StdOut|  
