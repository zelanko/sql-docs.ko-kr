---
title: "SQL Server 장애 조치 클러스터에서 보고서 서버 데이터베이스 호스팅 | Microsoft Docs"
ms.custom: 
ms.date: 03/30/2016
...
# <a name="host-a-report-server-database-in-a-sql-server-failover-cluster"></a>SQL Server 장애 조치(failover) 클러스터에서 보고서 서버 데이터베이스 호스팅
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서는 장애 조치 클러스터링을 지원 하므로 여러 디스크를 사용 하 여 하나 이상의 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스. 장애 조치 클러스터링은 보고서 서버 데이터베이스에 대해서만 지원되므로 보고서 서버 서비스를 장애 조치 클러스터의 일부로 실행할 수 없습니다.  
