...
-   URL 액세스를 사용하면 SOAP API에 비해 보고서 렌더링 성능이 약간 높아집니다.  
 ...
 보고서 뷰어 컨트롤에는 기능이 풍부한 Windows 클라이언트 애플리케이션용과 [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] 애플리케이션용의 두 가지 버전이 있습니다. 이러한 컨트롤은 로컬 처리 및 원격 처리 모드를 모두 지원합니다. 로컬 처리 모드의 경우 응용 프로그램은 보고서 정의 및 데이터 집합을 제공하고 보고서 처리를 시작합니다. 원격 처리 모드의 경우에는 데이터 검색 및 보고서 처리는 보고서 서버에서 수행되며 컨트롤은 표시 및 보고서 탐색에 사용됩니다. 이 모델에서는 데스크톱에서 엔터프라이즈 수준까지 확장될 수 있는 기능이 풍부한 응용 프로그램을 구축할 수 있습니다.  
