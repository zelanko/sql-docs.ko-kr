---
description: 보고서 서버 항목에 대한 SharePoint 사이트 및 목록 사용 권한 참조
...
|항목 편집|목록의 항목, 문서 라이브러리의 문서, 문서의 웹 토론 설명을 각각 편집하고 문서 라이브러리의 웹 파트 페이지를 사용자 지정합니다.<br /><br /> 구독을 만들고 해당 사용자가 만든 구독을 편집합니다.|X|X||보고서 기록 스냅샷을 포함하여 문서의 이전 버전을 봅니다. 보고서 및 기타 문서에 대한 항목 속성을 편집합니다. 보고서 처리 옵션을 설정합니다. 보고서에 매개 변수를 설정합니다. 데이터 원본 속성을 편집합니다. 보고서 기록 스냅샷을 만듭니다. 보고서 작성기에서 보고서 모델이나 모델 기반 보고서를 열고 변경 내용을 파일에 저장합니다. 모델의 엔터티에 클릭 광고 보고서를 할당합니다. 보고서 정의, 공유 데이터 원본, 보고서 모델 또는 리소스를 최신 버전으로 바꿉니다(파일 바꾸기, 메타데이터 유지). 보고서나 모델에서 참조되는 종속 항목을 관리합니다. 특정 보고서를 기준으로 보고서 뷰어 웹 파트를 사용자 지정합니다.<br /><br /> [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 배달 확장 프로그램을 사용하여 보고서를 대상 위치에 배달하는 구독을 만들고 변경 및 삭제합니다. 알림 관리 권한이 있는 사용자와 구독 소유자만 이러한 동작을 수행할 수 있습니다.|  
...
|버전 보기|목록 항목이나 문서의 이전 버전을 봅니다.|X|X|X|문서의 이전 버전과 보고서 스냅샷을 봅니다.|  
|버전 삭제|목록 항목이나 문서의 이전 버전을 삭제합니다.|X|X||문서의 이전 버전과 보고서 스냅샷을 삭제합니다.|  
...
|경고 관리|웹 사이트의 모든 사용자에 대해 알림을 관리합니다.|X|||사이트의 모든 구독을 만들고 변경 및 삭제합니다.|  
