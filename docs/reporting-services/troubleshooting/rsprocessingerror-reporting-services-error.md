...
-   보고서 서버의 로컬 관리자로 실행 중인 경우 로그 파일에서 `ReportProcessingException`을 검색합니다. 로그 항목에 자세한 정보가 있습니다. 보고서 서버 로그 파일은 일반적으로 \< *드라이브*>: files\microsoft SQL Server\MSRS12 합니다. MSSQLSERVER\Reporting Services\LogFiles\ReportServerService__*datetimestamp*. 로그입니다. 자세한 내용은 [Reporting Services 로그 파일 및 소스](../../reporting-services/report-server/reporting-services-log-files-and-sources.md)를 참조하세요.  
...
-   평가 하지 못했습니다.는  *\<속성 이름 >*합니다. 오류가 있는 데이터 집합 필드 참조:  *\<오류 문자열 >*합니다.  
