---
title: 서버 및 데이터베이스 연결 문제 해결 | Microsoft Docs
description: 이 항목을 사용하여 보고서 서버에 연결할 때 발생하는 문제를 해결할 수 있습니다. 이 항목에서는 '예기치 않은 오류' 메시지에 대한 정보도 제공합니다.
ms.date: 12/16/2019
...
* Management Studio의 **서버에 연결** 대화 상자에서 **서버 이름**에 보고서 서버 URL을 입력합니다. 기본값은 `https://<your server name>/reportserver`입니다. 또는 SQL Server 2008 Express with Advanced Services를 사용 중인 경우 `https://<your server name>/reportserver$sqlexpress`입니다.  
...
오류에 다음 추가 텍스트가 포함되어 있는 경우, 데이터베이스 엔진 인스턴스를 실행하는 데 사용되는 계정에 대해 암호가 만료된 것입니다. "서버에 대한 연결을 구성하는 동안 오류가 발생했습니다. SQL Server에 연결할 때 기본 설정에서 SQL Server가 원격 연결을 허용하지 않기 때문에 이 오류가 발생할 수 있습니다. (**공급자: SQL Server 네트워크 인터페이스, 오류: 26 - 지정된 서버/인스턴스를 찾는 중 오류가 발생했습니다)** ." 이 오류를 해결하려면 암호를 다시 설정하십시오.   
