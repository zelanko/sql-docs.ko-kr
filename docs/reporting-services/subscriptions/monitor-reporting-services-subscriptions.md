...
ms.topic: "article"
helpviewer_keywords: 
  - "구독 [Reporting Services], 비활성"
  - "구독 [Reporting Services], 상태"
  - "모니터링 [Reporting Services], 구독"
  - " 상태 정보 [Reporting Services] "
  - " 비활성 구독 [Reporting Services] "
...
|\<filename> 파일에 쓰지 못했습니다. \<message>|파일 공유 위치에 배달하지 못했음을 나타냅니다. 이 메시지는 파일 공유 배달 확장 프로그램에서 제공합니다.|  
