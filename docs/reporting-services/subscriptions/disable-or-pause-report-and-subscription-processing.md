...
description: 구독 관리, 공유 일정 일시 중지, 공유 데이터 원본을 사용하지 않도록 설정, 보고서 액세스 차단, 구독 사용 권한 관리, 배달 확장 프로그램 제거 등을 수행합니다.
ms.date: 06/19/2019
...
>*구독 설정 및 해제*는 SQL 2016 Reporting Services의 새로운 기능입니다. 새로운 사용자 인터페이스 옵션을 통해 신속하게 구독을 설정하고 해제할 수 있습니다. 해제된 구독에는 일정과 같은 다른 구성 속성이 유지되며 손쉽게 설정이 가능합니다. 프로그래밍 방식으로 구독을 설정하고 해제하거나 어떤 구독이 해제되었는지에 대한 감사를 수행할 수 있습니다.  
  ...
  ![구독 페이지의 구독 설정 및 해제 단추 ](../../reporting-services/subscriptions/media/disable-or-pause-report-and-subscription-processing/subscription-enable-and-disable-buttons.png)  
  
웹 포털에서 개별 구독의 **내 구독** 페이지 또는 **구독** 페이지에서 구독의 위치를 탐색하여 찾을 수 있습니다. 구독을 하나 이상 선택한 후 리본에서 해제 단추 또는 설정 단추 를 클릭합니다(위쪽 이미지 참조). 상태 열은 각각 "사용 안 함"이나 "사용"으로 변경됩니다.  
  
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]는 구독이 설정되거나 해제될 때 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 로그에 행을 기록합니다. 예를 들어, 보고서 서버 로그 파일에:  
...
4.  **역할** 노드를 확장하고 원하는 역할을 선택합니다.  
  
5.  역할을 마우스 오른쪽 단추로 클릭한 다음 **속성**을 선택합니다.  
  
6.  **개별 구독 관리** 및 **모든 구독 관리** 작업을 선택 취소합니다.  
...
-   SharePoint 라이브러리(SharePoint 통합 모드 보고서 서버와 통합된 SharePoint 사이트에서만 사용 가능)  
 ...
 전자 메일 배달을 사용하려면 먼저 구성해야 합니다. 구성하지 않으면 사용할 수 없습니다. 자세한 내용은 [전자 메일 설정 - Reporting Services 기본 모드(구성 관리자)](../install-windows/e-mail-settings-reporting-services-native-mode-configuration-manager.md)를 참조하세요.  
  
 특정 확장 프로그램을 해제하려면 **RSReportServer.config** 파일에서 해당 확장 프로그램 항목을 제거합니다. 자세한 내용은 [Reporting Services 구성 파일](../../reporting-services/report-server/reporting-services-configuration-files.md) 및 [전자 메일 설정 - Reporting Services 기본 모드(구성 관리자)](../install-windows/e-mail-settings-reporting-services-native-mode-configuration-manager.md)를 참조하세요.  
