...
helpviewer_keywords: 
  - "일정 일시 중지"
  - "구독 [Reporting Services], 일시 중지"
  - "보고서 처리 [Reporting Services], 일시 중지"
  - "공유 데이터 원본 [Reporting Services]"
  - "구독 처리 일시 중지"
  - "보고서 처리 일시 중지"
  - "보고서 처리 일시 중지"
  - "공유 데이터 원본 해제"
  - "역할 [Reporting Services], 수정"
  - "공유 일정 [Reporting Services], 일시 중지"
...
>  [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]의 새로운 기능! **구독 설정 및 해제**. 새로운 사용자 인터페이스 옵션을 통해 신속하게 구독을 설정하고 해제할 수 있습니다. 해제된 구독에는 일정과 같은 다른 구성 속성이 유지되며 손쉽게 설정이 가능합니다. 프로그래밍 방식으로 구독을 설정하고 해제하거나 어떤 구독이 해제되었는지에 대한 감사를 수행할 수 있습니다.  
 ...
 ![PowerShell 관련 내용](../../analysis-services/instances/install-windows/media/rs-powershellicon.png "PowerShell 관련 내용") **Windows PowerShell을 사용하여 구독 해제:** 다음 PowerShell 스크립트를 사용하여 특정 구독을 해제합니다. 서버 이름 및 구독 ID를 업데이트합니다.  
...
```  
 ...
 ![PowerShell 관련 내용](../../analysis-services/instances/install-windows/media/rs-powershellicon.png "PowerShell 관련 내용") **Windows PowerShell을 사용하여 해제된 구독 모두 나열:** 다음 PowerShell 스크립트를 사용하여 현재 기본 모드 보고서 서버의 해제된 구독을 모두 나열합니다. 서버 이름을 업데이트합니다.  
...
```  
 ...
 ![PowerShell 관련 내용](../../analysis-services/instances/install-windows/media/rs-powershellicon.png "PowerShell 관련 내용") **Windows PowerShell을 사용하여 해제된 구독 모두 설정:** 다음 PowerShell 스크립트를 사용하여 현재 해제되어 있는 모든 구독을 설정합니다. 서버 이름을 업데이트합니다.  
...
```  
 ...
 ![PowerShell 관련 내용](../../analysis-services/instances/install-windows/media/rs-powershellicon.png "PowerShell 관련 내용") **Windows PowerShell을 사용하여 모든 구독 해제:** 다음 PowerShell 스크립트를 사용하여 **모든** 구독을 해제합니다.  
...
-   **SharePoint 모드:** ![SharePoint 설정](../../analysis-services/media/as-sharepoint2013-settings-gear.png "SharePoint 설정")**사이트 설정**에서 **공유 일정 관리**를 선택합니다. 일정을 선택하고 **선택한 일정 일시 중지**를 클릭합니다.  
...
-   SharePoint 라이브러리(SharePoint 통합 모드 보고서 서버와 통합된 SharePoint 사이트에서만 사용 가능)  
 ...
 전자 메일 배달을 사용하려면 먼저 구성해야 합니다. 구성하지 않으면 사용할 수 없습니다. 자세한 내용은 [메일 배달을 위한 보고서 서버 구성(SSRS 구성 관리자)](http://msdn.microsoft.com/ko-kr/b838f970-d11a-4239-b164-8d11f4581d83)을 참조하세요.  
  
 특정 확장 프로그램을 해제하려면 **RSReportServer.config** 파일에서 해당 확장 프로그램 항목을 제거합니다. 자세한 내용은 [Reporting Services 구성 파일](../../reporting-services/report-server/reporting-services-configuration-files.md) 및 [메일 배달을 위한 보고서 서버 구성(SSRS 구성 관리자)](http://msdn.microsoft.com/ko-kr/b838f970-d11a-4239-b164-8d11f4581d83)을 참조하세요.  
...
## 관련 항목:  
 ...
 [보안 속성 페이지, 항목&#40;보고서 관리자&#41;](../Topic/Security%20Properties%20Page,%20Items%20\(Report%20Manager\).md)  
