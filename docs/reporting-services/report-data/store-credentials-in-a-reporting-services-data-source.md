---
title: "Reporting Services 데이터 원본에 자격 증명을 저장 합니다. | Microsoft Docs"
ms.custom:
- SQL2016_New_Updated
ms.date: 09/23/2015
...
##  <a name="bkmk_top"></a> 저장된 자격 증명에 대한 보안 정책 요구 사항  
 ![as_powerpivot_refresh_sss_set_key](../../analysis-services/power-pivot-sharepoint/media/as-powerpivot-refresh-sss-set-key.gif "as_powerpivot_refresh_sss_set_key") 필요한 보고서 서버의 다음 보안 정책 중 하나에 대해 구성 된 저장 된 자격 증명에 사용할 계정입니다. 환경에서 필요한 최저 수준의 권한을 지닌 정책을 선택하는 것이 좋습니다.  
...
1.  기본 모드 보고서 관리자에서 보고서가 들어 있는 폴더를 찾습니다. 항목 상황에 맞는 메뉴 ![ssrs 항목에 대 한 보고서 관리자에서 상황에 맞는 메뉴](../../reporting-services/report-data/media/ssrs-report-manager-item-context-menu.png "ssrs 항목에 대 한 보고서 관리자에서 상황에 맞는 메뉴")합니다.  
...
8.  **적용**을 클릭합니다.  
     ...
     ![맨 위 링크를 다시 사용 되는 화살표 아이콘](../../analysis-services/instances/media/uparrow16x16.gif "위쪽 링크를 다시 사용 되는 화살표 아이콘") [저장 된 자격 증명에 대 한 보안 정책 요구 사항](#bkmk_top)  
  
##  <a name="bkmk_stored_credentials_data_source_sharepoint"></a> 보고서별 데이터 원본에 대해 저장된 자격 증명 구성(SharePoint 모드)  
  
1.  보고서가 포함 된 문서 라이브러리를 찾은 다음 열기 메뉴를 클릭 ![ssrs 항목에 대 한 문서 라이브러리 상황에 맞는 메뉴](../../reporting-services/report-data/media/ssrs-sharepoint-item-context-menu.png "ssrs 항목에 대 한 문서 라이브러리 상황에 맞는 메뉴")합니다.  
  
2.  두 번째 열기 메뉴 ![ssrs 항목에 대 한 문서 라이브러리 상황에 맞는 메뉴](../../reporting-services/report-data/media/ssrs-sharepoint-item-context-menu.png "ssrs 항목에 대 한 문서 라이브러리 상황에 맞는 메뉴") 클릭 하 고 **데이터 원본 관리**합니다.  
...
2.  상황에 맞는 메뉴 ![ssrs 항목에 대 한 보고서 관리자에서 상황에 맞는 메뉴](../../reporting-services/report-data/media/ssrs-report-manager-item-context-menu.png "ssrs 항목에 대 한 보고서 관리자에서 상황에 맞는 메뉴") 클릭 하 고 **관리**합니다.  
  
3.  **데이터 원본 유형** 목록에서 데이터 원본의 데이터를 처리하는 데 사용할 데이터 처리 확장 프로그램을 지정합니다.  
  
4.  **연결 문자열**에는 보고서 서버가 데이터 원본에 연결하는 데 사용하는 연결 문자열을 지정합니다. [!INCLUDE[msCoName](../../includes/msconame-md.md)]연결 문자열에 자격 증명을 지정 하지 않는 것이 좋습니다.  
...
2.  상황에 맞는 메뉴 ![ssrs 항목에 대 한 문서 라이브러리 상황에 맞는 메뉴](../../reporting-services/report-data/media/ssrs-sharepoint-item-context-menu.png "ssrs 항목에 대 한 문서 라이브러리 상황에 맞는 메뉴") 다음 두 번째 상황에 맞는 메뉴를 클릭 하 고 ![ssrs 항목에 대 한 문서 라이브러리 상황에 맞는 메뉴](../../reporting-services/report-data/media/ssrs-sharepoint-item-context-menu.png "ssrs 항목에 대 한 문서 라이브러리 상황에 맞는 메뉴")합니다.  
  
3.  **데이터 원본 정의 편집**을 클릭합니다.  
  
4.  **데이터 원본 유형** 목록에서 데이터 원본의 데이터를 처리하는 데 사용할 데이터 처리 확장 프로그램을 지정합니다.  
  
5.  **연결 문자열**에는 보고서 서버가 데이터 원본에 연결하는 데 사용하는 연결 문자열을 지정합니다. [!INCLUDE[msCoName](../../includes/msconame-md.md)]연결 문자열에 자격 증명을 지정 하지 않는 것이 좋습니다.  
...
6.  사용자 이름 및 암호를 입력합니다.  
    ...
    -   계정이 Windows 도메인 사용자 계정을 경우이 형식 지정: \<도메인 >\\< 계정\>를 선택한 후 **Windows 자격 증명으로 사용 합니다.**  
