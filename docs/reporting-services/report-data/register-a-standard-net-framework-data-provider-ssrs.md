...
helpviewer_keywords: 
  ...
  - "Reporting Services용 .NET Framework 데이터 공급자"
...
---
# 표준 .NET Framework 데이터 공급자 등록(SSRS)
...
4.  [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 데이터 공급자에 대한 항목을 추가합니다.  
    ...
    |**형식**|<xref:System.Data.IDbConnection> 인터페이스를 구현하는 클래스의 정규화된 네임스페이스 뒤에 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 데이터 공급자 어셈블리 이름(.dll 파일 이름 확장명 포함 안 함)이 쉼표로 구분되어 결합된 목록을 입력합니다.|  
