...
Power BI Report Server와 SQL Server Reporting Services 2016 이상은 모두 페이지를 매긴 보고서를 위해 **관리형 ODP.NET**을 사용합니다. 다음 단계는 Oracle ODAC 드라이버 12.2 이상을 사용하는 경우에만 필요합니다. Oracle ODAC 드라이버 12.2 이상은 새 Oracle 홈을 설치할 때 전체 머신에 적용되지 않는 구성에 기본 설치되기 때문입니다. 다음 단계에서는 ODAC 18.x 파일을 c:\oracle64에 설치했고 Oracle.ManagedDataAccess.dll 및 Oracle.DataAccess.dll의 파일 버전이 4.122.18.3이라고 가정합니다.
...
보고서 작성기는 페이지를 매긴 보고서의 작성을 위해 **관리형 ODP.NET**을 사용합니다. 다음 단계는 Oracle ODAC 드라이버 12.2 이상을 사용하는 경우에만 필요합니다. Oracle ODAC 드라이버 12.2 이상은 새 Oracle 홈을 설치할 때 전체 머신에 적용되지 않는 구성에 기본 설치되기 때문입니다. 다음 단계에서는 ODAC 18.x 파일을 c:\oracle32에 설치했고 Oracle.ManagedDataAccess.dll의 파일 버전이 4.122.18.3이라고 가정합니다.
...
Power BI 보고서는 **관리되지 않는 ODP.NET**을 사용합니다. 다음 단계는 Oracle ODAC 드라이버 12.2 이상을 사용하는 경우에만 필요합니다. Oracle ODAC 드라이버 12.2 이상은 새 Oracle 홈을 설치할 때 전체 머신에 적용되지 않는 구성에 기본 설치되기 때문입니다. 다음 단계에서는 ODAC 18.x 파일을 64비트의 경우 c:\oracle64에, 32비트의 경우 c:\oracle32에 설치했고 Oracle.DataAccess.dll의 파일 버전이 4.122.18.3이라고 가정합니다. 다음 단계를 수행하여 관리되지 않는 ODP.NET을 등록합니다.
