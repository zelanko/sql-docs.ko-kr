...
이 [!INCLUDE[ssRSnoversion_md](../includes/ssrsnoversion-md.md)] 자습서 단원에서는 웹 포털을 사용하여 받는 사람에게 배달될 보고서를 선택합니다. 사용자가 정의하는 데이터 기반 구독은 [기본 테이블 보고서 만들기&#40;SSRS 자습서&#41;](../reporting-services/create-a-basic-table-report-ssrs-tutorial.md) 자습서에서 만든 **Sales Order** 보고서를 배포합니다.  다음 단계에서는 보고서에서 데이터를 가져오는 데 사용되는 데이터 원본 연결 정보를 수정합니다. **저장된 자격 증명**을 사용하여 보고서 데이터 원본에 액세스하는 보고서만 데이터 기반 구독을 통해 배포할 수 있습니다. 저장된 자격 증명은 무인 보고서 처리에 필요합니다.  
...
1.  **보고서 데이터** 창에서 매개 변수 폴더를 확장하고 **Ordernumber** 매개 변수를 두 번 클릭합니다.  이 매개 변수는 이전 단계에서 데이터 집합에 매개 변수를 추가할 때 자동으로 생성되었습니다. **새로 만들기**를 클릭한 다음 **매개 변수...**를 클릭합니다.  
