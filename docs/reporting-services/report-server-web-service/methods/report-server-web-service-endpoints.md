...
> [!IMPORTANT]  
>  보고서 서버가 SharePoint 통합 모드로 구성 된 경우는 <xref:ReportService2005> Api 돌아갑니다는 **rsOperationNotSupportedSharePointMode** 오류입니다. 보고서 서버가 기본 모드로 구성 된 경우는 <xref:ReportService2006> Api 돌아갑니다는 **rsOperationNotSupportedNativeMode** 오류입니다. 마찬가지로 <xref:ReportService2010>의 모드별 API를 잘못된 모드에 사용하면 API에서 해당 오류를 반환합니다.  
...
```  
 ...
 자세한 내용은 참조 [SOAP API 액세스](../../../reporting-services/report-server-web-service/accessing-the-soap-api.md)합니다.  
...
```  
 ...
 자세한 내용은 참조 하십시오 [SOAP API 액세스](../../../reporting-services/report-server-web-service/accessing-the-soap-api.md)합니다.  
...
|프록시 끝점|Description|  
|--------------------|-----------------|  
|<xref:ReportService2006>|SharePoint 통합 모드로 구성된 보고서 서버를 관리하기 위한 API를 제공합니다.<br /><br /> 참고:이 끝점은에서 더 이상 사용 [!INCLUDE[ssKilimanjaro](../../../includes/sskilimanjaro-md.md)]합니다.|  
