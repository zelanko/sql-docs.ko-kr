---
title: 텍스트 상자(보고서 작성기) | Microsoft Docs
description: 테이블 또는 행렬에서 텍스트 상자도 포함하는 셀에 대해 알아봅니다. 이 텍스트 상자는 보고서 작성기에서 독립 실행형 텍스트 상자와 같은 방식으로 서식을 지정할 수 있습니다.
ms.date: 03/14/2017
...
> [!NOTE]  
>  보고서 데이터 세트 필드 값을 직접 보고서 디자인 화면이나 보고서 디자인 화면의 입력란으로 끌어서 놓으면 보고서를 실행할 때 결과 집합의 첫 번째 값만 표시됩니다. 필드의 모든 값을 보려면 먼저 테이블, 행렬 또는 목록 데이터 영역을 만든 다음 필드를 데이터 영역의 셀로 끌어 놓아야 합니다. 이렇게 하면 보고서를 실행할 때 해당 필드의 모든 값이 표시됩니다.  
...
## <a name="rotating-a-text-box"></a>입력란 회전  
 ...
 정적 텍스트, 보고서 데이터 세트의 필드 또는 계산 데이터가 포함된 입력란을 회전할 수 있습니다. 입력란은 보고서 본문, 테이블이나 행렬 또는 보고서 머리글 및 바닥글에서 독립 실행형으로 설정할 수 있습니다.  
