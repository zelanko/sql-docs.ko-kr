...
>  마법사는 보고서 작성기에서만 사용할 수 있습니다.  
 ...
 데이터 행에 대한 행 핸들은 기호 정보를 보여 줍니다(![3개의 평행선이 있는 정보 행에 대한 행 핸들](../../reporting-services/report-design/media/rs-icontablix-detailsrow.png "3개의 평행선이 있는 정보 행에 대한 행 핸들")). 이러한 행에서 데이터를 표시하려면 보고서 데이터 창의 필드를 머리글 행 또는 정보 행의 테이블 셀로 끌어옵니다. 두 행이 동시에 채워집니다. 열을 더 추가하려면 삽입 지점이 보일 때까지 해당 필드를 테이블로 끌어옵니다. 데이터 집합 필드를 테이블에 추가한 후에는 날짜 및 통화에 대한 기본 서식을 변경하여 보고서에 표시되는 방법을 제어할 수 있습니다. 다음 다이어그램에서는 Date, Order, Product, Qty 및 Line Total 필드가 있는 테이블 데이터 영역을 보여 줍니다.  
 ...
 ![열 머리글이 굵게 설정된 테이블의 미리 보기](../../reporting-services/media/rs-basictabledetailsformattedpreview.gif "열 머리글이 굵게 설정된 테이블의 미리 보기")  
...
##  <a name="AddingRowGroups"></a> 테이블에 행 그룹 추가  
 ...
 ![Date 및 Order 기준으로 그룹화된 테이블](../../reporting-services/media/rs-basictablegroupspreview.gif "Date 및 Order 기준으로 그룹화된 테이블")  
...
##  <a name="RemovingHidingRows"></a> 정보 행 제거 또는 숨기기  
 ...
 ![디자인 뷰, 2개의 그룹이 있고 정보가 없는 테이블](../../reporting-services/report-design/media/rs-basictablegroupsdrilldownnodetailsdesign.gif "디자인 뷰, 2개의 그룹이 있고 정보가 없는 테이블")  
...
>  정보 행을 제거한 후 각 셀의 식에 적절한 집계 식이 지정되어 있는지 확인합니다. 필요한 경우 식을 편집하여 집계 함수를 원하는 대로 지정합니다.  
 ...
 ![미리 보기, 그룹은 있고 정보 행이 없는 테이블](../../reporting-services/report-design/media/rs-basictablegroupsnodetailspreview.gif "미리 보기, 그룹은 있고 정보 행이 없는 테이블")  
