...
manager: "erikre"
---
# 보고서 매개 변수의 사용 가능한 값 추가, 변경 또는 삭제(보고서 작성기 및 SSRS)
 ...
 정적 값 목록 또는 보고서 데이터 집합에서 가져온 목록을 제공할 수 있습니다. 필요에 따라 레이블 필드를 지정하여 값에 대한 이름을 제공할 수 있습니다. 예를 들어 `ProductID` 필드를 기반으로 하는 매개 변수의 경우 매개 변수 레이블에 `ProductName` 필드를 표시할 수 있습니다. 보고서를 실행할 때 사용자는 제품 이름에서 선택할 수 있지만 실제 선택된 값은 해당 `ProductID`입니다.  
...
### 보고서 매개 변수에 대한 사용 가능한 값을 추가 또는 변경하려면  
...
2.  **사용 가능한 값**을 클릭합니다. 사용 가능한 값 옵션을 선택합니다.  
    ...
    -   **쿼리에서 값 가져오기** 를 클릭하여 이 매개 변수에 대한 값과 필요에 따라 이름을 검색하는 기존 데이터 집합의 이름을 제공합니다.  
        ...
        > [!IMPORTANT]  
        >  동일한 데이터 집합에 보고서 매개 변수에 대한 해당 쿼리 매개 변수가 있는 경우 보고서를 실행하려고 하면 보고서에서 오류 메시지를 표시합니다. 다른 데이터 집합을 사용하여 값을 검색하면 이 오류를 해결할 수 있습니다.  
