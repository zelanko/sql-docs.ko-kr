...
ms.topic: "article"
helpviewer_keywords: 
  - "배포 [Reporting Services], SharePoint 통합 모드의 보고서"
...
5.  (옵션) **TargetDataSourceFolder**에 SharePoint 라이브러리 또는 라이브러리 폴더의 URL(예: *http://TestServer/TestSite/Documents/DataSources*)을 입력합니다.  
  
     값을 지정하지 않으면 **TargetReportFolder** 값이 사용됩니다.  
  
6.  **TargetReportFolder**에 라이브러리 또는 라이브러리 폴더의 URL(예: *http://TestServer/TestSite/Documents/Reports*)을 입력합니다.  
  
7.  **TargetServerURL**에 SharePoint 최상위 사이트 또는 하위 사이트에 대한 URL을 입력합니다. 사이트를 지정하지 않으면 기본 최상위 사이트(예: *http://servername*, *http://servername/site* 또는 *http://servername/site/subsite*)가 사용됩니다.  
