...
다음으로 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)]를 설치 하 고 구성 합니다.

### <a name="install-sql-server-2019"></a>SQL Server 2019 설치

SQL Server 설치 설치 마법사 또는 명령 프롬프트를 사용 하 여 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)]를 설치 합니다.

1. `Setup.exe`를 열고 설치 마법사의 단계를 따릅니다.
...
1. [!INCLUDE[ssMDScfgmgr](../includes/ssmdscfgmgr-md.md)]를 연 다음 왼쪽 창에서 **데이터베이스 구성** 을 선택 합니다.
...
1. **데이터베이스 서버** 페이지에서 **SQL Server 인스턴스** 필드를 완성 한 다음 **인증 유형을**선택 합니다. **연결 테스트** 를 선택 하 여 선택한 인증 유형을 통해 데이터베이스에 연결 하는 데 자격 증명을 사용할 수 있는지 확인 합니다. **다음**을 선택합니다.
   ...
   > [!NOTE]
   > - SQL Server 인스턴스는 `xxxxxxx.xxxxxxx.database.windows.net`처럼 보입니다.
   > - 관리 되는 인스턴스의 경우 **"SQL Server 계정"** 및 **"현재 사용자-Active Directory 통합"** 인증 유형을 선택 합니다.
   > - **현재 사용자 –** 인증 유형으로 통합 Active Directory 선택 하는 경우 **사용자 이름** 필드는 읽기 전용 이며 현재 로그온 한 Windows 사용자 계정을 표시 합니다. Azure VM (가상 컴퓨터)에서 SQL Server 2019 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)]를 실행 하는 경우 **사용자 이름** 필드에 vm의 로컬 관리자 계정에 대 한 vm 이름 및 사용자 이름이 표시 됩니다.
...
1. **사용자 이름** 필드에서 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)]에 대 한 기본 슈퍼 사용자의 Windows 계정을 지정 합니다. 슈퍼 사용자는 모든 기능 영역에 액세스할 수 있으며 모든 모델을 추가, 삭제 및 업데이트할 수 있습니다.
...
1. [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)] 데이터베이스의 설정에 대 한 요약을 보려면 **다음** 을 선택 합니다. **다음** 을 다시 선택 하 여 데이터베이스를 만듭니다. **진행률 및 마침** 페이지가 표시 됩니다.
...
1. [!INCLUDE[ssMDScfgmgr](../includes/ssmdscfgmgr-md.md)]**데이터베이스 구성** 페이지에서 **데이터베이스 선택**을 선택 합니다.

1. **연결**을 선택 하 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)] 데이터베이스를 선택한 다음 **확인**을 선택 합니다.
...
1. [!INCLUDE[ssMDScfgmgr](../includes/ssmdscfgmgr-md.md)]의 왼쪽 창에서 **웹 구성** 을 선택 합니다.
...
1. **응용 프로그램 풀** 섹션에서 다른 사용자 이름을 입력 하 고 암호를 입력 한 다음 **확인**을 선택 합니다.
   ...
   > [!NOTE]
   > 사용자가 최근에 만든 Active Directory 통합 인증을 사용 하 여 데이터베이스에 액세스할 수 있는지 확인 합니다. 또는 나중에 `web.config`에서 연결을 변경할 수 있습니다.
...
1. **연결** 을 선택 하 고 웹 응용 프로그램과 연결할 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)] 데이터베이스를 선택 합니다. **확인**을 선택합니다.

   웹 사이트 설정을 완료 했습니다. 이제 **웹 구성** 페이지에 선택한 웹 사이트, 만든 웹 응용 프로그램 및 응용 프로그램과 관련 된 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)] 데이터베이스가 표시 됩니다.
...
1. **적용**을 선택합니다. **구성 완료** 메시지가 표시 됩니다. 메시지 상자에서 **확인** 을 선택 하 여 웹 응용 프로그램을 시작 합니다. 웹 사이트 주소를 `http://server name/web application/`합니다.

## <a name="configure-authentication"></a>인증 구성

관리 되는 인스턴스 데이터베이스를 웹 응용 프로그램에 연결 하려면 다른 인증 유형을 변경 해야 합니다.

`C:\Program Files\Microsoft SQL Server\150\Master Data Services\WebApplication`에서 `web.config` 파일을 찾습니다. 다른 인증 유형을 변경 하 여 관리 되는 인스턴스 데이터베이스에 연결 하려면 connectionString을 수정 합니다.

다음 샘플 연결 문자열에 표시 된 것 처럼 기본 인증 유형이 `Active Directory Integrated` 됩니다.
...
### <a name="upgrade-includessmdsshort_mdincludesssmdsshort-mdmd"></a>업그레이드 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)]
...
**2019 누적 업데이트 SQL Server**설치한 후에 `The client version is incompatible with the database version` 오류가 발생할 수 있습니다.
![SQLServer2019-MI-UpgradeDBPage](../master-data-services/media/mds-sqlserver2019-config-mi-upgradedbpage.png "mSQLServer2019-MI_UpgradeDBPage ")
...
1. 웹 응용 프로그램과 연결 된 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)] 데이터베이스를 선택 합니다. **연결**을 선택한 다음 **확인**을 선택 합니다.

   ![SQLServer2019-MI-ConnectDBName](../master-data-services/media/mds-sqlserver2019-config-mi-connectdbname.png "SQLServer2019-MI_ConnectDBName")

1. **데이터베이스 업그레이드** ...를 선택 합니다. 을 참조하세요.
