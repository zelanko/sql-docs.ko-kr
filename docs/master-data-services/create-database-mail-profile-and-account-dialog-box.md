 ...
 *데이터베이스 메일 프로필은* 데이터베이스 메일 계정의 모음입니다. *데이터베이스 메일 계정에는* SMTP 서버로 전자 메일 메시지를 보내는 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 데 사용하는 정보가 포함되어 있습니다. [!INCLUDE[ssMDScfgmgr](../includes/ssmdscfgmgr-md.md)]에서 프로필 및 계정을 만들면 계정이 프로필에 자동으로 추가되고 해당 계정 정보가 전자 메일을 보내는 데 사용됩니다.  
...
|컨트롤 이름|설명|  
|------------------|-----------------|  
|**메일 주소**|계정에 대한 전자 메일 주소의 이름을 입력합니다. 전자 메일이 보낸 전자 메일 주소이며*domain_name* *email_name*@형식이어야 합니다. 이메일 주소의 예는 sales@contoso.com입니다.|  
...
|**이 서버에는 보안 연결(SSL)이 필요합니다.**|이전에 는 SSL(보안 소켓 계층)이라고 불리던 TLS(전송 계층 보안)를 사용하여 통신을 암호화합니다.|  
...
|컨트롤 이름|설명|  
|------------------|-----------------|  
|**데이터베이스 엔진 서비스 자격 증명을 사용한 Windows 인증**|데이터베이스 메일SMTP 서버에서 인증을 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] [!INCLUDE[ssDEnoversion](../includes/ssdenoversion-md.md)] 위해 Windows 서비스 계정의 자격 증명을 사용하도록 지정합니다.|  
...
## <a name="see-also"></a>참고 항목  
 [데이터베이스 구성 페이지 &#40;마스터 데이터 서비스 구성 관리자&#41;](../master-data-services/database-configuration-page-master-data-services-configuration-manager.md)   
