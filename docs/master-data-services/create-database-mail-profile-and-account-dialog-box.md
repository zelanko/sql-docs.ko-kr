---
description: 데이터베이스 메일 프로필 및 계정 만들기 대화 상자
 ...
 *데이터베이스 메일 프로필* 은 데이터베이스 메일 계정 컬렉션입니다. *데이터베이스 메일 계정은* 에서 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 전자 메일 메시지를 SMTP 서버로 보내기 위해 사용 하는 정보를 포함 합니다. [!INCLUDE[ssMDScfgmgr](../includes/ssmdscfgmgr-md.md)]에서 프로필 및 계정을 만들면 계정이 프로필에 자동으로 추가되고 해당 계정 정보가 전자 메일을 보내는 데 사용됩니다.  
...
|컨트롤 이름|설명|  
|------------------|-----------------|  
|**메일 주소**|계정에 대한 전자 메일 주소의 이름을 입력합니다. 전자 메일을 보내는 전자 메일 주소 이며 *email_name* @ *domain_name*형식 이어야 합니다. 이메일 주소의 예는 sales@contoso.com입니다.|  
...
|**이 서버에는 보안 연결(SSL)이 필요합니다.**|TLS (전송 계층 보안)를 사용 하 여 이전에 SSL(Secure Sockets Layer) (SSL)로 알려진 통신을 암호화 합니다.|  
...
|컨트롤 이름|설명|  
|------------------|-----------------|  
|**데이터베이스 엔진 서비스 자격 증명을 사용한 Windows 인증**|데이터베이스 메일 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] [!INCLUDE[ssDEnoversion](../includes/ssdenoversion-md.md)] 에서 SMTP 서버 인증에 Windows 서비스 계정의 자격 증명을 사용 하도록 지정 합니다.|  
