...
[!INCLUDE[tsql-appliesto-ss2008-all_md](../../includes/tsql-appliesto-ss2008-all-md.md)]
  ...
  쿼리 결과 집합에 반환되는 행을 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]에서 지정한 행 수 또는 행의 백분율로 제한합니다. 결과 집합은 첫 번째 제한 TOP와 ORDER BY 절과 함께에서 사용할 경우 *N* 첫 번째 반환 순서가 지정 된 행의 수이 고; 그렇지 않으면, *N* 순서에 대 한 행의 수입니다. 이 절을 사용하여 SELECT 문에서 반환되거나 INSERT, UPDATE, MERGE 또는 DELETE 문의 영향을 받는 행 수를 지정할 수 있습니다.  
 ...
 *expression*  
 반환할 행의 수를 지정하는 숫자 식입니다. *식* 암시적으로 변환 됩니다는 **float** %가 고, 그렇지 않으면 지정 된 값을 변환할 **bigint**합니다.  
  
 PERCENT  
 첫 번째 쿼리가 반환 하는 나타냅니다 *식* 결과 집합에서 행의 백분율입니다. 소수 값은 다음 정수 값으로 올림됩니다.  
  
 WITH  TIES  
 제한된 결과 집합의 마지막 위치에 대해 일치하는 둘 이상의 행을 반환하려고 할 때 사용합니다. 함께 사용 해야는 **ORDER BY** 절. **WITH TIES** 하면 더 많은 행에 지정 된 값 보다 반환 될 수도 있습니다 *식*합니다. 예를 들어 경우 *식* 가 5로 설정 되었지만 2 추가 행의 값과 일치는 **ORDER BY** 행 5에 결과 집합 열 7 개의 행이 포함 됩니다.  
...
-   SELECT 문의 일부로 쿼리 최적화 프로그램에서 값을 고려할 수 *식* 쿼리를 최적화 하는 과정에서 TOP 또는 FETCH 절에 있습니다. SET ROWCOUNT는 쿼리를 실행하는 문 외부에서 사용되기 때문에 쿼리 계획에서 이 값을 고려할 수 없습니다.  
...
## <a name="interoperability"></a>상호 운용성  
 TOP 식은 트리거로 인해 실행될 수 있는 문에 영향을 주지 않습니다. **삽입** 및 **삭제** 트리거에서 테이블 진정으로 INSERT, UPDATE, MERGE 또는 DELETE 문에 의해 영향을 받는 행만 반환 합니다. 예를 들어 TOP 절이 사용된 INSERT 문으로 인해 INSERT TRIGGER가 실행된 경우  
...
## <a name="examples-includesssdwfullincludessssdwfull-mdmd-and-includesspdwincludessspdw-mdmd"></a>예: [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] 및[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]  
 다음 예제에서는 쿼리 조건과 일치 하는 상위 31 행을 반환 합니다. **ORDER BY** 31 번째 행의 알파벳 순서에 따라 처음 31 행이 반환 되도록 절을 사용 하 여 `LastName` 열입니다.  
  
 사용 하 여 **TOP** ties를 지정 하지 않고 있습니다.  
...
```  
 ...
 결과: 33 행이 반환 Brown 라는 3 직원 31 행에 대해 일치 하기 때문에 있습니다.  
