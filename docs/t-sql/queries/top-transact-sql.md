...
다음은 [!INCLUDE[ssSDW](../../includes/sssdwfull-md.md)] 및 [!INCLUDE[ssPDW](../../includes/sspdw-md.md)]에 대한 구문입니다.
...
SELECT 문에서 ORDER BY 절을 항상 TOP 절과 함께 사용합니다. 이는 방법은 TOP의 영향을 받는 행을 예측 가능한 방식으로 나타내는 유일한 방법이기 때문입니다.  
...
-   SELECT 문의 일부로 쿼리 최적화 프로그램은 쿼리를 최적화하는 과정에서 TOP 또는 FETCH 절에 있는 *expression* 값을 고려할 수 있습니다. SET ROWCOUNT는 쿼리를 실행하는 문 외부에서 사용하기 때문에 쿼리 계획에서 이 값을 고려할 수 없습니다.  
  
## <a name="compatibility-support"></a>호환성 지원  
이전 버전과의 호환성을 위해 SELECT 문에서 괄호를 사용하는 것은 선택 사항입니다. SELECT 문에서는 TOP에 항상 괄호를 사용하는 것이 좋습니다. 이렇게 하면 INSERT, UPDATE, MERGE 및 DELETE 문에서 해당 절을 사용해야 하는 경우 일관성이 유지됩니다. 
  
## <a name="interoperability"></a>상호 운용성  
TOP 식은 트리거로 인해 실행될 수 있는 문에 영향을 주지 않습니다. 트리거에서 **inserted** 및 **deleted** 테이블은 INSERT, UPDATE, MERGE 또는 DELETE 문의 영향을 받는 행만 반환합니다. 예를 들어 TOP 절을 사용한 INSERT 문으로 인해 INSERT TRIGGER가 실행된 경우  
...
ORDER BY 절이 연산자(이 경우 UNION ALL) 결과를 정렬하기 전에 TOP 절이 논리적으로 실행되므로 예기치 않은 결과가 반환됩니다. 따라서 이전 쿼리는 임의의 빨간색 차와 임의의 파란색 차를 반환한 다음 이 합집합의 결과를 가격순으로 정렬합니다. 다음 예에서는 원하는 결과를 얻을 수 있도록 이 쿼리를 올바르게 작성하는 방법을 보여 줍니다.  
...
TOP과 ORDER BY를 하위 SELECT 작업에서 사용하면 ORDER BY 절의 결과가 TOP 절에 적용되고 UNION 작업의 결과를 정렬하지 않습니다.  
...
INSERT, UPDATE, MERGE 또는 DELETE에서 TOP을 사용하면 참조된 행이 어떠한 순서로도 정렬되지 않습니다. 또한 이 문에서 ORDER BY 절을 직접 지정할 수 없습니다. TOP을 사용하여 시간순으로 행을 삽입, 삭제 또는 수정하려면 하위 SELECT 문에서 지정된 ORDER BY 절과 함께 TOP을 사용합니다. 이 문서에서 다음 예제 섹션을 참조하세요.  
  
분할된 뷰의 UPDATE 및 DELETE 문에서는 TOP을 사용할 수 없습니다.  
...
#### <a name="b-using-top-with-a-variable"></a>B. TOP에 변수 사용  
...
#### <a name="c-specifying-a-percentage"></a>C. 백분율 지정  
다음 예에서는 PERCENT를 사용하여 쿼리 결과 집합에 반환되는 직원 수를 지정합니다. `HumanResources.Employee` 테이블에 290명의 직원이 있습니다. 290의 5%는 소수이므로 값이 다음 정수로 반올림됩니다.  
...
TOP을 사용하여 시간 순서로 행을 삭제하려면 하위 SELECT 문에서 ORDER BY와 함께 TOP을 사용합니다. 다음 쿼리는 `PurchaseOrderDetail` 테이블에서 기한이 가장 빠른 10개의 행을 삭제합니다. 10개의 행만 삭제하기 위해 하위 SELECT 문에서 지정한 열(`PurchaseOrderID`)은 테이블의 기본 키입니다. 하위 SELECT 문에 키가 아닌 열을 사용하면 지정한 열에 중복 값이 있을 경우 10개가 넘는 행이 삭제될 수 있습니다.  
...
#### <a name="b-using-top-to-limit-the-number-of-rows-inserted"></a>B. TOP를 사용하여 삽입되는 행 수 제한  
...
TOP을 사용하여 시간 순서로 행을 삽입하려면 하위 SELECT 문에서 ORDER BY와 함께 TOP을 사용합니다. 다음 예제에서는 이 작업을 수행하는 방법을 보여 줍니다. OUTPUT  절은 `EmployeeSales` 테이블에 삽입되는 행을 표시합니다. 이제 정의되지 않은 행이 아니라 ORDER BY 절의 결과에 따라 상위 5명의 직원이 삽입됩니다.  
...
#### <a name="c-using-top-to-limit-the-number-of-rows-updated"></a>C. TOP를 사용하여 업데이트되는 행 수 제한  
다음 예에서는 TOP 절을 사용하여 테이블의 행을 업데이트합니다. UPDATE와 함께 TOP (*n*) 절을 사용하면 임의 개수의 행에 대해 업데이트 작업이 실행됩니다. 즉, UPDATE 문이 WHERE 절에 정의된 조건을 충족하는 행의 수(*n*)를 선택합니다. 다음 예에서는 한 영업 직원의 고객 10명을 다른 영업 직원에게 지정합니다.  
...
결과: Brown이라는 3명의 직원이 31번째 행에 대해 동률이기 때문에 33개 행이 반환됩니다.  
