...
-   에 대해 **마스터**, 인스턴스가 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가 단일 사용자 모드에서 실행 합니다.  
-   이외의 다른 데이터베이스에 대해 **마스터**, ALTER DATABASE 문을 사용 하 여 하 여 단일 사용자 모드로 데이터베이스를 배치 된에 있지만 합니다.  
-   읽기 전용 데이터베이스에 대해 실행된 경우  
-   ALTER DATABASE 문을 사용하여 응급 모드로 설정된 데이터베이스에 대해 실행된 경우  
-   에 대해 **tempdb**합니다. 이 경우 내부 제한 사항으로 인해 데이터베이스 스냅숏을 만들 수 없습니다.  
...
> [!NOTE]  
>  WITH TABLOCK 옵션을 사용하여 DBCC CHECKALLOC 또는 DBCC CHECKDB의 동등한 부분을 실행하려면 데이터베이스 X 잠금이 필요합니다. 이 데이터베이스 잠금을 설정할 수 없습니다 **tempdb** 또는 **마스터** 다른 모든 데이터베이스 에서도 실패할 수 있습니다.  
  
> [!NOTE]  
>  DBCC CHECKDB 실패에 대해 실행 될 때 **마스터** 내부 데이터베이스 스냅숏을 만들 수 없는 경우.  
  
## <a name="progress-reporting-for-dbcc-commands"></a>DBCC 명령에 대한 진행률 보고  
**sys.dm_exec_requests** 카탈로그 뷰는 진행률 및 DBCC CHECKDB, CHECKFILEGROUP 및 CHECKTABLE 명령 실행의 현재 단계에 대 한 정보를 포함 합니다. **percent_complete** 열에 있는 명령의 완료 비율에 표시 및 **명령** 열 명령 실행의 현재 단계를 보고 합니다.
...
|DBCC ALLOC CHECK|이 단계 동안 데이터베이스의 할당 구조를 검사합니다.<br /><br /> 참고: DBCC CHECKALLOC 동일한 검사를 수행합니다.|진행률이 보고되지 않습니다.|  
...
|DBCC SSB CHECK|이 단계 동안 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Service Broker 개체를 검사합니다.<br /><br /> 참고: DBCC CHECKTABLE이 실행 하는 경우이 단계가 실행 되지 않습니다.|진행률이 보고되지 않습니다.|  
|DBCC CHECKCATALOG|이 단계 동안 데이터베이스 카탈로그의 일관성을 검사합니다.<br /><br /> 참고: DBCC CHECKTABLE이 실행 하는 경우이 단계가 실행 되지 않습니다.|진행률이 보고되지 않습니다.|  
...
|[DBCC FREESYSTEMCACHE](../../t-sql/database-console-commands/dbcc-freesystemcache-transact-sql.md)|[DBCC CLONEDATABASE](https://support.microsoft.com/en-us/kb/3177838) <br /><br /> **적용 대상**: [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 서비스 팩 2입니다.|  
