 ...
 *table_name*  
 통계 표시를 포함 하는 테이블의 이름입니다. 테이블에는 외부 테이블 수 없습니다.  
  
 *대상*  
 통계 정보를 표시할 인덱스, 통계 또는 열의 이름입니다. *대상* 를 대괄호로 묶인 단일 따옴표, double, 따옴표 또는 따옴표 없음. 경우 *대상* 은 기존 인덱스의 이름 또는 테이블 또는 인덱싱된 뷰를이 대상에 대 한 통계 정보에 대 한 통계 반환 됩니다. 경우 *대상* 기존 열의 이름 및이 열에는 자동으로 만든된 통계가 있는 경우 해당 자동 생성 통계에 대 한 정보가 반환 됩니다. 열 대상에 자동으로 생성된 통계가 없는 경우에는 오류 메시지 2767이 반환됩니다.  
 [!INCLUDE[ssSDW](../../includes/sssdw-md.md)] 및 [!INCLUDE[ssPDW](../../includes/sspdw-md.md)], *대상* 열 이름이 될 수 없습니다.  
...
|문자열 인덱스|'예'는 통계 개체에 LIKE 연산자를 사용하는 쿼리 조건자(예: `WHERE ProductName LIKE '%Bike'`)의 카디널리티 예상치 정확도를 높이기 위한 문자열 요약 통계가 있음을 나타냅니다. 문자열 요약 통계 히스토그램에서 별도로 저장 되 고 유형은 때 통계 개체의 첫 번째 키 열에 만들어진 **char**, **varchar**, **nchar**, **nvarchar**, **varchar (max)**, **nvarchar (max)**, **텍스트**, 또는 **ntext.**합니다.|  
|필터 식|통계 개체에 포함된 테이블 행의 하위 집합에 대한 조건자입니다. NULL = 필터링되지 않은 통계입니다. 필터링 된 조건자에 대 한 자세한 내용은 참조 [Create Filtered Indexes](../../relational-databases/indexes/create-filtered-indexes.md)합니다. 필터링 된 통계에 대 한 자세한 내용은 참조 [통계](../../relational-databases/statistics/statistics.md)합니다.|  
|필터링되지 않은 행|필터 식을 적용하기 전 테이블에 있는 전체 행 수입니다. 필터 식이 NULL이면 필터링되지 않은 행과 행이 동일합니다.|  
|샘플 비율 유지|샘플링 비율을 명시적으로 지정 하지 않는 통계 업데이트에 사용 되는 샘플 비율을 유지 합니다. 값이 0 이면 없음 지속형된 샘플 비율이이 통계에 대해 설정 됩니다.<br /><br /> **적용 대상:** [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP1 CU4| 
...
## <a name="permissions-for-includesssdwincludessssdw-mdmd-and-includesspdwincludessspdw-mdmd"></a>에 대 한 권한을 [!INCLUDE[ssSDW](../../includes/sssdw-md.md)] 및[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]  
DBCC SHOW_STATISTICS는 테이블 또는 다음 중 하나에서 멤버 자격에 대 한 SELECT 권한이 필요합니다.
-   고정 서버 역할(fixed server role)  
-   db_owner 고정된 데이터베이스 역할  
-   db_ddladmin 고정된 데이터베이스 역할  
...
DBCC SHOW_STATISTICS 컨트롤 노드 수준에서 셸 데이터베이스에 저장 하는 통계를 표시 합니다. 의해 자동 생성 된 통계를 표시 하지 않습니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 계산 노드에 있습니다.
  
DBCC SHOW_STATISTICS 외부 테이블에서 지원 되지 않습니다.
...
다음 예에서는 `AK_Address_rowguid` 인덱스에 표시된 통계 정보를 HISTOGRAM 데이터로 제한합니다.
