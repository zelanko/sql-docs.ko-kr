...
REPAIR_REBUILD  
 ...
 > [!NOTE]  
 > REPAIR 옵션은 최후의 수단으로만 사용하십시오. 오류를 복구하려면 백업에서 복원하는 것이 좋습니다. 복구 작업이 수행될 경우 테이블 자체나 테이블 간에 존재할 수 있는 제약 조건이 고려되지 않습니다. 실행 중인 하나 이상의 제약 조건에 지정 된 테이블이 포함 된 경우 좋습니다 `DBCC CHECKCONSTRAINTS` 복구 작업 후 합니다.
 > REPAIR를 사용 해야 하는 경우 실행 `DBCC CHECKTABLE` 없이 복구 옵션을 사용할 복구 수준을 확인 합니다. REPAIR_ALLOW_DATA_LOSS 수준을 사용 하려는 경우 실행 하기 전에 데이터베이스를 백업 하는 것이 좋습니다 `DBCC CHECKTABLE` 이 옵션을 사용 합니다.  
...
MAXDOP  
 **적용 대상**: [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] (부터는 [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] s p 2부터 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]).  
...
- 부터는 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)], 지속형된 계산된 열, UDT 열 및 필터링 된 인덱스에 대 한 추가 검사 비용이 많이 드는 식 평가 방지 하기 위해 기본적으로 실행 되지 것입니다. 이러한 변경에는 이러한 개체를 포함 하는 데이터베이스에 대 한 CHECKDB 기간의 크게 줄여 줍니다. 그러나 이러한 개체의 물리적 일관성 검사 항상 완료 됩니다. EXTENDED_LOGICAL_CHECKS 옵션을 지정 하는 경우에 식 평가 아니라 수행 됩니다 (인덱싱된 뷰, XML 인덱스 및 공간 인덱스) 이미 있는 논리적 검사가 EXTENDED_LOGICAL_CHECKS 옵션의 일부로 합니다.
-  호환성 수준이 90 이면 ([!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]) 이하인 NOINDEX가 지정 되지 않으면 DBCC CHECKTABLE 수행 물리적 및 논리적 일관성 검사는 단일 테이블 또는 인덱싱된 뷰에 대 한 모든 해당 비클러스터형 및 XML 인덱스 모두 합니다. 공간 인덱스는 지원되지 않습니다.
