...
### <a name="a-returning-the-starting-position-of-an-expression"></a>A. 식의 시작 위치 반환  
다음 예제에서는 검색된 `bicycle` 문자열 값 변수에서 `@document`을 검색합니다.
...
### <a name="b-searching-from-a-specific-position"></a>B. 특정 위치에서 검색  
다음 예제에서는 선택적 *start_location* 매개 변수를 사용하여 검색된 `vital` 문자열 값 변수의 다섯 번째 문자에서 `@document`에 대한 검색을 시작합니다.
...
다음 예제에서는 `'Test'`에서 `'This is a Test'` 문자열에 대한 대/소문자 구분 검색을 보여 줍니다.
...
다음 예제에서는 `'TEST'`에서 `'This is a Test'` 문자열에 대한 대/소문자를 구분하지 않는 검색을 보여 줍니다.
...
다음 예제에서는 `is`의 위치 1(첫 번째 문자)에서 시작하여 `This is a string` 문자열 중 `This is a string` 문자열의 첫 번째 위치를 반환합니다.
