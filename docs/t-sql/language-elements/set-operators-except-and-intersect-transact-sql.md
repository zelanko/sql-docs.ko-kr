...
---
# <a name="set-operators---except-and-intersect-transact-sql"></a>-연산자를 제외 하 고 설정 하 고 INTERSECT (TRANSACT-SQL)
[!INCLUDE[tsql-appliesto-ss2008-all-md](../../includes/tsql-appliesto-ss2008-all-md.md)]
 ...
 INTERSECT는 왼쪽 및 오른쪽 입력된 쿼리 연산자에서 출력 인 고유한 행을 반환 합니다.  
...
## <a name="arguments"></a>인수  
 \<*query_specification*> | ( \< *query_expression*>)  
 다른 쿼리 사양이나 쿼리 식의 데이터와 비교할 데이터를 반환하는 쿼리 사양 또는 쿼리 식입니다. EXCEPT 또는 INTERSECT 연산의 일부인 열의 정의는 같을 필요는 없지만 암시적 변환을 통해 비교할 수 있어야 합니다. 비교를 수행 하 고 결과 확인할 반환 하는 데 사용 되는 형식에 대 한 규칙에 따라 데이터 형식이 다를 때 [데이터 형식 우선 순위](../../t-sql/data-types/data-type-precedence-transact-sql.md)합니다.  
 ...
 쿼리 사양이 나 식은 반환할 수 없습니다 **xml**, **텍스트**, **ntext**, **이미지**, 또는 이진이 아닌 CLR 사용자 정의 형식 열 이러한 데이터 형식은 비교할 수 없는 합니다.  
  
 EXCEPT  
 오른쪽 쿼리에서 반환 되지 않는 EXCEPT 연산자의 왼쪽에 쿼리의 모든 고유한 값을 반환 합니다.  
  
 INTERSECT  
 INTERSECT 연산자의 왼쪽과 오른쪽에 있는 두 쿼리에 의해 반환된 고유한 값을 반환합니다.  
  
## <a name="remarks"></a>주의  
 규칙에 따라 필요한 비교가 수행 됩니다 EXCEPT의 오른쪽 및 왼쪽에 있는 쿼리에 의해 반환 되는 비교 가능한 열의 데이터 형식 또는 INTERSECT 연산자는 서로 다른 데이터 정렬 문자 데이터 형식 때 [ 데이터 정렬 선행 규칙](../../t-sql/statements/collation-precedence-transact-sql.md)합니다. 이러한 변환을 수행할 수 없는 경우 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]에서 오류를 반환합니다.  
...
3.  식에서 EXCEPT와 UNION의 위치를 기준으로 왼쪽에서 오른쪽으로 계산되는 EXCEPT 및 UNION  
 ...
 EXCEPT 연산이의 그래픽 실행 계획 기능을 사용 하 여 표시 되 면 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], 작업으로 표시는 [왼쪽 앤티 세미 조인](../../relational-databases/showplan-logical-and-physical-operators-reference.md), INTERSECT 연산은 및는 [왼쪽된 세미 조인](../../relational-databases/showplan-logical-and-physical-operators-reference.md)합니다.  
  
## <a name="examples"></a>예  
 다음 예제를 사용 하 여 보여는 `INTERSECT` 및 `EXCEPT` 연산자입니다. 첫 번째 쿼리는 해당 결과를 `Production.Product` 및 `INTERSECT`와 비교하기 위해 `EXCEPT` 테이블의 모든 값을 반환합니다.  
