...
## <a name="factors-affecting-the-duration-of-checkpoint-operations"></a>검사점 작업 기간에 영향을 주는 요인  
 일반적으로 검사점 작업에 필요한 시간은 기록해야 할 더티 페이지 수에 따라 증가합니다. 다른 애플리케이션의 성능에 미치는 영향을 최소화하기 위해 기본적으로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서는 검사점 작업이 기록하는 빈도를 조정합니다. 기록 빈도를 줄이면 검사점 작업을 완료하는 데 필요한 시간이 늘어납니다. CHECKPOINT 명령에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]checkpoint_duration*값이 지정되지 않으면*에서 수동 검사점에 대해 이 전략을 사용합니다.  
