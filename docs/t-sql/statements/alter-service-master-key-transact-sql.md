...
## <a name="arguments"></a>인수  
 FORCE  
 데이터 손실 위험이 있는 경우에도 서비스 마스터 키가 다시 생성되어야 함을 나타냅니다. 자세한 내용은 참조 [SQL Server 서비스 계정 변경](#_changing) 이 항목의 뒷부분에 나오는 합니다.  
...
##  <a name="_changing"></a>SQL Server 서비스 계정 변경  
 ...
 FORCE 옵션을 사용하면 프로세스에서 현재 마스터 키를 검색할 수 없거나 키로 암호화된 모든 개인 키의 암호를 해독할 수 없는 경우에도 키 다시 생성 프로세스가 계속됩니다. 재생성 실패할 경우에 FORCE 사용 하 고 사용 하 여 서비스 마스터 키를 복원할 수 없습니다는 [RESTORE SERVICE MASTER KEY](../../t-sql/statements/restore-service-master-key-transact-sql.md) 문.  
...
>  서비스 마스터 키는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 암호화 계층의 루트입니다. 서비스 마스터 키는 트리에 있는 모든 다른 키와 암호를 직접 또는 간접적으로 보호합니다. 강제 다시 생성 중에 종속된 키의 암호를 해독할 수 없는 경우 키가 보호하는 데이터가 손실됩니다.  
 ...
 SQL을 다른 컴퓨터로 이동하는 경우 동일한 서비스 계정을 사용하여 SMK를 해독해야 하며 SQL Server에서 컴퓨터 계정 암호화를 자동으로 해결합니다.  
