...
|DROP EVENT \<event_specifier>|*\<event_specifier>* 로 식별되는 이벤트를 삭제합니다. \<event_specifier>가 이벤트 세션에서 유효해야 합니다.|  
...
|SET { *target_parameter_name*= \<value> [, ... *n* ] }|대상 매개 변수를 설정합니다. 대상 매개 변수는 sys.dm_xe_object_columns 뷰에 column_type 'customizable' 및 object_name = *target_name* 으로 표시됩니다.<br /><br /> **참고!** 링 버퍼 대상을 사용하는 경우 XML 출력의 데이터 잘림을 피하려면 max_memory 대상 매개 변수를 2048KB로 설정하는 것이 좋습니다. 다양한 대상 유형을 사용할 경우에 대한 자세한 내용은 [SQL Server 확장 이벤트 대상](/previous-versions/sql/sql-server-2016/bb630339(v=sql.130))을 참조하세요.|  
|DROP TARGET \<event_target_specifier>|\<event_target_specifier>로 식별되는 대상을 삭제합니다. \<event_target_specifier>가 이벤트 세션에서 유효해야 합니다.|  
