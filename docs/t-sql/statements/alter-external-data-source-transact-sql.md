...
## <a name="arguments"></a>인수  
 ...
 LOCATION = 'server_name_or_IP' 외부 데이터 원본에 대한 연결 프로토콜 및 경로를 제공합니다.

 RESOURCE_MANAGER_LOCATION = '\<IP 주소;포트>'(Azure SQL Data Warehouse에 적용되지 않음) Hadoop Resource Manager 위치를 지정합니다. 지정된 경우 쿼리 최적화 프로그램은 Hadoop의 계산 기능을 사용하여 PolyBase 쿼리의 데이터를 사전 처리하도록 선택할 수 있습니다. 이것은 비용 기반 결정입니다. 조건자 푸시 다운이라고 불리는 이 기능은 Hadoop과 SQL 간에 전송되는 데이터 양을 크게 줄여 쿼리 성능을 향상시킵니다.
