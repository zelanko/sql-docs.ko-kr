...
> [!NOTE]  
> **Azure [!INCLUDE[ssSDS](../../includes/sssds-md.md)]** V12는 2014년 12월에 릴리스되었습니다. 해당 릴리스의 한 가지 측면은 새로 만든 데이터베이스는 해당 호환성 수준을 120으로 설정했다는 것입니다. 2015 SQL 데이터베이스는 기본값이 120으로 남아 있지만 수준 130에 대한 지원을 시작했습니다.  
> 
> **2016년 6월 중순**부터 [!INCLUDE[ssSDS](../../includes/sssds-md.md)]에서 기본 호환성 수준은 **새로 만든** 데이터베이스에 대해 120 대신 130입니다. 2016년 6월 중순 이전에 만든 기존 데이터베이스는 영향을 받지 않으며 현재 호환성 수준을 유지합니다(100, 110 또는 120). 
> 
> 일반적으로 데이터베이스에 대해 수준 130을 원하지만 110 **카디널리티 추정** 알고리즘을 선호하는 이유가 있는 경우 [ALTER DATABASE SCOPED CONFIGURATION&#40;Transact-SQL&#41;](../../t-sql/statements/alter-database-scoped-configuration-transact-sql.md) 및 특히 해당 키워드 `LEGACY_CARDINALITY_ESTIMATION = ON`을 참조하세요.  
>  
>  [!INCLUDE[ssSDS](../../includes/sssds-md.md)]에서 두 호환성 수준 간의 가장 중요한 쿼리의 성능 차이를 평가하는 방법에 대한 내용은 [Azure SQL Database에서 호환성 수준 130으로 향상된 쿼리 성능](http://azure.microsoft.com/documentation/articles/sql-database-compatibility-level-query-performance-130/)을 참조하세요.
