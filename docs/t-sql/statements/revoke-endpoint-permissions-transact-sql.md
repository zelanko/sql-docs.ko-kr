---
title: "REVOKE 끝점 사용 권한 (Transact SQL) | Microsoft Docs"
ms.custom: 
ms.date: 08/10/2017
...
### <a name="b-revoking-take-ownership-permission-with-the-cascade-option"></a>2. CASCADE 옵션을 지정하여 TAKE OWNERSHIP 권한 취소  
 다음 예에서는 취소 `TAKE OWNERSHIP` 끝점에 대 한 `Shipping83` 에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 사용자 `PKomosinski` 있는 모든 보안 주체 로부터 `PKomosinski` 부여 `TAKE OWNERSHIP` 에 `Shipping83`합니다.  
