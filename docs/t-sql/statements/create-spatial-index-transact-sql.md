...
[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]는 세 부분으로 구성된 이름 형식(database_name.[schema_name].object_name)을 지원합니다. 단, database_name이 현재 데이터베이스이거나 database_name이 tempdb이고 object_name이 #으로 시작해야 합니다.  
