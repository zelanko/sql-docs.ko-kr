...
## <a name="arguments"></a>인수  
 DATABASE  
 **지 원하는:**[복원  ](../../t-sql/statements/restore-statements-transact-sql.md)  
...
> [!NOTE]  
>  일반적으로 마지막으로 복원된 로그가 비상 로그 백업입니다. A *비상 로그 백업* 은 로그 백업 수행 되는 데이터베이스에서 오류가 발생 한 후 일반적으로 데이터베이스를 복원 하기 전에. 손상될 가능성이 있는 데이터베이스에서 비상 로그 백업을 수행하면 아직 백업되지 않은 로그(비상 로그)를 캡처하여 작업이 손실되지 않도록 방지할 수 있습니다. 자세한 내용은 [비상 로그 백업&#40;SQL Server&#41;](../../relational-databases/backup-restore/tail-log-backups-sql-server.md)을 참조하세요.  
 ...
 **지 원하는:**[복원  ](../../t-sql/statements/restore-statements-transact-sql.md)  
 ...
 로그나 전체 데이터베이스가 복원되는 데이터베이스입니다. 변수로 제공한 경우 (**@***database_name_var*),이 이름은 문자열 상수를 지정 ( **@**   *database_name_var* = *데이터베이스*_*이름*) 또는 문자 문자열 데이터 형식의 변수를 제외 하 고는 **ntext** 또는 **텍스트** 데이터 형식입니다.  
 ...
 **지 원하는:**[복원  ](../../t-sql/statements/restore-statements-transact-sql.md)  
 ...
 단순 복구 모델을 사용 하는 데이터베이스에 대 한 파일 및 파일 그룹 옵션은 대상 파일 또는 파일 그룹이 읽기 전용 있거나 부분 복원 인 경우에 허용 (결과 [존재 하지 않는 파일 그룹](../../relational-databases/backup-restore/remove-defunct-filegroups-sql-server.md)).  
...
-   마지막으로 백업 된 전에 복원할 파일이 읽기 전용으로 설정 된 경우-트랜잭션 로그가 적용 될 필요는 없습니다 하 고 RESTORE 문에서 이러한 상황의 사용자에 게 알립니다.  
  
-   백업에 주 파일 그룹이 포함되고 부분 복원이 수행되고 있는 경우. 이런 경우에는 로그가 백업 세트에서 자동으로 복원되기 때문에 복원 로그가 필요하지 않습니다.  
  
파일  **=**  { *logical_file_name_in_backup*| **@***logical_file_name_in_backup_var*}  
 데이터베이스 복원에 포함할 파일의 이름을 지정합니다.  
  
파일 그룹  **=**  { *logical_filegroup_name* | **@***logical_filegroup_name_var* }  
 데이터베이스 복원에 포함할 파일 그룹의 이름을 지정합니다.  
  
 **참고** 지정된 된 파일 그룹은 읽기 전용 및 경우에 부분 복원 (즉, WITH PARTIAL이 사용 됨) 하는 경우에 FILEGROUP은 단순 복구 모델에서 사용할 수 있습니다. 복원되지 않은 읽기/쓰기 파일 그룹은 존재하지 않는 것으로 표시되므로 결과 데이터베이스로 복원할 수 없습니다.  
...
>  페이지 복원은 복구되지 않습니다.  
 ...
 페이지 복원에 대 한 자세한 내용은 참조 [페이지 복원 &#40; SQL Server &#41; ](../../relational-databases/backup-restore/restore-pages-sql-server.md).  
  
 [ **,**...*n* ]  
 여러 개의 파일 및 파일 그룹과 페이지를 쉼표로 구분된 목록에 지정할 수 있음을 나타내는 자리 표시자입니다. 사용할 수 있는 숫자에는 제한이 없습니다.  
  
{ \<backup_device > [ **,**... *n* ]| \<database_snapshot >} 일반적으로 백업을 복원 하려는 백업 장치를 지정 합니다. 또는 RESTORE DATABASE 문의 FROM 절에서 데이터베이스를 되돌릴 데이터베이스 스냅숏의 이름을 지정할 수도 있습니다. 이런 경우 WITH 절은 사용할 수 없습니다.  
 ...
 **지 원하는:**[복원](../../t-sql/statements/restore-statements-transact-sql.md), [RESTORE FILELISTONLY](../../t-sql/statements/restore-statements-filelistonly-transact-sql.md), [RESTORE HEADERONLY](../../t-sql/statements/restore-statements-headeronly-transact-sql.md), [RESTORE LABELONLY](../../t-sql/statements/restore-statements-labelonly-transact-sql.md), [ RESTORE REWINDONLY](../../t-sql/statements/restore-statements-rewindonly-transact-sql.md), 및 [RESTORE VERIFYONLY](../../t-sql/statements/restore-statements-verifyonly-transact-sql.md)합니다.  
 ...
 만든 백업 장치의의 식별자에 대 한 규칙을 따라야 하는 논리적 이름 **sp_addumpdevice** 에서 데이터베이스를 복원 합니다. 변수로 제공한 경우 (**@***logical_backup_device_name_var*), 백업 장치 이름은 문자열 상수를 지정 ( **@**  *logical_backup_device_name_var* = *logical_backup_device_name*) 또는 문자 문자열 데이터 형식의 변수를 제외 하 고는 **ntext** 또는 **텍스트** 데이터 형식입니다.  
  
 {디스크 | 테이프}  **=**  { **'***physical_backup_device_name***'**  |   **@**  *physical_backup_device_name_var* }  
 지정한 디스크나 테이프 장치에서 백업을 복원할 수 있습니다. 디스크 및 테이프의 장치 유형은 장치의 실제 이름 (예를 들어 전체 경로 및 파일 이름)으로 지정 해야: `DISK ='Z:\SQLServerBackups\AdventureWorks.bak'` 또는 `TAPE ='\\\\.\TAPE0'`합니다. 변수로 지정 된 경우 (**@***physical_backup_device_name_var*), 장치 이름은 문자열 상수를 지정 ( **@**  *physical_backup_device_name_var* = '*physcial_backup_device_name*') 또는 문자 문자열 데이터 형식의 변수를 제외 하 고는 **ntext**또는 **텍스트** 데이터 형식입니다.  
  
 네트워크 서버에 UNC 이름(컴퓨터 이름 포함)을 사용하는 경우 디스크의 장치 유형을 지정합니다. UNC 이름의 사용 하는 방법에 대 한 자세한 내용은 참조 하십시오. [백업 장치 &#40; SQL Server &#41; ](../../relational-databases/backup-restore/backup-devices-sql-server.md).  
...
\<database_snapshot >:: =  
**지 원하는:**[데이터베이스 복원  ](../../t-sql/statements/restore-statements-transact-sql.md)  
  
DATABASE_SNAPSHOT  **=**  *database_snapshot_name*  
 데이터베이스에서 지정한 데이터베이스 스냅숏으로 되돌립니다 *database_snapshot_name*합니다. DATABASE_SNAPSHOT 옵션은 전체 데이터베이스 복원에만 사용할 수 있습니다. 되돌리기 작업에서는 데이터베이스 스냅숏이 전체 데이터베이스 백업을 대신합니다.  
  
 되돌리기 작업을 수행하려면 지정한 데이터베이스 스냅숏이 데이터베이스에서 유일한 스냅숏이어야 합니다. 되돌리기 작업 중에 데이터베이스 스냅숏과 대상 데이터베이스는 둘 다 `In restore`로 표시됩니다. 자세한 내용은 "주의" 섹션을 참조 하십시오. [데이터베이스 복원](../../t-sql/statements/restore-statements-transact-sql.md)합니다.  
  
### <a name="with-options"></a>WITH 옵션  
 복원 작업에서 사용할 옵션을 지정합니다. 각 옵션을 사용하는 문에 대한 간략한 설명은 이 항목의 뒷부분에 나오는 "WITH 옵션 지원에 대한 요약"을 참조하십시오.  
  
> [!NOTE]  
>  옵션으로 구성 됩니다 여기에서 "구문" 섹션에서와 동일한 순서로 [복원 {데이터베이스 | 로그}](../../t-sql/statements/restore-statements-transact-sql.md)합니다.  
 ...
 *standby_file_name* 위치는 데이터베이스의 로그에 저장 된 대기 파일을 지정 합니다. 기존 파일이 지정한 이름을 사용하고 있으면 해당 파일을 덮어쓰고 그렇지 않으면 [!INCLUDE[ssDE](../../includes/ssde-md.md)]에서 파일을 만듭니다.  
...
>  지정한 대기 파일 이름을 포함하는 드라이브의 여유 공간이 모두 사용된 경우에는 복원 작업이 중지됩니다.  
 ...
 RECOVERY와 NORECOVERY 비교의 "주의" 섹션을 참조 하십시오. [복원](../../t-sql/statements/restore-statements-transact-sql.md)합니다.  
  
LOADHISTORY  
 **지 원하는:**[RESTORE VERIFYONLY  ](../../t-sql/statements/restore-statements-verifyonly-transact-sql.md)  
  
 복원 작업에 대 한 정보를 로드 되도록 지정 된 **msdb** 기록 테이블입니다. LOADHISTORY 옵션은 단일 백업 세트를 확인에 대 한 정보에 대 한 정보를 로드 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 미디어에 저장 된 백업으로 백업 설정 및 복원 기록 테이블에는 **msdb** 데이터베이스입니다. 기록 테이블에 대 한 자세한 내용은 참조 하십시오. [시스템 테이블 &#40; Transact SQL &#41; ](../../relational-databases/system-tables/system-tables-transact-sql.md).  
  
#### <a name="generalwithoptions--n-"></a>\<general_WITH_options > [,...n]  
 일반 WITH 옵션은 모두 RESTORE DATABASE 및 RESTORE LOG 문에서 지원됩니다. 이러한 옵션 중 일부에서 아래 설명 된 대로 하나 이상의 보조 문에서 지원 됩니다.  
 ...
 **지 원하는:**[복원](../../t-sql/statements/restore-statements-transact-sql.md) 및 [RESTORE VERIFYONLY  ](../../t-sql/statements/restore-statements-verifyonly-transact-sql.md)  
  
 데이터 또는 로그 파일을 지정 하 여 논리적 이름이 지정 된 *logical_file_name_in_backup* 로 지정 된 위치에 복원 하 여 설정할지 *operating_system_file_name*합니다. 백업 세트에 있는 데이터 또는 로그 파일의 논리적 파일 이름은 백업 세트 생성 시 데이터베이스의 해당 논리적 이름과 일치합니다.  
  
*n*추가 MOVE 문을 지정 하는 나타내는 자리 표시자입니다. 백업 세트에서 새 위치로 복원할 모든 논리적 파일에 대해 MOVE 문을 지정합니다. 기본적으로는 *logical_file_name_in_backup* 파일을 원래 위치로 복원 됩니다.  
...
>  백업 세트에서 논리적 파일 목록을 가져오려면 RESTORE FILELISTONLY를 사용하십시오.  
 ...
 와 함께 사용할 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 스냅숏은 백업 MOVE 옵션 파일 Azure에 다시 배치 하는 데에 사용할 수 있습니다 하는 원본 blob와 동일한 저장소 계정 내에서 blob입니다. 로컬 파일에 또는 다른 저장소 계정에 스냅숏 백업을 복원 하려면 MOVE 옵션을 사용할 수 없습니다.  
...
**적용 대상**: [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] SP1 CU2-[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]
 ...
 Microsoft Azure Blob 저장소 서비스로 백업을 복원 하는 경우에 사용 합니다.  
  
> [!NOTE]  
>  와 [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 까지 SP1 CU2 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)], URL에서 복원할 때만 단일 장치에서 복원할 수 있습니다. 사용 해야 하는 URL에서 복원 하는 경우 여러 장치에서 복원 하기 위해 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 통해 [현재 버전](http://go.microsoft.com/fwlink/p/?LinkId=299658)) 공유 액세스 서명 (SAS) 토큰을 사용 해야 합니다. 자세한 내용은 참조 [Enable SQL Server Managed Backup to Microsoft Azure](../../relational-databases/backup-restore/enable-sql-server-managed-backup-to-microsoft-azure.md) 및 [Powershell 포함 Azure Storage에서 공유 액세스 서명 (SAS) 토큰으로 SQL 자격 증명 만들기 간소화](http://blogs.msdn.com/b/sqlcat/archive/2015/03/21/simplifying-creation-sql-credentials-with-shared-access-signature-sas-keys-on-azure-storage-containers-with-powershell.aspx)합니다.  
...
-   데이터베이스 이름이 백업 세트에 기록된 데이터베이스 이름과 다를 경우  
 ...
 REPLACE 옵션을 사용 하 여 영향 정보를 참조 하십시오. [restore&#40; Transact SQL &#41; ](../../t-sql/statements/restore-statements-transact-sql.md).  
 ...
 **지 원하는:**[복원](../../t-sql/statements/restore-statements-transact-sql.md), [RESTORE FILELISTONLY](../../t-sql/statements/restore-statements-filelistonly-transact-sql.md), [RESTORE HEADERONLY](../../t-sql/statements/restore-statements-headeronly-transact-sql.md), 및 [RESTORE VERIFYONLY](../../t-sql/statements/restore-statements-verifyonly-transact-sql.md)합니다.  
 ...
 기본값은 지정 되지 않은 경우 **1**, RESTORE HEADERONLY를 제외 하 고 모든 백업 세트는 미디어 세트에서 처리 되는 경우. 자세한 내용은 이 항목의 뒷부분에 나오는 "백업 세트 지정"을 참조하십시오.  
...
> [!NOTE]  
>  메모리 내 OLTP 파일 그룹 또는 데이터베이스의 FILESTREAM을 구성한 경우 또는 포함 `MAXTRANSFERSIZE` 복원할 때에는 백업을 만들 때 사용 된 것 보다 크거나 이어야 합니다.  
...
>  페이지 체크섬은 백업 체크섬이 사용될 때만 백업 작업에 사용됩니다.  
 ...
 백업 체크섬 작업에 대 한 자세한 내용은 참조 하세요. [가능한 미디어 오류 시 백업 및 복원 &#40; SQL Server &#41; ](../../relational-databases/backup-restore/possible-media-errors-during-backup-and-restore-sql-server.md).  
 ...
 **지 원하는:**[복원](../../t-sql/statements/restore-statements-transact-sql.md) 및 [RESTORE VERIFYONLY  ](../../t-sql/statements/restore-statements-verifyonly-transact-sql.md)  
 ...
 새로 백분율이 완료될 때마다 메시지를 표시하여 진행 상태를 측정하는 데 사용됩니다. 경우 *백분율* 를 생략 하면 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 각 10%가 약 완료 후 메시지를 표시 합니다.  
...
>  UNLOAD/NOUNLOAD는 세션 기간 동안이나 다른 옵션을 지정하여 다시 설정할 때까지 유지되는 세션 설정입니다.  
 ...
 NOUNLOAD  
 후 복원 작업에서 테이프를 테이프 드라이브에 로드 된 상태로 유지 되도록 지정 합니다.  
...
로그 전달을 사용 하려면 복제를 설정할 때 KEEP_REPLICATION을 사용 합니다. 그러면 대기 중인 서버에서 데이터베이스 백업이나 로그 백업이 복원되고 데이터베이스가 복구될 때 복제 설정이 제거되지 않도록 할 수 있습니다. NORECOVERY 옵션을 사용하여 백업을 복원하는 동안 이 옵션을 지정할 수 없습니다. 복원 후 복제가 제대로 실행되도록 하려면 다음을 수행하십시오.  
  
-   **msdb** 및 **마스터** 웜 대기 서버에서 데이터베이스와 동기화 되어야 합니다는 **msdb** 및 **마스터** 주 데이터베이스 서버입니다.  
 ...
 **지 원하는:**[복원  ](../../t-sql/statements/restore-statements-transact-sql.md)  
 ...
 KEEP_CDC로 데이터베이스를 복원 해도 변경 데이터 캡처 작업을 만들지 않습니다. 데이터베이스 복원 후 로그에서 변경 내용을 추출하려면 복원된 데이터베이스에 대한 캡처 프로세스 작업 및 정리 작업을 다시 만드십시오. 정보를 참조 하십시오. [sys.sp_cdc_add_job&#40; Transact SQL &#41; ](../../relational-databases/system-stored-procedures/sys-sp-cdc-add-job-transact-sql.md).  
  
 변경 데이터 캡처를 사용 하 여 데이터베이스 미러링을 사용 하는 방법에 대 한 정보를 참조 하십시오. [변경 데이터 캡처 및 기타 SQL Server 기능](../../relational-databases/track-changes/change-data-capture-and-other-sql-server-features.md)합니다.  
 ...
 **지 원하는:**[데이터베이스 복원  ](../../t-sql/statements/restore-statements-transact-sql.md)  
 ...
 ENABLE_BROKER  
 지정 하는 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 즉시 메시지를 보낼 수 있도록 복원이 끝날 때 메시지 배달 활성화 됩니다. 기본적으로 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 메시지 배달은 복원 작업 동안 해제됩니다. 데이터베이스에는 기존 Service Broker 식별자가 유지됩니다.  
...
> [!NOTE]  
>  RESTORE_DATABASE 및 restore_log 지정 지정 시간 WITH 옵션은 RESTORE LOG만 지원 하지만 *이라는* 인수입니다.  
  
 { STOPAT | STOPATMARK | STOPBEFOREMARK }   
 
 STOPAT  **=**  { **'***datetime***'**  |   **@**  *datetime_var* }  
 날짜 및 지정 된 시간에 맞는 원래의 상태로 데이터베이스를 복원할 수 지정는 *datetime* 또는  **@**  *datetime_var* 매개 변수입니다. 날짜 및 시간을 지정 하는 방법에 대 한 정보를 참조 하십시오. [날짜 및 시간 데이터 형식 및 함수 &#40; Transact SQL &#41; ](../../t-sql/functions/date-and-time-data-types-and-functions-transact-sql.md).  
  
 STOPAT에는 변수를 사용 하는 경우 변수 이어야 합니다 **varchar**, **char**, **smalldatetime**, 또는 **datetime** 데이터 형식입니다. 지정한 날짜와 시간 전에 작성된 트랜잭션 로그 레코드만 데이터베이스에 적용됩니다.  
...
>  지정한 STOPAT 시간이 마지막 LOG 백업 이후의 시간이면 데이터베이스는 RESTORE LOG가 NORECOVERY로 실행된 것처럼 복구되지 않은 상태가 됩니다.  
 ...
 STOPATMARK  **=**  { **'***이라는***'** | **'**lsn: *lsn_number***'** } [AFTER **'***datetime***'** ]  
 지정된 복구 지점으로의 복구를 지정합니다. 지정된 트랜잭션은 복구에 포함되지만 트랜잭션이 실제로 생성될 때 원래 커밋된 경우에만 커밋됩니다.  
  
 RESTORE DATABASE 및 RESTORE LOG를 모두 지원은 *lsn_number* 매개 변수입니다. 이 매개 변수는 로그 시퀀스 번호를 지정합니다.  
  
 *이라는* 매개 변수는 RESTORE LOG 문에서 지원 합니다. 이 매개 변수는 로그 백업에서 트랜잭션 표시를 식별합니다.  
  
 RESTORE LOG 문에서 after *datetime* 가 지정 된 이름의 첫 번째 표시 지점에서 복구가 중지 생략 합니다. After *datetime* 에서 또는 이후에 지정한 이름이 있는 첫 번째 표시 지점에서 복구가 중지 지정 된 *datetime*합니다.  
  
> [!NOTE]  
>  지정한 표시, LSN 또는 시간이 마지막 LOG 백업 이후의 시간이면 데이터베이스는 RESTORE LOG가 NORECOVERY로 실행된 것처럼 복구되지 않은 상태가 됩니다.  
  
 자세한 내용은 참조 [관련 데이터베이스를 일관 되 복구 &#40;에 표시 된 트랜잭션을 사용 하 여 전체 복구 모델 &#41; ](../../relational-databases/backup-restore/use-marked-transactions-to-recover-related-databases-consistently.md) 및 [로그 시퀀스 번호 &#40;에 복구 SQL Server &#41; ](../../relational-databases/backup-restore/recover-to-a-log-sequence-number-sql-server.md).  
  
 STOPBEFOREMARK  **=**  { **'***이라는***'** | **'**lsn: *lsn_number***'** } [AFTER **'***datetime***'** ]  
 ...
 *이라는* 매개 변수는 RESTORE LOG 문에서 지원 합니다. 이 매개 변수는 로그 백업에서 트랜잭션 표시를 식별합니다.  
 ...
 RESTORE LOG 문에서 after *datetime* 를 생략 하면 지정 된 이름의 첫 번째 표시 지점 바로 앞 복구가 중지 됩니다. After *datetime* 복구가 중지에서 지정된 된 이름이 있는 첫 번째 표시 하기 바로 전에 또는 후 지정 된 *datetime*합니다.  
...
## <a name="specifying-a-backup-set"></a>백업 세트 지정  
 A *백업 세트* 단일 성공한 백업 작업에서 백업이 포함 되어 있습니다. RESTORE, RESTORE FILELISTONLY, RESTORE HEADERONLY 및 RESTORE VERIFYONLY 문은 지정한 백업 장치의 미디어 세트 내에 있는 단일 백업 세트에서 작동합니다. 미디어 세트 내에서 필요한 백업을 지정해야 합니다. 백업 세트의 *backup_set_file_number* 는 [RESTORE HEADERONLY](../../t-sql/statements/restore-statements-headeronly-transact-sql.md) 문을 사용하여 가져올 수 없습니다.  
  
 복원할 백업 세트를 지정하는 옵션은 다음과 같습니다.  
  
 파일  **=** { *backup_set_file_number* | **@***backup_set_file_number* }  
  
 여기서 *backup_set_file_number* 백업 미디어 세트의 위치를 나타냅니다. A *backup_set_file_number* 1 (파일 = 1) 첫 번째 백업에서 백업 미디어 세트 및 *backup_set_file_number* 2 (파일 = 2)와 두 번째 백업 세트를 나타냅니다.  
  
 이 옵션의 동작은 다음 표에 설명 된 대로 문에 따라 달라 집니다.  
...
|RESTORE HEADERONLY|기본적으로 미디어 세트에 있는 모든 백업 세트가 처리됩니다. RESTORE HEADERONLY 결과 집합에는 각 백업 세트에 대 한 정보를 반환 합니다. 포함 하 여 해당 **위치** 은 미디어 세트에서 합니다. 으로 해당 위치 번호를 사용 하 여 지정된 된 백업 세트에 정보를 반환 하는 *backup_set_file_number* 파일 옵션에는 값입니다.<br /><br /> 참고: 테이프 미디어에 대 한 헤더 복원만 처리는 로드 된 테이프에 백업 세트 합니다.|  
|RESTORE VERIFYONLY|기본 *backup_set_file_number* 는 1입니다.|  
  
> [!NOTE]  
>  백업 세트를 지정 하는 파일 옵션은 데이터베이스 파일, 파일을 지정 하는 파일 옵션 상관이 없습니다  **=**  { *logical_file_name_in_backup*  |   **@**  *logical_file_name_in_backup_var* }.  
  
## <a name="summary-of-support-for-with-options"></a>WITH 옵션 지원에 대한 요약  
 RESTORE 문에서 지원 됩니다 같은 WITH 옵션: BLOCKSIZE, BUFFERCOUNT, MAXTRANSFERSIZE, PARTIAL, KEEP_REPLICATION, {RECOVERY | NORECOVERY | STANDBY}, REPLACE, RESTART, RESTRICTED_USER 및 {STOPAT | STOPATMARK | STOPBEFOREMARK}  
...
|{언로드 &#124; NOUNLOAD}|√|√|√|√|√|√|  
 ...
 <sup>1</sup> 파일  **=**  *backup_set_file_number*과 다릅니다 {파일 | 파일 그룹을 (를)입니다.  
