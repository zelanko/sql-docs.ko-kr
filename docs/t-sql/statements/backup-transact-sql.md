...
> 전체 데이터베이스 백업만 수행할 수는 **마스터** 데이터베이스입니다.  
 ...
 지정 하 여 특정 시간이 나 백업 내의 트랜잭션 로그 백업을 복원할 수 있습니다 `WITH STOPAT`, `STOPATMARK`, 또는 `STOPBEFOREMARK` 에 프로그램 [RESTORE LOG](../../t-sql/statements/restore-statements-transact-sql.md) 문.  
  
> [!NOTE]  
>  일반적인 로그 백업 후 일부 트랜잭션 로그 레코드가 비활성 상태가 지정 하지 않으면 `WITH NO_TRUNCATE` 또는 `COPY_ONLY`합니다. 하나 이상의 가상 로그 파일 내의 모든 레코드가 비활성 상태가 된 후에는 로그가 잘립니다. 일상적인 로그 백업 후 로그가 잘리지 않을 경우 로그 잘림이 지연되는 것일 수 있습니다. 자세한 내용은 참조 [로그 잘림을 지연 시킬 수 있는 요소](../../relational-databases/logs/the-transaction-log-sql-server.md#FactorsThatDelayTruncation)합니다.  
  
 { *database_name* | **@ * * * database_name_var* }은 데이터베이스를 트랜잭션 로그, 부분 데이터베이스 또는 전체 데이터베이스 백업 됩니다. 변수로 제공한 경우 (**@***database_name_var*),이 이름은 문자열 상수를 지정 (**@***database_name_var***= * * * 데이터베이스 이름*)로 변수 문자 데이터 형식을 제외 하 고에 대 한 문자열의 **ntext** 또는 **텍스트** 데이터 형식입니다.  
 ...
 *n*  
 ...
 자세한 내용은의 "미디어 패밀리가 미러된 미디어 세트의" 참조는 [주의](#general-remarks) 이 항목의 뒷부분에 나오는 섹션.  
...
> [!NOTE]  
> 기본적으로 `BACKUP DATABASE` 전체 데이터베이스 백업을 만듭니다.  
  
 자세한 내용은 [차등 백업&#40;SQL Server&#41;](../../relational-databases/backup-restore/differential-backups-sql-server.md)을 참조하세요.  
  
 ENCRYPTION  
 백업에 대한 암호화를 지정하는 데 사용됩니다. 사용 하 여 백업을 암호화 하거나 지정 하는 암호화 알고리즘을 지정할 수 있습니다 `NO_ENCRYPTION` 에 없는 다음 백업이 암호화 합니다. 암호화는 백업 파일을 보호하는 데 권장되는 방법입니다. 지정할 수 있는 알고리즘의 목록은 다음과 같습니다.  
...
> [!WARNING]  
> 와 함께에서 암호화가 사용 하는 경우는 `FILE_SNAPSHOT` 인수, 메타 데이터 파일 자체는 지정 된 암호화 알고리즘을 사용 하 여 암호화 되 고 시스템을 확인 하는 [투명 한 데이터 암호화 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption.md) 완료 에 대 한 데이터베이스입니다. 데이터 자체에 대 한 추가 암호화 되지 않은 오류 발생합니다. 데이터베이스 암호화 되지 않은 이거나 백업 문이 실행 된 암호화 하기 전에 완료 되지 않은 경우 백업이 실패 합니다.  
...
> [!NOTE]  
> 복원 작업에 백업 세트를 지정 하려면 사용 된 `FILE = <backup_set_file_number>` 옵션입니다. 백업 세트를 지정 하는 방법에 대 한 자세한 내용은 "백업 세트 지정"의 참조 [RESTORE 인수 &#40; Transact SQL &#41; ](../../t-sql/statements/restore-statements-arguments-transact-sql.md).
...
-   와 함께 사용할 경우 `BACKUP DATABASE`, `COPY_ONLY` 옵션 차등 기반으로 사용할 수 없는 전체 데이터베이스 백업을 만듭니다. 차등 비트맵은 업데이트되지 않으며 차등 백업은 복사 전용 백업이 없는 것처럼 작동합니다. 후속 차등 백업은 가장 최근의 기존 전체 백업을 기반으로 사용합니다.  
  
    > [!IMPORTANT]  
    > 경우 `DIFFERENTIAL` 및 `COPY_ONLY` 를 함께 사용 되는 `COPY_ONLY` 무시 되 고 차등 백업이 생성 됩니다.  
  
-   와 함께 사용할 경우 `BACKUP LOG`, `COPY_ONLY` 만듭니다는 *복사 전용 로그 백업은*, 트랜잭션 로그를 자르지 않는 합니다. 복사 전용 로그 백업은 로그 체인에 영향을 주지 않으며 기타 로그 백업은 복사 전용 백업이 없는 것처럼 작동합니다.  
...
와 백업 압축을 사용 하는 방법에 대 한 내용은 [투명 한 데이터 암호화 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption.md) 설정 된 데이터베이스 참조는 [주의](#general-remarks) 섹션.
...
> [!NOTE]  
> 만료 날짜를 무시 하려면 사용 하 여는 `SKIP` 옵션입니다.  
...
> [!NOTE]  
> 간의 상호 작용에 대 한 정보에 대 한 { **NOINIT** | INIT}와 { **NOSKIP** | SKIP을 (를) 참조 [주의](#general-remarks) 이 항목의 뒷부분에 나오는 합니다.  
...
-   백업 세트가 아직 만료되지 않은 경우. 자세한 내용은 참조는 `EXPIREDATE` 및 `RETAINDAYS` 옵션입니다.  
-   BACKUP 문에 지정된 백업 세트 이름이 백업 미디어에 있는 이름과 일치하지 않을 경우. 자세한 내용은 이 섹션의 앞부분에 나오는 NAME 옵션을 참조하십시오.  
  
이러한 검사를 무시 하려면는 `SKIP` 옵션입니다.  
...
> [!IMPORTANT]  
> 사용 하 여 `FORMAT` 신중 하 게 합니다. 미디어 세트의 볼륨을 포맷하면 전체 미디어 세트를 사용할 수 없게 됩니다. 예를 들어 기존 스트라이프 미디어 세트에 속하는 테이프 하나를 초기화하면 전체 미디어 세트를 사용할 수 없게 됩니다.  
  
형식 지정 `SKIP`; `SKIP` 명시적으로 지정할 필요가 없습니다.  
  
MEDIADESCRIPTION  **=**  { *텍스트* | **@ * * * text_variable* }  
미디어 세트에 대한 설명 텍스트를 최대 255자까지 지정합니다.  
  
MEDIANAME  **=**  { *media_name* | **@ * * * media_name_variable* }  
전체 백업 미디어 세트에 대한 미디어 이름을 지정합니다. 미디어 이름은 128 자 미만 이어야 합니다 경우 `MEDIANAME` 지정을 백업 볼륨에 이미 이전에 지정한 미디어 이름과 일치 해야 합니다. 지정하지 않거나 SKIP 옵션을 지정하면 미디어 이름에 대한 확인을 수행하지 않습니다.  
...
> [!NOTE]  
> 사용 하는 방법에 대 한 중요 한 정보는 `BUFFERCOUNT` 옵션는 [잘못 된 BufferCount 데이터 전송 옵션 OOM 상태가 발생할 수 있습니다](http://blogs.msdn.com/b/sqlserverfaq/archive/2010/05/06/incorrect-buffercount-data-transfer-option-can-lead-to-oom-condition.aspx) 블로그.  
  
MAXTRANSFERSIZE  **=**  { *maxtransfersize*  |   ***@**  maxtransfersize_variable*} 가장 큰 전송 단위 간에 사용 되는 바이트 단위로 지정 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 와 백업 미디어입니다. 가능한 값은 최대 4194304바이트(4MB)까지 65536바이트(64KB)의 배수입니다.  

> [!NOTE]  
> 데이터베이스가 구성 하는 경우 SQL 기록기 서비스를 사용 하 여 백업을 만들 때 [FILESTREAM](../../relational-databases/blob/filestream-sql-server.md), 포함 또는 [메모리 액세스에 최적화 된 파일 그룹](../../relational-databases/in-memory-oltp/the-memory-optimized-filegroup.md), 하면 `MAXTRANSFERSIZE` 복원 시 여야 합니다 보다 크거나 같은 경우는 `MAXTRANSFERSIZE` 백업을 만들 때 사용한입니다. 

> [!NOTE]  
> 에 대 한 [투명 한 데이터 암호화 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption.md) 단일 데이터 파일을 기본으로 사용 되는 데이터베이스 `MAXTRANSFERSIZE` 은 65536 (64KB)입니다. TDE 암호화 아닌 기본 데이터베이스에 대 한 `MAXTRANSFERSIZE` 은 1048576 (1MB) 사용 하는 경우, 디스크와 65536 (64KB) VDI 또는 테이프를 사용 하는 경우.
> TDE와 백업 압축을 사용 하는 방법에 대 한 자세한 내용은 데이터베이스 암호화에 대 한 참조는 [주의](#general-remarks) 섹션.
...
> [!NOTE]  
> 사용 하는 경우 `NOREWIND`, 인스턴스의 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 동일한 프로세스에서 실행 중인 BACKUP 또는 RESTORE 문에서 사용 될 때까지 테이프 드라이브의 소유권을 갖고는 `REWIND` 또는 `UNLOAD` 옵션 또는 서버 인스턴스가 종료 됩니다. 테이프를 열어 두면 다른 프로세스에서 테이프를 액세스하는 것을 방지합니다. 열려 있는 테이프 목록을 표시 하 고 열려 있는 테이프를 닫고 하는 방법에 대 한 정보를 참조 하십시오. [백업 장치 &#40; SQL Server &#41; ](../../relational-databases/backup-restore/backup-devices-sql-server.md).  
  
{ **언로드** | NOUNLOAD}    

> [!NOTE]  
> `UNLOAD`및 `NOUNLOAD` 지정 하 여 재설정 될 때까지 또는 세션의 수명 기간 동안 유지 되는 세션 설정 됩니다.  
...
> [!NOTE]  
> 테이프 백업 장치에 백업에는 `BLOCKSIZE` 백업 작업의 성능에 영향을 하는 옵션입니다. 이 옵션은 일반적으로 테이프 장치에 쓰는 경우에만 성능에 영향을 줍니다.  
  
**로그 관련 옵션**  
  
이러한 옵션에만 사용 `BACKUP LOG`합니다.  
...
{NORECOVERY | 대기  **=**  *undo_file_name* }  
  ...
  건너뜁니다 로그 잘림에 대 알아본 후 데이터베이스를 RESTORING 상태로 원자적으로 최상의 로그 백업을 수행 하려면 사용 된 `NO_TRUNCATE` 및 `NORECOVERY` 옵션을 함께 합니다.  
...
NO_TRUNCATE  
지정 하는 로그 잘리지 있고는 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 데이터베이스의 상태에 관계 없이 백업을 시도 하도록 합니다. 따라서 수행한 백업 `NO_TRUNCATE` 불완전 한 메타 데이터가 포함 될 수 있습니다. 이 옵션을 사용하면 데이터베이스가 손상된 경우에도 로그를 백업할 수 있습니다.  
  
BACKUP LOG의 NO_TRUNCATE 옵션은 COPY_ONLY와 CONTINUE_AFTER_ERROR를 모두 지정하는 것과 같습니다.  
  
없이 `NO_TRUNCATE` 옵션을 데이터베이스가 온라인 상태 여야 합니다. 지정 하 여 백업을 만들 수 일시 중단 상태의 데이터베이스가 있으면 `NO_TRUNCATE`합니다. 백업 된 경우에 사용할 수 없습니다 데이터베이스가 OFFLINE 또는 EMERGENCY 상태에 있으면 하지만 `NO_TRUNCATE`합니다. 데이터베이스 상태에 대 한 정보를 참조 하십시오. [데이터베이스 상태](../../relational-databases/databases/database-states.md)합니다.  
  
## <a name="about-working-with-sql-server-backups"></a>SQL Server 백업 작업에 대 한  
...
> [!NOTE]  
> `BACKUP LOG WITH NO_LOG` 및 `WITH TRUNCATE_ONLY` 옵션 지원 되지 않습니다. 전체 또는 대량 로그 복구 모델 복구 사용 시 데이터베이스에서 로그 백업 체인을 제거해야 할 경우 단순 복구 모델로 전환하십시오. 자세한 내용은 [데이터베이스 복구 모델 보기 또는 변경&#40;SQL Server&#41;](../../relational-databases/backup-restore/view-or-change-the-recovery-model-of-a-database-sql-server.md)을 참조하세요.  
...
###  <a name="Backup_Devices_and_Media_Sets"></a>백업 장치와 미디어 세트 사용  
  
#### <a name="backup-devices-in-a-striped-media-set-a-stripe-set"></a>스트라이프 미디어 세트 (스트라이프 세트)의 백업 장치  
 A *세트 스트라이프* 집합 데이터 블록으로 분할 되 고 고정 순서로 분산 있는 디스크 파일입니다. 스트라이프 세트에 사용 되는 백업 장치의 수가 동일 하 게 유지 해야 합니다 (미디어도 다시 초기화 하지 않는 한 `FORMAT`).  
...
#### <a name="working-with-a-mirrored-media-set"></a>미러된 미디어 세트 사용  
 ...
 미러된 미디어 세트로 백업하려면 모든 미러가 있어야 합니다. 지정 된 미러된 미디어 세트에 백업 하는 `TO` 첫 번째 미러를 지정 하 고 지정 절은 `MIRROR TO` 각 추가 미러 절.  
  
 미러된 미디어 세트에 대 한 각 `MIRROR TO` 수와 유형의 장치 TO 절과 동일한 절 나열 해야 합니다. 다음 예에서는 2개의 미러를 포함하고 미러당 3개의 장치를 사용하는 미러된 미디어 세트에 기록합니다.  
...
##### <a name="media-families-in-mirrored-media-sets"></a>미러된 미디어 세트의 미디어 패밀리  
 에 지정 된 각 백업 장치는 `TO` BACKUP 문의 절 미디어 패밀리에 해당 합니다. 예를 들어 경우는 `TO` 절에 장치가 3 개 나열, 백업 데이터를 기록 3 개의 미디어 패밀리. 미러된 미디어 세트에서 모든 미러에는 각 미디어 패밀리의 복사본이 있어야 합니다. 이로 인해 장치 수가 모든 미러에서 동일해야 합니다.  
...
||NOINIT|INIT|  
|------|------------|----------|  
|NOSKIP|볼륨에 유효한 미디어 헤더가 있으면 확인 미디어 이름을 일치 하는 주어진 `MEDIANAME`있는 경우. 이름이 일치하면 기존 백업 세트를 모두 유지하면서 백업 세트를 추가합니다.<br /> 볼륨에 유효한 미디어 헤더가 포함되어 있지 않으면 오류가 발생합니다.|볼륨에 유효한 미디어 헤더가 포함되어 있으면 다음 검사를 수행합니다.<br /><ul><li>경우 `MEDIANAME` 지정 된, 지정 된 미디어 이름이 미디어 헤더의 미디어 이름과 일치 하는지 확인 합니다.<sup> 1</sup></li><li>미디어에 만료되지 않은 백업 세트가 없는지 확인합니다. 만료되지 않은 백업 세트가 있으면 백업을 종료합니다.</li></ul><br />이러한 검사를 통과하면 미디어 헤더만 유지하며 미디어의 모든 백업 세트를 덮어씁니다.<br /> 볼륨에 유효한 미디어 헤더가 포함 되어 있지 않으면, 지정 된 사용 하 여 생성 `MEDIANAME` 및 `MEDIADESCRIPTION`있는 경우.|  
|SKIP|볼륨에 유효한 미디어 헤더가 포함되어 있으면 기존 백업 세트를 모두 유지하면서 백업 세트를 추가합니다.|볼륨에 유효한 포함 되어 있으면<sup>2</sup> 미디어 헤더를 미디어 헤더만 유지 하며 미디어의 모든 백업 세트를 덮어씁니다.<br /> 미디어가 비어 있으면 지정 된를 사용 하 여 미디어 헤더를 생성 `MEDIANAME` 및 `MEDIADESCRIPTION`있는 경우.|  

<sup>1</sup> 적절 한 고정된 데이터베이스 또는 서버 역할에 백업 작업을 수행 하려면 사용자가 속해야 합니다.    

<sup>2</sup> 유효성 MTF 버전 번호와 기타 헤더 정보가 포함 됩니다. 지정한 버전이 지원되지 않거나 예기치 못한 값일 경우 오류가 발생합니다.  
...
백업에서 지 원하는 `RESTART` 이전 버전의 이전 버전과 호환성을 제공 하는 옵션 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]합니다. 하지만 RESTART은 아무 효과가 없습니다.  
  
## <a name="general-remarks"></a>일반 설명  
...
와 백업 압축을 사용 하는 경우 [투명 한 데이터 암호화 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption.md) 사용 데이터베이스 단일 데이터 파일을 사용 하는 것이 좋습니다 사용 하는 `MAXTRANSFERSIZE` 설정 **65536 (64KB) 보다 큰**합니다.   
부터는 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)],이 사용 TDE에 대 한 최적화 된 압축 알고리즘 먼저 페이지를 해독 하 고, 압축, 다시 암호화 하는 데이터베이스를 암호화 합니다. 사용 하는 경우 `MAXTRANSFERSIZE = 65536` (64KB) tde로 암호화 된 데이터베이스와 함께 백업 압축 직접 암호화 된 페이지를 압축 하 고 압축 비율을 생성 하지 않을 수 있습니다. 자세한 내용은 참조 [TDE 설정 데이터베이스에 대 한 백업 압축](http://blogs.msdn.microsoft.com/sqlcat/2016/06/20/sqlsweet16-episode-1-backup-compression-for-tde-enabled-databases/)합니다.

> [!NOTE]  
> Tde로 암호화 된 데이터베이스에 대 한 최적화 된 압축 알고리즘을 될 때 자동으로 사용 됩니다.
> * URL 백업 수행을 사용 하는 경우이 경우 기본 `MAXTRANSFERSIZE` 1048576 (1MB)으로 변경 되 고 더 낮은 값을 강제로 적용 되지 않습니다.
> * 데이터베이스에 여러 개의 데이터 파일 있으며이 경우 기본 `MAXTRANSFERSIZE` 65536의 배수 값으로 변경 (64 KB)을 낮은 값으로 변경 되지 않습니다 (같은 `MAXTRANSFERSIZE = 65536`). 
...
-   파일 관리 작업 등의 `ALTER DATABASE` 문을 사용 하 여는 `ADD FILE` 또는 `REMOVE FILE` 옵션입니다.  
