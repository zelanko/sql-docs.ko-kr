 ...
 **\<process_affinity >:: =**  
 ...
 { CPU_ID | CPU_ID  TO  CPU_ID } [ ,...n ]  
 하나 이상의 CPU를 포함하는 목록입니다. CPU Id는 0부터 시작 **정수** 값입니다.  
 ...
 { NUMA_node_ID | NUMA_node_ID  TO NUMA_node_ID } [ ,...n ]  
 하나 이상의 NUMA  노드를 포함하는 목록입니다. NUMA 노드 Id는 0부터 시작 **정수** 값입니다.  
...
**적용 대상**: [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 부터 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]까지  
 ...
 PATH  =  {  'os_file_path'  |  DEFAULT  }  
 진단 로그의 위치를 나타내는 경로입니다. 기본 위치는 \<\MSSQL\Log >의 설치 폴더 내에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 장애 조치 클러스터 인스턴스의 합니다.  
...
SQLDUMPEREDUMPFLAGS  
 SQL  Server  SQLDumper  유틸리티에서 생성되는 덤프 파일의 형식을 결정합니다. 기본 설정은 0입니다. 자세한 내용은 참조 [SQL Server Dumper 유틸리티 기술 자료 문서](http://go.microsoft.com/fwlink/?LinkId=206173)합니다.  
  
 SQLDUMPERDUMPPATH  =  {  'os_file_path'  |  DEFAULT  }  
 SQLDumper  유틸리티에서 덤프 파일을 저장하는 위치입니다. 자세한 내용은 참조 [SQL Server Dumper 유틸리티 기술 자료 문서](http://go.microsoft.com/fwlink/?LinkId=206173)합니다.  
  
 SQLDUMPERDUMPTIMEOUT  =  {  'dump_time-out'  |  DEFAULT  }  
 SQL  Server  오류 시 SQLDumper  유틸리티에서 덤프를 생성하기 위한 제한 시간 값(밀리초)입니다. 기본값은 0이며 이는 덤프를 완료하는 데 시간 제한이 없음을 의미합니다. 자세한 내용은 참조 [SQL Server Dumper 유틸리티 기술 자료 문서](http://go.microsoft.com/fwlink/?LinkId=206173)합니다.  
  
 FAILURECONDITIONLEVEL  =  {  'failure_condition_level'  |  DEFAULT  }  
 SQL  Server  장애 조치(failover)  클러스터 인스턴스가 장애 조치(failover)되거나 다시 시작되는 조건입니다. 기본값은 3이며,  이는 치명적인 서버 오류 시 SQL  Server  리소스가 장애 조치(failover)되거나 다시 시작됨을 의미합니다. 이 및 다른 오류 상태 수준에 대 한 자세한 내용은 참조 [Configure FailureConditionLevel Property Settings](../../sql-server/failover-clusters/windows/configure-failureconditionlevel-property-settings.md)합니다.  
...
**적용 대상**: [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 부터 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]까지  
 ...
 HADR 클러스터 컨텍스트  **=**  { **'***remote_windows_cluster***'** | 로컬}  
 서버 인스턴스의 HADR  클러스터 컨텍스트를 지정된 WSFC(Windows  Server  장애 조치(Failover)  클러스터링)  클러스터로 전환합니다. *HADR 클러스터 컨텍스트* 서버 인스턴스가 호스팅하는 가용성 복제본에 대 한 메타 데이터를 관리 하는 어떤 Windows Server 장애 조치 클러스터링 (WSFC) 클러스터를 결정 합니다. 새 WSFC 클러스터에서 [!INCLUDE[ssHADR](../../includes/sshadr-md.md)] 또는 상위 버전 인스턴스로의 클러스터 간 [!INCLUDE[ssSQL11SP1](../../includes/sssql11sp1-md.md)] 마이그레이션을 수행하는 동안에만 SET HADR CLUSTER CONTEXT 옵션을 사용합니다.  
 ...
 *windows_cluster*  
 ...
 자세한 내용은 참조 [서버 인스턴스의 HADR 클러스터 컨텍스트 &#40; 변경 SQL Server &#41; ](../../database-engine/availability-groups/windows/change-the-hadr-cluster-context-of-server-instance-sql-server.md).  
...
**적용 대상**: [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 부터 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]까지  
 ...
 ON  
 버퍼 풀 확장 옵션을 설정합니다. 이 옵션은 SSD(반도체 드라이브)와 같은 비휘발성 저장소를 사용해서 버퍼 풀 크기를 확장하여 무결한 데이터 페이지를 풀에 저장합니다. 이 기능에 대 한 자세한 내용은 참조 [버퍼 풀 확장](../../database-engine/configure-windows/buffer-pool-extension.md)합니다. 버퍼 풀 확장을 모든 SQL Server 버전에서 사용할 수 없는 경우 자세한 내용은 참조 [버전 및 SQL Server 2016에 대 한 지원 되는 기능](../../sql-server/editions-and-supported-features-for-sql-server-2016.md)합니다.  
  
 FILENAME = 'os_file_path_and_name'  
 버퍼 풀 확장 캐시 파일의 디렉터리 경로 및 이름을 정의합니다. 파일 확장명은 .BPE로 지정해야 합니다. 파일 이름을 수정하기 전에 버퍼 풀 확장을 해제해야 합니다.  
  
 크기 = *크기* [ **KB** | MB | GB]  
 캐시 크기를 정의합니다. 기본 크기 사양은 KB입니다. 최소 크기는 최대 서버 메모리 크기입니다. 최대 한도는 최대 서버 메모리 크기의 32배입니다. 최대 서버 메모리에 대 한 자세한 내용은 참조 하십시오. [sp_configure &#40; Transact SQL &#41; ](../../relational-databases/system-stored-procedures/sp-configure-transact-sql.md).  
...
**적용 대상**: [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 부터 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]까지  
 ...
 ON  
 자동 분할 큰 NUMA 하드웨어 노드 더 작은 NUMA 노드로 분할할 수 있습니다. 실행 값을 변경 하려면 데이터베이스 엔진의 다시 시작 합니다.  
  
 OFF  
 자동 소프트웨어 큰 NUMA 하드웨어 노드를 더 작은 NUMA 노드로 분할을 사용 하지 않도록 설정 합니다. 실행 값을 변경 하려면 데이터베이스 엔진의 다시 시작 합니다.  

> [!WARNING]  
> 소프트 NUMA 옵션 및 SQL Server 에이전트와 ALTER SERVER CONFIGURATION 문의 동작 문제가 알려진 있습니다.  다음은 작업의 권장 되는 순서입니다.  
> 1) SQL Server 에이전트의 인스턴스를 중지 합니다.  
> 2) ALTER SERVER 못함 소프트 NUMA 옵션을 실행 합니다.  
> 3) SQL Server 인스턴스 다시 시작 합니다.  
> 4) SQL Server 에이전트의 인스턴스를 시작 합니다.  
  
**추가 정보:** 되돌아갑니다 T-SQL을 다시 구성 명령이 실행될지는 ALTER SERVER CONFIGURATION SET SOFTNUMA 명령 사용 하 여 SQL Server 서비스가 다시 시작 되기 전에, 다음 SQL Server 에이전트 서비스가 중지 될 때 실행 되는 경우는 ALTER SERVER CONFIGURATION 전에 있었던 돌아가기 SOFTNUMA 설정입니다. 
  
## <a name="general-remarks"></a>일반적인 주의 사항  
 이 문은 다시 시작 하지 않아도 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]별도로 명시 하지 않는 한, 합니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 장애 조치(failover)  클러스터 인스턴스의 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 클러스터 리소스를 다시 시작할 필요가 없습니다.  
...
## <a name="permissions"></a>Permissions  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] 리소스 DLL은 로컬 시스템 계정으로 실행 합니다. 따라서 로컬 시스템 계정에는 진단 로그 옵션의 지정된 경로에 대한 읽기 및 쓰기 권한이 있어야 합니다.  
...
|[장애 조치 클러스터의 속성 설정](#Failover)|HealthCheckTimeout|  
...
-   NUMA 노드 0-3 Cpu 0-63 그룹 0:  
-   그룹 1: NUMA 노드 4-7, Cpu 64에서 127  
-   그룹 2: NUMA 노드에 8-12, Cpu 128-191  
...
###  <a name="Failover"></a>장애 조치 클러스터의 속성 설정  
...
## <a name="see-also"></a>관련 항목:  
 [소프트 NUMA &#40; SQL Server &#41;](../../database-engine/configure-windows/soft-numa-sql-server.md)   
 [서버 인스턴스 &#40; HADR 클러스터 컨텍스트 변경 SQL Server &#41;](../../database-engine/availability-groups/windows/change-the-hadr-cluster-context-of-server-instance-sql-server.md)   
