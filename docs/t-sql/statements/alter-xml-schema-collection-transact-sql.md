...
## <a name="remarks"></a>주의  
 ...
 다음 예제에서는 새 \<요소 > 기존 네임 스페이스에 `http://MySchema/test_xml_schema` 컬렉션에 `MyColl`합니다.  
...
```  
 ...
 `ALTER XML SCHEMA`는 `<anotherElement>` 요소를 이전에 정의한 네임스페이스인 `http://MySchema/test_xml_schema`에 추가합니다.  
 ...
 스키마 컬렉션에 이미 lax 유효성 검사 와일드 카드 또는 형식의 요소가 포함 된 경우 **xs: anytype**, 새 전역 요소, 형식 또는 특성 선언을 스키마 컬렉션에 추가 하면 모든 유효성 재검사는 저장 된 스키마 컬렉션에 의해 제한 되는 데이터입니다.  
...
```  
 ...
 에 스키마 컬렉션을 저장할 수는 **xml** 유형 열입니다. 이 경우 XML 스키마 컬렉션을 만들려면 다음 단계를 수행합니다.  
...
### <a name="c-importing-a-schema-that-does-not-specify-a-target-namespace"></a>3. 대상 네임스페이스를 지정하지 않는 스키마 가져오기  
 포함 하지 않는 스키마는 **targetNamespace** 특성을 컬렉션에서 가져올, 다음 예제와 같이 해당 구성 요소는 빈 문자열 대상 네임 스페이스와 연결 합니다. 컬렉션으로 가져온 하나 이상의 스키마를 연결하지 않으면 잠재적으로 관련이 없는 여러 개의 스키마 구성 요소가 기본 빈 문자열 네임스페이스에 연결되게 됩니다.  
