...
## <a name="arguments"></a>인수  
 ...
 schema_name  
 보안 정책이 속한 스키마의 이름입니다. 스키마 바인딩 때문에 *schema_name*이 필요 합니다.  
 ...
 *\<block_dml_operation>*  
 차단 조건자가 적용될 특정 DML 작업입니다. AFTER는 DML 작업 수행(INSERT 또는 UPDATE) 후 조건자가 행의 값에 따라 평가되도록 지정합니다. BEFORE는 DML 작업 수행(UPDATE 또는 DELETE) 전에 조건자가 행의 값에 따라 평가되도록 지정합니다. 작업이 지정되지 않은 경우 조건자는 모든 작업에 적용됩니다.  
