...
> [!NOTE]  
>  관계형 인덱스를 만들려면 참조 [CREATE index&#40; Transact SQL &#41; ](../../t-sql/statements/create-index-transact-sql.md). 공간 인덱스를 만드는 방법에 대 한 정보를 참조 하세요. [CREATE SPATIAL index&#40; Transact SQL &#41; ](../../t-sql/statements/create-spatial-index-transact-sql.md).  
...
-   각 **xml** 테이블의 열에에서는 하나의 기본 XML 인덱스 및 보조 XML 인덱스 여러 개 있을 수 있습니다.  
  
-   에 기본 XML 인덱스는 **xml** 열에 보조 XML 인덱스를 만들기 전에 열이 있어야 합니다.  
  
-   단일만 XML 인덱스를 만들 수 **xml** 열입니다. 비 XML 인덱스를 만들 수 없습니다**xml** 열 라고도에 관계형 인덱스를 만들는 **xml** 열입니다.  
  
-   에 주 / 보조 XML 인덱스를 만들 수 없습니다는 **xml** 뷰를 사용 하 여 테이블 반환 변수에서의 열 **xml** 열 또는 **xml** 변수를 입력 합니다.  
  
-   계산에 기본 XML 인덱스를 만들 수 없습니다 **xml** 열입니다.  
  
-   SET 옵션 설정은 인덱싱된 뷰 및 계산 열 인덱스에 필요한 설정과 같아야 합니다. 특히, ARITHABORT 옵션으로 설정 되어 있어야 ON에 값이 삽입, 삭제 또는 업데이트 될 때 및 XML 인덱스가 생성 될 때는 **xml** 열입니다.  
 ...
 *index_name*  
 ...
 기본 XML 인덱스 이름은 문자로 시작할 수 없습니다:  **#** ,  **##** ,  **@** , 또는  **@@**  .  
...
## <a name="remarks"></a>주의  
 파생 된 **xml** 데이터 형식이 될 수 있습니다 인덱싱될 키 또는 키가 아닌 열은 계산된 열 데이터 형식이 인덱스 키 열 또는 키가 아닌 열으로 허용 되는 하기만 합니다. 계산에 기본 XML 인덱스를 만들 수 없습니다 **xml** 열입니다.  
  
 XML 인덱스에 대 한 정보를 보려면 사용 하 여는 [sys.xml_indexes](../../relational-databases/system-catalog-views/sys-xml-indexes-transact-sql.md) 카탈로그 뷰에 있습니다.  
  
 XML 인덱스에 대 한 자세한 내용은 참조 [XML 인덱스 &#40; SQL Server &#41; ](../../relational-databases/xml/xml-indexes-sql-server.md).  
  
## <a name="additional-remarks-on-index-creation"></a>인덱스 작성에 대한 추가 주의 사항  
 인덱스 생성에 대 한 자세한 내용은의 "주의" 섹션을 참조 하십시오. [CREATE index&#40; Transact SQL &#41; ](../../t-sql/statements/create-index-transact-sql.md).  
