...
라이브러리는 사용자로 범위 지정된 데이터베이스에 추가됩니다. 즉, 라이브러리 이름은 특정 사용자 또는 소유자의 컨텍스트 내에서 고유한 것으로 간주되며 라이브러리 이름은 사용자별로 고유해야 합니다. 예를 들어 두 사용자 **RUser1** 및 **RUser2**는 모두 R 라이브러리 `ggplot2`를 개별적으로 업로드할 수도 있고 따로 업로드할 수도 있습니다.
...
SQL Server 2017 릴리스에서는 blob을 데이터 원본으로 사용할 수 없습니다.

## <a name="permissions"></a>사용 권한

`CREATE ANY EXTERNAL LIBRARY` 권한이 필요합니다.
...
실제로 인기 있는 패키지에 대한 패키지 종속성은 대개 이처럼 간단한 예제보다 훨씬 더 복잡합니다. 예를 들어 ggplot2에는 30개가 넘는 패키지가 필요할 수 있으며 그러한 패키지에는 서버에서 사용할 수 없는 추가 패키지가 필요할 수 있습니다. 누락된 패키지 또는 잘못된 패키지 버전을 사용하면 설치가 실패할 수 있습니다.

단지 패키지 매니페스트만 조사하여 모든 종속성을 결정하기는 어려울 수 있으므로 [miniCRAN](https://cran.r-project.org/web/packages/miniCRAN/index.html) 또는 [iGraph](http://igraph.org/redirect.html) 같은 패키지를 사용하여 설치를 성공적으로 완료하는 데 필요할 수 있는 모든 패키지를 식별하는 것이 좋습니다.
...
패키지 파일을 서버의 위치에 저장하는 기능이 없는 경우 패키지 콘텐츠를 변수에 전달할 수도 있습니다. 다음 예제에서는 비트를 16진수 리터럴로 전달하여 라이브러리를 만듭니다.

```SQL
CREATE EXTERNAL LIBRARY customLibrary FROM (CONTENT = 0xabc123) WITH (LANGUAGE = 'R');
```

여기서 16진수 값은 가독성을 위해 잘렸습니다.
...
데이터베이스에서 패키지 라이브러리를 삭제하려면 다음 명령문을 사용합니다.
...
> [!NOTE]
> [!INCLUDE[ssnoversion](../../includes/ssnoversion.md)]의 `DROP` 문과 달리, 이 명령문은 사용자 권한을 지정하는 선택적 매개 변수를 지원합니다. 이 옵션을 사용하여 소유권 역할을 가진 사용자는 일반 사용자가 업로드한 라이브러리를 삭제할 수 있습니다.
