...
[!INCLUDE[tsql-appliesto-ss2008-all_md](../../includes/tsql-appliesto-ss2008-all-md.md)]
  ...
  테이블이 나 뷰에 관계형 인덱스를 만듭니다. 클러스터형 또는 비클러스터형 btree 인덱스 중 하나 이기 때문에 rowstore 인덱스를 라고도 합니다. 테이블의 데이터를 넣기 전에 rowstore 인덱스를 만들 수 있습니다. Rowstore 인덱스를 사용 하 여 쿼리 성능 향상을 위해 특히 쿼리에 특정 열에서 선택 하거나 필요한 값을 특정 순서로 정렬 하는 경우.  
  
  
> [!NOTE]  
>  Azure SQL 데이터 웨어하우스 및 병렬 데이터 웨어하우스를 현재 Unique 제약 조건을 지원 하지 않습니다. Unique 제약 조건을 참조 하는 모든 예제만 SQL Server 및 Azure SQL 데이터베이스에 적용 됩니다.    
...
-   SQL Server 2016 및 Azure SQL 데이터베이스에서 데이터 웨어하우징 쿼리 성능을 개선 하기 위해 columnstore 인덱스에서 비클러스터형 인덱스를 사용 합니다. 참조 [Columnstore 인덱스-데이터 웨어하우스](../../relational-databases/indexes/columnstore-indexes-data-warehouse.md)  
...
>  병렬 인덱스 작업의 일부 버전에서 사용할 수 없는 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]합니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 버전에서 지원되는 기능 목록은 [SQL Server 2016 버전에 대한 버전 및 지원하는 기능](../../sql-server/editions-and-supported-features-for-sql-server-2016.md)을 참조하세요.  
 ...
 ON PARTITIONS **(** { \<partition_number_expression > | \<범위 >} [ **,**...  *n*  ] **)** **적용할**: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 통해 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 및 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]합니다.  
...
## <a name="specifying-index-options"></a>인덱스 옵션 지정  
 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]에는 새 인덱스 옵션이 추가되었으며 옵션 지정 방법도 수정되었습니다. 이전 버전과 호환 되는 구문에서 WITH *option_name* WITH 같습니다 **(** \<option_name >  **=**  ON **)**. 인덱스 옵션을 설정하면 다음 규칙이 적용됩니다. 
  
-   새 인덱스 옵션 WITH를 사용 하 여 지정할 수 있습니다 **(***option_name*  **=**  ON | OFF**)**합니다.  
  
-   옵션은 동일한 문에 이전 버전과 호환되는 구문 및 새 구문 모두를 사용하여 지정할 수 없습니다. 예를 들어 WITH 지정 **(**DROP_EXISTING**,** 온라인  **=**  ON**)** 때문에 문이 실패 합니다.  
  
-   WITH를 사용 하 여 옵션을 지정 해야 합니다는 XML 인덱스를 만들 때 **(***option_name*  **=**  ON | OFF**)**합니다.  
...
-   Columnstore 인덱스가 이미 있는 경우에 데이터 웨어하우스 테이블에 클러스터형 또는 비클러스터형 rowstore 인덱스입니다. 이 동작은 rowstore와 columnstore 인덱스를 같은 테이블에 공존할 수 있는 SMP SQL Server와에서 다릅니다.  
...
## <a name="version-notes"></a>버전 정보  
 SQL 데이터베이스 파일 그룹 및 filestream 옵션을 지원 하지 않습니다.  
...
### <a name="p-add-a-column-to-an-index"></a>16. 인덱스에 열 추가  
 다음 예에서는 dbo에서 두 개의 열이 있는 IX_FF 인덱스를 만듭니다. FactFinance 테이블입니다.  다음 문은 동일한 이름 및 열이 하나 더 해당 인덱스를 다시 작성 하는 방법을 보여 줍니다.  
