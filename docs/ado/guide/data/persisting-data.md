...
-   **이동:** 이동 중에 응용 프로그램을 수행 하는 경우 것이 중요 변경 하 고 다음 나중에 데이터베이스에 다시 연결 하 고 수 있는 커밋된 새 레코드를 추가 하는 기능을 제공 합니다.  
  
-   **자주 업데이트 되는 조회:** 주로 응용 프로그램에서 테이블 조회로 사용 됩니다-세금 테이블 예를 들어 상태입니다. 자주 업데이트 되 고 읽기 전용. 응용 프로그램이 시작 될 때마다 서버에서이 데이터를 읽고, 대신 응용 프로그램 단순히 데이터를 로드할 수에서 로컬로 지속형 **레코드 집합**합니다.  
