---
title: 커서 및 잠금 이해 | Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.suite: sql
ms.tgt_pltfrm: ''
ms.topic: conceptual
helpviewer_keywords:
- locks [ADO]
- cursors [ADO]
ms.assetid: c1b7d7e6-1707-4ce2-863f-0c6dea967df6
caps.latest.revision: 10
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: bdbbaa5d57bdaa1bf3d9d784f3bb378a833db9cc
ms.sourcegitcommit: 62826c291db93c9017ae219f75c3cfeb8140bf06
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 06/11/2018
ms.locfileid: "35273042"
---
# <a name="understanding-cursors-and-locks"></a>커서 및 잠금 이해
응용 프로그램의 데이터 액세스 요구 사항에 대 한 가장 효율적인 커서 유형을 선택할 수 있는 커서의 작동 방법을 이해 하는 것이 유용 합니다. 덜 최적화 커서 구성 매우 느려질 데이터 액세스 작업을 만들 수 있습니다.  
  
 ADO의 다양 한 기능 **레코드 집합** 개체 유형과 위치도 커서 잠금 유형에 따라 결정 됩니다.  
  
 이 섹션에서는 다음 항목을 다룹니다.  
  
-   [커서란?](../../../ado/guide/data/what-is-a-cursor.md)  
  
-   [커서 유형](../../../ado/guide/data/types-of-cursors-ado.md)  
  
-   [커서 위치의 중요성](../../../ado/guide/data/the-significance-of-cursor-location.md)  
  
-   [OLE DB용 Microsoft 커서 서비스](../../../ado/guide/data/the-microsoft-cursor-service-for-ole-db.md)  
  
-   [잠금이란?](../../../ado/guide/data/what-is-a-lock.md)  
  
-   [CacheSize 사용](../../../ado/guide/data/using-cachesize.md)  
  
-   [커서 및 잠금 특성](../../../ado/guide/data/cursor-and-lock-characteristics.md)
