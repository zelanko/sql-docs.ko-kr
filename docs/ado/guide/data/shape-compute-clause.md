...
```  
 ...
 이 명령은 두 수준이 있는 모양 있는 **레코드 집합** 을 엽니다. 부모 수준은 집계 열 ( **Recordset** `SUM(rs.population)` ), 자식 **레코드 집합** 을 참조 하는 열 () `rs` 및 자식 **레코드 집합** ()을 그룹화 하는 열을 포함 하는 생성 된 레코드 집합입니다 `state` . 자식 수준은 쿼리 명령 ()에서 반환 하는 **레코드 집합** 입니다 `select * from demographics` .  
