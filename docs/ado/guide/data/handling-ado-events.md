...
ADO 이벤트 모델은 작업이 시작 되기 전이나 완료 된 후에 *이벤트*또는 알림을 발행 하는 특정 동기 및 비동기 ADO 작업을 지원 합니다. 이벤트는 실제로 응용 프로그램에서 정의 하는 이벤트 처리기 루틴에 대 한 호출입니다.  
 ...
 작업을 완료 한 후에 발생 하는 이벤트는 ADO를 비동기식으로 사용 하는 경우 특히 중요 합니다. 예를 들어 비동기 [레코드 집합](../../../ado/reference/ado-api/open-method-ado-recordset.md) 을 시작 하는 응용 프로그램은 작업이 완료 될 때 실행 완료 이벤트에 의해 알려집니다.  
  
 ADO 이벤트 모델을 사용 하면 응용 프로그램에 약간의 오버 헤드가 추가 되지만 루프를 사용 하 여 개체의 [상태](../../../ado/reference/ado-api/state-property-ado.md) 속성 모니터링과 같은 비동기 작업을 처리 하는 다른 방법 보다 훨씬 더 많은 유연성이 제공 됩니다.  
