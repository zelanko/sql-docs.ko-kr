...
## <a name="server-side-cursors"></a>서버 쪽 커서  
 ...
 그러나 것 서버 쪽 커서를 모든 활성 클라이언트에 대 한 귀중 한 서버 리소스를 소비-적어도 일시적으로-는 점도 중요 합니다. 서버 하드웨어에 모든 활성 클라이언트에서 요청한 서버 쪽 커서를 관리할 수 있도록 적절 하 게 계획 해야 합니다. 또한 서버 쪽 커서를 느려질 수 있습니다만 단일 행 액세스할-사용 가능한 batch 커서가 없습니다.  
