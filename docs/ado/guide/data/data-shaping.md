---
description: 데이터 셰이핑
title: 데이터 셰이핑 | Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- data shaping [ADO]
ms.assetid: 62bd7dc9-45b5-4ca9-8b52-457325e0ce9e
author: rothja
ms.author: jroth
ms.openlocfilehash: 2a15e137fd3dfcd8c863bbe2fb1d5bc5fe756851
ms.sourcegitcommit: e700497f962e4c2274df16d9e651059b42ff1a10
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/17/2020
ms.locfileid: "88453545"
---
# <a name="data-shaping"></a>데이터 셰이핑
이 섹션에서는 다음 항목을 다룹니다.  
  
-   [데이터 셰이핑 개요](../../../ado/guide/data/data-shaping-overview.md)  
  
-   [데이터 ShapingExample](../../../ado/guide/data/data-shaping-example.md)  
  
-   [다시 셰이핑](../../../ado/guide/data/reshaping.md)  
  
-   [최하위 집계](../../../ado/guide/data/grandchild-aggregates.md)  
  
-   [중간에 COMPUTE 명령을 사용한 매개 변수화된 명령](../../../ado/guide/data/parameterized-commands-with-intervening-compute-commands.md)  
  
-   [계층적 레코드 집합 유지](../../../ado/guide/data/persisting-hierarchical-recordsets.md)  
  
-   [데이터 셰이프에 필요한 공급자](../../../ado/guide/data/required-providers-for-data-shaping.md)  
  
-   [일반적인 셰이핑 명령](../../../ado/guide/data/shape-commands-in-general.md)  
  
-   [셰이프 APPEND 절](../../../ado/guide/data/shape-append-clause.md)  
  
-   [셰이프 COMPUTE 절](../../../ado/guide/data/shape-compute-clause.md)  
  
-   [계층적 레코드 집합 구성](../../../ado/guide/data/fabricating-hierarchical-recordsets.md)  
  
-   [계층적 레코드 집합에서 행에 액세스](../../../ado/guide/data/accessing-rows-in-a-hierarchical-recordset.md)  
  
-   [공식적인 셰이프 문법](../../../ado/guide/data/formal-shape-grammar.md)  
  
-   [Visual Basic for Applications 함수](../../../ado/guide/data/visual-basic-for-applications-functions.md)
