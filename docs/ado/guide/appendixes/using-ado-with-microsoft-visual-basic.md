---
title: Microsoft Visual Basic 혹은 Visual Basic for Applications로 ADO 사용하기 | Microsoft Docs
ms.prod: sql
...
---
# <a name="using-ado-with-microsoft-visual-basic-and-visual-basic-for-applications"></a>Microsoft Visual Basic 혹은 Visual Basic for Applications로 ADO 사용하기
Visual Basic을 사용하든 Visual Basic for Application을 사용하든 ADO 프로젝트를 설정하고 ADO 코드를 작성하는 작업은 거의 비슷합니다. 이 항목에서는 Visual Basic 혹은 Visual Basic for Applications에서 ADO를 사용하는 방법에 대한 유의사항과 차이점에 대해 다룹니다.
...
1.  Visual Basic 메뉴에서 **프로젝트**를 클릭하고 **참조**를 클릭합니다..
...
3.  **확인**을 클릭합니다.
 ...
 Visual Basic for Applications에서도 쉽게 ADO를 사용할 수 있습니다. 예: Microsoft Access
...
다른 방법으로, **Dim** 문으로 선언한 다음 초기화 하는 두 단계로 처리할 수도 있습니다.
...
> [!NOTE]
> 자신의 프로젝트에서 ADO 라이브러리를 올바르게 참조했다면, **Dim** 문과 함께 `ADODB`라는 식별자를 반드시 사용할 필요는 없습니다. 그러나, 이 식별자를 사용하면 다른 라이브러리와의 이름 충돌을 확실히 방지할 수 있습니다.

> [!NOTE]
> 예를 들어, 동일한 프로젝트에서 ADO와 DAO를 모두 참조에 포함시켰다면, 다음 코드와 같이 **Recordset** 개체를 인스턴스화할 때 받드시 식별자를 명시해야 합니다.
...
**CreateObject** 메서드를 이용하여 인스턴스화 된 개체는 후기 바인딩 처리됩니다. 이것은 데이터 타입을 확정할 수 없으며, 그로 인해 명령줄 완성 기능이 비활성화 된다는 뜻 입니다. 그러나, 이렇게 처리하면 자신의 프로젝트에서 ADO 라이브러리를 참조하지 않아도 되고, (런타임 시에) 특정 버전의 개체로 설정할 수도 있습니다. 다음 예를 참고하십시오.

```
Set conn1 = CreateObject("ADODB.Connection.2.0") As Object
```

이렇게 함으로써, 명시적으로 ADO 버전 2.0 타입 라이브러리를 지정한 개체를 만들 수 있습니다.

**CreateObject** 메서드를 사용하여 개체를 인스턴스화 하는 작업은 **Dim** 문을 사용한 것보다 일반적으로 느립니다.

## <a name="handling-events"></a>이벤트 다루기
Microsoft Visual Basic에서 ADO 이벤트를 다루려면, **WithEvents** 키워드를 이용하여 모듈 수준의 변수를 선언해야 합니다. 이 변수는 클래스 모듈의 일부로 선언되어야 하고, 모듈 수준에 선언되어야 합니다. ADO 이벤트를 다루는 것에 대한 논의는 [ADO 이벤트 다루기](../../../ado/guide/data/handling-ado-events.md)를 참고하십시오.
