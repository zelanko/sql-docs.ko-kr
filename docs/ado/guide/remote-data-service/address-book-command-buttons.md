...
## <a name="building-the-sql-query"></a>SQL 쿼리 작성  
 Find_OnClick Sub 프로시저의 첫 번째 부분에서는 텍스트 문자열을 전역 SQL SELECT 문에 추가 하 여 SQL 쿼리를 한 번에 하나씩 작성 합니다. 먼저 `myQuery` 데이터 원본 테이블의 모든 데이터 행을 요청 하는 SQL SELECT 문으로 변수를 설정 합니다. 그런 다음 하위 프로시저는 페이지에서 4 개의 입력 상자를 각각 검색 합니다.  
  
 프로그램은 SQL 문을 작성 하는 데 사용 되는 단어를 사용 하기 때문에 `like` 쿼리는 정확히 일치 하는 항목이 아닌 부분 문자열 검색입니다.  
