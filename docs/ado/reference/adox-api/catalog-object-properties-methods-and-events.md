---
description: 카탈로그 개체 속성, 메서드 및 이벤트
title: Catalog 개체 속성, 메서드 및 이벤트 | Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- Catalog object [ADOX], members
ms.assetid: 90a05168-87d0-45e8-8b32-0b64fec74ad2
author: rothja
ms.author: jroth
ms.openlocfilehash: 3b1496230281d7199774475437c0d12be67e7cfe
ms.sourcegitcommit: e700497f962e4c2274df16d9e651059b42ff1a10
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/17/2020
ms.locfileid: "88440405"
---
# <a name="catalog-object-properties-methods-and-events"></a>카탈로그 개체 속성, 메서드 및 이벤트
## <a name="propertiescollections"></a>속성/컬렉션  
 [ActiveConnection 속성](../../../ado/reference/adox-api/activeconnection-property-adox.md)  
  
 [Groups 컬렉션](../../../ado/reference/adox-api/groups-collection-adox.md)  
  
 [프로시저 컬렉션](../../../ado/reference/adox-api/procedures-collection-adox.md)  
  
 [Tables 컬렉션](../../../ado/reference/adox-api/tables-collection-adox.md)  
  
 [사용자 컬렉션](../../../ado/reference/adox-api/users-collection-adox.md)  
  
 [Views 컬렉션](../../../ado/reference/adox-api/views-collection-adox.md)  
  
## <a name="methods"></a>메서드  
 [메서드 만들기](../../../ado/reference/adox-api/create-method-adox.md)  
  
 [GetObjectOwner 메서드](../../../ado/reference/adox-api/getobjectowner-method-adox.md)  
  
 [SetObjectOwner 메서드](../../../ado/reference/adox-api/setobjectowner-method.md)  
  
## <a name="events"></a>이벤트  
 없음  
  
## <a name="see-also"></a>참고 항목  
 [Catalog 개체(ADOX)](../../../ado/reference/adox-api/catalog-object-adox.md)
