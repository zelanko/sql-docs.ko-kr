 ...
 *SkipRows*  
 (선택 사항) **긴** 값을 해당 기본값은 현재 행의 행 오프셋을 지정 하는 0 또는 *시작* 검색을 시작할 책갈피입니다. 기본적으로 현재 행에서 검색이 시작 됩니다.  
  
 *SearchDirection*  
 (선택 사항) A [SearchDirectionEnum](../../../ado/reference/ado-api/searchdirectionenum.md) 검색 방향 검색으로 다음 사용 가능한 행 또는 현재 행에서 시작 되어야 하는지 여부를 지정 하는 값입니다. 끝에 실패 한 검색이 중지 되는 **Recordset** 값이 **adSearchForward**합니다. 시작 부분에 실패 한 검색이 중지 되는 **Recordset** 값이 **adSearchBackward**합니다.  
