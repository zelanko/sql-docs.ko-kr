 ...
 *adStatus*  
 항상 **Adstatusok**를 반환 하는 [eventstatusenum](./eventstatusenum.md) 값입니다.  
  
 **Connectcomplete** 가 호출 되 면이 매개 변수는 **WillConnect** 이벤트에서 보류 중인 연결 취소가 요청 된 경우 **adstatuscancel** 로 설정 됩니다.  
  
 두 이벤트를 반환 하기 전에이 매개 변수를 **adStatusUnwantedEvent** 로 설정 하 여 후속 알림이 발생 하지 않도록 합니다. 그러나 [연결](./connection-object-ado.md) 을 닫았다가 다시 열면 이러한 이벤트가 다시 발생 합니다.  
