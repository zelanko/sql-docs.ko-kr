---
description: Stream 개체 속성, 메서드 및 이벤트
title: Stream 개체 속성, 메서드 및 이벤트 | Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: ado
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- Stream object [ADO], members
ms.assetid: 9b0eed90-c38c-4aa6-9040-5827f8c46b94
author: rothja
ms.author: jroth
ms.openlocfilehash: 309ad17e4d65f33ff109c2cdda87a0fb471f081a
ms.sourcegitcommit: 18a98ea6a30d448aa6195e10ea2413be7e837e94
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/27/2020
ms.locfileid: "88988534"
---
# <a name="stream-object-properties-methods-and-events"></a>Stream 개체 속성, 메서드 및 이벤트
## <a name="properties"></a>속성  
 [Charset 속성](./charset-property-ado.md)  
  
 [EOS 속성](./eos-property.md)  
  
 [LineSeparator 속성](./lineseparator-property-ado.md)  
  
 [Mode 속성](./mode-property-ado.md)  
  
 [Position 속성](./position-property-ado.md)  
  
 [Size 속성(ADO 스트림)](./size-property-ado-stream.md)  
  
 [State 속성](./state-property-ado.md)  
  
 [Type 속성(ADO 스트림)](./type-property-ado-stream.md)  
  
## <a name="methods"></a>메서드  
 [Cancel 메서드](./cancel-method-ado.md)  
  
 [Close 메서드](./close-method-ado.md)  
  
 [CopyTo 메서드](./copyto-method-ado.md)  
  
 [Flush 메서드](./flush-method-ado.md)  
  
 [LoadFromFile 메서드](./loadfromfile-method-ado.md)  
  
 [Open 메서드(ADO 스트림)](./open-method-ado-stream.md)  
  
 [Read 메서드](./read-method.md)  
  
 [ReadText 메서드](./readtext-method.md)  
  
 [SaveToFile 메서드](./savetofile-method.md)  
  
 [SetEOS 메서드](./seteos-method.md)  
  
 [SkipLine 메서드](./skipline-method.md)  
  
 [Stat 메서드](./stat-method.md)  
  
 [Write 메서드](./write-method.md)  
  
 [WriteText 메서드](./writetext-method.md)  
  
## <a name="events"></a>이벤트  
 없음  
  
## <a name="see-also"></a>참고 항목  
 [스트림 개체(ADO)](./stream-object-ado.md)