---
title: CubeDef 개체 속성, 메서드 및 이벤트 | Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- CubeDef object [ADO MD], members
ms.assetid: 5dac737a-b77e-47d3-9228-cd52b7a97b0a
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: 2b0f5877343246a6328e9e709b7f205ef82f0922
ms.sourcegitcommit: 61381ef939415fe019285def9450d7583df1fed0
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 10/01/2018
ms.locfileid: "47789691"
---
# <a name="cubedef-object-properties-methods-and-events"></a>CubeDef 개체 속성, 메서드 및 이벤트
## <a name="propertiescollections"></a>속성/컬렉션  
 [Description 속성](../../../ado/reference/ado-md-api/description-property-ado-md.md)  
  
 [Dimensions 컬렉션](../../../ado/reference/ado-md-api/dimensions-collection-ado-md.md)  
  
 [Name 속성](../../../ado/reference/ado-md-api/name-property-ado-md.md)  
  
 [속성 컬렉션](../../../ado/reference/ado-api/properties-collection-ado.md)  
  
## <a name="methods"></a>메서드  
 [GetSchemaObject 메서드](../../../ado/reference/ado-md-api/getschemaobject-method-ado-md.md)  
  
## <a name="events"></a>이벤트  
 없음  
  
## <a name="see-also"></a>관련 항목  
 [CubeDef 개체(ADO MD)](../../../ado/reference/ado-md-api/cubedef-object-ado-md.md)
