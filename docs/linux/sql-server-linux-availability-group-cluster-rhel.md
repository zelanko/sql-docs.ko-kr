...
>[!WARNING]
>자동 장애 조치 후 때 `start-failure-is-fatal = true` 리소스 관리자 리소스를 시작 하려고 합니다. 첫 번째 시도에 실패 한 경우는 수동으로 실행 `pcs resource cleanup <resourceName>` 하는 리소스 실패 횟수를 정리 하 고 구성을 다시 설정 합니다.
