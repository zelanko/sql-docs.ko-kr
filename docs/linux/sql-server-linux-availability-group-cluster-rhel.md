---
title: SQL Server 가용성 그룹에 대해 RHEL 클러스터 구성
titleSuffix: SQL Server
description: RHEL(Red Hat Enterprise Linux) 실행 시 가용성 그룹 클러스터에 대해 알아봅니다.
author: MikeRayMSFT
ms.author: mikeray
ms.reviewer: vanto
ms.date: 03/12/2019
...
>[!IMPORTANT]
>STONITH를 사용하지 않도록 설정하는 기능은 테스트 목적으로만 제공됩니다. 프로덕션 환경에서 Pacemaker를 사용하려는 경우 해당 환경에 따라 STONITH 구현을 계획하고 사용 상태로 유지해야 합니다. RHEL은 클라우드 환경(Azure 포함) 또는 Hyper-V용 펜싱 에이전트를 제공하지 않습니다. 따라서 클러스터 공급업체도 이러한 환경에서 프로덕션 클러스터를 실행하기 위한 지원을 제공하지 않습니다. 이 문제의 해결 방법을 개발 중이며, 이후 릴리스에서 제공될 예정입니다.
