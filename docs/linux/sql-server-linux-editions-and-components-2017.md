...
ms.locfileid: "47628831"
---
# <a name="editions-and-supported-features-of-sql-server-2017-on-linux"></a>버전 및 Linux의 SQL Server 2017의 지원 되는 기능

[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-linuxonly](../includes/appliesto-ss-xxxx-xxxx-xxx-md-linuxonly.md)]

이 문서에서는 Linux의 SQL Server 2017의 다양 한 버전에서 지원 되는 기능의 세부 정보를 제공 합니다. 버전 및 지원 되는 Windows의 SQL Server 기능에 대 한 참조 [SQL Server 2017-Windows](../sql-server/editions-and-components-of-sql-server-2017.md)합니다.  
...
|---------------------------------------|----------------|  
|Enterprise|프리미엄 제품인 [!INCLUDE[ssNoVersion](../includes/ssNoVersion-md.md)] Enterprise edition은 중요 업무용 워크 로드에 대 한 높은 서비스 수준을 사용 하도록 설정 하는 매우 빠른 성능 포괄적인 고성능 데이터 센터 기능을 제공 합니다.|  
|표준|[!INCLUDE[ssNoVersion](../includes/ssNoVersion-md.md)] Standard edition 부서와 소규모 조직이 응용 프로그램을 실행 하기 위한 기본 데이터 관리를 제공 하며 온-프레미스 및 클라우드 용 공용 개발 도구 지원-최소한의 IT 리소스를 사용 하 여 효과적인 데이터베이스 관리를 사용 하도록 설정 합니다.|  
...
<sup>1</sup> Enterprise edition Server + 클라이언트 액세스 라이선스 (CAL) 기반 라이선스가 (새 계약에 사용할 수 없음)를 사용 하 여 SQL Server 인스턴스당 20 개의 코어로 제한 됩니다. 코어 기반 서버 라이선스 모델에서는 제한이 없습니다. 자세한 내용은 [SQL Server의 버전별 계산 용량 제한](../sql-server/compute-capacity-limits-by-edition-of-sql-server.md)합니다.  
 
##  <a name="RDBMSHA"></a> RDBMS 고가용성  
  
|기능|Enterprise|표준|Web|Express|  
|-------------|----------------|--------------|---------|------------------------|  
|로그 전달|사용자 계정 컨트롤|예|예|아니요|  
|백업 압축|사용자 계정 컨트롤|예|아니오|아니요| 
|데이터베이스 스냅숏|사용자 계정 컨트롤|아니오|아니요|아니요|
|Always On 장애 조치 클러스터 인스턴스의<sup>1</sup>|사용자 계정 컨트롤|예|아니오|아니요| 
|Always On 가용성 그룹<sup>2</sup>|사용자 계정 컨트롤|아니오|아니요|아니요|
|기본 가용성 그룹 <sup>3</sup>|아니요|사용자 계정 컨트롤|아니오|아니요|
|최소 복제본 커밋 가용성 그룹|사용자 계정 컨트롤|예|아니오|아니요|
|클러스터가 없는 가용성 그룹|사용자 계정 컨트롤|예|아니오|아니요|
|온라인 페이지 및 파일 복원|사용자 계정 컨트롤|아니오|아니요|아니요|
|온라인 인덱싱|사용자 계정 컨트롤|아니오|아니요|아니요|
|다시 시작 가능한 온라인 인덱스 다시 작성|사용자 계정 컨트롤|아니오|아니요|아니요|
|온라인 스키마 변경|사용자 계정 컨트롤|아니오|아니요|아니요|
|빠른 복구|사용자 계정 컨트롤|아니오|아니요|아니요|
|미러된 백업|사용자 계정 컨트롤|아니오|아니요|아니요|
|Hot Add 메모리 및 CPU|사용자 계정 컨트롤|아니오|아니요|아니요|
|암호화된 백업|사용자 계정 컨트롤|예|아니오|아니요|
|Microsoft Azure에 하이브리드 백업(URL에 백업)|사용자 계정 컨트롤|예|아니오|아니요|
  
<sup>1</sup> Enterprise edition에서 노드 수는 최대 운영 체제입니다. Standard 버전에서는 두 개의 노드가 지원됩니다. 
...
|기능|Enterprise|표준|Web|Express|  
|-------------|----------------|--------------|---------|------------------------| 
|Columnstore <sup>1</sup>|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|  
|클러스터형 columnstore 인덱스의 큰 개체 이진 파일|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|  
|온라인 비클러스터형 columnstore 인덱스 다시 작성|사용자 계정 컨트롤|아니오|아니요|아니요|
|메모리 내 OLTP <sup>1</sup>|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|
|영구 주 메모리|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|
|테이블 및 인덱스 분할|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|  
|데이터 압축|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|
|리소스 관리자|사용자 계정 컨트롤|아니오|아니요|아니요|  
|분할된 테이블 병렬 처리|사용자 계정 컨트롤|아니오|아니요|아니요|
|NUMA 인식 및 큰 페이지 메모리 및 버퍼 배열 할당|사용자 계정 컨트롤|아니오|아니요|아니요|
|IO 리소스 관리|사용자 계정 컨트롤|아니오|아니요|아니요|  
|지연된 내구성|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|
|자동 튜닝|사용자 계정 컨트롤|아니오|아니요|아니요|
|일괄 처리 모드 적응 조인|사용자 계정 컨트롤|아니오|아니요|아니요|
|일괄 처리 모드 메모리 부여 피드백|사용자 계정 컨트롤|아니오|아니요|아니요|
|다중 문 테이블 반환 함수에 대한 인터리브 실행|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|
|대량 삽입 기능 개선|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|


<sup>1</sup> 메모리 내 OLTP 데이터 크기 및 Columnstore 세그먼트 캐시는 크기 조정 제한 섹션에서 버전별로 지정된 메모리 양으로 제한됩니다. 최대 병렬 처리 수준도 제한됩니다. 인덱스 작성에 대 한 프로세스 병렬 처리 (DOP) 수준은 Standard edition에 대 한 2 DOP 및 Web 및 Express edition에 대 한 DOP를 1로 제한 됩니다. 디스크 기반 테이블과 메모리 최적화 테이블에서 생성된 columnstore 인덱스가 해당합니다.

##  <a name="RDBMSS"></a> RDBMS 보안  
  
|기능|Enterprise|표준|Web|Express|
|-------------|----------------|--------------|---------|------------------------------------| 
|행 수준 보안|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|  
|항상 암호화|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|동적 데이터 마스킹|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|   
|기본 감사|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|미세 감사|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|투명한 데이터베이스 암호화|사용자 계정 컨트롤|아니오|아니요|아니요|   
|사용자 정의 역할|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|포함된 데이터베이스|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|백업을 위한 암호화|사용자 계정 컨트롤|예|아니오|아니요|  
...
|PowerShell 스크립팅 지원|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|데이터 계층 응용 프로그램 구성 요소 작업 지원 - 추출, 배포, 업그레이드, 삭제|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|정책 자동화(일정 및 변경 내용 검사)|사용자 계정 컨트롤|예|예|아니오|아니요|   
|성능 데이터 수집기|사용자 계정 컨트롤|예|예|아니오|아니요| 
|표준 성능 보고서|사용자 계정 컨트롤|예|예|아니오|아니요| 
|계획 지침을 위한 계획 지침 및 계획 고정|사용자 계정 컨트롤|예|예|아니오|아니요|   
|인덱스 뷰의 직접 쿼리(NOEXPAND 힌트 사용)|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|인덱싱된 뷰의 자동 유지 관리|사용자 계정 컨트롤|예|예|아니오|아니요| 
|분산형 분할 뷰|사용자 계정 컨트롤|아니오|아니요|아니요| 
|병렬 인덱스 작업|사용자 계정 컨트롤|아니오|아니요|아니요|  
|쿼리 최적화 프로그램의 인덱싱된 뷰 자동 사용|사용자 계정 컨트롤|아니오|아니요|아니요| 
|병렬 일관성 검사|사용자 계정 컨트롤|아니오|아니요|아니요| 
|SQL Server 유틸리티 제어 지점|사용자 계정 컨트롤|아니오|아니요|아니요|    

##  <a name="Programmability"></a> Programmability  
  
|기능|Enterprise|표준|Web|Express 
|-------------|----------------|--------------|---------|------------------------|  
|JSON|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|   
|쿼리 저장소|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|   
|임시 테이블|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|   
|네이티브 XML 지원|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|XML 인덱싱|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|MERGE 및 UPSERT 기능|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|   
|날짜 및 시간 데이터 형식|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|  
|국제화 지원|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|전체 텍스트 및 의미 체계 검색|사용자 계정 컨트롤|예|예|예|아니요| 
|쿼리에서 언어 지정|사용자 계정 컨트롤|예|예|예|아니요|   
|Service Broker(메시징)|사용자 계정 컨트롤|사용자 계정 컨트롤|아니요(클라이언트 전용)|아니요(클라이언트 전용)|아니요(클라이언트 전용)|   
|Transact-SQL 엔드포인트|사용자 계정 컨트롤|예|예|아니오|아니요| 
|그래프|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|  
...
버전에서 지 원하는 Integration Services (SSIS) 기능에 대 한 내용은 [!INCLUDE[ssNoVersion_md](../includes/ssnoversion-md.md)]를 참조 하세요 [SQL Server 버전에서 지 원하는 Integration Services 기능](../integration-services/integration-services-features-supported-by-the-editions-of-sql-server.md)합니다.

##  <a name="SLS"></a> 공간 및 위치 서비스  
  
|기능 이름|Enterprise|표준|Web|Express|  
|------------------|----------------|--------------|---------|------------------------------------|------------------------|
|공간 인덱스|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|   
|평면 및 측지 데이터 형식|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤| 
|고급 공간 라이브러리|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|   
|산업 표준 공간 데이터 형식 가져오기/내보내기|사용자 계정 컨트롤|예|예|사용자 계정 컨트롤|   
