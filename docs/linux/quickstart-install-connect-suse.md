...
description: "이 빠른 시작 자습서에서는 SQL Server 2017 SUSE Linux Enterprise Server에 설치할 로컬 폴더를 만들고 sqlcmd를 사용하여 데이터베이스를 쿼리하는 방법을 보여줍니다."
author: rothja
ms.author: jroth
manager: jhubbard
ms.date: 10/02/2017
...
이 빠른 시작 자습서에서는 먼저 SUSE Linux Enterprise Server (SLES) v12 SP2에 SQL Server 2017을 설치합니다. 다음으로 첫 번째 데이터베이스를 만들고 쿼리를 실행하기 위해서 **sqlcmd**를 사용해서 연결합니다.
...
**2GB 이상** 메모리를 가진 SLES v12 SP2 컴퓨터가 있어야 합니다. 파일 시스템은 **XFS** 또는 **EXT4**이어야 합니다. **BTRFS**와 같은 다른 파일 시스템은 지원되지 않습니다.
