...
Availaiblity GA (일반) 릴리스는 다음과 같은 향상 된 기능 및 수정 포함 되어 있습니다.

- 이제 NFS에서 데이터베이스 파일을 호스팅할 수 있습니다. NFS 공유 디스크 시나리오에 컨테이너 플랫폼에 대 한 원격 저장소를 탑재 및 Windows 용 Docker에 대 한 폴더를 탑재 하는 문제를 해결 합니다.
- 다른 기타 버그 수정 및 개선 합니다.
