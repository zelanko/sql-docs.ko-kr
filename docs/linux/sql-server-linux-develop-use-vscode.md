---
title: Linux의 SQL Server 용 Visual Studio Code mssql 확장 사용 | Microsoft Docs
...
---
# <a name="use-visual-studio-code-to-create-and-run-transact-sql-scripts-on-linux"></a>Visual Studio Code를 사용 하 여 만들고 Linux에서 TRANSACT-SQL 스크립트를 실행 합니다.

[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-linuxonly](../includes/appliesto-ss-xxxx-xxxx-xxx-md-linuxonly.md)]

이 문서에서는 사용 하는 *mssql* Linux에서 SQL Server 데이터베이스를 개발 하려면 Visual Studio Code 용 확장 합니다.

## <a name="install-and-start-visual-studio-code"></a>설치 하 고 Visual Studio Code 시작

Visual Studio Code는 Linux, macOS 및 확장을 지 원하는 Windows 용 그래픽 코드 편집기입니다. 

1. [다운로드 하 여 Visual Studio Code 설치] 컴퓨터에 있습니다.
...
합니다 [Visual Studio Code 용 mssql 확장] SQL Server에 연결할 수 있도록 TRANSACT-SQL (T-SQL)를 사용 하 여 쿼리 및 결과 확인 합니다.
...
> [!TIP] 
> 도 만들기 및 사용자 설정 파일에서 연결 프로필을 편집할 수 있습니다 (*settings.json*). 설정 파일을 열려면 **파일** > **기본 설정** > **설정**합니다. 자세한 내용은 [연결 프로필을 관리]합니다.
...
1. 선택 **연결 프로필 만들기** SQL Server에 대 한 새 연결 프로필을 만들려고 합니다.
   
1. 지시에 따라 새 연결 프로필에 대 한 속성을 지정 합니다. 키를 눌러 각 값을 지정한 후 **Enter** 를 계속 합니다. 
   
   1. **서버 이름 또는 ADO 연결 문자열**: SQL Server 인스턴스 이름을 지정 합니다. 사용 하 여 *localhost* 로컬 컴퓨터에 SQL Server 인스턴스에 연결 합니다. 원격 SQL Server에 연결 하려면 SQL Server 대상의 이름 또는 IP 주소를 입력 합니다. 포트를 지정 하는 경우 이름에서 분리 하려면 쉼표를 사용 합니다. 예를 들어 포트 1401에서 실행 되는 로컬 서버에 대 한 입력 *1401 localhost*합니다. 
      
      >[!NOTE]
      >데이터베이스에 대해 여기에서 키를 눌러 ADO 연결 문자열을 입력할 수도 있습니다 **Enter**필요에 따라 이름, 연결 프로필 및 키를 눌러 **Enter** 다시 연결 하 여 프로필을 만듭니다. 
      
   1. **데이터베이스 이름** (선택 사항): 사용 하려는 데이터베이스입니다. 새 데이터베이스를 만들려면 데이터베이스를 지정 하지 이름 및 키를 눌러 **Enter** 를 계속 합니다. 
      
   1. **인증 유형**: 키를 눌러 **Enter** 선택할 **SQL 로그인**합니다. 
      
   1. **사용자 이름**: 서버의 데이터베이스에 액세스할 수 있는 사용자의 이름을 입력 합니다.
      
   1. **암호**: 지정된 사용자의 암호를 입력합니다.
      
   1. **암호 저장**: 키를 눌러 **Enter** 선택할 **예** 암호를 저장 합니다. 선택 **No** 연결 프로필 사용 될 때마다 암호를 입력 해야 하는 합니다. 
      
   1. **프로필 이름** (선택 사항): 같은 연결 프로필에 대 한 이름을 입력 *localhost 프로필*합니다. 
   
   선택한 후 **Enter**, Visual Studio Code 연결 프로필을 만들고 SQL Server에 연결 합니다. 
   
   > [!TIP]
   > 연결이 실패 하는 경우의 오류 메시지에서 문제를 진단 하려고 합니다 **출력** Visual Studio Code에서 패널입니다. 열려는 합니다 **출력** 패널에서 **뷰** > **출력**합니다. 또한 검토 해야 합니다 [연결 문제 해결 권장 사항이]합니다.
...
1. 코드 조각을 바꿉니다 `DatabaseName` 사용 하 여 `TutorialDB`:
...
> [!TIP]
> Mssql 명령에 대 한 바로 가기 키를 사용자 지정할 수 있습니다. 참조 [바로 가기를 사용자 지정]합니다.
...
1. 형식 *sql* mssql 표시할 명령 또는 형식 *sqluse*를 선택한 후는 **MS SQL:Use 데이터베이스** 명령입니다.
...
1. 코드 조각에서 입력 *직원* 테이블 이름 및 *dbo* 스키마 이름에 대 한 합니다.
   
1. 다음 코드에 표시 된 대로 열을 만듭니다.
...
1. 4 개의 행을 삽입 하려면 다음 문을 추가 합니다 **직원** 테이블입니다. 
   ...
   > [!TIP]
   > 입력 하는 동안 해당 문을 완료 하는 데 T-SQL IntelliSense를 사용 합니다.
...
사용 하 여 mssql 확장에 영향을 주는에 대 한 자세한 내용은 참조는 [mssql 확장 프로젝트 wiki]합니다.

Visual Studio Code 사용에 대 한 자세한 내용은 참조는 [Visual Studio Code 설명서](https://code.visualstudio.com/docs)합니다.

[Visual Studio Code 용 mssql 확장]:https://aka.ms/mssql-marketplace
[다운로드 하 여 Visual Studio Code 설치]:https://code.visualstudio.com/Download
...
[연결 문제 해결 권장 사항이]:./sql-server-linux-troubleshooting-guide.md#connection
[바로 가기를 사용자 지정]:https://github.com/Microsoft/vscode-mssql/wiki/customize-shortcuts
[자습서: TRANSACT-SQL 문 작성]:https://docs.microsoft.com/sql/t-sql/tutorial-writing-transact-sql-statements
[TRANSACT-SQL 참조 (데이터베이스 엔진)]:https://docs.microsoft.com/sql/t-sql/language-reference
