...
> [!TIP]
> Windows 컴퓨터에서 SSMS를 실행 하도록가 없는 경우 새 고려 [SQL Server Operations Studio](../sql-operations-studio/index.md)합니다. SQL Server를 관리 하기 위한 그래픽 도구를 제공 하 고 Linux와 Windows 모두에서 실행 됩니다.
...
1. 에 **서버에 연결** 창에서 다음 정보를 입력 (SSMS 이미 실행 중인 경우 클릭 **연결 > 데이터베이스 엔진** 열려는 **서버에 연결** window):
   ...
   | **인증** | Linux의 SQL Server 2017을 사용 하 여 **SQL Server 인증**합니다. |
...
에 연결 된 상태를 *마스터* 데이터베이스 서버에서 데이터베이스 만들기 및 수정 하거나 수 기존 데이터베이스를 삭제 합니다. 다음 단계는 Management Studio를 통해 여러 일반 데이터베이스 관리 작업을 수행 하는 방법을 설명 합니다. 이러한 작업을 수행 하려면 해야 연결한 합니다 *마스터* Linux의 SQL Server 2017을 설정할 때 만든 서버 수준 보안 주체 로그인을 사용 하 여 데이터베이스입니다.

### <a name="create-a-new-database"></a>새 데이터베이스 만들기

1. SSMS를 시작 하 고 Linux의 SQL Server 2017에서 서버에 연결
