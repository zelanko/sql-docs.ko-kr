---
title: SQL Server 가용성 그룹에 대해 Ubuntu 클러스터 구성
titleSuffix: SQL Server
description: Ubuntu용 가용성 그룹 클러스터 만들기에 대한 자세한 정보
author: MikeRayMSFT
ms.author: mikeray
ms.reviewer: vanto
ms.date: 04/30/2018
...
3. Pacemaker와 같은 클러스터 리소스 관리자를 구성합니다. 이러한 지침이 이 문서에 나와 있습니다.
   
   클러스터 리소스 관리자를 구성하는 방법은 특정 Linux 배포판에 따라 다릅니다. 

   >[!IMPORTANT]
   >프로덕션 환경에는 고가용성을 위한 STONITH와 같은 펜싱 에이전트가 필요합니다. 이 설명서의 데모에서는 펜싱 에이전트를 사용하지 않습니다. 데모는 테스트 및 유효성 검사를 위해서만 제공됩니다. 
   
   >Linux 클러스터는 펜싱을 사용하여 클러스터를 알려진 상태로 되돌립니다. 펜싱을 구성하는 방법은 배포판 및 환경에 따라 달라집니다. 현재, 일부 클라우드 환경에서는 펜싱을 사용할 수 없습니다. 자세한 내용은 [RHEL 고가용성 클러스터의 지원 정책 - 가상화 플랫폼](https://access.redhat.com/articles/29440)을 참조하세요.
...
Pacemaker 클러스터 공급업체에서는 STONITH를 사용하도록 설정하고 지원되는 클러스터 설정에 대해 펜싱 디바이스를 구성해야 합니다. Cluster Resource Manager가 노드의 상태 또는 노드에 있는 리소스의 상태를 확인할 수 없는 경우에는 펜싱을 사용하여 클러스터를 알려진 상태로 다시 전환합니다. 리소스 수준 펜싱은 주로 리소스를 구성하여 중단 시 데이터 손상이 발생하지 않도록 합니다. 예를 들어, DRBD(Distributed Replicated Block Device)에서 리소스 수준 펜싱을 사용하여 통신 링크가 가동 중단될 때 노드의 디스크를 오래된 것으로 표시할 수 있습니다. 노드 수준 펜싱은 노드가 리소스를 실행하지 않도록 합니다. 이 기능은 노드를 다시 설정하여 수행되며, 해당 Pacemaker 구현을 STONITH("shoot the other node in the head" 의미)라고 합니다. Pacemaker는 서버에 대한 무정전 전원 공급 장치 또는 관리 인터페이스 카드와 같은 다양한 펜싱 디바이스를 지원합니다. 자세한 내용은 [Pacemaker 클러스터 기초](https://clusterlabs.org/pacemaker/doc/en-US/Pacemaker/1.1/html/Clusters_from_Scratch/) 및 [펜싱 및 Stonith](https://clusterlabs.org/doc/crm_fencing.html)를 참조하세요. 
...
>[!IMPORTANT]
>STONITH를 사용하지 않도록 설정하는 것은 테스트 목적으로만 수행됩니다. 프로덕션 환경에서 Pacemaker를 사용하려는 경우 작업 환경에 따라 STONITH 구현을 계획하고 사용 상태로 유지해야 합니다. 현재, 클라우드 환경(Azure 포함) 또는 Hyper-V에 대한 펜싱 에이전트는 없습니다. 따라서 클러스터 공급업체는 이러한 환경에서 프로덕션 클러스터를 실행하기 위한 지원을 제공하지 않습니다. 

## <a name="set-cluster-property-cluster-recheck-interval"></a>클러스터 속성 cluster-recheck-interval 설정

`cluster-recheck-interval`은 클러스터에서 리소스 매개 변수, 제약 조건 또는 기타 클러스터 옵션의 변경 내용을 확인하는 폴링 간격을 나타냅니다. 복제본이 가동 중단되면 클러스터는 `failure-timeout` 값 `cluster-recheck-interval` 및 값에 바인딩된 간격으로 복제본을 다시 시작하려고 합니다. 예를 들어, `failure-timeout`을 60초로 설정하고 `cluster-recheck-interval`을 120초로 설정한 경우 60초보다 크고 120초보다 작은 간격으로 다시 시작이 시도됩니다. failure-timeout을 60초로 설정하고 cluster-recheck-interval을 60초보다 큰 값으로 설정하는 것이 좋습니다. cluster-recheck-interval을 작은 값으로 설정하는 것은 바람직하지 않습니다.

속성 값을 `2 minutes`로 업데이트하려면 다음을 실행합니다.
...
> [!IMPORTANT] 
> Pacemaker 클러스터에서 관리하는 가용성 그룹 리소스가 이미 있는 경우 사용 가능한 최신 Pacemaker 패키지 1.1.18-11.el7을 사용하는 모든 배포판에서는 해당 값이 false일 때 start-failure-is-fatal 클러스터 설정의 동작이 변경됩니다. 이 변경은 장애 조치(failover) 워크플로에 영향을 줍니다. 주 복제본이 가동 중단되면 클러스터는 사용 가능한 보조 복제본 중 하나로 장애 조치(failover)되어야 합니다. 대신, 사용자는 클러스터가 실패한 주 복제본을 계속 시작하려고 한다는 것을 알 수 있습니다. 영구 가동 중단으로 인해 주 복제본이 온라인 상태가 되지 않으면 클러스터는 사용 가능한 다른 보조 복제본으로 장애 조치(failover)되지 않습니다. 이러한 변경으로 인해 start-failure-is-fatal을 설정하기 위한 이전 권장 구성이 더 이상 유효하지 않으며 설정을 기본값인 `true`로 되돌려야 합니다. 또한 AG 리소스는 `failover-timeout` 속성을 포함하도록 업데이트해야 합니다. 
>
>속성 값을 `true`로 업데이트하려면 다음을 실행합니다.
>
>```bash
>sudo pcs property set start-failure-is-fatal=true
>```
>
>기존 AG 리소스 속성 `failure-timeout`을 `60s`로 업데이트하고 다음을 실행합니다(`ag1`을 가용성 그룹 리소스의 이름으로 대체).
>
>```bash
...
## <a name="create-a-sql-server-login-for-pacemaker"></a>Pacemaker를 위한 SQL Server 서버 로그인 만들기
...
1. 사용자가 node1에서 node2까지 가용성 그룹 기본 복제본에 대해 `pcs resource move`를 실행합니다.
...
>[!IMPORTANT]
>클러스터를 구성하고 가용성 그룹을 클러스터 리소스로 추가한 후에는 Transact-SQL을 사용하여 가용성 그룹 리소스를 장애 조치(failover)할 수 없습니다. Linux의 SQL Server 클러스터 리소스는 WSFC(Windows Server 장애 조치(Failover) 클러스터)에 있을 때처럼 운영 체제와 긴밀하게 결합되지 않습니다. SQL Server 서비스는 클러스터의 존재 여부를 인식하지 못합니다. 모든 오케스트레이션은 클러스터 관리 도구를 통해 수행됩니다. RHEL 또는 Ubuntu에서 `pcs`를 사용합니다. 
