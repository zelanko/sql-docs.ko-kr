...
SQL Server Linux 컨테이너 이미지에 대 한 설명서의 모든 Ubuntu 기반 컨테이너를 가리킵니다. SQL Server 2019 미리 보기부터, Red Hat Enterprise Linux (RHEL)를 기반으로 하는 컨테이너를 사용할 수 있습니다. 컨테이너 리포지토리에 변경 **mcr.microsoft.com/mssql/server:2019-CTP3.1-ubuntu** 하 **mcr.microsoft.com/mssql/rhel/server:2019-CTP3.1** 모든 docker 명령을 합니다.
...
- 도 프로덕션 버전을 실행 하려면 개발자 컨테이너 이미지를 구성할 수 있습니다. 프로덕션 버전을 실행 하려면 다음 단계를 사용 합니다.

프로시저를 실행 하 고 요구 사항을 검토 합니다 [퀵 스타트](quickstart-install-connect-docker.md)합니다. 사용 하 여 프로덕션 버전을 지정 해야 합니다 **MSSQL_PID** 환경 변수입니다. 다음 예제에서는 Enterprise Edition에 대 한 최신 SQL Server 2017 컨테이너 이미지를 실행 하는 방법을 보여 줍니다.
...
- 에 지정 된 최소 메모리 및 디스크 요구 사항을 충족 해야 합니다 [필수 구성 요소](quickstart-install-connect-docker.md#requirements) 빠른 시작 문서의 섹션입니다.
