...
|-----------|----------------------------|  
|데이터 형식|**SSMA는 아래에 나열 된 다음을 제외한 모든 형식을 매핑합니다.**<br /><br />CLOB:이 형식의 작업에 대 한 일부 기본 함수는 없습니다 (예: CLOB_EMPTY()) 지원<br /><br />BLOB:이 형식의 작업에 대 한 일부 기본 함수는 없습니다 (예: BLOB_EMPTY()) 지원<br /><br />DBLOB:이 형식의 작업에 대 한 일부 기본 함수는 없습니다 (예: DBLOB_EMPTY()) 지원|  
|사용자 정의 형식|**SSMA 매핑합니다 다음 사용자 정의:**<br /><br />고유한 형식<br /><br />구조적된 형식<br /><br />참고 – SQL PL 데이터 형식: 약한 커서 유형이 지원 되지 않습니다.|  
|특별 한 레지스터|**SSMA는 아래에 나열 된 레지스터를 매핑합니다.**<br /><br />현재 타임 스탬프<br /><br />현재 날짜<br /><br />현재 시간<br /><br />현재 표준 시간대<br /><br />현재 사용자<br /><br />SESSION_USER 및 사용자<br /><br />SYSTEM_USER<br /><br />현재 CLIENT_APPLNAME<br /><br />현재 CLIENT_WRKSTNNAME<br /><br />현재 잠금 제한 시간<br /><br />현재 스키마<br /><br />현재 서버<br /><br />현재 격리<br /><br />특별 한 등록을 다른 SQL server 의미 체계에 매핑되지 않습니다.|  
|CREATE TABLE|**SSMA는 다음과 같은 예외가 CREATE TABLE을 매핑합니다.**<br /><br />다차원 클러스터링 (MDC) 테이블<br /><br />범위 클러스터형 테이블 (RCT)<br /><br />분할된 테이블<br /><br />분리 된 테이블<br /><br />데이터를 캡처 절<br /><br />IMPLICITLY 숨겨진 옵션<br /><br />휘발성 옵션|  
|CREATE VIEW|SSMA ' WITH 로컬 CHECK OPTION으로 ' CREATE VIEW 매핑하지만 다른 옵션이 SQL server 의미 체계에 매핑되어 있지 않으므로|  
|CREATE  INDEX|**SSMA는 다음과 같은 예외가 CREATE INDEX를 매핑합니다.**<br /><br />XML 인덱스<br /><br />겹치는 없이 BUSINESS_TIME 옵션<br /><br />분할 된 절<br /><br />사양만 옵션<br /><br />EXTEND를 사용 하 여 옵션<br /><br />MINPCTUSED 옵션<br /><br />페이지 나누기 옵션|  
|트리거|**SSMA는 다음과 같은 트리거 의미 체계를 매핑합니다.**<br /><br />이후/트리거 모든 행에 대 한<br /><br />각 문은 트리거 /FOR 후<br /><br />하기 전에 모든 행에 대 한 및 INSTEAD OF / / 모든 행 트리거에 대 한|  
|시퀀스|매핑됩니다.|  
|SELECT 문|**다음과 같은 예외가 SSMA 지도 선택합니다.**<br /><br />변경 테이블 참조 데이터 절-부분적으로 매핑된 하지만 최종 테이블은 지원 되지 않습니다.<br /><br />테이블 참조 절-부분적으로 매핑된 경우에 테이블-참조, 외부 테이블 참조, analyze_table 식 이지만 컬렉션 파생 테이블, xmltable 식 SQL server 의미 체계에 매핑되어 있지 않으므로<br /><br />기간 사양 절-매핑되지 않습니다.<br /><br />절은 계속 처리기 – 매핑되지 않습니다.<br /><br />입력 한 상관 관계 절-매핑되지 않습니다.<br /><br />동시 액세스 결정 절-매핑되지 않습니다.|  
|값 문|매핑됩니다.|  
|INSERT 문|매핑됩니다.|  
|UPDATE 문|S**SMA 다음 제외 하 고 업데이트를 매핑합니다.**<br /><br />SQL server 의미 체계에 테이블 참조 절-전용-테이블-참조 매핑되지 않은<br /><br />기간 절-매핑되지 않습니다.|  
|MERGE 문|**SSMA는 다음과 같은 예외가 병합을 매핑합니다.**<br /><br />각 절에 여러 번 vs 단일-각 절의 제한 된 발생에 대 한 SQL server 의미 체계에 매핑된<br /><br />신호 절-SQL Server 의미 체계에 매핑되지 않는<br /><br />혼합 된 업데이트 및 삭제 절-SQL Server 의미 체계에 매핑되지 않는<br /><br />기간-절-SQL Server 의미 체계에 매핑되지 않는|  
|DELETE 문|**SSMA 지도 다음과 같은 경우는 예외를 삭제합니다.**<br /><br />SQL server 의미 체계에 테이블 참조 절-전용-테이블-참조 매핑되지 않은<br /><br />기간 절-SQL Server 의미 체계에 매핑되지 않는|  
|격리 수준 및 잠금 유형|매핑됩니다.|  
|프로시저 (SQL)|매핑됩니다.|  
|프로시저 (외부)|수동 업데이트를 해야 합니다.|  
|프로시저 (원본)|SQL Server 의미 체계에 매핑되지 않습니다.|  
|대입문|매핑됩니다.|  
|프로시저에 대 한 CALL 문|매핑됩니다.|  
|CASE 문|매핑됩니다.|  
|FOR 문|매핑됩니다.|  
|GOTO 문|매핑됩니다.|  
|IF 문|매핑됩니다.|  
...
|WHILE 문|매핑됩니다.|  
|GET 진단 문|**SSMA는 다음과 같은 예외가 진단을 가져올를 매핑합니다.**<br /><br />ROW_COUNT – 매핑됩니다.<br /><br />DB2_RETURN_STATUS – 매핑됩니다.<br /><br />MESSAGE_TEXT – 매핑됩니다.<br /><br />DB2_SQL_NESTING_LEVEL-SQL Server 의미 체계에 매핑되지 않는<br /><br />DB2_TOKEN_STRING-SQL Server 의미 체계에 매핑되지 않는|  
|커서|**SSMA는 다음과 같은 예외가 커서를 매핑합니다.**<br /><br />ALLOCATE 커서 문-SQL Server 의미 체계에 매핑되지 않는<br /><br />연결 로케이터 문-SQL Server 의미 체계에 매핑되지 않는<br /><br />DECLARE CURSOR 문에-Returnability 절은 SQL server 의미 체계에 매핑되지 않은<br /><br />FETCH 문이 – 부분 매핑을 합니다. 대상으로 하는 변수는만 지원 됩니다. 설명자가 잘못 된 SQL server 의미 체계에 매핑되지 않은|  
|변수|매핑됩니다.|  
|예외, 처리기 및 조건|**SSMA는 다음과 같은 예외가 "예외 처리를"를 매핑합니다.**<br /><br />종료 처리기 – 매핑됩니다.<br /><br />취소 처리기 – 매핑됩니다.<br /><br />처리기 – 계속 매핑되지 않습니다.<br /><br />조건-것에 매핑되지 않고 SQL server 의미 체계입니다.|  
|동적 SQL|매핑되지 않습니다.|  
|별칭|매핑됩니다.|  
