...
3.  **AssessmentReportGenerationSample.xml:** 이 샘플에는 사용할 수 있는 사용자가 분석을 위해 데이터를 마이그레이션하고 변환 하기 전에 그 xml 평가 보고서를 생성 하려면 사용자 수 있습니다.  
...
4.  **SqlStatementConversionSample.xml:** 이 샘플에 사용자가 해당 생성할 수 있도록 `t-sql` 원본 데이터베이스에 대 한 스크립트 `sql` 명령 입력으로 제공 합니다.  
