...
다음 예에서는 레코드를 반환 하는 다른 함수 `print_emp` `get_employeeTbl` 를 호출 하는 프로시저를 만들었습니다.
...
SSMA는 위의 주 프로시저 `print_emp`코드를 변환 하려고 할 때 레코드를 반환 하는 호출 된 함수의 `get_emploreeTbl` 작업을 해결 하지 않으므로 다음과 같은 오류 메시지를 생성 합니다.

> O2SS0359: 함수 호출 식의 반환 형식에 대 한 설명을 가져올 수 없습니다.

## <a name="possible-remedies"></a>가능한 해결 방법

위의 오류를 해결 하는 방법은 SQL Server 코드를 다시 작성 하는 것입니다. 스칼라 함수, 인라인 테이블 반환 함수 및 다중 문 테이블 반환 함수를 지원 하기 SQL Server 때문에 호출 된 함수의 T-sql 코드 내에서 `@mytable` 임시 테이블을 선언할 수 있습니다. 코드에서 동일한 비즈니스 논리를 사용 하 여이 테이블을 채운 다음이 테이블을 다시 호출 환경으로 반환 합니다. 또한 호출 하는 프로시저에서 테이블 변수를 사용 하 여 호출 된 함수의 반환 값 (이 경우에는 레코드 집합)을 저장 해야 합니다.
