...
아래 예제에서는 함수 `MATH_PKG` `GET_SUM`를 포함 하는 패키지를 만들었습니다.
...
SSMA는 위의 문을 변환 하려고 할 때 필수 매개 변수 또는 잘못 된 `GET_SUM` 인수 없이 호출 되는 함수를 확인할 수 없기 때문에 다음과 같은 오류 메시지가 생성 됩니다.

> O2SS0101: 매개 변수가 잘못 되었습니다.

## <a name="possible-remedies"></a>가능한 해결 방법

위의 오류를 해결 하는 방법은 Oracle에서 코드를 다시 작성 하는 것입니다. 실제로 Oracle에서는 문에서 위의 패키지를 호출할 때에 대 `GET_SUM`한 호출에서 잘못 된 수 또는 인수 형식의 PL/SQL 오류도 발생 시킵니다.
...
다음은 위의 Oracle 예제에 대 한 재작성 코드입니다. 아래는 `(n1 INT DEFAULT 0)` `GET_SUM` 함수의 첫 번째 매개 변수에 기본값을 할당 합니다.
