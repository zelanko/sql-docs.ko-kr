...
|4|-x/xmloutput|예|xmloutputfile|이 옵션은 XML 형식의 콘솔 출력을 나타냅니다. 이 옵션을 지정 하지 않으면 기본 출력은 텍스트 형식입니다.<br /><br />Xmloutputfile을 지정 하지 않으면 XML 출력이로 전달 됩니다 `STDOUT` .<br /><br />Xmloutputfile은 콘솔 출력이 XML 형식으로 작성 되는 파일의 이름입니다.|  
