...
description: Oracle 용 SSMA를 사용 하 여 Oracle에서 SQL Server 인스턴스로 변환한 데이터베이스 개체를 로드 하는 방법에 대해 알아봅니다.
ms.prod: sql
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
...
수정 하지 않고 변환 된 데이터베이스 개체를 SQL Server으로 로드 하려는 경우 SSMA에서 데이터베이스 개체를 직접 만들거나 다시 만들 수 있습니다. 이 방법은 쉽고 간단 하지만 [!INCLUDE[tsql](../../includes/tsql-md.md)] 저장 프로시저를 제외 하 고 SQL Server 개체를 정의 하는 코드를 사용자 지정할 수 없습니다.  
  
개체를 만드는 데 사용 되는을 수정 하려는 경우 [!INCLUDE[tsql](../../includes/tsql-md.md)] 또는 개체 생성을 보다 세부적으로 제어 하려는 경우 SSMA를 사용 하 여 스크립트를 만듭니다. 그런 다음 해당 스크립트를 수정 하 고, 각 개체를 개별적으로 만들 수 있으며, SQL Server 에이전트를 사용 하 여 해당 개체 만들기를 예약할 수 있습니다.  
...
SQL Server 개체 정의를 하나 이상의 스크립트로 저장 한 후에 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 는를 사용 하 여 스크립트를 보고 수정할 수 있습니다.  
...
쿼리 편집기를 사용 하 여 스크립트를 실행 하는 방법에 대 한 자세한 내용은 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] [!INCLUDE[tsql](../../includes/tsql-md.md)] SQL Server 온라인 설명서의 "쿼리"를 참조 하십시오.  
