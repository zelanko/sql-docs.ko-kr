...
SSMA 테스터는 테스트 사례를 실행할 때 테스트를 위해 선택한 개체를 실행 하 고 확인 결과에 대 한 보고서를 만듭니다. 두 플랫폼에서 결과가 동일 하면 테스트에 성공 했습니다. Oracle과 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 간의 개체 대응은 현재 ssma 프로젝트의 스키마 매핑 설정에 따라 결정 됩니다.  
...
다음 개체는에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]확인 된 테이블의 스키마에 생성 됩니다.  
