...
-   Oracle 개체를 변환 하려면 [!INCLUDE[tsql](../../includes/tsql_md.md)] 구문에서 메타 데이터를 업데이트 [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)], 변환 된 구문에 스크립트를 저장 하려면 계정이의 인스턴스에 로그온 할 수 있는 권한이 있어야 [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]합니다.  
...
-   SSMA에 의해 생성 된 코드를 실행 하려면 계정 있어야 **Execute** 의 모든 사용자 정의 함수에 대 한 권한을 **ssma_oracle** 대상 데이터베이스의 스키마입니다. 이러한 함수는 Oracle 시스템 함수의 동일한 기능을 제공 하 고 변환 된 개체에 의해 사용 됩니다.  
