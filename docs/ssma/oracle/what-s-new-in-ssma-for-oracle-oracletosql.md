...
Oracle 용 SSMA의 v8.2 릴리스 하도록 향상 되었습니다.

* DBMS_OUTPUT 지원을 추가 합니다. 설정/해제 합니다.
* CAST AS 제거 부동 소수점 BINARY_FLOAT 및 BINARY_DOUBLE 열에 대 한 기본 데이터 마이그레이션 쿼리 합니다.
* 현재 값이 변경 된 경우 새로 고침 시퀀스를 수정 합니다.
* 같은 이름의 열이 있는 경우 잘못 해석 의사 (pseudo) 열 (ROWNUM 등)와 관련 된 버그를 수정 합니다.
* 모호한 확인 되지 않은 식별자를 사용 하 여 루프에 대 한 변환 발생 하는 충돌을 해결 합니다.

또한이 버전의 품질 및 변환 메트릭 뿐만 아니라 수정에 대 한 개선 하기 위한 수정 대상된 집합을 포함:
...
Oracle 용 SSMA v8.1 릴리스의 품질 및 변환 메트릭을 향상 하도록 설계 된 대상된 수정 사항을 향상 되었습니다.
...
Oracle 용 SSMA v8.0 릴리스의 품질 및 변환 메트릭을 개선 하기 위한 대상된 수정 사항을 사용 하 여 향상 되었습니다. 이 릴리스에 또한 다음과 같은 새로운 기능을 제공합니다.
...
* 에 대 한 지원 **Azure SQL Database Managed Instance** 대상으로 합니다. 이제 Azure SQL Database Managed Instance를 대상으로 하는 새 프로젝트를 만들 수 있습니다.
  ...
  > [!NOTE]
  > 확장 팩 Oracle 용 SSMA Azure SQL Database 관리 되는 인스턴스에서 원격 설치를 허용 하도록 업데이트 되었습니다.
  >
  > ![확장 팩 Oracle 용 SSMA](../media/ssma-oracle-ext-pack.png)

  Azure SQL Database Managed Instance를 대상으로 할 때 테스터 및 서버 쪽 데이터 마이그레이션 등의 일부 기능은 지원 되지 않습니다. 자세히 알아보세요 [여기](https://blogs.msdn.microsoft.com/datamigration/2019/02/17/migrate-your-oracle-database-to-azure-sql-database-managed-instance-using-ssma-8-0/)합니다.
...
* 관리 되는 공식 NET 드라이버를 사용 하 여 Oracle에 연결할 수 있습니다. OCI 드라이버를 더 이상 사용 하 여 SQL Server Migration Assistant for Oracle에 대 한 필수 구성 요소.

* VARCHAR로 ROWID 및 UROWID를 기본적으로 매핑할 수 있습니다. 'uniqueidentifier' 명시적 ROWID 열에 대 한 데이터 마이그레이션에 맞게 변경 합니다.
...
* 추가 SQL Server의 temporal 테이블에 Oracle 못하실지도 보관 테이블 변환 합니다.
  ...
  > [!NOTE]
  > SSMA는 Oracle 플래시 백 데이터 보관 테이블에서 기록 데이터를 복사 하지 않습니다. 결과적으로, 기록 데이터 마이그레이션 프로세스 중에 수동으로 복사 해야 합니다. 또한 SSMA 시스템 테이블로 처리 되기 때문에 SQL Server 메타 데이터 탐색기에서 기록 테이블을 표시 하지 않습니다, SQL Server Management Studio에서 기록 테이블을 볼 수 있습니다.
  >
...
Oracle 용 SSMA의 2016 년 3 월 미리 보기 릴리스의 대 한 지원을 추가 합니다.  
  
* SQL Server 2016으로 마이그레이션입니다.  
* 마이그레이션 Oracle 행 수준 사용 하는 보안 (몇 가지 제한 사항이).  
* SQL Server 열 저장소를 메모리 내 테이블에서 마이그레이션 Oracle입니다.  
...
RowType에 대 한 지원을 추가 하는 Oracle 용 SSMA의 2012 년 1 월 릴리스 및 RecordType 입력된 매개 변수 기본값으로 NULL로 설정 합니다.  
...
추가 하는 Oracle 용 SSMA의 2010 년 7 월 릴리스:

* SQL Server 2008 R2로 마이그레이션에 대 한 지원입니다.  
* 명령줄 실행에 대 한 새 SSMA 콘솔 응용 프로그램입니다.  
* 서버 쪽 및 클라이언트 쪽 데이터 마이그레이션 엔진을 사용 하 여 데이터 마이그레이션을 지원 합니다.  
* 데이터 마이그레이션의 "사용자 지정 선택" 문에 대해 지원 합니다.  
* Oracle에서 11g R2 마이그레이션에 대 한 지원입니다.  
...
추가 하는 Oracle 용 SSMA의 2007 년 8 월 릴리스:

* 새 테스터 구성 요소를 사용 하 여 생성, 관리 및 변환 된 SQL 코드를 확인 하려면 테스트 사례를 실행할 수 있습니다.  
* SQL 변환기를 Oracle 하위 형식, 컬렉션 및 로컬 모듈의 변환에 대 한 지원이 추가 되었습니다.  
* 새 동기화 기능을 사용 하면 SQL Server 데이터베이스를 사용 하 여 특정 개체를 동기화 할 수 있습니다.  
* 새 변환 옵션입니다.  
