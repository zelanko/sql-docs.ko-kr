---
description: Sybase ASE 데이터베이스 개체 변환(SybaseToSQL)
...
마이그레이션 프로세스의 다음 단계에서는 변환 된 [데이터베이스 개체를 SQL Server/SQL Azure (SybaseToSQL)로 로드](./loading-converted-database-objects-into-sql-server-sybasetosql.md)합니다.  
