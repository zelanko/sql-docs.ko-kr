...
**프로젝트 설정** 대화 상자의 변환 페이지에는 Ssma에서 Sybase (Sybase Server Enterprise) 구문을 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 SQL Azure 구문으로 변환 하는 방법을 사용자 지정 하는 설정이 포함 되어 있습니다.  
...
-   **변환 및 수정** 을 선택 하 여 ssma에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]사용할 식을 변환 하 고 수정 합니다. 리터럴 상수의 바이트 순서가 반대로 바뀝니다. 다른 모든 이진 값 (예: 이진 변수 및 열)은 오류로 표시 됩니다. ASE 서버에 이진 값을 변경 해야 하는 바이트 순서가 있는 경우이 값을 사용 합니다.  
...
**같음 확인 변환**  
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]/SQL Azure에서는 ANSI_NULLS 설정이 on 인 경우 같음 비교에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]null 값이 포함 되어 있으면/SQL Azure UNKNOWN을 반환 합니다. ANSI_NULLS 해제 된 경우에는 비교 되는 열과 식 또는 두 식이 모두 null 이면 null 값을 포함 하는 같음 비교가 true를 반환 합니다. 기본적으로 (ANSINULL OFF) Sybase ASE 같음 비교는 ANSI_NULLS [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]OFF를 사용 하는/SQL Azure와 동일 하 게 작동 합니다.  
  
-   **단순 변환**을 선택 하는 경우 ssma는 null 값에 대 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]한 추가 검사 없이 ASE 코드를/SQL Azure 구문으로 변환 합니다. /SQL Azure에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]ANSI_NULLS 꺼져 있거나 case를 기준으로 같음 비교를 수정 하려는 경우이 설정을 사용 합니다.  
...
**서식 문자열**  
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]/SQL Azure는 PRINT 및 RAISERROR 문의 *format_string* 인수를 더 이상 지원 하지 않습니다. *Format_string* 변수는 대체 가능 매개 변수를 문자열에 직접 넣고 런타임에 매개 변수를 대체 하는 것을 지원 합니다. 대신에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 는 문자열 리터럴 또는 변수를 사용 하 여 작성 된 문자열을 사용 하 여 전체 문자열이 필요 합니다. 자세한 내용은 온라인 설명서의 [!INCLUDE[tsql](../../includes/tsql-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] "PRINT ()" 항목을 참조 하십시오.  
...
-   PRINT 및 RAISERROR 함수에 문자열 리터럴을 사용 하려면 **새 문자열 만들기**를 선택 합니다.  
    ...
    *Format_string* 다음 문에서와 같이 변수입니다.  
    ...
    ```  
    **새 문자열 모드 만들기** 를 사용 하는 경우 ssma는 CONCAT_NULL_YIELDS_NULL [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 옵션이 OFF 라고 가정 합니다. 따라서 SSMA는 null 인수를 확인 하지 않습니다.  
  
-   SSMA에서 각 PRINT 및 RAISERROR 문에 대해 새 변수를 작성 하 게 한 다음 해당 변수를 문자열 값으로 사용 하려면 **새 변수 만들기**를 선택 합니다.  
  
    이 모드에서 PRINT 또는 RAISERROR 문이 자리 표시자 및 지역 변수를 사용 하지 않는 경우 SSMA는 모든 이중% 문자 (%%)를 대체 합니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]/SQL Azure 구문을 준수 하는 1% 문자  
...
이 설정은 ASE 사용자 메시지 번호에 추가 하 여 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 사용자 메시지로 변환 하는 번호를 지정 합니다. 에서 사용자 메시지를 사용 **하는 경우** 에는이 값을 큰 값으로 변경 해야 할 수 있습니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 이는 변환 된 메시지 번호가 기존 메시지 번호와 충돌 하지 않도록 하기 위한 것입니다.  
...
-   낙관적 모드에서 SSMA는 CONCAT_NULL_YIELDS_NULL [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 옵션이 off 인 것으로 가정 하 고 NULL 인수를 확인 하지 않습니다.  
...
**DATALENGTH 함수**  
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]DATALENGTH 함수에서 반환 하는 값은 값이 단일 공간이 면/SQL Azure와 ASE는 다릅니다. 이 경우/SQL Azure [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]는 0을 반환 하 고 ASE는 1을 반환 합니다.  
...
**INDEX_COL 함수**  
ASE는 INDEX_COL 함수에 대 한 선택적 *user_id* 인수를 지원 합니다. 그러나/ [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]SQL Azure에서는이 인수를 지원 하지 않습니다. *User_id* 인수를 사용 하는 경우이 함수를 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]/SQL Azure 구문으로 변환할 수 없습니다.  
