...
**원인:** 일부 쿼리 느려질 수 업사이징 후 다음과 같은 이유로:  
...
**원인:** RecordSet.AddNew Jet에서를 호출한 후 레코드를 업데이트 하기 전에 자동 증가 열은 사용할 수 있습니다. 사실 이것이 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 SQL Azure입니다. Id 열의 새 값의 새 값은 새 레코드를 저장 한 후에 사용할 수 있습니다.  
...
**원인:** 레코드를 추가 하는 경우는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 테이블의 고유 인덱스 필드에 기본값을 할당 하지 않으면 값을 해당 필드에 새 레코드에 나타나지 않으면에서 테이블을 다시 열 때까지 경우 VBA를 사용 하 여 SQL Azure 테이블 또는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 SQL Azure입니다. 새 레코드의 값을 가져오려고 시도 하면 다음 오류 메시지가 나타납니다.  
  
`Run-time error '3167' Record is deleted.`  
  
**해결 방법:** 여는 경우는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 VBA 코드를 사용 하 여 테이블을 포함 하는 SQL Azure `dbSeeChanges` 옵션을 다음 예제와 같이:  
...
**원인:** 쿼리는 고유 인덱스에 포함 된 모든 열이 없는 경우에 쿼리를 사용 하 여 새 값을 추가할 수 없습니다.  
  
**해결 방법:** 하나 이상의 고유 인덱스에 포함 된 모든 열이 쿼리의 일부가 되는지 확인 합니다.  
  
### <a name="you-cannot-modify-a-linked-table-schema-with-access"></a>액세스를 사용 하 여 연결 된 테이블 스키마를 수정할 수 없습니다.  
**원인:** 데이터 마이그레이션 후 연결 테이블 사용자 액세스에서는 테이블의 스키마를 수정할 수 없습니다.  
  
**해결 방법:** 테이블 스키마를 사용 하 여 수정할 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], 액세스의 링크를 업데이트 합니다.  
