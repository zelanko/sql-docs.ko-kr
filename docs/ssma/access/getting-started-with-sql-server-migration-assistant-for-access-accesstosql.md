...
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Access 용 SSMA (Migration Assistant)를 사용 하면 Access 데이터베이스 개체를 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 AZURE sql db 개체로 신속 하 게 변환 하 고, 결과 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 개체를 또는 azure sql db로 업로드 하 고, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] access에서 또는 azure sql db로 데이터를 마이그레이션할 수 있습니다. 필요한 경우 또는 Azure SQL DB에서 기존 Access 프런트 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 엔드 응용 프로그램 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 을 계속 사용할 수 있도록 액세스 테이블을 또는 azure sql db 테이블에 연결할 수도 있습니다.  
...
SSMA를 설치한 후에는 도구를 사용 하 여 Access 데이터베이스를 또는 Azure SQL DB로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 마이그레이션하기 전에 ssma 사용자 인터페이스를 익히는 데 도움이 됩니다. 메타 데이터 탐색기, 메타 데이터, 도구 모음, 출력 창 및 오류 목록 창이 포함 된 SSMA 사용자 인터페이스가 다음 다이어그램에 표시 됩니다.  
...
- 또는 Azure SQL DB로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Access 데이터베이스 개체의 인벤토리 내보내기
- 또는 Azure SQL DB로의 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 변환을 평가 하는 보고서를 만듭니다.
- 액세스 스키마를 또는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] AZURE SQL DB 스키마로 변환 합니다.

도구 모음 및 메뉴를 사용 하 여 이러한 작업을 수행할 수도 있습니다.  
  
또한 인스턴스에 연결 해야 합니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. 연결에 성공 하면 메타 데이터 탐색기에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터베이스 계층이 표시 됩니다. 액세스 스키마를 스키마로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 변환한 후 메타 데이터 탐색기에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 변환 된 스키마를 선택 하 고에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]스키마를 로드할 수 있습니다.  
  
새 프로젝트의 드롭다운에서 마이그레이션 대화 상자에서 Azure SQL DB를 선택한 경우 Azure SQL DB에 연결 해야 합니다. 성공적으로 연결 되 면 azure sql db 데이터베이스의 계층이 Azure SQL DB 메타 데이터 탐색기에 나타납니다. 액세스 스키마를 Azure SQL DB 스키마로 변환한 후에는 Azure SQL DB 메타 데이터 탐색기에서 변환 된 스키마를 선택 하 고에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]스키마를 로드할 수 있습니다.  
  
변환 된 스키마를 또는 Azure [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sql db로 로드 한 후에는를 반환 하 여 메타 데이터 탐색기에 액세스 하 고 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Access 데이터베이스에서 또는 azure sql db 데이터베이스로 데이터를 마이그레이션할 수 있습니다. 필요한 경우 액세스 테이블을 또는 Azure SQL DB 테이블 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에 연결할 수도 있습니다.  
...
-   변환할 개체를 선택 하 고 개체를 구문 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 으로 변환 합니다. 자세한 내용은 [Access 데이터베이스 개체 변환](converting-access-database-objects-accesstosql.md)을 참조 하세요.  
  
-   데이터 마이그레이션을 위한 개체를 선택 하 고 해당 개체의 데이터를 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]로 마이그레이션합니다. 자세한 내용은 [SQL Server로 Access 데이터 마이그레이션](migrating-access-data-into-sql-server-azure-sql-db-accesstosql.md)을 참조 하세요.  
  
-   액세스 및 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 테이블 연결 및 연결 해제  
  
#### <a name="sql-server-or-azure-sql-db-metadata-explorer"></a>SQL Server 또는 Azure SQL DB 메타 데이터 탐색기  
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]또는 Azure SQL DB 메타 데이터 탐색기에 인스턴스 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 AZURE sql db에 대 한 정보가 표시 됩니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 AZURE SQL DB 인스턴스에 연결 하는 경우 ssma는 해당 인스턴스에 대 한 메타 데이터를 검색 하 여 프로젝트 파일에 저장 합니다.  
...
각 메타 데이터 탐색기의 오른쪽에는 선택한 개체를 설명 하는 탭이 있습니다. 예를 들어 액세스 메타 데이터 탐색기에서 테이블을 선택 하는 경우 **테이블**, **형식 매핑**, **속성**및 **데이터**라는 네 개의 탭이 표시 됩니다. 메타 데이터 탐색기 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에서 테이블을 선택 하면 **테이블**, **SQL**및 **데이터**라는 세 개의 탭이 표시 됩니다.  
...
-   메타 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터 탐색기에서 **테이블** 탭의 테이블 및 인덱스 속성을 변경할 수 있습니다. 스키마를에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]로드 하기 전에 이러한 변경 작업을 수행 하십시오. 자세한 내용은 [Access 데이터베이스 개체 변환](converting-access-database-objects-accesstosql.md)을 참조 하세요.  
...
|**스키마 변환**|선택한 액세스 스키마를 또는 Azure [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SQL DB 스키마로 변환 합니다.<br /><br />이 명령은 Access Metadata Explorer에서 개체를 선택한 경우에만 사용할 수 있습니다.|  
|**데이터 마이그레이션**|Access 데이터베이스에서 또는 Azure SQL DB [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 로 데이터를 마이그레이션합니다. 이 명령을 실행 하기 전에 액세스 스키마를 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 AZURE sql db 스키마로 변환한 다음 개체를 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 또는 azure sql db로 로드 해야 합니다.<br /><br />이 명령은 Access Metadata Explorer에서 개체를 선택한 경우에만 사용할 수 있습니다.|  
|**중지**|개체를 또는 Azure SQL DB 구문으로 변환 하 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 는 등의 방법으로 현재 프로세스를 중지 합니다.|  
