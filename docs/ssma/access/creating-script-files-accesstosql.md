---
description: 스크립트 파일 만들기 (AccessToSQL)
title: 스크립트 파일 만들기 (AccessToSQL) | Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 08/17/2017
ms.reviewer: ''
...
4.  **다시 연결 공급자:** 이를 통해 사용자는 연결 오류가 발생 하는 경우 다시 연결 설정을 지정할 수 있습니다. 이는 원본 서버와 대상 서버 둘 다에 대해 설정할 수 있습니다.  
...
7.  **작업 중지:** 중간 작업 중에 사용자가 작업을 중지 하려는 경우에는 **' Ctrl + C '** 바로 가기 키를 사용할 수 있습니다. Access 용 SSMA 콘솔은 작업이 완료 될 때까지 기다리거나 콘솔 실행을 종료 합니다.  
...
서버 연결 매개 변수는 스크립트 파일 또는 서버 연결 파일에서 정의할 수 있습니다. 자세한 내용은 [서버 연결 파일 &#40;만들기&#41;](../../ssma/access/creating-the-server-connection-files-accesstosql.md) 섹션을 참조 하세요.  
...
예를 들어 Access 데이터베이스의 특정 테이블에 대 한 일반적인 데이터 마이그레이션은 데이터베이스 테이블의 계층 구조를 따릅니다 &gt; .  
...
스크립트 전체 목록- [SSMA 콘솔 &#40;AccessToSQL를 실행](../../ssma/access/executing-the-ssma-console-accesstosql.md) 하 여 찾을 수 있습니다&#41;  
  
## <a name="script-file-validation"></a>스크립트 파일 유효성 검사  
사용자는 ' 스키마 ' 폴더에서 사용 가능한 스키마 정의 파일 **' A2SSConsoleScriptSchema '** 에 대해 스크립트 파일의 유효성을 쉽게 검사할 수 있습니다.  
  
## <a name="next-step"></a>다음 단계
콘솔 운영의 다음 단계에서는 [AccessToSQL&#41;&#40;변수 값 파일을 만듭니다 ](../../ssma/access/creating-variable-value-files-accesstosql.md).  
