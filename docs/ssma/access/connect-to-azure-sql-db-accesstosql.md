...
---
# <a name="connect-to-azure-sql-db-accesstosql"></a>Azure SQL DB에 연결 (AccessToSQL)
...
2.  데이터베이스가 없는 경우 두 개의 메뉴 항목이 표시 됩니다.  
    ...
    2.  항상 사용 하도록 설정 된 **새 데이터베이스를 만들어** 사용자가 SQL Azure 계정에서 새 azure 데이터베이스를 만들 수 있도록 합니다. 이 메뉴 항목을 클릭 하면 데이터베이스 이름과 크기가 있는 azure 데이터베이스 만들기 대화 상자가 표시 됩니다.  
