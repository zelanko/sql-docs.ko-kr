...
1.  **텍스트 지우기:** ' 암호 ' 노드의 값 특성에 데이터베이스 암호를 입력 합니다. 스크립트 파일 또는 서버 연결 파일의 서버 섹션에서 서버 정의 노드 아래에 있습니다.  
    ...
    **암호화 된 암호:** 이 경우 지정 된 암호는 ProtectedStorage의 로컬 컴퓨터에 암호화 된 형식으로 저장 됩니다.  
    ...
    -   **암호 보안**  
        ...
        -   를 사용 `SSMAforAccessConsole.exe` 하 여 `-securepassword` 를 실행 하 고 명령줄에 스위치를 추가 하 여 서버 정의 섹션에서 password 노드를 포함 하는 서버 연결 또는 스크립트 파일을 전달 합니다.  
    ...
    -   **암호화 된 암호 제거**  
        ...
        를 `SSMAforAccessConsole.exe` 사용 하 여`-securepassword` 를 `-remove` 실행 하 고 명령줄에서 서버 id를 전달 하는 스위치를 사용 하 여 로컬 컴퓨터에 있는 보호 된 저장소 파일에서 암호화 된 암호를 제거 합니다.  
    ...
    -   **암호가 암호화 된 서버 Id 나열**  
        ...
        를 사용 하 여 SSMAforAccessConsole .exe를 `-securepassword` 실행 `-list` 하 고 명령줄에서 스위치를 사용 하 여 암호가 암호화 된 모든 서버 id를 나열 합니다.  
