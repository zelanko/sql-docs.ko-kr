...
description: TestCondition 클래스를 확장하여 SQL Server 단위 테스트 디자이너용 사용자 지정 테스트 조건을 만드는 방법을 확인합니다. 사용자 지정 테스트 조건의 예를 봅니다.
ms.prod: sql
ms.technology: ssdt
ms.topic: conceptual
...
각 테스트 스크립트에 대해 하나의 일괄 처리만 지원됩니다. 따라서 테스트 조건은 항상 첫 번째 배열 요소를 검사합니다. 배열 요소에는 데이터 세트이 포함되어 있고 이 데이터 세트에는 테스트 스크립트의 반환된 결과 집합이 포함되어 있습니다. 이 예의 코드는 데이터 세트의 데이터 테이블에 적절한 수의 열이 포함되어 있는지 확인합니다. 자세한 내용은 데이터 세트을 참조하십시오.  
