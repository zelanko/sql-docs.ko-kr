...
-   [방법: 응용 프로그램을 빌드한 후 예약된 테스트 구성 및 실행](http://msdn.microsoft.com/library/ms182465(VS.100).aspx)  
  
-   [기본 빌드 정의 만들기](http://msdn.microsoft.com/library/ms181716(VS.100).aspx)  
...
3.  [속성] 창에서 **테스트 범주**를 클릭한 후 오른쪽 끝 열에서 줄임표(...)를 클릭합니다.  
...
4.  *BuildComputer***.sqlunitttest.config**를 입력하고 Enter 키를 누릅니다. 여기서 *BuildComputer*는 빌드 에이전트가 실행되는 컴퓨터의 이름입니다.  
...
1.  Team Foundation Server를 실행하는 컴퓨터에 연결합니다.  
    ...
    자세한 내용은 [소스 제어 탐색기 사용](http://msdn.microsoft.com/library/ms181370(VS.100).aspx)을 참조하세요.  
  
2.  솔루션이 아직 소스 제어에 없으면 소스 제어에 추가합니다.  
  
    자세한 내용은 [버전 제어에 프로젝트 또는 솔루션 추가](http://msdn.microsoft.com/library/ms181374(VS.100).aspx)를 참조하세요.  
  
3.  **보기**를 클릭한 후 **보류 중인 체크 인**을 클릭합니다.  
  
4.  솔루션의 모든 파일을 체크 인합니다.  
  
    자세한 내용은 [보류 중인 변경 내용 체크 인](http://msdn.microsoft.com/library/ms181411(VS.100).aspx)을 참조하세요.  
  
    > [!NOTE]  
    > 자동화된 테스트를 만들고 관리하는 방법을 제어하는 특정 팀 프로세스가 있을 수 있습니다. 예를 들어 프로세스에 따라 코드를 해당 코드로 실행되는 테스트와 함께 체크 인하기 전에 빌드를 로컬로 확인해야 할 수 있습니다.  
  
    **솔루션 탐색기**에서 체크 인되었음을 나타내는 자물쇠 아이콘이 각 파일 옆에 나타납니다. 자세한 내용은 [버전 제어 파일 및 폴더 속성 보기](http://msdn.microsoft.com/library/ms245468(VS.100).aspx)를 참조하세요.  
...
3.  **빌드 정의**, **빌드 에이전트** 및 **이 빌드의 저장 폴더** 필드의 값이 모두 적합한지 확인한 후 **큐**를 클릭합니다.  
    ...
    **빌드 탐색기**의 **큐 대기** 탭이 표시됩니다. 자세한 내용은 [완료된 빌드 관리 및 보기(Visual Studio 2010)](http://msdn.microsoft.com/library/ms181730(VS.100).aspx) 또는 [빌드 탐색기에서 빌드 관리(Visual Studio 2012)](http://msdn.microsoft.com/library/ms181732.aspx)를 참조하세요.  
  
## <a name="see-also"></a>참고 항목  
[SQL Server 단위 테스트 실행](../ssdt/running-sql-server-unit-tests.md)  
[기본 빌드 정의 만들기](http://msdn.microsoft.com/library/ms181716(VS.100).aspx)  
[큐에 빌드 대기](http://msdn.microsoft.com/library/ms181722(VS.100).aspx)  
[실행 중인 빌드의 진행률 모니터링](http://msdn.microsoft.com/library/ms181724(VS.100).aspx)  
