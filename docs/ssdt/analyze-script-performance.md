...
또한 응용 프로그램이 데이터베이스에 전송하는 응용 프로그램 쿼리 및 업데이트를 분석하고 이러한 쿼리 및 업데이트가 데이터베이스에 포함된 데이터 및 데이터베이스 스키마와 상호 작용하는 방식을 분석하면 일부 성능 문제를 해결하는 데 큰 도움이 됩니다. 실행 계획에서는 SQL Server 쿼리 최적화 프로그램에서 선택한 데이터 검색 방법을 그래픽으로 표시하고 특정 문 및 쿼리의 실행 비용을 보여 줍니다. 따라서 이를 통해 SQL Server에서 SQL 쿼리를 처리하는 방식을 이해하고 성능 저하를 가져오는 원인을 쉽게 확인할 수 있습니다.  
  
## <a name="using-client-statistics"></a>클라이언트 통계 사용  
Transact\-SQL 편집기에서 스크립트나 쿼리를 실행하는 경우 해당 실행에 대해 응용 프로그램 프로필, 네트워크 및 시간 통계 등의 클라이언트 통계를 수집하도록 선택할 수 있습니다. 이러한 메트릭을 통해 스크립트의 효율성을 측정하거나 다른 스크립트를 벤치마킹할 수 있습니다.  
...
수집된 통계에 자세한 내용은 [쿼리 창 통계 창](http://msdn.microsoft.com/en-us/library/aa216969(SQL.80).aspx) 및 [이 항목의 “클라이언트 통계 탭” 섹션](http://msdn.microsoft.com/en-us/library/aa833205.aspx)을 참조하세요.  
...
그래픽 실행 계획 출력은 오른쪽에서 왼쪽으로, 위에서 아래로 읽습니다. 분석되는 일괄 처리의 각 쿼리가 표시되며, 일괄 처리의 총 비용 중 각 쿼리 비용이 차지하는 백분율도 표시됩니다. 각 단계의 비용 및 작업과 같은 추가 정보를 보려면 마우스를 그래픽 계획의 [논리 및 물리 연산자 아이콘](http://msdn.microsoft.com/en-us/library/ms175913.aspx) 위에 놓습니다.  
...
코드 분석에 대한 자세한 내용은 [데이터베이스 코드를 분석하여 코드 품질 향상](http://msdn.microsoft.com/en-us/library/dd172133.aspx)을 참조하십시오.  
