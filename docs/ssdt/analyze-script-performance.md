...
수집된 통계에 자세한 내용은 [쿼리 창 통계 창](https://msdn.microsoft.com/library/aa216969(SQL.80).aspx) 및 [이 항목의 "클라이언트 통계 탭" 섹션](/previous-versions/visualstudio/visual-studio-2010/aa833205(v=vs.100))을 참조하세요.  
...
그래픽 실행 계획 출력은 오른쪽에서 왼쪽으로, 위에서 아래로 읽습니다. 분석되는 일괄 처리의 각 쿼리가 표시되며, 일괄 처리의 총 비용 중 각 쿼리 비용이 차지하는 백분율도 표시됩니다. 각 단계의 비용 및 작업과 같은 추가 정보를 보려면 마우스를 그래픽 계획의 [논리 및 물리 연산자 아이콘](/previous-versions/sql/sql-server-2008-r2/ms175913(v=sql.105)) 위에 놓습니다.  
...
코드 분석에 대한 자세한 내용은 [데이터베이스 코드를 분석하여 코드 품질 향상](/previous-versions/visualstudio/visual-studio-2010/dd172133(v=vs.100))을 참조하세요.  
