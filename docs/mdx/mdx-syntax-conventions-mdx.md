...
|\<label> ::=|구문 블록의 이름을 나타냅니다. 이 규칙은 문에서 한번 이상 사용될 수 있는 긴 구문의 일부 또는 구문 단위를 그룹화하고 레이블을 붙일 때 사용됩니다. 구문 블록이 사용 될 수 있는 각 위치는 꺾쇠 괄호로 묶여 있는 레이블로 표시 됩니다: \<레이블 >.|  
