---
description: MDX 문 참조(MDX)
title: MDX 문 참조 (MDX) | Microsoft Docs
ms.date: 06/04/2018
ms.prod: sql
ms.technology: analysis-services
ms.custom: mdx
ms.topic: reference
ms.author: owend
ms.reviewer: owend
author: minewiskan
ms.openlocfilehash: bc16426b29deda2419f861a5456a2807f37648b8
ms.sourcegitcommit: e700497f962e4c2274df16d9e651059b42ff1a10
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/17/2020
ms.locfileid: "88500506"
---
# <a name="mdx-statement-reference-mdx"></a>MDX 문 참조(MDX)


  MDX 문은 다음 표의 설명에서와 같이 3가지 그룹으로 구분됩니다.  
  
## <a name="in-this-section"></a>섹션 내용  
  
|항목|설명|  
|-----------|-----------------|  
|[MDX 스크립팅 문&#40;MDX&#41;](../mdx/mdx-scripting-statements-mdx.md)|MDX 스크립트 내의 쿼리 컨텍스트, 범위 및 제어 흐름을 관리하는 MDX 스크립팅 문에 대한 정보가 포함됩니다.|  
|[Mdx 데이터 정의 문은 MDX를 &#40;&#41;](../mdx/mdx-data-definition-statements-mdx.md)|다차원 개체를 만들고, 삭제하고, 조작하는 MDX 데이터 정의 문에 대한 정보가 포함됩니다.|  
|[Mdx 데이터 조작 문은 MDX를 &#40;&#41;](../mdx/mdx-data-manipulation-statements-mdx.md)|다차원 개체로부터 데이터를 검색하고 조작하는 MDX 데이터 조작 문에 대한 정보가 포함됩니다.|  
  
## <a name="see-also"></a>참고 항목  
 [MDX 언어 참조&#40;MDX&#41;](../mdx/mdx-language-reference-mdx.md)  
  
  
