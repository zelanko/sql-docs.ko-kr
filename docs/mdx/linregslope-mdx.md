...
ms.locfileid: "34741542"
---
# <a name="linregslope-mdx"></a>LinRegSlope(MDX)


  집합의 선형 회귀를 계산 하 고 회귀선의 기울기 값을 반환 y = ax + b 합니다.  
 ...
 **LinRegSlope** 함수는 y 축에 대 한 값을 가져오는 첫 번째 숫자 식에 대해 지정 된 집합을 계산 합니다. 그런 다음 두 번째 숫자 식(지정된 경우)에 대해 지정된 집합 식을 계산하여 X축 값을 구합니다. 두 번째 숫자 식이 지정되지 않은 경우 이 함수는 지정된 집합의 현재 셀 컨텍스트를 X축 값으로 사용합니다. X축 인수를 지정하지 않는 방식은 Time 차원에서 자주 사용됩니다.  
  
 포인트의 집합을 가져온 후의 **LinRegSlope** 함수는 회귀선의 기울기를 반환 합니다 (한 위 수식의).  
  
> [!NOTE]  
>  **LinRegSlope** 함수는 빈 셀 이나 텍스트 또는 논리 값을 포함 하는 셀은 무시 합니다. 그러나 값이 0인 셀은 포함시킵니다.  
