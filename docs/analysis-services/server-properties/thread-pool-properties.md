...
-   **ThreadPool \ Process** 풀은 집계, 인덱싱 및 커밋 작업을 포함하여 오래 걸리는 저장소 엔진 작업을 위한 풀입니다. ROLAP 스토리지 모드에서도 처리 스레드 풀의 스레드가 사용됩니다.  
 ...
 **PerNumaNode** 속성을 설정할지 여부를 고려할 때 **IOProcess** 스레드가 사용되는 방식을 알고 있으면 보다 합리적인 결정을 내릴 수 있습니다.  
 ...
 이전에 설명한 대로 **IOProcess** 풀은 다차원 엔진에서 스토리지 엔진 쿼리와 연관된 IO 작업에 사용됩니다.  
