---
title: "명명된 집합 정의 | Microsoft Docs"
ms.custom: ""
ms.date: "03/04/2017"
...
## Core Products 명명된 집합 정의  
  
1.  [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] Tutorial 큐브에 대한 큐브 디자이너의 **계산** 탭으로 전환한 후 도구 모음에서 **폼 보기**를 클릭합니다.  
...
9. 속성 창에서 **DiscretizationMethod** 속성을 **Automatic**으로 변경하고 **DiscretizationBucketCount** 속성을 **5**로 변경합니다. 자세한 내용은 [특성 멤버 그룹화&#40;불연속화&#41;](../analysis-services/multidimensional-models/group-attribute-members-discretization.md)를 참조하세요.  
...
13. **식** 상자의 Exists 집합 식에서 Members 함수 뒤, 닫는 괄호 앞에 쉼표를 추가한 후 **메타데이터** 창에서 **83 - 100**을 끌어다 쉼표 뒤에 놓습니다.  
    ...
    ![[대형 대리점]에 대한 계산 식 창](../analysis-services/media/l6-named-set-02.gif "[대형 대리점]에 대한 계산 식 창")  
...
5.  다음 그림에 표시된 것처럼 Product 차원을 확장한 다음 Category 및 Subcategory를 행 영역에 추가합니다.  
    ...
    ![하위 범주 특성의 구성원](../analysis-services/media/l6-named-set-03.gif "하위 범주 특성의 구성원")  
...
6.  **메타데이터** 창의 **Product** 차원에서 **Core Products**를 필터 영역으로 끌어옵니다.  
    ...
    **Category** 특성의 **Bike** 멤버와 **Bike** 하위 범주의 멤버만 큐브에 그대로 남아 있습니다. **Core Products** 명명된 집합이 하위 큐브를 정의하는 데 사용되기 때문입니다. 다음 그림에 표시된 것처럼 이 하위 큐브는 하위 큐브 내의 **Product** 차원에 포함된 **Category** 특성의 멤버를 **Core Product** 명명된 집합의 멤버로 제한합니다.  
    ...
    ![핵심 제품 명명된 집합의 구성원](../analysis-services/media/l6-named-set-04.gif "핵심 제품 명명된 집합의 구성원")  
...
7.  **메타데이터** 창에서 **Reseller**를 확장하고 필터 영역에 **Large Resellers**를 추가합니다.  
    ...
    ![명명된 집합을 2개 포함하는 필터 창](../analysis-services/media/l6-named-set-05.gif "명명된 집합을 2개 포함하는 필터 창")  
