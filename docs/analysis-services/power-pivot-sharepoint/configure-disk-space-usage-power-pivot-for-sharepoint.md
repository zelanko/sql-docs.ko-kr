---
title: "디스크 공간 사용 구성(SharePoint용 Power Pivot) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
---
# 디스크 공간 사용 구성(SharePoint용 Power Pivot)
...
## SharePoint용 Power Pivot에서 캐시된 데이터베이스를 관리하는 방법  
...
## 디스크 공간 사용량을 확인하는 방법  
 ...
 캐시에는 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 데이터베이스만 포함됩니다. [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 데이터베이스는 단일 부모 폴더(백업 폴더) 아래의 여러 파일에 저장됩니다. [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 데이터베이스가 Excel 통합 문서의 내부 데이터로 사용되기 때문에 데이터베이스 이름은 설명이 포함되지 않은 GUID 기반 이름입니다. **\<serviceApplicationName>** 아래의 GUID 폴더는 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 데이터베이스의 부모 폴더입니다. [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 데이터베이스가 서버에 로드되면 각 데이터베이스에 대한 추가 폴더가 만들어집니다.  
...
## 캐시된 파일의 저장에 사용되는 디스크 공간 크기를 제한하는 방법  
