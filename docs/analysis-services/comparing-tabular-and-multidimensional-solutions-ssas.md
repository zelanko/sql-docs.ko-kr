---
title: Analysis Services 테이블 형식 및 다차원 모델을 비교할 | Microsoft Docs
ms.date: 05/08/2018
ms.prod: sql
...
ms.locfileid: "53072547"
---
# <a name="comparing-tabular-and-multidimensional-solutions"></a>테이블 형식 및 다차원 솔루션 비교
[!INCLUDE[ssas-appliesto-sqlas](../includes/ssas-appliesto-sqlas.md)]

  SQL Server Analysis Services는 비즈니스 인텔리전스 의미 체계 모델을 만드는 여러 가지 방식: SharePoint에 대 한 테이블 형식, 다차원 및 파워 피벗입니다.
 ...
 테이블 형식 및 다차원 솔루션 SSDT를 사용 하 여 작성 되 고 독립 실행형에서 실행 되는 기업 BI 프로젝트에 사용 됩니다 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 온-프레미스 인스턴스를 생성 하 고 테이블 형식 모델에 대 한는 [Azure Analysis Services](https://azure.microsoft.com/services/analysis-services/) 서버에는 클라우드입니다. 두 솔루션 모두 BI 클라이언트와 쉽게 통합되는 고성능 분석 데이터베이스를 제공합니다. 하지만 각 솔루션은 구축,  사용 및 배포되는 방법이 다릅니다. 이 항목에서는 대부분 사용자에게 적합한 접근 방식을 식별할 수 있도록 이 두 가지 유형을 비교합니다.  
  
 새 프로젝트의 경우 일반적으로 테이블 형식 모델 권장 합니다. 테이블 형식 모델 디자인, 테스트 및 배포 속도가 더 빠르게 됩니다. 및 최신 셀프 서비스 BI 응용 프로그램을 더욱 효율적으로 사용 되며 클라우드 Microsoft에서 서비스 됩니다.  
  
##  <a name="bkmk_overview"></a> 모델링 유형에 대한 개요  
 Analysis Services를 처음 접하는 사용자를 위해 다음 표에서는 다양한 모델을 열거하고, 접근 방식을 요약하며, 초기 릴리스 수단을 식별합니다.  
 
 > [!NOTE]  
>  **Azure Analysis Services** 1200 이상 호환성 수준의 테이블 형식 모델을 지원 합니다. 그러나이 항목에서 설명 하는 모든 테이블 형식 모델링 기능 Azure Analysis Services에서 지원 됩니다. 만들기 및 Azure Analysis services 테이블 형식 모델 배포 상태인 동안 동일 온-프레미스에 대 한 차이점을 이해 하는 것이 반드시 합니다. 자세한 내용은를 참조 하세요. [Azure Analysis Services 란?](https://docs.microsoft.com/azure/analysis-services/analysis-services-overview)
...
|SQL Server 2016의 테이블 형식|관계형 모델링 구문 (모델, 테이블, 열)의 테이블 형식 메타 데이터 개체 정의에 명시 [TMSL Tabular Model Scripting Language ()](../analysis-services/tabular-model-scripting-language-tmsl-reference.md) 하 고 [개체 모델 TOM (테이블 형식)](../analysis-services/tabular-model-programming-compatibility-level-1200/introduction-to-the-tabular-object-model-tom-in-analysis-services-amo.md) 코드입니다.|SQL Server 2016(호환성 수준 1200)| 
|SQL Server 2017에서에서 테이블 형식|관계형 모델링 구문 (모델, 테이블, 열)의 테이블 형식 메타 데이터 개체 정의에 명시 [TMSL Tabular Model Scripting Language ()](../analysis-services/tabular-model-scripting-language-tmsl-reference.md) 하 고 [개체 모델 TOM (테이블 형식)](../analysis-services/tabular-model-programming-compatibility-level-1200/introduction-to-the-tabular-object-model-tom-in-analysis-services-amo.md) 코드입니다.|SQL Server 2017 (1400 호환성 수준)| 
...
|Power Pivot|원래는 추가 기능이었지만 지금은 Excel에 완전히 통합되어 있습니다. 내부 테이블 형식 인프라를 통해서만 지원되는 시각적 모델링입니다. PowerPivot 모델을 SSDT로 가져와 Analysis Services 인스턴스에서 실행되는 새 테이블 형식 모델을 만들 수 있습니다.|Excel 및 PowerPivot BI Desktop을 통해|  
 ...
 <sup>1</sup> 호환성 수준 테이블 형식 메타 데이터 엔진 및 시나리오에 대 한 지원으로 인해 현재 릴리스에서 중요 한 요소는 더 높은 수준 에서만 사용할 수 있는 기능입니다. 이상 버전에서는 이전 호환성 수준 지원 하지만 새 모델을 만들거나 기존 모델을 서버 버전에서 지 원하는 가장 높은 호환성 수준 업그레이드 것이 좋습니다.
...
|연결된 개체|예|예(연결된 테이블)|
|M 식|아니요|예<sup>1</sup>|
|다 대 다 관계|예|아니요 (하지만 [양방향 교차 필터](../analysis-services/tabular-models/bi-directional-cross-filters-tabular-models-analysis-services.md) 1200 이상 호환성 수준의)| 
|명명된 집합|예|아니요| 
|비정형 계층 구조|예|예<sup>1</sup>|  
|부모-자식 계층 구조|예|예(DAX를 통해)|
|파티션|사용자 계정 컨트롤|사용자 계정 컨트롤| 
|큐브 뷰|사용자 계정 컨트롤|사용자 계정 컨트롤|
|행 수준 보안|사용자 계정 컨트롤|예| 
|개체 수준 보안|예|예<sup>1</sup>|
|반가산적 측정값|사용자 계정 컨트롤|사용자 계정 컨트롤| 
...
|쓰기 저장(writeback)|예|아니요| 
 ...
 <sup>1</sup> 참조 [Compatibility Level for Tabular Analysis Services에서 모델](../analysis-services/tabular-models/compatibility-level-for-tabular-models-in-analysis-services.md) 호환성 수준 간의 기능적 차이점에 대 한 정보에 대 한 합니다.  
 ...
 **모델의 크기 및 리소스 편차(메모리 내 또는 디스크)**  
 ...
 테이블 형식 데이터베이스는 메모리 내에서 실행되거나, 외부 데이터베이스에 쿼리 실행을 오프로드하는 DirectQuery 모드에서 실행됩니다. 테이블 형식 메모리 내 분석에 대 한 데이터베이스는 데이터를 제외한 모든 또한 쿼리를 지원 하기 위해 생성 하는 추가적인 데이터 구조로 로드 뿐만 아니라 데 충분 한 메모리가 있어야 합니다. 즉 메모리에 완전히 저장 됩니다.  
  
 SQL Server 2016에서 수정 된 DirectQuery는 이전 보다 제한 적고 성능 향상에 있습니다. 스토리지 및 쿼리 실행에 백 엔드 관계형 데이터베이스를 활용하면 이전보다 더 적합하게 대규모 테이블 형식 모델을 구축할 수 있습니다.  
  
 지금까지 프로덕션 환경에서 가장 큰 데이터베이스는 해당 용도 맞게 최적화 각각 전용된 하드웨어에서 독립적으로 실행 하는 처리 및 쿼리 워크 로드를 사용 하 여 다차원입니다.  테이블 형식 데이터베이스가 이를 빠르게 따라잡고 있으며, DirectQuery의 새로운 발전이 격차를 더욱 해소하도록 도와줍니다.  
 ...
 **지원되는 데이터 원본**  
 ...
 테이블 형식 모델은 관계형 데이터 원본, 데이터 피드 및 일부 문서 형식에서 데이터를 가져올 수 있습니다. 또한 테이블 형식 모델을 사용 하 여 ODBC 공급자에 대 한 OLE DB를 사용할 수 있습니다. 1400 호환성 수준의 테이블 형식 모델 데이터 원본에서 가져올 수 있는 다양 한 크게 증가 제공 합니다. 소개는 최신 데이터 가져오기 데이터 쿼리 및 M 수식 쿼리 언어를 활용 하 여 SSDT에서 기능을 가져오기 때문입니다.   
...
-   테이블 형식 모델 데이터베이스는 DAX 계산, DAX 쿼리 및 MDX 쿼리를 지원합니다. 이는 모든 호환성 수준에서 적용됩니다. 스크립트 언어는 ASSL (XMLA)를 통해 호환성 수준 1050-1103 및 TMSL (XMLA)에 대 한 호환성 수준 1200 이상에 대 한 합니다. 
...
-   다차원 model 데이터베이스 차원 및 셀 수준의 보안, 역할 기반 권한을 사용 하 여 사용할 수 있습니다.  

-   [!INCLUDE[ssGemini](../includes/ssgemini-md.md)] 통합 문서는 SharePoint 사용 권한을 통해 파일 수준에서 보안이 유지됩니다.  
  
 [!INCLUDE[ssGemini](../includes/ssgemini-md.md)] 통합 문서는 테이블 형식 모드 서버에 복원될 수 있습니다. 파일 복원 되 면에 행 수준 보안을 포함 하 여 모든 테이블 형식 모델링 기능을 사용할 수 있도록 SharePoint에서 분리 됩니다.  
...
|모델링 도구|사용 방법|  
|-------------------|--------------|  
|[!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)]|사용 하 여 만드는 테이블 형식, 다차원 및 데이터 마이닝 솔루션입니다. 이 제작 환경에서는 Visual Studio 셸을 사용하여 작업 영역, 속성 창 및 개체 탐색을 제공합니다. Visual Studio를 이미 사용 중인 기술 사용자는 비즈니스 인텔리전스 애플리케이션을 생성할 때 대부분 이 도구를 선호합니다.|  
...
##  <a name="bkmk_client"></a> 클라이언트 응용 프로그램 지원  
 -General, 테이블 형식 및 다차원 솔루션 (MSOLAP, 방법은 AMOMD, ADOMD) Analysis Services 클라이언트 라이브러리 중 하나 이상을 사용 하 여 클라이언트 응용 프로그램을 지원 합니다. 예를 들어, Excel, Power BI Desktop 및 사용자 지정 응용 프로그램입니다.   
