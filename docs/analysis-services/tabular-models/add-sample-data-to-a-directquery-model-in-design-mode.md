...
---
# 디자인 모드에서 DirectQuery 모델에 샘플 데이터 추가
...
## 샘플 파티션 만들기
 이러한 지침은 호환성 수준 1200에서 만들었거나 이 수준으로 업그레이드한 테이블 형식 모델에 대한 것입니다. 더 낮은 호환성 수준의 모델은 다른 속성을 사용하여 캐시된 데이터를 가져옵니다. 속성 설명은 [SSMS에서 DirectQuery 모드 사용](../../analysis-services/tabular-models/enable-directquery-mode-in-ssms.md)을 참조하세요.  
...
2.  **테이블** > **속성**을 클릭하여 파티션 관리 대화 상자를 엽니다.  
    ...
    기본 파티션은 **(Direct Query) \<table name>**입니다. 전체 데이터 뷰입니다. 이 파티션을 삭제하지 마세요. 이 파티션은 모델을 배포할 때 사용됩니다.  
