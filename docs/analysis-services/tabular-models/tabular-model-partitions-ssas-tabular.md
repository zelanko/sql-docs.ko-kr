...
##  <a name="bkmk_parallelProc"></a> 병렬 처리  
 [!INCLUDE[ssASCurrent](../../includes/ssascurrent-md.md)] 에는 파티션이 두 개 이상 포함된 테이블에 대한 병렬 처리가 포함되어 처리 성능이 향상됩니다. 병렬 처리에 대한 구성 설정은 없습니다(참고 참조). 병렬 처리는 테이블을 처리하거나 동일한 테이블 및 프로세스에 대해 여러 파티션을 선택할 때 기본적으로 발생합니다. 테이블의 파티션을 독립적으로 처리하도록 선택할 수 있습니다.  
  
> [!NOTE]  
>  새로 고침 작업을 순서대로 실행할지 또는 병렬로 실행할지를 지정하려면 **maxParallism** 속성 옵션을 [Sequence 명령(TMSL)](../../analysis-services/tabular-models-scripting-language-commands/sequence-command-tmsl.md)과 함께 사용할 수 있습니다. TMSL은 테이블 형식 1200 모델에서만 지원됩니다.
...
##  <a name="bkmk_process_partitions"></a> 파티션 처리  
 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]의 **파티션** 대화 상자 또는 스크립트를 사용하여 파티션을 다른 파티션과 별개로 처리(새로 고침)할 수 있습니다. 처리 옵션은 다음과 같습니다.  
