...
# 테이블 형식 모델(SSAS)
 ...
 DirectQuery는 모델이 너무 커서 메모리 내에 포함할 수 없거나 데이터 휘발성으로 인해 적절한 처리 전략을 적용할 수 없는 경우 대신 사용 가능한 쿼리 모드입니다. 이번 릴리스에서는 추가 데이터 원본이 지원되고, DirectQuery 모델의 계산된 테이블 및 열을 처리하는 기능이 제공되며, 백 엔드 데이터베이스로 전송되는 DAX 식을 통한 행 수준 보안이 보장되고, 쿼리 최적화로 인해 이전 버전보다 처리량이 더 높아져 DirectQuery가 메모리 내 모델과 더욱 근접한 기능을 제공합니다. 최신 향상 기능을 사용하려면 이번 릴리스에 도입된 호환성 수준 1200을 설정해야 합니다.  
...
## 관련 항목:  
 ...
 [Analysis Services에서 사용되는 도구 및 응용 프로그램](../../analysis-services/tools-and-applications-used-in-analysis-services.md)   
