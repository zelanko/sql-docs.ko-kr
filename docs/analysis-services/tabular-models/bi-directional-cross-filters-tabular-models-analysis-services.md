---
title: "SQL Server 2016 Analysis Services의 테이블 형식 모델에 대한 양방향 교차 필터 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/07/2017"
...
---
# SQL Server 2016 Analysis Services의 테이블 형식 모델에 대한 양방향 교차 필터
 ...
 SSDT에서는 테이블 관계 대화 상자의 필터 방향 옵션을 통해 교차 필터를 설정합니다. 이 기능은 [Visual Studio 2015용 SSDT(SQL Server Data Tools)](http://go.microsoft.com/fwlink/p/?LinkID=627574)의 1200 호환성 수준에서 만든 테이블 형식 모델에만 사용할 수 있습니다.  
  
 교차 필터에는 단방향 필터링과 양방향 필터링의 두 가지 유형이 있습니다. 단방향은 해당 관계에서 팩트 테이블과 차원 테이블 간의 기존 다 대 일 필터 방향입니다. 양방향은 두 관계 모두에 공통적인 하나의 테이블을 사용하여 하나의 관계에 대한 필터 컨텍스트를 다른 테이블 관계에 대한 필터 컨텍스트로 사용할 수 있도록 해주는 교차 필터입니다.  
...
4.  모델 호환성 수준이 **SQL Server 2016 RTM(1200)**으로 설정되어 있는지 확인합니다.  
...
## 관련 항목:  
 ...
 [PowerPivot과 SSAS 테이블 형식 모델 간의 간단한 다 대 다 관계를 처리하는 방법에 대한 실제 예제](http://social.technet.microsoft.com/wiki/contents/articles/22202.a-practical-example-of-how-to-handle-simple-many-to-many-relationships-in-power-pivotssas-tabular-models.aspx)   
