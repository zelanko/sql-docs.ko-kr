...
|속성|기본 설정|Description|  
|--------------|---------------------|-----------------|  
|**인코딩 힌트**|Default|처리를 최적화 하기 위해 인코딩을 지정 합니다. 값 인코딩을 집계에 일반적으로 사용 되는 숫자 열에 대 한 쿼리 성능을 향상 시킬 수 있습니다. 해시 인코딩을 group by 열 (대개 차원 테이블 값) 및 외래 키입니다. 문자열 열은 항상 인코딩된 해시입니다.|  
