---
title: "SSMS에서 DirectQuery 모드를 사용하도록 설정 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/01/2017"
...
# SSMS에서 DirectQuery 모드를 사용하도록 설정
 ...
 이 항목에서는 호환성 수준 1200에서 메모리 내 테이블 형식 모델을 만들고 유효성을 검사했으며 DirectQuery 액세스를 사용하도록 설정하고 연결 문자열을 업데이트만 하면 되는 것으로 가정합니다. 낮은 호환성 수준에서 시작하는 경우 먼저 수동으로 업그레이드해야 합니다. 단계는 [Analysis Services 업그레이드](../../database-engine/install-windows/upgrade-analysis-services.md)를 참조하세요.  
...
-   모델에 DirectQuery 모드에서 유효성 검사 오류를 일으킬 수 있는 기능이 없는지 확인한 다음, 모델에 대한 데이터 저장소 모드를 메모리 내에서 DirectQuery로 변경합니다.  
     ...
     기능 제한 사항 목록은 [DirectQuery 모드&#40;SSAS 테이블 형식&#41;](../../analysis-services/tabular-models/directquery-mode-ssas-tabular.md)에 소개되어 있습니다.  
...
## 2a단계: 테이블 형식 1200 데이터베이스를 DirectQuery 모드로 전환  
...
## 2b단계: 테이블 형식 1100-1103 데이터베이스를 DirectQuery 모드로 전환  
...
-   DirectQuery 모드에 사용된 기본 파티션이 절대 처리되지 않는 방식으로 캐시에 파티션을 구성할 수 있으며 항상 관계형 원본을 참조해야 합니다. 여러 가지 방법으로 파티션을 사용하여 모델 디자인 및 보고 환경을 최적화할 수 있습니다. 자세한 내용은 [DirectQuery 모델에서 파티션 정의&#40;SSAS 테이블 형식&#41;](../../analysis-services/tabular-models/define-partitions-in-directquery-models-ssas-tabular.md)를 참조하세요.  
  
-   모델을 배포한 후 기본적으로 사용되는 연결 방법을 변경할 수 있습니다. 예를 들어 모델을 사용하는 보고서나 쿼리를 모두 철저히 테스트한 후에만 테스트에 혼합 모드를 사용하고 모델을 **DirectQuery 전용** 모드로 전환할 수 있습니다. 자세한 내용은 [DirectQuery의 기본 연결 방법 설정 또는 변경](../Topic/Set%20or%20Change%20the%20Preferred%20Connection%20Method%20for%20DirectQuery.md)을 참조하세요.  
  
## 3단계: 데이터베이스에서 연결 속성 확인  
...
1.  개체 탐색기에서 **연결**을 확장한 후 연결을 두 번 클릭하여 속성을 봅니다.  
     ...
     DirectQuery 모델의 경우 데이터베이스에 대해 단 하나의 연결만 정의되어 있어야 하며, 데이터 원본은 관계형이고 지원되는 데이터베이스 형식이어야 합니다. [지원되는 데이터 원본&#40;SSAS 테이블 형식&#41;](../../analysis-services/tabular-models/data-sources-supported-ssas-tabular.md)을 참조하세요.  
