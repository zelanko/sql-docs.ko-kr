---
title: Analysis Services 테이블 형식 모델에서 지 원하는 데이터 형식 | Microsoft Docs
ms.date: 05/07/2018
ms.prod: sql
...
-   두 숫자를 연결 하려고 하면 문자열로 표시 되 고 연결 합니다. 다음 식은 "1234": = 12 & 34를 반환합니다.  
...
### <a name="table-of-implicit-data-conversions"></a>암시적 데이터 변환 테이블  
...
> [!NOTE]  
>  텍스트 데이터 형식은 이 표에 포함되어 있지 않습니다. 여러 경우에 따라 텍스트 형식으로 표현 될 때 모델 디자이너 숫자 형식을 결정 하 고 숫자로 표시 하려고 합니다.  
...
#### <a name="subtraction--"></a>빼기(-)  
 다음 표에서 행 머리글은 피 감수 (왼쪽) 및 열 머리글은 감수 (오른쪽):  
