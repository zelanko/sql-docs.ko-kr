---
title: "테이블 형식 모델 번역(Analysis Services ) | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/01/2017"
...
---
# 테이블 형식 모델 번역(Analysis Services )
  [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 은(는) 1200 호환성 수준에서 생성된 테이블 형식 모델에 문자열 번역 지원을 추가합니다. 모델 내의 단일 개체는 이름 또는 설명이 여러 개로 번역되어 모델 정의 내에서 다중 언어 버전을 지원할 수 있습니다.  
...
3.  선택 사항으로 가져오기 옵션을 지정합니다.  
    ...
    |로그 파일에 가져오기 결과 쓰기|로그 파일은 기본적으로 프로젝트 폴더에 저장됩니다. 가져오기를 완료한 후에 정확한 파일 경로를 입력합니다. 로그 파일 이름은 SSDT_Translations_Log_\<timestamp>입니다.|  
