...
-   **동적 디자인 및 배포** 디자인 및 배포 후 즉시 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 개인 설정 확장 프로그램 사용자에 해당 개체와 기능에 대 한 액세스는 다음 번 사용자 세션 시작 합니다.  
  
-   **인터페이스 독립성** 만드는 데 사용할 수 있는 인터페이스에 관계 없이 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 개인 설정 확장 프로그램 사용자 개체와 기능에 액세스할 수 있는 인터페이스를 사용할 수 있습니다.  
  
-   **세션 컨텍스트** [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 개인 설정 확장 프로그램은 기존 인프라에서 영구적인 개체가 및 큐브를 다시 처리할 필요가 없습니다. 이러한 프로그램은 사용자가 데이터베이스에 연결하는 시점에 사용자에 대해 노출 및 생성되며 해당 사용자 세션 동안 사용 가능한 상태로 유지됩니다.  
  
-   **신속한 배포** 공유 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 이 찾는 방법 또는 위치에 대 한 세부적인된 사양을 연구할 필요 없이 다른 소프트웨어 개발자를 사용 하 여 개인 설정 확장 프로그램 기능을 확장 합니다.  
 ...
 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 개인 설정 확장 프로그램은 기존 관리 어셈블리 아키텍처를 간단 하 고 세련 된 수정 및 통해 노출 되는 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] <xref:Microsoft.AnalysisServices.AdomdServer> 모델, MDX (Multidimensional Expressions) 구문 및 스키마 행 집합 개체입니다.  
...
>  [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)]는 사용자 지정 특성을 정의하여 코드를 설명하고 런타임 동작에 영향을 미칩니다. 자세한 내용은 항목을 참조 하세요. "[특성 개요](http://go.microsoft.com/fwlink/?LinkId=82929),"에 [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)] MSDN의 개발자 가이드.  
 ...
 이벤트  
 개체를 만드는 세션 이벤트에 의해 트리거됩니다 **에-큐브-OpenedCubeOpened** 하 고 **에서-큐브-ClosingCubeClosing**합니다.  
...
### <a name="mdx-extensions-and-enhancements"></a>MDX 확장 및 향상된 기능  
 CREATE MEMBER 명령은 향상 되었습니다를 **캡션** 속성을 **display_folder** 속성 및 **associated_measure_group** 속성입니다.  
  
 계산 중 업데이트가 필요하고 이로 인해 우선 순위의 손실이 발생할 때 멤버 다시 만들기를 방지하기 위해 UPDATE MEMBER 명령이 추가되었습니다. 계산된 멤버를 다른 부모로 이동 업데이트에서는 계산된 멤버의 범위를 변경할 수 없습니다 또는 다른 정의할 **solveorder**합니다.  
  
 CREATE SET 명령은 향상 되었습니다 합니다 **캡션** 속성을 **display_folder** 속성을 새 **정적 | 동적** 키워드입니다. *정적* 집합이 생성 시에만 계산 되는 것을 의미 합니다. *동적* 집합은 쿼리에서 사용 될 때마다 계산을 의미 합니다. 기본값은 **정적** 키워드를 생략 합니다.  
...
### <a name="schema-rowsets-extensions"></a>스키마 행 집합 확장  
 MDSCHEMA_MEMBERS에 *범위* 열이 추가 됩니다. 범위 값은 다음과 같습니다. MDMEMBER_SCOPE_GLOBAL=1, MDMEMBER_SCOPE_SESSION=2.  
  
 Mdschema_sets *set_evaluation_context* 열이 추가 됩니다. 값은 계산 컨텍스트를 설정 합니다. MDSET_RESOLUTION_STATIC = 1, MDSET_RESOLUTION_DYNAMIC = 2.  
