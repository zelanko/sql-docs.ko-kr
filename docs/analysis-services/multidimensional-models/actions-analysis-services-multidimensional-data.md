---
title: "동작(Analysis Services - 다차원 데이터) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "동작 [Analysis Services]"
  - "동작 [Analysis Services], 동작 정보"
  - "MDX [Analysis Services], 동작"
  - "큐브 [Analysis Services], 동작"
  - "OLAP 개체 [Analysis Services], 동작"
...
-   동작이 발생하는 큐브에서 선택된 섹션과 연관된 동작 요소(URL, HTML, DataSet, RowSet 및 기타 요소)를 반환하는 표준 동작  
 ...
 유형은 실행할 동작의 종류입니다. <xref:Microsoft.AnalysisServices.Action>은 추상 클래스이므로 이 동작을 실행하려면 파생 클래스 중 하나를 사용해야 합니다. 두 가지 동작, 드릴스루와 보고는 미리 정의되어 있는데 해당 파생 클래스로 각각 <xref:Microsoft.AnalysisServices.DrillThroughAction> 및 <xref:Microsoft.AnalysisServices.ReportAction>이 있습니다. 나머지 동작은 <xref:Microsoft.AnalysisServices.StandardAction> 클래스에서 처리됩니다.  
