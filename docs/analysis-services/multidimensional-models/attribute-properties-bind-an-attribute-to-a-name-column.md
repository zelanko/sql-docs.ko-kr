---
title: "이름 열에 특성 바인딩 | Microsoft Docs"
ms.custom: 
ms.date: 03/01/2017
ms.prod: analysis-services
ms.prod_service: analysis-services
ms.service: 
ms.component: 
ms.reviewer: 
ms.suite: pro-bi
ms.technology:
- analysis-services
- analysis-services/multidimensional-tabular
- analysis-services/data-mining
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- binding attributes [Analysis Services]
- name columns [Analysis Services]
- attributes [Analysis Services], binding
ms.assetid: 467f0cf3-8691-476d-a7fb-a5df4e374eaf
caps.latest.revision: "29"
author: Minewiskan
ms.author: owend
manager: kfile
ms.workload: Inactive
ms.openlocfilehash: 819c027aa1adfbfae7d6057e988b1d93327233f4
ms.sourcegitcommit: f1a6944f95dd015d3774a25c14a919421b09151b
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/08/2017
---
# <a name="attribute-properties---bind-an-attribute-to-a-name-column"></a>특성 속성-이름 열에 특성 바인딩
[!INCLUDE[ssas-appliesto-sqlas](../../includes/ssas-appliesto-sqlas.md)]이 절차를 사용 하 여 수동으로 이름 열에 특성을 바인딩하는 방법을 설명는 **특성** 차원 디자이너에서을 사용 하 여 창 고 **개체 바인딩** 대화 상자.  
  
### <a name="to-bind-an-attribute-to-a-name-column"></a>이름 열에 특성을 바인딩하려면  
  
1.  차원 디자이너에서 특성을 만들려는 차원을 엽니다.  
  
2.  **차원 구조** 탭의 **특성** 창에서 구성할 특성을 마우스 오른쪽 단추로 클릭한 다음 **속성**을 클릭합니다.  
  
3.  **속성** 창에서 **NameColumn** 속성을 찾은 다음 **(새로 만들기)**를 선택합니다.  
  
4.  **개체 바인딩** 대화 상자에서 **바인딩 유형**으로 **열 바인딩**을 선택합니다.  
  
5.  **원본 열** 목록에서 특성이 바인딩될 열을 선택한 다음 **확인**을 클릭합니다.  
  
  
