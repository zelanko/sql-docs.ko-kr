---
title: "Usage 속성 설정 | Microsoft Docs"
ms.custom: 
ms.date: 03/13/2017
ms.prod: analysis-services
ms.prod_service: analysis-services
ms.service: 
ms.component: 
ms.reviewer: 
ms.suite: pro-bi
ms.technology:
- analysis-services
- analysis-services/multidimensional-tabular
- analysis-services/data-mining
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- attributes [Analysis Services], usage setting
- usage options [Analysis Services]
ms.assetid: 7b0ebc58-94b9-4523-8994-e7bc796b0bd8
caps.latest.revision: "35"
author: Minewiskan
ms.author: owend
manager: kfile
ms.workload: Inactive
ms.openlocfilehash: 78e7b70874a86c08821dd3398b364d9fcf678b41
ms.sourcegitcommit: f1a6944f95dd015d3774a25c14a919421b09151b
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/08/2017
---
# <a name="attribute-properties---set-usage-property"></a>특성 속성-사용법 속성 설정
[!INCLUDE[ssas-appliesto-sqlas](../../includes/ssas-appliesto-sqlas.md)]사용 하 여 특성 사용법을 설정할 수 있습니다는 **차원 구조** 보기에 액세스 하는 차원 디자이너에서에서 [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)]합니다.  
  
 특성 사용법을 설정할 때 변경 내용은 차원을 처리할 때까지 반영되지 않습니다. 자세한 내용은 [다차원 모델 처리&#40;Analysis Services&#41;](../../analysis-services/multidimensional-models/processing-a-multidimensional-model-analysis-services.md)를 참조하세요.  
  
### <a name="to-set-usage-for-an-attribute"></a>특성 사용법을 설정하려면  
  
1.  솔루션 탐색기에서 차원을 마우스 오른쪽 단추로 클릭한 다음 **열기**를 클릭합니다.  
  
     기본적으로 **차원 구조** 뷰가 열립니다.  
  
2.  **특성**에서 사용법을 설정할 특성을 마우스 오른쪽 단추로 클릭하고 **특성 사용법 설정**을 가리킨 다음 아래 옵션 중 하나를 클릭합니다.  
  
    -   **Regular**  
  
    -   **Key**  
  
    -   **Parent**  
  
## <a name="see-also"></a>관련 항목:  
 [특성 및 특성 계층](../../analysis-services/multidimensional-models-olap-logical-dimension-objects/attributes-and-attribute-hierarchies.md)   
 [차원에 특성 추가](../../analysis-services/multidimensional-models/attribute-properties-add-an-attribute-to-a-dimension.md)  
  
  
