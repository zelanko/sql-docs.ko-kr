...
ms.topic: "article"
helpviewer_keywords: 
  - "데이터 원본 뷰 [Analysis Services], 바인딩"
  - "DSO, 바인딩"
  - "Analysis Services Scripting Language, 데이터 원본"
  - "큐브 [Analysis Services], 바인딩"
  - "OLAP 마이닝 모델 [Analysis Services Scripting Language]"
  - "바인딩 [Analysis Services Scripting Language]"
  - "다시 바인딩 [Analysis Services Scripting Language]"
  - "ASSL, 바인딩"
  - "관계형 마이닝 모델 [ASSL]"
  - "데이터 원본 [Analysis Services Scripting Language]"
  - "ASSL, 데이터 원본"
  - "차원 [Analysis Services], 바인딩"
  - "측정값 [Analysis Services], 바인딩"
  - "관계형 데이터 원본 [Analysis Services Scripting Language]"
  - "Analysis Services Scripting Language, 바인딩"
  - "장으로 구성된 행 집합"
  - "세분성"
  - "마이닝 모델 [Analysis Services], 데이터 원본"
  - "인라인 바인딩 [ASSL]"
  - "아웃오브 라인 바인딩"
  - "측정값 그룹 [Analysis Services], 바인딩"
  - "파티션 [Analysis Services], 바인딩"
...
## MeasureGroups, Measures 및 Partitions에 대한 바인딩  
...
### 아웃오브 라인 바인딩 유형 정의  
 확장 **Bindings** 컬렉션 내에서 ASSL은 **Binding**당 여러 개체에 대한 바인딩 컬렉션을 허용합니다. 각 **Bindings**에는 확장 개체 참조가 있습니다. 확장 개체 참조는 개체 참조와 비슷하지만 보조 개체(예: 차원 특성 및 측정값 그룹 특성)도 참조할 수 있습니다. 이 개체는 \<*Object*>\<*/Object*> 태그가 없다는 점을 제외하면 **Process** 명령의 **Object** 요소에 일반적인 플랫 형식을 취합니다.  
  
 바인딩이 지정되는 각 개체는 \<*object*>ID 형식의 XML 요소로 식별됩니다(예: **DimensionID**). \<*object*>ID 형식으로 개체를 가능한 한 명확히 식별한 다음에는 바인딩이 지정되는 요소를 식별합니다. 이는 일반적으로 **Source**입니다. 한 가지 일반적인 경우는 **Source** 가 **DataItem**의 속성인 경우로, 특성의 열 바인딩이 이에 해당합니다. 이 경우에는 **DataItem** 태그를 지정하지 않고 바인딩할 열에 바로 있는 것처럼 **Source** 속성만 지정합니다.  
