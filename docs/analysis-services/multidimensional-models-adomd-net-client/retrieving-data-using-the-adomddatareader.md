...
2.  **데이터를 검색 합니다.**  
     ...
     명령은 쿼리를 실행 하면 ADOMD.NET에서 결과 반환 합니다 **Resultset** 형식 테이블 형식에 대 한 데이터를 평면화 할 XML for Analysis 사양에에서 설명 된 대로 <xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader> 개체입니다. 테이블 형식은 분석 데이터의 일정하지 않은 차원 때문에 분석 데이터 쿼리에는 잘 사용되지 않습니다.  
    ...
    -   <xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader.Read%2A> 개체의 <xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader> 메서드를 호출합니다.  
         ...
         <xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader.Read%2A> 메서드는 쿼리 결과에서 행을 가져옵니다. 이름 또는 열의 서 수 참조를 전달할 수 있습니다 합니다 [항목](https://msdn.microsoft.com/library/ms131793(v=sql.130).aspx) 반환된 된 행의 각 열에 액세스 하는 속성입니다. 예를 들어 현재 행의 첫째 열 이름이 ColumnName인 경우 `reader[0].ToString()` 또는 `reader["ColumnName"].ToString()`은 현재 행의 첫째 열에 있는 내용을 반환합니다.  
...
## <a name="see-also"></a>관련 항목  
 ...
 [CellSet을 사용 하 여 데이터를 검색 합니다.](../../analysis-services/multidimensional-models-adomd-net-client/retrieving-data-using-the-cellset.md)   
