---
title: "연결 규칙 모델에서 항목 집합 필터링 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
ms.prod: "sql-server-2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "analysis-services"
  - "analysis-services/data-mining"
ms.tgt_pltfrm: ""
ms.topic: "article"
helpviewer_keywords: 
  - "항목 집합 [Analysis Services]"
  - "항목 집합 필터링 [Analysis Services]"
  - "마이닝 모델 뷰어 [Analysis Services], 항목 집합"
ms.assetid: 3ed919ea-8598-45d2-a4a0-b1b3357a4ab1
caps.latest.revision: 27
author: "Minewiskan"
ms.author: "owend"
manager: "jhubbard"
caps.handback.revision: 27
---
# 연결 규칙 모델에서 항목 집합 필터링
   [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]에서 **연결 규칙 뷰어의** 항목 집합 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 탭에 표시되는 항목 집합을 필터링할 수 있습니다.  
  
### 항목 집합을 필터링하려면  
  
1.  **의 데이터 마이닝 디자이너에 있는** 마이닝 모델 뷰어 [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)]탭에서 **Microsoft 연결 규칙 뷰어** 의 **항목 집합**탭을 클릭합니다.  
  
2.  **항목 집합 필터** 상자에 규칙 조건을 입력합니다. 예를 들어 "Touring-1000 = existing"을 규칙 조건으로 사용할 수 있습니다.  
  
3.  **입력**을 클릭합니다.  
  
 이렇게 하면 항목 집합이 필터링되어 선택한 항목을 포함하는 항목 집합만 표시됩니다. 상자는 대/소문자를 구분하지 않습니다. 목록에서 이전 필터를 선택할 수 있도록 필터가 메모리에 저장됩니다.  
  
## 관련 항목:  
 [마이닝 모델 뷰어 태스크 및 방법](../../analysis-services/data-mining/mining-model-viewer-tasks-and-how-tos.md)  
  
  