...
|12|주기성|값이 모델의 주기 구조를 나타냄을 가리킵니다.<br /><br /> ARIMA 모델이 포함된 시계열 모델에만 적용됩니다.<br /><br /> 참고: Microsoft 시계열 알고리즘은 학습 데이터를 기반으로 주기 구조를 자동으로 검색 합니다. 따라서 최종 모델의 예측에는 모델을 만들 때 매개 변수로 제공하지 않은 주기성 값이 포함될 수 있습니다.|  
...
-   **모델 콘텐츠 항목:** 알고리즘 유형별로 각 노드의 의미를 설명 하 고 특정 모델 유형에 가장 관심 있는 노드에 대 한 지침을 제공 합니다.  
  
-   **쿼리 항목:** 결과 해석 하는 방법에 지침을 특정 모델 유형에 대 한 쿼리 예제를 제공 합니다.  
