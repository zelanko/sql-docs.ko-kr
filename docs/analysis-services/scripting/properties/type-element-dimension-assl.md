...
|-----------|-----------------|  
|*일반*|차원이 일반 차원입니다.|  
|*Time*|차원이 시간 차원입니다.<br /><br /> 참고:이 값 차원이 시간 차원과 관련 된 기능을 지원함을 나타냅니다.|  
|*Geography*|차원이 지리적 특성을 포함합니다.|  
|*조직*|차원이 조직적 특성을 포함합니다.|  
|*BillOfMaterials*|차원이 제품 구성 정보(BOM) 특성을 포함합니다.|  
|*계정*|차원이 계정 관련 특성을 포함합니다.<br /><br /> 참고:이 값 차원이 계정 차원과 관련 된 기능을 지원함을 나타냅니다.|  
...
|*프로 모션*|차원이 홍보 행사 관련 특성을 포함합니다.|  
 ...
 부모에 해당 하는 요소가 **형식** Analysis Management Objects (AMO) 개체 모델은 <xref:Microsoft.AnalysisServices.Dimension>합니다.  
