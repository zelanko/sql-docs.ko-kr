...
|**메타 데이터**|**DBTYPE_WSTR**|아니오|CSDLBI 스키마에 따라 데이터 원본 및 그 속성의 XML 정의를 포함하는 문자열입니다.|  
