---
title: Analysis Services에서 통화 변환 | Microsoft Docs
ms.date: 05/09/2019
ms.prod: sql
...
ms.locfileid: "65527398"
---
# <a name="currency-conversions-in-analysis-services"></a>Analysis Services에서 통화 변환

[!INCLUDE[ssas-appliesto-sqlas-aas](../includes/ssas-appliesto-sqlas-aas.md)]

 Analysis Services MDX (Multidimensional Expressions) 스크립트에서 기능 조합을 사용 하 여 다중 통화를 지 원하는 큐브에 통화 환산 지원을 제공할.  
  
## <a name="currency-conversion-terminology"></a>통화 변환 용어  

 다음 용어는 통화 변환 기능을 설명 하기 위해 사용 됩니다.  
  
 **피벗 통화** -는 환율이 요율 측정값 그룹에 입력 되는 통화입니다.  
  
 **현지 통화** -는는 트랜잭션을 저장 하는 데 사용 되는 통화 측정값을 변환할 기반 합니다.  
  
현지 통화 중 하나로 식별할 수 있습니다.  
...
**보고 통화** -는 트랜잭션이 피벗 통화에서 변환 되는 통화입니다.  
  
> [!NOTE]  
>  다 대 일 통화 변환의 경우 피벗 통화와 보고 통화가 같습니다.  
  
 **통화 차원** -다음 설정으로 정의 된 데이터베이스 차원:  
...
**요율 측정값 그룹** -다음 설정으로 정의 된 큐브의 측정값 그룹:  
...
**보고 통화 차원** -차원 통화 변환을 정의한 후 비즈니스 인텔리전스 마법사에서 정의 된 해당 통화 변환에 대 한 보고 통화를 포함 하는입니다. 보고 통화 차원은 통화 차원의 주 차원 테이블에서 명명된 쿼리를 기반으로 합니다. 이 쿼리는 요율 측정값 그룹과 연결된 통화 차원이 기반으로 하는 데이터 원본 뷰에 정의되어 있습니다.  다음 설정으로 보고 통화 차원이 정의됩니다.   
...
### <a name="conversion-types"></a>변환 형식  
...
### <a name="currency-conversion-in-multidimensional-models-by-using-business-intelligence-wizard"></a>비즈니스 인텔리전스 마법사를 사용 하 여 다차원 모델에서 통화 변환  
