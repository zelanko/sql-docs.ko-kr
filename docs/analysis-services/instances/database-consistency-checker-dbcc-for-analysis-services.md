...
[!INCLUDE[ssas-appliesto-sqlas](../../includes/ssas-appliesto-sqlas.md)]DBCC는 Analysis Services 인스턴스에서 다차원 및 테이블 형식 데이터베이스에 대 한 주문형 데이터베이스 유효성 검사를 제공합니다. SSMS(SQL Server Management Studio)의 MDX 또는 XMLA 쿼리 창에서 DBCC를 실행하고 SSMS의 SQL Server Profiler 또는 xEvent 세션에서 DBCC 출력을 추적할 수 있습니다.  
...
4.  SQL Server Profiler에서 DBCC 활동은 DBCC의 이벤트 하위 클래스가 있는 **Command** 이벤트를 통해 나타납니다.  
     ...
     ![ssas dbcc-프로파일러 eventsubclass](../../analysis-services/instances/media/ssas-dbcc-profiler-eventsubclass.PNG "dbcc 프로파일러 eventsubclass ssas")  
