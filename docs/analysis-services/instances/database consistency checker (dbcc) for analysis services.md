---
title: "Analysis Services의 테이블 형식 및 다차원 데이터베이스에 대한 DBCC(데이터베이스 일관성 검사기) | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/04/2017"
...
manager: "erikre"
---
# Analysis Services의 테이블 형식 및 다차원 데이터베이스에 대한 DBCC(데이터베이스 일관성 검사기)
...
-   새 테이블 형식 모델 데이터베이스의 메타데이터는 호환성 수준 1200에서 **TableName** 및 **PartitionName**과 같은 설명자로 구성됩니다.  
...
## 테이블 형식 1200 데이터베이스에 대한 명령 구문  
 호환성 수준이 1200인 테이블 형식 데이터베이스는 개체 정의를 위해 테이블 형식 메타데이터를 사용합니다. 다음 예제에서는 SQL Server 2016 기능 수준에서 만든 테이블 형식 데이터베이스에 대한 전체 DBCC 구문을 보여 줍니다.  
  
 두 구문 간의 주요 차이점에는 최신 XMLA 네임스페이스, \<Object> 요소 없음, \<Model> 요소 없음 등이 있습니다(데이터베이스당 모델이 한 개만 있음).  
...
## 다차원 및 테이블 형식 110x 데이터베이스에 대한 명령 구문  
...
### Management Studio에서 DBCC 명령 실행  
...
### SQL Server Profiler 2016에서 DBCC 출력 추적  
...
4.  SQL Server Profiler에서 DBCC 활동은 DBCC의 이벤트 하위 클래스가 있는 **Command** 이벤트를 통해 나타납니다.  
     ...
     상태 메시지는 "\<object>의 일관성을 확인하는 중", "\<object> 확인 시작됨" 또는 "\<object> 확인 완료됨"으로 시작됩니다.  
  
    > [!NOTE]  
    >  CTP 3.0에서 개체는 내부 이름으로 식별됩니다. 예를 들어 Categories 계층은 H$Categories-\<objectID>로 명시됩니다. 향후 CTP에서 내부 이름은 사용자에게 친숙한 이름으로 바꿔야 합니다.  
  
     오류 메시지는 아래 표에 나열되어 있습니다.  
  
### SSMS의 xEvent 세션에서 DBCC 출력 추적  
...
## 참조: 테이블 형식 데이터베이스에 대한 일관성 검사 및 오류  
...
|DBCC 명령|DBCC 명령은 DBCC 작업을 통해 진행되므로 여러 개의 상태 메시지를 보고합니다.  시작하기 전에 개체의 데이터베이스, 테이블 또는 열 이름을 포함하는 상태 메시지를 보고하고, 각 개체 검사를 완료한 후에 다시 보고합니다.|\<objectname> \<objecttype>의 일관성을 검사하는 중입니다. 단계: 사전 검사<br /><br /> \<objectname> \<objecttype>의 일관성을 검사하는 중입니다. 단계: 사후 검사|  
  
## 오류 상태에 대한 일반적인 해결 방법  
...
|오류|문제점|해결 방법|  
|-----------|-----------|----------------|  
|**메타데이터 관리자에서 오류가 발생했습니다.**<br /><br /> 개체 참조 '\<objectID>'이(가) 잘못되었습니다. 메타데이터 클래스 계층 구조와 일치하지 않습니다.|형식이 잘못된 명령|명령 구문을 확인합니다. 부모 개체 중 하나 이상을 지정하지 않고 하위 수준 개체를 포함시킨 경우가 많습니다.|  
|**메타데이터 관리자에서 오류가 발생했습니다.**<br /><br /> ID가 '\<parentobjectID>'인 \<parentobject>에 ID가 '\<objectID>'인 \<object>가 없거나 사용자에게 개체 액세스 권한이 없습니다.|인덱스 손상(다차원)|개체와 모든 종속 개체를 다시 처리합니다.|  
|**파티션의 일관성 검사 중에 오류가 발생했습니다.**<br /><br /> \<데이터베이스-이름> 데이터베이스에서 \<큐브-이름>에 대해 \<측정값-그룹-이름> 측정값 그룹의 \<파티션-이름> 파티션 일관성을 검사하는 동안 오류가 발생했습니다. 파티션 또는 인덱스를 다시 처리하여 손상을 해결하세요.|인덱스 손상(다차원)|개체와 모든 종속 개체를 다시 처리합니다.|  
...
|**테이블 메타데이터 손상**<br /><br /> \<테이블-이름> 테이블의 메타데이터 파일이 손상되었습니다. 주 테이블을 DataFileList 노드 아래에서 찾을 수 없습니다.|메타데이터 손상(테이블 형식만)|프로젝트를 삭제하고 다시 배포하거나 백업에서 복원하여 다시 처리합니다.<br /><br /> 자세한 내용은 [How to handle corruption in Analysis Services databases (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)(Analysis Services 데이터베이스에서 손상을 처리하는 방법(블로그))를 참조하세요.|  
|**저장소 계층 손상**<br /><br /> 저장소 계층 손상: \<부모-이름> \<부모-형식>에서 \<형식-이름>의 컬렉션이 손상되었습니다.|메타데이터 손상(테이블 형식만)|프로젝트를 삭제하고 다시 배포하거나 백업에서 복원하여 다시 처리합니다.<br /><br /> 자세한 내용은 [How to handle corruption in Analysis Services databases (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)(Analysis Services 데이터베이스에서 손상을 처리하는 방법(블로그))를 참조하세요.|  
|**시스템 테이블이 없습니다.**<br /><br /> 시스템 테이블 \<테이블-이름>이 없습니다.|개체 손상(테이블 형식만)|개체와 모든 종속 개체를 다시 처리합니다.|  
|**테이블 통계가 손상되었습니다.**<br /><br /> 시스템 테이블 \<테이블-이름>의 통계가 없습니다.|메타데이터 손상(테이블 형식만)|프로젝트를 삭제하고 다시 배포하거나 백업에서 복원하여 다시 처리합니다.<br /><br /> 자세한 내용은 [How to handle corruption in Analysis Services databases (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)(Analysis Services 데이터베이스에서 손상을 처리하는 방법(블로그))를 참조하세요.|  
  
## msmdsrv.ini 구성 파일을 통한 데이터베이스 로드 작업에서 자동 일관성 검사를 사용하지 않도록 설정  
