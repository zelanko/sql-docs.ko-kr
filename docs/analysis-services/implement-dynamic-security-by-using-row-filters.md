---
title: "행 필터를 사용하여 동적 보안 구현 | Microsoft Docs"
ms.custom: ""
ms.date: "02/27/2017"
...
동적 보안을 구현하려면 두 개의 새 DAX 함수인 [USERNAME 함수(DAX)](http://msdn.microsoft.com/ko-kr/22dddc4b-1648-4c89-8c93-f1151162b93f) 및 [LOOKUPVALUE 함수(DAX)](http://msdn.microsoft.com/ko-kr/73a51c4d-131c-4c33-a139-b1342d10caab)를 사용합니다. 행 필터 수식에서 적용되는 이 함수는 새 역할에서 정의됩니다. 이 수식은 LOOKUPVALUE 함수를 사용하여 Employee Security 테이블의 값을 지정한 다음 이 값을 USERNAME 함수에 전달하며, 이 함수는 로그온한 사용자의 사용자 이름이 이 역할에 속하도록 지정합니다. 그런 다음 사용자는 역할의 행 필터에 지정된 데이터만 검색할 수 있습니다. 이 시나리오에서는 영업 직원이 자신이 멤버로 속한 영업 지역에 대한 인터넷 매출 데이터만 찾아볼 수 있도록 지정합니다.  
...
## dimSalesTerritory 테이블을 AW Internet Sales Tabular Model 프로젝트에 추가합니다.  
...
#### Sales Territory 테이블에서 열의 이름을 바꾸려면  
...
4.  워크시트를 **Sample Employee**로 저장합니다.  
...
7.  **붙여넣기 미리 보기** 대화 상자의 **테이블 이름**에 **Employee Security**를 입력합니다.  
...
## Internet Sales, Geography 및 Sales Territory 테이블 간의 관계 만들기  
...
#### Internet Sales, Geography 및 Sales Territory 테이블 간의 관계를 만들려면  
...
## 클라이언트 응용 프로그램에서 Employee Security 테이블 숨기기  
...
8.  **Employee Security** 테이블의 **DAX 필터** 열에 다음 수식을 입력합니다.  
...
## Sales Employees by Territory 사용자 역할 테스트  
...
#### Sales Employees by Territory 사용자 역할을 테스트하려면  
...
[LOOKUPVALUE 함수(DAX)](http://msdn.microsoft.com/ko-kr/73a51c4d-131c-4c33-a139-b1342d10caab)  
