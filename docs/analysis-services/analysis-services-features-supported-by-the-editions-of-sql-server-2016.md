---
title: SQL Server 버전에서 지 원하는 analysis Services 기능 | Microsoft Docs
ms.date: 07/10/2019
ms.prod: sql
...
|메모리 내 스토리지 모드|예|예|||||예|  
|DirectQuery 모드|예|예 (SQL Server 2019부터)|||||예|  

## <a name="multidimensional-models"></a>다차원 모델 
  
|기능|Enterprise|Standard|Web|Express with Advanced Services|Express with Tools|Express|개발자|  
|-------------|----------------|--------------|---------|------------------------------------|------------------------|-------------|---------------|  
|반가산적 측정값|예|아니오 <sup> [1](#sameas)</sup>|||||예|  
...
<a name="sameas">[1] </a> Standard edition에서 LastChild 반 가산적 측정값은 지원 되지만 None, FirstChild, FirstNonEmpty, LastNonEmpty, AverageOfChildren 및 ByAccount와 같은 다른 반 가산적 측정값 없습니다. Sum, Count, Min, Max와 같은 가산적 측정값과 비가산적 측정값(DistinctCount)은 모든 버전에서 지원됩니다. 

<a name="wb">[2] </a> SQL Server Analysis Services 2019에서 이상 차원 쓰기 저장은 지원 되지 않습니다.
 
<a name="linkmd">[3] </a> 연결 측정값과 동일한 데이터베이스 내에서 다른 데이터베이스 또는 인스턴스에서 제외할 차원 standard edition을 지원 합니다.
...
> [!NOTE]
> 데이터 마이닝은 [사용 되지 않는](analysis-services-backward-compatibility-sql2017.md#deprecated-features) SQL Server Analysis Services 2017의 합니다.
