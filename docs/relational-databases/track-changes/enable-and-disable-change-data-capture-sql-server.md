---
title: "변경 데이터 캡처 설정 및 해제(SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/06/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "변경 데이터 캡처 [SQL Server], 테이블 설정"
  - "변경 데이터 캡처 [SQL Server], 데이터베이스 설정"
  - "변경 데이터 캡처 [SQL Server], 데이터베이스를 사용하지 않도록 설정"
  - "변경 데이터 캡처 [SQL Server], 테이블을 사용하지 않도록 설정"
 ...
 **순 변경 내용을 쿼리하는 함수**  
 ...
 캡처 인스턴스에는 항상 정의된 간격 내에서 발생한 모든 변경 테이블 항목을 반환하는 데 사용되는 테이블 반환 함수가 포함됩니다. 이 함수 이름은 "cdc.fn_cdc_get_all_changes_"에 캡처 인스턴스 이름을 추가하여 지정합니다. 자세한 내용은 [cdc.fn_cdc_get_all_changes_&#60;capture_instance&#62;&#40;Transact-SQL&#41;](../Topic/cdc.fn_cdc_get_all_changes_%3Ccapture_instance%3E%20%20\(Transact-SQL\).md)를 참조하세요.  
  
 *@supports_net_changes* 매개 변수가 1로 설정된 경우 캡처 인스턴스에 순 변경 함수도 생성됩니다. 이 함수는 호출에 지정된 간격 내에 변경된 각 개별 행에 대해 하나의 변경만 반환합니다. 자세한 내용은 [cdc.fn_cdc_get_net_changes_&#60;capture_instance&#62;&#40;Transact-SQL&#41;](../Topic/cdc.fn_cdc_get_net_changes_%3Ccapture_instance%3E%20\(Transact-SQL\).md)를 참조하세요.  
