...
ms.openlocfilehash: 3bfac7be7260f9361ea424f46168c8033e094144
...
[!INCLUDE[SNAC_Deprecated](../../includes/snac-deprecated.md)]
 ...
 참고: unixODBC 2.3.1를 2.3.4, 설명자 필드는 SQL_CA_SS_TYPE_NAME SQLSetDescField 통해 TVP 이름을 설정할 때와 Linux 및 Mac ODBC 드라이버에 대 한 unixODBC 않습니다 자동으로 변환 하지 ANSI 및 유니코드 간의 정확한에 따라 문자열 함수 호출 (SQLSetDescFieldA / SQLSetDescFieldW). 항상 또는 사용할 SQLBindParameter SQLSetDescFieldW 유니코드 (utf-16) 문자열이 있는 TVP 이름을 설정 해야 하는 합니다.
...
|매개 변수|열을 비롯한 테이블 반환 매개 변수가 아닌 유형에 대한 관련 특성|테이블 반환 매개 변수에 대한 관련 특성|  
|---------------|--------------------------------------------------------------------------------|----------------------------------------------------|  
...
|*ColumnSize*|IPD의 SQL_DESC_LENGTH 또는 SQL_DESC_PRECISION<br /><br /> 값에 따라이 *ParameterType*합니다.|SQL_DESC_ARRAY_SIZE<br /><br /> 매개 변수 포커스를 테이블 반환 매개 변수로 설정할 때 SQL_ATTR_PARAM_SET_SIZE를 사용하여 설정할 수도 있습니다.<br /><br /> 테이블 반환 매개 변수의 경우 이 값은 테이블 반환 매개 변수 열 버퍼의 행 수입니다.|  
|*DecimalDigits*|IPD의 SQL_DESC_PRECISION 또는 SQL_DESC_SCALE|사용되지 않습니다. 0이어야 합니다.<br /><br /> 이 매개 변수는 하지 0, SQLBindParameter는 sqlstate 반환 SQL_ERROR와 진단 레코드가 생성 될 경우 = HY104 및 "잘못 된 정밀도 또는 배율" 메시지입니다.|  
|*ParameterValuePtr*|APD의 SQL_DESC_DATA_PTR|SQL_CA_SS_TYPE_NAME<br /><br /> 저장 프로시저 호출의 경우 선택 사항이며 필요하지 않은 경우 NULL을 지정할 수 있습니다. 프로시저 호출이 아닌 SQL 문에는 지정해야 합니다.<br /><br /> 또한 이 매개 변수는 변수 행 바인딩을 사용할 때 응용 프로그램에서 이 테이블 반환 매개 변수를 식별하는 데 사용할 수 있는 고유 값으로도 사용됩니다. 자세한 내용은 이 항목의 뒷부분에 나오는 "가변 테이블 반환 매개 변수 행 바인딩" 섹션을 참조하십시오.<br /><br /> SQLBindParameter에 대 한 호출에는 테이블 반환 매개 변수 형식 이름의 지정 하는 경우 ANSI 응용 프로그램으로 빌드된 응용 프로그램 에서도 유니코드 값으로 지정 되어야 합니다. 매개 변수에 대해 사용 되는 값 *StrLen_or_IndPtr* SQL_NTS 또는 문자열 길이의 sizeof (wchar)를 곱한 이름 이어야 합니다.|  
...
2.  각 테이블 반환 매개 변수의 서 수에 SQL_SOPT_SS_PARAM_FOCUS를 설정 하는 SQLSetStmtAttr를 호출 합니다.  
  
    1.  각 테이블 반환 매개 변수에 대 한 SQLBindParameter, SQLSetDescRec, 또는 SQLSetDescField 호출을 사용 하 여 테이블 반환 매개 변수 열을 바인딩합니다.  
  
    2.  각 테이블 반환 매개 변수 열에 대해 기본값이 지정 하는 것을 SQL_CA_SS_COL_HAS_DEFAULT_VALUE를 1로 설정 하는 SQLSetDescField를 호출 합니다.  
  
3.  SQL_SOPT_SS_PARAM_FOCUS를 0으로 설정 SQLSetStmtAttr를 호출 합니다. SQLExecute 하기 전에 수행 해야 하거나 SQLExecDirect 라고 합니다. 그렇지 않으면 SQL_ERROR가 반환되고 SQLSTATE=HY024 및 "특성 값 SQL_SOPT_SS_PARAM_FOCUS가 잘못되었습니다. 실행 시 0이어야 합니다"라는 메시지가 포함된 진단 레코드가 생성됩니다.  
  
4.  집합 *StrLen_or_IndPtr* 경우 SQLExecute 또는 SQLExecDirect의 다음 호출에 전송 될 행 개수 또는 행이 없는 테이블 반환 매개 변수에 대해서는 sql_default_param으로 SQL_DESC_OCTET_LENGTH_PTR 또는 테이블 반환 매개 변수 행이 있습니다. *StrLen_or_IndPtr* 또는 SQL_DESC_OCTET_LENGTH_PTR 설정할 수 없습니다 SQL_NULL_DATA로 테이블 반환 매개 변수에 대 한 (테이블 반환 매개 변수 구성 열 일 수 있음) 되지만 테이블 반환 매개 변수는 null을 허용 하지 않습니다. 잘못 된 값으로 설정 되어이 SQLExecute 또는 SQLExecDirect SQL_ERROR를 반환 하 고 SQLSTATE 된 진단 레코드가 생성 됩니다 = HY090 및 "매개 변수에 대 한 잘못 된 문자열 또는 버퍼 길이가 \<p >", 여기서 p는 매개 변수 번호입니다.  
  
5.  SQLExecute 또는 SQLExecDirect를 호출합니다.  
  
 테이블 반환 매개 변수 열 값 개별적으로 전달 해야 하는 경우 입력 *StrLen_or_IndPtr* SQL_LEN_DATA_AT_EXEC로 설정 됩니다 (*길이*) 또는 SQL_DATA_AT_EXEC 열에 대 한 합니다. 이는 매개 변수 배열을 사용할 때 값을 개별적으로 전달하는 것과 유사합니다. 드라이버;에 대 한 데이터를 요청 하 고 처럼 SQLParamData 모든 실행 시 데이터 매개 변수를 가진 배열 행을 표시 하지 않습니다. 이 응용 프로그램 주의 해야 합니다. 응용 프로그램은 드라이버에서 값을 요청하는 순서에 대해 어떠한 가정도 할 수 없습니다.  
