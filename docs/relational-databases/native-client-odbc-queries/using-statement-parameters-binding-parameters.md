...
-   호출을 [SQLFreeStmt](../../relational-databases/native-client-odbc-api/sqlfreestmt.md) 와 *옵션* 문 핸들에 연결 된 모든 매개 변수를 해제 하는 매개 변수가 SQL_RESET_PARAMS로 설정 합니다.  
...
-   열 단위 바인딩은 각 매개 변수가 고유한 변수 배열에 바인딩될 때 발생합니다.  
     ...
     호출 하 여 지정 된 열 단위 바인딩 [SQLSetStmtAttr](../../relational-databases/native-client-odbc-api/sqlsetstmtattr.md) 와 *특성* SQL_ATTR_PARAM_BIND_TYPE을 설정 및 *ValuePtr* SQL_PARAM_BIND_BY_COLUMN로 설정 합니다.  
  
-   행 단위 바인딩은 SQL 문의 모든 매개 변수가 매개 변수의 개별 변수를 포함하는 구조체 배열에 하나의 단위로 바인딩될 때 발생합니다.  
  
     호출 하 여 지정 된 행 단위 바인딩 **SQLSetStmtAttr** 와 *특성* SQL_ATTR_PARAM_BIND_TYPE로 설정 하 고 *ValuePtr* 보관 하는 구조체의 크기를 설정에서 프로그램 변수입니다.  
...
-   저장 프로시저를 만들 때 지정한 것과 다른 순서로 응용 프로그램에서 매개 변수를 지정할 수 있습니다.  
 ...
 경우 **SQL_DESC_NAME** 설정 된 저장된 프로시저 매개 변수 쿼리에서 모든 저장된 프로시저 매개 변수 설정 해야 **SQL_DESC_NAME**.  리터럴에 사용 됩니다 저장된 프로시저 호출에서 매개 변수가 있는 **SQL_DESC_NAME** 을 설정 하면 리터럴 형식을 사용 해야 *' 이름*=*값*', 여기서 *이름* 저장된 프로시저 매개 변수의 이름입니다 (예를 들어, @p1). 자세한 내용은 [이름 (명명 된 매개 변수)에 의해 매개 변수 바인딩](http://go.microsoft.com/fwlink/?LinkId=167215).  
