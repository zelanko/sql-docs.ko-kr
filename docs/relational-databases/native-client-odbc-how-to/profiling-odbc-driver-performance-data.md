---
title: 프로파일 드라이버 성능 데이터(ODBC) | 마이크로 소프트 문서
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
...
1.  **제어판에서** **관리 도구를** 두 번 클릭한 다음 **ODBC(데이터 원본)를**두 번 클릭합니다. 또는 odbcad32.exe를 호출할 수도 있습니다.  
...
5.  Microsoft SQL Server 구성 DSN 마법사에서 **로그 ODBC 드라이버 통계를 사용하여 로그 파일로 페이지를 이동합니다.**  
  
6.  **로그 파일에 ODBC 드라이버 통계 로그**를 선택합니다. 상자에서 통계를 기록할 파일 이름을 입력합니다. 선택적으로 **찾아보기를** 클릭하여 통계 로그에 대한 파일 시스템을 찾아봅을 클릭합니다.  
  
### <a name="to-log-driver-performance-data-programmatically"></a>드라이버 성능 데이터를 프로그래밍 방식으로 기록하려면  
  
1.  성능 데이터 로그 파일의 전체 경로 및 파일 이름을 SQL_COPT_SS_PERF_DATA_LOG [SQLSetConnectAttr로](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 호출합니다. 다음은 그 예입니다.  
  
    ```  
    "C:\\Odbcperf.log"  
    ```  
  
2.  SQL_COPT_SS_PERF_DATA 및 SQL_PERF_START [사용하여 SQLSetConnectAttr에](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 전화하여 성능 데이터 로깅을 시작합니다.  
  
3.  선택적으로 SQL_COPT_SS_LOG_NOW 및 NULL을 사용하여 [SQLSetConnectAttr을](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 호출하여 성능 데이터 로그 파일에 성능 데이터의 탭 구분 된 레코드를 작성합니다. 이 작업은 애플리케이션 실행 시 여러 번 수행할 수 있습니다.  
  
4.  SQL_COPT_SS_PERF_DATA 및 SQL_PERF_STOP [사용하여 SQLSetConnectAttr에](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 전화하여 성능 데이터 로깅을 중지합니다.  
  
### <a name="to-pull-driver-performance-data-into-an-application"></a>드라이버 성능 데이터를 애플리케이션으로 가져오려면  
  
1.  SQL_COPT_SS_PERF_DATA 및 SQL_PERF_START [SQLSetConnectAttr에](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 전화하여 성능 데이터 프로파일링을 시작합니다.  
  
2.  [SQLGetConnectAttr에](../../relational-databases/native-client-odbc-api/sqlgetconnectattr.md) SQL_COPT_SS_PERF_DATA 및 SQLPERF 구조에 대한 포인터의 주소를 호출합니다. 첫 번째 호출에서는 현재 성능 데이터가 포함된 올바른 SQLPERF 구조의 주소에 대한 포인터를 설정합니다. 드라이버는 성능 구조의 데이터를 지속적으로 새로 고치지 않습니다. 응용 프로그램은 최신 성능 데이터로 구조를 새로 고쳐야 할 때마다 [SQLGetConnectAttr에](../../relational-databases/native-client-odbc-api/sqlgetconnectattr.md) 대한 호출을 반복해야 합니다.  
  
3.  SQL_COPT_SS_PERF_DATA 및 SQL_PERF_STOP [사용하여 SQLSetConnectAttr에](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 전화하여 성능 데이터 로깅을 중지합니다.  
