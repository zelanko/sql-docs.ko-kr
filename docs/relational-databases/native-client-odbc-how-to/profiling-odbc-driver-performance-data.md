...
1.  호출 [SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) SQL_COPT_SS_PERF_DATA_LOG 및 성능 데이터 로그 파일의 전체 경로 파일 이름을 사용 합니다. 예를 들어:  
  
    ```  
    "C:\\Odbcperf.log"  
    ```  
  
2.  호출 [SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 성능 데이터 기록을 시작 하려면 SQL_COPT_SS_PERF_DATA 및 SQL_PERF_START를 사용 합니다.  
  
3.  필요에 따라 호출 [SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) SQL_COPT_SS_LOG_NOW 및 NULL 성능 데이터 로그 파일에 탭으로 구분 된 기록 성능 데이터를 쓰려고 합니다. 이 작업은 응용 프로그램 실행 시 여러 번 수행할 수 있습니다.  
  
4.  호출 [SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 성능 데이터 기록을 중지 하려면 SQL_COPT_SS_PERF_DATA 및 SQL_PERF_STOP을 사용 합니다.  
  
### <a name="to-pull-driver-performance-data-into-an-application"></a>드라이버 성능 데이터를 응용 프로그램으로 가져오려면  
  
1.  호출 [SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 성능 데이터 프로 파일링을 시작 하려면 SQL_COPT_SS_PERF_DATA 및 SQL_PERF_START를 사용 합니다.  
  
2.  호출 [SQLGetConnectAttr](../../relational-databases/native-client-odbc-api/sqlgetconnectattr.md) SQL_COPT_SS_PERF_DATA 및 SQLPERF 구조에 대 한 포인터의 주소를 사용 합니다. 첫 번째 호출에서는 현재 성능 데이터가 포함된 올바른 SQLPERF 구조의 주소에 대한 포인터를 설정합니다. 드라이버는 성능 구조의 데이터를 지속적으로 새로 고치지 않습니다. 응용 프로그램에 대 한 호출을 반복 해야 [SQLGetConnectAttr](../../relational-databases/native-client-odbc-api/sqlgetconnectattr.md) 를 보다 최신 성능 데이터로 구조를 새로 고쳐야 할 때마다 합니다.  
  
3.  호출 [SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 성능 데이터 기록을 중지 하려면 SQL_COPT_SS_PERF_DATA 및 SQL_PERF_STOP을 사용 합니다.  
