...
[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]은(는) 분석 워크로드를 가속화하기 위한 새로운 기능인 columnstore 인덱스를 도입했습니다. 후속 릴리스에서 사용 사례를 확장하고 columnstore 인덱스의 성능을 개선해왔습니다. 지금까지 이러한 모든 기능을 단일 기능으로 표시하고 문서화했습니다. 테이블에 columnstore 인덱스를 만들면 분석 워크로드가 "더 빠르게 진행"됩니다. 그러나 내부적으로는 관련이 있으나 고유한 두 가지 기술 집합이 사용됩니다.
...
3. columnstore 인덱스를 만들면 워크로드의 트랜잭션 부분에 너무 많은 오버헤드가 추가됩니다. **또는** 애플리케이션이 columnstore 인덱스가 아직 지원되지 않는 기능에 의존하므로 columnstore 인덱스를 만드는 작업이 실현 가능하지 않습니다.
...
4. rowstore의 새 일괄 처리 모드에 대한 현재 제한 검색: 메모리 내 OLTP 테이블 또는 디스크에 있는 힙 및 B-트리 이외의 인덱스에는 이러한 제한이 적용되지 않습니다. LOB 열을 가져오거나 필터링하는 경우에도 제한이 적용되지 않습니다. 이 제한 사항에는 스파스 열 집합 및 XML 열이 포함됩니다.
