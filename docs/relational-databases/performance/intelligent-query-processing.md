...
SQL Server 2012는 분석 워크로드를 가속화하기 위한 새로운 기능인 columnstore 인덱스를 도입했습니다. 후속 릴리스에서 사용 사례를 확장하고 columnstore 인덱스의 성능을 개선해왔습니다. 지금까지 이러한 모든 기능을 단일 기능으로 나타내고 문서화했습니다. 즉, 테이블에서 columnstore 인덱스를 만들면 분석 워크로드가 "더 빠르게 진행”됩니다. 그러나 내부적으로는 관련이 있으나 고유한 두 가지 기술 집합이 사용됩니다.
...
- **일괄 처리 모드** 처리를 사용하면 쿼리 연산자는 한 번에 하나의 행이 아니라 일괄 처리 행에 대해 작업을 수행하여 훨씬 더 효율적으로 데이터를 처리할 수 있습니다. 향상된 여러 다른 확장성이 일괄 처리 모드와 관련이 있습니다.
