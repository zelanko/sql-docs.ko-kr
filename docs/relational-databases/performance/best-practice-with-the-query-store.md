...
##  <a name="Insight"></a> Azure SQL Database에서 Query Performance Insight 사용  
 Azure [!INCLUDE[ssSDS](../../includes/sssds-md.md)]에서 쿼리 저장소를 실행하는 경우, **Query Performance Insight**를 사용하여 시간에 따른 DTU 사용을 분석할 수 있습니다. [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]를 사용하여 CPU, 메모리, I/O 등 모든 쿼리의 자세한 리소스 사용 정보를 가져올 수 있지만, Query Performance Insight는 데이터베이스의 전반적인 DTU 사용에 대한 영향을 빠르고 효율적으로 확인할 수 있는 방법을 제공합니다. 자세한 내용은 [Azure SQL 데이터베이스 Query Performance Insight](https://azure.microsoft.com/documentation/articles/sql-database-query-performance/)를 참조하세요.
...
##  <a name="Verify"></a> 쿼리 저장소에서 쿼리 데이터를 계속 수집하는지 확인  
...
##  <a name="Drop"></a> 포함하는 개체에 대해 DROP 및 CREATE 패턴 방지
...
##  <a name="Renaming"></a> 강제 계획이 포함된 쿼리의 경우 데이터베이스 이름 변경 방지  
