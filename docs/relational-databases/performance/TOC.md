...
# [Showplan XML 이벤트를 개별적으로 저장(SQL Server 프로파일러)](save-showplan-xml-events-separately-sql-server-profiler.md)
# [성능 모니터링 및 튜닝 도구](performance-monitoring-and-tuning-tools.md)
# [SQL Server 데이터베이스 엔진 및 Azure SQL 데이터베이스에 대한 성능 센터](performance-center-for-sql-server-database-engine-and-azure-sql-database.md)
# [성능 문제 격리](isolate-performance-problems.md)
# [계획 지침에 쿼리 힌트 연결](attach-query-hints-to-a-plan-guide.md)
# [Windows 응용 프로그램 로그 보기(Windows)](view-the-windows-application-log-windows.md)
