...
## <a name="properties"></a>속성  
 BulkLoad  
 대량 로드를 수행할지 여부를 지정합니다. (다음에 나오는 SchemaGen, SGDropTables, 및 SGUseID 속성 참조) 스키마만 생성 하 고 대량 로드를 수행 하지 않으려면이 속성이 유용 합니다. 이 속성은 부울 속성입니다. 이 속성이 TRUE로 설정된 경우 XML 대량 로드가 실행되고 FALSE로 설정된 경우 XML 대량 로드가 실행되지 않습니다.  
...
> [!NOTE]  
>  FALSE로이 속성을 유지 하려면 있어야 **ALTER TABLE** 대상 테이블에 대 한 권한. 자세한 내용은 [ALTER TABLE&#40;Transact-SQL&#41;](../../../t-sql/statements/alter-table-transact-sql.md)을 참조하세요.  
 ...
 ConnectionCommand  
 XML 대량 로드 사용 해야 하는 기존 연결 개체 (예를 들어 ADO 또는 ICommand 명령 개체)를 식별 합니다. ConnectionString 속성을 사용 하 여 연결 문자열을 지정 하는 대신 ConnectionCommand 속성을 사용할 수 있습니다. ConnectionCommand를 사용 하는 경우 트랜잭션 속성을 TRUE로 설정 되어야 합니다.  
...
>  FALSE로이 속성을 유지 하려면 있어야 **ALTER TABLE** 대상 테이블에 대 한 권한. 자세한 내용은 [ALTER TABLE&#40;Transact-SQL&#41;](../../../t-sql/statements/alter-table-transact-sql.md)을 참조하세요.  
 ...
 IgnoreDuplicateKeys 속성은 TRUE로 설정 하는 경우 테이블에 삽입 된 모든 레코드에 대 한 COMMIT 문을 실행 됩니다. 이로 인해 성능이 저하됩니다. 속성 파일을 사용 하 여 트랜잭션 동작 구현 되기 때문에 트랜잭션 속성을 FALSE로 설정 되는 경우에 TRUE로 설정할 수 있습니다.  
...
> [!NOTE]  
>  이 속성이 TRUE로 않으려면 있어야 **ALTER TABLE** 대상 테이블에 대 한 권한. 그렇지 않은 경우에는 값을 FALSE로 설정해야 합니다. 자세한 내용은 [ALTER TABLE&#40;Transact-SQL&#41;](../../../t-sql/statements/alter-table-transact-sql.md)을 참조하세요.  
 ...
 SchemaGen  
 대량 로드 작업을 수행하기 전에 필요한 테이블을 만들지 여부를 지정합니다. 이 속성은 부울 속성입니다. 이 속성이 TRUE로 설정된 경우 매핑 스키마에 식별된 테이블이 생성됩니다. 이 경우 데이터베이스가 있어야 합니다. 하나 이상의 테이블에에서 이미 있으면 데이터베이스를 SGDropTables 속성을 기존 테이블을 삭제 한 다음 다시 생성 되는지 여부를 결정 합니다.  
  
 SchemaGen 속성의 기본값은 FALSE입니다. SchemaGen 새로 만든된 테이블에 PRIMARY KEY 제약 조건을 만들지 않습니다. 그러나 SchemaGen, 만들어지지는 FOREIGN KEY 제약 조건 데이터베이스의 경우 일치 하는 것이 있습니다 **sql: relationship** 하 고 **sql:-필드** 매핑 스키마에서 주석 및 키 필드의 구성 하는 경우 단일 열입니다.  
...
-   오버플로 사용 하 여 지정 된 모든 열에 대 한 데이터를 반환 합니다 [sql:overflow-필드](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/bulk-load-xml/annotation-interpretation-sql-overflow-field.md) 에 [xml 데이터 형식](../../../t-sql/xml/xml-transact-sql.md) 형식입니다.  
  
 SGDropTables  
 기존 테이블을 삭제하고 다시 만들지 여부를 지정합니다. SchemaGen 속성을 TRUE로 설정 된 경우이 속성을 사용 합니다. SGDropTables FALSE 이면 기존 테이블이 유지 됩니다. 이 속성이 TRUE이면 기존 테이블이 삭제되고 다시 생성됩니다.  
  
 기본값은 FALSE입니다.  
  
 SGUseID  
 매핑 스키마에서 특성 인지 여부는 지정 취급할 **id** 형식은 테이블을 만들 때 PRIMARY KEY 제약을 만드는 데 사용할 수 있습니다. SchemaGen 속성을 TRUE로 설정 된 경우이 속성을 사용 합니다. SchemaGen 유틸리티에서는 특성을 사용 하는 SGUseID TRUE 인 경우 **dt: type = "id"** 기본 키 열으로 지정 되 고 테이블을 만들 때 적절 한 PRIMARY KEY 제약 조건을 추가 합니다.  
...
> [!NOTE]  
>  이진 데이터를 로드 하는 경우 (bin.hex, bin.base64 XML 데이터 형식 binary, image [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 데이터 형식), 트랜잭션 속성을 FALSE로 설정 해야 합니다.  
