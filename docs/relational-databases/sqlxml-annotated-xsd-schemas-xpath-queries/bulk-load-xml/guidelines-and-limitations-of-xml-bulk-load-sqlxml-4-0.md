...
-   스키마를 사용 하 여 오버플로 열의 지정 하지 않은 경우는 **sql:overflow-필드** 주석, XML 대량 로드는 XML 문서에 포함 되어 있지만 매핑 스키마에 설명 되지 않은 데이터가 무시 합니다.  
    ...
    ```  
     ...
     이러한 XML 대량 로드 무시  **\<순서 >** 요소입니다. 하지만 사용 하는 경우는 **sql:overflow-필드**이 열에서 소비 되지 않은 모든 데이터를 저장 하는 XML 대량 로드는 오버플로 열으로 열을 식별 하는 스키마에 주석이 있습니다.  
...
-   SchemaGen 속성을 지정 하는 경우 (예를 들어 SchemaGen = true), 매핑 스키마에 식별 된 테이블이 생성 됩니다. 하지만 SchemaGen 예외적으로 이러한 테이블에 모든 제약 조건 (예: PRIMARY KEY/FOREIGN KEY 제약 조건)을 만들지 않습니다: 관계의 기본 키를 구성 하는 XML 노드는 XML 유형의 ID 가진 것으로 정의 된 경우 (즉, **형식 = "xsd: id"** XSD에 대 한) 및 SGUseID 속성 SchemaGen에 대 한 True로 설정 된 경우에서 기본 키 생성 뿐만 아니라 ID 형식 노드에서 있지만 매핑 스키마 관계에서 기본 키/외래 키 관계가 생성 됩니다.  
...
-   일부 속성 플래그는 다른 속성 플래그와 함께 사용할 수 없습니다. 예를 들어 대량 로드는 지원 되지 **Ignoreduplicatekeys = true** 와 함께 **Keepidentity = false**합니다. 때 **Keepidentity = false**, 대량 로드에서는 서버 키 값을 생성 합니다. 테이블는 **IDENTITY** 키에 대 한 제약 조건입니다. 서버에서는 하지 생성 중복 키 있으며 필요 하지 않음을 의미 **Ignoreduplicatekeys** 로 설정 해야 **true**합니다. **Ignoreduplicatekeys** 로 설정 해야 **true** 만 행이 있는 테이블로 들어오는 데이터 로부터 기본 키 값을 업로드 하는 경우와 기본 키 값이 충돌할 가능성이 있습니다.  
