---
title: "XPath 쿼리 (SQLXML 4.0)에서 축 지정 | Microsoft Docs"
ms.custom: 
ms.date: 03/04/2017
...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]다음 예제에서는 XPath 쿼리에 축을 지정 하는 방법을 보여 줍니다.  
  
 이 예의 XPath 쿼리는 SampleSchema1.xml에 포함된 매핑 스키마에 대해 지정되었습니다. 이 예제 스키마에 대 한 정보를 참조 하십시오. [XPath 예제 &#40;에 대 한 샘플 주석이 추가 된 XSD 스키마 SQLXML 4.0 &#41; ](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/samples/sample-annotated-xsd-schema-for-xpath-examples-sqlxml-4-0.md).  
  
## <a name="examples"></a>예  
  
### <a name="a-retrieve-child-elements-of-the-context-node"></a>1. 컨텍스트 노드의 자식 요소 검색  
 다음 XPath 쿼리에서 모든 선택은  **\<연락처 >** 컨텍스트 노드의 자식 요소:  
  
```  
/child::Contact  
```  
  
 쿼리에서 `child` 는 축이고 및 `Contact` 는 노드 테스트 (TRUE 이면 `Contact` 는  **\<요소 >** 노드를 때문에 \<요소 > 연결 된 주 노드 유형에 `child` 축).  
...
### <a name="b-retrieve-grandchildren-of-the-context-node"></a>2. 컨텍스트 노드의 손자 검색  
 다음 XPath 쿼리에서 모든 선택은  **\<순서 >** 요소 자식을  **\<고객 >** 컨텍스트 노드의 요소 자식을:  
  
```  
/child::Customer/child::Order  
```  
  
 쿼리에서 `child` 는 축이고와 `Customer` 및 `Order` 는 노드 테스트 (이러한 노드 테스트 Customer와 Order는 경우 TRUE를  **\<요소 >** 노드 때문에  **\<요소 >** 노드는에 대 한 주 노드는 **자식** 축). 일치 하는 각 노드  **\<고객 >**, 해당 하는 노드  **\<Orders >** 결과에 추가 됩니다. 만  **\<순서 >** 결과 집합에 반환 됩니다.  
...
### <a name="d-specify-the-attribute-axis"></a>4. 특성 축 지정  
 다음 XPath 쿼리에서 모든 선택은  **\<고객 >** 인 컨텍스트 노드의 자식 요소는 **CustomerID** 특성 값이 1:  
  
```  
/child::Customer[attribute::CustomerID="1"]  
```  
  
 조건자에서 `attribute::CustomerID`, `attribute` 는 축이고와 `CustomerID` 는 노드 테스트 (경우 `CustomerID` 는 노드 테스트 결과 TRUE, 특성 때문에  **\<특성 >** 노드는의 주 노드를 `attribute` 축).  
