---
title: XPath 쿼리 (SQLXML 4.0)를 사용 하 여 소개 | Microsoft Docs
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
...
> [!NOTE]  
>  SQLXML 4.0의 XPath 쿼리를 이해하려면 템플릿 및 매핑 스키마와 같은 관련 개념과 XML 뷰에 대해 잘 알고 있어야 합니다. 자세한 내용은 [주석이 추가 된 XSD 스키마 소개 &#40;SQLXML 4.0&#41;](../../relational-databases/sqlxml/annotated-xsd-schemas/introduction-to-annotated-xsd-schemas-sqlxml-4-0.md), 및 World Wide Web Consortium (W3C) 정의한 XPath 표준을 합니다.  
...
```  
 ...
 이 문서의  **\<고객 >** 요소 노드인지를 **cid** 는 특성 노드, 및 **"중요"** 는 텍스트 노드입니다.  
  
 XPath는 XML 문서에서 노드 집합을 선택하는 데 사용되는 그래프 탐색 언어입니다. 각 XPath 연산자는 이전 XPath 연산자에서 선택한 노드 집합을 기반으로 노드 집합을 선택합니다. 예를 들어, 지정 된 집합  **\<고객 >** XPath 노드 모두를 선택할 수 있습니다  **\<순서 >** 사용 하 여 노드를 **날짜** 값특성 **"7/14/1999"** 합니다. 결과 노드 집합에는 주문 날짜가 1999년 7월 14일인 모든 주문이 포함됩니다.  
...
-   **루트 쿼리**  
     ...
     SQLXML 4.0은 루트 쿼리(/)를 지원하지 않습니다. 각 XPath 쿼리는 최상위 수준에서 시작 해야 합니다  **\<ElementType >** 스키마에 있습니다.  
...
-   **문서 순서**  
    ...
    > [!NOTE]  
    >  일부 경우에는 **키 필드** 주석 또는에서 키를 **관계** 주석 문서 순서가 결정적이 될 수 있습니다. 자세한 내용은 이러한 주석의 주된 용도 아닙니다. 그러나 참조 [키 열 사용을 식별 하는 sql:-필드 &#40;SQLXML 4.0&#41; ](../../relational-databases/sqlxml-annotated-xsd-schemas-using/identifying-key-columns-using-sql-key-fields-sqlxml-4-0.md) 하 고 [지정 관계를 사용 하 여 sql: 관계 &#40;SQLXML 4.0&#41;](../../relational-databases/sqlxml-annotated-xsd-schemas-using/specifying-relationships-using-sql-relationship-sqlxml-4-0.md)합니다.  
  
-   **데이터 형식**  
  
     SQLXML 4.0에서는 XPath 구현에 제약이 따릅니다 **문자열**를 **번호**, 및 **부울** 데이터 형식입니다. 자세한 내용은 [XPath 데이터 형식 &#40;SQLXML 4.0&#41;](../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/xpath-data-types-sqlxml-4-0.md)합니다.  
...
|기능|항목|  
|-------------|----------|  
|Axes|**상위**, **상위 또는 자신의**를 **하위**를 **하위 항목 또는 자체 (/ /)** 를 **다음**,  **다음 형제**, **네임 스페이스**합니다 **위의**, **이전 형제**|  
|숫자 값 조건자||  
|산술 연산자|mod|  
|노드 함수|**상위**, **상위 또는 자신의**를 **하위**를 **하위 항목 또는 자체 (/ /)** 를 **다음**,  **다음 형제**, **네임 스페이스**합니다 **위의**, **이전 형제**|  
