...
> [!NOTE]
> **MAXDOP(최대 병렬 처리 수준)** 제한은 요청별이 아니라 태스크별로 설정됩니다. 즉, 병렬 쿼리를 실행하는 동안 단일 요청은 여러 태스크를 생성할 수 있으며, 각 태스크는 MAXDOP 제한까지 여러 작업자를 사용할 수 있습니다. MAXDOP에 대한 자세한 내용은 [최대 병렬 처리 수준 서버 구성 옵션 구성](../database-engine/configure-windows/configure-the-max-degree-of-parallelism-server-configuration-option.md)을 참조하세요.
