---
title: "OOM(메모리 부족) 문제 해결 | Microsoft Docs"
ms.custom: ""
ms.date: "08/29/2016"
...
-   MAX_MEMORY_PERCENT의 값을 늘립니다.   
    ...
    > 자세한 내용은 [최선의 구현 방법: VM 환경에서 메모리 내 OLTP 사용](../Topic/Using%20In-Memory%20OLTP%20in%20a%20VM%20Environment.md) 항목을 참조하세요.  
...
##### 필수적이지 않은 메모리 액세스에 최적화된 테이블 행을 삭제하고 가비지 수집 대기  
...
##### 리소스 풀에서 MAX_MEMORY_PERCENT의 값 늘리기  
 메모리 내 테이블에 대한 명명된 리소스 풀을 만들지 않은 경우 이 리소스 풀을 만들고 [!INCLUDE[hek_2](../../includes/hek-2-md.md)] 데이터베이스를 이 리소스 풀에 바인딩해야 합니다. 리소스 풀을 만들고 [!INCLUDE[hek_2](../../includes/hek-2-md.md)] 데이터베이스를 리소스 풀에 바인딩하는 방법에 대한 지침은 [메모리 액세스에 최적화된 테이블이 있는 데이터베이스를 리소스 풀에 바인딩](../../relational-databases/in-memory-oltp/bind-a-database-with-memory-optimized-tables-to-a-resource-pool.md) 항목을 참조하세요.  
...
## 참고 항목  
 [메모리 내 OLTP의 메모리 관리](../Topic/Managing%20Memory%20for%20In-Memory%20OLTP.md)   
 ...
 [최선의 구현 방법: VM 환경에서 메모리 내 OLTP 사용](../Topic/Using%20In-Memory%20OLTP%20in%20a%20VM%20Environment.md)  
