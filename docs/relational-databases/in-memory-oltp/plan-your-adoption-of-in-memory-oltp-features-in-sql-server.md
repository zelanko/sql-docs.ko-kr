---
title: "SQL Server에 메모리 내 OLTP 기능 채택 계획 | Microsoft Docs"
ms.custom: ""
ms.date: "10/05/2016"
...
### A.3 메모리 액세스에 최적화된 테이블로 변환하려면 테이블이 오프라인 상태여야 함
...
### A.4 메모리 내 OLTP 기능이 응용 프로그램에 적합한지에 대한 지침

메모리 내 기능이 특정 응용 프로그램의 성능을 향상할 수 있는지에 대한 지침은 다음을 참조하세요.
...
특정 메모리 내 시나리오에서 지원되지 않는 기능은 다음에 설명되어 있습니다.
...
쿼리는 READPAST [테이블 힌트](Table%20Hints%20%28Transact-SQL%29.md)를 메모리 액세스에 최적화된 테이블에 적용할 수 없습니다.
...
- [SEQUENCE](../../t-sql/statements/create-sequence-transact-sql.md) 개체는 메모리 액세스에 최적화된 테이블과 함께 사용할 수 없습니다.
...
### C.2 DBCC CHECKDB는 메모리 액세스에 최적화된 테이블의 유효성을 검사할 수 없음
...
1. [트랜잭션 로그를 백업](Back%20Up%20a%20Transaction%20Log%20%28SQL Server%29.md)합니다.
...
- 디스크 기반 테이블을 기반으로 하여 [columnstore 인덱스](Columnstore%20Indexes%20Guide.md)를 클러스터형 인덱스로도 작성할 수 있습니다. 그러나 메모리 액세스에 최적화된 테이블에서는 columnstore 인덱스를 클러스터화할 수 없습니다.
...
### E.3 네이티브 프로시저에서 UPDATE 또는 DELETE 문에 조인 사용 불가
