...
그러나 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 구문은 메모리 액세스에 최적화된 테이블의 데이터에 액세스하는 해석된 [!INCLUDE[tsql](../../includes/tsql-md.md)] 저장 프로시저에서 지원되지 않습니다.  
...
해석된 [!INCLUDE[tsql](../../includes/tsql-md.md)]을 사용하여 명시적 또는 암시적 트랜잭션에서 메모리 액세스에 최적화된 테이블에 액세스하는 경우 적어도 다음 중 하나를 수행해야 합니다.  
