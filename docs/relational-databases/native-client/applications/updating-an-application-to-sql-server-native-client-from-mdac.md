...
-   MDAC 연결 문자열을 부울 값을 사용 (**true**)에 대 한는 **Trusted_Connection** 키워드입니다. A [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 연결 문자열을 사용 해야 **예** 또는 **없습니다**합니다.  
...
-   데이터 형식의 값을 반환할 **varchar (max)**, **nvarchar (max)**, **varbinary (max)**, **xml**, **udt**, 다른 큰 개체 유형의 버전의 클라이언트에 반환 될 수 없습니다 또는 이전의 [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)]합니다. 이러한 유형을 반환 값으로 사용하려는 경우 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client를 사용해야 합니다.  
...
-   MDAC 응용 프로그램이 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]에 연결하면 다음 그림과 같이 [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)]에서 도입된 데이터 형식이 [!INCLUDE[ssVersion2000](../../../includes/ssversion2000-md.md)] 호환 데이터 형식으로 나타납니다.  
    ...
    |**xml**|**ntext**|  
     ...
     이 형식 매핑은 열 메타데이터에 대해 반환되는 값에 영향을 줍니다. 예를 들어 한 **텍스트** 열에 최대 크기가 2147483647, 하지만 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC의 최대 크기를 보고 **varchar (max)** SQL_SS_LENGTH_UNLIMITED 및 열[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]Native Client OLE DB의 최대 크기를 보고 **varchar (max)** 열으로 2,147,483,647 또는 플랫폼에 따라-1입니다.  
