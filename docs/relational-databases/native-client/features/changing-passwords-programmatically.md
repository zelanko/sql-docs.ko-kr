...
> [!NOTE]  
>  가능한 경우 런타임에 자격 증명을 입력하라는 메시지를 사용자에게 표시하고 해당 자격 증명을 지속형 형식으로 저장하지 마십시오. 증명을 암호화 해야 자격 증명을 저장 해야 하는 경우 사용 하는 [Win32 crypto API](http://go.microsoft.com/fwlink/?LinkId=64532)합니다. 암호의 사용에 대 한 자세한 내용은 참조 [Strong Passwords](../../../relational-databases/security/strong-passwords.md)합니다.  
...
### <a name="ole-db-user-interface-password-expiration"></a>OLE DB 사용자 인터페이스 암호 만료  
 ...
 DBPROP_INIT_PROMPT 다른 값으로 설정한 경우 사용자에 게 표시 합니다 **SQL Server 로그인** 대화 상자에서 암호 만료 여부에 관계 없이 합니다. 클릭할 수 있습니다는 **옵션** 단추를 확인 **암호 변경** 암호를 변경 합니다.  
  
 사용자가 확인을 클릭 하 고 암호가 만료 되었습니다 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 입력 하 고 사용 하 여 새 암호를 확인 하 라는 메시지를 **SQL Server 암호 변경** 대화 합니다.  
  
#### <a name="ole-db-prompt-behavior-and-locked-accounts"></a>OLE DB 프롬프트 동작 및 잠긴 계정  
 계정이 잠겨 연결 시도가 실패할 수 있습니다. 표시 된 후이 문제가 발생 합니다 **SQL Server 로그인** 대화 상자에서 서버 오류 메시지가 사용자에 게 표시 되 고 연결 시도가 중단 됩니다. 표시 된 발생할 수도 합니다 **SQL Server 암호 변경** 사용자가 이전 암호에 대 한 잘못 된 값을 입력 하는 경우 대화 상자. 이 경우 동일한 오류 메시지가 표시되고 연결 시도가 중단됩니다.  
...
>  SSPROP_AUTH_OLD_PASSWORD는 암호가 만료된 경우에만 사용되기 때문에 저장하면 안 됩니다.  
 ...
 Windows 인증을 사용 하는 경우 이전 암호를 지정 하면 DB_E_ERRORSOCCURRED 또는 db_s_errorsoccurred가 이전 암호 지정 여부 REQUIRED 또는 OPTIONAL로 각각 DBPROPSTATUS_의 상태 값에 따라 CONFLICTINGBADVALUE 반환 됩니다 *dwStatus*합니다. 이 검색 될 때 **idbinitialize:: Initialize** 라고 합니다.  
...
### <a name="odbc-user-interface-password-expiration"></a>ODBC 사용자 인터페이스 암호 만료  
 ...
 하는 경우 **DriverCompletion** 설정 된 다른 값으로 사용자에 게 표시 합니다 **SQL Server 로그인** 대화 상자에서 암호 만료 여부에 관계 없이 합니다. 클릭할 수 있습니다는 **옵션** 단추를 확인 **암호 변경** 암호를 변경 합니다.  
