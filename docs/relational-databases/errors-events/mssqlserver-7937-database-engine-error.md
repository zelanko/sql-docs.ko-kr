...
|이벤트 원본|MSSQLSERVER|  
|구성 요소|SQLEngine|  
|심볼 이름|DBCC2_FS_MISSING_COLUMN_DIRECTORY|  
|메시지 텍스트|테이블 오류: 개체 ID O_ID, 인덱스 ID I_ID, 파티션 ID PN_ID의 열 ID C_ID에 대한 FileStream 디렉터리를 찾을 수 없습니다.|  
  
## <a name="explanation"></a>설명  
지정한 열은 파티션에 있지만 해당 FILESTREAM 디렉터리를 FILESTREAM 데이터 공간에서 찾을 수 없습니다.  
