...
SQL 프로세스에서 래치를 얻기 위해 대기하는 중입니다. 이 문제는 완료하는 데 시간이 너무 오래 걸리는 I/O 작업으로 인해 발생할 수 있습니다. 일반적으로 이 오류 유형은 시스템 프로세스를 차단하는 다른 태스크로 인해 발생합니다. 하드웨어 오류로 인해 이 오류가 발생하는 경우도 있습니다.  이 오류 메시지가 발생하면 컴퓨터와 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 응답이 중지되는 것을 확인할 수 있습니다.
