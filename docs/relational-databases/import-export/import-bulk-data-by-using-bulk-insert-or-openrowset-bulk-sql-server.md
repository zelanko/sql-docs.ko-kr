---
title: "BULK INSERT 또는 OPENROWSET(BULK...)를 사용하여 데이터 대량 가져오기 | Microsoft Docs"
ms.custom: ""
ms.date: "07/26/2016"
...
ms.topic: "article"
helpviewer_keywords: 
  - "BULK INSERT 문, 원격 데이터 파일에서 데이터 가져오기"
  - "대량 가져오기 [SQL Server], 방법"
  - "대량 내보내기 [SQL Server], 방법"
  - "OPENROWSET 함수, BULK INSERT"
  - "대량 가져오기 [SQL Server], 보안"
  - "대량 행 집합 공급자 [SQL Server]"
  - "대량 내보내기 [SQL Server], BULK INSERT 문"
  - "원격 데이터 액세스 [SQL Server], 대량 가져오기"
  - "대량 가져오기 [SQL Server], BULK INSERT 문"
  - "Transact-SQL 대량 내보내기/가져오기 작업"
...
> **참고** BULK INSERT 또는 OPENROWSET(BULK…)을 사용할 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 버전에서 가장을 처리하는 방법을 이해해야 합니다. 자세한 내용은 이 항목의 뒷부분에 나오는 "보안 고려 사항"을 참조하십시오.  
...
## OPENROWSET(BULK…) 함수  
 ...
 INSERT 문에서 사용하는 경우 OPENROWSET(BULK...)은 테이블 힌트를 지원합니다. 또한 TABLOCK과 같은 일반적인 테이블 힌트 외에도 BULK 절에는 IGNORE_CONSTRAINTS(CHECK 제약 조건만 무시), IGNORE_TRIGGERS, KEEPDEFAULTS 및 KEEPIDENTITY와 같은 특수 테이블 힌트도 사용할 수 있습니다. 자세한 내용은 [테이블 힌트&#40;Transact-SQL&#41;](../Topic/Table%20Hints%20\(Transact-SQL\).md)를 참조하세요.  
  
 BULK 옵션의 추가 사용법에 대한 자세한 내용은 [OPENROWSET&#40;Transact-SQL&#41;](../../t-sql/functions/openrowset-transact-sql.md)를 참조하세요.  
  
## INSERT...SELECT * FROM OPENROWSET(BULK...) 문 - 예제:
...
> **참고** 클라이언트는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]와 독립적으로 파일을 읽기 때문에 **bcp** 유틸리티에는 이러한 제한 사항이 적용되지 않습니다.  
