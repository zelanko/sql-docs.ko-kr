---
title: SQL Server XTP 트랜잭션 로그 | Microsoft 문서
description: SQL Server의 메모리 내 OLTP 트랜잭션 로그 작업에 관련된 카운터를 포함하는 SQL Server XTP 트랜잭션 로그 성능 개체에 대해 알아봅니다.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.prod_service: database-engine
ms.reviewer: ''
ms.technology: performance
ms.topic: conceptual
ms.assetid: d16491ef-4a25-4e02-88a7-ab7c0812904a
author: WilliamDAssafMSFT
ms.author: wiassaf
ms.openlocfilehash: dafddb3a8c0880881413db2ff51330cbe1a04cb0
ms.sourcegitcommit: 0e0cd9347c029e0c7c9f3fe6d39985a6d3af967d
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/02/2020
ms.locfileid: "96505443"
---
# <a name="sql-server-xtp-transaction-log"></a>SQL Server XTP 트랜잭션 로그
 [!INCLUDE [SQL Server](../../includes/applies-to-version/sqlserver.md)]

  SQL Server XTP 트랜잭션 로그 성능 개체에는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]의 메모리 내 OLTP 트랜잭션 로그 작업에 관련된 카운터가 포함됩니다.  
  
 이 표에서는 **SQL Server XTP 트랜잭션 로그** 카운터에 대해 설명합니다.  
  
|카운터|Description|  
|-------------|-----------------|  
|**Log bytes written/sec**|메모리 내 OLTP 엔진에 의해 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 트랜잭션 로그에 기록된 초당 바이트 수입니다(평균).|  
|**Log records written/sec**|메모리 내 OLTP 엔진에 의해 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 트랜잭션 로그에 기록된 초당 레코드 수입니다(평균).|  
  
## <a name="see-also"></a>참고 항목  
 [SQL Server XTP&#40;메모리 내 OLTP&#41; 성능 카운터](../../relational-databases/performance-monitor/sql-server-xtp-in-memory-oltp-performance-counters.md)  
  
  
