---
title: "CLR 트리거 만들기 | Microsoft Docs"
ms.custom: ""
ms.date: "03/16/2017"
...
-   .NET Framework 지원 언어로 트리거를 클래스로 정의합니다. CLR에서 트리거를 프로그래밍하는 방법은 [CLR 트리거](../Topic/CLR%20Triggers.md)를 참조하세요. 그런 다음 적절한 언어 컴파일러를 사용하여 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)]에서 어셈블리를 빌드하기 위한 클래스를 컴파일합니다.  
...
> [!NOTE]  
>  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]의 CLR 코드 실행 기능은 기본적으로 해제되어 있습니다. 관리 코드 모듈을 참조하는 데이터베이스 개체를 만들고 변경하고 삭제할 수 있지만 [sp_configure(Transact-SQL)](../../relational-databases/system-stored-procedures/sp-configure-transact-sql.md)을 사용하여 [clr enabled 옵션](../../database-engine/configure-windows/clr-enabled-server-configuration-option.md)을 설정하지 않는 한 이러한 참조는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 실행되지 않습니다.  
