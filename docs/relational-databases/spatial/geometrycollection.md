...
ms.topic: "article"
helpviewer_keywords: 
  - "GeomCollection geometry 하위 유형 [SQL Server]"
...
```  
 ...
 다음 예에서는 **GeometryCollection** 인스턴스의 **LinesString** 인스턴스가 허용되지 않으므로 `System.FormatException`이 발생합니다.  
...
## 예  
 다음 예제에서는 `Point` 인스턴스 및 `Polygon` 인스턴스가 들어 있는 SRID 1의 Z 값으로 `geometry``GeometryCollection`을 인스턴스화합니다.  
