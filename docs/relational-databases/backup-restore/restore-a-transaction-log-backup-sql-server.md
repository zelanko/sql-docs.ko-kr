...
description: 이 문서에서는 SQL Server에서 SQL Server Management Studio 또는 Transact-SQL을 사용하여 트랜잭션 로그 백업을 복원하는 방법을 설명합니다.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
-   백업은 만든 순서대로 복원해야 합니다. 특정 트랜잭션 로그 백업을 복원하려면 먼저 커밋되지 않은 트랜잭션을 롤백하지 않고, 즉 WITH NORECOVERY로 다음과 같은 이전 백업을 복원해야 합니다.  
    ...
    -   전체 데이터베이스 백업이나 차등 백업 이후(복원하는 경우), 특정 트랜잭션 로그 백업 이전에 수행된 전체 트랜잭션 로그 백업 로그 백업이 로그 체인에 따라 간격 없이 생성된 순서대로 적용되어야 합니다.  
         ...
         트랜잭션 로그 백업에 대한 자세한 내용은 [트랜잭션 로그 백업&#40;SQL Server&#41;](../../relational-databases/backup-restore/transaction-log-backups-sql-server.md) 및 [트랜잭션 로그 백업 적용&#40;SQL Server&#41;](../../relational-databases/backup-restore/apply-transaction-log-backups-sql-server.md)을 참조하세요.  
...
1.  RESTORE LOG 문을 실행하여 트랜잭션 로그 백업을 적용합니다. 이때 다음을 지정합니다.  
    ...
    -   NORECOVERY 절  
     ...
     여기서 *database_name* 은 데이터베이스의 이름이고 &lt;backup_device&gt;는 복원 중인 로그 백업이 포함된 디바이스의 이름입니다.  
