...
1. **백업 유형** 드롭다운 목록에서 **전체**를 선택합니다. 
1. **백업 구성 요소** 아래에서 **데이터베이스**를 선택합니다. 
1. **대상** 섹션에서 **백업할 위치** 드롭다운 목록을 사용하여 백업 대상을 선택합니다. **추가**를 클릭하여 다른 백업 개체 및/또는 대상을 추가합니다. 백업 대상을 제거하려면 해당 대상을 선택한 다음 **제거**를 클릭합니다. 기존 백업 대상의 내용을 보려면 선택한 다음 **내용**을 클릭합니다.
1. (선택 사항) **미디어 옵션** 및 **백업 옵션** 페이지에서 다른 사용 가능한 설정을 검토합니다. 다양한 백업 옵션에 대한 자세한 정보는 [일반 페이지](back-up-database-general-page.md), [미디어 옵션 페이지](back-up-database-media-options-page.md) 및 [백업 옵션 페이지](back-up-database-backup-options-page.md)를 참조하세요. 
...
- URL에 백업할 경우 **미디어 덮어쓰기** 옵션은 **미디어 옵션** 페이지에서 사용하지 못할 수 있습니다. 
...
이 예제에서는 `SQLTestDB` 데이터베이스가 `F:\MSSQL\BAK`에서 디스크로 백업됩니다.  `SQLTestDB` 의 이전 백업은 수행되었습니다.
1.  **개체 탐색기**에서 SQL Server 데이터베이스 엔진의 인스턴스에 연결한 다음 해당 인스턴스를 확장합니다.
...
6.  `F:\MSSQL\BAK\Sales_20160801.bak` 파일 이름 **입력란에** 을 입력합니다.
7.  **확인**을 선택합니다.
8.  **확인**을 선택합니다.
...
1.  **개체 탐색기**에서 SQL Server 데이터베이스 엔진의 인스턴스에 연결한 다음 해당 인스턴스를 확장합니다.
1. **새 쿼리** 창을 열고 다음 명령을 실행하여 `SQLTestDB` 데이터베이스 안에 [**데이터베이스 마스터 키**](../../relational-databases/security/encryption/create-a-database-master-key.md)와 [**인증서**](../../t-sql/statements/create-certificate-transact-sql.md)를 만듭니다 
...
1.  **개체 탐색기**에서 **데이터베이스**를 확장하고 `SQLTestDB`를 마우스 오른쪽 단추로 클릭한 다음, **작업**을 가리키고 **백업...** 을 클릭합니다.
...
#### <a name="striped-backup-to-url-and-a-sql-server-credential-already-exists"></a>URL로 스트라이프 백업 및 SQL Server 자격 증명에 이미 있는 경우
...
#### <a name="a-shared-access-signature-exists-and-a-sql-server-credential-does-not-exist"></a>공유 액세스 서명이 있고 SQL Server 자격 증명이 없는 경우
...
#### <a name="a-shared-access-signature-does-not-exist"></a>공유 액세스 서명이 없는 경우
