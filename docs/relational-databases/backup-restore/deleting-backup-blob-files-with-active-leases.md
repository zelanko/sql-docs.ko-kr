...
description: SQL Server 백업 또는 복원이 실패하면 Azure Storage의 Blob이 분리될 수 있습니다. 분리된 Blob을 삭제하는 방법을 알아봅니다.
ms.custom: ''
ms.date: 08/17/2017
ms.prod: sql
...
임대 유형에 대한 자세한 내용은 이 [문서](/rest/api/storageservices/Lease-Blob)를 참조하세요.  
...
1. **임대 해제:** 권한 있는 요청은 임대 ID를 제공하지 않고 임대를 해제할 수 있습니다. 자세한 내용은 [여기](/rest/api/storageservices/Lease-Blob) 를 참조하십시오.  
...
> [!IMPORTANT]
> PowerShell 2.0을 실행하는 경우 Microsoft WindowsAzure.Storage.dll 어셈블리를 로드하는 데 문제가 있을 수 있습니다. 문제 해결을 위해 [PowerShell](/powershell/)을 업그레이드하는 것이 좋습니다. 다음 해결 방법을 사용하여 다음과 같이 powershell.exe.config 파일을 만들거나 수정하여 런타임에 .NET 2.0 및 .NET 4.0 어셈블리를 로드할 수도 있습니다.  
>
> ```xml
> <?xml version="1.0"?>
>     <configuration>
