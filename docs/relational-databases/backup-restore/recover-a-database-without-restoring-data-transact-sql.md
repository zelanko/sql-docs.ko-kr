---
title: "데이터를 복원하지 않고 데이터베이스 복구(Transact-SQL) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
helpviewer_keywords: 
  - "복원 [SQL Server], 복구 전용"
  - "복구 전용 시나리오 [SQL Server]"
  - "데이터베이스 복원 [SQL Server], 복구 전용"
  - "복구 [SQL Server], 복구 전용"
  - "데이터베이스 복구 [SQL Server]"
  - "데이터베이스 복원 [SQL Server], 복구 전용"
  - "복구 [SQL Server], 데이터를 복원하지 않고"
...
-   데이터베이스가 대기 모드에 있고 다른 로그 백업을 적용하지 않고 데이터베이스를 업데이트할 수 있게 하려는 경우  
 ...
 복구 전용 데이터베이스 복원의 [RESTORE](../Topic/RESTORE%20\(Transact-SQL\).md) 구문은 다음과 같습니다.  
...
## 복구 전용 파일 복원  
 ...
 복구 전용 파일 복원의 [RESTORE](../Topic/RESTORE%20\(Transact-SQL\).md) 구문은 다음과 같습니다.  
