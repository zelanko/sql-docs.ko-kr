---
title: "복구 모델(SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "07/16/2016"
...
helpviewer_keywords: 
  ...
  - "대량 로그 복구 모델 [SQL Server]"
  - "복구 [SQL Server], 복구 모델"
  - "트랜잭션 로그 복원 [SQL Server], 복구 모델"
  - "데이터베이스 백업 [SQL Server], 복구 모델"
  - "복구 모델 [SQL Server], 정보"
  ...
  - "백업 [SQL Server], 복구 모델"
  - "복구 모델 [SQL Server]"
  - "복구 모델 [SQL Server], 트랜잭션 로그 관리"
  - "데이터베이스 복원 [SQL Server], 복구 모델"
  - "트랜잭션 로그 복원 [SQL Server]"
  - "로그 [SQL Server], 복구 모델"
  - "데이터베이스 복원 [SQL Server], 복구 모델"
  - "전체 복구 모델 [SQL Server]"
  - "트랜잭션 로그 백업 [SQL Server], 복구 모델"
...
|--------------------|-----------------|------------------------|-------------------------------|  
|**간단**|로그 백업 없음<br /><br /> 로그 공간을 자동으로 회수하여 공간 요구 사항을 적게 유지함으로써 트랜잭션 로그 공간을 관리할 필요가 없도록 함 단순 복구 모델에서의 데이터베이스 백업에 대한 자세한 내용은 [전체 데이터베이스 백업&#40;SQL Server&#41;](../../relational-databases/backup-restore/full-database-backups-sql-server.md)을 참조하세요.<br /><br /> 트랜잭션 로그 백업이 필요한 작업에는 단순 복구 모델이 지원되지 않습니다. 다음 기능은 단순 복구 모드에서 사용할 수 없습니다.<br /><br /> -로그 전달<br /><br /> -Always On 또는 데이터베이스 미러링<br /><br /> -데이터 손실 없는 미디어 복구<br /><br /> -지정 시간 복원|가장 최근 백업 이후의 변경 내용은 보호되지 않음. 재해가 발생할 경우 이러한 변경 내용을 다시 실행해야 함|백업의 끝으로만 복구 가능 자세한 내용은 [전체 데이터베이스 복원&#40;단순 복구 모델&#41;](../../relational-databases/backup-restore/complete-database-restores-simple-recovery-model.md)을 참조하세요. <br><br> 단순 복구 모델에 대한 자세한 내용은 [MSSQLTips!](https://www.mssqltips.com)의 동료들이 제공한 [SQL Server 단순 복구 모델](https://www.mssqltips.com/sqlservertutorial/4/sql-server-simple-recovery-model/)을 참조하세요.|  
