...
> [!IMPORTANT]
>  매개 **@keepfulltextindexfile** 변수는 이후 버전의 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 제거 될 예정입니다. 새 개발 작업에서는 이 매개 변수를 사용하지 말고 현재 이 매개 변수를 사용하는 애플리케이션은 가능한 한 빨리 수정하십시오.  
...
## <a name="obtaining-exclusive-access"></a>배타적 액세스 권한 얻기  
 ...
 예를 들어 다음 `ALTER DATABASE` 문은 현재 사용자가 모두 데이터베이스와의 연결을 끊은 후 [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] 데이터베이스에 대 한 단독 액세스 권한을 얻습니다.  
