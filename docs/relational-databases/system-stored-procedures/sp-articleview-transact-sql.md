...
## <a name="remarks"></a>설명  
 **sp_articleview** 게시 된 아티클을 정의 하는 뷰를 만들고이 뷰의 ID를 [sysarticles &#40;transact-sql&#41;](../../relational-databases/system-tables/sysarticles-transact-sql.md) 테이블의 **sync_objid** 열에 삽입 하 고, restriction 절의 텍스트를 **filter_clause** 열에 삽입 합니다. 모든 열이 복제 되 고 **filter_clause**없으면 [sysarticles &#40;transact-sql&#41;](../../relational-databases/system-tables/sysarticles-transact-sql.md) 테이블의 **SYNC_OBJID** 는 기본 테이블의 ID로 설정 되 고 **sp_articleview** 사용은 필요 하지 않습니다.  
  
 수직 필터링 된 테이블을 게시 하려면 (즉, 열을 필터링 하려면) 먼저 *sync_object* 매개 변수 없이 **sp_addarticle** 를 실행 하 고, 복제할 각 열에 대해 [sp_articlecolumn &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articlecolumn-transact-sql.md) (수직 필터 정의)을 실행 한 다음 **sp_articleview** 를 실행 하 여 게시 된 아티클을 정의 하는 뷰를 만듭니다.  
  
 행을 필터링 하기 위해 행 필터링 된 테이블을 게시 하려면 *필터* 매개 변수 없이 [transact-sql&#41;&#40;sp_addarticle](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md) 를 실행 합니다. *Filter_clause*를 비롯 한 모든 매개 변수를 제공 하 여 [sp_articlefilter &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articlefilter-transact-sql.md)를 실행 합니다. 그런 다음 **sp_articleview**를 실행 하 여 동일한 *filter_clause*를 포함 하는 모든 매개 변수를 제공 합니다.  
  
 수직 및 수평 필터링 된 테이블을 게시 하려면 *sync_object* 또는 *필터* 매개 변수 없이 [transact-sql&#41;&#40;sp_addarticle](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md) 를 실행 합니다. 복제 될 각 열에 대해 [sp_articlecolumn &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articlecolumn-transact-sql.md) 한 번씩 실행 한 다음 [sp_articlefilter &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articlefilter-transact-sql.md) 및 **sp_articleview**를 실행 합니다.  
  
 아티클이 이미 게시 된 아티클을 정의 하는 뷰를 포함 하는 경우 **sp_articleview** 기존 뷰를 삭제 하 고 새 뷰를 자동으로 만듭니다. 뷰**를 수동으로 만든 경우 (** [sysarticles &#40;transact-sql&#41;](../../relational-databases/system-tables/sysarticles-transact-sql.md) **5**) 기존 보기는 삭제 되지 않습니다.  
  
 게시 된 아티클을 수동으로 정의 하는 보기 및 사용자 지정 필터 저장 프로시저를 만드는 경우 **sp_articleview**를 실행 하지 마십시오. 대신 이러한 매개 변수를 해당 *형식* 값과 함께 [transact-sql&#41;&#40;sp_addarticle ](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md)하는 *필터* 및 *sync_object* 매개 변수로 제공 합니다.  
