...
> [!IMPORTANT]  
> [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]의 잠금에 대 한 정보를 얻으려면 [dm_tran_locks](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md) 동적 관리 뷰를 사용 합니다.  
...
`[ @spid1 = ] 'session ID1'`은 사용자가 잠금 정보를 필요로 하는 **dm_exec_sessions** 의 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 세션 ID 번호입니다. *SESSION ID1* 는 **int** 이며 기본값은 NULL입니다. **Sp_who** 를 실행 하 여 세션에 대 한 프로세스 정보를 가져옵니다. *SESSION ID1* 를 지정 하지 않으면 모든 잠금에 대 한 정보가 표시 됩니다.  
  
`[ @spid2 = ] 'session ID2'`은 *세션 ID1* 와 동시에 잠금이 있을 수 있으며 사용자가 정보를 얻고자 하는 dm_exec_sessions의 다른 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 세션 ID 번호입니다 **.** *SESSION ID2* 는 **int** 이며 기본값은 NULL입니다.  
  
## <a name="return-code-values"></a>반환 코드 값  
 0(성공)  
  
## <a name="result-sets"></a>결과 집합  
 **Sp_lock** 결과 집합에는 **\@spid1** 및 **\@spid2** 매개 변수에 지정 된 세션이 보유 한 각 잠금에 대 한 행이 하나씩 포함 되어 있습니다. **\@spid1** 또는 **\@spid2** 를 모두 지정 하지 않으면 결과 집합은 [!INCLUDE[ssDE](../../includes/ssde-md.md)]인스턴스에서 현재 활성 상태인 모든 세션에 대 한 잠금을 보고 합니다.  
...
|**Resource**|**nchar(32)**|잠긴 리소스를 식별하는 값입니다. 값의 형식은 **유형** 열에서 식별 되는 리소스 유형에 따라 달라 집니다.<br /><br /> **유형** 값: **리소스** 값<br /><br /> RID: fileid: pagenumber: rid 형식의 식별자입니다. 여기서 fileid는 페이지가 포함 된 파일을 식별 하 고 pagenumber는 행이 포함 된 페이지를 식별 하며 rid는 페이지의 특정 행을 식별 합니다. fileid는 **database_files** 카탈로그 뷰의 **file_id** 열과 일치 합니다.<br /><br /> 키: [!INCLUDE[ssDE](../../includes/ssde-md.md)]에서 내부적으로 사용 되는 16 진수입니다.<br /><br /> PAG: fileid: pagenumber 형식의 숫자입니다. 여기서 fileid는 페이지가 포함 된 파일을 식별 하 고 pagenumber는 페이지를 식별 합니다.<br /><br /> EXT: 익스텐트의 첫 번째 페이지를 식별 하는 번호입니다. 이 번호의 형식은 fileid:pagenumber입니다.<br /><br /> 탭: **ObjId** 열에서 테이블이 이미 식별 되었으므로 정보가 제공 되지 않습니다.<br /><br /> DB: **dbid** 열에서 데이터베이스가 이미 식별 되었으므로 정보가 제공 되지 않았습니다.<br /><br /> FIL: **database_files** 카탈로그 뷰의 **file_id** 열과 일치 하는 파일의 식별자입니다.<br /><br /> 앱: 잠겨 있는 응용 프로그램 리소스에 고유한 식별자입니다. DbPrincipleId:\<형식에서\<해시 된 값 > > 리소스 문자열의 처음 2 ~ 16 자입니다.<br /><br /> MD: 리소스 유형에 따라 달라집니다. 자세한 내용은 [ &#40;dm_tran_locks transact-sql&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md)의 **resource_description** 열에 대 한 설명을 참조 하십시오.<br /><br /> HBT: 제공 된 정보가 없습니다. 대신 **dm_tran_locks** 동적 관리 뷰를 사용 하십시오.<br /><br /> AU: 제공 된 정보가 없습니다. 대신 **dm_tran_locks** 동적 관리 뷰를 사용 하십시오.|  
