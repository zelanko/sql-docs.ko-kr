...
`[ @spid2 = ] 'session ID2'` 다른 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 의 세션 ID 번호 **sys.dm_exec_sessions** 와 동일한 시간에 잠금을 해야 할 수 있는 *session ID1* 에 대 한 사용자도는 정보입니다. *세션 ID2* 됩니다 **int** 이며 기본값은 NULL입니다.  
  
## <a name="return-code-values"></a>반환 코드 값  
 0(성공)  
  
## <a name="result-sets"></a>결과 집합  
 **sp_lock** 결과 집합에 지정 된 세션에서 보유 한 각 하나의 행을 포함 합니다 **@spid1** 하 고 **@spid2** 매개 변수입니다. 모두 **@spid1** 나 **@spid2** 를 지정 하면 결과 설정 보고서 잠금을 모든 세션에 대 한 현재 활성 상태인 인스턴스는 [!INCLUDE[ssDE](../../includes/ssde-md.md)]합니다.  
...
|**형식**|**nchar(4)**|잠금 유형입니다.<br /><br /> RID = RID(행 식별자)로 식별되는 테이블의 단일 행에 대한 잠금입니다.<br /><br /> KEY = 직렬화할 수 있는 트랜잭션에서 키의 범위를 보호하는 인덱스 내의 잠금입니다.<br /><br /> PAG = 데이터 또는 인덱스 페이지에 대한 잠금입니다.<br /><br /> EXT = 익스텐트에 대한 잠금입니다.<br /><br /> TAB = 모든 데이터와 인덱스가 포함된 전체 테이블에 대한 잠금입니다.<br /><br /> DB = 데이터베이스에 대한 잠금입니다.<br /><br /> FIL = 데이터베이스 파일에 대한 잠금입니다.<br /><br /> APP = 애플리케이션이 지정한 리소스에 대한 잠금입니다.<br /><br /> MD = 메타데이터 또는 카탈로그 정보에 대한 잠금입니다.<br /><br /> HBT = 힙 또는 B-트리 인덱스에 대한 잠금입니다. 이 정보는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서는 완전하지 않습니다.<br /><br /> AU = 할당 단위에 대한 잠금입니다. 이 정보는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서는 완전하지 않습니다.|  
|**Resource**|**nchar(32)**|잠긴 리소스를 식별하는 값입니다. 값 형식은에서 식별 된 리소스의 유형에 따라 달라 집니다 합니다 **형식** 열:<br /><br /> **형식** 값: **리소스** 값<br /><br /> RID: Fileid: pagenumber 형식의 식별자: fileid는 페이지가 포함 된 파일을 식별 하는 위치를 제거 pagenumber는 행이 포함 된 페이지를 식별 하며 rid는 페이지에서 특정 행을 식별 합니다. fileid 일치 하는 **file_id** 열에는 **sys.database_files** 카탈로그 뷰.<br /><br /> 키: 내부적으로 사용 하는 16 진수 숫자는 [!INCLUDE[ssDE](../../includes/ssde-md.md)]합니다.<br /><br /> PAG: 숫자의 형식은 fileid: pagenumber, 여기서 fileid는 페이지가 포함 된 파일을 식별 하 고 pagenumber는 페이지를 식별 합니다.<br /><br /> EXT: 범위에서 첫 번째 페이지를 식별 하는 번호입니다. 이 번호의 형식은 fileid:pagenumber입니다.<br /><br /> 탭: 제공 된 테이블에서 이미 식별 되었으므로 정보가 없습니다 합니다 **ObjId** 열입니다.<br /><br /> DB: 정보가 데이터베이스에서 이미 식별 되었으므로 제공 되지 않습니다 합니다 **dbid** 열입니다.<br /><br /> 파일: 일치 하는 파일의 식별자를 **file_id** 열에는 **sys.database_files** 카탈로그 뷰.<br /><br /> 앱: 잠긴 응용 프로그램 리소스의 고유 식별자입니다. 형식은 DbPrincipleId에서:\<16 자 리소스 문자열의 처음 두 >\<해시 된 값 >입니다.<br /><br /> MD: 리소스 유형에 따라 달라집니다. 자세한 내용은 참조에 대 한 설명을 **resource_description** 열에서 [sys.dm_tran_locks &#40;TRANSACT-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md).<br /><br /> HBT: 정보가 제공 되지 않습니다. 사용 된 **sys.dm_tran_locks** 동적 관리 뷰를 대신 합니다.<br /><br /> AU: 정보가 제공 되지 않습니다. 사용 된 **sys.dm_tran_locks** 동적 관리 뷰를 대신 합니다.|  
...
|**상태**|**nvarchar(5)**|잠금 요청 상태입니다.<br /><br /> CNVRT: 다른 모드에서 변환 되는 잠금 있지만 변환이 충돌 하는 모드를 사용 하 여 잠금을 보유 하는 다른 프로세스에 의해 차단 되었습니다.<br /><br /> 권한 부여: 잠금을 획득 했습니다.<br /><br /> 대기 시간: 잠금 충돌 하는 모드를 사용 하 여 잠금을 보유 하는 다른 프로세스에 의해 차단 됩니다.|  
