...
## <a name="arguments"></a>인수  
 [  **@spid1 =** ] **'***session ID1***'**  
 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 의 세션 ID 번호 **sys.dm_exec_sessions** 는 사용자가 잠금 정보입니다. *session ID1* 됩니다 **int** 이며 기본값은 NULL입니다. 실행할 **sp_who** 세션에 대 한 프로세스 정보를 얻을 수 있습니다. 하는 경우 *session ID1* 지정 하지 않으면 모든 잠금에 대 한 정보가 표시 됩니다.  
  
 [  **@spid2 =** ] **'***세션 ID2***'**  
 다른 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 의 세션 ID 번호 **sys.dm_exec_sessions** 와 동일한 시간에 잠금을 해야 할 수 있는 *session ID1* 에 대 한 사용자도는 정보입니다. *세션 ID2* 됩니다 **int** 이며 기본값은 NULL입니다.  
...
|**리소스**|**nchar(32)**|잠긴 리소스를 식별하는 값입니다. 값 형식은에서 식별 된 리소스의 유형에 따라 달라 집니다 합니다 **형식** 열:<br /><br /> **형식** 값: **리소스** 값<br /><br /> RID: fileid: pagenumber 형식의 식별자: fileid는 페이지가 포함 된 파일을 식별 하는 위치를 제거 pagenumber는 행이 포함 된 페이지를 식별 하며 rid는 페이지에서 특정 행을 식별 합니다. fileid 일치 하는 **file_id** 열에는 **sys.database_files** 카탈로그 뷰.<br /><br /> 키: 내부적으로 사용 되는 16 진수는 [!INCLUDE[ssDE](../../includes/ssde-md.md)]합니다.<br /><br /> PAG: 숫자로 형식 fileid: pagenumber, 여기서 fileid는 페이지가 포함 된 파일을 식별 하 고 pagenumber는 페이지를 식별 합니다.<br /><br /> EXT: 범위에서 첫 번째 페이지를 나타내는 숫자입니다. 이 번호의 형식은 fileid:pagenumber입니다.<br /><br /> 탭: 정보 테이블에서 이미 식별 되었으므로 제공 합니다 **ObjId** 열입니다.<br /><br /> DB: 정보를 데이터베이스에서 이미 식별 되었으므로 제공 합니다 **dbid** 열입니다.<br /><br /> FIL: 일치 하는 파일의 식별자를 **file_id** 열에는 **sys.database_files** 카탈로그 뷰.<br /><br /> : 앱의 고유 식별자 잠긴 응용 프로그램 리소스입니다. 형식은 DbPrincipleId에서:\<16 자 리소스 문자열의 처음 두 >\<해시 된 값 >입니다.<br /><br /> MD: 리소스 유형에 따라 달라집니다. 자세한 내용은 참조에 대 한 설명을 **resource_description** 열에서 [sys.dm_tran_locks &#40;TRANSACT-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md).<br /><br /> HBT: 정보가 제공 되지 않습니다. 사용 된 **sys.dm_tran_locks** 동적 관리 뷰를 대신 합니다.<br /><br /> AU: 정보가 제공 되지 않습니다. 사용 된 **sys.dm_tran_locks** 동적 관리 뷰를 대신 합니다.|  
...
|**상태**|**nvarchar(5)**|잠금 요청 상태입니다.<br /><br /> CNVRT:는 잠금이 다른 모드에서 변환 되는 있지만 변환이 충돌 하는 모드를 사용 하 여 잠금을 보유 하는 다른 프로세스에 의해 차단 되었습니다.<br /><br /> 권한 부여: 잠금을 획득 했습니다.<br /><br /> WAIT: 잠금 충돌 하는 모드를 사용 하 여 잠금을 보유 하는 다른 프로세스에 의해 차단 됩니다.|  
