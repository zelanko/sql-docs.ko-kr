...
> [!IMPORTANT]  
> [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)] @no__t의 잠금에 대 한 정보를 가져오려면, [dm](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md) _astrstrrg_l 동적 관리 뷰를 사용 합니다.  
...
`[ @spid1 = ] 'session ID1'`은 사용자가 잠금 정보를 필요로 하는 **_exec_sessions** 의 @no__t 1 세션 ID 번호입니다. *SESSION ID1* 는 **int** 이며 기본값은 NULL입니다. 세션에 대 한 프로세스 정보를 가져오려면 **sp_who** 를 실행 합니다. *SESSION ID1* 를 지정 하지 않으면 모든 잠금에 대 한 정보가 표시 됩니다.  
  
`[ @spid2 = ] 'session ID2'`은 *세션 ID1* 와 동시에 잠금을 가질 수 있는 **_exec_sessions** 의 또 다른 @no__t 1 세션 ID 번호입니다. *SESSION ID2* 는 **int** 이며 기본값은 NULL입니다.  
  
## <a name="return-code-values"></a>반환 코드 값  
 0(성공)  
  
## <a name="result-sets"></a>결과 집합  
 **Sp_lock** 결과 집합에는 **\@spid1** **@no__t 및 4spid2** 매개 변수에 지정 된 세션에서 보유 한 각 잠금에 대 한 행이 하나씩 포함 되어 있습니다. **@No__t-1spid1** **@no__t 또는 3spid2** 를 모두 지정 하지 않은 경우에는 결과 집합에서 현재 @no__t 활성화 된 모든 세션에 대 한 잠금을 보고 합니다.-4입니다.  
...
|**Resource**|**nchar(32)**|잠긴 리소스를 식별하는 값입니다. 값의 형식은 **유형** 열에서 식별 되는 리소스 유형에 따라 달라 집니다.<br /><br /> **유형** 기본값 **리소스** 기본값<br /><br /> RID: Fileid: pagenumber: rid 형식의 식별자입니다. 여기서 fileid는 페이지가 포함 된 파일을 식별 하 고 pagenumber는 행이 포함 된 페이지를 식별 하며 rid는 페이지의 특정 행을 식별 합니다. fileid는 **database_files** 카탈로그 뷰의 **file_id** 열과 일치 합니다.<br /><br /> 키인지 @No__t-0에 의해 내부적으로 사용 되는 16 진수입니다.<br /><br /> PAG Fileid: pagenumber 형식의 숫자입니다. 여기서 fileid는 페이지가 포함 된 파일을 식별 하 고 pagenumber는 페이지를 식별 합니다.<br /><br /> 확장 범위에서 첫 번째 페이지를 식별 하는 번호입니다. 이 번호의 형식은 fileid:pagenumber입니다.<br /><br /> 5D **ObjId** 열에서 테이블이 이미 식별 되었으므로 정보가 제공 되지 않습니다.<br /><br /> DB: 데이터베이스가 **dbid** 열에서 이미 식별 되었으므로 정보가 제공 되지 않습니다.<br /><br /> FIL **Database_files** 카탈로그 뷰의 **file_id** 열과 일치 하는 파일의 식별자입니다.<br /><br /> 다운로드 잠겨 있는 응용 프로그램 리소스에 고유한 식별자입니다. DbPrincipleId: \<은 \<의 해시 된 값 > >.<br /><br /> MD: 리소스 유형에 따라 달라집니다. 자세한 내용은 resource_description [열에서 &#40;&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md) 열에 대 한 설명을 참조 하십시오.<br /><br /> HBT: 제공 된 정보가 없습니다. 대신, cbb\\ststststststststrstrgggs<br /><br /> AU 제공 된 정보가 없습니다. 대신, cbb\\ststststststststrstrgggs|  
...
|**상태**|**nvarchar(5)**|잠금 요청 상태입니다.<br /><br /> CNVRT: 잠금이 다른 모드에서 변환 되 고 있지만 충돌 하는 모드의 잠금을 보유 하는 다른 프로세스에 의해 변환이 차단 됩니다.<br /><br /> 부여 잠금을 가져왔습니다.<br /><br /> 대기한 충돌 하는 모드의 잠금을 보유 하는 다른 프로세스에 의해 잠금이 차단 되었습니다.|  
  
## <a name="remarks"></a>설명  
 다음과 같은 방법으로 읽기 작업의 잠금을 제어할 수 있습니다.  
  
-   SET TRANSACTION ISOLATION LEVEL을 사용하여 세션의 잠금 수준을 지정합니다. 구문 및 제한 사항에 대해서는 [트랜잭션 격리 수준 &#40;설정 transact-sql&#41;](../../t-sql/statements/set-transaction-isolation-level-transact-sql.md)을 참조 하세요.  
...
## <a name="see-also"></a>관련 항목  
 ...
 [_os_tasks &#40;transact-sql&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-os-tasks-transact-sql.md)   
 [_os_threads &#40;transact-sql&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-os-threads-transact-sql.md)  
