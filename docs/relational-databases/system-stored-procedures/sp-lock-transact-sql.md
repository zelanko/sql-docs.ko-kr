...
|**Resource**|**nchar(32)**|잠긴 리소스를 식별하는 값입니다. 값 형식은에서 식별 된 리소스의 유형에 따라 달라 집니다 합니다 **형식** 열:<br /><br /> **형식** 값: **리소스** 값<br /><br /> RID: fileid:pagenumber:rid 형식의 식별자입니다. 여기서 fileid는 페이지가 포함된 파일을 식별하고 pagenumber는 행이 포함된 페이지를 식별하며 rid는 페이지의 특정 행을 식별합니다. fileid 일치 하는 **file_id** 열에는 **sys.database_files** 카탈로그 뷰.<br /><br /> KEY: [!INCLUDE[ssDE](../../includes/ssde-md.md)]에서 내부적으로 사용되는 16진수입니다.<br /><br /> PAG: fileid:pagenumber 형식의 수입니다. fileid는 페이지가 포함된 파일을 식별하고 pagenumber는 페이지를 식별합니다.<br /><br /> EXT: 익스텐트의 첫 번째 페이지를 식별하는 번호입니다. 이 번호의 형식은 fileid:pagenumber입니다.<br /><br /> TAB: 제공 된 테이블에서 이미 식별 되었으므로 정보가 없습니다 합니다 **ObjId** 열입니다.<br /><br /> DB: 정보가 데이터베이스에서 이미 식별 되었으므로 제공 되지 않습니다 합니다 **dbid** 열입니다.<br /><br /> FIL: 일치 하는 파일의 식별자를 **file_id** 열에는 **sys.database_files** 카탈로그 뷰.<br /><br /> APP: 잠긴 응용 프로그램 리소스의 고유 식별자입니다. 형식은 DbPrincipleId에서:\<16 자 리소스 문자열의 처음 두 >\<해시 된 값 >입니다.<br /><br /> MD: 리소스 유형에 따라 달라집니다. 자세한 내용은 참조에 대 한 설명을 **resource_description** 열에서 [sys.dm_tran_locks &#40;TRANSACT-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md).<br /><br /> HBT: 정보가 제공되지 않습니다. 사용 된 **sys.dm_tran_locks** 동적 관리 뷰를 대신 합니다.<br /><br /> AU: 정보가 제공되지 않습니다. 사용 된 **sys.dm_tran_locks** 동적 관리 뷰를 대신 합니다.|  
...
|**상태**|**nvarchar(5)**|잠금 요청 상태입니다.<br /><br /> CNVRT: 다른 모드에서 잠금을 변환하는 중 충돌하는 모드의 잠금을 보유한 다른 프로세스로 인해 변환이 차단되었습니다.<br /><br /> GRANT: 잠금을 획득했습니다.<br /><br /> WAIT: 충돌하는 모드의 잠금을 보유한 다른 프로세스로 인해 잠금이 차단되었습니다.|  
