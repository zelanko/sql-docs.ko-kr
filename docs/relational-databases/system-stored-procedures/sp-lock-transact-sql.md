...
> [!IMPORTANT]  
> [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)]의 잠금 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]에 대 한 정보를 얻으려면 [dm_tran_locks](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md) 동적 관리 뷰를 사용 합니다.  
...
`[ @spid2 = ] 'session ID2'`은 ( [!INCLUDE[ssDE](../../includes/ssde-md.md)] 는) ID1의 다른 세션 ID 번호입니다 **dm_exec_sessions.** *세션* 을 사용 하는 동시에 잠금이 있을 수 있으며 사용자가 정보를 필요로 하는 경우에도 마찬가지입니다. *SESSION ID2* 는 **int** 이며 기본값은 NULL입니다.  
...
-   FROM 절에서 테이블 잠금 힌트를 사용하여 테이블의 개별 참조에 대한 잠금 수준을 지정합니다. 구문 및 제한 사항에 대해서는 [transact-sql&#41;&#40;테이블 힌트 ](../../t-sql/queries/hints-transact-sql-table.md)를 참조 하세요.  
 ...
 세션과 연결되지 않은 모든 분산 트랜잭션은 분리된 트랜잭션입니다. 
  [!INCLUDE[ssDE](../../includes/ssde-md.md)]에서는 모든 분리된 트랜잭션에 SPID 값으로 -2를 할당하여 사용자가 차단 분산 트랜잭션을 쉽게 식별하도록 합니다. 자세한 내용은 [표시된 트랜잭션을 사용하여 관련 데이터베이스를 일관되게 복구&#40;전체 복구 모델&#41;](../../relational-databases/backup-restore/use-marked-transactions-to-recover-related-databases-consistently.md)을 참조하세요.  
