...
백업 장치에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 발급 하는 최대 입력 또는 출력 요청의 크기 (바이트)를 `[ @max_transfer_size = ] 'max_transfer_size'` 합니다. *max_transfersize* 는 **int** 이며 NULL 일 수 있습니다.  
...
`[ @history_retention_period = ] 'history_retention_period'`은 기록이 유지 되는 시간 (분)입니다. *history_retention_period* 은 **int**입니다. 지정 된 값이 없는 경우에는 1440 값이 사용 됩니다.  
