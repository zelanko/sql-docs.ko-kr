 ...
 *stmt*  
 커서 결과 집합을 정의하는 필수 매개 변수입니다. 모든 문자열 형식 (유니코드, 크기 등)에 대 한 유효한 쿼리 문자열 (구문 및 바인딩)은 유효한 *stmt* 값 형식으로 사용할 수 있습니다.  
...
|0x100000|FAST_FORWARD_ACCEPTABLE|  
 ...
 요청 된 값이 *stmt*에 의해 정의 된 커서에 적합 하지 않을 수 있기 때문에이 매개 변수는 입력 및 출력 모두로 사용 됩니다. 이러한 경우 SQL Server에서 적절한 값을 할당합니다.  
 ...
 *Scrollopt*와 마찬가지로에서는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 요청 된 *ccopt* 값을 재정의할 수 있습니다.  
  
 *개수*  
 AUTO_FETCH에 사용할 인출 버퍼 행 수입니다. 기본값은 20개 행입니다. *행 개수* 는 입력 값과 반환 값으로 할당 될 때 다르게 동작 합니다.  
  
|입력 값일 때|반환 값일 때|  
|--------------------|---------------------|  
|AUTO_FETCH *scrollopt* 값이 지정 된 경우 *행 개수는* 인출 버퍼에 넣을 행 수를 나타냅니다.<br /><br /> 참고: AUTO_FETCH 지정 된 경우 >0은 유효한 값 이지만 무시 됩니다.|*Scrollopt* AUTO_FETCH 값이 지정 된 경우를 제외 하 고 결과 집합의 행 수를 나타냅니다.|  
  
-  
  
 *boundparam*  
 추가 매개 변수의 사용을 나타냅니다. *boundparam* 는 *scrollopt* PARAMETERIZED_STMT 값이 ON으로 설정 된 경우 지정 해야 하는 선택적 매개 변수입니다.  
 ...
 *Rowcount* 매개 변수를 반환 값으로 지정 하면 다음 결과 집합이 발생 합니다.  
  
 -1  
 행 수가 알 수 없는 상태이거나 적합하지 않는 경우 반환됩니다.  
  
 -n  
 비동기 채우기가 적용되는 경우 반환됩니다. *Scrollopt* AUTO_FETCH 값이 지정 될 때 인출 버퍼에 배치 된 행 수를 나타냅니다.  
 ...
 *Stmt* 에서 저장 프로시저의 실행을 지정 하는 경우 입력 매개 변수는 *stmt* 문자열의 일부로 상수로 정의 되거나 *boundparam* 인수로 지정 될 수 있습니다. 이 방법을 사용하면 선언된 변수를 바인딩된 매개 변수로 전달할 수 있습니다.  
...
## <a name="scrollopt-parameter"></a>scrollopt 매개 변수  
 처음 다섯 개의 *scrollopt* 값 (KEYSEY, DYNAMIC, FORWARD_ONLY, STATIC 및 FAST_FORWARD)은 함께 사용할 수 없습니다.  
...
## <a name="ccopt-parameter"></a>ccopt 매개 변수  
 처음 네 개의 값 (READ_ONLY, SCROLL_LOCKS 및 두 개의 *낙관적 값)* 은 함께 사용할 수 없습니다.  
  
> [!NOTE]  
>  처음 4 개의 *ccopt* 값 중 하나를 선택 하면 커서가 읽기 전용인 지, 아니면 잠금 또는 낙관적 메서드를 사용 하 여 업데이트 손실을 방지할 수 있습니다. *Ccopt* 값을 지정 하지 않으면 기본값은 낙관적입니다.  
 ...
 CHECK_ACCEPTED_TYPES ON 이면 마지막 4 개 값 (READ_ONLY_ACCEPTABLE, SCROLL_LOCKS_ACCEPTABLE 및 *OPTIMISTIC_ACCEPTABLE 값 중* 하나) 중 하나 이상이 설정 되어 있어야 합니다.  
  
 위치 지정 업데이트 및 삭제 함수는 인출 버퍼 내 에서만 수행할 수 있으며 *ccopt* 값이 SCROLL_LOCKS 또는 낙관적 인 경우에만 수행할 수 있습니다. 지정된 값이 SCROLL_LOCKS이면 작업 성공이 보장됩니다. 지정된 값이 OPTIMISTIC인 경우 행이 마지막으로 인출된 이후 변경되었으면 작업이 실패합니다.  
...
## <a name="bound_param-parameter"></a>bound_param 매개 변수  
 코드의 오류 메시지에 따라 PARAMETERIZED_STMT 지정 된 경우 매개 변수 이름은 *paramdef* 여야 합니다. PARAMETERIZED_STMT가 지정되지 않은 경우에는 오류 메시지에 이름이 지정되지 않습니다.  
 ...
 *{지역 변수 이름 데이터 형식} [,... 개의*  
 ...
 후속 매개 변수는 문에서 *지역 변수 이름을* 대체할 값을 전달 하는 데 사용 됩니다.  
