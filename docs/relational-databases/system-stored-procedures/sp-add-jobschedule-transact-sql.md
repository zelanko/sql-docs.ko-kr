...
일정이 추가 된 작업의 `[ @job_id = ] job_id` 작업 id 번호입니다. *job_id* 은 **uniqueidentifier**이며 기본값은 없습니다.  
  
일정을 추가할 작업의 이름 `[ @job_name = ] 'job_name'`입니다. *job_name* 은 **nvarchar (128)** 이며 기본값은 없습니다.  
  
> [!NOTE]  
>  *Job_id* 또는 *job_name* 를 지정 해야 하지만 둘 다 지정할 수는 없습니다.  
  
일정의 `[ @name = ] 'name'` 이름입니다. *name* 은 **nvarchar (128)** 이며 기본값은 없습니다.  
  
`[ @enabled = ] enabled_flag`는 일정의 현재 상태를 나타냅니다. *enabled_flag* 은 **tinyint**이며 기본값은 **1** (사용)입니다. **0**인 경우 일정을 사용할 수 없습니다. 일정을 사용하지 않으면 작업이 실행되지 않습니다.  
  
작업이 실행 될 시기를 나타내는 값 `[ @freq_type = ] frequency_type`입니다. *frequency_type* 은 **int**이며 기본값은 **0**이 고 다음 값 중 하나일 수 있습니다.  
...
작업이 실행 되는 `[ @freq_interval = ] frequency_interval` 날짜입니다. *frequency_interval* 은 **int**이며 기본값은 0이 고 다음 표에 나와 있는 것 처럼 *frequency_type* 의 값에 따라 달라 집니다.  
...
각 작업 실행 사이에 발생 하는 *frequency_subday_type* 기간의 `[ @freq_subday_interval = ] frequency_subday_interval` 수입니다. *frequency_subday_interval* 은 **int**이며 기본값은 0입니다.  
  
`[ @freq_relative_interval = ] frequency_relative_interval` *frequency_type* 가 **32** (매월 상대적)로 설정 된 경우 *frequency_interval* 을 추가로 정의 합니다.  
...
예약 된 작업 실행 사이에 발생 하는 주 또는 월 수를 `[ @freq_recurrence_factor = ] frequency_recurrence_factor` 합니다. *frequency_recurrence_factor* 는 *frequency_type* 이 **8**, **16**또는 **32**로 설정 된 경우에만 사용 됩니다. *frequency_recurrence_factor* 은 **int**이며 기본값은 0입니다.  
  
작업 실행을 시작할 수 있는 `[ @active_start_date = ] active_start_date` 날짜입니다. *active_start_date* 는 **int**이며 기본값은 없습니다. 날짜 형식은 YYYYMMDD입니다. *Active_start_date* 설정 된 경우 날짜는 19900101 보다 크거나 같아야 합니다.  
...
작업 실행을 중지할 수 있는 `[ @active_end_date = ] active_end_date` 날짜입니다. *active_end_date* 는 **int**이며 기본값은 없습니다. 날짜 형식은 YYYYMMDD입니다.  
  
작업 실행을 시작 하기 위해 *active_start_date* 와 *active_end_date* 사이의 모든 날짜에 대 한 `[ @active_start_time = ] active_start_time` 시간입니다. *active_start_time* 는 **int**이며 기본값은 없습니다. 시간 형식은 HHMMSS이며 24시간제를 사용합니다.  
  
*active_start_date* 와 *active_end_date* 사이의 모든 날짜에 대 한 시간을 `[ @active_end_time = active_end_time_` 하 여 작업 실행을 종료 합니다. *active_end_time* 는 **int**이며 기본값은 없습니다. 시간 형식은 HHMMSS이며 24시간제를 사용합니다.  
  
일정을 성공적으로 만든 경우 일정에 할당 된 일정 id 번호를 `[ @schedule_id = schedule_idOUTPUT` 합니다. *schedule_id* 은 **int**형식의 출력 변수 이며 기본값은 없습니다.  
...
-   **SQLAgentOperatorRole**  
 ...
 ## <a name="example"></a>예제
 다음 예에서는 매주 토요일 오전 2:00 시에 실행 되는 `SaturdayReports`에 작업 일정을 할당 합니다.
```sql  
