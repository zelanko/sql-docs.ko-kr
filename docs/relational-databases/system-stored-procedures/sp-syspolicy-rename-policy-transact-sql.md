...
`[ @name = ] 'name'` 이름을 바꿀 정책의 이름이입니다. *이름* 됩니다 **sysname**, 경우 지정 해야 합니다 *policy_id* NULL입니다.  
  
`[ @policy_id = ] policy_id` 이름을 바꿀 정책의 식별자가입니다. *policy_id* 됩니다 **int**, 경우 지정 해야 합니다 *이름* NULL입니다.  
  
`[ @new_name = ] 'new_name'` 정책의 새 이름이입니다. *new_name* 됩니다 **sysname**, 이며 반드시 지정 해야 합니다. NULL 또는 빈 문자열일 수 없습니다.  
...
## <a name="remarks"></a>설명  
 ...
 에 대 한 값을 지정 해야 합니다 *이름을* 하거나 *policy_id*합니다. 둘 다 NULL일 수는 없습니다. 이러한 값을 가져오려면 msdb.dbo.syspolicy_policies 시스템 뷰를 쿼리합니다.  
