...
`[ @server = ] 'server'`옵션을 설정할 서버의 이름입니다. *서버* 는 **sysname**이며 기본값은 없습니다.  
...
|**데이터 정렬 이름**|원격 데이터 **정렬 사용** 이 **true** 이 고 데이터 원본이 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터 원본이 아닌 경우 원격 데이터 원본에서 사용 하는 데이터 정렬의 이름을 지정 합니다. 이름은 반드시 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 지원하는 데이터 정렬 중 하나여야 합니다.<br /><br /> 
