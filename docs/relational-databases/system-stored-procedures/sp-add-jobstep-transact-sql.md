...
`[ @job_id = ] job_id`단계를 추가할 작업의 id입니다. *job_id* 는 **uniqueidentifier**이며 기본값은 NULL입니다.  
...
`[ @step_id = ] step_id`작업 단계의 시퀀스 id 번호입니다. 단계 id 번호는 **1** 부터 시작 하 여 간격 없이 증가 합니다. 기존 시퀀스에 단계를 삽입하는 경우에는 시퀀스 번호가 자동으로 조정됩니다. *Step_id* 을 지정 하지 않으면 값이 제공 됩니다. *step_id* 은 **int**이며 기본값은 NULL입니다.  
...
> [!IMPORTANT]  
>  작업 단계에서 사용되는 모든 토큰에 이스케이프 매크로를 사용해야 하며 그렇지 않으면 작업 단계가 실패합니다. 또한 이제 토큰 이름을 괄호로 묶고 토큰 구문의 시작 부분에 달러 기호(`$`)를 사용해야 합니다. 예를 들어 다음과 같은 가치를 제공해야 합니다.  
>   
>  `$(ESCAPE_`*매크로 이름*`(DATE))`  
...
`[ @additional_parameters = ] 'parameters'`매개 변수는 **ntext**이며 기본값은 NULL입니다. [!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]  
...
|**2**|실패로 종료|  
|**3**|다음 단계로 이동|  
|**4**|*On_success_step_id* 단계로 이동 합니다.|  
  
`[ @on_success_step_id = ] success_step_id`단계가 성공 하 고 *success_action* 이 **4**인 경우이 작업을 실행할 단계의 ID입니다. *success_step_id* 은 **int**이며 기본값은 **0**입니다.  
...
|**2** (기본값)|실패로 종료|  
|**3**|다음 단계로 이동|  
|**4**|*On_fail_step_id* 단계로 이동 합니다.|  
  
`[ @on_fail_step_id = ] fail_step_id`단계가 실패 하 고 *fail_action* 가 **4**인 경우이 작업에서 실행할 단계의 ID입니다. *fail_step_id* 은 **int**이며 기본값은 **0**입니다.  
  
`[ @server = ] 'server'`서버는 **nvarchar (30)** 이며 기본값은 NULL입니다. [!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]  
...
`[ @output_file_name = ] 'file_name'`이 단계의 출력이 저장 되는 파일의 이름입니다. *file_name* 는 **nvarchar (200)** 이며 기본값은 NULL입니다. *file_name* 에는 *명령*아래에 나열 된 하나 이상의 토큰이 포함 될 수 있습니다. 이 매개 [!INCLUDE[tsql](../../includes/tsql-md.md)]변수는, **CmdExec**, **PowerShell**, [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]또는 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 하위 시스템에서 실행 되는 명령에만 사용할 수 있습니다.  
...
`[ @proxy_id = ] proxy_id`작업 단계가 실행 되는 프록시의 id입니다. *proxy_id* 은 **int**형식 이며 기본값은 NULL입니다. *Proxy_id* 을 지정 하지 않으면 *proxy_name* 가 지정 되지 않고 *user_name* 이 지정 되지 않습니다. 작업 단계는 에이전트에 대 한 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 서비스 계정으로 실행 됩니다.  
  
`[ @proxy_name = ] 'proxy_name'`작업 단계가 실행 되는 프록시의 이름입니다. *proxy_name* 는 **sysname**형식 이며 기본값은 NULL입니다. *Proxy_id* 을 지정 하지 않으면 *proxy_name* 가 지정 되지 않고 *user_name* 이 지정 되지 않습니다. 작업 단계는 에이전트에 대 한 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 서비스 계정으로 실행 됩니다.  
 ...
 *Proxy_name* 또는 *proxy_id*로 프록시를 식별할 수 있습니다.  
