...
## <a name="arguments"></a>인수  
 ...
 때의 여러 인스턴스 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 설치 된 컴퓨터에서 인스턴스는 별도 서버에 있는 것 처럼을 작동 합니다. 참조 하 여 명명된 된 인스턴스를 지정 *server* 으로 *servername\instancename*합니다.  
  
 [ **@local =** ] **'LOCAL'**  
 로컬 서버로 추가할 서버를 지정합니다. **@local** 됩니다 **varchar(10)**, 기본값은 NULL입니다. 지정 **@local** 으로 **로컬** 정의 **@server** 원인을 확인 하 고 로컬 서버 이름으로는 @@SERVERNAME 값을 반환 하는 함수 *server*합니다.  
  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 설치 프로그램은 설치 중에 이 변수를 컴퓨터 이름으로 설정합니다. 기본적으로 컴퓨터 이름이 방식으로 사용자 인스턴스에 연결할 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 추가 구성이 필요 하지 않고 있습니다.  
  
 로컬 정의는 [!INCLUDE[ssDE](../../includes/ssde-md.md)]을 다시 시작해야 적용됩니다. 각 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 인스턴스에서 로컬 서버는 하나만 정의할 수 있습니다.  
  
 [ **@duplicate_ok =** ] **'duplicate_OK'**  
 서버 이름 중복을 허용할지 여부를 지정합니다. **@duplicate_OK** 됩니다 **varchar(13)**, 기본값은 NULL입니다. **@duplicate_OK** 값을 하나만 사용할 수 있습니다 **duplicate_OK** 또는 NULL입니다. 하는 경우 **duplicate_OK** 지정을 이미 추가 되는 서버 이름이 있으면 오류가 발생 하지 않습니다. 명명 된 매개 변수 사용 되지 않는 경우 **@local** 지정 해야 합니다.  
...
## <a name="examples"></a>예  
 다음 예제에서는 변경 내용을 합니다 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 호스팅하는 컴퓨터의 이름에 대 한 항목이 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에 `ACCOUNTS`입니다.  
