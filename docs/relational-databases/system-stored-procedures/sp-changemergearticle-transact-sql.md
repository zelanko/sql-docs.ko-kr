...
## <a name="arguments"></a>인수  
 ...
 [  **@property=**] **'***속성***'**  
 지정된 아티클 및 게시에 대해 변경할 속성입니다. *속성* 은 **nvarchar (30)**, 수 값 중 하나에 나열 될 테이블입니다.  
  
 [  **@value=**] **'***값***'**  
 지정한 속성의 새 값입니다. *값* 은 **nvarchar (1000)**, 수 값 중 하나에 나열 될 테이블입니다.  
...
|**logical_record_level_conflict_detection**|**true**|논리적 레코드에 변경 사항이 발생하면 충돌이 감지됩니다. 사용 하려면 **logical_record_level_conflict_resolution** 에 대해 설정할 **true**합니다.|  
...
||**0x00**|스냅숏 에이전트의 스크립팅을 사용 하지 않도록 설정 하 고 제공 되는 스크립트를 사용 하 여 **creation_script**합니다.|  
...
||**0x200000000**|날짜 및 시간 데이터 형식으로 변환 (**날짜**, **시간**, **datetimeoffset**, 및 **datetime2**)는에 도입 된 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 이전 버전에서 지원 되는 데이터 형식에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]합니다.|  
 ...
 **1** 병합 아티클에 대 한 변경을 의미 인해 기존 구독이 다시 초기화 해야 할 구독을 다시 초기화할 수 있는 권한을 부여 합니다.  
 ...
 **sp_changemergearticle** 병합 복제에 사용 됩니다.  
 ...
 때문에 **sp_changemergearticle** 하는 데 사용 하 여 초기에 지정 된 아티클 속성을 변경할 [sp_addmergearticle](../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md), 참조 [sp_addmergearticle](../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md) 이러한 속성에 대 한 자세한 내용은 합니다.  
  
 다음 속성을 변경 하려면 새 스냅숏을 생성 해야 하 고의 값을 지정 해야 **1** 에 대 한는 *force_invalidate_snapshot* 매개 변수:  
...
-   **subset_filterclause**  
 ...
 다음 속성을 변경 하려면 기존 구독을 다시 초기화의 값을 지정 해야 **1** 에 대 한는 *force_reinit_subscription* 매개 변수:  
...
-   **logical_record_level_conflict_resolution**  
 ...
 에 대 한 3의 값을 지정 하는 경우 **partition_options**, 메타 데이터를 정리 하는 병합 에이전트가 실행 되 고 분할 된 스냅숏은 더 빨리 만료 될 때마다 합니다. 이 옵션을 사용할 때는 구독자가 요청한 분할된 스냅숏을 활성화해야 합니다. 자세한 내용은 [Snapshots for Merge Publications with Parameterized Filters](../../relational-databases/replication/snapshots-for-merge-publications-with-parameterized-filters.md)을 참조하세요.  
  
 설정할 때의 **column_tracking** 속성에는 테이블이 이미 다른 병합 게시에 게시 된 경우 열 추적 동일 해야 하는이 테이블을 기반으로 하는 기존 아티클이 사용 하는 값으로. 이 매개 변수는 테이블 아티클에만 해당합니다.  
  
 여러 게시에서 동일한 기본 테이블을 기반으로 하는 문서를 게시 하는 경우 변경 된 **delete_tracking** 속성 또는 **compensate_for_errors** 한 아티클에 대 한 속성에 동일한 변경으로 인해 동일한 테이블을 기반으로 하는 다른 문서를 만들 수 있습니다.  
  
 병합 프로세스가 사용하는 게시자 로그인/사용자 계정에 올바른 테이블 사용 권한이 없는 경우 잘못된 변경은 충돌로 기록됩니다.  
  
 값을 변경할 때 **schema_option**, 시스템 비트 단위 업데이트를 수행 하지 않습니다. 즉, 설정한 **schema_option** 를 사용 하 여 **sp_changemergearticle**기존 비트 설정이 해제 될 수 있습니다. 기존 설정을 유지 하려면 수행 해야 [& (비트 AND)](../../t-sql/language-elements/bitwise-and-transact-sql.md) 설정 하는 값과의 현재 값 사이 *schema_option*, 를실행하여얻는[sp_helpmergearticle](../../relational-databases/system-stored-procedures/sp-helpmergearticle-transact-sql.md)합니다.  
  
> [!CAUTION]  
>  때 하면 많은 (아마도 수십만)는 게시의 아티클에 실행 **sp_changemergearticle** 문서 중 하나에 대 한 실행을 완료 하는 데 시간이 오래 걸릴 수 있습니다.  
...
## <a name="permissions"></a>Permissions  
 구성원만는 **sysadmin** 고정된 서버 역할 또는 **db_owner** 고정된 데이터베이스 역할을 실행할 수 있는 **sp_changemergearticle**합니다.  
