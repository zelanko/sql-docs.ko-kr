...
## <a name="remarks"></a>설명  
 **Xp_cmdshell** 로 생성 된 Windows 프로세스는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 서비스 계정과 동일한 보안 권한을 갖습니다.  
...
> [!IMPORTANT]
>  일괄 처리 내에서 **xp_cmdshell** 실행 되 고 오류를 반환 하는 경우 일괄 처리가 실패 합니다. 이 동작은 기존 버전과 달라진 동작입니다. 이전 버전 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 의에서는 일괄 처리가 계속 실행 됩니다.  
...
## <a name="permissions"></a>사용 권한  
 ...
 비관리자가 **xp_cmdshell**를 사용할 수 있도록 하 고 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에서 권한이 낮은 계정의 보안 토큰을 사용 하 여 자식 프로세스를 만들 수 있도록 허용 하려면 다음 단계를 수행 합니다.  
...
2.  **Sp_xp_cmdshell_proxy_account** 시스템 프로시저를 사용 하 여 최소 권한 계정을 사용 하도록 **xp_cmdshell** 를 구성 합니다.  
    ...
    > [!NOTE]  
    >  개체 탐색기에서 서버 이름의 **속성** 을 마우스 오른쪽 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 단추로 클릭 하 고 **서버 프록시 계정** 섹션의 **보안** 탭을 확인 하 여을 사용 하 여이 프록시 계정을 구성할 수도 있습니다.  
  
3.  에서 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]master 데이터베이스를 사용 하 여 특정 `GRANT exec ON xp_cmdshell TO N'<some_user>';` **sysadmin** 이 아닌 사용자에 게 **xp_cmdshell**실행 기능을 제공 하는 문을 실행 합니다. 지정한 사용자가 master 데이터베이스에 있어야 합니다.  
  
 이제 관리자가 아닌 사용자가 **xp_cmdshell** 를 사용 하 여 운영 체제 프로세스를 시작할 수 있으며, 이러한 프로세스는 구성한 프록시 계정의 권한으로 실행 됩니다. CONTROL SERVER 권한이 있는 사용자 ( **sysadmin** 고정 서버 역할의 멤버)는 **xp_cmdshell**에 의해 시작 되는 자식 프로세스 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에 대 한 서비스 계정의 사용 권한을 계속 받습니다.  
