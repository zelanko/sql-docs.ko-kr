...
ms.locfileid: "43035092"
---
# <a name="sysspcdccleanupchangetable-transact-sql"></a>sys.sp_cdc_cleanup_change_table(Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]

  변경 테이블을 기반으로 지정 된 현재 데이터베이스에서 행을 제거 *low_water_mark* 값입니다. 이 저장 프로시저는 변경 테이블 정리 프로세스를 직접 관리하려는 사용자를 위해 제공됩니다. 이 프로시저는 변경 테이블에 있는 데이터의 모든 소비자에게 영향을 주므로 사용할 때 주의해야 합니다.  
...
-   정리 에이전트 작업에서 삭제 실패를 보고하는 경우  
     ...
     관리자는 이 저장 프로시저를 명시적으로 실행하여 실패한 작업을 다시 시도할 수 있습니다. 지정된 된 캡처 인스턴스의 정리를 다시 시도 하려면 sys.sp_cdc_cleanup_change_table을 실행 하 고 NULL을 지정 합니다 @low_water_mark 매개 변수입니다.  
