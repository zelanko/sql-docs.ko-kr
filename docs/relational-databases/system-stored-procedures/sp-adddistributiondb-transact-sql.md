...
`[ @min_distretention = ] min_distretention` 최소 보존 기간 배포 데이터베이스에서 트랜잭션을 삭제 하기 전의 시간입니다. *min_distretention* 됩니다 **int**, 기본값은 0 시간입니다.  
  
`[ @max_distretention = ] max_distretention` 트랜잭션을 삭제 하기 전의 시간을 최대 보존 기간이입니다. *max_distretention* 됩니다 **int**, 기본값: 72 시간입니다. 배포의 최대 보존 기간보다 오래된 복제 명령을 받지 않은 구독은 비활성 상태로 표시되어 다시 초기화해야 합니다. RAISERROR 21011 오류는 각 비활성 구독에 대해 발생됩니다. 값이 **0** 즉 복제 된 트랜잭션을 배포 데이터베이스에 저장 되지 않습니다.  
  
`[ @history_retention = ] history_retention` 기록을 보존할 시간입니다. *history_retention* 됩니다 **int**, 기본값은 48 시간입니다.  
  
`[ @security_mode = ] security_mode` 배포자에 연결할 때 사용할 보안 모드가입니다. *security_mode* 됩니다 **int**, 기본값은 1입니다. **0** 지정 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인증 **1** Windows 통합 인증을 지정 합니다.  
  
`[ @login = ] 'login'` 로그인 이름은 배포자에 연결할 때 배포 데이터베이스를 만들려고 합니다. 이 경우 필요한 *security_mode* 로 설정 된 **0**합니다. *login*은 **sysname**이며 기본값은 NULL입니다.  
  
`[ @password = ] 'password'` 배포자에 연결할 때 암호가 사용 됩니다. 이 경우 필요한 *security_mode* 로 설정 된 **0**합니다. *암호* 됩니다 **sysname**, 기본값은 NULL입니다.  
  
`[ @createmode = ] createmode` *createmode* 됩니다 **int**, 기본값은 1 이며 다음 값 중 하나일 수 있습니다.  
...
`[ @deletebatchsize_xact = ] deletebatchsize_xact` MSRepl_Transactions 테이블에서 만료 된 트랜잭션의 정리 하는 동안 사용할 일괄 처리 크기를 지정 합니다. *deletebatchsize_xact* 됩니다 **int**, 기본값은 5000입니다. 이 매개 변수는 SQL Server 2012 SP4 및 SQL Server 2016 SP2 릴리스 뒤에 SQL Server 2017에 처음 도입 되었습니다.  

`[ @deletebatchsize_cmd = ] deletebatchsize_cmd` MSRepl_Commands 테이블에서 만료 된 명령 중 정리 하는 동안 사용할 일괄 처리 크기를 지정 합니다. *deletebatchsize_cmd* 됩니다 **int**, 기본값은 2000입니다. 이 매개 변수는 SQL Server 2012 SP4 및 SQL Server 2016 SP2 릴리스 뒤에 SQL Server 2017에 처음 도입 되었습니다. 
