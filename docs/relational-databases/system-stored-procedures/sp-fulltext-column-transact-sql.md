---
title: sp_fulltext_column (Transact SQL) | Microsoft 문서
ms.custom: ''
ms.date: 06/10/2016
ms.prod: sql
...
## <a name="arguments"></a>인수  
 ...
 한 부분 또는 두 부분으로 구성된 테이블 이름입니다. 테이블은 반드시 현재 데이터베이스에 있어야 합니다. 테이블에 전체 텍스트 인덱스가 있어야 합니다. *qualified_table_name* 는 **nvarchar(517)**, 기본값이 없고.  
  
 [ **@colname=** ] **'***column_name***'**  
 열 이름 *qualified_table_name*. 열은 문자, 해야 **varbinary (max)** 하거나 **이미지** 열 계산된 열일 수 없습니다. *열 _ 이름* 는 **sysname**, 기본값은 없음입니다.  
...
|값|Description|  
|-----------|-----------------|  
|**add**|추가 하는 *열 _ 이름* 의 *qualified_table_name* 테이블의 비활성 전체 텍스트 인덱스에 있습니다. 이 동작으로 전체 텍스트 인덱싱에 열을 사용할 수 있습니다.|  
|**drop**|제거 *열 _ 이름* 의 *qualified_table_name* 테이블의 비활성 전체 텍스트 인덱스에서.|  
  
 [ **@language=** ] **'***language_term***'**  
 열에 저장된 데이터의 언어입니다. 에 포함 되는 언어 목록은 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]를 참조 하십시오 [sys.fulltext_languages &#40;Transact SQL&#41;](../../relational-databases/system-catalog-views/sys-fulltext-languages-transact-sql.md).  
  
> [!NOTE]  
>  열에 여러 언어 또는 지원되지 않는 언어의 데이터가 있을 때는 중립을 사용하십시오. 기본값은 구성 옵션인 '기본 전체 텍스트 언어'에 의해 지정됩니다.  
  
 [ **@type_colname =** ] **'***type_column_name***'**  
 열 이름 *qualified_table_name* 의 문서 종류를 보유 하는 *열 _ 이름*. 이 열 이어야 합니다 **char**, **nchar**하십시오 **varchar**, 또는 **nvarchar**. 데이터 유형의 경우에 사용 됩니다 *열 _ 이름* 형식이 **varbinary (max)** 또는 **이미지**. *type_column_name* 는 **sysname**, 기본값은 없음입니다.  
...
## <a name="permissions"></a>사용 권한  
 사용자의 구성원 이어야 합니다.는 **db_ddladmin** 고정 데이터베이스 역할의 구성원 또는 **db_owner** 고정 데이터베이스 역할 또는 테이블 소유자입니다.  
