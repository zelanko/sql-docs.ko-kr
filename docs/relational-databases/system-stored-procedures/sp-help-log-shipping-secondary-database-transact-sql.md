...
`[ @secondary_id = ] 'secondary_id'` 로그 전달 구성의 보조 서버에 대 한 ID입니다. *secondary_id* 은 **uniqueidentifier** 이며 NULL 일 수 없습니다.  
...
## <a name="remarks"></a>설명  
 *Secondary_database* 매개 변수를 포함 하는 경우 결과 집합에 해당 보조 데이터베이스에 대 한 정보가 포함 됩니다. *secondary_id* 매개 변수를 포함 하는 경우 결과 집합에 해당 보조 id와 연결 된 모든 보조 데이터베이스에 대 한 정보가 포함 됩니다.  
  
 **sp_help_log_shipping_secondary_database** 는 보조 서버의 **master** 데이터베이스에서 실행 해야 합니다.  
