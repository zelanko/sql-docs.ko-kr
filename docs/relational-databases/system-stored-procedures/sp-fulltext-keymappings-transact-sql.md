 ...
 *table_id*  
 전체 텍스트 인덱싱된 테이블의 개체 ID입니다. 잘못 된을 지정 하는 경우 *table_id*, 오류가 반환 됩니다. 테이블의 개체 ID를 가져오는 방법에 대 한 정보를 참조 하십시오. [OBJECT_ID &#40;TRANSACT-SQL&#41;](../../t-sql/functions/object-id-transact-sql.md)합니다.  
...
|Key|*|지정한 테이블의 전체 텍스트 키 값입니다.<br /><br /> 매핑 테이블에 전체 텍스트 키가 없으면 빈 행 집합이 반환됩니다.|  
 ...
 <sup>*</sup> 키에 대 한 데이터 형식이 기본 테이블의 전체 텍스트 키 열의 데이터 형식과 동일 합니다.  
...
> [!NOTE]  
>  이 예제에 사용 하 여 섹션는 `Production.ProductReview` 목차는 [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] 예제 데이터베이스. 제공 하는 예제를 실행 하 여이 인덱스를 만들 수는 `ProductReview` 테이블에 [CREATE FULLTEXT INDEX &#40;TRANSACT-SQL&#41;](../../t-sql/statements/create-fulltext-index-transact-sql.md)합니다.  
  
### <a name="a-obtaining-all-the-key-and-docid-values"></a>1. 모든 Key 및 DocId 값 가져오기  
 다음 예제에서는 [DECLARE](../../t-sql/language-elements/declare-local-variable-transact-sql.md) 지역 변수를 만들고 문을 `@table_id` 의 ID를 할당할 수는 `ProductReview` 해당 값으로는 테이블입니다. 이 예제를 실행 **sp_fulltext_keymappings** 지정 `@table_id` 에 대 한는 *table_id* 매개 변수입니다.  
  
> [!NOTE]  
>  사용 하 여 **sp_fulltext_keymappings** 으로 *table_id* 매개 변수는 작은 테이블에 적합 합니다.  
...
### <a name="b-obtaining-the-docid-value-for-a-specific-key-value"></a>2. 특정 Key 값에 대한 DocId 값 가져오기  
 다음 예에서는 DECLARE 문을 사용하여 지역 변수 `@table_id`를 만들고 `ProductReview` 테이블의 ID를 해당 값으로 할당합니다. 예제를 실행 **sp_fulltext_keymappings** 지정 `@table_id` 에 대 한는 *table_id* NULL에 대 한 매개 변수는 *docid* 매개 변수 및 4는 에대한*키* 매개 변수입니다.  
  
> [!NOTE]  
>  사용 하 여 **sp_fulltext_keymappings** 으로 *table_id* 변수가 작은 테이블에 적합 합니다.  
