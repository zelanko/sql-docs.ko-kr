...
|**immediate_sync**|**true**|스냅숏 에이전트가 실행될 때마다 게시에 대한 동기화 파일이 생성되거나 다시 생성됩니다. 구독 전에 스냅숏 에이전트가 완료된 경우 구독자는 구독 후에 바로 동기화 파일을 받을 수 있습니다. 새 구독은 스냅숏 에이전트를 가장 최근에 실행하여 생성된 최신 동기화 파일을 가져옵니다. *independent_agent* 수도 있어야 **true**합니다. 에 대 한 자세한 내용은 아래 설명 부분을 참조 하세요 **immediate_sync**합니다.|  
