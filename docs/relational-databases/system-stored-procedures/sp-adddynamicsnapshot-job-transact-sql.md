...
`[ @suser_sname = ] 'suser_sname'`구독자에서 [SUSER_SNAME](../../t-sql/functions/suser-sname-transact-sql.md) 함수의 값으로 필터링 되는 구독에 대 한 필터링 된 데이터 스냅숏을 만들 때 사용 되는 값입니다. *suser_sname* 은 **sysname**이며 기본값은 없습니다. 이 함수를 사용 하 여 게시를 동적으로 필터링 하지 않는 경우 *suser_sname* 은 NULL 이어야 합니다.  
  
`[ @host_name = ] 'host_name'`구독자에서 [HOST_NAME](../../t-sql/functions/host-name-transact-sql.md) 함수의 값으로 필터링 되는 구독에 대 한 필터링 된 데이터 스냅숏을 만들 때 사용 되는 값입니다. *host_name* 은 **sysname**이며 기본값은 없습니다. 이 함수를 사용 하 여 게시를 동적으로 필터링 하지 않는 경우 *host_name* 은 NULL 이어야 합니다.  
  
`[ @dynamic_snapshot_jobname = ] 'dynamic_snapshot_jobname'`만들어진 필터링 된 데이터 스냅숏 작업의 이름입니다. *dynamic_snapshot_jobname* 는 **sysname**이며 기본값은 NULL이 고 선택적 출력 매개 변수입니다. 지정 된 경우 *dynamic_snapshot_jobname* 는 배포자에서 고유한 작업으로 확인 되어야 합니다. 지정하지 않은 경우 작업 이름이 자동으로 생성되고 결과 집합에 반환됩니다. 생성되는 이름은 다음과 같습니다.  
...
|**8**|*frequency_interval* 은 다음 중 하나 이상입니다 ( [ &#124; &#40;비트 or&#41; &#40;&#41; transact-sql](../../t-sql/language-elements/bitwise-or-transact-sql.md) 논리 연산자와 함께 사용).<br /><br /> **1** = 일요일 &#124; **2** = 월요일 &#124; **4** = 화요일 &#124; **8** = 수요일 &#124; **16** = 목요일 &#124; **32** = 금요일 &#124; **64** = 토요일|  
...
`[ @frequency_subday = ] frequency_subday`*Frequency_subday_interval*의 단위를 지정 합니다. *frequency_subday* 은 **int**이며 다음 값 중 하나일 수 있습니다.  
...
`[ @frequency_subday_interval = ] frequency_subday_interval`각 작업 실행 사이에 발생 하는 *frequency_subday* 기간 수입니다. *frequency_subday_interval* 는 **int**이며 기본값은 5입니다.  
...
|**dynamic_snapshot_jobid**|**uniqueidentifier**|배포자에서 에이전트 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 작업을 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 고유 하 게 식별 합니다.|  
  
## <a name="return-code-values"></a>반환 코드 값  
 0(성공) 또는 1(실패)  
  
## <a name="remarks"></a>설명  
 **sp_adddynamicsnapshot_job** 는 매개 변수가 있는 필터를 사용 하는 게시에 대 한 병합 복제에 사용 됩니다.  
 ...
 **Sysadmin** 고정 서버 역할 또는 **db_owner** 고정 데이터베이스 역할의 멤버만이 **sp_adddynamicsnapshot_job**을 실행할 수 있습니다.  
