...
`[ @new_name = ] 'new_name'`경고의 새 이름입니다. 이름은 고유 해야 합니다. *new_name* 는 **sysname**이며 기본값은 NULL입니다.  
...
`[ @severity = ] severity`경고 정의에 대 한 새 심각도 수준 ( **1** 에서 **25**까지)입니다. 지정 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 된 심각도를 사용 하 여 Windows 응용 프로그램 로그에 전송 된 모든 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 메시지는 경고를 활성화 합니다. *심각도* 는 **int**이며 기본값은 NULL입니다. 심각도 수준은 경고에 대 한 메시지 ID 설정이 **0**인 경우에만 사용할 수 있습니다.  
...
`[ @job_id = ] job_id`작업 id입니다. *job_id* 는 **uniqueidentifier**이며 기본값은 NULL입니다. *Job_id* 를 지정 하는 경우 *job_name* 를 생략 해야 합니다.  
  
`[ @job_name = ] 'job_name'`이 경고에 대 한 응답으로 실행 되는 작업의 이름입니다. *job_name* 는 **sysname**이며 기본값은 NULL입니다. *Job_name* 을 지정 하면 *job_id* 를 생략 해야 합니다.  
...
## <a name="remarks"></a>설명  
 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Windows 응용 프로그램 로그에 기록 된 **sysmessages** 만 경고를 발생 시킬 수 있습니다.  
