...
## <a name="arguments"></a>인수  
 ...
 [  **@article=**] **'***문서***'**  
 아티클의 이름입니다. *문서* 은 **sysname**, 모든의 기본값입니다. 즉시 업데이트 게시의 경우 *문서* 해야 **모든**, 그렇지 않으면 저장된 프로시저가 게시를 건너뛰고 오류를 보고 합니다.  
  
 [  **@subscriber=**] **'***구독자***'**  
 구독자의 이름입니다. *구독자* 은 **sysname**, 기본값은 없습니다.  
  
 [  **@destination_db=**] **'***destination_db***'**  
 대상 데이터베이스의 이름입니다. *destination_db* 은 **sysname**, 모든의 기본값입니다.  
  
 [  **@for_schema_change=**] **'***for_schema_change***'**  
 게시 데이터베이스에서 스키마가 변경되어 그 결과로 다시 초기화가 발생하는지 여부를 표시합니다. *for_schema_change* 은 **비트**, 기본값은 0입니다. 경우 **0**,으로 아티클 일부가 아닌 전체 게시가 다시 초기화 되는 즉시 업데이트를 허용 하는 게시에 대 한 활성 구독이 다시 활성화 됩니다. 이는 스키마가 변경되어 다시 초기화가 시작됨을 의미합니다. 경우 **1**, 스냅숏 에이전트가 실행 될 때까지 활성 구독이 다시 활성화 되지 않습니다.  
  
 [  **@publisher=** ] **'***게시자***'**  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 이외의 게시자를 지정합니다. *게시자* 은 **sysname**, 기본값은 NULL입니다.  
  
> [!NOTE]  
>  *게시자* 에 사용할 수 없습니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 게시자입니다.  
  
 [  **@ignore_distributor_failure=** ] *ignore_distributor_failure*  
 배포자가 없거나 오프라인 상태인 경우에도 다시 초기화할 수 있도록 합니다. *ignore_distributor_failure* 은 **비트**, 기본값은 0입니다. 경우 **0**, 배포자가 없거나 오프 라인 상태인 경우 다시 초기화가 실패 합니다.  
  
 [  **@invalidate_snapshot=** ] *invalidate_snapshot*  
 기존 게시 스냅숏을 무효화합니다. *invalidate_snapshot* 은 **비트**, 기본값은 0입니다. 경우 **1**, 게시에 대 한 새 스냅숏이 생성 됩니다.  
 ...
 **sp_reinitsubscription** 피어 투 피어 트랜잭션 복제에 지원 되지 않습니다.  
 ...
 초기 스냅숏이 자동으로 적용되고 게시가 업데이트할 수 있는 구독을 허용하는 구독의 경우 배포 에이전트는 가장 최신 스키마 및 스냅숏 에이전트가 이전에 만든 대량 복사 프로그램 파일을 사용하여 구독을 다시 동기화합니다. 배포 에이전트는 사용자가 실행 된 후 즉시 구독을 다시 동기화 **sp_reinitsubscription**배포 에이전트가 없는 경우, 고, 그렇지 않으면 동기화 메시지 간격 (후 발생할 수 있습니다, 배포 에이전트 명령 프롬프트 매개 변수에서 지정한: **MessageInterval**).  
  
 **sp_reinitsubscription** 은 구독에 아무런 영향 초기 스냅숏이 수동으로 적용 됩니다.  
  
 게시에 대 한 익명 구독을 다시 동기화 하려면 전달 **모든** 또는 NULL을 *구독자*합니다.  
...
## <a name="permissions"></a>Permissions  
 구성원만는 **sysadmin** 고정된 서버 역할의 멤버는 **db_owner** 고정된 데이터베이스 역할 또는 구독의 작성자 실행할 수 **sp_reinitsubscription** .  
