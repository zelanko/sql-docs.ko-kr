...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md.md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
  ...
  제출된 커서 문이나 일괄 처리에 대한 실행 계획을 컴파일한 다음 커서를 만들고 채웁니다. sp_cursorprepexec는 sp_cursorprepare 및 sp_cursorexecute의 함수를 결합 합니다. 이 프로시저는 TDS (tabular data stream) 패킷에서 ID = 5를 지정 하 여 호출 합니다.  
 ...
 *준비 된 핸들*  
 는 생성 된 준비 핸들 식별자입니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] *준비 된 핸들* 은 필수 이며 **int**를 반환 합니다.  
...
> [!NOTE]  
>  Stmt 값을 지정 하는 규칙은 sp_cursoropen에 대 한 규칙과 동일 합니다. 단, *stmt* 문자열 데이터 형식은 **ntext**여야 합니다.  
...
|0x80000|OPTIMISITC_ACCEPTABLE|  
 ...
 Scrollpt[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 와마찬가지로는 요청한 것과 다른 값을 할당할 수 있습니다.  
...
|입력 값일 때|반환 값일 때|  
|--------------------|---------------------|  
|AUTO_FETCH이 FAST_FORWARD로 지정 된 경우 커서 *rowcount* 는 인출 버퍼에 넣을 행 수를 나타냅니다.|결과 집합의 행 수를 나타냅니다. *Scrollopt* AUTO_FETCH 값을 지정 하는 경우 *rowcount* 는 인출 버퍼에 인출 된 행 수를 반환 합니다.|  
