...
모델은 R 또는 Python을 사용 하 여 만들어야 하지만, 대상 데이터베이스 엔진 인스턴스에서 이진 형식으로 직렬화 되 고 저장 된 후에는 R 또는 Python 통합이 설치 되지 않은 경우에도 해당 데이터베이스 엔진 인스턴스에서 사용 될 수 있습니다. 자세한 내용은 [sp_rxPredict를 사용한 실시간 점수 매기기](https://docs.microsoft.com/sql/advanced-analytics/real-time-scoring)를 참조 하세요.
...
#### <a name="python-transformations-supplied-by-microsoftml"></a>Python: microsoftml에서 제공 하는 변환
  ...
  + [범주](https://docs.microsoft.com/machine-learning-server/python-reference/microsoftml/categorical)
