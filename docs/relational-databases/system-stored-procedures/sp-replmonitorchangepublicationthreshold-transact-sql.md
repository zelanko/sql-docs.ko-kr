 ...
 *Metric_id* 또는 *thresholdmetricname*중 하나를 지정 해야 합니다. *Thresholdmetricname* 가 지정 된 경우 *metric_id* 는 NULL 이어야 합니다.  
  
`[ @thresholdmetricname = ] 'thresholdmetricname'`변경 중인 게시 임계값 메트릭의 이름입니다. *thresholdmetricname* 는 **sysname**이며 기본값은 NULL입니다. *Thresholdmetricname* 또는 *metric_id*중 하나를 지정 해야 합니다. *Metric_id* 가 지정 된 경우 *thresholdmetricname* 는 NULL 이어야 합니다.  
