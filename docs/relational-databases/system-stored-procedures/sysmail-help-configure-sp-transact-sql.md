...
검색할 구성 설정의 이름을 `[ @parameter_name = ] 'parameter_name'` 합니다. 이 값을 지정 하면 구성 설정의 값이 **\@parameter_value** OUTPUT 매개 변수에 반환 됩니다. **\@parameter_name** 지정 되지 않은 경우이 저장 프로시저는 인스턴스의 모든 데이터베이스 메일 구성 설정을 포함 하는 결과 집합을 반환 합니다.  
 ...
 **\@parameter_name** 지정 되었지만 **\@parameter_value**에 대 한 출력 매개 변수가 제공 되지 않은 경우이 저장 프로시저는 출력을 생성 하지 않습니다.  
