...
ms.locfileid: "33260456"
---
# <a name="spdescribeparameterencryption-transact-sql"></a>sp_describe_parameter_encryption (Transact SQL)
[!INCLUDE[tsql-appliesto-ss2016-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-ss2016-asdb-xxxx-xxx-md.md)]

  지정 된 분석 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문과 항상 암호화 기능을 사용 하 여 보호 되는 데이터베이스 열에 해당 하는 매개 변수를 확인 하려면 해당 매개 변수입니다. 암호화 된 열에 해당 하는 매개 변수에 대 한 암호화 메타 데이터를 반환 합니다.  
 ...
 **sp_describe_parameter_encryption** 두 개의 결과 집합을 반환 합니다.  
  
-   결과 집합의 지정 된 매개 변수를 데이터베이스 열에 대해 구성 된 암호화 키를 설명 하는 [!INCLUDE[tsql](../../includes/tsql-md.md)] 에 해당 하는 문입니다.  
  
-   결과 집합을 어떻게 특정 구성 요소를 설명 하는 암호화 되어야 합니다. 이 결과 첫 번째 결과 집합에 설명 된 키 참조를 설정 합니다.  
...
|**column_encryption_key_encryption_algorithm_name**|**sysname**|열 암호화 키의 암호화 값을 생성 하는 데 사용 되는 암호화 알고리즘의 이름입니다.|  
 ...
 두 번째 결과 집합의 각 행에 대 한 하나의 매개 변수 암호화 메타 데이터를 포함합니다.  
...
## <a name="remarks"></a>주의  
 A [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 상시 암호화 지원 클라이언트 드라이버를 자동으로 호출 **sp_describe_parameter_encryption** 응용 프로그램에서 발급 하는 매개 변수가 있는 쿼리에 대 한 암호화 메타 데이터를 검색 합니다. 드라이버는 암호화 메타 데이터를 사용 하 여 상시 암호화로 보호 된 데이터베이스 열에 해당 하는 매개 변수 값을 암호화 하 고 암호화 된 응용 프로그램에 의해 전송 된 일반 텍스트 매개 변수 값을 대체 그 후 데이터베이스 엔진에 쿼리를 전송 하기 전에 매개 변수 값  
  
## <a name="permissions"></a>Permissions  
 필요는 **VIEW ANY COLUMN ENCRYPTION KEY DEFINITION** 및 **VIEW ANY COLUMN MASTER KEY DEFINITION** 데이터베이스에서 권한입니다.  
