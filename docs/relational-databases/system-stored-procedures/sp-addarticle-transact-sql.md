 ...
 **true로 설정** 하면 선언 된 기본 키를 제외한 모든 열을 지웁니다. 기본값은 없고 고유 키 열을 포함 하는 null 허용 열입니다. [Sp_articlecolumn](../../relational-databases/system-stored-procedures/sp-articlecolumn-transact-sql.md)를 사용 하 여 열을 추가 합니다.  
...
|**인덱싱된 뷰 logbased manualboth**|수동 필터 및 수동 뷰가 있는 로그 기반의 인덱싱된 뷰 아티클입니다. 이 옵션을 사용 하려면 *sync_object* 및 *필터* 매개 변수를 모두 지정 해야 합니다. 이 유형의 아티클은 기본 테이블을 별도로 게시할 필요가 없습니다. Oracle 게시자에 대해서는 지원되지 않습니다.|  
|**인덱싱된 뷰 logbased manualfilter**|수동 필터가 있는 로그 기반의 인덱싱된 뷰 아티클입니다. 이 옵션을 사용 하려면 *sync_object* 및 *필터* 매개 변수를 모두 지정 해야 합니다. 이 유형의 아티클은 기본 테이블을 별도로 게시할 필요가 없습니다. Oracle 게시자에 대해서는 지원되지 않습니다.|  
|**인덱싱된 뷰 logbased manualview**|수동 뷰가 있는 로그 기반의 인덱싱된 뷰 아티클입니다. 이 옵션을 사용 하려면 *sync_object* 매개 변수를 지정 해야 합니다. 이 유형의 아티클은 기본 테이블을 별도로 게시할 필요가 없습니다. Oracle 게시자에 대해서는 지원되지 않습니다.|  
|**인덱싱된 뷰 스키마만**|스키마가 있는 인덱싱된 뷰 전용입니다. 이 유형의 아티클은 기본 테이블도 게시해야 합니다.|  
|**logbased** 기본|로그 기반 아티클입니다.|  
...
|**serializable proc exec**|직렬화할 수 있는 트랜잭션 컨텍스트 내에서 실행된 경우에만 저장 프로시저의 실행을 복제합니다. Oracle 게시자에 대해서는 지원되지 않습니다.<br /><br /> 프로시저 실행을 복제 하려면 명시적 트랜잭션 내에서 프로시저를 실행 해야 합니다.|  
|**스키마만 보기**|스키마가 있는 뷰 전용입니다. Oracle 게시자에 대해서는 지원되지 않습니다. 이 옵션을 사용할 때는 기본 테이블도 게시해야 합니다.|  
...
`[ @sync_object = ] 'sync_object'`이 아티클의 스냅숏을 나타내는 데 사용 되는 데이터 파일을 생성 하는 데 사용 되는 테이블 또는 뷰의 이름입니다. *sync_object* 은 **nvarchar (386)** 이며 기본값은 NULL입니다. NULL 인 경우 [sp_articleview](../../relational-databases/system-stored-procedures/sp-articleview-transact-sql.md) 를 호출 하 여 출력 파일을 생성 하는 데 사용 되는 뷰를 자동으로 만듭니다. 이는 [sp_articlecolumn](../../relational-databases/system-stored-procedures/sp-articlecolumn-transact-sql.md)를 사용 하 여 열을 추가한 후에 발생 합니다. NULL이 아닌 경우 뷰가 수동으로 생성된 것으로 가정하며 뷰가 생성되지 않습니다.  
...
|-----------|-----------------|  
|**NONE**|아무 작업도 수행 합니다.|  
|**Sp_MSins_ 호출**<br /> **_테이블_**  기본<br /><br /> 또는<br /><br /> **Custom_stored_procedure_name 호출**|구독자에서 실행할 저장 프로시저를 호출합니다. 이 복제 방법을 사용 하려면 *schema_option* 을 사용 하 여 저장 프로시저의 자동 생성을 지정 하거나 아티클의 각 구독자의 대상 데이터베이스에 지정한 저장 프로시저를 만듭니다. *custom_stored_procedure* 은 사용자가 만든 저장 프로시저의 이름입니다. <strong>sp_MSins_*테이블*</strong>  에는 매개 변수의 *_table* 부분에 있는 대상 테이블의 이름이 포함 됩니다. *Destination_owner* 가 지정 된 경우 대상 테이블 이름 앞에 붙습니다. 예를 들어 구독자에서 **프로덕션** 스키마가 소유한 `CALL sp_MSins_ProductionProductCategory` **제품 범주** 테이블의 경우 매개 변수는입니다. 피어 투 피어 복제 토폴로지의 아티클에 대 한 자세한 내용은 *테이블* 에 GUID 값이 추가 됩니다. 구독자 업데이트에는 *custom_stored_procedure* 을 지정할 수 없습니다.|  
...
|-----------|-----------------|  
|**NONE**|아무 작업도 수행 합니다.|  
|**CALLsp_MSdel_**<br /> **_테이블_**  기본<br /><br /> 또는<br /><br /> **Custom_stored_procedure_name 호출**|구독자에서 실행할 저장 프로시저를 호출합니다. 이 복제 방법을 사용 하려면 *schema_option* 을 사용 하 여 저장 프로시저의 자동 생성을 지정 하거나 아티클의 각 구독자의 대상 데이터베이스에 지정한 저장 프로시저를 만듭니다. *custom_stored_procedure* 은 사용자가 만든 저장 프로시저의 이름입니다. <strong>sp_MSdel_*테이블*</strong>  에는 매개 변수의 *_table* 부분에 있는 대상 테이블의 이름이 포함 됩니다. *Destination_owner* 가 지정 된 경우 대상 테이블 이름 앞에 붙습니다. 예를 들어 구독자에서 **프로덕션** 스키마가 소유한 `CALL sp_MSdel_ProductionProductCategory` **제품 범주** 테이블의 경우 매개 변수는입니다. 피어 투 피어 복제 토폴로지의 아티클에 대 한 자세한 내용은 *테이블* 에 GUID 값이 추가 됩니다. 구독자 업데이트에는 *custom_stored_procedure* 을 지정할 수 없습니다.|  
|**XCALL sp_MSdel_**<br /> **_table_**<br /><br /> 또는<br /><br /> **XCALL custom_stored_procedure_name**|XCALL 스타일 매개 변수를 사용하는 저장 프로시저를 호출합니다. 이 복제 방법을 사용 하려면 *schema_option* 을 사용 하 여 저장 프로시저의 자동 생성을 지정 하거나 아티클의 각 구독자의 대상 데이터베이스에 지정한 저장 프로시저를 만듭니다. 구독자 업데이트의 경우 사용자가 만든 저장 프로시저를 지정할 수 없습니다.|  
...
|-----------|-----------------|  
|**NONE**|아무 작업도 수행 합니다.|  
|**Sp_MSupd_ 호출**<br /> **_table_**<br /><br /> 또는<br /><br /> **Custom_stored_procedure_name 호출**|구독자에서 실행할 저장 프로시저를 호출합니다. 이 복제 방법을 사용 하려면 *schema_option* 을 사용 하 여 저장 프로시저의 자동 생성을 지정 하거나 아티클의 각 구독자의 대상 데이터베이스에 지정한 저장 프로시저를 만듭니다.|  
|**MCALL sp_MSupd_**<br /> **_table_**<br /><br /> 또는<br /><br /> **MCALL custom_stored_procedure_name**|MCALL 스타일 매개 변수를 사용하는 저장 프로시저를 호출합니다. 이 복제 방법을 사용 하려면 *schema_option* 을 사용 하 여 저장 프로시저의 자동 생성을 지정 하거나 아티클의 각 구독자의 대상 데이터베이스에 지정한 저장 프로시저를 만듭니다. *custom_stored_procedure* 은 사용자가 만든 저장 프로시저의 이름입니다. <strong>sp_MSupd_*테이블*</strong>  에는 매개 변수의 *_table* 부분에 있는 대상 테이블의 이름이 포함 됩니다. *Destination_owner* 가 지정 된 경우 대상 테이블 이름 앞에 붙습니다. 예를 들어 구독자에서 **프로덕션** 스키마가 소유한 `MCALL sp_MSupd_ProductionProductCategory` **제품 범주** 테이블의 경우 매개 변수는입니다. 피어 투 피어 복제 토폴로지의 아티클에 대 한 자세한 내용은 *테이블* 에 GUID 값이 추가 됩니다. 구독자 업데이트의 경우 사용자가 만든 저장 프로시저를 지정할 수 없습니다.|  
|**SCALL sp_MSupd_**<br /> **_테이블_**  기본<br /><br /> 또는<br /><br /> **SCALL custom_stored_procedure_name**|SCALL 스타일 매개 변수를 사용하는 저장 프로시저를 호출합니다. 이 복제 방법을 사용 하려면 *schema_option* 을 사용 하 여 저장 프로시저의 자동 생성을 지정 하거나 아티클의 각 구독자의 대상 데이터베이스에 지정한 저장 프로시저를 만듭니다. *custom_stored_procedure* 은 사용자가 만든 저장 프로시저의 이름입니다. <strong>sp_MSupd_*테이블*</strong>  에는 매개 변수의 *_table* 부분에 있는 대상 테이블의 이름이 포함 됩니다. *Destination_owner* 가 지정 된 경우 대상 테이블 이름 앞에 붙습니다. 예를 들어 구독자에서 **프로덕션** 스키마가 소유한 `SCALL sp_MSupd_ProductionProductCategory` **제품 범주** 테이블의 경우 매개 변수는입니다. 피어 투 피어 복제 토폴로지의 아티클에 대 한 자세한 내용은 *테이블* 에 GUID 값이 추가 됩니다. 구독자 업데이트의 경우 사용자가 만든 저장 프로시저를 지정할 수 없습니다.|  
|**XCALL sp_MSupd_**<br /> **_table_**<br /><br /> 또는<br /><br /> **XCALL custom_stored_procedure_name**|XCALL 스타일 매개 변수를 사용하는 저장 프로시저를 호출합니다. 이 복제 방법을 사용 하려면 *schema_option* 을 사용 하 여 저장 프로시저의 자동 생성을 지정 하거나 아티클의 각 구독자의 대상 데이터베이스에 지정한 저장 프로시저를 만듭니다. 구독자 업데이트의 경우 사용자가 만든 저장 프로시저를 지정할 수 없습니다.|  
...
`[ @schema_option = ] schema_option`지정 된 아티클에 대 한 스키마 생성 옵션의 비트 마스크입니다. *schema_option* 은 **이항 (8)** 이며 [| (비트 or)](../../t-sql/language-elements/bitwise-or-transact-sql.md) 다음 값 중 하나 이상의 곱입니다.  
...
|값|설명|  
|-----------|-----------------|  
|**0x00**|스냅숏 에이전트에서 스크립팅을 사용 하지 않도록 설정 하 고 *creation_script*를 사용 합니다.|  
...
|**0x20000000**|에서[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 도입 된 large object 데이터 형식 (**nvarchar (max)** , **varchar (max)** 및 **varbinary (max)** )을에서 [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)]지원 되는 데이터 형식으로 변환 합니다.|  
...
|**0x200000000**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 도입[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 된 날짜 및 시간 데이터 형식 (date, **time**, datetimeoffset 및 datetime2)을 이전 버전의에서 지원 되는 데이터 형식으로 변환 합니다.|  
...
|**0x1000000000**|8000 바이트 보다 큰 CLR (공용 언어 런타임) Udt (사용자 정의 형식)를 **varbinary (max)** 로 변환 하 여를 실행 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]하는 구독자에 UDT 형식의 열을 복제할 수 있도록 합니다.|  
|**0x2000000000**|Hierarchyid 데이터 형식을 **varbinary (max)** 로 변환 하 여 **hierarchyid** 형식의 열을를 실행 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]하는 구독자로 복제할 수 있습니다. 복제 된 테이블에서 **hierarchyid** 열을 사용 하는 방법에 대 한 자세한 내용은 [hierarchyid &#40;transact-sql&#41;](../../t-sql/data-types/hierarchyid-data-type-method-reference.md)을 참조 하세요.|  
...
`[ @destination_owner = ] 'destination_owner'`대상 개체의 소유자 이름입니다. *destination_owner* 는 **sysname**이며 기본값은 NULL입니다. *Destination_owner* 를 지정 하지 않으면 소유자는 다음 규칙에 따라 자동으로 지정 됩니다.  
...
|게시는 문자 모드 대량 복사를 사용하여 초기 스냅샷을 생성하며 이는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 이외 구독자를 지원합니다.|할당되지 않습니다.|  
 ...
 이외 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 구독자를 지원 하려면 *destination_owner* 가 NULL 이어야 합니다.  
...
`[ @source_object = ] 'source_object'`게시할 데이터베이스 개체입니다. *source_object* 는 **sysname**이며 기본값은 NULL입니다. *Source_table* 가 null 인 경우 *source_object* 는 null 일 수 없습니다. *source_table*대신 *source_object* 를 사용 해야 합니다. 스냅숏 또는 트랜잭션 복제를 사용 하 여 게시할 수 있는 개체 유형에 대 한 자세한 내용은 [데이터 및 데이터베이스 개체 게시](../../relational-databases/replication/publish/publish-data-and-database-objects.md)를 참조 하세요.  
...
`[ @pub_identity_range = ] pub_identity_range`아티클의 *identityrangemanagementoption* 가 **auto** 로 설정 되어 있거나 *auto_identity_range* 가 **true**로 설정 된 경우 게시자의 범위 크기를 제어 합니다. *pub_identity_range* 는 **bigint**이며 기본값은 NULL입니다. *Oracle 게시자에 대해서는 지원 되지 않습니다*.  
  
`[ @identity_range = ] identity_range`아티클의 *identityrangemanagementoption* 가 **auto** 로 설정 되거나 *auto_identity_range* 가 **true**로 설정 된 경우 구독자의 범위 크기를 제어 합니다. *identity_range* 는 **bigint**이며 기본값은 NULL입니다. *Auto_identity_range* 가 **true**로 설정 된 경우 사용 됩니다. *Oracle 게시자에 대해서는 지원 되지 않습니다*.  
...
`[ @force_invalidate_snapshot = ] force_invalidate_snapshot`이 저장 프로시저가 수행한 동작으로 인해 기존 스냅숏이 무효화 될 수 있음을 승인 합니다. *force_invalidate_snapshot* 는 **bit**이며 기본값은 0입니다.  
...
`[ @use_default_datatypes = ] use_default_datatypes`Oracle 게시자에서 아티클을 게시할 때 기본 열 데이터 형식 매핑을 사용할지 여부입니다. *use_default_datatypes* 는 bit 이며 기본값은 1입니다.  
  
 **1** = 기본 아티클 열 매핑이 사용 됩니다. [Sp_getdefaultdatatypemapping](../../relational-databases/system-stored-procedures/sp-getdefaultdatatypemapping-transact-sql.md)를 실행 하 여 기본 데이터 형식 매핑을 표시할 수 있습니다.  
  
 **0** = 사용자 지정 아티클 열 매핑이 정의 되었으므로 **sp_addarticle**에 의해 [sp_articleview](../../relational-databases/system-stored-procedures/sp-articleview-transact-sql.md) 가 호출 되지 않습니다.  
  
 *Use_default_datatypes* 을 **0**으로 설정 하면 기본값에서 변경 되는 각 열 매핑에 대해 [sp_changearticlecolumndatatype](../../relational-databases/system-stored-procedures/sp-changearticlecolumndatatype-transact-sql.md) 를 한 번 실행 해야 합니다. 모든 사용자 지정 열 매핑을 정의한 후에는 [sp_articleview](../../relational-databases/system-stored-procedures/sp-articleview-transact-sql.md)를 실행 해야 합니다.  
  
> [!NOTE]  
>  이 매개 변수는 Oracle 게시자에 대해서만 사용해야 합니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 게시자에 대해 *use_default_datatypes* 를 **0** 으로 설정 하면 오류가 생성 됩니다.  
...
|NULL (기본값)|*Auto_identity_range* 의 값이 **true**가 아니면 기본값은 **none** 입니다. 피어 투 피어 토폴로지에서 기본값은 **수동으로** 설정 됩니다 (*auto_identity_range* 는 무시 됨).|  
  
 이전 버전과의 호환성을 위해 *identityrangemanagementoption* 의 값이 NULL 이면 *auto_identity_range* 값이 확인 됩니다. 그러나 *identityrangemanagementoption* 의 값이 NULL이 아닌 경우에는 *auto_identity_range* 값이 무시 됩니다.  
...
## <a name="remarks"></a>설명  
 **sp_addarticle** 은 스냅숏 복제 또는 트랜잭션 복제에 사용 됩니다.  
  
 기본적으로 복제는 열 데이터 형식이 복제에서 지원되지 않으면 원본 테이블의 어떤 열도 게시하지 않습니다. 이러한 열을 게시 해야 하는 경우 [sp_articlecolumn](../../relational-databases/system-stored-procedures/sp-articlecolumn-transact-sql.md) 를 실행 하 여 열을 추가 해야 합니다.  
...
-   테이블에 **타임 스탬프** 열이 있는 경우 열을 **timestamp**로 복제 하려면 *schema_option* 에 0x08를 포함 해야 합니다.  
  
-   *Ins_cmd*, *upd_cmd*및 *del_cmd*에는 **SQL** 값을 지정할 수 없습니다.  
 ...
 *Vertical_partition* 가 **true**로 설정 된 경우 **sp_addarticle** 는 [sp_articleview](../../relational-databases/system-stored-procedures/sp-articleview-transact-sql.md) 를 호출할 때까지 (마지막 [sp_articlecolumn](../../relational-databases/system-stored-procedures/sp-articlecolumn-transact-sql.md) 가 추가 된 후) 뷰가 생성 되는 것을 지연 시킵니다.  
  
 게시에서 구독 업데이트를 허용 하 고 게시 된 테이블에 **uniqueidentifier** 열이 없는 경우 **sp_addarticle** 는 자동으로 테이블에 **uniqueidentifier** 열을 추가 합니다.  
  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스가 아닌 구독자 (다른 유형의 복제)로 복제 하는 경우 **INSERT**, **UPDATE**및 [!INCLUDE[tsql](../../includes/tsql-md.md)] **DELETE** 명령에 대해서만 문이 지원 됩니다.  
 ...
 또는 `@del_cmd = 'NONE'` 를`@ins_cmd = 'NONE'`설정 하는 경우 제한 된 업데이트가 발생할 때 이러한 명령을 보내지 않으면 **업데이트** 명령의 전파가 영향을 받을 수도 있습니다. 바인딩된 업데이트는 구독자에 대한 DELETE/INSERT 쌍으로 복제되는 게시자의 UPDATE 문입니다.  
  
## <a name="default-schema-options"></a>기본 스키마 옵션  
 이 표에서는 사용자가 *schema_options* 을 지정 하지 않은 경우 복제에 의해 설정 되는 기본값을 설명 합니다. 여기서이 값은 복제 유형 (위에 표시 됨)과 아티클 유형 (첫 번째 열에 표시 됨)에 따라 달라 집니다.  
...
|**인덱싱된 뷰 logbased manualfilter**|**0x30F3**|**0x3071**|  
|**인덱싱된 뷰 logbased manualview**|**0x30F3**|**0x3071**|  
...
> [!NOTE]
>  게시가 지연 업데이트를 사용 하도록 설정 된 경우에는 테이블에 표시 된 기본값에 *schema_option* 값 **0x80** 이 추가 됩니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 게시가 아닌 경우의 기본 *schema_option* 은 **0x050d3**입니다.  
  
## <a name="valid-schema-options"></a>유효한 스키마 옵션  
 다음 표에서는 복제 유형 (위에 표시 됨)과 아티클 유형 (첫 번째 열에 표시 됨)에 따라 허용 되는 *schema_option* 값에 대해 설명 합니다.  
...
|**인덱싱된 뷰 logbased**|모든 옵션|**0x02** 를 제외한 모든 옵션|  
|**인덱싱된 뷰 logbased manualfilter**|모든 옵션|**0x02** 를 제외한 모든 옵션|  
|**인덱싱된 뷰 logbased manualview**|모든 옵션|**0x02** 를 제외한 모든 옵션|  
|**인덱싱된 뷰 logbase manualboth**|모든 옵션|**0x02** 를 제외한 모든 옵션|  
...
|**프로시저 스키마 전용**|**0x01**, **0x20**, **0x2000**, **0x400000**, **0x800000**, **0x2000000**, **0x800000**, **0x10000000**, **0x20000000**, **0x40000000**및 **0x80000000**|**0x01**, **0x20**, **0x2000**, **0x400000**, **0x800000**, **0x2000000**, **0x800000**, **0x10000000**, **0x20000000**, **0x40000000**및 **0x80000000**|  
|**스키마만 보기**|**0x01**, **0x010**, **0x020**, **0x040**, **0x0100**, **0x2000**, **0x40000**, **0x100000**, **0x200000**, **0x400000**, **0x800000**, **0x200000**, **0x800000**, **0x40000000**및 **0x80000000**|**0x01**, **0x010**, **0x020**, **0x040**, **0x0100**, **0x2000**, **0x40000**, **0x100000**, **0x200000**, **0x400000**, **0x800000**, **0x200000**, **0x800000**, **0x40000000**및 **0x80000000**|  
|**func 스키마만**|**0x01**, **0x20**, **0x2000**, **0x400000**, **0x800000**, **0x2000000**, **0x800000**, **0x10000000**, **0x20000000**, **0x40000000**및 **0x80000000**|**0x01**, **0x20**, **0x2000**, **0x400000**, **0x800000**, **0x2000000**, **0x800000**, **0x10000000**, **0x20000000**, **0x40000000**및 **0x80000000**|  
|**인덱싱된 뷰 스키마만**|**0x01**, **0x010**, **0x020**, **0x040**, **0x0100**, **0x2000**, **0x40000**, **0x100000**, **0x200000**, **0x400000**, **0x800000**, **0x200000**, **0x800000**, **0x40000000**및 **0x80000000**|**0x01**, **0x010**, **0x020**, **0x040**, **0x0100**, **0x2000**, **0x40000**, **0x100000**, **0x200000**, **0x400000**, **0x800000**, **0x200000**, **0x800000**, **0x40000000**및 **0x80000000**|  
  
> [!NOTE]
>  지연 업데이트 게시의 경우 **0x8000** 및 **0x80** 의 *schema_option* 값을 설정 해야 합니다. 이외[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 게시에 대해 지원 되는 *schema_option* 값은 다음과 같습니다. **0x01**, **0x02**, **0x10**, **0x40**, **0x80**, **0x1000**, **0x4000** 및 **0x8000**입니다.  
  
## <a name="example"></a>예제  
 [!code-sql[HowTo#sp_AddTranArticle](../../relational-databases/replication/codesnippet/tsql/sp-addarticle-transact-sql_1.sql)]  
  
## <a name="permissions"></a>사용 권한  
 **Sysadmin** 고정 서버 역할 또는 **db_owner** 고정 데이터베이스 역할의 멤버만이 **sp_addarticle**을 실행할 수 있습니다.  
