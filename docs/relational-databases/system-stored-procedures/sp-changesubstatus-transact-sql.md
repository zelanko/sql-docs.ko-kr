...
`[ @publication = ] 'publication'` 게시의 이름이입니다. *게시* 됩니다 **sysname**, 기본값은 **%** 합니다. 하는 경우 *게시* 지정 하지 않으면 모든 게시에 영향을 받습니다.  
  
`[ @article = ] 'article'` 아티클의 이름이입니다. 이 이름은 게시에 대해 고유해야 합니다. *문서* 됩니다 **sysname**, 기본값은 **%** 합니다. 하는 경우 *문서* 지정 하지 않으면 모든 아티클에 적용 됩니다.  
  
`[ @subscriber = ] 'subscriber'` 상태를 변경할 구독자의 이름이입니다. *구독자* 됩니다 **sysname**, 기본값은 **%** 합니다. 하는 경우 *구독자* 지정 하지 않으면 지정 된 아티클에 모든 구독자에 대 한 상태가 변경 됩니다.  
...
`[ @frequency_type = ] frequency_type` 배포 태스크를 예약 하는 빈도입니다. *frequency_type* 됩니다 **int**, 기본값은 NULL입니다.  
  
`[ @frequency_interval = ] frequency_interval` 설정 된 빈도에 적용 하려면 값인 *frequency_type*합니다. *frequency_interval* 됩니다 **int**, 기본값은 NULL입니다.  
  
`[ @frequency_relative_interval = ] frequency_relative_interval` 배포 태스크 날짜가입니다. 이 매개 변수를 사용 하면 *frequency_type* 32 (매월 상대)로 설정 됩니다. *frequency_relative_interval* 됩니다 **int**, 이며 다음이 값 중 하나일 수 있습니다.  
...
`[ @frequency_subday = ] frequency_subday` 정의 된 기간 동안 다시 예약 하의 분 단위 빈도입니다. *frequency_subday* 됩니다 **int**, 이며 다음이 값 중 하나일 수 있습니다.  
...
`[ @dts_package_name = ] 'dts_package_name'` 데이터 변환 서비스 (DTS) 패키지의 이름을 지정합니다. *dts_package_name* 되는 **sysname**, 기본값은 NULL 사용 하 여 합니다. 명명 된 예 패키지용 **DTSPub_Package** 지정 하는 경우 `@dts_package_name = N'DTSPub_Package'`합니다.  
...
`[ @dts_package_location = ] dts_package_location` 패키지 위치를 지정합니다. *dts_package_location* 되는 **int**, 기본값은 **0**합니다. 하는 경우 **0**, 패키지 위치는 배포자입니다. 하는 경우 **1**, 패키지 위치는 구독자의 합니다. 패키지의 위치 일 수 있습니다 **배포자** 하거나 **구독자**합니다.  
  
`[ @skipobjectactivation = ] skipobjectactivation` [!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]  
  
`[ @distribution_job_name = ] 'distribution_job_name'` 배포 작업의 이름이입니다. *distribution_job_name* 됩니다 **sysname**, 기본값은 NULL입니다.  
