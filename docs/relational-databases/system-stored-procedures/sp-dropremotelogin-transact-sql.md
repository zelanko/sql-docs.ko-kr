 ...
 *Remoteserver* 만 지정 하면 해당 원격 서버에 대 한 모든 원격 로그인이 로컬 서버에서 제거 됩니다. 또한 *로그인* 을 지정 하면 해당 특정 로컬 로그인에 매핑된 *remoteserver* 의 모든 원격 로그인이 로컬 서버에서 제거 됩니다. *Remote_name* 도 지정한 경우에는 *remoteserver* 의 원격 사용자에 대 한 원격 로그인만 로컬 서버에서 제거 됩니다.  
 ...
 원격 로그인은 이전 버전의 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]를 사용하는 경우에만 필요합니다. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 버전 7.0 이상 버전에서는 연결된 서버의 로그인을 사용합니다. **Sp_addlinkedsrvlogin** 및 **sp_droplinkedsrvlogin** 를 사용 하 여 연결 된 서버 로그인을 추가 및 제거 합니다.  
