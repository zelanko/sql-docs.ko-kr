...
`[ @publication = ] 'publication'` 게시의 이름이입니다. *게시* 됩니다 **sysname**, 기본값은 없습니다. 반드시 게시가 이미 존재해야 합니다.  
...
`[ @frequency_type = ] frequency_type` 병합 에이전트가 실행 될 때 지정 된 값입니다. *frequency_type* 됩니다 **int**, 이며 다음 값 중 하나일 수 있습니다.  
...
`[ @frequency_interval = ] frequency_interval` 병합 에이전트가 실행 되는 한 일입니다. *frequency_interval* 됩니다 **int**, 이며 다음 값 중 하나일 수 있습니다.  
...
`[ @frequency_relative_interval = ] frequency_relative_interval` 각 월에 예약 된 병합 발생 빈도 간격입니다. *frequency_relative_interval* 됩니다 **int**, 이며 다음이 값 중 하나일 수 있습니다.  
...
`[ @frequency_subday = ] frequency_subday` 단위입니다 *frequency_subday_interval*합니다. *frequency_subday* 됩니다 **int**, 이며 다음 값 중 하나일 수 있습니다.  
...
`[ @frequency_subday_interval = ] frequency_subday_interval` 에 대 한 빈도로 *frequency_subday* 각 병합 간에 발생 합니다. *frequency_subday_interval* 됩니다 **int**, 기본값은 NULL입니다.  
...
`[ @hostname = ] 'hostname'` 반환 된 값을 재정의 [HOST_NAME](../../t-sql/functions/host-name-transact-sql.md) 매개 변수가 있는 필터의 WHERE 절에이 함수는 사용 하는 경우. *호스트 이름* 됩니다 **sysname**, 기본값은 NULL입니다.  
