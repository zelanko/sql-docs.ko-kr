...
## <a name="arguments"></a>인수  
 [  **@objname=**] **'***이름***'**  
 에 있는 모든 개체의 이름인 **sysobjects** 모든 사용자 정의 데이터 형식의 또는 **systypes** 테이블입니다. *이름* 은 **nvarchar (**776**)**, 기본값은 NULL입니다. 데이터베이스 이름은 허용되지 않습니다.  예: 'Person.AddressType' 또는 [Person.AddressType] 두 개 또는 세 부분으로 된 이름은 구분 해야 합니다.   
...
3.  경우 *이름* 이 데이터베이스 개체는 데이터 형식 이외의 **sp_help** 설정된 함께 추가 결과 집합을 지정 된 개체의 형식에 따라이 결과 반환 합니다.  
    ...
    -   제약 조건에 대해 반환되는 추가 결과 집합입니다.  
        ...
        |**제약 조건 이름**|**nvarchar (**128**)**|제약 조건의 이름입니다.|  
