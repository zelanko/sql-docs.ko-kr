---
title: sp_dropalias (TRANSACT-SQL) | Microsoft Docs
ms.custom: ''
ms.date: 09/18/2015
ms.prod: sql
ms.prod_service: database-engine
ms.component: system-stored-procedures
ms.reviewer: ''
ms.suite: sql
ms.technology: system-objects
ms.tgt_pltfrm: ''
ms.topic: language-reference
dev_langs:
- TSQL
ms.assetid: 46615017-c4f2-4586-91d9-4d66b5311ae9
ms.author: vanto
manager: craigg
ms.openlocfilehash: b461f0f1c9f6d3d3696e544ec479faca4138e1ed
ms.sourcegitcommit: 182b8f68bfb345e9e69547b6d507840ec8ddfd8b
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/27/2018
ms.locfileid: "43017797"
---
# <a name="spdropalias-transact-sql"></a>sp_dropalias(Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2012-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2012-xxxx-xxxx-xxx-md.md)]

  **sp_dropalias** 에 대 한 제거 된 [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]합니다. 구문에 대 한 참조 [sp_dropalias (TRANSACT-SQL)](https://msdn.microsoft.com/library/ms177513\(v=sql.105\).aspx) 이전 설명서에서.  
  
## <a name="syntax"></a>구문  
  
```  
See previous version.  
```  
  
## <a name="remarks"></a>Remarks  
  
