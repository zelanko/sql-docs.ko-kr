...
## <a name="remarks"></a>주의  
 ...
 각 스냅숏과 이와 연결된 데이터는 명시적 트랜잭션 내에서 삭제된 다음 커밋됩니다. 따라서 제거 작업을 수동으로 중지 하거나 변수에 지정한 값 @duration 초과 되 면 커밋되지 않은 데이터 유지 됩니다. 이 데이터는 다음에 작업이 실행될 때 제거될 수 있습니다.  
...
### <a name="b-specifying-retention-and-duration-values"></a>2. 보존 및 기간 값 지정  
 다음 예에서는 관리 데이터 웨어하우스에서 7일보다 오래된 데이터를 제거합니다. 또한는 @duration 작업을 5 분을 더 이상 실행할 수 있도록 매개 변수를 지정 합니다.  
...
### <a name="c-specifying-an-instance-name-and-collection-set"></a>3. 인스턴스 이름 및 컬렉션 집합 지정  
 다음 예에서는 관리 데이터 웨어하우스에서 지정된 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스의 지정된 컬렉션 집합에 대한 데이터를 제거합니다. 때문에 @retention_days 를 지정 하지 않으면 제거 될 수 있는 컬렉션 집합에 대 한 행이 결정 core.snapshots 뷰의 valid_through 열에 값이 사용 됩니다.  
