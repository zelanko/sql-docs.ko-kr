...
## <a name="arguments"></a>인수  
 ...
 계획 지침의 이름입니다. 계획 지침 이름은 현재 데이터베이스 범위에 적용됩니다. *plan_guide_name* 는 [식별자](../../relational-databases/databases/database-identifiers.md) 에 대 한 규칙을 따라야 하며 숫자 기호 (#)로 시작할 수 없습니다. *Plan_guide_name* 의 최대 길이는 124 자입니다.  
  
 [ \@stmt = ] N'*statement_text*'  
 계획 지침을 만들 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문입니다. 쿼리 최적화 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 프로그램에서 *statement_text*일치 하는 쿼리를 인식 하면 *plan_guide_name* 가 적용 됩니다. 계획 지침을 성공적으로 만들려면 *statement_text* \@는 type, \@module_or_batch 및 \@params 매개 변수로 지정 된 컨텍스트에 표시 되어야 합니다.  
  
 *statement_text* 는 쿼리 최적화 프로그램이 module_or_batch 및 \@ \@params로 식별 된 일괄 처리 또는 모듈 내에서 제공 된 해당 문과 일치 시킬 수 있는 방식으로 제공 되어야 합니다. 자세한 내용은 "주의" 섹션을 참조하십시오. *Statement_text* 의 크기는 서버의 사용 가능한 메모리에 의해서만 제한 됩니다.  
  
 [\@type = ]N'{ OBJECT | SQL | TEMPLATE }'  
 *Statement_text* 이 표시 되는 엔터티의 형식입니다. *Statement_text* 와 *plan_guide_name*를 일치 시키는 컨텍스트를 지정 합니다.  
  
 OBJECT  
 *Statement_text* 가 현재 데이터베이스의 [!INCLUDE[tsql](../../includes/tsql-md.md)] 저장 프로시저, 스칼라 함수, 다중 문 테이블 반환 함수 또는 [!INCLUDE[tsql](../../includes/tsql-md.md)] DML 트리거의 컨텍스트에 표시 됨을 나타냅니다.  
  
 SQL  
 모든 메커니즘을 통해로 전송할 수 있는 독립 실행형 문 또는 일괄 처리의 컨텍스트에 *statement_text* 를 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 표시 합니다. [!INCLUDE[tsql](../../includes/tsql-md.md)]CLR (공용 언어 런타임) 개체 또는 확장 저장 프로시저에 의해 전송 되거나 EXEC N '*sql_string*'을 사용 하 여 전송 된 문은 서버에서 일괄 처리로 처리 되므로 ' sql ' 형식 \@ **=** 으로 식별 해야 합니다. SQL이 지정 된 경우 쿼리 힌트 매개 변수화 {강제 | SIMPLE} \@힌트 매개 변수를 지정할 수 없습니다.  
 ...
 *Statement_text* 이 표시 되는 개체의 이름 또는 *statement_text* 가 표시 되는 일괄 처리 텍스트를 지정 합니다. 일괄 처리 텍스트는 USE*database* 문을 포함할 수 없습니다.  
  
 응용 프로그램에서 전송 된 일괄 처리와 일치 하는 계획 지침을 위해 *batch_tex*t는로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]전송 되는 문자에 대 한 문자를 동일한 형식으로 제공 해야 합니다. 이 일치 작업을 더 효과적으로 처리하기 위해 내부 변환은 수행되지 않습니다. 자세한 내용은 설명 섹션을 참조하세요.  
  
 [*schema_name*.] *object_name* 는 statement_text를 포함 하 [!INCLUDE[tsql](../../includes/tsql-md.md)] 는 저장 프로시저, 스칼라 함수, 다중 문 테이블 반환 함수 또는 [!INCLUDE[tsql](../../includes/tsql-md.md)] DML 트리거의 이름을 지정합니다. *Schema_name* 을 지정 하지 않으면 *schema_name* 는 현재 사용자의 스키마를 사용 합니다. NULL이 지정 되 고 \@type = ' SQL ' 인 경우 module_or_batch의 \@값은 stmt의 \@값으로 설정 됩니다. Type = ' TEMPLATE **\'** , module_or_batch \@는 NULL 이어야 합니다. \@  
...
-   *statement_text* 은 sp_executesql을 사용 하 여 전송 되 고 \@params 매개 변수 값은 지정 되거나 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 매개 변수화 후에 내부적으로 문을 전송 합니다. 데이터베이스 API(ODBC, OLE DB, ADO.NET 등)에서 매개 변수가 있는 쿼리의 전송은 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에 sp_executesql 또는 API 서버 커서 루틴에 대한 호출로 나타나므로 SQL 또는 TEMPLATE 계획 지침으로 일치시킬 수도 있습니다.  
  
 parameter_name data_type은 sp_executesql을 사용 하 여에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 전송 되거나 매개 변수화 후 내부적으로 전송 되는 것과 정확히 같은 형식으로 제공 되어야 합니다.  *\@* 자세한 내용은 설명 섹션을 참조하세요. 일괄 처리에 매개 변수가 없는 경우 NULL이 지정되어야 합니다. \@매개 변수 크기는 사용 가능한 서버 메모리에 의해서만 제한 됩니다.  
  
 [\@hint =] {N'OPTION (*q* [, ... *n* ]) ' | N '*XML_showplan*' | N  
 N'OPTION (*q* [, *... n* ])  
 Stmt와 일치 \@하는 쿼리에 연결할 option 절을 지정 합니다. \@힌트는 SELECT 문의 OPTION 절과 동일 해야 하며 쿼리 힌트의 유효한 시퀀스를 포함할 수 있습니다.  
 ...
 NULL  
 쿼리의 OPTION 절에 지정된 기존 힌트는 쿼리에 적용되지 않음을 나타냅니다. 자세한 내용은 [OPTION 절 &#40;transact-sql&#41;](../../t-sql/queries/option-clause-transact-sql.md)을 참조 하세요.  
...
## <a name="plan-guide-matching-requirements"></a>계획 지침 일치 요구 사항  
 Type = ' SQL ' \@또는 \@type = ' TEMPLATE '을 지정 하는 계획 지침이 쿼리와 성공적으로 일치 하려면 *batch_text* 및  *\@parameter_name data_type* [, *... n* ]은 응용 프로그램에서 전송한 것과 정확히 같은 형식으로 제공 되어야 합니다. 이는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 컴파일러가 수신한 것과 정확히 같게 일괄 처리 텍스트를 제공해야 함을 의미합니다. 실제 일괄 처리 및 매개 변수 텍스트를 캡처하기 위해 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)]를 사용할 수 있습니다. 자세한 내용은 [SQL Server Profiler를 사용 하 여 계획 지침 만들기 및 테스트](../../relational-databases/performance/use-sql-server-profiler-to-create-and-test-plan-guides.md)를 참조 하세요.  
  
 Type \@= ' SQL ' 및 \@module_or_batch가 \@NULL로 설정 되 면 module_or_batch 값이 \@stmt 값으로 설정 됩니다. 즉, *statement_text* 에 대 한 값은에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]전송 되는 문자에 대 한 문자를 정확히 같은 형식으로 제공 해야 합니다. 이 일치 작업을 더 효과적으로 처리하기 위해 내부 변환은 수행되지 않습니다.  
  
 가 statement_text의 값과 *batch_text* 및  *\@parameter_name data_type* [, ... [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  *n* ] 또는 type = \@ **\'** OBJECT ' 인 경우 *object_name*내부의 해당 쿼리 텍스트에 다음 문자열 요소를 고려 하지 않습니다.  
...
-   후행 세미콜론  
 ...
 예 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 를 들어는 *statement_text* 문자열 `N'SELECT * FROM T WHERE a = 10'` 을 다음 *batch_text*와 일치 시킬 수 있습니다.  
...
## <a name="see-also"></a>관련 항목  
 ...
 [_exec_query_stats &#40;transact-sql&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-exec-query-stats-transact-sql.md)   
