...
## <a name="remarks"></a>설명  
 **sp_replcmds** 는 트랜잭션 복제의 로그 판독기 프로세스에서 사용 됩니다.  
...
>  원본 데이터베이스의 테이블 이름은 소유자 이름에 의해 한정되므로 대상 데이터베이스의 테이블 소유자는 반드시 같은 소유자 이름을 사용해야 합니다.  
 ...
 텍스트 포인터가 동일한에서 검색 되지 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 않았기 때문에 **sp_replcmds** 가 텍스트 명령을 복제할 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 수 없는 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 오류 로그 및 Windows 응용 프로그램 로그에 경고 메시지 번호 18759이 추가 됩니다. 트랜잭션과.  
  
## <a name="permissions"></a>사용 권한  
 **Sysadmin** 고정 서버 역할 또는 **db_owner** 고정 데이터베이스 역할의 멤버만이 **sp_replcmds**을 실행할 수 있습니다.  
