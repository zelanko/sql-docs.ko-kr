...
## <a name="arguments"></a>인수  
 ...
 [ **@message_id =** ] *message_id*  
 경고를 정의하는 메시지 오류 번호입니다. (일반적으로 오류 번호에 해당 합니다 **sysmessages** 테이블입니다.) *message_id* 됩니다 **int**, 기본값은 **0**합니다. 하는 경우 *심각도* 경고를 정의 하는 데 사용 됩니다 *message_id* 여야 **0** 또는 NULL입니다.  
  
> [!NOTE]  
>  만 **sysmessages** Microsoft Windows 응용 프로그램 로그에 기록 하는 오류를 보내도록 경고를 발생할 수 있습니다.  
  
 [  **@severity =** ] *심각도*  
 심각도 수준 (에서 **1** 를 통해 **25**) 경고를 정의 하는 합니다. 모든 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에 저장 된 메시지를 **sysmessages** 테이블에 전송 합니다 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 표시 된 심각도로 Windows 응용 프로그램 로그에 경고를 전송 하면 합니다. *심각도* 됩니다 **int**, 기본값은 0입니다. 하는 경우 *message_id* 경고를 정의 하는 데 사용 됩니다 *심각도* 여야 **0**합니다.  
  
 [  **@enabled =** ] *사용 하도록 설정*  
 경고의 현재 상태를 나타냅니다. *사용 하도록 설정* 됩니다 **tinyint**, 기본값은 1 (사용). 하는 경우 **0**, 경고가 사용 되지 않으며 발생 하지 않습니다.  
  
 [  **@delay_between_responses =** ] *delay_between_responses*  
 경고에 대한 응답에 걸리는 대기 시간(초)입니다. *delay_between_responses*됩니다 **int**, 기본값은 **0**, 있습니다 (각 경고의 발생 응답을 생성 하는 데 사용) 하는 응답에 걸리는 대기 시간이 없음 됨을 의미 합니다. 응답은 다음 두 가지 형식 중 한 가지 또는 두 가지 모두를 사용할 수 있습니다.  
...
-   실행할 작업  
 ...
 [ **@include_event_description_in =** ] *include_event_description_in*  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 오류에 대한 설명을 알림 메시지의 일부로 포함할지 여부를 나타냅니다. *include_event_description_in*됩니다 **tinyint**, 기본값은 **5** (전자 메일 및 **net send**), 하나를 가질 수와 함께 다음이 값 중 더는 **또는** 논리 연산자입니다.  
