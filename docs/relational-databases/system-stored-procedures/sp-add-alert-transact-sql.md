...
`[ @severity = ] severity`경고를 정의 하는 심각도 수준 ( **1** 부터 **25**까지)입니다. 표시 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 된 심각도로 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Windows 응용 프로그램 로그에 **sysmessages** 테이블에 저장 된 메시지는 경고를 전송 합니다. *심각도* 는 **int**이며 기본값은 0입니다. *Message_id* 를 사용 하 여 경고를 정의 하는 경우 *심각도* 가 **0**이어야 합니다.  
