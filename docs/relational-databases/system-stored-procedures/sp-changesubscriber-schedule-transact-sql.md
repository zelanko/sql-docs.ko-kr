...
`[ @frequency_subday = ] frequency_subday`정의 된 기간 동안 다시 예약 하는 빈도 (분)입니다. *frequency_subday* 는 **int**이며 기본값은 **4**입니다.  
...
`[ @active_end_time_of_day = ] active_end_time_of_day`하루 중에서 배포 태스크가 중지 되도록 예약 된 시간입니다. *active_end_time_of_day* 은 **int**이며 기본값은 **235959**입니다 .이는 11:59:59를 의미 합니다. 24시간제를 기준으로 오후 11:59:59를 나타냅니다.  
 ...
 **Sysadmin** 고정 서버 역할의 멤버만 **sp_changesubscriber_schedule**을 실행할 수 있습니다.  
