...
## <a name="remarks"></a>주의  
 OLE Automation 저장 프로시저를 사용하는 모든 클라이언트는 하나의 실행 환경을 공유합니다. 하나의 클라이언트 호출 하는 경우 **sp_OAStop** 모든 클라이언트에 대해 공유 실행 환경이 중지 됩니다. 호출 하면 실행 환경이 중지 된 후 **sp_OACreate** 실행 환경을 다시 시작 합니다.  
