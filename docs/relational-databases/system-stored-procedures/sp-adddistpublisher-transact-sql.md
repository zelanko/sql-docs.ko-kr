...
|**1** (기본값)|배포자의 복제 에이전트는 Windows 인증을 사용하여 게시자에 연결합니다.|  
 ...
 [  **@login=**] **'***로그인***'**  
 로그인입니다. 이 매개 변수는 필요한 경우 *security_mode* 은 **0**합니다. *login*은 **sysname**이며 기본값은 NULL입니다. 이 매개 변수는 복제 에이전트가 게시자에 연결할 때 사용합니다.  
