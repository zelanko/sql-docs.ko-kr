...
|[@end_ip_address =] 'end_ip_address'|**VARCHAR(50)**|서버 수준 방화벽 설정 범위에서 가장 높은 IP 주소입니다. 이 값보다 작거나 같은 IP 주소는 [!INCLUDE[ssSDS](../../includes/sssds-md.md)] 서버에 연결을 시도할 수 있습니다. 가능한 가장 높은 IP 주소는 `255.255.255.255`입니다.<br /><br /> 참고: Azure 연결 시도는이 필드와 *start_ip_address* 필드가 모두 이면 `0.0.0.0`허용 됩니다.|  
...
## <a name="remarks"></a>설명  
 ...
 시작 IP 주소와 끝 IP 주소가 같은 `0.0.0.0`서버 수준 방화벽 설정을 추가 하면 Azure에서 [!INCLUDE[ssSDS](../../includes/sssds-md.md)] 서버에 액세스할 수 있습니다. 서버 수준 방화벽 설정의 용도를 기억할 수 있도록 하는 *이름* 매개 변수에 값을 제공 합니다.  
...
## <a name="examples"></a>예  
 다음 코드는 Azure에서 액세스할 수 있도록 하는 라는 `Allow Azure` 서버 수준 방화벽 설정을 만듭니다. 가상 master 데이터베이스에서 다음을 실행 합니다.  
