...
|**update_languages**|NULL|전체 텍스트 검색에 등록된 언어 목록과 필터를 업데이트합니다. 언어는 인덱싱을 구성할 때 전체 텍스트 쿼리에 지정됩니다. 필터는 같은 데이터 형식, 저장 된.docx 등 해당 파일 형식에서 텍스트 정보를 추출 하는 필터 데몬 호스트에서 사용 **varbinary**, **varbinary (max)**, **이미지** , 또는 **xml**, 전체 텍스트 인덱싱에 대 한 합니다.<br /><br /> 자세한 내용은 [등록된 필터와 단어 분리기 보기 및 변경](../../relational-databases/search/view-or-change-registered-filters-and-word-breakers.md)을 참조하세요.|  
