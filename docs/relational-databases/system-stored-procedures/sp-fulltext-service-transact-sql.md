...
`[ @action = ] 'action'` 변경 하거나 다시 설정할 속성이입니다. *동작* 됩니다 **nvarchar(100),** 기본값은 없습니다. 목록은*c*속성, 해당 설명 및 설정할 수 있는 값이 아래 표를 참조 합니다 *값* 인수입니다. 이 인수는 데이터 형식, 현재 실행 값, 최소값 또는 최대값, 사용 중단 상태(해당되는 경우)와 같은 속성을 반환합니다.  
