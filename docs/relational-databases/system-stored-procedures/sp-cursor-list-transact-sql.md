...
|cursor_rows|**int**|결과 집합의 행 수입니다. 자세한 내용은 [@@CURSOR_ROWS](../../t-sql/functions/cursor-rows-transact-sql.md)합니다.|  
...
-   API 서버 커서를 커서 이름을 SQLSetCursorName 이라고 하는 ODBC 응용 프로그램에서 열립니다.  
  
 커서가 반환한 결과 집합의 특성 설명을 보려면 sp_describe_cursor_columns를 사용하십시오. 커서가 참조하는 기본 테이블의 보고서를 보려면 sp_describe_cursor_tables를 사용하십시오. sp_describe_cursor는 sp_cursor_list와 이지만 지정된 된 커서에 대해서만 동일한 정보를 보고 합니다.  
