...
`[ @ignore_distributor = ] ignore_distributor`은이 저장 프로시저가 배포자에 연결 되지 않고 실행 되는지 여부를 나타냅니다. *ignore_distributor* 은 **bit**이며 기본값은 **0**입니다. 즉, 배포자가 게시 데이터베이스의 새 상태로 연결 되어 업데이트 되어야 합니다. 값 **1** 은 배포자에 액세스할 수 없고 게시를 사용 하지 않도록 설정 하는 **sp_replicationdboption** 사용 되는 경우에만 지정 해야 합니다.  
