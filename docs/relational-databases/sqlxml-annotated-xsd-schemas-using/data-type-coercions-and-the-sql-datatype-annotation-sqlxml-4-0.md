...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]XSD 스키마에서는 **xsd: type** 특성은 요소 또는 특성의 XSD 데이터 형식을 지정 합니다. XSD 스키마를 사용하여 데이터베이스에서 데이터를 추출할 경우 지정된 데이터 형식이 데이터 서식 지정에 사용됩니다.  
...
> [!NOTE]  
>  반환 된 값 중 일부 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 를 사용 하 여 지정 된 XML 데이터 형식과 호환 되지 않을 수 있습니다 **xsd: type**변환이 가능 때문에 (예를 들어 변환할 "XYZ"는  **10 진수** 데이터 형식)에 게 없거나 해당 데이터 형식의 범위를 초과 하는 값 (-100000으로 변환 하는 예를 들어는 **UnsignedShort** XSD 유형). 호환되지 않는 형식 변환은 XML 문서를 유효하지 않게 만들거나 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 오류를 발생시킬 수 있습니다.  
...
-   XML 데이터 형식 **base64Binary** 지도를 다양 한 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터 형식 (**이진**, **이미지**, 또는 **varbinary**). XML 데이터 형식에 매핑할 **base64Binary** 특정 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 사용 하 여 데이터 형식으로 **sql: datatype** 주석입니다. 이 주석은 특성이 매핑될 열의 명시적인 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터 형식을 지정합니다. 데이터가 데이터베이스에 저장될 경우 이 주석을 사용하는 것이 좋습니다. 지정 하 여는 **sql: datatype** 주석을 명시적으로 식별할 수 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터 형식입니다.  
...
### <a name="a-specifying-xsdtype"></a>1. xsd:type 지정  
 XSD 방법을 보여 주는이 예제 **날짜** 를 사용 하 여 지정 된 형식에는 **xsd: type** 스키마의 특성 결과 XML 문서에 영향을 줍니다. 이 스키마에서는 AdventureWorks 데이터베이스에 Sales.SalesOrderHeader 테이블의 XML 뷰를 제공합니다.  
...
-   지정 **xsd: type = 날짜** 에 **OrderDate** 특성을 반환 하는 값의 날짜 부분 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에 대 한는 **OrderDate** 특성이 표시 됩니다.  
  
-   지정 **xsd: type = 시간** 에 **ShipDate** 특성에서 반환 되는 값의 시간 부분 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에 대 한는 **ShipDate** 특성이 표시 됩니다.  
  
-   지정 하지 않는 **xsd: type** 에 **DueDate** 특성에서 반환 되는 동일한 값 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 표시 됩니다.  
