...
-   **1**: 제공 된 데이터가 유효 하지 않으면 SQLSTATE 22007 및 "잘못 된 날짜 시간 형식" 메시지와 진단 레코드가 생성 됩니다.  
  
-   **2**: 시간 필드는 0 이어야 합니다 또는 진단 레코드 SQLSTATE 22008 및 "일부가 잘렸습니다" 메시지가 생성 됩니다.  
  
-   **3**: 초 소수 0 이거나 SQLSTATE 22008 및 "일부가 잘렸습니다" 메시지가 진단 레코드가 생성 됩니다.  
  
-   **4**: 날짜 구성 요소가 무시 됩니다.  
  
-   **5**: 시간대가 클라이언트의 표준 시간대 설정으로 설정 되어 있습니다.  
  
-   **6**: 시간이 0으로 설정 됩니다.  
  
-   **7**: 날짜를 현재 날짜로 설정 됩니다.  
  
-   **8**: 시간에서 클라이언트의 표준 시간대에서 UTC로 변환 됩니다. 변환 중 오류가 발생하면 SQLSTATE 22008 및 "Datetime 필드 오버플로" 메시지가 포함된 진단 레코드가 생성됩니다.  
  
-   **9**: 문자열을 구문 분석 하 고 날짜, datetime, datetimeoffset, 또는 첫 번째 문장 부호 문자를 발생 하 고 나머지 구성 요소가 있는지에 따라 시간 값으로 변환 합니다. 그런 다음 위 표에서 이 프로세스에 의해 발견된 원본 형식에 대한 규칙에 따라 문자열이 대상 형식으로 변환됩니다. 데이터를 구분 분석하는 중에 오류가 발견되면 SQLSTATE 22018 및 "캐스트 사양의 문자 값이 올바르지 않습니다"라는 메시지가 표시되고 진단 레코드가 생성됩니다. datetime 및 smalldatetime 매개 변수의 경우 연도가 이러한 형식에서 지원하는 범위를 벗어나면 SQLSATE 22007 및 "잘못된 날짜 시간 형식입니다"라는 메시지가 표시되고 진단 레코드가 생성됩니다.  
...
-   **10**: 데이터 손실 잘림이 발생 SQLSTATE 22008 및 "잘못 된 시간 형식" 메시지와 진단 레코드가 생성 됩니다. 이 오류는 값이 서버에서 사용된 UTC 범위로 표현할 수 있는 범위 밖에 있는 경우에도 발생합니다.  
  
-   **11**: 데이터의 바이트 길이가 SQL 형식에 필요한 구조체의 크기를 같지 않고, SQLSTATE 22003 및 "숫자 값 범위를 벗어났습니다." 메시지를 사용 하 여 진단 레코드가 생성 됩니다.  
  
-   **12**: 원시 TDS smalldatetime 또는 datetime 형식의 서버 데이터의 바이트 길이가 4 또는 8 인 경우 전송 됩니다. 바이트 길이가 SQL_TIMESTAMP_STRUCT의 크기와 정확히 일치하는 데이터는 TDS 형식의 datetime2로 변환됩니다.  
  
-   **13**: 데이터 손실 잘림이 발생 SQLSTATE 22001 및 "문자열 데이터를 바로 잘렸습니다" 메시지 진단 레코드가 생성 됩니다.  
...
-   **N/A**: 기존 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 이전 동작이 유지 됩니다.  
