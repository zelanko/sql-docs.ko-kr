...
[!INCLUDE[SNAC_Deprecated](../../includes/snac-deprecated.md)]
 ...
 다음 표에서 설명하는 변환은 클라이언트에서 수행되는 변환에 해당합니다. 클라이언트가 매개 변수에 대해 다른 서버에 정의 된 초 소수 부분 자릿수를 지정 하는 있는 경우에 클라이언트 변환이 성공 하더라도 서버에서 오류를 반환 하는 경우 **SQLExecute** 또는  **SQLExecuteDirect** 라고 합니다. 반면 ODBC에서 소수 자릿수 초 잘림을 모두 오류로 처리 하는 특히 합니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 는 반올림 동작; 예를 들어 반올림에서 이동 하면 **datetime2(6)** 하려면 **datetime2(2)**. datetime 열 값은 1/300초로 반올림되며 smalldatetime 열은 서버에 의해 0초로 설정됩니다.  
...
-   **-**: 변환이 지원 됩니다. SQLSTATE 07006 및 "제한된 데이터 형식 특성을 위반했습니다"라는 메시지가 표시되고 진단 레코드가 생성됩니다.  
  
-   **1**: 제공 된 데이터가 유효 하지 SQLSTATE 22007 및 "datetime 형식이 잘못 되었습니다" 메시지를 사용 하 여는 진단 레코드가 생성 됩니다.  
  
-   **2**: 시간 필드가 0 이거나 SQLSTATE 22008 및 "일부가 잘렸습니다" 메시지가 포함 된 진단 레코드가 생성 됩니다.  
  
-   **3**: 소수 자릿수 초가 0 이어야 하며 또는 SQLSTATE 22008 및 "일부가 잘렸습니다" 메시지가 포함 된 진단 레코드가 생성 됩니다.  
...
-   **10**: SQLSTATE 22008 및 "잘못 된 시간 형식 입니다" 메시지가 포함 된 진단 레코드가 생성 됩니다 잘림을 수행 하 여 데이터 손실이 발생 하는 경우. 이 오류는 값이 서버에서 사용된 UTC 범위로 표현할 수 있는 범위 밖에 있는 경우에도 발생합니다.  
  
-   **11**: 데이터의 바이트 길이가 SQL 형식에 필요한 구조의 크기 같지 않음, SQLSTATE 22003 및 "숫자 값이 범위를 벗어났습니다" 메시지가 진단 레코드가 생성 됩니다.  
  
-   **12**: 원시 TDS smalldatetime 또는 datetime 형식으로 서버 데이터의 바이트 길이가 4 또는 8 인 경우 전송 됩니다. 바이트 길이가 SQL_TIMESTAMP_STRUCT의 크기와 정확히 일치하는 데이터는 TDS 형식의 datetime2로 변환됩니다.  
  
-   **13**: SQLSTATE 22001 및 "문자열 데이터, 오른쪽이 잘렸습니다" 메시지가 포함 된 진단 레코드가 생성 됩니다 잘림을 수행 하 여 데이터 손실이 발생 하는 경우.  
