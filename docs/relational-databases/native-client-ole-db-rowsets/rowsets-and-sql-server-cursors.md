...
-   다음과 같은 서버 커서  
 ...
 소비자는 특정 행 집합 속성을 설정하여 행 집합에 다른 커서 동작을 요청할 수 있습니다. 소비자가 이러한 행 집합 속성 중 하나를 설정하지 않거나 모두 기본 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 값으로 설정하면 Native Client OLE DB 공급자는 기본 결과 집합을 사용하여 행 집합을 구현합니다. 이러한 속성 중 하나가 기본값이 아닌 값으로 설정된 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 경우 네이티브 클라이언트 OLE DB 공급자는 서버 커서를 사용하여 행 집합을 구현합니다.  
...
|DBPROP_REMOVEDELETED|VARIANT_TRUE|행 집합을 통해 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터를 업데이트할 수 없습니다. 행 집합은 정방향 스크롤만 지원합니다. 상대적인 행 위치 지정은 지원됩니다. 다른 속성으로 제한되지 않은 경우 명령 텍스트에 ORDER BY 절이 포함될 수 있습니다.|  
 ...
 서버 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 커서에서 지원하는 네이티브 클라이언트 OLE DB 공급자 행집합은 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **IOpenRowset::OpenRowset** 메서드를 사용하여 기본 테이블이나 뷰에서 쉽게 만들 수 있습니다. *rgPropertySets* 매개 변수에 필요한 행 집합 속성 세트를 전달하여 이름으로 테이블이나 뷰를 지정합니다.  
...
## <a name="sql-server-cursor-block-size"></a>SQL Server 커서 블록 크기  
 커서가 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 네이티브 클라이언트 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] OLE DB 공급자 행 집합을 지원하는 경우 행의 요소 수는 **IRowset::GetNextRows** 또는 **IRowsetLocate::GetRowsAt** 메서드의 배열 매개 변수를 처리하여 커서 블록 크기를 정의합니다. 배열의 핸들로 표시된 행이 커서 블록의 멤버입니다.  
