...
ms.locfileid: "81300263"
---
# <a name="updating-data-in-sql-server-cursors"></a>SQL Server 커서의 데이터 업데이트
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

  커서를 통해 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터를 가져오고 업데이트할 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 때 네이티브 클라이언트 OLE DB 공급자 소비자 응용 프로그램은 다른 클라이언트 응용 프로그램에 적용되는 동일한 고려 사항 및 제약 조건에 구속됩니다.  
 ...
 트랜잭션이 오랫동안 열려 있을 수 있도록 설계된 클라이언트 애플리케이션의 경우 트랜잭션 격리 수준 때문에 행 위치 지정에 상당한 시간 지연이 발생할 수 있습니다. 기본적으로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 네이티브 클라이언트 OLE DB 공급자는 DBPROPVAL_TI_READCOMMITTED 지정한 읽기 커밋된 격리 수준을 사용합니다. 네이티브 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 클라이언트 OLE DB 공급자는 행 집합 동시성을 읽기 전용으로 설정할 때 더티 읽기 격리를 지원합니다. 따라서 소비자는 수정이 가능한 행 집합에 더 높은 수준의 격리를 요청할 수 있지만 더 낮은 수준은 요청할 수 없습니다.  
 ...
 **IRowsetUpdate::Update를**사용하는 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 네이티브 클라이언트 OLE DB 공급자는 표시된 각 행을 처리하려고 시도합니다. 모든 행에 대한 잘못된 데이터, 길이 또는 상태 값으로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인해 발생하는 오류는 Native Client OLE DB 공급자 처리를 중지하지 않습니다. 업데이트에 참여하는 행은 모두 수정되거나 전혀 수정되지 않습니다. 소비자는 반환된 *prgRowStatus* 배열을 검사하여 네이티브 클라이언트 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] OLE DB 공급자가 DB_S_ERRORSOCCURRED 반환할 때 특정 행에 대한 오류를 확인해야 합니다.  
