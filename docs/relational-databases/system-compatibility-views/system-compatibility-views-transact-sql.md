...
[!INCLUDE[tsql-appliesto-ss2012-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2012-xxxx-xxxx-xxx-md.md)]
 ...
 카탈로그 뷰로 업그레이드해야 하는 또 다른 이유는 사용자 ID 및 유형 ID를 저장하는 호환성 뷰 열이 NULL을 반환하거나 산술 오버플로를 트리거할 수 있기 때문입니다. 이것은 사용자, 그룹 및 역할과 데이터 형식을 32,767개 이상 만들 수 있기 때문입니다. 예를 들어 32,768 명의 사용자를 만들고 다음 다음 쿼리를 실행 하는 경우: `SELECT * FROM sys.sysusers`합니다. ARITHABORT를 ON으로 설정하는 경우 산술 오버플로 오류가 발생하여 쿼리가 실패하게 되고 ARITHABORT를 OFF로 설정 하는 경우는 **uid** 열 NULL을 반환 합니다.  
...
|**uid**|**sysprocesses**|**sys.dm_exec_requests**|  
 ...
 시스템에 SQL Server 2000에서 지원 되지 않는으로 발표 된 테이블로 사용자 데이터베이스에서 참조 하는 경우 (같은 **syslanguages** 하거나 **syscacheobjects**), 이제에서 역호환성 뷰로 바인딩됩니다는 **sys** 스키마입니다. SQL Server 2000 시스템 테이블이 여러 버전에서 더 이상 사용되지 않으므로 이는 새로운 변경 사항이 아닙니다.  
  
 예: 라는 사용자 테이블을 만드는 **syslanguages** 사용자-데이터베이스에서 SQL Server 2008에서는 문이 `SELECT * from dbo.syslanguages;` 에서 해당 데이터베이스는 반환 값을 사용자 테이블에서. SQL Server 2012부터,이 연습에서에서 데이터가 반환 됩니다 시스템 뷰 **sys.syslanguages**합니다.  
