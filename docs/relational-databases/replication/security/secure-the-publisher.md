---
title: "게시자 보안 설정 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
> [!NOTE]  
>  복제는 각 게시의 게시자에 PAL 멤버 자격을 적용할 역할을 만듭니다. 폼에서 역할의 이름을 **Msmerge_***\< PublicationID>* 병합 복제 및 **MSReplPAL_***\< PublicationDatabaseID>***_***\< PublicationID>* 트랜잭션 및 스냅숏 복제에 대 한 합니다.  
  
 기본적으로 PAL에는 게시 생성 시의 **sysadmin** 고정 서버 역할의 멤버와 게시를 만드는 데 사용된 로그인이 포함됩니다. 기본적으로 모든 로그인의 구성원 인는 **sysadmin** 고정된 서버 역할 또는 **db_owner** 게시 데이터베이스의 고정된 데이터베이스 역할은 PAL에 명시적으로 추가 되지 않고 게시를 구독할 수 있습니다.  
...
-   PAL에 Windows 계정이 포함되어 있고 도메인에서 Active Directory를 사용할 경우 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 를 실행하는 계정에 Active Directory에서 읽을 수 있는 권한이 있어야 합니다. Windows 계정에 문제가 발생하면 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 를 실행하는 계정에 충분한 사용 권한이 있는지 확인합니다. 자세한 내용은 Windows 설명서를 참조하세요.  
  
 PAL을 관리 하기 위해 참조 [게시 액세스 목록에서 로그인 관리](../../../relational-databases/replication/security/manage-logins-in-the-publication-access-list.md)합니다.  
...
## FTP 스냅숏 배달  
 UNC 공유가 아닌 FTP 공유를 통해서만 스냅숏을 사용할 수 있도록 지정한 경우 FTP 액세스를 구성할 때 로그인과 암호를 지정해야 합니다. 자세한 내용은 참조 [FTP를 통해 스냅숏 배달](../../../relational-databases/replication/publish/deliver-a-snapshot-through-ftp.md)합니다.  
...
## 큐 판독기 에이전트  
 지정된 배포자와 관련된 모든 게시자와 게시(지연 업데이트 구독 허용)에 하나의 큐 판독기 에이전트가 있습니다. 자세한 내용은 참조 [트랜잭션 게시에 대 한 업데이트 구독을 사용 하도록 설정](../../../relational-databases/replication/publish/enable-updating-subscriptions-for-transactional-publications.md)합니다.  
