---
title: "조인 추가 또는 편집 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
1.  **필터링된 테이블을 확인하고 조인된 테이블을 선택하십시오.**  
    ...
    -   새 조인을 추가 하는 경우를 확인 하는 테이블에는 **필터링 된 테이블** 텍스트 상자는 올바른 (올바르지 않으면 클릭 **취소**, 에 올바른 테이블을 선택 합니다.는 **테이블 행 필터** 페이지를 클릭 하 여 **조인 추가** 이 대화 상자로 돌아갑니다). 다음에서 테이블을 선택 된 **조인 된 테이블** 드롭다운 목록 상자입니다.  
...
2.  **조인 문을 작성하십시오.**  
    ...
    -   새 조인을 추가하는 경우 **작성기를 사용하여 문 작성** 또는 **조인 문 직접 작성**을 선택합니다. 수동으로 조인 작성을 시작하면 작성기를 사용할 수 없습니다.  
         ...
         작성기를 사용 하 여 선택 하는 경우 열을 사용 하 여 표의 (**함께**, **필터링 된 테이블 열**, **연산자**, 및 **조인 된 테이블 열**) 조인 문을 작성 하 합니다. 모눈의 각 열을 드롭다운 목록 상자가 들어 있어 두 개의 열과 연산자를 선택할 수 있습니다 (**=**, **<>**, **<=**, **\<**, **>=**, **>**, **같은**). 결과는 **미리 보기** 텍스트 영역에 표시됩니다. 조인에 둘 이상의 열 쌍을 포함 하는 경우에 결합을 선택 (**AND** 또는 **OR**)에서 **함께** 열을 두 개 이상의 열과 다른 연산자를 입력 합니다.  
