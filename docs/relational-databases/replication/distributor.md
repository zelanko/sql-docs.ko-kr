...
-   게시에 대한 스냅숏 파일이 저장되어 있는 경우(일반적으로 배포자에 저장되어 있음) 추가 디스크 공간  
...
## 옵션  
 **' \< 서버 이름>' 역할을 자체 배포자로 사용. SQL Server에서 배포 데이터베이스와 로그를 만듭니다.**  
 ...
 **추가**  
 배포자를 나열 하지 않으면 사용 하려는 서버를 클릭 하는 경우 **추가** 하는 서버를 식별 하 고 목록에 추가 합니다.  
