---
title: "웹 동기화를 위한 IIS 7 구성 | Microsoft Docs"
ms.custom: ""
ms.date: "09/12/2016"
...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx_md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  이 항목의 절차는 수동으로 구성 프로세스를 안내 있습니다 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 인터넷 정보 서비스 (IIS) 버전 7 이상에 대 한 병합 복제에 대 한 웹 동기화에 사용 합니다. 
  
 IIS 7 이상이 구성은 웹 동기화를 사용 하는 데 필요한 세 단계 중 첫 번째 숫자입니다.  
  
 전체 구성 프로세스의 개요를 참조 하십시오. [웹 동기화 구성](../../relational-databases/replication/configure-web-synchronization.md)합니다.  
...
웹 동기화는 iis에서 5.0 버전부터 지원 됩니다. 구성할 웹 동기화 마법사의 IIS 버전 5 및 6는 IIS 버전 7.0 이상 사용할 수 없습니다. **IIS 서버에서 웹 동기화 구성 요소를 사용 하 여 SQL Server 2012 부터는 복제와 함께 SQL Server를 설치 해야 합니다. 이 무료 SQL Server Express edition을 수 있습니다.**
  
#### SQL Server 복제 수신기를 설치 및 구성하려면  
  
1.  IIS 컴퓨터에 SQL Server 복제를 설치 합니다.

2. IIS를 실행하는 컴퓨터에 replisapi.dll에 대한 새 파일 디렉터리를 만듭니다. 원하는 있지만 디렉터리 아래에 만드는 것이 좋습니다 위치에 디렉터리를 만들 수는 \<*드라이브*>: \Inetpub 디렉터리입니다. 예를 들어, 디렉터리를 만들 \<*드라이브*>: \Inetpub\SQLReplication\\합니다.  
  
3.  디렉터리에서 replisapi.dll 복사 [!INCLUDE[ssInstallPathVar](../../includes/ssinstallpathvar-md.md)]com\ 1 단계에서 만든 파일 디렉터리에 있습니다.  
...
6.  IIS에 가상 디렉터리를 만듭니다. 가상 디렉터리는 4단계에서 만든 웹 사이트 아래에 만들어야 하며 1단계에서 만든 디렉터리에 매핑해야 합니다. 이 디렉터리에 대한 사용 권한은 가능한 제한적으로 할당하는 것이 좋습니다. 적어도 **읽기** 및 **실행** 권한을 선택해야 합니다.  
    ...
    1.   **인터넷 정보 서비스 (IIS) 관리자**, 에 **연결** 창에서 마우스 오른쪽 단추로 클릭 **기본 웹 사이트**, 를 선택한 다음 **가상 디렉터리 추가**합니다.  
  
    2.  **별칭**에 **SQLReplication**를 입력합니다.  
  
    3.  에 대 한 **실제 경로**, 입력 **\< 드라이브>: \Inetpub\SQLReplication\\**, 를 클릭 하 고 **확인**합니다.  
  
7.  replisapi.dll이 실행될 수 있도록 IIS를 구성합니다.  
  
    1.   **인터넷 정보 서비스 (IIS) 관리자**, 클릭 **기본 웹 사이트**합니다.  
    ...
    5.   **모듈** 드롭 다운 목록에서 **IsapiModule**합니다.  
  
    6.  에 대 한 **실행 파일**, 입력 **\< 드라이브>: \Inetpub\SQLReplication\replisapi.dll**합니다.  
    ...
    10. Replisapi.dll이 **사용** 처리기 매핑 아래에 나열되어 있는지 확인합니다. 에 **비활성화** 목록 하 고 있는 경우 Replisapi 항목을 마우스 오른쪽 단추로 클릭 한 다음 **기능 사용 권한 편집**합니다. **실행** 상자를 선택한 다음 **확인**을 클릭합니다.  
...
1.   **인터넷 정보 서비스 (IIS) 관리자**, 로컬 서버 노드를 차례로 클릭 한 다음는 **기본 웹 사이트** (또는 웹 동기화 사이트는 기본 웹 사이트와에서 다른 경우).  
  
2.  가운데 창에서 두 번 클릭 **SSL 설정**합니다.  
  
3.  **SSL 필요** 옵션을 선택합니다. **클라이언트 인증서**아래에서 **무시** 단추가 선택되어 있는지 확인합니다.  
  
#### 테스트를 위한 자체 서명된 인증서를 만들려면  
  
1.   **인터넷 정보 서비스 (IIS) 관리자**, 로컬 서버 노드를 클릭 한 다음 가운데 창에서 두 번 클릭에 **서버 인증서**합니다.  
  
2.  에 **작업** 창에서 클릭 **자체 서명 된 인증서 만들기**합니다.  
  
3.  에 **자체 서명 된 인증서 만들기** 대화 상자에서 인증서에 대 한 이름을 입력 하 고 클릭 한 다음 **확인**합니다.  
  
###### 인증서를 웹 사이트에 바인딩하려면  
  
1.  에 **연결** 창에서 클릭 된 **기본 웹 사이트** (또는 웹 동기화 사이트와 기본 웹 사이트와에서 다른 경우).  
  
2.  **동작** 창에서 **바인딩**을 클릭한 다음 **추가**를 클릭합니다. **사이트 바인딩 추가** 대화 상자가 나타납니다.  
  
3.   **형식** 드롭 다운 목록에서 **https**합니다. **IP 주소** 및 **포트**에 대한 기본 설정은 그대로 둡니다.  
  
4.   **SSL 인증서** 드롭 다운 목록에서 "테스트를 위해 자체 서명 된 인증서를 만들려면"에서 만든 인증서를 클릭 **확인**, 를 클릭 하 고 **닫기**합니다.  
  
###### 인증서를 테스트하려면  
  
1.   **인터넷 정보 서비스 (IIS) 관리자**, 클릭 **기본 웹 사이트입니다.**  
  
2.   **작업** 창에서 클릭 **찾아보기 \*: 443(https)**합니다.  
  
3.  Internet Explorer가 열리고 "이 웹 사이트의 보안 인증서에 문제가 있습니다."라는 메시지가 표시됩니다. 이 경고에서는 연결된 인증서가 공인 CA에 의해 발급되지 않았으며 신뢰할 수 없을 수도 있음을 알립니다. 이 예상 되는 경고, 이므로 클릭 **(권장 하지 않음)이 웹이 사이트를 계속 탐색**합니다.  
...
1.  IIS를 실행하는 컴퓨터에 로컬 계정을 만듭니다.  
    ...
    1.  **서버 관리자**를 엽니다. 시작 메뉴에서 마우스 오른쪽 단추로 클릭 **컴퓨터**, 를 클릭 하 고 **관리**합니다.  
  
    2.  **서버 관리자**에서 **구성**을 확장한 다음 **로컬 사용자 및 그룹**을 확장합니다.  
  
    3.  마우스 오른쪽 단추로 클릭 **사용자**, 를 클릭 하 고 **새 사용자**합니다.  
...
2.  IIS_IUSRS 그룹에 계정을 추가합니다.  
    ...
    2.  마우스 오른쪽 단추로 클릭 **IIS_IUSRS**, 를 클릭 하 고 **그룹에 추가**합니다.  
  
    3.  에 **IIS_IUSRS 속성** 대화 상자를 클릭 하 여 **추가**합니다.  
  
    4.  **사용자, 컴퓨터 또는 그룹 선택** 대화 상자에 1단계에서 만든 계정을 추가합니다.  
  
    5.  확인 **이 위치에서** (도메인이 아닌) 로컬 컴퓨터의 이름을 표시 합니다. 이 필드에 로컬 컴퓨터 이름이 표시되지 않는 경우 **위치**를 클릭합니다. **위치** 대화 상자에서 로컬 컴퓨터를 선택한 다음 **확인**을 클릭합니다.  
  
    6.  에 **사용자 선택** 대화 상자 및 **IIS_IUSRS 속성** 대화 상자를 클릭 하 여**확인**합니다.  
  
3.  replisapi.dll을 포함하는 폴더에 최소 계정 사용 권한을 부여합니다.  
  
    1.  Windows 탐색기에서 replisapi.dll에 대해 만든 폴더를 마우스 오른쪽 단추로 클릭 하 고 클릭 **속성**합니다.  
  
    2.  **보안** 탭에서 **편집**을 클릭합니다.  
  
    3.   **에 대 한 권한을 \< foldername>** 대화 상자, 클릭 **추가** 1 단계에서 만든 계정을 추가 합니다.  
  
    4.  확인 **이 위치에서** (도메인이 아닌) 로컬 컴퓨터의 이름을 표시 합니다. 이 필드에 로컬 컴퓨터 이름이 표시되지 않는 경우 **위치**를 클릭합니다. **위치** 대화 상자에서 로컬 컴퓨터를 선택한 다음 **확인**을 클릭합니다.  
  
    5.  계정 권한을 부여 하 고만 확인 **읽기**, **읽기 및 실행**, 및 **폴더 내용** 사용 권한입니다.  
  
    6.  디렉터리에 대한 액세스가 필요 없는 사용자나 그룹을 선택하고 **제거**를 클릭한 다음 **확인**을 클릭합니다.  
  
4.  응용 프로그램 풀 만들기 **인터넷 정보 서비스 (IIS) 관리자**:  
  
    1.   **인터넷 정보 서비스 (IIS) 관리자**, 에 **연결** 창에서 로컬 서버 노드를 확장 합니다.  
  
    2.  마우스 오른쪽 단추로 클릭 **응용 프로그램 풀**, 를 클릭 하 고 **응용 프로그램 풀 추가**합니다.  
  
    3.  응용 프로그램 풀의 이름을 입력하고 남아 있는 필드에 대한 기본값을 그대로 둔 다음 **확인**을 클릭합니다.  
  
    > [!NOTE]  
    >  동기화 클라이언트가 세 개 이상 동시에 실행되도록 하려면 웹 가든을 만들어야 합니다. 자세한 내용은 "웹 가든 만들기"를 참조 [웹 동기화 구성](../../relational-databases/replication/configure-web-synchronization.md)합니다.  
  
5.  계정과 응용 프로그램 풀을 연결합니다.  
  
    1.   **인터넷 정보 서비스 (IIS) 관리자**, 로컬 서버 노드를 확장 한 다음 클릭 **응용 프로그램 풀**합니다.  
  
    2.  작성 하 고 클릭 한 다음 응용 프로그램 풀을 마우스 오른쪽 단추로 클릭 **응용 프로그램 풀 기본값 설정**합니다.  
...
6.  응용 프로그램 풀을 복제 웹 사이트와 연결합니다.  
    ...
    1.   **인터넷 정보 서비스 (IIS) 관리자**, 로컬 서버 노드를 확장 한 다음 클릭는 **기본 웹 사이트** (또는 웹 동기화 사이트는 기본 웹 사이트와에서 다른 경우).  
  
    2.  **동작** 창의 **웹 사이트 관리**에서 **고급 설정**을 클릭합니다.  
  
    3.  **고급 설정** 대화 상자에서 **응용 프로그램 풀**의 오른쪽에 있는 줄임표 단추를 클릭합니다.  
  
    4.   **응용 프로그램 풀** 드롭 다운 목록에서 4 단계에서 만든 응용 프로그램 풀을 선택 하 고 클릭 **확인**합니다.  
...
2.  구독자에서는 Internet Explorer에서 replisapi.dll에 대한 주소에 `?diag` (예: 예를 들어: **https://server.domain.com/directory/replisapi.dll?diag**합니다.  
    ...
    > [!NOTE]  
    >  위의 예에서 **server.domain.com** 정확한로 대체 해야 **발급 대상** 이름 아래에 나열 된는 **서버 인증서** IIS 관리자에서 섹션입니다.  
