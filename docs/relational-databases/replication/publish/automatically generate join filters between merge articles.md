...
# 병합 아티클 간의 조인 필터 집합 자동 생성(SQL Server Management Studio)
  조인 필터 집합을 자동으로 생성 된 **테이블 행 필터** 새 게시 마법사의 페이지 또는 **행 필터** 의 페이지는 **게시 속성-\< 게시>** 대화 상자. 마법사를 사용 하 고 대화 상자에 액세스 하는 방법에 대 한 자세한 내용은 참조 [게시를 만들](../../../relational-databases/replication/publish/create-a-publication.md) 및 [보기 및 게시 속성을 수정](../../../relational-databases/replication/publish/view-and-modify-publication-properties.md)합니다.  
  
> [!NOTE]  
>  조인 필터 집합을 자동으로 생성 하는 경우는 **게시 속성-\< 게시>** 대화 상자는 게시에 구독이 초기화 된 후 새 스냅숏을 생성 하 고 변경을 수행한 후 모든 구독을 다시 초기화 해야 합니다. 속성 변경에 대 한 요구 사항에 대 한 자세한 내용은 참조 [변경 게시 및 아티클 속성](../../../relational-databases/replication/publish/change-publication-and-article-properties.md)합니다.  
  
 테이블 집합에 대해 수동으로 조인 필터를 만들거나 테이블에 정의된 외래 키와 기본 키 간의 관계를 기반으로 복제에서 필터를 자동으로 생성할 수 있습니다. 만들기에 대 한 자세한 내용은 조인 필터를 수동으로 참조 하십시오. [정의 하 고는 조인 필터 간의 병합 아티클을 수정](../../../relational-databases/replication/publish/define-and-modify-a-join-filter-between-merge-articles.md)합니다.  
  
### 병합 아티클 간의 조인 필터 집합을 자동으로 생성하려면  
  
1.  에 **테이블 행 필터** 새 게시 마법사의 페이지 또는 **행 필터** 의 페이지는 **게시 속성-\< 게시>**, 클릭 **추가**, 를 클릭 하 고 **자동으로 필터 생성**.  
