---
title: 가용성 그룹에서 배포 데이터베이스 구성
description: Always On 가용성 그룹을 사용하여 SQL Server 복제용 배포 데이터베이스를 구성합니다.
...
- 수신기 이름을 배포자의 이름으로 사용하여 복제 작업을 만듭니다. 배포 서버에서 생성되는 복제 스냅샷, 로그 판독기 및 배포 에이전트(밀어넣기 구독) 작업이 배포 DB용 AG의 모든 보조 복제본에서 생성됩니다.
 >[!NOTE]
...
1. `distribution` 데이터베이스를 삭제하고 스냅샷 디렉터리를 유지하려면 다음을 실행합니다. 
