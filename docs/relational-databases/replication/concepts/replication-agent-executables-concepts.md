...
### <a name="running-the-snapshot-agent"></a>스냅샷 에이전트 실행  
 이 예제 배치 파일은 명령 프롬프트에서 스냅샷 에이전트를 호출하여 **AdvWorksSalesOrdersMerge** 게시에 대한 스냅샷을 생성합니다. (아래의 스크립트는 [!INCLUDE[ssSQL15_md](../../../includes/sssql15-md.md)] 파일(버전 130) 경로를 사용합니다. [!INCLUDE[ssNoVersion_md](../../../includes/ssnoversion-md.md)] 버전의 파일을 가리키도록 스크립트를 조정해야 합니다.)  
