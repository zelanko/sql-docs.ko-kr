...
|저장 프로시저가 참조하는 개체가 필요합니다.|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 테이블 및 사용자 정의 함수 같은 게시 된 저장된 프로시저에서 참조 하는 모든 개체는 구독자에서 사용할 되도록 요구 합니다. 참조된 개체가 이 게시에서 아티클로 게시되지 않으면 구독자에서 이를 수동으로 만들어야 합니다.|**뒤로** 단추를 사용하여 **아티클** 페이지로 이동합니다. 필요한 개체를 추가합니다.|  
