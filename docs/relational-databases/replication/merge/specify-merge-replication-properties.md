...
1.  [ 매개 변수에 ](../../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md)1**또는**2**값을 지정하고**sp_addmergearticle`@subscriber_upload_options`을 실행합니다. 다음은 숫자 값과 이에 해당하는 동작입니다.  
...
2.  1단계에서 반환된 값이 **0**이면 [에 ](../../../relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql.md)subscriber_upload_options**값,** 및 `@property`에 **1** 값, `@force_invalidate_snapshot`에 다음 동작에 해당하는 `@force_reinit_subscription`1**또는**2**값을 지정하고**sp_changemergearticle`@value`을 실행합니다.  
...
1.  게시 데이터베이스의 게시자에서 [을 지정하고 ](../../../relational-databases/system-stored-procedures/sp-helpmergepublication-transact-sql.md)sp_helpmergearticle`@publication`을 실행합니다. 대화형 해결 프로그램을 사용할 결과 집합의 각 아티클에 대해 **allow_interactive_resolver** 값을 확인합니다.   
    -   이 값이 **1**이면 대화형 해결 프로그램이 사용됩니다.    
    -   값이 **0**이면 각 아티클에서 먼저 대화형 해결 프로그램을 설정해야 합니다. 이렇게 하려면 [, ](../../../relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql.md)을 지정한 다음 `@publication`에 `@article`allow_interactive_resolver**값,** 에 `@property`true**값을 지정하고**sp_changemergearticle`@value`을 실행합니다.    
...
3.  구독 데이터베이스의 구독자에서 다음 매개 변수를 지정하여 [sp_addmergepullsubscription_agent](../../../relational-databases/system-stored-procedures/sp-addmergepullsubscription-agent-transact-sql.md)를 실행합니다.    
    ...
    -   **에** true`@enabled_for_syncmgr` 값    
    -   **에** true`@use_interactive_resolver` 값    
    -   병합 에이전트에 필요한 보안 계정 정보. 자세한 내용은 [끌어오기 구독 만들기](../../../relational-databases/replication/create-a-pull-subscription.md)를 참조하세요.    
...
1.  게시 데이터베이스의 게시자에서 [sp_addmergearticle](../../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md)을 실행합니다. `@publication`에 아티클이 속한 게시의 이름, `@article`에 아티클의 이름, `@source_object`에 게시되는 데이터베이스 개체, **에** true`@allow_interactive_resolver` 값을 지정합니다. 자세한 내용은 [아티클을 정의](../../../relational-databases/replication/publish/define-an-article.md)을 참조하세요.  
...
1.  게시 데이터베이스의 게시자에서 [에 다음 값 중 하나를 지정하고 ](../../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md)sp_addmergearticle`@column_tracking`을 실행합니다.  
...
2.  게시 데이터베이스의 게시자에서 [sp_changemergearticle](../../../relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql.md)을 실행합니다. **에** column_tracking`@property` 값, `@value`에 다음 값 중 하나를 지정합니다.  
  
    -   **true** - 아티클에 대해 열 수준 추적을 사용합니다.    
    -   **false** - 행 수준 추적을 사용합니다(기본값).  
  
     **및**에 모두 `@force_invalidate_snapshot`1`@force_reinit_subscription` 값을 지정합니다. 
...
게시 데이터베이스의 게시자에서 [sp_addmergearticle&#40;Transact-SQL&#41;](../../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md)을 실행합니다. **에** false`@delete_tracking` 값을 지정합니다. 자세한 내용은 [아티클을 정의](../../../relational-databases/replication/publish/define-an-article.md)을 참조하세요.
...
2.  1단계의 값이 **1**이면 게시 데이터베이스의 게시자에서 [sp_changemergearticle&#40;Transact-SQL&#41;](../../../relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql.md)을 실행합니다. `delete_tracking`에 `@property` 값, **에** false`@value` 값을 지정합니다.  
...
2.  게시 데이터베이스의 게시자에서 [sp_changemergearticle&#40;Transact-SQL&#41;](../../../relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql.md)을 실행합니다. 이때 **에** processing_order`@property` 값, `@value`에 처리 순서를 나타내는 정수 값을 지정합니다.  
