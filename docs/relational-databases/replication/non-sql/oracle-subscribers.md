---
title: "Oracle 구독자 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
helpviewer_keywords: 
  ...
  - "Oracle 구독자 [SQL Server 복제]"
  - "SQL Server 이외 구독자, Oracle"
  - "다른 유형의 구독자, Oracle"
...
## Oracle 구독자에 대한 고려 사항  
 이 항목에서 다루는 고려 사항 외에도 [비-SQL Server 구독자](../../../relational-databases/replication/non-sql/non-sql-server-subscribers.md), Oracle 구독자로 복제할 때 다음 사항을 고려 하십시오.  
...
-   Oracle에서는 빈 문자열과 NULL 값을 모두 NULL로 처리합니다. [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 열을 NOT NULL로 정의한 다음 해당 열을 Oracle 구독자로 복제하는 경우 이것은 중요한 문제입니다. 변경 내용을 Oracle 구독자에 적용할 때 오류가 발생하지 않게 하려면 다음 중 하나를 수행해야 합니다.  
    ...
    -   오류에 대한 알림을 배포 에이전트 기록 로그에 받은 다음 계속 처리할 수 있으면 배포 에이전트에 **–SkipErrors** 매개 변수를 사용합니다. Oracle 오류 코드 1400 지정 (**-SkipErrors1400**).  
  
    -   생성 된 create 테이블 스크립트를 수정, 빈 문자열과 연결 된 가능성이 있는 모든 문자 열에서 NOT NULL 특성을 제거 및 수정된 된 스크립트의 @creation_script 매개 변수를 사용 하 여 문서에 대 한 사용자 지정 만들기 스크립트 매개 변수로 제공 [sp_addarticle](../../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md)합니다.  
  
-   Oracle 구독자에서는 0x4071 스키마 옵션을 지원합니다. 스키마 옵션에 대 한 자세한 내용은 참조 [sp_addarticle (& a) #40; TRANSACT-SQL & #41;](../../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md)합니다.  
  
## SQL Server에서 Oracle로 데이터 형식 매핑  
...
|**10 진수 (1-38, 0-38)**|NUMBER(1-38, 0-38)|  
|**float(53)**|FLOAT|  
|**FLOAT**|FLOAT|  
|**geography**|BLOB|  
|**geometry**|BLOB|  
