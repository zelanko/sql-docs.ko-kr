---
title: "Oracle 게시자에 대한 관리 고려 사항 | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Oracle 게시 [SQL Server 복제], 관리 고려 사항"
  - "복제 관리, Oracle 게시"
...
### Oracle Import  
 Oracle import 옵션을 설정할 수 **무시** 'y'로 또는 ' n ' (기본값은 ' n '). 경우 **무시** 로 설정 된 ' n ', 테이블을 삭제 하 고 가져오는 동안 다시 만들어집니다. 이렇게 하면 복제 트리거가 제거되고 복제는 해제됩니다. **ignore** 를 'y'로 설정하면 가져오기 작업에서 행을 기존 테이블로 로드하려고 하며 이로 인해 복제 트리거가 발생됩니다. 그러므로 가져오기 도구를 사용하여 복제된 테이블로 가져올 때는 **ignore** 를 'y'로 설정해야 합니다.  
  
### SQL*Loader  
 SQL을 사용한\*로더는 옵션을 설정할 수 있습니다 **직접** 'true' 또는 'false'로 (기본값은 'false'). **direct** 를 'false'로 설정하면 기존 INSERT 문을 사용하여 행이 삽입되고 이로 인해 복제 트리거가 발생됩니다. **direct** 를 'true'로 설정하면 로드는 최적화되지만 트리거가 발생되지 않습니다. 그러므로 **직접** sql 복제 된 테이블에 로드할 때 'false'로 설정 된 * Loader 도구입니다.  
...
## 복제 개체 삭제 또는 수정  
 게시자 수준의 추적 테이블, 트리거, 시퀀스 또는 저장 프로시저를 삭제하거나 수정할 경우 게시자를 삭제하고 다시 구성해야 합니다. 이러한 개체의 목록 일부를 참조 하십시오. [Oracle 게시자에 개체를 만들](../../../relational-databases/replication/non-sql/objects-created-on-the-oracle-publisher.md)합니다.  
