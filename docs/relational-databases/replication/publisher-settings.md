...
## 옵션  
 **게시자 연결**  
 시작 하려면 클릭은 **서버에 연결** 보고 연결 속성을 변경할 수 있으며 복제 모니터를 자격 증명 대화 상자를 게시자에 연결 하기 위해 사용 합니다.  
 ...
 **복제 모니터가 시작될 때 자동으로 연결**  
 복제 모니터가 자동으로 배포자에 연결하여 대화 상자 상단의 표에 선택된 게시자의 상태 정보를 검색하도록 하려면 선택합니다. 복제 모니터를 시작한 후 수동으로 연결 해야이 확인란의 선택을 취소 합니다: 복제 모니터의 왼쪽된 창에서 게시자를 마우스 오른쪽 단추로 클릭 하 고 클릭 **연결**합니다.  
  
 **이 게시자 및 해당 게시의 상태를 자동으로 새로 고침**  
 복제 모니터가 대화 상자 상단의 표에 선택된 게시자의 상태를 자동으로 새로 고치도록 하려면 선택합니다. 이 옵션을 선택하면 복제 모니터가 배포자를 폴링하여 게시자 및 해당 게시에 대한 상태 정보를 얻습니다. 폴링 간격은 **새로 고침 빈도** 옵션으로 설정됩니다. 복제 모니터의 새로 고침에 대 한 자세한 내용은 참조 하십시오. [캐싱, 새로 고침 및 복제 모니터 성능](../../relational-databases/replication/monitor/caching-refresh-and-replication-monitor-performance.md)합니다.  
