...
description: SQL Server 배포자는 트랜잭션 복제를 위한 복제 및 트랜잭션에 대한 메타데이터 및 기록을 저장하는 배포 데이터베이스를 호스트합니다.
ms.custom: ''
ms.date: 03/07/2017
ms.prod: sql
...
-   게시에 대한 스냅샷 파일이 배포자에 저장되어 있는 경우(일반적으로 배포자에 저장되어 있음) 추가 디스크 공간  
...
-   이 배포자를 사용하는 모든 게시자에 대해 기본적으로 사용되는 스냅샷 폴더. 이 폴더가 이미 공유되어 있고 적절한 권한 집합이 설정되어 있는지 확인합니다. 자세한 내용은 [스냅샷 폴더 보안 설정](../../relational-databases/replication/security/secure-the-snapshot-folder.md)을 참조하세요.  
