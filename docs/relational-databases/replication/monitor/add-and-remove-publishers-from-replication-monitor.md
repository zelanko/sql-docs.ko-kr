---
title: "복제 모니터에서 게시자 추가 및 제거 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
---
# 복제 모니터에서 게시자 추가 및 제거
  복제 모니터를 실행하는 서버가 게시자인 경우 자동으로 모니터에 추가됩니다. **게시자 추가** 대화 상자를 통해 게시자를 추가할 수 있습니다. 게시자를 추가하면 게시자는 모니터의 왼쪽 창에 있는 그룹에 표시됩니다. **내 게시자** 그룹이 기본적으로 포함되지만 새 그룹을 만들어 하나 이상의 복제 토폴로지를 관리할 수 있습니다. 복제 모니터를 시작 하는 방법에 대 한 정보를 참조 하십시오. [복제 모니터 시작](../../../relational-databases/replication/monitor/start-the-replication-monitor.md)합니다.  
  
### SQL Server 게시자를 추가하려면  
  
1.  마우스 오른쪽 단추로 클릭는 **복제 모니터** 노드 또는 왼쪽된 창에서 노드를 그룹화 하 고 클릭 한 다음 게시자 **게시자 추가**합니다.  
...
5.  게시자 및 배포자의 이름에 표시 되는 **다음 게시자의 모니터링을 시작** 표입니다.  
...
7.  복제 모니터에서 게시자가 표시되는 그룹을 선택합니다. 새 그룹을 만들려면 클릭 **새 그룹**, 다음 그룹 이름을 입력 하 고 그룹 선택의 **이 게시자를 다음 그룹에 표시** 목록입니다.  
...
### 같은 배포자를 사용하는 하나 이상의 게시자를 추가하려면  
...
5.  배포자와 각 게시자의 이름에 표시 되는 **다음 게시자의 모니터링을 시작** 눈금. 게시자가 복제 모니터에 이미 추가된 경우에는 표에 게시자가 표시되지 않습니다.  
...
### 게시자 및 게시자 그룹에 대한 설정을 수정하려면  
  
1.  왼쪽된 창에서 게시자를 마우스 오른쪽 단추로 누른 **게시자 설정**합니다.  
  
2.  **게시자 설정** 대화 상자에서 변경을 수행합니다.  
  
    -   복제 모니터가 서버에 연결할 때 사용하는 자격 증명을 변경하려면 **게시자 연결** 또는 **배포자 연결**을 클릭한 다음 **서버에 연결** 입력란에 자격 증명을 입력합니다.  
  
    -   게시자 그룹에서 다른를 이동 하려면 선택에서 게시자는 **다음 게시자의 모니터링을 시작** 눈금을 다음에 새 그룹을 선택 하 고는 **이 게시자를 다음 그룹에 표시** 목록.  
