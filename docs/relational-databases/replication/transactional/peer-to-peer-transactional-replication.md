---
title: "피어 투 피어 트랜잭션 복제 | Microsoft Docs"
ms.custom: ""
ms.date: "08/29/2016"
...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx_md](../../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  피어 투 피어 복제 라고도 하는 여러 서버 인스턴스에 걸쳐 데이터 복사본을 유지 하 여 확장 및 고가용성 솔루션을 제공 *노드*합니다. 트랜잭션 복제를 기반으로 구축된 피어 투 피어 복제는 트랜잭션 측면에서 일관적인 변경 내용을 거의 실시간으로 전파합니다. 따라서 읽기 작업을 확장해야 하는 응용 프로그램은 클라이언트의 읽기 작업을 여러 노드에 배포할 수 있습니다. 여러 노드의 데이터가 거의 실시간으로 유지 관리되므로 피어 투 피어 복제는 데이터 중복을 제공하며 이러한 중복은 데이터의 가용성을 높여 줍니다.  
...
-   오른쪽의 모든 업데이트 노드에으로 연결 됩니다 **B**합니다. 여기에서 업데이트 노드에 복제 **A**합니다. 경우 **B** 오프 라인 (예: 유지 관리) 응용 프로그램 서버에 모든 작업을 지시할 수 **A**합니다. 때 **B** 다시 온라인, 업데이트, 흐를 수 및 응용 프로그램 서버에서 모든 업데이트를 이동할 수 다시 **B** 옮기거나 계속 되도록 **A**합니다.  
...
-   피어 투 피어 복제는의 엔터프라이즈 버전 에서만에서 사용할 수 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]합니다.  
  
-   피어 투 피어 복제에 참여하는 모든 데이터베이스에는 동일한 스키마 및 데이터가 있어야 합니다.  
  
    -   개체 이름, 개체 스키마 및 게시 이름이 동일해야 합니다.  
  
    -   게시에서 스키마 변경 내용의 복제를 허용해야 합니다(게시 속성 (이 설정의 **1** 게시 속성에 대 한 **replicate_ddl**, 기본 설정입니다.) 자세한 내용은 참조 [게시 데이터베이스에 스키마 변경을 확인](../../../relational-databases/replication/publish/make-schema-changes-on-publication-databases.md)합니다.  
...
-   백업이나 **'replication support only'** 옵션을 사용하여 구독을 초기화해야 합니다. 자세한 내용은 참조 [초기화는 Transactional Subscription Without a Snapshot](../../../relational-databases/replication/initialize-a-transactional-subscription-without-a-snapshot.md)합니다.  
  
-   ID 열은 사용하지 않는 것이 좋습니다. ID를 사용하는 경우 참여하는 각 데이터베이스에서 테이블에 할당된 범위를 수동으로 관리해야 합니다. 자세한 내용은 "할당 범위에 대 한 Id 범위 수동 관리" 섹션을 참조 [Id 열 복제](../../../relational-databases/replication/publish/replicate-identity-columns.md)합니다.  
...
-   배포 에이전트 매개 변수 **-SubscriptionStreams** 및 로그 판독기 에이전트 매개 변수 **-MaxCmdsInTran**합니다.  
  
-   아티클 속성 **@destination_owner** 및 **@destination_table**합니다.

-   피어 투 피어 트랜잭션 복제에서는 피어 투 피어 게시에 대 한 단방향 트랜잭션 구독을 만들 수 없습니다.   
  
 다음 속성에는 특별히 고려할 사항이 있습니다.  
  
-   게시 속성 **@allow_initialize_from_backup** 의 값이 **true**합니다.  
  
-   아티클 속성 **@replicate_ddl** 의 값이 **true**; **@identityrangemanagementoption** 의 값이 **수동**; 및 **@status** 해당 옵션을 사용 해야 **24** 설정 됩니다.  
  
-   아티클 속성에 대 한 값 **@ins_cmd**, **@del_cmd**, 및 **@upd_cmd** 로 설정할 수 없습니다 **SQL**합니다.  
  
-   구독 속성 **@sync_type** 의 값이 **none** 또는 **자동**합니다.  
...
|토폴로지에 노드 추가|전체 토폴로지의 노드 2개(정지 필요 없음). 대신 `sync_type = 'initialize with backup'`를|노드 3개 이상(정지 필요)|`sync_type = 'replication support only'`: 정지 필요|`sync_type = 'initialize with backup'` 및 `'initialize from lsn'`: 정지 필요 없음|  
 ...
 토폴로지 스키마 변경(아티클 추가 또는 삭제) 시에는 시스템을 정지해야 합니다. 자세한 내용은 참조 [#40; 및 피어-투-피어 토폴로지 관리 복제 TRANSACT-SQL 프로그래밍 &#41;](../../../relational-databases/replication/administration/administer-a-peer-to-peer-topology-replication-transact-sql-programming.md)합니다.  
  
 토폴로지에서 노드를 제거할 때는 시스템을 정지할 필요가 없습니다.  
  
 아티클 속성을 사용 하 여 변경  [sp_changearticle](../../../relational-databases/system-stored-procedures/sp-changearticle-transact-sql.md) 정지 필요 없습니다. (P2P)에 대 한 허용 가능한 변경이 `description`, `ins_cmd`, `upd_cmd`, 및 `del_cmd` 속성입니다.  
...
-   아티클 삭제: 모든 노드의 상태를 일관되게 유지하려면 토폴로지를 정지해야 합니다.  
 ...
 자세한 내용은 참조 [정지 복제 토폴로지 및 #40; 복제 TRANSACT-SQL 프로그래밍 &#41;](../../../relational-databases/replication/administration/quiesce-a-replication-topology-replication-transact-sql-programming.md) 및 [#40; 및 피어-투-피어 토폴로지 관리 복제 TRANSACT-SQL 프로그래밍 &#41;](../../../relational-databases/replication/administration/administer-a-peer-to-peer-topology-replication-transact-sql-programming.md)합니다.  
