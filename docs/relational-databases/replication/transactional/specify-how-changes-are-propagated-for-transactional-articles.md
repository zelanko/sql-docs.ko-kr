---
title: "트랜잭션 아티클에 대한 변경 내용을 전파하는 방법 지정 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
-   **sp_MSins_ \<** *tablename* **>**, 를 삽입 합니다.  
  
-   **sp_MSupd_ \<** *tablename* **>**, 를 업데이트 합니다.  
  
-   **sp_MSdel_ \<** *tablename* **>**, 삭제 처리 합니다.  
  
  **\<***tablename***>** 에 사용 되는 프로시저는 문서 게시에 추가 된 방법 및 구독 데이터베이스의 소유자는 다르지만 같은 이름의 테이블을 포함 하는 여부에 따라 달라 집니다.  
...
-   일반적으로 구독자에 있는 스키마는 게시자에 있는 스키마는 동일하지만 열 필터링을 사용할 경우 게시자 스키마의 하위 집합일 수도 있습니다. 데이터를 이동할 때 구독자에 있는 스키마가 게시자에 있는 스키마의 하위 집합이 되지 않도록 스키마를 변환하려면 [!INCLUDE[ssISCurrent](../../../includes/ssiscurrent-md.md)] 를 사용하는 것이 좋습니다. 자세한 내용은 참조 [SQL Server Integration Services](../../../integration-services/sql-server-integration-services.md)합니다.  
  
-   게시된 테이블에 대해 스키마 변경을 적용하면 사용자 지정 프로시저를 다시 생성해야 합니다. 자세한 내용은 참조 [다시 생성 사용자 지정 트랜잭션 프로시저 스키마 변경 내용을 반영 하려면](../../../relational-databases/replication/transactional/regenerate-custom-transactional-procedures-to-reflect-schema-changes.md)합니다.  
  
-   에 대해 1 보다 큰 값을 사용 하는 경우 **-SubscriptionStreams** 배포 에이전트의 매개 변수를 확인 해야 있는 기본 키 열에 대 한 업데이트가 성공 합니다. 예를 들어  
...
-   VCALL 구문. 업데이트할 수 있는 구독에 사용합니다. 내부적으로만 사용됩니다.  
 ...
 각 방법은 구독자로 전파되는 데이터 양에 차이가 있습니다. 예를 들어 SCALL은 실제로 업데이트가 적용된 열 값만 전달합니다. 이와 대조적으로 XCALL은 업데이트 적용 여부에 상관없이 모든 열 및 각 열에 대한 모든 이전 데이터 값이 필요합니다. 대부분의 경우 업데이트에는 SCALL이 적합하지만 응용 프로그램에서 업데이트하는 동안 모든 데이터를 필요로 하는 경우에는 XCALL이 적합합니다.  
...
#### SCALL 구문  
 UPDATE 저장 프로시저  
 UPDATE 문을 처리 하는 저장된 프로시저에 업데이트 된 값 변경, 기본 키 열에 대 한 원래 값이 옵니다, 뒤에 비트 마스크 하 여 해당 열에 대해서만 전달 됩니다 (**binary (n)**) 변경 된 열을 나타내는 매개 변수입니다. 다음 예에서 열 2(c2)는 변경되지 않은 것입니다.  
...
#### MCALL 구문  
 UPDATE 저장 프로시저  
 UPDATE 문을 처리 하는 저장된 프로시저는 비트 마스크 다음의 기본 키 열에 대 한 원래 값 다음에 아티클에 정의 된 모든 열에 대 한 업데이트 된 값 전달 됩니다 (**binary (n)**) 변경 된 열을 나타내는 매개 변수입니다.  
...
> [!NOTE]  
>  XCALL을 사용 하는 경우는 이전 이미지 값에 대 한 **텍스트** 및 **이미지** 열 NULL 것으로 예상 됩니다.  
