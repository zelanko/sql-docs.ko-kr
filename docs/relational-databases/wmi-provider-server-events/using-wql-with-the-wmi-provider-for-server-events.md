...
```  
 ...
 `SchemaName='X' AND ObjectType='Y' AND ObjectName='Z'` 모두 `WHERE` 절에 지정 된 경우 스키마 `X`의 개체 `Z`에 직접 이벤트 알림을 등록 하려고 시도 합니다. 클라이언트에 사용 권한이 있으면 등록이 성공합니다. 현재 개체 수준 이벤트는 큐 에서만 지원 되 고 QUEUE_ACTIVATION *event_type*에 대해서만 지원 됩니다.  
