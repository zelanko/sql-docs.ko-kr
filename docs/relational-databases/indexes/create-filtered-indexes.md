...
-   필터링된 인덱스는 하나의 테이블에서 정의되고 간단한 [비교 연산자](../../t-sql/language-elements/comparison-operators-transact-sql.md)만 지원합니다. 여러 테이블을 참조하거나 복잡한 논리를 사용하는 필터 식이 필요할 경우 뷰를 만들어야 합니다. 필터링된 인덱스는 `LIKE` 연산자를 지원하지 않습니다. 
