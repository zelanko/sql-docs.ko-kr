---
title: "인덱스 다시 구성 및 다시 작성 | Microsoft Docs"
ms.custom: ""
ms.date: "04/29/2016"
...
helpviewer_keywords: 
  - "큰 개체 조각 모음"
  - "인덱스 [SQL Server], 다시 구성"
  - "인덱스 다시 구성 [SQL Server]"
  - "인덱스 다시 구성"
  - "큰 개체 데이터 형식 조각 모음"
  - "인덱스 조각화 [SQL Server]"
  - "인덱스 다시 작성 [SQL Server]"
  - "인덱스 다시 작성"
  - "인덱스 [SQL Server], 다시 작성"
  - "인덱스 조각 모음"
  - "비클러스터형 인덱스 [SQL Server], 조각 모음"
  - "조각 모음 [SQL Server]"
  - "인덱스 조각 모음 [SQL Server]"
  - "LOB 데이터 [SQL Server], 조각 모음"
  - "클러스터형 인덱스, 조각 모음"
...
-   `ALTER INDEX REORGANIZE` 문을 사용하면 작업에서 파일 그룹 내의 다른 파일이 아닌 동일한 파일에만 임시 작업 페이지를 할당할 수 있으므로 인덱스가 포함된 데이터 파일에 사용 가능한 공간이 있어야 합니다.  그러므로 파일 그룹에 사용 가능한 빈 페이지가 있어도 사용자에게 오류 1105 "'PRIMARY' 파일 그룹이 꽉 찼으므로 데이터베이스 \<데이터베이스 이름>의 개체 \<인덱스 이름>.\<테이블 이름>에 공간을 할당할 수 없습니다."가 발생할 수 있습니다.
