...
ms.locfileid: "78937680"
---
# <a name="reorganize-and-rebuild-indexes"></a>인덱스 다시 구성 및 다시 빌드

[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

이 문서에서는 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 또는 [!INCLUDE[tsql](../../includes/tsql-md.md)]을 사용하여 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 조각난 인덱스를 다시 구성하거나 다시 작성하는 방법을 설명합니다.
...
## <a name="how-to-remedy-index-fragmentation"></a>인덱스 조각화를 해결하는 방법

인덱스를 다시 구성하거나 다시 작성하여 인덱스 조각화 문제를 해결할 수 있습니다. 파티션 구성표에 빌드한 분할된 인덱스의 경우 전체 인덱스나 인덱스의 단일 파티션에서 다음 방법 중 하나를 사용할 수 있습니다.
...
## <a name="detecting-fragmentation"></a><a name="Fragmentation"></a> 조각화 검색

사용할 인덱스 조각 모음 방법을 결정하기 위한 첫 번째 단계는 인덱스를 분석하여 조각화 수준을 확인하는 것입니다.
...
대체로 작은 양의 조각화를 제거할 경우 얻게 되는 이점보다 인덱스를 재구성하거나 다시 빌드하는 데 필요한 CPU 비용이 훨씬 크기 때문에, 일반적으로 두 명령 중 하나를 사용하여 매우 낮은 수준의 조각화(5% 미만)를 처리하는 것은 바람직하지 않습니다.
...
인덱스 조각화 수준을 파악한 후에는 다음 표를 사용하여 가장 적합한 조각화 수정 방법을 확인합니다.
...
## <a name="check-index-fragmentation"></a>인덱스 조각화 확인

### <a name="check-index-fragmentation-using-ssmanstudiofull"></a><a name="SSMSProcedureFrag"></a>[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]를 사용하여 인덱스 조각화 확인

> [!NOTE]
> [!INCLUDE[ssManStudio](../../includes/ssManStudio-md.md)]는 SQL Server에서 columnstore 인덱스의 조각화를 컴퓨팅하는 데 사용할 수 없으며 Azure SQL Database에서 인덱스의 조각화를 컴퓨팅하는 데 사용할 수 없습니다. 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] [예제](#TsqlProcedureFrag)를 사용합니다.
...
### <a name="check-index-fragmentation-using-tsql"></a><a name="TsqlProcedureFrag"></a>[!INCLUDE[tsql](../../includes/tsql-md.md)]를 사용하여 인덱스 조각화 확인
...
## <a name="remove-fragmentation"></a>조각화 제거
