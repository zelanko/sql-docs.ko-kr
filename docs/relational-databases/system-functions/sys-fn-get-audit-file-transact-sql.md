 ...
 *file_pattern*  
 ...
 - **SQL Server**:
    ...
    이 인수에는 경로와 파일 이름이 모두 포함되어야 합니다. 드라이브 문자나 네트워크 공유를 경로로 사용할 수 있으며, 파일 이름에 와일드카드를 사용할 수 있습니다. 단일 별표 (*)를 사용 하 여 감사 파일 집합에서 여러 파일을 수집할 수 있습니다. 다음은 그 예입니다.  
 ...
 - **Azure SQL Database 또는 Azure SQL Data Warehouse**:
    ...
    이 인수는 blob URL (저장소 끝점 및 컨테이너 포함)을 지정 하는 데 사용 됩니다. 별표 와일드 카드를 지원 하지 않는 경우 전체 blob 이름 대신 부분 파일 (blob) 이름 접두사를 사용 하 여이 접두사로 시작 하는 여러 파일 (blob)을 수집할 수 있습니다. 다음은 그 예입니다.
      ...
      - **/\> Storage_endpoint\>\<컨테이너\>ServerName\>-특정 데이터베이스에 대 한 모든 감사 파일 (blob)을 수집/\</ \<\<**    
...
| client_ip | **nvarchar(128)** | **적용 대상**: AZURE SQL DB + SQL Server (2017부터 시작)<br /><br />    클라이언트 응용 프로그램의 원본 IP |  
| connection_id | GUID | **적용**대상: AZURE SQL DB 및 관리 되는 인스턴스<br /><br /> 서버에 있는 연결의 ID입니다. |
| data_sensitivity_information | nvarchar(4000) | **적용 대상**: AZURE SQL DB에만 해당<br /><br /> 데이터베이스의 분류 된 열을 기반으로 감사 된 쿼리에서 반환 되는 정보 유형 및 민감도 레이블입니다. [Azure SQL Database 데이터 검색 및 분류](https://docs.microsoft.com/azure/sql-database/sql-database-data-discovery-and-classification) 에 대 한 자세한 정보 |
...
| duration_milliseconds | **bigint** | **적용**대상: AZURE SQL DB 및 관리 되는 인스턴스<br /><br /> 쿼리 실행 기간 (밀리초) |
...
- **Azure SQL Database**
  ...
  이 예제에서는 이름이 `ShiraServer/MayaDB/SqlDbAuditing_Audit/2017-07-14/10_45_22_173_1.xel`인 파일에서 읽습니다.  
  ...
  이 예에서는로 `Sh`시작 하는 서버에서 모든 감사 로그를 읽습니다. 
