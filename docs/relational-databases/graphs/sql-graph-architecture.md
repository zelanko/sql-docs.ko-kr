...
|기본 제공   |Description  |
|---  |---  |
|OBJECT_ID_FROM_NODE_ID |Object_id를 node_id에서 추출  |
|GRAPH_ID_FROM_NODE_ID  |graph_id는 node_id에서 추출  |
|NODE_ID_FROM_PARTS |Object_id 및는 graph_id node_id를 생성 합니다.  |
|OBJECT_ID_FROM_EDGE_ID |Object_id edge_id에서 추출  |
|GRAPH_ID_FROM_EDGE_ID  |Edge_id에서 identity를 추출 합니다.  |
...
|태스크   |관련된 항목  |참고
|---  |---  |---  |
|CREATE TABLE |[CREATE TABLE&#40;Transact-SQL&#41;](../../t-sql/statements/create-table-sql-graph.md)|`CREATE TABLE ` 가 이제 AS 노드 또는 지 AS 테이블 만들기를 지원 하도록 확장 됩니다. 특성 정의 하는 edge 테이블 수 되거나 모든 사용자에 없을 수 있습니다.  |
...
|MERGE |[MERGE&#40;Transact-SQL&#41;](../../t-sql/statements/merge-transact-sql.md)  |`MERGE` 노드 또는 지 테이블에는 문이 지원 되지 않습니다.  |
...
* 사용자는 $from_id 및 UPDATE 문을 사용 하 여에 지의 $to_id 열을 업데이트할 수 없습니다. 연결 하는 노드를 업데이트 하려면 사용자가 새 노드를 가리키는 새 edge를 삽입 하 고 이전에 삭제 해야 합니다.
