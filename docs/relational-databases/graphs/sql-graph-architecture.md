...
노드 테이블은 그래프 스키마의 엔터티를 나타냅니다. 사용자 정의 열과 함께 노드 테이블을 만들 때마다 데이터베이스에서 지정 된 노드를 고유 하 `$node_id` 게 식별 하는 암시적 열이 생성 됩니다. 의 `$node_id` 값은 자동으로 생성 되며 해당 노드 테이블과 내부적 `object_id` 으로 생성 된 bigint 값의 조합입니다. 그러나 `$node_id` 열을 선택 하면 JSON 문자열 형식의 계산 된 값이 표시 됩니다. `$node_id` 또한은 (는) 16 진수 문자열을 포함 하는 내부 이름에 매핑되는 의사 (pseudo) 열입니다. 테이블에서를 `$node_id` 선택 하면 열 이름이로 `$node_id_<hex_string>`표시 됩니다. 쿼리에서 의사 (pseudo) 열 이름을 사용 하는 것이 좋습니다. 내부 `$node_id` 열을 쿼리 하 고 16 진수 문자열을 사용 하 여 내부 이름을 사용 하지 않는 것이 좋습니다.

사용자가 노드 테이블을 만들 때 `$node_id` 열에 unique 제약 조건 또는 인덱스를 만드는 것이 좋지만, 하나를 만들지 않은 경우 기본 고유 비클러스터형 인덱스가 자동으로 만들어집니다. 그러나 graph 의사 열의 인덱스는 기본 내부 열에 생성 됩니다. 즉, `$node_id` 열에 생성 된 인덱스가 내부 `graph_id_<hex_string>` 열에 표시 됩니다.   
...
|열 이름    |설명  |
|---   |---  |
|`$edge_id`   |데이터베이스의 지정 된 가장자리를 고유 하 게 식별 합니다. 이는 생성 된 열이 고 값은에 지 테이블의 object_id와 내부적으로 생성 된 bigint 값의 조합입니다. 그러나 `$edge_id` 열을 선택 하면 JSON 문자열 형식의 계산 된 값이 표시 됩니다. `$edge_id`은 (는) 16 진수 문자열을 포함 하는 내부 이름에 매핑되는 의사 (pseudo) 열입니다. 테이블에서를 `$edge_id` 선택 하면 열 이름이로 `$edge_id_\<hex_string>`표시 됩니다. 쿼리에서 의사 (pseudo) 열 이름을 사용 하는 것이 좋습니다. 내부 `$edge_id` 열을 쿼리 하 고 16 진수 문자열을 사용 하 여 내부 이름을 사용 하지 않는 것이 좋습니다. |
|`$from_id`   |에 지 `$node_id` 가 발생 한 노드의를 저장 합니다.  |
|`$to_id`   |에 지 `$node_id` 가 종료 되는 노드의를 저장 합니다. |

지정 된 가장자리에 연결할 수 있는 노드는 `$from_id` 및 `$to_id` 열에 삽입 된 데이터에 의해 제어 됩니다. 첫 번째 릴리스에서는에 지 테이블에 대 한 제약 조건을 정의 하 여 두 노드 유형의 연결을 제한할 수 없습니다. 즉, 가장자리는 해당 형식에 관계 없이 그래프의 두 노드를 연결할 수 있습니다.

열과 마찬가지로 사용자는에 지 테이블을 만들 때 `$edge_id` 열에 대 한 고유한 인덱스나 제약 조건을 만드는 것이 좋지만, 하나는 생성 되지 않은 경우 기본 고유 비클러스터형 인덱스가 자동으로 만들어집니다. `$node_id` 이 열입니다. 그러나 graph 의사 열의 인덱스는 기본 내부 열에 생성 됩니다. 즉, `$edge_id` 열에 생성 된 인덱스가 내부 `graph_id_<hex_string>` 열에 표시 됩니다. 또한 OLTP 시나리오의 경우 사용자가 (`$from_id`, `$to_id`) 열에 대 한 인덱스를 만들어 가장자리 방향으로 더 빠르게 조회할 수 있도록 하는 것이 좋습니다.  
...
다음 새 비트 형식 열이 SYS에 추가 됩니다. 표의. 이 `is_node` 1로 설정 된 경우이는 테이블이 노드 테이블이 고가 1로 설정 된 `is_edge` 경우 테이블이에 지 테이블 임을 나타냅니다.
...
뷰에 `sys.columns` 는 노드 및에 `graph_type` 지 `graph_type_desc`테이블의 열 유형을 나타내는 추가 열 및가 포함 되어 있습니다.
 
|열 이름 |데이터 형식 |설명 |
|--- |---|--- |
|graph_type |ssNoversion |값 집합이 포함 된 내부 열입니다. 그래프 열에 대해 1-8 사이에 값이 있고 다른 값에 대해서는 NULL입니다.  |
|graph_type_desc |nvarchar(60)  |값 집합이 포함 된 내부 열 |
 
다음 표에서는 열에 대 한 `graph_type` 유효한 값을 나열 합니다.
...
|$from_id_\<hex_string> |NVARCHAR   |0  |노드의 외부`node_id`  |
...
다음과 같은 기본 제공 함수가 추가 됩니다. 이를 통해 사용자는 생성 된 열에서 정보를 추출할 수 있습니다. 이러한 메서드는 사용자의 입력에 대 한 유효성을 검사 하지 않습니다. 사용자가 잘못 `sys.node_id` 된를 지정 하는 경우 메서드는 적절 한 부분을 추출 하 고 반환 합니다. 예를 들어 OBJECT_ID_FROM_NODE_ID는를 `$node_id` 입력으로 사용 하 고 테이블의 OBJECT_ID을 반환 합니다 .이 노드는에 속합니다. 
 
|기본 제공   |설명  |
|---  |---  |
|OBJECT_ID_FROM_NODE_ID |에서 object_id의 압축을 풉니다.`node_id`  |
|GRAPH_ID_FROM_NODE_ID  |에서 graph_id의 압축을 풉니다.`node_id`  |
|NODE_ID_FROM_PARTS |`object_id` 및에서 node_id 생성`graph_id`  |
|OBJECT_ID_FROM_EDGE_ID |추출 `object_id` 원본`edge_id`  |
|GRAPH_ID_FROM_EDGE_ID  |Id 추출`edge_id`  |
|EDGE_ID_FROM_PARTS |및 `edge_id` id `object_id` 에서 생성  |



## <a name="transact-sql-reference"></a>Transact-SQL 참조 
그래프 개체 [!INCLUDE[tsql-md](../../includes/tsql-md.md)] 를 만들고 쿼리할 수 있는 SQL Server 및 Azure SQL Database에 도입 된 확장에 대해 알아봅니다. 쿼리 언어 확장은 ASCII art 구문을 사용 하 여 그래프를 쿼리하고 트래버스하는 데 도움이 됩니다.
 
### <a name="data-definition-language-ddl-statements"></a>DDL (데이터 정의 언어) 문
...
|에 지 제약 조건 만들기    |[에 지 &#40;제약 조건 transact-sql&#41;](../../relational-databases/tables/graph-edge-constraints.md)  |사용자는 이제에 지 테이블에 대 한에 지 제약 조건을 만들어 특정 의미 체계를 적용 하 고 데이터 무결성을 유지할 수 있습니다.  |
|DROP TABLE |[DROP TABLE&#40;Transact-SQL&#41;](../../t-sql/statements/drop-table-transact-sql.md)  |를 사용 하 여 관계형 테이블과 동일한 방식으로 노드 및에 `DROP TABLE`지 테이블을 삭제할 수 있습니다. 그러나이 릴리스에서는 노드 또는 노드 테이블을 삭제할 때 삭제 된 노드를 가리키는 가장자리가 없으며, 노드 또는 노드 테이블을 삭제 하는 경우에는 연속 되는 가장자리를 삭제할 필요가 없습니다. 노드 테이블을 삭제 하는 경우 사용자는 해당 노드 테이블의 노드에 연결 된 모든 가장자리를 수동으로 삭제 하 여 그래프의 무결성을 유지 하는 것이 좋습니다.  |


### <a name="data-manipulation-language-dml-statements"></a>DML (데이터 조작 언어) 문

|태스크   |관련 문서  |참고
|---  |---  |---  |
|INSERT |[INSERT&#40;Transact-SQL&#41;](../../t-sql/statements/insert-sql-graph.md)|노드 테이블에 삽입 하는 것은 관계형 테이블에 삽입 하는 것과 다릅니다. `$node_id` 열 값이 자동으로 생성 됩니다. `$node_id` 또는`$edge_id` 열에 값을 삽입 하려고 하면 오류가 발생 합니다. 사용자는에 지 테이블 `$from_id` 에 `$to_id` 삽입 하는 동안 및 열에 대 한 값을 제공 해야 합니다. `$from_id``$to_id` 및`$node_id` 는 지정 된에 지가 연결 하는 노드의 값입니다.  |
...
|UPDATE |[UPDATE&#40;Transact-SQL&#41;](../../t-sql/queries/update-transact-sql.md)  |UPDATE 문을 사용 하 여 사용자 정의 열의 값을 업데이트할 수 있습니다. 내부 `$node_id`그래프 열 `$edge_id` ,,`$to_id`및는 업데이트할 수 없습니다. `$from_id`  |
...
* 사용자는 update 문을 `$from_id` 사용 `$to_id` 하 여에 지의 및 열을 업데이트할 수 없습니다. Edge가 연결 하는 노드를 업데이트 하기 위해 사용자는 새 노드를 가리키는 새에 지를 삽입 하 고 이전 노드를 삭제 해야 합니다.
