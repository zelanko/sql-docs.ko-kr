---
title: "계층적 데이터(SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "계층 [SQL Server], 지원할 테이블"
  - "hierarchyid [데이터베이스 엔진], 개념"
  - "계층적 테이블 [데이터베이스 엔진]"
  - "SqlHierarchyId"
  - "hierarchyid [데이터베이스 엔진]"
  - "계층적 쿼리 [SQL Server], hierarchyid 데이터 형식 사용"
...
-   웹 페이지 간 링크의 그래프  
 ...
 [hierarchyid](../Topic/hierarchyid%20\(Transact-SQL\).md) 를 데이터 형식으로 사용하여 계층 구조가 있는 테이블을 만들거나 다른 위치에 저장된 데이터의 계층 구조를 설명할 수 있습니다. [의](../t-sql/data-types/hierarchyid-data-type-method-reference.md) hierarchyid 함수 [!INCLUDE[tsql](../includes/tsql-md.md)] 를 사용하면 계층적 데이터를 쿼리하고 관리할 수 있습니다.  
...
###  <a name="findclr"></a> CLR을 사용하여 상위 항목 찾기  
 한 계층의 두 노드를 사용하는 일반적인 작업에는 수준이 가장 낮은 공통 상위 항목을 찾는 작업이 있습니다. **hierarchyid** 형식을 [!INCLUDE[tsql](../includes/tsql-md.md)] 또는 CLR에서 모두 사용할 수 있으므로 두 방법을 모두 사용하여 이러한 작업을 코드로 작성할 수 있습니다. 그러나 성능이 향상되는 CLR을 사용하는 것이 좋습니다.  
...
```  
 ...
 다음 [!INCLUDE[tsql](../includes/tsql-md.md)] 예제에서 **ListAncestor** 및 **CommonAncestor** 메서드를 사용하려면 다음과 유사한 코드를 실행하여 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]에서 DLL을 빌드하고 **HierarchyId_Operations** 어셈블리를 만듭니다.  
