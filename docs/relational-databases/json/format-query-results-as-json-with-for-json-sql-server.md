...
[!INCLUDE[tsql-appliesto-ss2016-asdb-xxxx-xxx_md](../../includes/tsql-appliesto-ss2016-asdb-xxxx-xxx-md.md)]
  ...
  **SELECT** 문에 **FOR JSON** 절을 추가하여 데이터를 SQL Server에서 JSON으로 내보내거나 쿼리 결과를 JSON으로 서식 지정합니다.  
  
 **FOR JSON** 절을 사용하는 경우 출력의 구조를 명시적으로 지정하거나 SELECT 문의 구조에 따라 출력이 결정되도록 할 수 있습니다.  
  
-   **FOR JSON 절과 함께 PATH 모드를 사용합니다**. **PATH** 모드를 **FOR JSON** 절과 함께 사용하는 경우 JSON 출력의 형식을 완전하게 제어할 수 있습니다. 래퍼 개체를 만들고 복잡한 속성을 중첩할 수 있습니다.  
  
-   **FOR JSON 절과 함께 AUTO 모드를 사용합니다**. **AUTO** 모드를 **FOR JSON** 절과 함께 사용하는 경우 JSON 출력은 SELECT 문의 구조를 기반으로 자동으로 서식 지정됩니다.  
  
 **FOR JSON** 절을 사용하여 JSON 출력의 서식 지정 작업을 클라이언트 응용 프로그램에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]로 위임할 수 있습니다. 자세한 내용은 [SQL Server 및 클라이언트 앱에서 FOR JSON 출력 사용&#40;SQL Server&#41;](../../relational-databases/json/use-for-json-output-in-sql-server-and-in-client-apps-sql-server.md)을 참조하세요.  
  
 ![FOR JSON](../../relational-databases/json/media/jsonslides2forjson.png "FOR JSON")  
  
## FOR JSON 절과 함께 PATH 모드 사용  
 다음은 **PATH** 모드를 **FOR JSON** 절과 함께 사용하는 예제입니다.

**PATH** 모드에서는 점 구문(예: `'Item.Price'`)을 사용하여 중첩 출력을 서식 지정할 수 있습니다. 또한 이 예제에서는 **ROOT** 옵션을 사용하여 명명된 루트 요소를 지정합니다.  
...
-   구문 및 사용법은 [FOR 절&#40;Transact-SQL&#41;](../Topic/FOR%20Clause%20\(Transact-SQL\).md)을 참조하세요.  
...
## FOR JSON 절이 포함된 옵션을 사용하여 JSON 출력 제어  
 다음 옵션을 사용하여 **FOR JSON** 절의 출력을 제어합니다.  
  
 [ROOT 옵션을 사용하여 JSON 출력에 루트 노드 추가&#40;SQL Server&#41;](../../relational-databases/json/add-a-root-node-to-json-output-with-the-root-option-sql-server.md)  
 단일 최상위 요소를 JSON 출력에 추가하려면 **ROOT** 옵션을 사용합니다. **ROOT** 옵션을 지정하지 않은 경우 JSON 출력에는 루트 요소가 없습니다.  
  
 [INCLUDE_NULL_VALUES 옵션을 사용하여 JSON 출력으로 Null 값 포함&#40;SQL Server&#41;](../../relational-databases/json/include-null-values-in-json-include-null-values-option.md)  
 JSON 출력에 null 값을 포함하려면 **INCLUDE_NULL_VALUES** 옵션을 지정합니다. 이 옵션을 지정하지 않은 경우 쿼리 결과의 NULL 값에 대해서는 출력에 JSON 속성을 포함하지 않습니다.  
  
 [WITHOUT_ARRAY_WRAPPER 옵션을 사용하여 JSON 출력에서 대괄호 제거&#40;SQL Server&#41;](../../relational-databases/json/remove square brackets from json - without_array_wrapper option.md)  
 기본적으로 **FOR JSON** 절의 JSON 출력을 둘러싸고 있는 대괄호를 제거하려면 **WITHOUT_ARRAY_WRAPPER** 옵션을 지정합니다. 이 옵션을 사용하여 단일 JSON 개체를 출력으로 생성합니다. 이 옵션을 지정하지 않으면 JSON 출력은 대괄호로 묶입니다.  
  
 **FOR JSON** 절의 출력에 나타나는 자세한 내용은 이 섹션의 다음 항목을 참조하세요.  
...
3.  JSON 구문에 따라 열과 해당 값의 이름이 모두 이스케이프됩니다.  
...
## FOR JSON 및 SQL Server의 기본 제공 JSON 지원에 대해 자세히 알아보기  
