 ...
 *cParams*[in]  
 수가 SSPARAMPROPS 구조는 *rgParamProperties* 배열입니다. 이 숫자가 0 이면 **isscommandwithparameters:: Setparameterproperties** 명령에서 매개 변수에 대해 지정 된 모든 속성이 삭제 됩니다.  
...
## <a name="remarks"></a>Remarks  
 ...
 하는 경우 **isscommandwithparameters:: Setparameterproperties** 사용 하 여 매개 변수 정보가 아직 설정 되지 않은 매개 변수 속성을 지정 하 라고 **SetParameterInfo**, 공급자 E_ 반환 다음 오류 메시지를 사용 하 여 예기치 않은:  
  
 먼저 SetParameterInfo 메서드를 호출해야만 지정한 매개 변수에 대해 SetParameterProperties 메서드를 호출할 수 있습니다. 매개 변수 속성을 설정하기 전에 매개 변수 정보를 설정해야 합니다.  
  
 경우에 대 한 호출 **isscommandwithparameters:: Setparameterproperties** 매개 변수 정보가 설정 된 경우 일부 매개 변수 및 일부 매개 변수는 매개 변수 정보가 설정 되지 않은에 있는 dwStatus 속성이 포함 된 SSPARAMPROPS 속성 집합의 DBPROPSET DBSTATUS_NOTSET로 반환 됩니다.  
