...
-   정적 커서는 커서를 열 때 **tempdb에** 내장되어 있습니다. 항상 커서가 열렸을 당시의 결과 집합을 표시합니다. 데이터 변경 내용은 반영되지 않습니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 정적 커서는 항상 읽기 전용입니다. 정적 서버 커서는 **tempdb의**작업 테이블로 빌드되므로 커서 결과 집합의 크기가 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 허용하는 최대 행 크기를 초과할 수 없습니다.  
