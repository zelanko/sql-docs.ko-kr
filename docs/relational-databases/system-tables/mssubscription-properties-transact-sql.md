...
|**publisher_security_mode**|**int**|게시자에서 구현된 보안 모드입니다.<br /><br /> **0**  =  [!INCLUDE[msCoName](../../includes/msconame-md.md)] SQL Server 인증이 있습니다.<br /><br /> **1**  =  [!INCLUDE[msCoName](../../includes/msconame-md.md)] Windows 인증입니다.<br /><br /> **2** = 동기화 트리거는 정적 **sysservers** 항목을 사용 하 여 RPC (원격 프로시저 호출)를 수행 하며, *게시자* 는 **sysservers** 테이블에서 원격 서버 또는 연결 된 서버로 정의 되어야 합니다.|  
...
|**enabled_for_syncmgr**|**bit**|[!INCLUDE[msCoName](../../includes/msconame-md.md)] 동기화 관리자를 통해 구독을 동기화할 수 있는지 여부를 지정합니다.<br /><br /> **0** = 구독이 동기화 관리자에 등록 되지 않았습니다.<br /><br /> **1** = 구독이 동기화 관리자에 등록 되며를 시작 하지 않고 동기화 할 수 있습니다 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] .|  
...
|**internet_login**|**sysname**|인증을 사용 하 여 웹 동기화를 호스팅하는 웹 서버에 연결할 때 병합 에이전트에서 사용 하는 로그인입니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] .|  
|**internet_password**|**nvarchar (524)**|인증을 사용 하 여 웹 동기화를 호스팅하는 웹 서버에 연결할 때 병합 에이전트에서 사용 하는 로그인에 대 한 암호입니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] .|  
|**internet_security_mode**|**int**|웹 동기화를 호스팅하는 웹 서버에 연결할 때 사용 되는 인증 모드입니다. 여기서 값 **1** 은 Windows 인증을 의미 하 고 **0** 은 인증을 의미 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 합니다.|  
