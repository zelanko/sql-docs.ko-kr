...
|**job_history_id**|**int**|**Sysjobhistory** 테이블에서 행을 식별 하는 데 사용 됩니다.|  
|**next_scheduled_run_date**|**datetime**|작업을 실행하도록 예약된 다음 날짜와 시간입니다.|  

## <a name="example"></a>예제
이 예에서는 모든 SQL Server 에이전트 작업에 대 한 런타임 상태를 반환 합니다.  [!INCLUDE[tsql](../../includes/tsql-md.md)] 에서 다음 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]명령을 실행합니다.
```sql
SELECT sj.Name, 
    CASE
