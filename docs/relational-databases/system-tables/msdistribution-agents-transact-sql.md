...
|**job_step_uid**|**uniqueidentifier**|에이전트가 시작되는 SQL Server 에이전트 작업 단계의 고유 ID입니다.|  
|**subscriptionstreams**|**tinyint**|변경 내용을 구독자에 병렬로 일괄 적용하기 위해 허용되는 배포 에이전트당 연결 수를 설정합니다. 1에서 64 사이의 값 범위가 지원됩니다.|  
|**memory_optimized**|**bit**|1은 메모리 액세스에 최적화 된 테이블에 구독자를 사용할 수 있음을 나타냅니다.|  
|**job_login**|**sysname**||  
