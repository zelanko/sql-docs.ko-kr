...
|**queue_id**|**sysname**|지연 업데이트 구독의 큐 위치를 나타내는 식별자입니다. 지연 구독이 아닌 경우 이 값은 NULL입니다. MSMQ([!INCLUDE[msCoName](../../includes/msconame-md.md)] Message Queuing)를 기반으로 하는 게시의 경우 이 값은 구독에 사용할 큐를 고유하게 식별하는 GUID입니다. SQL Server 기반 큐 게시에 대 한 열 값이 들어 **SQL**합니다.<br /><br /> 참고:를 사용 하 여 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 메시지 큐 사용 되지 않으며 더 이상 지원 합니다.|  
|**queue_status**|**int**|[!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]|  
|**offload_enabled**|**bit**|에이전트를 원격으로 활성화할 수 있는지 여부를 나타냅니다.<br /><br /> **0** 지정는 에이전트를 원격으로 활성화할 수 없습니다.<br /><br /> **1** 에 지정 된 원격 컴퓨터를 원격으로 에이전트를 활성화할 수 있도록 지정 된 *offload_server* 속성입니다.|  
|**offload_server**|**sysname**|원격 에이전트 활성화에 사용할 서버의 네트워크 이름입니다.|  
|**dts_package_name**|**sysname**|DTS 패키지의 이름입니다. 패키지에 대 한 예를 들어 **DTSPub_Package**, 지정 `@dts_package_name = N'DTSPub_Package'`합니다.|  
|**dts_package_password**|**nvarchar (524)**|패키지의 암호입니다.|  
|**dts_package_location**|**int**|패키지 위치입니다. 패키지의 위치 일 수 있습니다 **배포자** 또는 **구독자**합니다.|  
...
|**job_step_uid**|**uniqueidentifier**|SQL Server 에이전트 작업의 고유 ID 에이전트를 시작 하는 단계입니다.|  
