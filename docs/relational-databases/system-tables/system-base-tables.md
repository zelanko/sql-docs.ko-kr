...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  시스템 기본 테이블은 특정 데이터베이스에 대한 메타데이터를 실제로 저장하는 기본 테이블입니다. **마스터** 데이터베이스는 다른 데이터베이스에서 찾을 수 없는 몇 가지 추가 테이블을 포함하기 때문에 이 점에서 특별합니다. 이러한 테이블에는 서버 차원 범위의 지속형 메타데이터가 들어 있습니다.  
...
## <a name="system-base-table-metadata"></a>시스템 기본 테이블 메타데이터  
 데이터베이스에 CONTROL, ALTER 또는 VIEW 정의 권한이 있는 권한은 **sys.objects** 카탈로그 보기에서 시스템 기본 테이블 메타데이터를 볼 수 있습니다. 또한 grante는 [OBJECT_NAME](../../t-sql/functions/object-name-transact-sql.md) 및 [OBJECT_ID](../../t-sql/functions/object-id-transact-sql.md)같은 기본 제공 함수를 사용하여 시스템 기본 테이블의 이름과 개체 ID를 확인할 수 있습니다.  
...
|**sys.sysidxstats**|모든 데이터베이스에 있습니다. 테이블 및 인덱싱된 뷰의 각 인덱스 또는 통계에 대한 행을 포함합니다.<br /><br /> 참고: 힙을 제외한 모든 인덱스는 인덱스와 이름이 같은 통계와 연결됩니다.|  
...
|**sys.sysdbreg**|**마스터** 데이터베이스에만 존재합니다. 등록된 각 데이터베이스에 대한 행을 포함합니다.|  
|**sys.sysxsrvs**|**마스터** 데이터베이스에만 존재합니다. 각 로컬 서버, 연결된 서버 또는 원격 서버에 대한 행을 포함합니다.|  
|**sys.sysrmtlgns**|이 시스템 기본 테이블은 **마스터** 데이터베이스에만 존재합니다. 각 원격 로그인 매핑에 대한 행을 포함합니다. 이는 해당 서버에서 들어오는 로그인을 실제 로컬 로그인으로 매핑하는 데 사용됩니다.|  
|**sys.syslnklgns**|**마스터** 데이터베이스에만 존재합니다. 각 연결된 로그인 매핑에 대한 행을 포함합니다. 연결된 로그인 매핑은 로컬 서버에서 해당 연결된 서버로 나가는 원격 프로시저 호출 및 분산 쿼리에 사용됩니다.|  
|**sys.sysxlgns**|**마스터** 데이터베이스에만 존재합니다. 각 서버 보안 주체에 대한 행을 포함합니다.|  
|**sys.sysdbfiles**|모든 데이터베이스에 있습니다. **dbid** 열이 0이면 행은 이 데이터베이스에 속한 파일을 나타냅니다. **마스터** 데이터베이스에서 **dbid** 열은 0이 아닐 수 있습니다. 이 경우 해당 행은 마스터 파일을 나타냅니다.|  
|**sys.sysusermsg**|**마스터** 데이터베이스에만 존재합니다. 각 행은 사용자 정의 오류 메시지를 나타냅니다.|  
|**sys.sysprivs**|모든 데이터베이스에 있습니다. 각 데이터베이스 또는 서버 수준 사용 권한에 대한 행을 포함합니다.<br /><br /> 참고: 서버 수준 권한은 **마스터** 데이터베이스에 저장됩니다.|  
...
|**sys.syslogshippers**|**마스터** 데이터베이스에만 존재합니다. 각 데이터베이스 미러링 모니터 서버에 대한 행을 포함합니다.|  
...
|**sys.sysendpts**|**마스터** 데이터베이스에만 존재합니다. 서버에 생성된 각 엔드포인트에 대한 행을 포함합니다.|  
|**sys.syswebmethods**|**마스터** 데이터베이스에만 존재합니다. 서버에 생성된 SOAP 기반 HTTP 엔드포인트에 정의된 각 SOAP 메서드에 대한 행을 포함합니다.|  
...
시스템 카탈로그 뷰를 통해 시스템 테이블의 데이터를 볼 수 있습니다. 시스템 기본 테이블에서 메타데이터를 업데이트하려면 적절한 TSQL 인터페이스(예: DDL 문)를 사용합니다. 시스템 테이블을 수동으로 업데이트할 수 없습니다. SQL Server는 시스템 테이블에 대한 직접 업데이트를 수행할 때 다음 메시지를 보고합니다.

### <a name="a-system-table-is-manually-updated"></a>시스템 테이블이 수동으로 업데이트됩니다.
Msg 17659: 경고: <id> 시스템 테이블 ID가 <id> 데이터베이스 ID에서 직접 업데이트되었으며 캐시 일관성이 유지되지 않았을 수 있습니다. SQL Server를 다시 시작해야 합니다.

### <a name="starting-a-database-with-a-system-table-that-was-manually-updated"></a>수동으로 업데이트된 시스템 테이블로 데이터베이스 시작
Msg 3859: 경고: 시스템 카탈로그는 데이터베이스 ID 17에서 직접 업데이트되었으며, 가장 최근에는 date_time.

### <a name="executing-the-dbcc_checkdb-command-after-a-system-table-is-manually-updated"></a>시스템 테이블이 수동으로 업데이트된 후 DBCC_CHECKDB 명령 실행
Msg 3859: 경고: 시스템 카탈로그는 데이터베이스 ID 17에서 직접 업데이트되었으며, 가장 최근에는 date_time.

시스템 테이블에 대한 수동 업데이트를 수행하고 문제가 발생하면 백업에서 복원하거나 영향을 받는 데이터베이스의 데이터를 새 데이터베이스로 복사하라는 메시지가 지정될 수 있습니다. [사용자 작업 오류 메시지에](https://docs.microsoft.com/sql/relational-databases/errors-events/mssqlserver-8992-database-engine-error?view=sql-server-ver15#user-action)대해 자세히 알아봅니다.
