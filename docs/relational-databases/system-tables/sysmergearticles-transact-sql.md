...
|**type**|**tinyint**|아티클 유형을 나타내며 다음 중 하나일 수 있습니다.<br /><br /> **10** = 테이블입니다.<br /><br /> **32** = 저장 프로시저 (스키마 전용).<br /><br /> **64** = 뷰 또는 인덱싱된 뷰 (스키마 전용).<br /><br /> **128** = 사용자 정의 함수 (스키마 전용).<br /><br /> **160** = 동의어 (스키마 전용).|  
|**objid**|**int**|개체 식별자입니다.|  
...
|**pre_creation_command**|**tinyint**|아티클이 구독 데이터베이스에서 생성될 때 수행할 기본 동작입니다.<br /><br /> **0 =** none-구독자에서 테이블이 이미 있는 경우 아무 작업도 수행 합니다.<br /><br /> **1** = drop-테이블 다시 만들기 전에 삭제 합니다.<br /><br /> **2** = delete-하위 집합 필터의 WHERE 절을 기반으로 하 여 삭제를 실행 합니다.<br /><br /> **3** = Truncate-동일 **2**, 하지만 행 대신 페이지를 삭제 합니다. 단, WHERE 절은 사용하지 않습니다.|  
...
|**before_image_objid**|**int**|추적 테이블의 개체 ID입니다. 추적 테이블 사용 하 여 게시를 만들 때 특정 키 열 값을 포함 *@keep_partition_changes*  =  **true**합니다.|  
|**before_view_objid**|**int**|뷰 테이블의 개체 ID입니다. 뷰는 행이 삭제 또는 업데이트되기 전에 특정 구독자에 속했는지를 추적하는 테이블에 있습니다. 게시를 사용 하 여 만들 경우에 적용 됩니다 *@keep_partition_changes*  =  **true입니다.**|  
...
|**fast_multicol_updateproc**|**bit**|병합 에이전트를 사용하도록 설정하여 한 UPDATE 문에서 같은 행의 여러 열에 변경 사항을 적용했는지 여부를 지정합니다.<br /><br /> **0** = 각 열에 대해 별도 업데이트로 변경 하는 문제입니다.<br /><br /> **1** = 한 문의 여러 열 업데이트는 UPDATE 문을 실행 합니다.|  
