...
|**failover\_mode**|**tinyint**|[장애 조치 모드](../../database-engine/availability-groups/windows/failover-and-failover-modes-always-on-availability-groups.md) 가용성 복제본 중 하나입니다.<br /><br /> 0 &#124; 자동 장애 조치 합니다. 복제본이 자동 장애 조치(failover)의 잠재적인 대상입니다.  가용성 모드를 동기 커밋으로 설정 하는 경우에 자동 장애 조치가 지원 됩니다 (**가용성\_모드** = 1) 있고 가용성 복제본이 현재 동기화 합니다.<br /><br /> 1 &#124; 수동 장애 조치 합니다. 수동 장애 조치(failover)로 설정된 보조 복제본에 대한 장애 조치(failover)를 데이터베이스 관리자가 수동으로 시작해야 합니다. 수행되는 장애 조치(failover)의 유형은 다음과 같이 보조 복제본이 동기화되는지 여부에 따라 달라집니다.<br /><br /> 가용성 복제본이 동기화되고 있지 않거나 아직 동기화 중인 경우 데이터가 손실될 수 있는 강제 장애 조치(failover)만 발생할 수 있습니다.<br /><br /> 가용성 모드를 동기 커밋으로 설정 된 경우 (**가용성\_모드** = 1) 및 데이터 손실이 발생할 수 없는 가용성 복제본이 현재 동기화 된 수동 장애 조치 합니다.<br /><br /> 가용성 복제본에서 모든 가용성 데이터베이스의 데이터베이스 동기화 상태 롤업을 보려면 사용 합니다 **동기화\_상태** 하 고 **동기화\_상태\_desc** 의 열을 [sys.dm_hadr_availability_replica_states](../../relational-databases/system-dynamic-management-views/sys-dm-hadr-availability-replica-states-transact-sql.md) 동적 관리 뷰. 롤업에서는 모든 가용성 데이터베이스의 동기화 상태와 해당 가용성 복제본의 가용성 모드를 고려합니다.<br /><br /> **참고:** 지정된 된 가용성 데이터베이스의 동기화 상태를 보려면 쿼리를 **동기화\_상태** 하 고 **동기화\_상태** 합니다 열의[sys.dm_hadr_database_replica_states](../../relational-databases/system-dynamic-management-views/sys-dm-hadr-database-replica-states-transact-sql.md) 동적 관리 뷰.|  
