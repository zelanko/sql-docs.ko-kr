...
|**유형**|**char(1)**|보안 주체 유형입니다.<br /><br /> A = 애플리케이션 역할<br /><br /> C = 인증서로 매핑된 사용자<br /><br /> E = Azure Active Directory의 외부 사용자<br /><br /> G = Windows 그룹<br /><br /> K = 비대칭 키로 매핑된 사용자<br /><br /> R = 데이터베이스 역할<br /><br /> S = SQL 사용자<br /><br /> U = Windows 사용자<br /><br /> X = Azure Active Directory 그룹 또는 응용 프로그램의 외부 그룹|  
...
|**owning_principal_id**|**int**|이 보안 주체를 소유하는 보안 주체의 ID입니다. 데이터베이스 역할을 제외한 모든 보안 주체는 **dbo**에서 소유 해야 합니다.|  
...
|**authentication_type**|**int**|**적용 대상**: [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 이상<br /><br /> 인증 유형을 나타냅니다. 다음은 가능한 값과 그에 대 한 설명입니다.<br /><br /> 0: 인증 안 함<br />1: 인스턴스 인증<br />2: 데이터베이스 인증<br />3: Windows 인증|  
...
> [!IMPORTANT]  
>  고정 데이터베이스 역할의 사용 권한은 `sys.database_permissions`에 표시 되지 않습니다. 따라서 데이터베이스 보안 주체가 여기에 나열되지 않은 추가 사용 권한을 가질 수 있습니다.  
...
### <a name="d-listing-permissions-on-schema-objects-within-a-database"></a>D: 데이터베이스 내의 스키마 개체에 대 한 사용 권한 나열  
 다음 쿼리는 `sys.database_principals` 및 `sys.database_permissions` `sys.schemas` `sys.objects`에 조인 하 여 특정 스키마 개체에 부여 되거나 거부 된 사용 권한을 나열 합니다.  
