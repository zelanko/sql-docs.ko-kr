...
|file_guid|**uniqueidentifier**|파일의 고유 식별자입니다.<br /><br /> NULL = 데이터베이스가 이전 버전의 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 업그레이드 되었습니다 (SQL Server 2005 및 이전 버전에 유효).|  
...
|credential_id|**int**|파일을 저장 하는 데 사용 되는 `sys.credentials`의 `credential_id`입니다. 예를 들어 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Azure 가상 머신에서 실행 중이 고 데이터베이스 파일이 Azure blob 저장소에 저장 된 경우 자격 증명은 저장소 위치에 대 한 액세스 자격 증명을 사용 하 여 구성 됩니다.|  
