...
|**sent_status**|**varchar(8)**|메일의 상태입니다. 가능한 값은<br /><br /> **전송** -메일을 보냈습니다.<br /><br /> **보내지 않은** -데이터베이스 메일이 메시지 보내기를 시도 합니다.<br /><br /> **다시 시도** -데이터베이스 메일 메시지를 보내려고 실패 했지만 다시 전송 하려고 합니다.<br /><br /> **실패 한** -데이터베이스 메일 메시지를 보낼 수 없습니다.|  
...
## <a name="permissions"></a>사용 권한  
 부여 **sysadmin** 고정된 서버 역할 및 **DatabaseMailUserRole** 데이터베이스 역할. 멤버에 의해 실행 된 **sysadmin** 고정 서버 역할,이 뷰에 모든 메시지를 표시 합니다. 그 밖의 다른 사용자는 자신이 제출한 메시지만 볼 수 있습니다.  
