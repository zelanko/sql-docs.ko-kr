...
|**is_autogrow_all_files**|**bit**|**적용 대상**: [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ([!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] ~ [현재 버전](http://go.microsoft.com/fwlink/p/?LinkId=299658)).<br /><br /> 1 = 파일에 파일 그룹의 모든 파일이 자동으로 증가 임계값을 증가 파일 그룹 충족 될 때.<br /><br /> 0 = 파일에만 파일 자동 증가 임계값을 증가 함에 따라 파일 그룹 충족 될 때. 기본값입니다.|  
