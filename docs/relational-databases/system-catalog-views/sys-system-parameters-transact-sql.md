...
|**max_length**|**smallint**|매개 변수의 최대 길이(바이트)입니다. 값은 열 데이터 형식이 인 경우-1 됩니다 **varchar (max)**, **nvarchar (max)**, **varbinary (max)**, 또는 **xml**합니다.|  
...
|**default_value**|**sql_variant**|경우 **has_default_value** 1 이면이 열의 값은 매개 변수 기본값의 값입니다; 그렇지 않으면 NULL입니다.|  
|**xml_collection_id**|**int**|0이 아닌 매개 변수의 데이터 형식이 **xml** XML이 입력 하 고 있습니다. 이 값은 매개 변수의 유효성 검사 XML 스키마 네임스페이스를 포함하는 컬렉션의 ID입니다.<br /><br /> 0 = XML 스키마 컬렉션이 없습니다.|  
