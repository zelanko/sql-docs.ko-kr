...
|-----------------|---------------|-----------------|  
|**database_id**|**int**|
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스 내에서 고유한 데이터베이스 ID입니다.|  
...
|**last_log_backup_lsn**|**숫자 (25, 0)**|다음 로그 백업의 시작 로그 시퀀스 번호입니다.<br /><br /> NULL 인 경우 데이터베이스가 단순 복구에 있거나 현재 데이터베이스 백업이 없기 때문에 트랜잭션 로그 백업을 수행할 수 없습니다.|  
...
|**fork_point_lsn**|**숫자 (25, 0)**|**First_recovery_fork_guid** (! =)이 같지 않은 경우 fork_point_lsn **recovery_fork_guid**은 **** 현재 분기 지점의 로그 시퀀스 번호입니다. 그렇지 않으면 값은 NULL입니다.|  
