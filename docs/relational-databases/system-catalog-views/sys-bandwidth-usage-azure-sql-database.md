...
ms.locfileid: "33181869"
---
# <a name="sysbandwidthusage-azure-sql-database"></a>sys.bandwidth_usage(Azure SQL Database)
[!INCLUDE[tsql-appliesto-xxxxxx-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-xxxxxx-asdb-xxxx-xxx-md.md)]

  **참고:이만 적용 됩니다 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]V11 합니다.**  
  
 각 데이터베이스에 사용 되는 네트워크 대역폭에 대 한 정보를 반환 된  **[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] V11 논리 서버**, 합니다. 지정된 데이터베이스에 대해 반환된 각 행에는 1시간 동안 이루어진 단일 방향 및 클래스 사용이 요약되어 있습니다.  
  
 **이 사용 되지는 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] V12 논리 서버입니다.**  
  
 **sys.bandwidth_usage** 다음 열이 뷰에 포함 합니다.  
...
### <a name="external-and-internal-classes"></a>External 및 Internal 클래스  
 지정된 된 시간에 사용 되는 각 데이터베이스에 대 한는 **sys.bandwidth_usage** 뷰 클래스와 대역폭 사용량의 방향을 보여 주는 행을 반환 합니다. 다음 예에서는 지정된 데이터베이스에 대해 노출될 수 있는 데이터를 보여 줍니다. 이 예에서 시간은 2012-04-21 17:00:00이고 피크 시간 중에 발생합니다. 데이터베이스 이름은 Db1입니다. 이 예제에서는 **sys.bandwidth_usage** 다음과 같이 Ingress 및 Egress 방향과 External 및 Internal 클래스의 모든 4 가지 조합에 대 한 행을 반환 했습니다.  
...
### <a name="interpreting-data-direction-for-includessgeodrincludesssgeodr-mdmd"></a>[!INCLUDE[ssGeoDR](../../includes/ssgeodr-md.md)]에 대한 데이터 방향 해석  
 [!INCLUDE[ssGeoDR](../../includes/ssgeodr-md.md)]의 경우 논리 master 데이터베이스의 양쪽 연속 복사 관계에 대역폭 사용 데이터가 표시됩니다. 따라서 쿼리하고 있는 논리 서버의 측면에서 수신 및 송신 방향 지표를 해석해야 합니다. 예를 들어 대상 서버에 원본 서버에서 1 MB의 데이터를 전송 하는 복제 스트림이 있는 것이 좋습니다. 이 경우 원본 서버에서 1 MB는 전송 된 총 데이터까지 계산 하 고 대상 서버에서 1 MB는 수신 된 데이터로 기록 됩니다.  
