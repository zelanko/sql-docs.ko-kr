 ...
 *Drivercompletion* 값에는 드라이버의 연결 대화 상자를 표시 해야 하거나 필요한 경우 **SQLDriverConnect** 에 올바른 *WindowHandle* 를 지정 해야 합니다. 잘못된 핸들을 지정하면 SQL_ERROR가 반환됩니다.  
 ...
 DRIVER 또는 DSN 키워드 중 하나를 지정합니다. 둘 모두 지정한 경우 ODBC 드라이버는 이 두 키워드 중 왼쪽의 키워드를 사용하고 다른 하나는 무시합니다. DRIVER를 지정 하거나 둘 중 가장 왼쪽에 있는 **SQLDriverConnect Drivercompletion**__ 매개 변수 값이 SQL_DRIVER_NOPROMPT 이면 서버 키워드와 적절 한 값이 필요 합니다.  
