...
|APP|아니요|예|호출 하는 응용 프로그램의 이름 **SQLBrowseConnect**.|  
...
## <a name="sqlbrowseconnect-support-for-high-availability-disaster-recovery"></a>고가용성 재해 복구를 위한 SQLBrowseConnect 지원  
 사용에 대 한 자세한 내용은 **SQLBrowseConnect** 에 연결 하는 [!INCLUDE[ssHADR](../../includes/sshadr-md.md)] 클러스터를 참조 하십시오. [고가용성 재해 복구에 대 한 SQL Server 네이티브 클라이언트 지원](../../relational-databases/native-client/features/sql-server-native-client-support-for-high-availability-disaster-recovery.md).  
