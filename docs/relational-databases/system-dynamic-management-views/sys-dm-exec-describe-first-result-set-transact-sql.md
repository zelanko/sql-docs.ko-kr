---
title: sys.dm_exec_describe_first_result_set (Transact SQL) | Microsoft 문서
ms.custom: ''
ms.date: 06/10/2016
ms.prod: sql
 ...
 **sys.dm_exec_describe_first_result_set** 가 동일한 결과 집합으로 정의 [sys.dm_exec_describe_first_result_set_for_object &#40;Transact SQL&#41; ](../../relational-databases/system-dynamic-management-views/sys-dm-exec-describe-first-result-set-for-object-transact-sql.md) 와 유사한 [sp_ describe_first_result_set &#40;Transact SQL&#41;](../../relational-databases/system-stored-procedures/sp-describe-first-result-set-transact-sql.md).  
 ...
 *\@tsql*  
 하나 이상의 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문입니다. *거래 SQL_batch* 수 있습니다 **nvarchar (***n***)** 또는 **nvarchar(max)**.  
  
 *\@매개 변수*  
 \@대 한 매개 변수 선언 문자열을 제공 하는 매개 변수는 [!INCLUDE[tsql](../../includes/tsql-md.md)] 유사한 sp_executesql 일괄 처리 합니다. 매개 변수 수 있습니다 **nvarchar (n)** 하거나 **nvarchar (max)** 합니다.  
  
 하는 하나의 문자열에 포함 된 모든 매개 변수의 정의 포함 하는 [!INCLUDE[tsql](../../includes/tsql-md.md)] *_batch*. 문자열은 유니코드 상수 또는 유니코드 변수여야 합니다. 각 매개 변수의 정의는 매개 변수 이름과 데이터 형식으로 구성됩니다. *n* 추가 매개 변수 정의 나타내는 자리 표시자입니다. Stmt에 지정 된 모든 매개 변수에서 정의 되어야 합니다 \@매개 변수입니다. 경우는 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문 또는 문의 일괄 처리에 매개 변수가 없습니다 \@params가 필요 하지 않습니다. NULL이 이 매개 변수의 기본값입니다.  
...
|**system_type_id**|**int**|System_type_id sys.types에 지정 된 대로 열 데이터 형식이 포함 되어 있습니다. CLR 형식의 경우 system_type_name 열에서 NULL을 반환해도 이 열은 값 240을 반환합니다.|  
...
|**order_by_is_descending**|**smallint NULL**|ordinal_in_order_by_list을 NULL이 아닌 경우는 **order_by_is_descending** 열을 ORDER BY 절이이 열에 대 한 방향을 보고 합니다. 그렇지 않으면 NULL을 보고합니다.|  
...
## <a name="remarks"></a>Remarks  
 이 함수와 같은 알고리즘을 사용 하 여 **sp_describe_first_result_set**. 자세한 내용은 [sp_describe_first_result_set &#40;Transact SQL&#41;](../../relational-databases/system-stored-procedures/sp-describe-first-result-set-transact-sql.md).  
...
|10|TEMPORARY_TABLE|일괄 처리가 임시 테이블을 포함 하 고에서 지원 되지 않습니다 때문에 결과 확인할 수 없습니다 **sp_describe_first_result_set** .|  
|11|UNSUPPORTED_STATEMENT|일괄 처리에 의해 지원 되지 않는 문 포함 결과 확인할 수 없습니다 **sp_describe_first_result_set** (예: 반입, 되돌리기 등.).|  
|12|OBJECT_TYPE_NOT_SUPPORTED|\@object_id 함수에 전달 되었습니다 (예: 저장된 프로시저 아님)를 지원 합니다.|  
|13|OBJECT_DOES_NOT_EXIST|\@함수에 전달 된 object_id 시스템 카탈로그에서 찾을 수 없습니다.|  
  
## <a name="permissions"></a>사용 권한  
 실행 하기 위한 권한이 필요 합니다 \@tsql 인수입니다.  
  
## <a name="examples"></a>예  
 항목에서 추가 예제를 보려면 [sp_describe_first_result_set &#40;Transact SQL&#41; ](../../relational-databases/system-stored-procedures/sp-describe-first-result-set-transact-sql.md) 사용 하 여 적용할 수 있습니다 **sys.dm_exec_describe_first_result_set**.  
...
### <a name="b-returning-information-about-a-procedure"></a>2. 프로시저에 대한 정보 반환  
 다음 예제에서는 두 개의 결과 집합을 반환 하는 pr_TestProc 저장된 프로시저를 만듭니다. 예제는 다음 **sys.dm_exec_describe_first_result_set** 절차에서 설정 하는 첫 번째 결과 대 한 정보를 반환 합니다.  
