...
> [!NOTE]  
>  통해 반환 된 정보 중 일부를이 함수에 매핑되는 [sys.syscacheobjects](../../relational-databases/system-compatibility-views/sys-syscacheobjects-transact-sql.md) 이전 버전과 호환성 보기.
 ...
 *plan_handle*  
 실행된 일괄 처리에 대한 쿼리 계획을 고유하게 식별하며 해당 계획은 계획 캐시에 있습니다. *plan_handle* 됩니다 **varbinary(64)** 합니다. 계획 핸들을 가져올 수 있습니다 합니다 [sys.dm_exec_cached_plans](../../relational-databases/system-dynamic-management-views/sys-dm-exec-cached-plans-transact-sql.md) 동적 관리 뷰.  
...
### <a name="evaluating-set-options"></a>SET 옵션 평가  
 반환 되는 값을 변환할 **set_options** 계획 컴파일 시 사용 된 옵션을 뺍니다의 값을 **set_options** 할 때까지 가능한 가장 큰 값을 사용 하 여 시작 하며, 값 0에 도달 합니다. 뺀 각 값은 쿼리 계획에 사용된 옵션에 해당합니다. 예를 들어 경우 값 **set_options** 이 251 인 계획을 사용 하 여 컴파일된 옵션은 ANSI_NULL_DFLT_ON (128), QUOTED_IDENTIFIER (64), ANSI_NULLS(32, ANSI_WARNINGS (16), CONCAT_NULL_YIELDS_NULL (8), Parallel plan(2 및 ANSI_PADDING (1)입니다.  
...
### <a name="evaluating-cursor-options"></a>커서 옵션 평가  
 반환 되는 값을 변환할 **required_cursor_options** 하 고 **acceptable_cursor_options** 계획 컴파일 시 사용 된 옵션을 뺍니다 열 값을 사용 하 여 시작 값 가능한 가장 큰 값을 0에 도달할 때까지 합니다. 뺀 각 값은 쿼리 계획에 사용된 커서 옵션에 해당합니다.  
