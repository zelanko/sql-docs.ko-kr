...
### <a name="a-calculate-fragmentation-to-decide-when-to-reorganize-or-rebuild-a-columnstore-index"></a>A. 조각화를 계산 하 여 columnstore 인덱스를 다시 구성 하거나 다시 작성 하는 시기를 결정 합니다.  
 ...
 이 예에서는 **dm_db_column_store_row_group_physical_stats** 을 다른 시스템 테이블과 조인한 다음 현재 데이터베이스의 각 `Fragmentation` 행 그룹에 대 한 예상 효율성으로 열을 계산 합니다. 단일 테이블에 대 한 정보를 찾으려면 **where** 절 앞에서 주석 하이픈을 제거 하 고 테이블 이름을 제공 합니다.  
