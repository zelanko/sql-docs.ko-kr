...
> [!NOTE]  
> 이를 호출 하 [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] 나 [!INCLUDE[ssPDW](../../includes/sspdw-md.md)], 이름을 사용 하 여 **sys.dm_pdw_nodes_tran_locks**합니다.  
  
|열 이름|데이터 형식|설명|  
|-----------------|---------------|-----------------|  
|**resource_type**|**nvarchar(60)**|리소스 유형을 나타냅니다. 값은 다음 중 하나일 수 있습니다. 데이터베이스, 파일, 개체, 페이지, 키, 범위, RID, 응용 프로그램, 메타 데이터, HOBT 또는 allocation_unit 중 하나일 수 있습니다.|  
...
|**request_status**|**nvarchar(60)**|이 요청의 현재 상태입니다. 가능한 값은 GRANTED, CONVERT, WAIT, LOW_PRIORITY_CONVERT, LOW_PRIORITY_WAIT 또는 ABORT_BLOCKERS입니다. 낮은 우선 순위 대기 및 중단 블 로커에 대 한 자세한 내용은 참조는 *low_priority_lock_wait* 부분 [ALTER INDEX &#40;TRANSACT-SQL&#41;](../../t-sql/statements/alter-index-transact-sql.md).|  
...
|**lock_owner_address**|**varbinary(8)**|이 요청을 추적하는 데 사용되는 내부 데이터 구조의 메모리 주소입니다. 이 열을 조인할 수는 사용 하 여 **resource_address** 열에서 **sys.dm_os_waiting_tasks**합니다.|  
...
## <a name="remarks"></a>설명  
 허용된 요청 상태는 요청자에게 리소스에 대한 잠금이 허용되었음을 나타냅니다. 대기 중인 요청은 해당 요청이 아직 허용되지 않았음을 나타냅니다. 대기 중인 요청 유형은 반환한 합니다 **request_status** 열:  
  
-   변환 요청 상태는 리소스에 대한 요청이 이미 허용되었으며 초기 요청에 대한 업그레이드가 현재 허용 대기 상태에 있음을 나타냅니다.  
  
-   대기 요청 상태는 리소스에 대한 요청이 아직 허용되지 않았음을 나타냅니다.  
  
 때문에 **sys.dm_tran_locks** 내부 잠금 관리자 데이터 구조에서이 정보는 일반에 오버 헤드가 처리를 추가 하지 않습니다 유지 관리에서 채워집니다. 뷰를 구체화하려면 잠금 관리자 내부 데이터 구조에 액세스해야 하므로 서버의 정상적인 처리에 사소한 영향을 줄 수 있습니다. 그러나 그 영향은 무시할 만한 수준이며 매우 많이 사용되는 리소스에만 영향을 미칩니다. 이 뷰의 데이터는 잠금 관리자의 현재 상태에 따라 달라지기 때문에 언제든지 변경될 수 있으며 잠금을 획득하고 해제할 때마다 행이 추가되거나 제거됩니다. 이 뷰에는 기록 정보가 없습니다.  
...
-   FROM 절에서 테이블 잠금 힌트를 사용하여 테이블의 개별 참조에 대한 잠금 수준을 지정합니다. 구문 및 제한 사항에 대 한 참조 [테이블 힌트 &#40;TRANSACT-SQL&#41;](../../t-sql/queries/hints-transact-sql-table.md)합니다.  
 ...
 한 세션 ID로 실행되는 리소스에 둘 이상의 잠금을 허용할 수 있습니다. 한 세션에서 실행 되는 다양 한 엔터티 소유할 수 있습니다 각 동일한 리소스에 대 한 잠금 및 정보에 표시 되는 **request_owner_type** 하 고 **request_owner_id** 열 반환한 **sys.dm_tran_locks**합니다. 경우 같은 여러 인스턴스 **request_owner_type** 존재 합니다 **request_owner_id** 열 각 인스턴스를 구분 하기 위해 사용 됩니다. 분산 트랜잭션의 경우 합니다 **request_owner_type** 하며 **request_owner_guid** 열에는 다양 한 엔터티 정보가 표시 됩니다.  
  
 예를 들어 세션 S1 잠금을 소유 하 고 공유에 **Table1**; 세션 S1에서 실행 되는 트랜잭션 T1에도 공유 잠금을 소유 하 고 **Table1**합니다. 이 경우에 **resource_description** 에서 반환 되는 열 **sys.dm_tran_locks** 같은 리소스의 두 인스턴스가 표시 됩니다. 합니다 **request_owner_type** 열에 세션 및 트랜잭션이 다른 하나의 인스턴스만 표시 됩니다. 또한 합니다 **resource_owner_id** 열의 다른 값이 적용 됩니다.  
...
## <a name="resource-details"></a>리소스 정보  
 다음 표에서에서 표현 되는 리소스를 나열 합니다 **resource_associated_entity_id** 열입니다.  
...
|PAGE|데이터 파일 내의 단일 페이지를 나타냅니다.|HoBt ID입니다. 이 값은 **sys.partitions.hobt_id**합니다. HoBt ID는 호출자가 제공할 수 있는 추가 정보이지만 모든 호출자가 이 정보를 제공할 수 있는 것은 아니기 때문에 PAGE 리소스에 대해 항상 HoBt ID를 사용할 수 있는 것은 아닙니다.|  
...
|METADATA.XML_INDEX_QNAME|[!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]|  
 ...
 다음 표에서 형식을 제공 합니다 **resource_description** 각 리소스 종류에 대 한 열입니다.  
  
|Resource|형식|설명|  
|--------------|------------|-----------------|  
|DATABASE|해당 사항 없음|데이터베이스 ID에서 사용할 수 있는 이미 합니다 **resource_database_id** 열입니다.|  
|FILE|<file_id>|이 리소스가 나타내는 파일의 ID입니다.|  
|OBJECT|<object_id>|이 리소스가 나타내는 개체의 ID입니다. 이 개체에 나열 된 모든 개체를 수 있습니다 **sys.objects**, 뿐 아니라 테이블입니다.|  
...
-   ddl_with_wait_at_low_priority  
 ...
 기존 XEvent **progress_report_online_index_operation** 추가 하 여 온라인 인덱스 작업 확장 되었습니다 **partition_number** 하 고 **partition_id**합니다.  
...
```  
 ...
 다음 쿼리에서는 잠금 정보를 표시합니다. 에 대 한 값 `<dbid>` 바꿔야 합니다 **database_id** 에서 **sys.databases**합니다.  
