...
ms.locfileid: "39544353"
---
# <a name="sysdmexecqueryresourcesemaphores-transact-sql"></a>sys.dm_exec_query_resource_semaphores(Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-all-md](../../includes/tsql-appliesto-ss2008-all-md.md)]

  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 현재 쿼리 리소스 세마포 상태에 대한 정보를 반환합니다. **sys.dm_exec_query_resource_semaphores** 일반적인 쿼리 실행 메모리 상태를 제공 하며, 시스템 메모리가 부족 하 여 액세스할 수 있는지 여부를 확인할 수 있습니다. 이 보기에서 가져온 메모리 정보를 보완 [sys.dm_os_memory_clerks](../../relational-databases/system-dynamic-management-views/sys-dm-os-memory-clerks-transact-sql.md) 서버 메모리 상태의 전체 현황을 제공 합니다. **sys.dm_exec_query_resource_semaphores** 일반 리소스 세마포와 대해 한 행 및 작은 쿼리 리소스 세마포에 대 한 다른 행을 반환 합니다. 작은 쿼리 세마포에 대 한 두 가지 요구 사항을 가지합니다  
  
-   요청한 메모리 부여 5MB 보다 작은 해야 합니다.  
  
-   쿼리 비용 보다 작거나 3 비용 단위 해야 합니다.  
  
> [!NOTE]  
>  이를 호출 하 [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] 나 [!INCLUDE[ssPDW](../../includes/sspdw-md.md)], 이름을 사용 하 여 **sys.dm_pdw_nodes_exec_query_resource_semaphores**합니다.  
...
|**total_memory_kb**|**bigint**|리소스 세마포가 보유한 메모리(KB)입니다. 시스템 메모리가 부족 하거나 강제 적용 된 최소 메모리가 자주 부여 되는 경우이 값 보다 클 수는 **target_memory_kb** 하거나 **max_target_memory_kb** 값입니다. 총 메모리는 사용 가능한 메모리와 부여된 메모리의 합계입니다.|  
...
## <a name="remarks"></a>Remarks  
 ...
 사용 하 여 **sys.dm_exec_query_resource_semaphores** 문제 해결에 대 한 이후 버전을 사용 하는 응용 프로그램에 포함 하지 마십시오 있지만 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]합니다.  
