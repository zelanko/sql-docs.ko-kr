...
[!INCLUDE[tsql-appliesto-ss2012-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2012-xxxx-xxxx-xxx-md.md)]
  ...
  동일한 Always On 가용성 그룹의 로컬 복제본과 각 원격 복제본에 대 한 행과 각 로컬 복제본에 대 한 행을 반환합니다. 각 행에는 지정된 된 복제본의 상태 정보가 들어 있습니다.  
...
|**recovery\_health**|**tinyint**|롤업을 합니다 **데이터베이스\_상태** 열의 합니다 [sys.dm_hadr_database_replica_states](../../relational-databases/system-dynamic-management-views/sys-dm-hadr-database-replica-states-transact-sql.md) 동적 관리 뷰. 가능한 값 및 해당 설명을 보려면은 다음과 같습니다.<br /><br /> 0: 진행 중에서입니다.  하나 이상의 조인 된 데이터베이스의 데이터베이스 상태가 ONLINE (**데이터베이스\_상태** 는 0이 아님).<br /><br /> 1: 온라인입니다. 조인된 된 데이터베이스 전체의 데이터베이스 상태가 ONLINE (**database_state** 0).<br /><br /> NULL : **is_local** = 0|  
|**recovery_health_desc**|**nvarchar(60)**|에 대 한 설명을 **recovery_health**하나씩입니다.<br /><br /> ONLINE_IN_PROGRESS<br /><br /> ONLINE<br /><br /> NULL|  
|**synchronization\_health**|**tinyint**|데이터베이스 동기화 상태에 대 한 롤업을 반영 하며 (**synchronization_state**)의 모든 가용성 데이터베이스 조인 (라고도 *복제본*) 및 복제본 (의 가용성 모드 동기-커밋 또는 비동기-커밋 모드). 롤업은 복제본에 최소 정상 누적된 상태는 데이터베이스를 반영 됩니다. 가능한 값 및 해당 설명이 같습니다.<br /><br /> 0: 올바른 상태가 아닙니다.   하나 이상의 조인된 데이터베이스가 NOT SYNCHRONIZING 상태입니다.<br /><br /> 1: 부분적으로 정상. 일부 복제본이 대상 동기화 상태에 없습니다: 동기-커밋 복제본을 동기화 해야 하 고 비동기-커밋 복제본을 동기화 해야 합니다.<br /><br /> 2: 정상 상태입니다. 모든 복제본이 대상 동기화 상태: 동기-커밋 복제본은 동기화 되어 있고 비동기-커밋 복제본은 동기화가 수행 합니다.|  
...
##  <a name="RolesAndOperationalStates"></a> 역할 및 작동 상태  
 역할 **역할**, 지정된 된 가용성 복제본의 상태 및 작동 상태를 반영 **operational_state**, 복제본이 모든 클라이언트 요청을 처리할 준비가 되었는지 여부에 대해 설명 합니다 가용성 복제본의 데이터베이스입니다. 다음 각 역할에 대해 가능한 작동 상태 요약이 제공 됩니다: 해결, 기본 및 보조 합니다.  
  
 **해결:** 가능한 작동 상태는 다음 표에 나와 있는 것 처럼 가용성 복제본 RESOLVING 역할인 경우.  
 ...
 **기본 사이트:** 가용성 복제본이 주 역할을 수행 하는 것이 현재 주 복제본입니다. 가능한 작동 상태는 다음 표에 나와 있는 것 처럼 됩니다.  
 ...
 **보조:** 가용성 복제본이 보조 역할을 수행 하는 것이 현재 보조 복제본입니다. 가능한 작동 상태는 아래 표에 표시 된 것과 같이 됩니다.  
