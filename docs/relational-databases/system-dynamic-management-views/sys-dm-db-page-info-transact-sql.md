...
|object_id |int |페이지를 소유 하는 개체의 ID |
|index_id |int |인덱스 (힙 데이터 페이지에 대 한 0)의 ID |
|partition_id |BIGINT |파티션의 ID |
|alloc_unit_id |BIGINT |할당 단위 ID |
|page_level |int |인덱스에서 페이지의 수준 (리프 = 0) |
...
|torn_bits |int |조각난된 쓰기가 검색 섹터당 1 비트입니다. 체크섬을 저장 하는 데도 사용 <br> 이 값은 데이터 손상을 검색할 사용 |
...
|diff_map_page_id |int |해당 하는 차등 비트맵 페이지의 페이지 ID |
...
|ml_page_id |int |해당 하는 최소 로깅 비트맵 페이지의 페이지 ID |
...
|free_data_offset |int |공간 데이터 영역의 끝 오프셋 |
