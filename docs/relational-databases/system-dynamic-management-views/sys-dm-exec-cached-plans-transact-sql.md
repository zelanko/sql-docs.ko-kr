...
> [!NOTE]  
>  또는 [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] [!INCLUDE[ssPDW](../../includes/sspdw-md.md)]에서이를 호출 하려면 이름 **sys. dm_pdw_nodes_exec_cached_plans**을 사용 합니다.  
...
|refcounts|**int**|이 캐시 개체를 참조하는 캐시 개체의 수입니다. 캐시에 항목을 입력 하려면 **Refcounts** 1 이상 이어야 합니다.|  
...
|cacheobjtype|**nvarchar (34)**|캐시에 있는 개체의 유형입니다. 값은 다음 중 하나일 수 있습니다.<br /><br /> Compiled Plan<br /><br /> Compiled Plan Stub<br /><br /> Parse Tree<br /><br /> Extended Proc<br /><br /> CLR Compiled Func<br /><br /> CLR Compiled Proc|  
|objtype|**nvarchar (16)**|개체의 유형입니다. 다음은 가능한 값과 해당 설명입니다.<br /><br /> Proc: 저장 프로시저<br />준비 됨: 준비 된 문<br />임시: 임시 쿼리 는 원격 [!INCLUDE[tsql](../../includes/tsql-md.md)] 프로시저 호출 대신 **osql** 또는 **sqlcmd** 를 사용 하 여 언어 이벤트로 제출 된를 나타냅니다.<br />ReplProc: Replication-filter-프로시저<br />트리거: 트리거<br />보기: 보기<br />기본값: 기본값<br />UsrTab: 사용자 테이블<br />SysTab: 시스템 테이블<br />Check: CHECK 제약 조건<br />규칙: 규칙|  
...
### <a name="c-returning-the-set-options-with-which-the-plan-was-compiled"></a>C. 계획 컴파일 시 사용된 SET 옵션의 반환  
 다음 예에서는 계획 컴파일 시 사용된 SET 옵션을 반환합니다. 계획 `sql_handle` 에 대 한도 반환 됩니다. PIVOT 연산자는 `set_options` 및 `sql_handle` 특성을 행이 아닌 열로 출력 하는 데 사용 됩니다. 에서 `set_options`반환 되는 값에 대 한 자세한 내용은 [dm_exec_plan_attributes &#40;transact-sql&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-exec-plan-attributes-transact-sql.md)을 참조 하십시오.  
