...
|2|**Object**|기본 테이블의 geometry 및 geography 데이터가 다시 계산된 후 **sys.dm_db_objects_impacted_on_version_change**로 확인한 모든 제약 조건의 유효성을 다시 검사해야 합니다. ALTER TABLE을 사용하여 제약 조건의 유효성을 다시 검증합니다. <br />예를 들면 다음과 같습니다. <br />`ALTER TABLE <tab> WITH CHECK CHECK CONSTRAINT <constraint name>`<br />또는<br />`ALTER TABLE <tab> WITH CHECK CONSTRAINT ALL`|  
