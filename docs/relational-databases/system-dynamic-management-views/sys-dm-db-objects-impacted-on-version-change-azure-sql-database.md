...
|dependency|**nvarchar(60)** NOT NULL|제약 조건 또는 인덱스에게 영향을 미치는 종속성에 대한 설명입니다. 업그레이드 시 생성되는 경고에 대해서도 동일한 값이 사용됩니다.<br /><br /> 예:<br /><br /> **공간** (에 대 한 내장 함수)<br /><br /> **기 하 도형** (시스템용: UDT)<br /><br /> **geography:: parse** (시스템 UDT 메서드의 경우)에 대 한|  
...
|2|**개체**|로 식별 되는 모든 제약 조건을 **sys.dm_db_objects_impacted_on_version_change** 기본 테이블에 geometry 및 geography 데이터 다시 계산 되 면 유효성을 재검사 해야 합니다. ALTER TABLE을 사용하여 제약 조건의 유효성을 다시 검증합니다. <br />예를 들어: <br />`ALTER TABLE <tab> WITH CHECK CHECK CONSTRAINT <constraint name>`<br />로 구분하거나 여러<br />`ALTER TABLE <tab> WITH CHECK CONSTRAINT ALL`|  
