...
|열 이름|이름|주 복제본에 대한 설명|  
|-----------------|---------------|----------------------------------------|  
|**database_id**|**int**|데이터베이스의 식별자입니다.|  
|**group_id**|**ssNoversion**|데이터베이스가 속한 가용성 그룹의 식별자입니다.|  
|**replica_id**|**ssNoversion**|가용성 그룹 내 가용성 복제본의 식별자입니다.|  
|**group_database_id**|**ssNoversion**|가용성 그룹 내 데이터베이스의 식별자입니다. 이 식별자는 이 데이터베이스가 조인되는 모든 복제본에서 동일합니다.|  
...
|**is_primary_replica**|**bit**|복제본이 주 복제본 이면 1을 반환 하 고 데이터베이스가 속한 가용성 그룹의 보조 복제본 인 경우 0을 반환 합니다. 분산 가용성 그룹 또는 활성 지역 복제 관계에서 주 데이터베이스 또는 보조 데이터베이스를 참조 하지 않습니다.<br /><br />**적용 대상:** [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]부터 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]까지|  
...
|**synchronization_health**|**tinyint**|가용성 복제본의 가용성 그룹에 조인 된 데이터베이스의 동기화 상태와 가용성 복제본의 가용성 모드 (동기 커밋 또는 비동기 커밋 모드)의 교차를 반영 하며, 다음 중 하나입니다. 다음 값<br /><br /> 0 = 비정상 상태입니다. 데이터베이스의 **synchronization_state** 0 (동기화 하지 않음)입니다.<br /><br /> 1 = 부분 정상입니다. 동기 커밋 가용성 복제본의 데이터베이스는 **synchronization_state** 가 1 (동기화 중) 인 경우 부분적으로 정상으로 간주 됩니다.<br /><br /> 2 = 정상 동기 커밋 가용성 복제본의 데이터베이스는 **synchronization_state** 가 2 (SYNCHRONIZED) 인 경우 정상으로 간주 되 고 비동기 커밋 가용성 복제본의 데이터베이스는 **synchronization_state** 경우 정상으로 간주 됩니다. 는 1 (동기화 중)입니다.|  
...
|**database_state_desc**|**nvarchar(60)**|가용성 복제본의 **database_state** 에 대 한 설명입니다.<br /><br /> ONLINE<br /><br /> RESTORING<br /><br /> RECOVERING<br /><br /> RECOVERY_PENDING<br /><br /> SUSPECT<br /><br /> EMERGENCY<br /><br /> OFFLINE<br /><br /> **참고:** **State_desc** 의 열과 동일 합니다.|  
...
|**recovery_lsn**|**numeric(25,0)**|주 복제본에서 주 데이터베이스가 복구 또는 장애 조치(failover) 후 새 로그 레코드를 쓰기 전의 트랜잭션 로그 끝입니다. 지정된 보조 데이터베이스에 대해 이 값이 현재 확정된 LSN(last_hardened_lsn)보다 작으면 recovery_lsn은 이 보조 데이터베이스에서 다시 동기화(즉, 되돌려서 다시 초기화)해야 하는 값입니다. 이 값이 현재 확정된 LSN보다 크거나 같으면 다시 동기화가 필요하지 않으며 발생하지 않습니다.<br /><br /> **recovery_lsn** 은 0으로 채워진 로그-블록 ID를 반영 합니다. 실제 LSN(로그 시퀀스 번호)이 아닙니다.|  
|**truncation_lsn**|**numeric(25,0)**|주 복제본의 주 데이터베이스의 경우 모든 해당 보조 데이터베이스에 최소 로그 잘림 LSN을 반영합니다. 백업 작업 등에 의해 로컬 로그 잘림이 차단되는 경우 이 LSN이 로컬 잘림 LSN보다 클 수 있습니다.<br /><br /> 지정된 보조 데이터베이스의 경우 해당 데이터베이스의 잘림 지점을 반영합니다.<br /><br /> **truncation_lsn** 은 0으로 채워진 로그-블록 ID를 반영 합니다. 실제 로그 시퀀스 번호가 아닙니다.|  
...
|**last_received_lsn**|**numeric(25,0)**|이 보조 데이터베이스를 호스팅하는 보조 복제본이 모든 로그 블록을 받은 지점을 식별하는 로그 블록 ID입니다.<br /><br /> **last_received_lsn** 은 0으로 채워진 로그-블록 ID를 반영 합니다. 실제 로그 시퀀스 번호가 아닙니다.|  
|**last_received_time**|**datetime**|보조 복제본에서 마지막으로 수신된 메시지의 로그 블록 ID를 읽은 시간입니다.|  
|**last_hardened_lsn**|**numeric(25,0)**|보조 데이터베이스에서 마지막으로 확정된 LSN의 로그 레코드를 포함하는 로그 블록의 시작입니다.<br /><br /> 현재 정책이 "지연"인 동기 커밋 데이터베이스 또는 비동기 커밋 주 데이터베이스에서 이 값은 NULL입니다. 다른 동기 커밋 주 데이터베이스의 경우 **last_hardened_lsn** 는 모든 보조 데이터베이스에 대해 확정 된 lsn의 최소값을 나타냅니다.<br /><br /> **참고: last_hardened_lsn** 는 0으로 채워진 로그-블록 ID를 반영 합니다. 실제 로그 시퀀스 번호가 아닙니다.|  
...
|**last_redone_lsn**|**numeric(25,0)**|보조 데이터베이스에서 마지막으로 다시 실행된 로그 레코드의 실제 로그 시퀀스 번호입니다. **last_redone_lsn** 는 항상 **last_hardened_lsn**보다 낮습니다.|  
