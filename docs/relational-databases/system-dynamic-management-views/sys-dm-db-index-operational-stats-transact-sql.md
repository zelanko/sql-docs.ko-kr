---
title: sys.dm_db_index_operational_stats (Transact SQL) | Microsoft 문서
ms.custom: ''
ms.date: 06/10/2016
ms.prod: sql
...
> [!NOTE]    
>  **sys.dm_db_index_operational_stats** 메모리 최적화 된 인덱스에 대 한 정보를 반환 하지 않습니다. 메모리 최적화 된 인덱스 사용에 대 한 내용은 [sys.dm_db_xtp_index_stats &#40;Transact SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-db-xtp-index-stats-transact-sql.md).    
 ...
 *database_id* | NULL | 0 | DEFAULT    
 데이터베이스의 ID입니다. *database_id* 는 **smallint**. 올바른 입력은 데이터베이스의 ID 번호, NULL, 0 또는 DEFAULT입니다. 기본값은 0입니다. 이 컨텍스트에서 NULL, 0 및 DEFAULT는 동등한 값입니다.    
    
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스의 모든 데이터베이스에 대한 정보를 반환하려면 NULL을 지정합니다. NULL을 지정 하면 *database_id*, NULL을 지정 해야 *object_id*, *이뤄지고*, 및 *partition_number*.    
    
 기본 제공 함수 [DB_ID](../../t-sql/functions/db-id-transact-sql.md) 를 지정할 수 있습니다.    
 ...
 *object_id* | NULL | 0 | DEFAULT    
 ...
 지정된 데이터베이스에 있는 모든 테이블 및 뷰에 대한 캐시된 정보를 반환하려면 NULL을 지정합니다. NULL을 지정 하면 *object_id*, NULL을 지정 해야 *이뤄지고* 및 *partition_number*.    
    
 *index_id* | 0 | NULL | -1 | DEFAULT    
 인덱스의 ID입니다. *인덱스* 는 **int**. 올바른 입력은 0, 인덱스 ID 번호 *object_id* 힙, NULL, 기본값 또는-1. 기본값은 -1입니다. 이 경우 NULL, -1 및 DEFAULT는 동등한 값입니다.    
    
 기본 테이블 또는 뷰의 모든 인덱스에 대한 캐시된 정보를 반환하려면 NULL을 지정합니다. NULL을 지정 하면 *이뤄지고*, NULL을 지정 해야 *partition_number*.    
    
 *partition_number* | NULL | 0 | 기본    
 개체의 파티션 번호입니다. *partition_number* 는 **int**. 유효한 입력은의 *partion_number* 의 인덱스 또는 힙에서 NULL, 0 또는 기본. 기본값은 0입니다. 이 컨텍스트에서 NULL, 0 및 DEFAULT는 동등한 값입니다.    
...
|**hobt_id**|**bigint**|**적용 대상**: [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ([!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] ~ [현재 버전](http://go.microsoft.com/fwlink/p/?LinkId=299658)), [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)].<br /><br /> 데이터 힙 또는 columnstore 인덱스에 대 한 내부 데이터를 추적 하는 B-트리 행 집합 ID입니다.<br /><br /> NULL – 내부 columnstore 행 집합을이 아닙니다.<br /><br /> 자세한 내용은 참조 하십시오. [sys.internal_partitions &#40;Transact SQL&#41;](../../relational-databases/system-catalog-views/sys-internal-partitions-transact-sql.md)|    
|**partition_number**|**int**|인덱스 또는 힙 내의 1부터 시작하는 파티션 번호입니다.|    
|**leaf_insert_count**|**bigint**|리프 수준 삽입의 누적 횟수입니다.|    
|**leaf_delete_count**|**bigint**|리프 수준 삭제의 누적 횟수입니다. leaf_delete_count만 먼저 유령으로 표시 되지 않는 삭제 된 레코드에 대 한 증가 합니다. 먼저 희미하게는 삭제 된 레코드에 대 한 **leaf_ghost_count** 는 대신 증가 합니다.|    
...
|**row_overflow_fetch_in_pages**|**bigint**|ROW_OVERFLOW_DATA 할당 단위에서 검색된 행 오버플로 데이터 페이지의 누적 개수입니다.<br /><br /> 이러한 페이지 형식의 열에 저장 된 데이터를 포함 **varchar(n)**, **nvarchar(n)**, **varbinary(n)**, 및 **sql_variant** 된을 행을 전달 합니다.|    
...
|**tree_page_latch_wait_count**|**bigint**|하위 집합의 **page_latch_wait_count** 상급 B-트리 페이지를 포함 하는. 힙 또는 columnstore 인덱스의 경우 항상 0입니다.|    
|**tree_page_latch_wait_in_ms**|**bigint**|하위 집합의 **page_latch_wait_in_ms** 상급 B-트리 페이지를 포함 하는. 힙 또는 columnstore 인덱스의 경우 항상 0입니다.|    
|**tree_page_io_latch_wait_count**|**bigint**|하위 집합의 **page_io_latch_wait_count** 상급 B-트리 페이지를 포함 하는. 힙 또는 columnstore 인덱스의 경우 항상 0입니다.|    
|**tree_page_io_latch_wait_in_ms**|**bigint**|하위 집합의 **page_io_latch_wait_in_ms** 상급 B-트리 페이지를 포함 하는. 힙 또는 columnstore 인덱스의 경우 항상 0입니다.|    
...
## <a name="remarks"></a>Remarks    
 ...
 사용할 수 있습니다 **sys.dm_db_index_operational_stats** 또는 테이블, 인덱스 또는 파티션의 쓸 읽고 확인 하는 테이블이 나 인덱스에 중요 한 I/O 작업이 발생 하는 또는 핫 사용자가 기다려야 하는 시간을 추적 하기 위해 얼룩이 있습니다.    
...
## <a name="column-remarks"></a>열에 대한 주의    
 값 **lob_orphan_create_count** 및 **lob_orphan_insert_count** 항상 같아야 합니다.    
    
 값 열에 **lob_fetch_in_pages** 및 **lob_fetch_in_bytes** 에 포함 된 열을 하나 이상의 LOB 열이 있는 비클러스터형 인덱스는 0 보다 커야 합니다. 자세한 내용은 [Create Indexes with Included Columns](../../relational-databases/indexes/create-indexes-with-included-columns.md)을 참조하세요. 마찬가지로, 값 열에 **row_overflow_fetch_in_pages** 및 **row_overflow_fetch_in_bytes** 색인 처리할 수 있는 열이 포함 된 경우 비클러스터형 인덱스는 0 보다 큰 수 행.    
    
## <a name="how-the-counters-in-the-metadata-cache-are-reset"></a>메타데이터 캐시의 카운터를 다시 설정하는 방법    
 반환 되는 데이터 **sys.dm_db_index_operational_stats** 시간 만큼 힙 또는 인덱스를 나타내는 메타 데이터 캐시 개체를 사용할 수 있습니다. 이 데이터는 영구적이지도 않고 트랜잭션 측면에서 일관되지도 않습니다. 즉, 이러한 카운터로는 인덱스가 사용되었는지 여부나 인덱스가 마지막으로 사용된 시기를 확인할 수 없습니다. 이 대 한 내용은 [sys.dm_db_index_usage_stats &#40;Transact SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-db-index-usage-stats-transact-sql.md).    
...
-   @ 개체 와일드 카드를 사용 하 여 지정된 된 데이터베이스의 모든 개체에 대 한 정보를 반환할 데이터베이스 상태 보기 권한을*object_id* = NULL    
    
-   @ 데이터베이스 와일드 카드를 사용 하 여 모든 데이터베이스에 대 한 정보를 반환 하도록 서버 상태 보기 권한을*database_id* = NULL    
    
 VIEW DATABASE STATE를 허용하면 특정 개체에 대해 거부된 CONTROL 권한에 관계없이 데이터베이스의 모든 개체가 반환됩니다.    
    
 VIEW DATABASE STATE를 거부하면 특정 개체에 대해 허용된 CONTROL 권한에 관계없이 데이터베이스의 모든 개체가 반환되지 않습니다. 또한, @ 데이터베이스 와일드 카드*database_id*= NULL이 지정 되 면 데이터베이스를 생략 합니다.    
