...
|**상태**|**int**|테이프의 상태입니다.<br /><br /> 1 = 열려 있음. 사용 가능.<br /><br /> 2 = 탑재 보류 중<br /><br /> 3 = 사용 중<br /><br /> 4 = 로드 중<br /><br /> **참고:** 테이프를 로드 하는 동안 (**상태 = 4**)을 미디어 레이블을 읽지 않습니다. 와 같은 미디어 레이블 값을 복사 하는 열 **media_sequence_number**을 미리 예상 하는 표시 값을 테이프에 실제 값에서 다를 수 있습니다. 레이블을 읽은 후 **상태** 변경 **3** (사용)의 미디어 레이블 열에는 다음 로드 된 실제 테이프가 반영 합니다.<br /><br /> Null을 허용하지 않습니다.|  
