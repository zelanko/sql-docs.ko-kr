...
## <a name="alter-non-temporal-table-to-be-a-system-versioned-temporal-table"></a>비temporal 테이블을 시스템 버전 관리 temporal 테이블로 변경
...
> [!IMPORTANT]
> DATETIME2의 전체 자릿수는 기본 테이블의 전체 자릿수와 일치해야 합니다. 다음 설명을 참조하세요.
...
- 숨겨진 열을 기본으로 사용하는 경우 `ALTER TABLE [tableName] ALTER COLUMN [columnName] ADD HIDDEN;` 명령을 사용하세요.
