...
ms.topic: "article"
helpviewer_keywords: 
  - "복원 중 파일 상태 [SQL Server]"
  - "파일 상태 확인"
  - "현재 파일 상태"
  - "파일 그룹 상태 확인"
  - "파일 상태 [SQL Server]"
  - "온라인 파일 상태"
  - "오프라인 파일 상태 [SQL Server]"
  - "파일 그룹 상태 보기"
  - "파일 상태 보기"
  - "주의 대상 파일 상태"
  - "파일 상태 복구 [SQL Server]"
  - "현재 파일 그룹 상태"
  - "복구 보류 파일 상태 [SQL Server]"
  - "파일 상태 표시"
  - "상태 [SQL Server], 파일"
  - "파일 그룹 상태 표시"
  - "존재하지 않는 파일 상태"
...
|OFFLINE|파일에 액세스할 수 없으며 디스크에 파일이 없을 수도 있습니다. 명시적 사용자 동작으로 인해 파일이 오프라인 상태가 되면 추가 사용자 작업이 수행될 때까지 오프라인 상태로 있습니다.<br /><br /> **\*\* 주의 \*\*** 파일이 손상되면 파일을 오프라인으로만 설정해야 합니다. 그러나 손상된 파일은 복원할 수 있습니다. 오프라인으로 설정된 파일을 온라인으로 설정할 수 있는 유일한 방법은 백업에서 파일을 복원하는 것입니다. 단일 파일 복원 방법은 [RESTORE&#40;Transact-SQL&#41;](../Topic/RESTORE%20\(Transact-SQL\).md)를 참조하세요.|  
