---
title: "다른 서버 인스턴스에서 데이터베이스를 사용할 수 있도록 할 때 메타데이터 관리(SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "데이터베이스 간 쿼리 [SQL Server]"
  - "로그인 [SQL Server], 다른 서버 인스턴스에서 다시 만들기"
  - "트리거 [SQL Server], DLL"
  - "사용자 정의 오류 메시지 [SQL Server]"
  - "사용 권한 [SQL Server], 메타데이터 액세스"
  - "전체 텍스트 엔진 [SQL Server]"
  - "메타데이터 [SQL Server], 다른 인스턴스에서 사용할 수 있는 데이터베이스"
  - "작업 [SQL Server 에이전트], 다른 서버 인스턴스에서 다시 만들기"
  - "장애 조치(Failover) [SQL Server], 메타데이터 관리"
  - "이벤트 알림 [SQL Server], 메타데이터"
  - "데이터베이스 미러링 [SQL Server], 메타데이터"
  - "시작 옵션 [SQL Server]"
  - "복원 [SQL Server], 다른 서버 인스턴스로"
  - "연결된 서버 [SQL Server], 메타데이터"
  - "서버 이벤트용 WMI 공급자, 메타데이터"
  - "데이터베이스 연결 [SQL Server]"
  - "로그 전달 [SQL Server], 메타데이터"
  - "암호화 [SQL Server], 메타데이터"
  - "서버 구성 [SQL Server]"
  - "분산 쿼리 [SQL Server], 메타데이터"
  - "확장 저장 프로시저 [SQL Server], 메타데이터"
  - "자격 증명 [SQL Server], 메타데이터"
  - "데이터베이스 복사"
...
##  <a name="user_defined_error_messages"></a> 사용자 정의 오류 메시지  
 사용자 정의 오류 메시지는 [sys.messages](../Topic/sys.messages%20\(Transact-SQL\).md) 카탈로그 뷰에 있습니다. 이 카탈로그 뷰는 **master**에 저장됩니다. 데이터베이스 응용 프로그램이 사용자 정의 오류 메시지를 사용하며 다른 서버 인스턴스에서 데이터베이스를 사용할 수 있는 경우 [sp_addmessage](../../relational-databases/system-stored-procedures/sp-addmessage-transact-sql.md)를 사용하여 대상 서버 인스턴스에서 이러한 사용자 정의 메시지를 추가합니다.  
...
> **중요!** [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] 대신 [CLR 통합](../Topic/Common%20Language%20Runtime%20\(CLR\)%20Integration%20Overview.md)을 사용하세요.  
