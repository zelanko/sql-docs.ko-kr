...
- **키 자격 증명 모음 중복**: 키 자료는 어떤 경우에도 Azure Key Vault를 벗어나지 않고 서버는 키 자격 증명 모음 외부의 캐시된 복사본에 액세스할 수 없으므로 Azure Key Vault 지역 복제를 구성해야만 한 Azure Key Vault 지역이 중단되더라도 키 자료에 대한 액세스가 유지됩니다.  지역 복제 데이터베이스는 단일 Azure Key Vault를 사용하는 경우 키 자료에 액세스할 수 없게 됩니다.
...
Azure Key Vault에서 키 자료의 고가용성을 유지할 수 있도록 키 자격 증명 모음에 대해 지역 복제를 구성해야 합니다.

- **중복 키 자격 증명 모음**: 각 지역 복제 서버는 별도의 키 자격 증명 모음에 액세스할 수 있으며, 동일한 Azure 지역에 공동 배치하는 것이 가장 이상적입니다. 각 서버에는 암호화되고 지리적 복제된 데이터베이스에 대한 TDE 보호기의 사본이 있으므로 권장되는 구성입니다. 서버의 Azure 지역 중 하나가 오프라인으로 전환되면 다른 서버는 지리적으로 복제된 데이터베이스에 계속 액세스할 수 있습니다.  한 키 자격 증명 모음을 사용할 수 없게 되면 서버가 다른 키 자격 증명 모음의 TDE 보호기 백업에 액세스할 수 있도록 신중하게 구성해야 합니다.     
