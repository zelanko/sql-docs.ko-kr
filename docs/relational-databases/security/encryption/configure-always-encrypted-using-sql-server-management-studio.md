...
*예제* `SSN`이 `Patients` 테이블의 암호화된 `char(11)` 열이라고 가정할 경우, 아래 스크립트는 Always Encrypted가 데이터베이스 연결에 사용되고, Always Encrypted에 대한 매개 변수화가 쿼리 편집기 창에 사용하도록 설정되며, `SSN` 열에 대해 구성된 열 마스터 키에 대한 액세스 권한이 있는 경우 SSN 열에서 `'795-73-9838'`이 포함된 행을 찾아서 `LastName` 열의 값을 반환합니다.   
...
데이터베이스 연결에 Always Encrypted를 사용하지 않도록 설정하려면  **서버에 연결** 대화 상자의 **추가 속성** 탭에서 **열 암호화 설정**의 설정을 제거하거나 `Column Encryption Setting=Disabled`를 지정합니다(기본값은 **사용 안 함**).   
...
#### <a name="permissions-for-querying-against-encrypted-columns"></a>암호화된 열에 대해 쿼리할 수 있는 권한
...
### <a name="permissions-for-dac-upgrade-if-always-encrypted-is-set-up"></a>Always Encrypted가 설정되어 있는 경우, DAC 업그레이드에 대한 권한

DACPAC 또는 대상 데이터베이스에서 Always Encrypted가 설정된 경우 DAC 업그레이드 작업을 수행하려면 DACPAC의 스키마와 대상 데이터베이스 스키마 간의 차이에 따라 아래 사용 권한 중 일부 또는 모두가 필요할 수 있습니다.
...
### <a name="permissions-for-database-migration-using-sql-server-import-and-export-wizard"></a>SQL Server 가져오기 및 내보내기 마법사를 사용한 데이터베이스 마이그레이션에 대한 권한
