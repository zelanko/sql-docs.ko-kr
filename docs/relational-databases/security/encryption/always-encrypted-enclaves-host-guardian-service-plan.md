---
title: 호스트 보호자 서비스 증명 계획 | Microsoft Docs
ms.custom: ''
ms.date: 10/12/2019
ms.prod: sql
ms.reviewer: vanto
...
[!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)]를 실행하는 컴퓨터는 도메인에 가입할 필요가 없지만, 이 경우에는 HGS 서버에서 사용하는 것과 다른 도메인이어야 합니다.
...
HGS 노드 사이에는 공유 저장소가 필요하지 않습니다. 증명 데이터베이스의 복사본은 각 HGS 서버에 저장되며 클러스터 서비스에서 네트워크를 통해 자동으로 복제합니다.

### <a name="network-connectivity"></a>네트워크 연결

SQL 클라이언트와 [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] 모두 HTTP를 통해 HGS와 통신할 수 있어야 합니다. 필요에 따라 TLS 인증서를 사용하여 HGS를 구성하여 암호화된 HTTPS 연결을 지원할 수 있습니다.
...
UEFI가 합법적인 Microsoft 서명 부팅 로더를 로드하고 루트킷 또는 부트킷이 하이퍼바이저 부팅 프로세스를 가로채지 않도록 TPM 증명에 보안 부팅이 필요합니다.
...
### <a name="hgs-server-prerequisites"></a>HGS Server 필수 구성 요소
...
| RAM | 8GB(최소) |
| NIC | 2개의 NIC 권장(클러스터 트래픽용 1개, HGS 서비스용 1개) |
