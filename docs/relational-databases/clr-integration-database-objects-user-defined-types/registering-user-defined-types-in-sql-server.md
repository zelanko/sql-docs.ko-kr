---
title: SQL Server의 사용자 정의 형식 등록 | Microsoft Docs
ms.custom: ''
ms.date: 03/16/2017
ms.prod: sql
ms.reviewer: ''
...
#### <a name="example"></a>예제  
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 에 Point 어셈블리를 등록 하는 문을 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에 **AdventureWorks** SAFE 권한 집합을 사용 하 여 데이터베이스입니다. WITH PERMISSION_SET 절을 생략하면 어셈블리가 SAFE 권한 집합을 사용하여 등록됩니다.  
...
```  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문을 사용 하 여 어셈블리를 등록 *< assembly_bits >* FROM 절에는 인수입니다. 이렇게 **varbinary** 값 파일을 바이트 스트림으로 나타냅니다.  
...
> [!NOTE]  
>  CREATE TYPE 구문은 네이티브 작성용 에서도 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 별칭 데이터 형식 및 대체 하기 위한 **sp_addtype** 별칭 데이터 형식을 만드는 수단으로 합니다. CREATE TYPE 구문의 일부 선택적 인수는 UDT 만들기와 관련이 있으며 기본 유형과 같은 별칭 데이터 형식을 만드는 데 사용할 수 없습니다.  
  
 자세한 내용은 [CREATE TYPE &#40;TRANSACT-SQL&#41;](../../t-sql/statements/create-type-transact-sql.md)합니다.  
  
#### <a name="example"></a>예제  
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문은 만듭니다 합니다 **지점** 형식입니다. EXTERNAL NAME은 두 부분으로 구성 된 명명 구문을 사용 하 여 지정 *AssemblyName*. *UDTName*합니다.  
...
### <a name="example"></a>예제  
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)]은 다음과 같은 순서로 실행해야 합니다. 첫 번째 참조 하는 테이블의 **지점** 형식은, 및 마지막으로 어셈블리 UDT 삭제할 수 있어야 합니다.  
...
### <a name="finding-udt-dependencies"></a>UDT 종속성 찾기  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 쿼리 매개 변수에서 UDT를 사용 하는 열을 모두 찾습니다 합니다 **AdventureWorks** 데이터베이스입니다.  
...
### <a name="example"></a>예제  
 UDT 어셈블리의 원본 코드를 변경하여 다시 컴파일한 후에는 ALTER ASSEMBLY 문이 사용됩니다. 이 문은 .dll 파일을 서버에 복사하고 새 어셈블리에 다시 바인딩합니다. 전체 구문의 경우, 참조 [ALTER ASSEMBLY &#40;TRANSACT-SQL&#41;](../../t-sql/statements/alter-assembly-transact-sql.md)합니다.  
...
### <a name="using-alter-assembly-to-add-source-code"></a>ALTER ASSEMBLY를 사용하여 원본 코드 추가  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] ALTER ASSEMBLY 문은 Point.cs 클래스 원본 코드를 추가 합니다 **지점** UDT입니다. 이 문은 Point.cs 파일에 있는 텍스트를 복사하여 "PointSource"라는 이름으로 데이터베이스에 저장합니다.  
 ...
 **file_id**  
 연결 된 첫 번째 개체를 사용 하 여 각 개체를 식별 하는 번호를 지정 **assembly_id** 1의 값입니다. 같은 연결 된 개체가 여러 개 있으면 **assembly_id**, 한 다음 각 후속 **file_id** 값이 1 씩 증가 합니다.  
  
 **content**  
 어셈블리 또는 파일의 16진수 표현입니다.  
  
 콘텐츠를 변환 하려면 CAST 또는 CONVERT 함수를 사용할 수 있습니다 합니다 **콘텐츠** 읽기 쉬운 텍스트로 열입니다. 다음 쿼리는 WHERE 절에 이름을 사용하여 결과 집합을 단일 행으로 제한함으로써 Point.cs 파일의 내용을 읽기 쉬운 텍스트로 변환합니다.  
...
### <a name="the-currency-udt-and-currency-conversion-function"></a>Currency UDT 및 통화 변환 함수  
 합니다 **통화** 에서 UDT를 **AdventureWorks** 샘플 데이터베이스는 권장 되는 UDT 및 관련된 함수 구성 방법에 대 한 유용한 예제를 제공 합니다. 합니다 **통화** UDT는 특정 culture의 화폐 시스템에 따라 금액을 처리에 사용 되며 달러, 유로 등 다양 한 통화 유형 저장할 수 있습니다. UDT 클래스는 문자열과으로 금액으로 문화권 이름을 노출 된 **10 진수** 데이터 형식입니다. 필요한 모든 직렬화 메서드는 클래스를 정의하는 어셈블리에 들어 있습니다. 다른 특정 문화권에서의 통화 변환을 구현 하는 함수 라는 외부 함수로 구현 됩니다 **ConvertCurrency**,이 함수는 별도 어셈블리에 있습니다. 합니다 **ConvertCurrency** 함수는 테이블에서 환율을 검색 하 여 해당 작업을 수행 합니다 **AdventureWorks** 데이터베이스입니다. 전환율의 소스를 변경 해야 하거나 영향을 주지 않고 어셈블리를 쉽게 수정할 수 있는 다른 기존 코드를 변경 해야 합니다는 **통화** UDT입니다.  
...
-   한 데이터베이스 테이블의 UDT 열에서 UDT 데이터를 선택하여 동일한 UDT 열이 있는 다른 한 데이터베이스에 삽입  
 ...
 Udt를 사용 하 여에 대 한 어떤 조치도 취할 필요가 없습니다 참고 때 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)] 에 작업 테이블을 만듭니다는 **tempdb** 시스템 데이터베이스입니다. 여기에 커서, 테이블 변수를 처리 하 고 사용자 정의 테이블 반환 함수 Udt를 포함 하는 투명 하 게 이용 **tempdb**합니다. 그러나의 임시 테이블을 명시적으로 만들면 **tempdb** UDT 열을 정의 하는 다음에 UDT를 등록 합니다 **tempdb** 사용자 데이터베이스를 동일 하 게 합니다.  
