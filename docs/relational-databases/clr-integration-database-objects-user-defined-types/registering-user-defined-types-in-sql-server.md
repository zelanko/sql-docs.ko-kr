---
title: SQL Server에서 사용자 정의 형식 등록 | 마이크로 소프트 문서
description: SQL Server에 설치하기 전에 UDT를 등록해야 합니다. 어셈블리를 등록하고 사용할 데이터베이스에 형식을 만들어야 합니다.
ms.custom: ''
ms.date: 03/16/2017
ms.prod: sql
ms.reviewer: ''
...
1.  **시각적 기본** 또는 **Visual C#** 언어 노드에서 새 **데이터베이스** 프로젝트를 만듭니다.  
...
5.  **빌드** 메뉴에서 **배포를**선택합니다. 그러면 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터베이스에 어셈블리가 등록되고 형식이 만들어집니다.  

## <a name="using-transact-sql-to-deploy-udts"></a>Transact-SQL을 사용하여 UDT 배포  
 UDT를 사용할 데이터베이스에 어셈블리를 등록할 때는 [!INCLUDE[tsql](../../includes/tsql-md.md)] CREATE ASSEMBLY 구문을 사용합니다. 등록된 어셈블리는 파일 시스템에 외부적으로 저장되는 것이 아니라 데이터베이스 시스템 테이블에 내부적으로 저장됩니다. UDT가 외부 어셈블리에 종속되어 있는 경우 해당 어셈블리도 데이터베이스에 로드해야 합니다. UDT를 사용할 데이터베이스에 UDT를 만들 때는 CREATE TYPE 문을 사용합니다. 자세한 내용은 거래 SQL&#41;및 유형 만들기 &#40;[거래-SQL&#41;&#40;](../../t-sql/statements/create-type-transact-sql.md) [어셈블리 만들기를](../../t-sql/statements/create-assembly-transact-sql.md) 참조하십시오.  
...
#### <a name="example"></a>예제  
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문은 **AdventureWorks** 데이터베이스에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 안전 권한 집합을 사용하여 포인트 어셈블리를 등록합니다. WITH PERMISSION_SET 절을 생략하면 어셈블리가 SAFE 권한 집합을 사용하여 등록됩니다.  
...
```  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문은 from 절에서 *<assembly_bits>* 인수를 사용하여 어셈블리를 등록합니다. 이 **varbinary** 값은 파일을 바이트 스트림으로 나타냅니다.  
...
> [!NOTE]  
>  CREATE TYPE 구문은 네이티브 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 별칭 데이터 형식을 만드는 데도 사용되며 **sp_addtype** 별칭 데이터 형식을 만드는 수단으로 대체하기 위한 것입니다. CREATE TYPE 구문의 일부 선택적 인수는 UDT 만들기와 관련이 있으며 기본 유형과 같은 별칭 데이터 형식을 만드는 데 사용할 수 없습니다.  
  
 자세한 내용은 [유형 만들기 &#40;거래-SQL&#41;](../../t-sql/statements/create-type-transact-sql.md)를 참조하십시오.  
  
#### <a name="example"></a>예제  
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문은 **Point** 유형을 만듭니다. 외부 이름은 *AssemblyName*의 두 부분으로 구성된 명명 구문을 사용하여 지정됩니다. *UDTName*.  
...
### <a name="example"></a>예제  
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)]은 다음과 같은 순서로 실행해야 합니다. 먼저 **점** UDT를 참조하는 테이블을 삭제한 다음 형식을 삭제한 다음 마지막으로 어셈블리를 삭제해야 합니다.  
...
### <a name="finding-udt-dependencies"></a>UDT 종속성 찾기  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 쿼리는 **AdventureWorks** 데이터베이스에서 UDT를 사용하는 모든 열과 매개 변수를 찾습니다.  
...
### <a name="example"></a>예제  
 UDT 어셈블리의 원본 코드를 변경하여 다시 컴파일한 후에는 ALTER ASSEMBLY 문이 사용됩니다. 이 문은 .dll 파일을 서버에 복사하고 새 어셈블리에 다시 바인딩합니다. 전체 구문은 ALTER [어셈블리 &#40;거래-SQL&#41;](../../t-sql/statements/alter-assembly-transact-sql.md)을 참조하십시오.  
...
### <a name="using-alter-assembly-to-add-source-code"></a>ALTER ASSEMBLY를 사용하여 원본 코드 추가  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] ALTER ASSEMBLY 문은 **point** UDT에 대한 Point.cs 클래스 소스 코드를 추가합니다. 이 문은 Point.cs 파일에 있는 텍스트를 복사하여 "PointSource"라는 이름으로 데이터베이스에 저장합니다.  
 ...
 **file_id**  
 지정된 **assembly_id** 연결된 첫 번째 개체가 1값을 지정하는 각 개체를 식별하는 숫자입니다. 동일한 **assembly_id**연결된 개체가 여러 개 있는 경우 이후의 각 **file_id** 값은 1씩 증가합니다.  
  
 **콘텐츠**  
 어셈블리 또는 파일의 16진수 표현입니다.  
  
 CAST 또는 CONVERT 함수를 사용하여 **콘텐츠** 열의 내용을 읽을 수 있는 텍스트로 변환할 수 있습니다. 다음 쿼리는 WHERE 절에 이름을 사용하여 결과 집합을 단일 행으로 제한함으로써 Point.cs 파일의 내용을 읽기 쉬운 텍스트로 변환합니다.  
 ...
 **AdventureWorks** 샘플 데이터베이스의 **통화** UDT는 UDT 및 관련 함수를 구성하는 데 권장되는 방법의 유용한 예제를 제공합니다. **통화** UDT는 특정 문화의 통화 시스템을 기반으로 돈을 처리하는 데 사용되며 달러, 유로 등과 같은 다양한 통화 유형을 저장 합니다. UDT 클래스는 문화권 이름을 문자열로 노출하고 금액이 **소수점** 데이터 유형으로 노출됩니다. 필요한 모든 직렬화 메서드는 클래스를 정의하는 어셈블리에 들어 있습니다. 한 문화에서 다른 문화로 통화 변환을 구현하는 함수는 **ConvertCurrency라는**외부 함수로 구현되며 이 함수는 별도의 어셈블리에 있습니다. **ConvertCurrency** 함수는 AdventureWorks 데이터베이스의 테이블에서 변환 속도를 검색하여 작업을 **수행합니다.** 전환율의 출처가 변경되거나 기존 코드가 변경되어야 하는 경우 **통화** UDT에 영향을 주지 않고 어셈블리를 쉽게 수정할 수 있습니다.  
  
 **통화** UDT 및 **ConvertCurrency** 함수에 대한 코드 목록은 공통 언어 런타임(CLR) 샘플을 설치하여 찾을 수 있습니다.  
...
-   한 데이터베이스 테이블의 UDT 열에서 UDT 데이터를 선택하여 동일한 UDT 열이 있는 다른 한 데이터베이스에 삽입  
 ...
 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)] **tempdb** 시스템 데이터베이스에서 작업 테이블을 만들 때 UDT를 사용하기 위한 작업을 수행할 필요가 없습니다. 여기에는 UDT를 포함하고 **tempdb를**투명하게 사용하는 커서, 테이블 변수 및 사용자 정의 테이블 값 함수의 처리가 포함됩니다. 그러나 UDT 열을 정의하는 **tempdb에** 임시 테이블을 명시적으로 만드는 경우 UDT는 사용자 데이터베이스와 동일한 방식으로 **tempdb에** 등록되어야 합니다.  
