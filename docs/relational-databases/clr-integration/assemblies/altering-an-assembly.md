---
title: 어셈블리 변경 | 마이크로 소프트 문서
description: ALTER 어셈블리를 사용하여 SQL Server에 등록된 어셈블리를 업데이트합니다. 사용 권한 집합을 변경하고 어셈블리에 대한 소스 코드 또는 기타 파일을 추가할 수도 있습니다.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
```  
 ...
 어셈블리의 사용 권한 집합이 **SAFE에서** **EXTERNAL_ACCESS** 또는 **안전하지 않음으로**변경되는 경우 **외부 액세스 어셈블리** 권한 또는 어셈블리에 대한 **안전하지 않은 어셈블리** 권한이 있는 비대칭 키 및 해당 로그인을 먼저 만들어야 합니다. 자세한 내용은 [Creating an Assembly](../../../relational-databases/clr-integration/assemblies/creating-an-assembly.md)을 참조하세요.  
