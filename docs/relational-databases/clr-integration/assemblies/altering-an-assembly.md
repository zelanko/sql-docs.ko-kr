...
description: ALTER ASSEMBLY를 사용 하 여 SQL Server에 등록 된 어셈블리를 업데이트 합니다. 권한 집합을 변경 하 고 어셈블리에 대 한 소스 코드나 기타 파일을 추가할 수도 있습니다.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
```  
 ...
 어셈블리의 권한 집합을 **SAFE** 에서 **EXTERNAL_ACCESS** 또는 **unsafe**로 변경 하는 경우에는 해당 어셈블리에 대 한 **외부 액세스 어셈블리** 권한 또는 **안전 하지 않은 어셈블리** 권한을 사용 하 여 비대칭 키와 해당 로그인을 먼저 만들어야 합니다. 자세한 내용은 [Creating an Assembly](../../../relational-databases/clr-integration/assemblies/creating-an-assembly.md)을 참조하세요.  
