...
1.  **빌드 메뉴에서** **빌드** \<프로젝트 이름 >를 선택 하 여 프로젝트를 빌드합니다.  
...
4.  [!INCLUDE[msCoName](../../includes/msconame-md.md)] Visual Basic 원본 파일의 경우 다음 명령을 실행합니다.  
 ...
 이러한 명령은/target 옵션을 C# 사용 하 여 Visual 또는 Visual Basic 컴파일러를 시작 하 여 라이브러리 DLL 빌드를 지정 합니다.  
...
1.  그런 다음 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]의 인스턴스에서 프로시저, 함수, 집계, 사용자 정의 형식 또는 트리거를 만들어야 합니다. **Helloworld** 어셈블리에 **프로시저** 클래스의 **helloworld** 라는 메서드가 포함 되어 있는 경우에는 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)]을 쿼리에 추가 하 여 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 **hello** 라는 프로시저를 만들 수 있습니다.  
