---
title: 어셈블리 설계 | 마이크로 소프트 문서
description: 이 문서에서는 어셈블리의 패키징, 관리 및 제한을 포함하여 SQL Server에서 호스트할 어셈블리를 디자인할 때 고려해야 할 요소에 대해 설명합니다.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
-   CLR 사용자 정의 함수에 의존하는 CLR 사용자 정의 유형 및 인덱스는 영구 데이터가 어셈블리에 의존하는 데이터베이스에 포함되도록 만들 수 있습니다. 어셈블리의 코드를 수정하면 데이터베이스에 있는 어셈블리에 의존하는 영구 데이터가 있는 경우 보다 복잡해지는 경우가 많습니다. 따라서 일반적으로 사용자 정의 유형과 사용자 정의 함수를 사용하는 인덱스와 같이 영구 데이터 종속 관계에 의존하는 코드를 이러한 영구 데이터 종속 관계가 없는 코드와 구분하는 것이 좋습니다. 자세한 내용은 [어셈블리 구현](../../relational-databases/clr-integration/assemblies-implementing.md) 및 [어셈블리 변경 &#40;Transact-SQL&#41;](../../t-sql/statements/alter-assembly-transact-sql.md)을 참조하십시오.  
...
## <a name="managing-assembly-security"></a>어셈블리 보안 관리  
 관리 코드를 실행할 때 .NET Code Access Security로 보호되는 리소스를 어셈블리에서 어느 정도까지 액세스할 수 있는지를 제어할 수 있습니다. 어셈블리를 만들거나 수정할 때 안전, EXTERNAL_ACCESS 또는 안전하지 않음이라는 세 가지 권한 집합 중 하나를 지정하여 이 작업을 수행합니다.  
...
### <a name="disallowed-net-framework-apis"></a>허용되지 않는 .NET Framework API  
 허용되지 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 않는 **HostProtectionAttributes** 중 하나에 추가된 모든 API는 SAFE 및 EXTERNAL_ACCESS 어셈블리에서 호출할 수 없습니다.  
