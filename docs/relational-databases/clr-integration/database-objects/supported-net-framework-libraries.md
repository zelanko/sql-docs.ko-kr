---
title: 지원되는 .NET 프레임워크 라이브러리 | 마이크로 소프트 문서
description: SQL Server에서 호스팅되는 CLR을 사용하면 지원되는 .NET Framework 클래스 라이브러리 및 데이터베이스에 등록하는 지원되지 않는 라이브러리를 사용하여 작성할 수 있습니다.
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
ms.reviewer: ''
...
## <a name="unsupported-libraries"></a>지원되지 않는 라이브러리  
 지원되지 않는 라이브러리도 관리되는 저장 프로시저, 트리거, 사용자 정의 함수, 사용자 정의 형식 및 사용자 정의 집계에서 호출할 수 있습니다. 지원되지 않는 라이브러리는 먼저 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] CREATE **ASSEMBLY** 문을 사용하여 데이터베이스에 등록되어야 코드에서 사용할 수 있습니다. 지원되지 않는 라이브러리를 서버에 등록하고 실행하는 경우 보안과 안정성을 검토하여 테스트해야 합니다.  
  
 예를 들어 **System.DirectoryServices** 네임스페이스는 지원되지 않습니다. 코드에서 호출하려면 **안전하지 않은** 권한으로 System.DirectoryServices.dll 어셈블리를 등록해야 합니다. **System.DirectoryServices** 네임스페이스의 클래스가 **SAFE** 또는 **EXTERNAL_ACCESS**에 대한 요구 사항을 충족하지 않기 때문에 **안전하지 않은** 권한이 필요합니다. 자세한 내용은 [CLR 통합 프로그래밍 모델 제한](../../../relational-databases/clr-integration/database-objects/clr-integration-programming-model-restrictions.md) 및 [CLR 통합 코드 액세스 보안을](../../../relational-databases/clr-integration/security/clr-integration-code-access-security.md)참조하십시오.  
  
## <a name="see-also"></a>참고 항목  
 [어셈블리 작성](../../../relational-databases/clr-integration/assemblies/creating-an-assembly.md)   
