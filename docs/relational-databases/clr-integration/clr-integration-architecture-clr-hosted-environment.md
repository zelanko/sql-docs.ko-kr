...
###### <a name="scalability-common-threading-scheduling-and-synchronization"></a>확장성: 일반 스레딩, 일정 예약 및 동기화  
 ...
 계산만 포함 하는 근접 루프와 같이 위의 작업을 수행 하지 않는 코드는 자동으로 스케줄러를 생성 하지 않으며,이로 인해 시스템의 다른 워크 로드에 대 한 대기 시간이 길어질 수 있습니다. 이러한 상황에서 개발자는 .NET Framework의 System.object. Sleep () 함수를 호출 하거나, 다음 코드 섹션에서 preemtive 모드 ()를 명시적으로 입력 하 여 명시적으로 생성 하는 것이 좋습니다. 장기 실행 될 것으로 예상 됩니다. 다음 코드 예제에서는 이러한 각 메서드를 사용 하 여를 수동으로 생성 하는 방법을 보여 줍니다.
