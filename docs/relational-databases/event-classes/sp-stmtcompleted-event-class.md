...
|CPU|**ssNoversion**|이벤트에 의해 사용된 CPU 시간(밀리초)입니다.|18|예|  
...
|EventClass|**ssNoversion**|이벤트 유형 = 45|27|아니오|  
...
|IntegerData|**ssNoversion**|추적에서 캡처된 이벤트 클래스에 의존하는 정수 값입니다.|25|예|  
|IntegerData2|**ssNoversion**|실행 중인 문의 종료 오프셋(바이트)입니다.|55|예|  
|IsSystem|**ssNoversion**|이벤트가 시스템 프로세스에서 발생했는지 아니면 사용자 프로세스에서 발생했는지를 나타냅니다. 1 = 시스템, 0 = 사용자|60|예|  
|LineNumber|**ssNoversion**|실행 중인 문의 줄 번호입니다.|5|예|  
...
|Reads|**bigint**|이벤트 대신 서버에서 수행한 논리적 디스크 읽기 수입니다.|16|예|  
...
|Writes|**bigint**|이벤트 대신 서버에서 수행한 물리적 디스크 쓰기 수입니다.|17|예|  
