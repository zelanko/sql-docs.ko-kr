...
|Duration|**bigint**|쿼리가 필요한 메모리를 가져오기 위해 기다려야 했던 밀리초 단위의 시간입니다. EventSubClass = 1(쿼리 대기)일 경우에만 유효합니다.|13|yes|  
|Error|**int**|사용되지 않습니다.|31|yes|  
|EventClass|**int**|이벤트 유형 = 67|27|예|  
|EventSequence|**int**|추적 내의 지정된 이벤트 시퀀스입니다.|51|예|  
|EventSubClass|**int**|이벤트 하위 클래스의 유형입니다.<br /><br /> 1=쿼리 대기<br /><br /> 2=쿼리 제한 시간|21|yes|  
