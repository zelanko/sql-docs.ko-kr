...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]는 Service Broker가 메시지를 전달할 때 Broker:Forwarded Message Sent 이벤트를 생성합니다.  
...
|DBUserName|**nvarchar**|메시지를 보낸 서비스의 Broker 인스턴스 ID입니다.|40|아니요|  
|EventClass|**int**|캡처된 이벤트 클래스 유형입니다. Broker:Forwarded Message Sent의 경우 항상 139입니다.|27|아니요|  
...
|ObjectId|**int**|메시지가 전달될 때 전달되는 메시지에 대한 시간 제한 값입니다.|22|아니요|  
|ObjectName|**nvarchar**|전달된 메시지의 메시지 ID입니다.|34|아니요|  
|OwnerName|**nvarchar**|메시지가 전송되는 Broker 식별자입니다.|37|아니요|  
|RoleName|**nvarchar**|대화 핸들의 역할입니다. 유효한 값은 다음과 같습니다.<br /><br /> 시작자 대화를 시작한 Broker입니다.<br /><br /> 대상 대화의 대상이 되는 Broker입니다.|38|아니요|  
...
|성공|**int**|전달 프로세스 동안 사용된 시간입니다.|23|아니요|  
|TargetLoginName|**nvarchar**|이 인스턴스가 메시지를 보내는 네트워크 주소입니다. 이 주소는 해당 메시지의 최종 대상과 다를 수도 있습니다.|42|아니요|  
