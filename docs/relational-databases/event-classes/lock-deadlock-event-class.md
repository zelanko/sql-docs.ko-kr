...
|DatabaseName|**nvarchar**|잠금을 얻은 데이터베이스 이름입니다.|35|사용자 계정 컨트롤|  
|Duration|**bigint**|잠금 요청이 발생한 시간과 교착 상태가 발생한 시간 사이의 간격(마이크로초)입니다.|13|사용자 계정 컨트롤|  
|EndTime|**datetime**|교착 상태가 끝난 시간입니다.|15|사용자 계정 컨트롤|  
...
|IntegerData|**int**|교착 상태 번호입니다. 서버가 시작되면 번호가 0부터 시작되어 교착 상태마다 증가합니다.|25|사용자 계정 컨트롤|  
...
|ObjectID|**int**|연결에 포함된 개체 ID입니다(사용 및 적용 가능한 경우).|22|사용자 계정 컨트롤|  
...
|TextData|**ntext**|획득한 잠금 유형에 따라 달라지는 텍스트 값입니다.|@shouldalert|사용자 계정 컨트롤|  
