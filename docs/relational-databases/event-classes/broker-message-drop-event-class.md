...
|**EventSubClass**|**nvarchar**|배달할 수 없는 메시지가 순차화된 메시지인지 여부를 나타냅니다. 다음 두 값 중 하나입니다.<br /><br /> **순차화된 메시지**. 배달할 수 없는 메시지가 순차화된 메시지입니다.<br /><br /> **순차화되지 않은 메시지**. 배달할 수 없는 메시지가 순차화된 메시지가 아닙니다.|21|yes|  
...
|**Severity**|**int**|이벤트 텍스트의 심각도 숫자입니다.|29|예|  
...
|**TextData**|**ntext**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]가 메시지를 배달할 수 없는 이유입니다.|1|yes|  
