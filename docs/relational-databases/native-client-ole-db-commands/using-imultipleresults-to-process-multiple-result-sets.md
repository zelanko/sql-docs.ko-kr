---
title: IMultipleResults, 여러 결과 세트
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
ms.locfileid: "81304455"
---
# <a name="using-imultipleresults-to-process-multiple-result-sets"></a>IMultipleResults를 사용하여 여러 결과 집합 처리
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

  소비자는 **IMultipleResults** 인터페이스를 사용하여 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 네이티브 클라이언트 OLE DB 공급자 명령 실행에서 반환된 결과를 처리합니다. 네이티브 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 클라이언트 OLE DB 공급자가 실행에 대한 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 명령을 제출하면 문을 실행하고 결과를 반환합니다.  
  
 클라이언트는 명령 실행을 통해 반환된 모든 결과를 처리해야 합니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 네이티브 클라이언트 OLE DB 공급자 명령 실행 결과 여러 행 집합 개체를 생성할 수 있으므로 **IMultipleResults** 인터페이스를 사용 하 여 응용 프로그램 데이터 검색 클라이언트 시작 된 왕복을 완료 합니다.  
...
```  
 ...
 소비자가 이 텍스트를 포함하는 명령을 실행하고 반환되는 결과 인터페이스로 행 집합을 요청하면 첫 번째 행 집합만 반환됩니다. 이 경우 소비자는 반환되는 행 집합에 있는 모든 행을 처리할 수 있습니다. 그러나 DBPROP_MULTIPLECONNECTIONS 데이터 원본 속성이 VARIANT_FALSE 설정되어 있고 MARS가 연결에서 활성화되지 않은 경우 명령이 취소될 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 때까지 세션 개체에서 다른 명령을 실행할 수 없습니다(네이티브 클라이언트 OLE DB 공급자는 다른 연결을 만들지 않음). 연결에서 MARS를 활성화하지 않으면 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 네이티브 클라이언트 OLE DB 공급자가 VARIANT_FALSE DBPROP_MULTIPLECONNECTIONS 경우 DB_E_OBJECTOPEN 오류를 반환하고 활성 트랜잭션이 있는 경우 E_FAIL 반환합니다.  
  
 네이티브 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 클라이언트 OLE DB 공급자는 스트리밍된 출력 매개 변수를 사용할 때 DB_E_OBJECTOPEN 반환하며 응용 프로그램은 **IMultipleResults::GetResults를** 호출하기 전에 반환된 모든 출력 매개 변수 값을 사용하지 않아 다음 결과 집합을 가져옵니다. MARS가 설정되어 있지 않은 상태에서 연결이 행 집합을 생성하지 않는 명령 또는 서버 커서가 아닌 행 집합을 생성하는 명령을 실행 중이고 DBPROP_MULTIPLECONNECTIONS 데이터 원본 속성이 VARIANT_TRUE로 설정되어 있는 경우, 트랜잭션이 오류를 반환하는 활성 상태가 아니면 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 공급자가 동시 명령 개체를 지원하기 위해 추가 연결을 만듭니다. 트랜잭션 및 잠금은 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]가 연결별로 관리합니다. 두 번째 연결이 생성되면 개별 연결의 명령이 잠금을 공유하지 않습니다. 한 명령이 다른 명령을 차단하지 않도록 다른 명령이 요청한 행의 잠금이 유지되도록 주의해야 합니다. MARS가 설정되어 있는 경우 연결에 여러 명령을 활성화할 수 있으며 명시적 트랜잭션이 사용 중인 경우 모든 명령이 공통 트랜잭션을 공유합니다.  
