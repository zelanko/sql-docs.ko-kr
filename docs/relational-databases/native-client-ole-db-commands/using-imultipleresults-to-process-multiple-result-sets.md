...
[!INCLUDE[SNAC_Deprecated](../../includes/snac-deprecated.md)]
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문은 여러 행 집합을 포함 하는 일부 행 데이터를 생성 합니다 **OrderDetails** 테이블 및 COMPUTE BY 절의 결과:  
...
```  
 ...
 소비자 명령을 취소할 수를 사용 하 여 [issabort:: Abort](../../relational-databases/native-client-ole-db-interfaces/issabort-abort-ole-db.md) 또는 명령 개체 및 파생된 행 집합에 대 한 모든 참조를 해제 합니다.  
  
 사용 하 여 **IMultipleResults** 모든 인스턴스에서 소비자가 명령 실행 하 여 생성 된 모든 행 집합을 가져올 수 있고 적절 하 게 명령 실행을 취소 하 고 사용에 대 한 세션 개체를 해제 하는 시기를 결정 하는 소비자 다른 명령입니다.  
  
> [!NOTE]  
>  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 커서를 사용할 때 명령을 실행하면 커서가 생성됩니다. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]는 커서 생성에 대한 성공이나 실패를 반환하므로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스에 대한 왕복은 명령 실행 결과가 반환되면 완료됩니다. 각 **GetNextRows** 호출 왕복 됩니다. 따라서 여러 활성 명령 개체가 존재할 수 있으며, 이러한 각각의 개체가 서버 커서에서 인출한 결과 행 집합을 처리할 수 있습니다. 자세한 내용은 [행 집합 및 SQL Server 커서](../../relational-databases/native-client-ole-db-rowsets/rowsets-and-sql-server-cursors.md)합니다.  
