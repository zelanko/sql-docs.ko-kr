---
title: "기술 참조(데이터베이스 엔진) | Microsoft 문서"
ms.custom: 
ms.date: 03/01/2017
ms.prod: sql-server-2016
ms.reviewer: 
ms.suite: 
ms.technology:
- database-engine
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- technical reference [SQL Server Database Engine]
- Database Engine [SQL Server], getting assistance
ms.assetid: e9533f6b-c48a-4f53-a7a0-379e092bb667
caps.latest.revision: 32
author: BYHAM
ms.author: rickbyh
manager: jhubbard
translationtype: Human Translation
ms.sourcegitcommit: f3481fcc2bb74eaf93182e6cc58f5a06666e10f4
ms.openlocfilehash: 9149c0ca25b016449a1fb9f6315ae4acda691420
ms.lasthandoff: 04/11/2017

---
# <a name="technical-reference-database-engine"></a>기술 참조(데이터베이스 엔진)
  [!INCLUDE[ssDE](../includes/ssde-md.md)] 의 [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)]에 대한 도구, 언어 및 관리 프로그래밍 참조 설명서를 찾습니다.  
  
 ![작은 파일 폴더 아이콘](../analysis-services/media/filefolder-small.png "Small File Folder Icon") **기능 참조**  
 [등록된 서버 F1 도움말](../tools/sql-server-management-studio/registered-servers-f1-help.md)  
  
 [SQL Server Management Studio 메뉴 도움말](http://msdn.microsoft.com/library/5b8b2e45-bee6-42c8-93d6-31432f4d5699)  
  
 [Visual Database Tools F1 도움말](http://msdn.microsoft.com/library/79d15b47-6759-4f48-b9ac-2c86a3cd9980)  
  
 ![작은 파일 폴더 아이콘](../analysis-services/media/filefolder-small.png "Small File Folder Icon") **명령 프롬프트 유틸리티 참조**  
 [bcp 유틸리티](../tools/bcp-utility.md)  
  
 [dta 유틸리티](../tools/dta/dta-utility.md)  
  
 [SqlLocalDB 유틸리티](../tools/sqllocaldb-utility.md)  
  
 [자세히...](../tools/command-prompt-utility-reference-database-engine.md)  
  
 ![작은 파일 폴더 아이콘](../analysis-services/media/filefolder-small.png "Small File Folder Icon") **데이터베이스 엔진 PowerShell Cmdlet**  
 [Invoke-PolicyEvaluation cmdlet](../powershell/invoke-policyevaluation-cmdlet.md)  
  
 [Invoke-Sqlcmd cmdlet](../powershell/invoke-sqlcmd-cmdlet.md)  
  
 ![작은 파일 폴더 아이콘](../analysis-services/media/filefolder-small.png "Small File Folder Icon") **오류 및 이벤트 참조**  
 [데이터베이스 엔진 이벤트 및 오류](../relational-databases/errors-events/database-engine-events-and-errors.md)  
  
 ![작은 파일 폴더 아이콘](../analysis-services/media/filefolder-small.png "Small File Folder Icon") **실행 계획 논리 및 물리 연산자 참조**  
 [실행 계획 논리 및 물리 연산자 참조](../relational-databases/showplan-logical-and-physical-operators-reference.md)  
  
  
