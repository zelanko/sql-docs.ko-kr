---
title: 디스크 IO 하위 시스템에서 읽기 다시 시도 문제 확인 - 정책 기반 관리
description: 이 규칙은 이벤트 로그에서 SQL Server 오류 메시지 825를 확인합니다. 이 메시지는 SQL Server가 디스크에서 데이터를 읽으려는 첫 번째 시도에 실패했음을 나타냅니다.
