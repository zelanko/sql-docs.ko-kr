...
## <a name="remarks"></a>설명  
 *$Arg* 지정 하지 않으면 double로 변환 된 컨텍스트 노드의 숫자 값이 반환 됩니다. SQL Server에서 인수가 없는 **fn: number ()** 는 컨텍스트 종속 조건자의 컨텍스트에서만 사용할 수 있습니다. 특히 사용 시 대괄호([])로 묶어야 합니다. 예를 들어 다음 식은 <`ROOT`> 요소를 반환 합니다.  
