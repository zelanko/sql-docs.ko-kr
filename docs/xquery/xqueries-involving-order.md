...
ms.locfileid: "33077140"
---
# <a name="xqueries-involving-order"></a>정렬 포함 XQuery
[!INCLUDE[tsql-appliesto-ss2012-xxxx-xxxx-xxx-md](../includes/tsql-appliesto-ss2012-xxxx-xxxx-xxx-md.md)]

  관계형 데이터베이스에는 시퀀스 개념이 없습니다. 예를 들어 "데이터베이스에서 첫 번째 고객 가져오기"와 같은 요청은 수행할 수 없습니다. 그러나 XML 문서를 쿼리 하 고 첫 번째 검색 수 \<고객 > 요소입니다. 그런 다음에는 항상 같은 고객을 검색할 수 있습니다.  
...
-   중괄호 안에 있는 식은 해당 평가 결과로 대체됩니다. 자세한 내용은 참조 [XML 생성 &#40;XQuery&#41;](../xquery/xml-construction-xquery.md)합니다.  
  
-   **@\*** 모든 특성을 두 번째 작업 센터 위치를 검색합니다.  
  
-   FLWOR 반복문(FOR ... RETURN)은 두 번째 작업 센터 위치의 모든 <`step`> 자식 요소를 검색합니다.  
  
-   [: column () 함수 (XQuery)](../xquery/xquery-extension-functions-sql-column.md) 생성 되는 XML의 관계형 값이 포함 됩니다.  
...
```  
 ...
 이전 쿼리에서는 단지 텍스트 노드만 검색합니다. 전체 하려는 경우 <`step`> 대신, 반환 된 요소가 제거 된 **string ()** 쿼리에서 함수:  
...
-   쿼리 생성 된 < Loca`tion`> 요소와 검색 데이터베이스에서 해당 특성 값입니다.  
...
-   이 쿼리에서는 FOR ... RETURN 루프를 사용하여 제품 모델 기능 설명을 검색합니다. **position ()** 함수는 처음 두 기능을 검색 하는 데 사용 됩니다.  
...
### <a name="d-find-the-first-two-tools-used-at-the-first-work-center-location-in-the-manufacturing-process-of-the-product"></a>4. 제품 제조 프로세스의 첫 번째 작업 센터 위치에서 사용되는 처음 두 개의 도구 찾기  
 제품 모델에 대해 다음 쿼리는 제조 프로세스의 작업 센터 위치 시퀀스에 따라 첫 번째 작업 센터 위치에서 사용되는 처음 두 개의 도구를 반환합니다. 에 저장 된 제조 지침에 대해 쿼리가 지정 됩니다는 **지침** 의 열은 **Production.ProductModel** 테이블입니다.  
...
### <a name="e-find-the-last-two-manufacturing-steps-at-the-first-work-center-location-in-the-manufacturing-of-a-specific-product"></a>5. 특정 제품 제조의 첫 번째 작업 센터 위치에서 마지막 두 개의 제조 단계 찾기  
 쿼리에서 **last ()** 함수를 마지막 두 제조 단계 검색 합니다.  
