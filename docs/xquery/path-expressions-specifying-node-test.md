...
### <a name="a-results-of-specifying-the-node-name-and-the-node-type-as-node-tests-in-a-path-expression"></a>A. 경로 식에서 노드 테스트로 노드 이름과 노드 유형을 지정한 결과  
 다음 예에서는 간단한 XML 문서를 **xml** 유형 변수에 할당 합니다. 문서는 다른 경로 식을 사용하여 쿼리됩니다. 그런 다음 결과가 비교됩니다.  
...
-   XQuery 프롤로그의 `namespace` 키워드는 쿼리 본문에 사용되는 접두사를 정의합니다. XQuery 프롤로그에 대 한 자세한 내용은 [Xquery 프롤로그](../xquery/modules-and-prologs-xquery-prolog.md) 를 참조 하십시오.  
