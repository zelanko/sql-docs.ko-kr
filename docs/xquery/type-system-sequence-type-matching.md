...
### <a name="example-determining-whether-a-sequence-is-empty"></a>예: 빈 시퀀스 인지 여부를 결정 합니다.  
 합니다 **empty ()** 시퀀스 유형은 시퀀스 유형의 식에 지정 된 식이 반환 되는 시퀀스에 빈 시퀀스 인지 여부를 확인 합니다.  
  
 다음 예제에서는 XML 스키마를 통해 합니다 <`root`> 요소 nilled 수입니다.  
...
```  
 ...
 이제 값을 지정 하는 형식화 된 XML 인스턴스는 <`root`> 요소를 `instance of empty()` False를 반환 합니다.  
...
```  
 ...
 경우는 <`root`> 요소는 인스턴스의 nilled, 해당 값이 빈 시퀀스 및 `instance of empty()` True를 반환 합니다.  
...
### <a name="example-cardinality-in-sequence-expressions"></a>예: 시퀀스 식의 카디널리티  
 이 예에서는 시퀀스 식의 카디널리티 효과를 보여 줍니다. 다음 XML 스키마 정의 <`root`>는 바이트 유형이 고 nillable 인 요소입니다.  
...
```  
 ...
 경우는 <`root`> 요소 nil 값이 빈 시퀀스입니다. 즉, `/root[1]` 식은 빈 시퀀스를 반환합니다. 따라서 `instance of xs:byte`는 False를 반환합니다. 이 경우의 기본 카디널리티는 1입니다.  
...
```  
 ...
 첫 번째 쿼리에서 식은 반환 요소의 형식화 된 값 <`a`>. 두 번째 쿼리 식은 요소를 반환 합니다. <`a`>. 두 개 모두 항목입니다. 따라서 두 쿼리는 모두 True를 반환합니다.  
...
```  
 ...
 다음 세 가지 쿼리에서 모든 XQuery 식은 요소 노드 자식 중 반환 된 <`root`> 요소입니다. 따라서 시퀀스 유형의 식인 `instance of node()`는 True를 반환하고 다른 두 개의 식인 `instance of text()` 및 `instance of document-node()`는 False를 반환합니다.  
...
```  
 ...
 다음 쿼리에서 `instance of document-node()` 식은 True를 반환 합니다의 부모를 <`root`> 요소는 문서 노드입니다.  
...
### <a name="example-a"></a>예 1  
 다음 XML 스키마 정의 `CustomerType` 복합 형식 여기서 <`firstName`> 및 <`lastName`> 요소는 선택 사항입니다. 지정된 XML 인스턴스에서 특정 고객의 이름이 있는지 여부를 확인해야 할 수 있습니다.  
...
```  
 ...
 다음 쿼리에서 `instance of element (firstName)` 결정할 식 여부를 첫 번째 자식 요소 <`customer`> 요소 이름이 <`firstName`>. 이 경우에는 True가 반환됩니다.  
...
```  
 ...
 제거 하는 경우는 <`firstName`> 인스턴스에서 요소, 쿼리가 False를 반환 합니다.  
...
### <a name="example-b"></a>예 2  
 ...
 다음 예제에서는 두 <`Customer`> 쿼리 되는 XML 인스턴스 요소는 두 가지 형식의 `CustomerType` 고 `SpecialCustomerType`입니다. 형식의 알아야 한다고 가정 합니다 <`Customer`> 식에서 반환한 요소입니다. 다음 XML 스키마 컬렉션은 `CustomerType` 및 `SpecialCustomerType` 유형을 정의합니다.  
...
```  
 ...
 이 XML 스키마 컬렉션은 형식화 된 만드는 데 **xml** 변수입니다. 이 변수에 할당 된 XML 인스턴스에 있는 두 개의 <`customer`> 라는 두 가지 종류의 요소입니다. 첫 번째 요소는 `CustomerType` 유형이며 두 번째 요소는 `SpecialCustomerType` 유형입니다.  
...
```  
 ...
 이전 쿼리의 식을 변경 하 고 두 번째를 검색 하는 경우 <`customer`> 요소 (`/x:customer)[2]`), `instance of` 는 True를 반환 합니다.  
  
### <a name="example-c"></a>예 3  
 이 예에서는 특성 테스트가 사용됩니다. 다음 XML 스키마는 CustomerID 및 Age 특성이 포함된 CustomerType 복합 유형을 정의합니다. Age 특성은 선택 사항입니다. 특정 XML 인스턴스에 대해 Age 특성에 있는지 여부를 확인 하려는 합니다 <`customer`> 요소입니다.  
