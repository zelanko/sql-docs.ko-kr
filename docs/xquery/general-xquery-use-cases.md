...
description: 제품 카탈로그에서 특정 데이터 검색, 검색 및 나열을 포함 하는 일반적인 XQuery 사용의 몇 가지 예를 봅니다.
ms.custom: ''
ms.date: 03/07/2017
ms.prod: sql
ms.prod_service: sql
...
-   WHERE 절에서는 **exist ()** 메서드를 사용 하 여 제품 카탈로그 설명이 포함 된 행만 찾습니다. 즉, <> 요소를 포함 하는 XML입니다 `ProductDescription` .  
...
```  
 ...
 다음 쿼리는 동일한 정보를 검색 하지만 카탈로그 설명이 포함 된 해당 제품 모델에 대해서만 <> 요소에 가중치, <`Weight`> 요소를 포함 합니다 `Specifications` . 이 예에서는 WITH XMLNAMESPACES를 사용하여 pd 접두사와 해당 네임스페이스 바인딩을 선언합니다. 이러한 방식으로 **query ()** 메서드와 **exist ()** 메서드에서 바인딩이 설명 되지 않습니다.  
...
```  
 ...
 이전 쿼리에서 WHERE 절에 있는 **xml** 데이터 형식의 **exist ()** 메서드는 `Weight` <> 요소에 <> 요소가 있는지 확인 `Specifications` 합니다.  
...
-   WHERE 절에서는 **exist ()** 메서드를 사용 하 여 <> 요소가 포함 된 제품 카탈로그 설명이 있는 행만 검색 `Picture` 합니다.  
  
-   WHERE 절은 **value ()** 메서드를 두 번 사용 하 여 <`Size`> 및 <> 요소의 값을 비교 합니다 `Angle` .  
...
### <a name="c-create-a-flat-list-of-the-product-model-name-and-feature-pairs-with-each-pair-enclosed-in-the-features-element"></a>C. 각 쌍이 요소에 포함 된 제품 모델 이름 및 기능 쌍의 단순 목록을 만듭니다. \<Features>  
 제품 모델 카탈로그 설명에서 XML에는 몇 가지 제품 기능이 포함됩니다. 이러한 모든 기능은 <> 요소에 포함 됩니다 `Features` . 쿼리에서 [Xml 생성 (XQuery)](../xquery/xml-construction-xquery.md) 을 사용 하 여 필요한 xml을 생성 합니다. 중괄호에 있는 식은 결과로 대체됩니다.  
...
-   $pd/p1: Features/*는 <>의 요소 노드 자식만 반환 `Features` 하지만 $pd/p1: features/node ()는 모든 노드를 반환 합니다. 여기에는 요소 노드, 텍스트 노드, 처리 명령 및 주석이 포함됩니다.  
...
### <a name="d-from-the-catalog-description-of-a-product-model-list-the-product-model-name-model-id-and-features-grouped-inside-a-product-element"></a>D. 제품 모델의 카탈로그 설명에서 제품 모델 이름, 모델 ID 및 요소 내에 그룹화 된 기능을 나열 합니다. \<Product>  
 다음 쿼리는 제품 모델의 카탈로그 설명에 저장 된 정보를 사용 하 여 요소 내에 그룹화 된 제품 모델 이름, 모델 ID 및 기능을 나열 합니다 \<Product> .  
...
### <a name="e-retrieve-product-model-feature-descriptions"></a>E. 제품 모델 기능 설명 검색  
 다음 쿼리는 `Product` **ProducModelID**, 제품 **modelname** 특성 및 처음 두 제품 기능을 포함 하는 <> 요소를 포함 하는 XML을 생성 합니다. 특히 처음 두 제품 기능은 <> 요소의 처음 두 자식 요소입니다 `Features` . 더 많은 기능이 있는 경우 빈 <> 요소를 반환 `There-is-more/` 합니다.  
...
### <a name="f-find-element-names-from-the-product-catalog-description-that-end-with-ons"></a>F. 제품 카탈로그 설명에서 "ons"로 끝나는 요소 이름 찾기  
 다음 쿼리는 카탈로그 설명을 검색 하 고 `ProductDescription` 이름이 "기능"으로 끝나는 <> 요소의 모든 요소를 반환 합니다.  
...
-   WHERE 절은 카탈로그 설명이 <> 요소에서 "Aerodynamic" 이라는 단어를 포함 하는 행만 검색 하는 데 사용 됩니다 `Summary` .  
...
### <a name="h-find-product-models-whose-catalog-descriptions-do-not-include-product-model-pictures"></a>H. 카탈로그 설명에 제품 모델 사진이 포함되지 않은 제품 모델 찾기  
 다음 쿼리는 카탈로그 설명에 <> 요소가 포함 되지 않은 제품 모델에 대 한 제품 모드 뚜껑을 검색 합니다 `Picture` .  
...
-   모든 제품 설명에는 <> 요소가 포함 되어 있으므로 `Picture` 이 경우 결과 집합이 비어 있습니다.  
