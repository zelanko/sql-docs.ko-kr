---
title: 비교 표현식(X쿼리) | 마이크로 소프트 문서
description: 일반, 값, 노드 및 노드 순서 비교 연산자가 포함된 XQuery 비교 식을 사용하는 방법을 알아봅니다.
ms.custom: ''
ms.date: 08/09/2016
ms.prod: sql
ms.prod_service: sql
...
```  
 ...
 다음 쿼리는 AdventureWorks 예제 데이터베이스에 제공된 제품 카탈로그의 제품 모델에 대한 크기가 작은 사진을 반환합니다. 이 쿼리는 `PD:ProductDescription/PD:Picture/PD:Size`에서 반환되는 원자 값 시퀀스와 단일 시퀀스 "small"을 비교합니다. 비교가 True이면 <Picture\> 요소를 반환합니다.  
...
```  
 ...
 다음 쿼리는 <숫자\> 요소의 전화 번호 시퀀스를 문자열 리터럴 "112-111-1111"과 비교합니다. 이 쿼리는 AdditionalContactInfo 열에 있는 전화 번호 요소의 시퀀스를 비교하여 특정 고객에 대한 특정 전화 번호가 문서에 있는지 확인합니다.  
...
```  
 ...
 쿼리에서 True를 반환합니다. 이는 번호가 문서에 있다는 의미입니다. 다음 쿼리는 위의 쿼리를 약간 수정한 버전입니다. 이 쿼리에서는 문서에서 검색한 전화 번호 값이 두 개의 전화 번호 값 시퀀스와 비교됩니다. 비교가 True이면 <\> 숫자 요소가 반환됩니다.  
...
|ge|크거나 같음|  
 ...
 예를 들어 다음 쿼리는 \<그림 크기가 "작음"인 제품 모델에 대한 그림> 요소를 검색합니다.  
...
-   \<크기> 요소 값은 지정된 원자값 "작은"과 비교됩니다.  
  
-   값 연산자는 원자성 값에서만 작동하므로 **data()** 함수는 노드 값을 검색하는 데 암시적으로 사용됩니다. 즉 `data($P/PD:Size) eq "small"`은 같은 결과를 생성합니다.  
...
## <a name="node-comparison-operator"></a>노드 비교 연산자  
 노드 비교 연산자는 **은**노드 유형에만 적용됩니다. 이 연산자가 반환하는 결과는 피연산자로 전달된 두 노드가 원본 문서의 같은 노드를 나타내는지 여부를 나타냅니다. 이 연산자는 두 피연산자가 같은 노드이면 True를 반환하고 그렇지 않으면 False를 반환합니다.  
...
-   `<<`: 문서 순서에서 **피연산자 1보다** 먼저 **피연산자 2를** 수행합니다.  
  
-   `>>`: 문서 순서대로 **파연산1이** **2를** 따릅니다.  
  
 다음 쿼리는 제품 카탈로그 설명에 \<특정 제품에 대한 \<문서 주문의 유지 관리> 요소 앞에 보증> 요소가 나타나는 경우 True를 반환합니다.  
...
-   **xml**데이터 형식의 **값()** 메서드가 쿼리에 사용됩니다.  
  
-   쿼리의 부울 결과가 **nvarchar(10)로** 변환되고 반환됩니다.  
