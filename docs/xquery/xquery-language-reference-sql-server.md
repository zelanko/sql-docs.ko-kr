---
title: X쿼리 언어 참조(SQL 서버) | 마이크로 소프트 문서
description: SQL Server에 대한 XQuery 언어에 대해 알아보고 전체 언어 참조를 봅니다.
ms.custom: ''
ms.date: 03/16/2017
ms.prod: sql
ms.prod_service: sql
...
[!INCLUDE[tsql-appliesto-ss2012-xxxx-xxxx-xxx-md](../includes/tsql-appliesto-ss2012-xxxx-xxxx-xxx-md.md)]
 ...
 자세한 내용은 [W3C XQuery 1.0 언어 사양을](https://go.microsoft.com/fwlink/?LinkId=48846)참조하십시오.  
  
 XQuery는 구조화되었거나 반구조화된 XML 데이터를 쿼리할 수 있는 언어입니다. 에 제공된 xml 데이터 형식 지원을 사용하면 문서를 데이터베이스에 저장한 다음 XQuery를 사용하여 쿼리할 수 있습니다. **xml** [!INCLUDE[ssDE](../includes/ssde-md.md)]  
 ...
 **xml** 형식의 변수 또는 열에 저장된 XML 인스턴스를 쿼리하려면 [xml 데이터 형식 메서드를](../t-sql/xml/xml-data-type-methods.md)사용합니다. 예를 들어 **xml** 형식의 변수를 선언하고 **xml** 데이터 형식의 **query() 메서드를** 사용하여 쿼리할 수 있습니다.  
...
```  
 ...
 다음 예제에서는 AdventureWorks 데이터베이스의 ProductModel 테이블에서 **xml** 형식의 지침 열에 대해 쿼리가 지정됩니다.  
...
```  
 ...
 XQuery에는 네임스페이스 선언 `declare namespace``AWMI=...`및 쿼리 `/AWMI:root/AWMI:Location[@LocationID=10]`식이 포함됩니다.  
  
 XMl 형식의 명령열에 대해 **xml** XQuery가 지정됩니다. xml 데이터 형식의 [쿼리() 메서드는](../t-sql/xml/query-method-xml-data-type.md) XQuery를 지정하는 데 사용됩니다.  
  
 다음 표에서는 [!INCLUDE[ssDE](../includes/ssde-md.md)]의 XQuery 구현 방식을 이해하는 데 도움이 될 수 있는 관련 항목들이 나열되어 있습니다.  
  
|항목|Description|  
|-----------|-----------------|  
|[XML 데이터&#40;SQL Server&#41;](../relational-databases/xml/xml-data-sql-server.md)|의 **xml**데이터 [!INCLUDE[ssDE](../includes/ssde-md.md)] 형식에 대한 지원과 이 데이터 형식에 대해 사용할 수 있는 방법에 대해 설명합니다. **xml** 데이터 형식은 XQuery 식이 실행되는 입력 XQuery 데이터 모델을 형성합니다.|  
|[XML 스키마 컬렉션&#40;SQL Server&#41;](../relational-databases/xml/xml-schema-collections-sql-server.md)|데이터베이스에 저장된 XML 인스턴스를 형식화하는 방법을 설명합니다. 즉, XML 스키마 컬렉션을 **xml** 형식 열과 연결할 수 있습니다. 열에 저장된 모든 항목은 유효성이 검사되고 컬렉션에 있는 스키마에 대해 형식화되며 XQuery에 대한 유형 정보를 제공합니다.|  
|||  
  
> [!NOTE]  
>  이 섹션의 구성은 W3C(World Wide Web Consortium) XQuery 초안 사양을 기반으로 합니다. 이 섹션에서 제공되는 일부 다이어그램은 이 사양에서 가져온 것입니다. 이 섹션에서는 Microsoft XQuery 구현과 W3C 사양을 비교하고 Microsoft XQuery와 W3C의 다른 점을 설명하고 지원되지 않는 W3C 기능에 대해 설명합니다. W3C 사양은 에서 [http://www.w3.org/TR/2004/WD-xquery-20040723](https://go.microsoft.com/fwlink/?LinkId=48846)사용할 수 있습니다.  
...
## <a name="see-also"></a>참고 항목  
 ...
 [SQL 서버&#41;&#40;XML 스키마 컬렉션](../relational-databases/xml/xml-schema-collections-sql-server.md)   
