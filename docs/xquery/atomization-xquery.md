...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
 ...
 예를 들어 다음 쿼리는 LaborHours 특성의 합계를 반환합니다. 이 경우 **data ()** 는 암시적으로 특성 노드에 적용 됩니다.  
...
```  
 ...
 또한 명시적으로 지정할 수 필요 하지는 않지만 **data ()** 함수:  
...
-   UpdatedLaborHoursV1 특성을 구성할 때 산술 연산자에 원자 값이 필요합니다. 따라서 **data ()** 는 LaborHours 특성에서 반환 되는 암시적으로 적용 (`$WC/@LaborHours`). 그런 다음 원자 값 1이 추가됩니다. UpdatedLaborHoursV2 특성의 명시적 응용 프로그램의 표시 **data ()**, 있지만 필요는 없습니다.  
...
```  
 ...
 원자화는 함수, 함수에서 반환 된 값에 전달 된 비교 식 매개 변수에도 발생 **캐스팅할** 식 및 by 절 순서 대로 전달 된 순서 식입니다.  
