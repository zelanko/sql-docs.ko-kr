...
description: 인수로 지정 된 0 개 이상의 문자열을 연결 하 여 만든 문자열을 반환 하는 XQuery 함수 concat ()에 대해 알아봅니다.
ms.custom: ''
ms.date: 03/09/2017
ms.prod: sql
ms.prod_service: sql
...
```  
 ...
 앞의 쿼리는 특정한 제품에 대한 정보를 검색합니다. 다음 쿼리는 XML 카탈로그 설명이 저장된 모든 제품에 대해 동일한 정보를 검색합니다. WHERE 절에 있는 **xml** 데이터 형식의 **exist ()** 메서드는 행의 xml 문서에 <> 요소가 있으면 True를 반환 합니다 `ProductDescription` .  
