...
| [자습서: RevoScalepR 심층 분석](deepdive-data-science-deep-dive-using-the-revoscaler-packages.md) | RevoScaleR 패키지의 기능을 사용하는 방법에 대해 알아봅니다. R과 SQL Server 간에 데이터를 이동하고 특정 작업에 맞게 컴퓨팅 컨텍스트를 전환합니다. 모델 및 플롯을 만들어 개발 환경과 데이터베이스 서버 간에 이동합니다. |
