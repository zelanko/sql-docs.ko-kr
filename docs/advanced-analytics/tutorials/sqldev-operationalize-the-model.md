---
title: 4 단원 R 모델을 사용 하 여 잠재적 결과 예측
description: T-sql 함수를 사용 하 여 SQL Server 저장 프로시저에 포함 된 R 스크립트를 운영 하는 방법을 보여 주는 자습서
ms.prod: sql
...
ms.locfileid: "68714731"
---
# <a name="lesson-4-run-predictions-using-r-embedded-in-a-stored-procedure"></a>4단원: 저장 프로시저에 포함 된 R을 사용 하 여 예측 실행
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]

이 문서는 SQL Server에서 R을 사용 하는 방법에 대 한 SQL 개발자를 위한 자습서의 일부입니다.

이 단계에서는 새 관찰에 대 한 모델을 사용 하 여 잠재적 결과를 예측 하는 방법을 알아봅니다. 모델은 다른 응용 프로그램에서 직접 호출할 수 있는 저장 프로시저에 래핑됩니다. 이 연습에서는 점수 매기기를 수행 하는 여러 가지 방법을 보여 줍니다.

- **일괄 처리 점수 매기기 모드**: SELECT 쿼리를 저장 프로시저에 대 한 입력으로 사용 합니다. 저장 프로시저는 입력된 사례(case)에 해당하는 관측 표를 반환합니다.
...
**Rxpredict** 저장 프로시저는 저장 프로시저에 RevoScaleR rxpredict 호출을 래핑하는 기본 구문을 보여 줍니다.
...
## <a name="batch-scoring-a-list-of-predictions"></a>일괄 처리 점수 매기기 (예측 목록)

보다 일반적인 시나리오는 일괄 처리 모드에서 여러 관찰에 대 한 예측을 생성 하는 것입니다. 이 단계에서는 일괄 처리 점수 매기기의 작동 방식을 살펴보겠습니다.

1.  사용할 더 작은 입력 데이터 집합을 가져와 시작 합니다. 이 쿼리는 승객 수 및 예측을 수행하는 데 필요한 다른 특성과 함께 "상위 10개"의 여정 목록을 만듭니다.
...
2. 에서[!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] **Rxpredictbatchoutput** 이라는 저장 프로시저를 만듭니다.
...
경우에 따라 여러 입력 값을 전달 하 고 해당 값을 기반으로 단일 예측을 가져와야 합니다. 예를 들어, Excel 워크시트, 웹 응용 프로그램 또는 Reporting Services 보고서를 설정 하 여 저장 프로시저를 호출 하 고 해당 응용 프로그램의 사용자가 입력 하거나 선택한 입력을 제공할 수 있습니다.

이 섹션에서는 승객 count, 트립 거리 등의 여러 입력을 사용 하는 저장 프로시저를 사용 하 여 단일 예측을 만드는 방법에 대해 알아봅니다. 저장 프로시저는 이전에 저장 된 R 모델을 기준으로 점수를 만듭니다.
...
1. **RxPredictSingleRow**저장 프로시저를 만듭니다.
...
3. 결과는 승객가 상대적으로 짧은 거리를 넘는 단일의 왕복 이기 때문에이 상위 10 회 여행에 대 한 팁의 확률 (0)을 표시 합니다.
...
[3단원: T-sql을 사용 하 여 R 모델 학습 및 저장](sqldev-train-and-save-a-model-using-t-sql.md)
