---
title: SQL Server 기계 학습에 대 한 리소스 풀 만들기 | Microsoft Docs
ms.prod: sql
...
1.  모든 기존 리소스 풀의 상태를 검토 합니다. 이해 해야 하는 서비스에서 기존 리소스를 사용 하는 것입니다.
2.  서버 리소스 풀을 수정 합니다.
...
1.  서버에 대 한 기본 풀에 할당 된 리소스를 확인 하려면 다음과 같은 문을 사용 합니다.
...
3.  이 서버 기본값 설정에서 외부 런타임 경우도 대부분 작업을 완료 하는 리소스 부족. 이를 변경하려면 다음과 같이 서버 리소스 사용을 수정해야 합니다.
...
3.  이러한 변경 내용을 적용하려면 다음과 같이 리소스 관리자를 다시 구성하고 다시 시작해야 합니다.
    ...
    > [!NOTE]
    >  이 시작; 방금 제안 된 설정 기계 학습 환경 및 작업에 대 한 올바른 균형을 확인 하려면 다른 서버 프로세스 측면에서 작업을 평가 해야 합니다.
...
1.  리소스 관리자의 구성 변경 내용은 서버 전체에 적용되고 서버에 대한 기본 풀을 사용하는 워크로드 및 외부 풀을 사용하는 워크로드에 영향을 줍니다.
     ...
     따라서 우선 순위가 있는 워크로드를 보다 세부적으로 제어하려면 사용자 정의 외부 리소스 풀을 새로 만들 수 있습니다. 또한 분류 함수를 정의하고 외부 리소스 풀에 할당해야 합니다. **외부** 키워드는 새 합니다.
...
## <a name="create-a-classification-function-for-machine-learning"></a>기계 학습에 대 한 분류 함수를 만듭니다
  
분류 함수는 들어오는 태스크를 검사하고 현재 리소스 풀을 사용하여 실행할 수 있는 태스크인지 확인합니다. 분류 함수의 조건을 충족하지 않는 태스크는 서버의 기본 리소스 풀에 다시 할당됩니다.
  
1. 분류자 함수 해야 사용 하도록 지정 하 여 리소스 관리자 리소스 풀을 확인 하 여 시작 합니다. 할당할 수는 **null** 분류자 함수에 대 한 자리 표시자로 합니다.
...
2.  각 리소스 풀에 대 한 분류자 함수에서 문 또는 리소스 풀에 할당 해야 하 고 있는 요청 유형을 정의 합니다.
...
적용 된 변경 내용이 있는지를 확인 하려면 각 이러한 인스턴스 리소스 풀과 연결 된 작업 그룹에 대 한 서버 메모리 및 CPU의 구성을 확인 해야:

+ 에 대 한 기본 풀은 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 서버
...
3.  해당 하는 경우 외부 리소스 풀으로 선호도 설정 되는 컴퓨터 리소스에 대 한 정보를 반환 하려면 다음 문을 실행 합니다.
...
+  [컴퓨터 학습 서비스에 대 한 리소스 관리](../../advanced-analytics/r/resource-governance-for-r-services.md)
