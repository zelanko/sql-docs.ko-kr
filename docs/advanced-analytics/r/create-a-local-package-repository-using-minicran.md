...
-   [수동으로 다운로드 하 여 개별적으로 패키지 복사](#bkmk_manual)
    ...
    다운로드 한 패키지에 대 한 설명 파일에 종속 된 패키지 목록을 찾을 수 있습니다. 
    
    패키지에 나열 되는 반면 **Imports** 두 번째 수준 종속성이 있을 수 있습니다. 이러한 이유로 사용 좋습니다는 **miniCRAN** 메서드.

> [!TIP]
> Azure 기계 학습에서 사용 하기 위해 패키지를 준비 하는 데 miniCRAN를 사용할 수 있는 것을 아십니까? 자세한 내용은이 블로그를 참조 하십시오.: [miniCRAN 미셸 Usuelli 하 여 Azure 기계 학습에서 사용 하 여](https://www.r-bloggers.com/using-minicran-in-azure-ml/) 
...
> [!NOTE]
> MiniCRAN 패키지 자체 18 다른 CRAN 패키지와 중에 curl 개발자 패키지 시스템 종속 RCurl 패키지는이에 따라 달라 집니다. 마찬가지로, 패키지 XML libxml2 개발자에 종속 됩니다. 좋습니다, 따라서 빌드하는 로컬 리포지토리 처음 전체 인터넷에 연결 된 컴퓨터에서 이러한 모든 종속성을 쉽게 만족할 수 있도록 합니다. 를 만든 후에 다른 위치에 저장소를 이동할 수 있습니다.
...
2.  수집 된 패키지를 저장할 로컬 폴더의 이름을 입력 합니다. 
    ...
    폴더 설명 하는 이름이 있어야 합니다. 예를 들어 "miniCRAN"를 사용 하지 않도록 하 고 대신 "GeneticsPackages" 또는 "TeamRPackages1.0.2"와 같은 텍스트를 입력 합니다.
...
1.  MiniCRAN를 설치한 후 다운로드 하려는 추가 패키지를 지정 하는 목록을 만듭니다.
    ...
    이 초기 목록에 종속성을 추가 하지 마십시오 **igraph** miniCRAN에서 사용 하는 패키지를 종속성 목록을 생성 합니다. 생성 된 종속성 그래프를 사용 하는 방법에 대 한 자세한 내용은 참조 [miniCRAN를 사용 하 여 패키지 종속성을 식별 하](https://cran.r-project.org/web/packages/miniCRAN/vignettes/miniCRAN-dependency-graph.html)합니다.
...
- SQL Server 데이터베이스에 패키지를 업로드 하 고 외부 라이브러리 만들기 문을 사용 하 여 설치 합니다. 이 옵션을 사용 하려면 SQL Server 2017 합니다. 참조 [SQL Server에 추가 R 패키지 설치](install-additional-r-packages-on-sql-server.md)합니다.

다음 절차에는 R 도구를 사용 하 여 패키지를 설치 하는 방법을 설명 합니다.

1. MiniCRAN 리포지토리 패키지를 설치 하려는 서버에 전체를 포함 하는 폴더를 복사 합니다.
...
4.  mininCRAN 리포지토리를 복사한 위치 서버의 위치를 지정 `server_repo`합니다.
...
> [!NOTE] 
> SQL Server에서 서버 관리자는 인스턴스에서 사용 하는 기본 라이브러리로 miniCRAN 리포지토리에서 패키지를 설치 해야 합니다. 
