...
[olapR](https://docs.microsoft.com/machine-learning-server/r-reference/olapr/olapr) 패키지는 SQL Server Analysis Services에서 호스트 되는 다차원 모델에 대 한 쿼리를 지원 합니다. MDX를 사용 하 여 기존 큐브에 대 한 쿼리를 작성 하 고, 차원 및 기타 큐브 개체를 탐색 하 고, 데이터를 검색 하는 기존 MDX 쿼리에 붙여 넣는 수 있습니다.

이 문서에서는 olapR 패키지의 두 가지 주요 용도를 설명합니다.

+ [R을 olapR 패키지에서 제공 하는 생성자를 사용 하 여 쿼리를 작성 합니다.](#buildMDX)
...
+ 4 단원까지 Analysinotes 서비스 자습서를 수행 하 여 이러한 예제에서 사용 되는 큐브를 만들기: [OLAP 큐브 만들기](../../analysis-services/multidimensional-modeling-adventure-works-tutorial.md)
...
접두사가 추가 되며 반환 결과의 항목  **->**  이전 멤버의 자식 항목을 나타냅니다.
