...
이 항목에서는 실시간 점수 매기기를 실행 하는 방법 및 SQL Server 2016 및 SQL Server 2017에서 네이티브 점수 매기기 기능에 대 한 지침 및 샘플 코드를 제공 합니다. 작은 규모로 일괄 처리 점수 매기기 작업의 성능을 개선 하기 위해 실시간 점수 매기기와 기본 점수 매기기의 목표가입니다.
...
+ **실시간 점수 매기기**: SQL Server 2016 또는 SQL Server 2017에서 저장 프로시저는 sp_rxPredict를 사용 합니다.

> [!NOTE]
> SQL Server 2017에 예측 함수를 사용 하는 것이 좋습니다.
> Sp_rxPredict 사용 하도록 하려면 SQLCLR 통합 사용 하도록 설정 합니다. 이 옵션을 사용 하기 전에 보안 측면을 고려 합니다.
...
+ 이 문서 작성 당시의 RevoScaleR 및 MicrosoftML 호환 모델을 만들 수 있습니다. 나중에 추가 하는 모델 유형에 사용할 수 있습니다. 현재 지원 되는 알고리즘 목록은 참조 [실시간 점수 매기기](../real-time-scoring.md)합니다.
...
이 예제에서는 모델을 만들 하 고 T-SQL에서 실시간 예측 함수를 호출 합니다.
...
> [!NOTE]
> 점수 매기기 작업를 사용 하는 실시간 검색 순서로 SQL CLR 기능 인스턴스에서 사용할 수 있어야 하 고 데이터베이스를 신뢰할 수 있음으로 표시 되어야 있어야 합니다. 스크립트를 실행 하면 이러한 작업은 수행 됩니다. 그러나이 작업의 추가 보안 문제도 고려해 야 합니다.
...
Sp에서 요구 하는 이진 형식\_rxPredict 예측에 대 한 동일 하 게 됩니다.

따라서 R 코드에 대 한 호출을 포함 [rxSerializeModel](https://docs.microsoft.com/r-server/r-reference/revoscaler/rxserializemodel)를 지정 해야 하 고 _realtimeScoringOnly_ = TRUE, 다음이 예제와 같이:
...
다른 저장된 프로시저와 마찬가지로 sp_rxPredict를 호출 합니다. 현재 릴리스에서 저장된 프로시저는 두 개의 매개 변수를 사용:  _@model_  이진 형식으로 모델에 대 한 및  _@inputData_  상태 평가에 사용 하는 데이터에 대 한 유효한 SQL 쿼리로 정의 .
...
> [!NOTE]
> 
> 에 대 한 호출 `sp_rxPredict` 점수 매기기에 대 한 입력된 데이터 모델의 요구 사항과 일치 하는 열을 포함 하는 경우 실패 합니다. 현재 다음.NET 데이터 형식만 지원 됩니다: float, short, ushort, double, long, ulong 및 문자열입니다.
> 
...
### <a name="realtime-scoring-in-microsoft-r-server"></a>Microsoft R Server에 점수 매기기 실시간

실시간 대 한 자세한 내용은 Microsoft R Server에 기반 하 여 분산된 환경에서 점수 매기기를 참조 하십시오는 [publishService](https://msdn.microsoft.com/microsoft-r/mrsdeploy/packagehelp/publishservice) 함수에서 사용할 수는 [mrsDeploy 패키지](https://msdn.microsoft.com/microsoft-r/mrsdeploy/mrsdeploy)를 지 원하는 실시간 새 점수 매기기 R 서버에서 실행 되는 웹 서비스에 대 한 모델을 게시 합니다.
