---
title: Machine Learning Services의 알려진 문제 | Microsoft Docs
ms.prod: sql
...
ms.locfileid: "51704111"
---
# <a name="known-issues-in-machine-learning-services"></a>Machine Learning Services의 알려진된 문제
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-winonly](../includes/appliesto-ss-xxxx-xxxx-xxx-md-winonly.md)]

이 문서는 기계 학습 SQL Server 2016 및 SQL Server 2017의 옵션으로 제공 되는 구성 요소를 사용 하 여 제한 사항 또는 알려진된 문제를 설명 합니다.

지정 하지 않을 경우이 정보를 모든 다음에 적용 됩니다.
...
- Machine Learning Services (In-database)를 R에 대 한
- Machine Learning Services (In-database) python
...
### <a name="r-script-runtime-error-sql-server-2017-cu5-cu7-regression"></a>R 스크립트 런타임 오류 (SQL Server 2017 CU5-CU7 회귀)
...
### <a name="unable-to-install-sql-server-machine-learning-features-on-a-domain-controller"></a>도메인 컨트롤러에 SQL Server machine learning 기능을 설치할 수 없습니다.
...
### <a name="install-the-latest-service-release-to-ensure-compatibility-with-microsoft-r-client"></a>Microsoft R Client와 호환성을 위해 최신 서비스 릴리스 설치
...
### <a name="r-components-missing-from-cu3-setup"></a>CU3 설치 프로그램에서 누락 된 R 구성 요소
...
### <a name="launchpad-services-fails-to-start-if-the-version-is-different-from-the-r-version"></a>실행 패드 서비스가 버전이 R 버전과 다른 경우 시작 되지 않음
...
### <a name="remote-compute-contexts-are-blocked-by-a-firewall-in-sql-server-instances-that-are-running-on-azure-virtual-machines"></a>원격 계산 컨텍스트는 Azure virtual machines에서 실행 중인 SQL Server 인스턴스의 방화벽에 의해 차단
...
대 안으로, Azure VM에서 엽니다 **고급 보안이 포함 된 Windows 방화벽**를 선택 **아웃 바운드 규칙**, 다음 규칙을 사용 하지 않도록 설정: **R 로컬 사용자 계정에 대 한 네트워크 액세스 차단 SQL Server 인스턴스 MSSQLSERVER**합니다. 또한 활성화 규칙을 그대로 유지 수 있지만 보안 속성을 변경 **안전한 경우 허용**합니다.
...
### <a name="performance-limits-when-libraries-used-by-sql-server-are-called-from-other-tools"></a>다른 도구에서 SQL Server에서 사용 되는 라이브러리 라고 하는 경우의 성능 제한
...
### <a name="sql-server-data-tools-does-not-support-permissions-required-by-external-scripts"></a>SQL Server Data Tools 외부 스크립트에 필요한 권한을 지원 하지 않습니다.
...
> *TSQL 모델: 리버스 엔지니어링 데이터베이스 경우 검색 오류가 발생 했습니다. 사용 권한을 인식할 수 없습니다 및 가져오지 않았습니다.*
...
### <a name="external-script-execution-is-throttled-due-to-resource-governance-default-values"></a>외부 스크립트 실행이 리소스 거 버 넌 스 기본 값으로 인해 제한
...
### <a name="access-denied-warning-when-executing-r-scripts-on-sql-server-in-a-non-default-location"></a>기본이 아닌 위치에 SQL Server에서 R 스크립트를 실행 하는 경우 경고를 거부 하는 액세스
...
### <a name="serialization-error-between-old-and-new-versions-of-revoscaler"></a>RevoScaleR의 이전 및 새 버전 간에 직렬화 오류
...
### <a name="real-time-scoring-does-not-correctly-handle-the-learningrate-parameter-in-tree-and-forest-models"></a>실시간 점수 매기기 제대로 처리 하지 않습니다 합니다 _learningRate_ 트리 및 포리스트 모델의 매개 변수
...
### <a name="avoid-clearing-workspaces-when-you-execute-r-code-in-a-includessnoversionincludesssnoversion-mdmd-compute-context"></a>R 코드를 실행 하면 작업 영역을 지우지 마세요는 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 계산 컨텍스트
...
### <a name="arguments-varstokeep-and-varstodrop-are-not-supported-for-sql-server-data-sources"></a>인수 *varsToKeep* 하 고 *varsToDrop* SQL Server 데이터 원본에 대해 지원 되지 않습니다
...
### <a name="limited-support-for-sql-data-types-in-spexecuteexternalscript"></a>Sp의 SQL 데이터 형식에 대 한 지원이 제한 됩니다\_실행할\_외부\_스크립트
...
문자열 데이터의 모든 왕복 [!INCLUDE[tsql](../includes/tsql-md.md)] r 차례로 [!INCLUDE[tsql](../includes/tsql-md.md)] 다시 손상 될 수 있습니다. R에서 사용 되는 다른 인코딩 원인은 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], 다양 한 데이터 정렬 및 R에서 지원 되는 언어 및 [!INCLUDE[tsql](../includes/tsql-md.md)]합니다. ASCII가 아닌 인코딩의 모든 문자열은 잠재적으로 잘못 처리될 수 있습니다.

R에 문자열 데이터를 보낼 때 변환할 ASCII 표현 가능한 경우.

이러한 제한도 SQL Server와 Python 간에 전달 되는 데이터에 적용 됩니다. 멀티 바이트 문자를 u t F-8로 전달 하 고 유니코드로 저장 해야 합니다.

### <a name="only-one-value-of-type-raw-can-be-returned-from-spexecuteexternalscript"></a>형식의 값을 하나만 `raw` 에서 반환할 수 있습니다 `sp_execute_external_script`
...
### <a name="variable-scoping-error-when-you-use-the-transformfunc-parameter"></a>TransformFunc 매개 변수를 사용 하는 경우 범위 지정 오류가 발생 하는 변수
...
병렬 처리 `rxExec` 여러 프로세스에서 향후 릴리스에 예정 되어 있습니다.

### <a name="increase-the-maximum-parameter-size-to-support-rxgetvarinfo"></a>RxGetVarInfo를 지원 하도록 최대 매개 변수 크기 늘리기
...
### <a name="call-to-pretrained-model-fails-if-path-to-model-is-too-long"></a>모델에 경로가 너무 깁니다. 미리 학습 된 모델 호출 실패
...
### <a name="error-when-saving-serialized-model-to-sql-server"></a>SQL Server에 대 한 모델을 직렬화 하는 저장 하는 동안 오류가 발생 했습니다.
...
### <a name="failure-to-initialize-a-varbinary-variable-causes-an-error-in-bxlserver"></a>BxlServer에서는 오류가 발생 하는 varbinary 변수 초기화 실패
...
### <a name="telemetry-warning-on-successful-execution-of-python-code"></a>Python 코드의 실행 성공 시 원격 분석 경고
