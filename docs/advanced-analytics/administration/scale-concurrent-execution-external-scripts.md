...
SQL Server Machine Learning Services의 작업자 계정에 대해 알아보고, 외부 스크립트의 동시 실행 수를 조정 하는 기본 구성을 변경 하는 방법에 대해 알아봅니다.

Machine Learning Services에 대 한 설치 프로세스의 일부로 새 Windows *사용자 계정 풀이* 만들어져 @no__t 1 서비스에서 작업 실행을 지원 합니다. 이러한 작업자 계정의 목적은 여러 SQL Server 사용자가 외부 스크립트를 동시에 실행 하는 것을 격리 하는 것입니다.

> [!Note]
> SQL Server 2019에서 **SQLRUserGroup** 에는 여러 작업자 계정 대신 단일 SQL Server 실행 패드 서비스 계정인 하나의 멤버만 있습니다. 이 문서에서는 SQL Server 2016 및 2017에 대 한 작업자 계정을 설명 합니다.
...
- 기본 인스턴스에서 그룹 이름은 **SQLRUserGroup**입니다. Python 또는 R을 사용 하는지 또는 둘 다를 사용 하는지에 관계 없이 이름이 동일 합니다.
...
이 풀의 계정 수에 따라 동시에 활성화할 수 있는 외부 스크립트 세션 수가 결정 됩니다.  기본적으로 20 명의 사용자가 한 번에 활성 Python 또는 R 세션을 가질 수 있는 20 개의 계정이 생성 됩니다. 20 개가 넘는 동시 스크립트를 실행 해야 하는 경우 작업자 계정 수를 늘릴 수 있습니다.

동일한 사용자가 여러 외부 스크립트를 동시에 실행 하는 경우 해당 사용자가 실행 하는 모든 세션이 동일한 작업자 계정을 사용 합니다. 예를 들어 단일 사용자에 게는 리소스를 허용 하는 한 100 개의 Python 또는 R 스크립트를 동시에 실행할 수 있지만 모든 스크립트는 단일 작업자 계정을 사용 하 여 실행 됩니다.

지원할 수 있는 작업자 계정 수와 단일 사용자가 실행할 수 있는 동시 세션 수는 서버 리소스에 의해서만 제한 됩니다. 일반적으로 메모리는 Python 또는 R 런타임을 사용할 때 발생 하는 첫 번째 병목 현상입니다.
