...
외부 라이브러리를 만들어 제공할 필요가 없습니다를 [classpath](#classpath) sp_execute_external_script 호출에서 합니다. SQL Server는 Java 클래스에 대 한 액세스를 자동으로 포함 됩니다 하 고 클래스 경로에 모든 특수 사용 권한을 설정할 필요가 없습니다.
...
에 인수 전달에 사용 된 @param sp_execute_external_script에서 매개 변수입니다. 메서드 자체는 모든 인수를 사용할 수 없습니다. 반환 형식은 void 여야 합니다.  
