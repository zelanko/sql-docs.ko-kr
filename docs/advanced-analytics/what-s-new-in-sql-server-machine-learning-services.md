...
| 릴리스 |기능 업데이트 |
|---------|----------------|
| CU | [실시간 점수 매기기](real-time-scoring.md)합니다. 실시간 점수 매기기를 최적화 된 이진 형식으로 저장 된 모델을 읽고은 R 런타임을 호출 하지 않고 예측을 생성할 네이티브 c + + 라이브러리에 의존 합니다. 이렇게 하면 점수 매기기 작업 훨씬 빠릅니다. 실시간 점수 매기기와 저장된 프로시저를 실행할 수도 있고 실시간 R 코드에서 점수 매기기를 수행할 수 있습니다. 실시간 점수 매기기는 최신 버전에는 인스턴스를 업그레이드 하는 경우 SQL Server 2016 가능 [!INCLUDE[rsql-platform-md](../includes/rsql-platform-md.md)]합니다. |
| 초기 릴리스 | R 패키지 및 인터프리터 호출 R에 대 한 T-SQL, 그 반대로 작동 합니다.<br/>조정 구성 요소 부분으로 구성 데이터를 청크 하 여 규모에 R 분석을 제공 하는 RevoScaleR 함수 및 분산 처리 및 결과 집계를 관리 합니다. SQL Server 2016 R Services (In-database)에서 RevoScaleR 엔진 brining 데이터 및 분석에서 동일한 처리 컨텍스트는 데이터베이스 엔진 인스턴스 통합 되어 있습니다. |
...
+ [SQL Server 컴퓨터 학습 서비스 설치](../advanced-analytics/python/setup-python-machine-learning-services.md)
