---
title: "SQL Server R Services의 알려진 문제 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/10/2017"
...
---
# SQL Server R Services의 알려진 문제
  이 항목에서는 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 및 [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)]의 [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)] 및 관련 구성 요소에 대한 제한 사항과 문제를 설명합니다.  
  
> [!NOTE]
> 초기 설치 및 구성과 관련된 추가 문제는 [업그레이드 및 설치 FAQ](../../advanced-analytics/r-services/upgrade-and-installation-faq-sql-server-r-services.md)에 나와 있습니다.  
  
## <a name="r-services-in-database"></a>R Services(In-Database)  
 이 섹션에서는 R 통합을 지원하는 데이터베이스 엔진의 기능과 관련된 문제를 보여 줍니다.  
...
### <a name="new-license-agreement-for-r-components-required-for-unattended-installs"></a>무인 설치에 필요한 R 구성 요소에 대한 새 사용권 계약  
 명령줄을 사용하여 [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)]가 설치되어 있는 SQL Server 인스턴스를 설치하는 경우 명령줄을 편집하여 새 사용권 계약 매개 변수 */IACCEPTROPENLICENSEAGREEMENT*를 사용해야 합니다. 올바른 인수를 사용하지 않으면 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 설치에 실패할 수 있습니다.  
  
### <a name="warning-of-incompatible-version-when-connecting-to-older-version-of-sql-server--r-services-from-a-client-using-includesssqlv14mdtokensqlv14sssqlv14mdmd"></a>[!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)]를 사용하여 클라이언트에서 이전 버전의 SQL Server R Services에 연결하는 경우 호환되지 않는 버전 경고 
...
**SqlBindR.exe** 도구는 호환되는 9.0 버전으로 SQL Server 인스턴스 업그레이드를 지원하기 위해 Microsoft R Server 9.0 릴리스에서 제공됩니다. 9.0으로 R Services 인스턴스 업그레이드 지원은 향후 서비스 릴리스의 일부로 SQL Server에 추가될 예정입니다. 향후 업그레이드 후보 버전에는 SQL Server 2016 RTM CU3 이상 및 SP1 이상과 SQL Server vNext CTP 1.1이 포함됩니다. 
...
해결 방법으로, 명령줄을 사용하고 이 예제에 표시된 대로 CU1에 대한 /MRCACHEDIRECTORY 인수를 지정하여 서비스 릴리스를 설치할 수 있습니다. 

`C:\<path to installation media>\SQLServer2016-KB3164674-x64.exe /Action=Patch /IACCEPTROPENLICENSETERMS /MRCACHEDIRECTORY=<path to CU1 CAB files>`

최신 CAB 파일을 가져오려면 [인터넷에 연결하지 않고 R 구성 요소 설치](../../advanced-analytics/r-services/installing-r-components-without-internet-access.md)를 참조하세요.

### <a name="windows-group-created-for-launchpad-must-have-an-account-in-the-sql-server-instance"></a>실행 패드에 대해 생성된 Windows 그룹의 계정이 SQL Server 인스턴스에 있어야 함
 SQL Server R Services 설치 시 신뢰할 수 있는 실행 패드에서 R 작업을 실행하는 데 사용되는 기본 이름 **SQLRUsers**로 Windows 사용자 그룹이 생성됩니다. Windows 통합 인증을 사용하여 원격 클라이언트에서 R 작업을 실행해야 하는 경우 R이 사용되는 SQL Server 인스턴스에 로그인할 수 있는 권한을 이 Windows 사용자 그룹에 부여해야 합니다.

**SQLRUsers** 그룹에 이 사용 권한이 없는 환경에서는 다음과 같은 오류가 표시될 수 있습니다.  
...
> [!NOTE]
> 공유 메모리를 사용하여 SQL Server Management Studio에서 R 작업을 실행하는 경우 R 작업에서 포함된 ODBC 호출을 사용할 때까지 이 제한을 발견하지 못할 수 있습니다. 
> 
> 원격 워크스테이션에서 SQL 로그인을 사용하는 경우에는 이 해결 방법이 필요하지 않습니다.

### <a name="launchpad-services-fails-to-start-if-version-is-different-than-r-version"></a>버전이 R 버전과 다른 경우 실행 패드 서비스가 시작되지 않음
데이터베이스 엔진에서 별도로 R Services를 설치하고 버전이 다른 경우 시스템 이벤트 로그에 다음 오류가 표시될 수도 있습니다. _다음 오류가 발생하여 SQL Server 실행 패드 서비스를 시작하지 못했습니다. 서비스가 적시에 시작 또는 제어 요청에 응답하지 않았습니다._
...
각 SQL Server 2016 릴리스에 필요한 R 버전 번호 목록을 보려면 [인터넷에 액세스하지 않고 R 구성 요소 설치](../../advanced-analytics/r-services/installing-r-components-without-internet-access.md)를 참조하세요.


### <a name="service-account-for-launchpad-requires-permission-replace-process-level-token"></a>실행 패드에 대한 서비스 계정에 프로세스 수준 토큰 바꾸기 권한이 필요함
 SQL Server R Services 설치 시 기본적으로 필요한 사용 권한으로 프로비전된 NT Service\MSSQLLaunchpad 계정을 사용하여 신뢰할 수 있는 실행 패드가 시작됩니다. 그러나 다른 계정을 사용하거나 이 계정과 연결된 권한을 변경하는 경우 실행 패드를 시작하지 못할 수 있습니다.
 
 실행 패드 서비스 계정이 로그인할 수 있는지 확인하려면 `Replace Process Level Token` 권한을 계정에 부여합니다. 자세한 내용은 [프로세스 수준 토큰 바꾸기](https://technet.microsoft.com/itpro/windows/keep-secure/replace-a-process-level-token)를 참조하세요.
...
SQL Server 2016의 RTM 빌드에서는 첫 번째 k 그룹의 CPU에 대해서만 프로세서 선호도를 설정할 수 있습니다. 예를 들어 서버가 2개의 k 그룹이 있는 2 소켓 시스템인 경우 첫 번째 k 그룹의 프로세서만 R 프로세스에 사용됩니다. R 스크립트 작업에 대한 리소스 관리를 구성하는 경우 동일한 제한이 적용됩니다.
...
### <a name="resource-governance-default-values"></a>리소스 관리 기본값  
Enterprise Edition에서 리소스 풀을 사용하여 외부 스크립트 프로세스를 관리할 수 있습니다. 일부 빌드에서는 R 프로세스에 할당할 수 있는 최대 메모리가 20%였습니다. 따라서 서버에 32GB의 RAM이 있는 경우 R 실행 파일(RTerm.exe 및 BxlServer.exe)은 단일 요청에 최대 6.4GB을 사용할 수 있었습니다. 

리소스 제한에 도달할 경우 현재 기본값을 확인하고, 20%가 충분하지 않으면 이 값을 변경하는 방법에 대한 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 설명서를 참조하세요.  

### <a name="avoid-clearing-workspaces-when-executing-r-code-in-a-includessnoversiontokenssnoversionmdmd-compute-context"></a>[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 계산 컨텍스트에서 R 코드를 실행하는 경우에는 작업 영역을 지우지 마세요.  
...
## <a name="microsoft-r-server-standalone"></a>Microsoft R Server(독립 실행형)  
 이 섹션에서는 Microsoft R Server 독립 실행형 버전과 관련된 문제를 보여 줍니다. 
  
### <a name="multiple-r-libraries-and-executables-are-installed-if-you-install-both-standalone-and-in-database"></a>독립 실행형 및 In-Database를 둘 다 설치하는 경우 R 라이브러리와 실행 파일이 여러 개 설치됨  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 설치 프로그램에는 Microsoft R Server(독립 실행형)를 설치하는 옵션이 포함되어 있습니다. Enterprise Edition에서 Microsoft R Server(독립 실행형) 옵션을 사용하여 R을 지원하지만 SQL Server와 상호 작용이 필요하지 않은 독립 실행형 Windows 서버를 설치할 수 있습니다.    

> [!TIP] 이 독립 실행형 옵션은 [!INCLUDE[tsql](../../includes/tsql-md.md)]에서 R을 사용하는 데 필요하지 **않습니다**.
> 
> [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)] 또는 Microsoft R Server(독립 실행형)에 연결할 수 있는 데이터 과학 클라이언트 컴퓨터를 설정해야 하는 경우 [Microsoft R Client](http://go.microsoft.com/fwlink/?LinkId=799768)를 사용하는 것이 좋습니다.  

동일한 컴퓨터에 R Services(In-Database)와 Microsoft R Server(독립 실행형)를 둘 다 설치하는 경우 Microsoft R Server(독립 실행형)에 대한 별도 R 인스턴스는 물론 R이 사용되는 각 SQL Server 인스턴스에 대한 별도 R 인스턴스가 생성됩니다.  예를 들어 기본 인스턴스 및 명명된 인스턴스와 R 서버(독립 실행형)를 설치한 경우 동일한 컴퓨터에 다음 세 개의 R 인스턴스가 있을 수 있습니다.  
  
-   **독립 실행형:** C:\Program Files\Microsoft SQL Server\130\R_SERVER  
  
-   **기본 인스턴스:** C:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\R_SERVICES  
  
-   **명명된 인스턴스:** C:\Program Files\Microsoft SQL Server\MSSQL13.<instance_name>\R_SERVICES  
  
> [!NOTE] 
> 
> [!INCLUDE[tsql](../../includes/tsql-md.md)] 컨텍스트에서만 데이터베이스 인스턴스와 연결된 R 라이브러리 및 도구를 사용합니다. 다른 R 도구를 사용하여 R을 실행해야 하는 경우 기본적으로 C:\Program Files\Microsoft SQL Server\130\R_SERVER에 설치된 R 서버(독립 실행형)에서 사용하는 R 라이브러리를 참조해야 합니다.  

### <a name="performance-limits-when-r-services-libraries-are-called-from-standalone-r-tools"></a>독립 실행형 R 도구에서 R Services 라이브러리를 호출하는 경우의 성능 제한

SQL Server R Services에서 사용하는 R 라이브러리는 기본적으로 C:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\R_SERVICES에 설치됩니다. RGui 등의 외부 R 응용 프로그램에서 SQL Server R Services에 대해 설치된 R 도구 및 라이브러리를 *호출할 수 있습니다*. 

그러나 이렇게 하면 성능이 제한됩니다. 예를 들어 SQL Server Enterprise Edition을 구매한 경우에도 외부 도구를 사용하여 R 코드를 실행하면 단일 스레드 모드에서 R이 실행됩니다. SQL Server 연결을 시작하고 자동으로 R Services 라이브러리를 호출할 [sp_execute_external_script](../../relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql.md)를 사용하여 R 코드를 실행하면 성능이 향상됩니다.

+ SQL Server에서 사용하는 R 라이브러리를 외부 R 도구에서 호출하지 마세요. 
+ 외부 도구를 사용하여 SQL Server 컴퓨터에서 R을 실행해야 하는 경우 별도 R 인스턴스를 설치한 다음 R 도구가 새 라이브러리를 가리키는지 확인해야 합니다. 
+ Enterprise Edition을 사용하는 경우 SQL Server 컴퓨터에 Microsoft R Server(독립 실행형)를 설치하는 것이 좋습니다. 그런 다음 R 코드를 실행하는 데 사용하는 외부 도구에서 기본적으로 C:\Program Files\Microsoft SQL Server\<version number>\R_SERVER에 설치된 R Server용 라이브러리를 참조합니다. 

자세한 내용은 [독립 실행형 R 서버 만들기](../../advanced-analytics/r-services/create-a-standalone-r-server.md)를 참조하세요.  
  
  
## <a name="general-r-issues"></a>일반적인 R 관련 문제  

 이 섹션에서는 R 연결 및 성능 도구와 관련된 문제를 보여 줍니다.  
  
### <a name="limited-support-for-sql-data-types-in-spexecuteexternalscript"></a>sp_execute_external_script에서 SQL 데이터 형식 지원이 제한됨  
 ...
 자세한 내용은 [R 데이터 형식 사용](../../advanced-analytics/r-services/working-with-r-data-types.md)을 참조하세요.  
...
### <a name="only-one-raw-value-can-be-returned-from-spexecuteexternalscript"></a>sp_execute_external_script에서는 하나의 원시 값만 반환될 수 있음  
...
### <a name="loss-of-precision"></a>정밀도 손실  
 ...
 암시적 데이터 형식 변환에 대한 자세한 내용은 [Working with R Data Types](../../advanced-analytics/r-services/working-with-r-data-types.md)을 참조하십시오.  
...
일반적으로 이전 버전을 제거하고 최신 버전의 SQL Server R Services, Microsoft R Server 또는 Microsoft R Client를 설치하는 것이 좋습니다.
...
### <a name="limited-support-for-revoscaler-rxexec"></a>RevoScaleR rxExec에 대한 제한된 지원  

 RC0에서는 `rxExec` 에서 제공하는 [!INCLUDE[rsql_rre-short](../../includes/rsql-rre-short-md.md)] 함수를 단일 스레드 모드에서만 사용할 수 있습니다.  
...
### <a name="issues-with-rxdtree-function"></a>rxDTree 함수 문제  
...
### <a name="using-the-r-productivity-environment"></a>R 생산성 환경 사용  

일부 [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)] 시험판 버전에는 Revolution Analytics를 통해 생성된 Windows용 R 개발 환경이 포함되어 있었습니다. 

그러나 [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)]와의 호환성을 위해 Revolution Analytics 도구 대신 Microsoft R Client 또는 Microsoft R Server를 설치하는 것이 좋습니다. [R Tools for Visual Studio](https://www.visualstudio.com/vs/rtvs/)는 Microsoft R 솔루션을 지원하는 다른 권장 클라이언트입니다.
