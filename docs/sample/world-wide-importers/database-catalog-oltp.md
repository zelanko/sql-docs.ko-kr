...
|Configuration_PopulateLargeSaleTable|Enterprise edition 변경을 적용 했습니다. 다음 추가 기록으로 2012 달력 연도 대 한 더 많은 양의 데이터를 채웁니다.|
...
|ReseedSequenceBeyondTableValue|동일한 시퀀스를 사용 하는 테이블의 값 보다 큰 다음 시퀀스 값의 위치를 변경 하는 데 사용 합니다. (같은 `DBCC CHECKIDENT` identity 열 동일 시퀀스에 대 한 하지만 잠재적으로 여러 테이블에 대 한).|
