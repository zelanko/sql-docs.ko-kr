...
|플랫폼|지원되는 버전|
|---------|---------|
|[!INCLUDE [azure-data-cli-azdata](../includes/azure-data-cli-azdata.md)]|사용 가능한 최신 버전을 사용하는 것이 가장 좋습니다. SQL Server 2019 CU5 릴리스부터 [!INCLUDE [azure-data-cli-azdata](../includes/azure-data-cli-azdata.md)]에는 서버와 별개의 의미 체계 버전이 있습니다. <br/><br/>`azdata –-version`을 실행하여 버전을 확인하세요.<br/><br/>최신 버전은 [릴리스 기록](#release-history)을 참조하세요.|
...
| 릴리스 <sup>1</sup> | BDC 버전    | [!INCLUDE [azure-data-cli-azdata](../includes/azure-data-cli-azdata.md)] 버전 <sup>2</sup>| 릴리스 날짜 |
|------------------|----------------|-----------------|--------------|
| [CU8](#cu8)      | 15.0.4073.23   | 20.2.2          | 2020년 10월 19일   |
...
<sup>1</sup> CU7는 BDC에 사용할 수 없습니다.

<sup>2</sup> [!INCLUDE [azure-data-cli-azdata](../includes/azure-data-cli-azdata.md)] 버전은 CU 릴리스 시점의 도구 버전을 반영합니다. `azdata`는 서버 릴리스와 별개로 릴리스할 수도 있으므로 최신 패키지를 설치하면 최신 버전을 가져올 수 있습니다. 최신 버전은 이전에 릴리스된 CU와 호환됩니다.
...
SQL Server 2019의 CU8(누적 업데이트 8) 릴리스입니다.

|패키지 버전 | 이미지 태그 |
|-----|-----|
|15.0.4073.23 |[2019-CU8-ubuntu-16.04]

이 릴리스에는 여러 수정 사항과 몇 가지 향상된 기능이 포함되어 있습니다.

### <a name="added-capabilities"></a>추가된 기능

- 시스템에서 관리하는 키 및 인증서를 사용하는 [SQL Server 빅 데이터 클러스터 미사용 암호화](encryption-at-rest-concepts-and-configuration.md).
   > [!CAUTION]
   > SQL Server BDC 미사용 데이터 암호화의 초기 릴리스입니다. 다음 문서를 검토합니다. 
   > - [[!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ss-nover.md)]의 보안 개념](concept-security.md)
   > - [미사용 데이터 암호화 개념 및 구성 가이드](encryption-at-rest-concepts-and-configuration.md)
- 데이터 가상화 시나리오에 대한 [Oracle 프록시 사용자](tutorial-query-oracle.md) 지원입니다.
...
- [!INCLUDE [azure-data-cli-azdata](../includes/azure-data-cli-azdata.md)]에는 서버와 별개인 고유한 의미 체계 버전이 있습니다. 클라이언트 버전과 서버 버전의 azdata 간에 모든 종속성이 제거되었습니다. 최신 향상 기능 및 수정 사항을 활용할 수 있도록 클라이언트와 서버 모두의 최신 버전을 사용하는 것이 좋습니다.
