...
description: 고가용성을 사용 하 여 빅 데이터 클러스터 SQL Server를 배포 하는 방법을 알아봅니다.
...
1. 배포 구성 파일에 고가용성 설정이 지정 된 경우 `containedag` 라는 단일 가용성 그룹이 만들어집니다. 기본적으로 `containedag`에는 주 복제본을 포함 하 여 세 개의 복제본이 있습니다. 가용성 그룹에 대 한 모든 CRUD 작업은 내부적으로 관리 됩니다.
1. 모든 데이터베이스는 `master` 및 `msdb`를 포함 하 여 가용성 그룹에 자동으로 추가 됩니다. Polybase 구성 데이터베이스는 각 복제본과 관련 된 인스턴스 수준 메타 데이터를 포함 하기 때문에 가용성 그룹에 포함 되지 않습니다.
1. 외부 끝점은 AG 데이터베이스에 연결 하기 위해 자동으로 프로 비전 됩니다. 이 끝점 `master-svc-external` AG 수신기의 역할을 수행 합니다.
...
다음 단계에서는 이러한 설정을 포함 하는 패치 파일을 만드는 방법과 `aks-dev-test` 또는 `kubeadm-dev-test` 구성 프로필에 적용 하는 방법을 보여 줍니다. 이러한 단계는 HA 특성을 추가 하기 위해 `aks-dev-test` 프로필을 패치 하는 방법에 대 한 예제를 안내 합니다. Kubeadm 클러스터에 대 한 배포의 경우 비슷한 패치가 적용 되지만, **끝점** 섹션에서 **ServiceType** 에 대해 *nodeport* 를 사용 하 고 있는지 확인 합니다.
...
주 복제본에 연결 하려면 `sql-server-master` 끝점을 사용 합니다. 이 끝점은 AG에 대 한 수신기 이기도 합니다. 모든 연결은 가용성 그룹의 컨텍스트에 있습니다. 예를 들어이 끝점을 사용 하는 기본 연결은 SQL Server 인스턴스 `master` 데이터베이스가 아니라 AG 내의 `master` 데이터베이스에 연결 됩니다.
...
- @No__t_0 끝점에 연결 하 여 주 복제본을 호스트 하는 pod를 확인 하 고 다음을 실행 합니다.
...
- 새 Kubernetes 서비스를 만들어 외부 끝점 노출
    ...
    Kubeadm 클러스터의 경우 아래 명령을 실행 합니다. @No__t_0을 이전 단계에서 반환 된 서버의 이름으로 바꾸고,를 만든 Kubernetes 서비스의 기본 이름으로 `serviceName` 하 고, `namespaceName` *를 BDC 클러스터의 이름과 바꿉니다.

    ```bash
    kubectl -n <namespaceName> expose pod <podName> --port=1533  --name=<serviceName> --type=NodePort
    ```

    Aks 클러스터를 실행 하는 경우에는 생성 되는 서비스의 형식이 `LoadBalancer` 된다는 점을 제외 하 고 동일한 명령을 실행 합니다. 예를 들어 

    ```bash
    kubectl -n <namespaceName> expose pod <podName> --port=1533  --name=<serviceName> --type=LoadBalancer
    ```

    다음은 aks에 대해 실행 되는이 명령의 예입니다. 여기에서 주 복제본을 호스팅하는 pod가 `master-0` 됩니다.
...
- 가용성 그룹에 데이터베이스를 추가합니다.
    ...
    다음 예에서는 인스턴스에서 복원 된 `sales` 라는 데이터베이스를 추가 합니다.
...
- @No__t_2 `RESTORE`와 같이 `CREATE DATABASE` 이외의 워크플로의 결과로 만들어진 데이터베이스는 가용성 그룹에 자동으로 추가 되지 않습니다. [인스턴스에 연결](#instance-connect) 하 고 데이터베이스를 가용성 그룹에 수동으로 추가 합니다.
- @No__t_0를 사용 하 여 서버 구성 설정을 실행 하는 등의 특정 작업을 수행 하려면 마스터 인스턴스에 연결 해야 합니다. 해당 하는 기본 끝점을 사용할 수 없습니다. [지침](#instance-connect) 에 따라 SQL Server 인스턴스에 연결 하 고 `sp_configure`를 실행 합니다.
