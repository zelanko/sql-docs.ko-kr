...
1. 메시지가 표시되면 다음 정보를 입력합니다.
   ...
   | **머신 크기** | AKS 클러스터의 노드에 사용할 [머신 크기](/azure/virtual-machines/windows/sizes)입니다(기본값 **Standard_D16s_v3**). |
   ...
   > [!IMPORTANT]
   > 일부 Azure 지역에서는 기본 머신 크기인 **Standard_D16s_v3**을 사용하지 못할 수도 있습니다. 다른 머신 크기를 선택하는 경우 클러스터의 노드에서 연결할 수 있는 총 디스크 수가 24개 이상인지 확인합니다. 클러스터의 영구적 볼륨 클레임마다 연결된 디스크 1개가 필요합니다. 현재, 빅 데이터 클러스터에는 24개의 영구적 볼륨 클레임이 필요합니다.
   >
   >사용 가능한 VM 유형을 식별하려면 다음 명령을 실행하세요.
   >
   ...
   > [!NOTE]
   > 빅 데이터 클러스터 배포 중에는 SQL Server `sa` 계정을 사용할 수 없습니다. 새 sysadmin 로그인은 **사용자 이름** 입력에 대해 지정된 것과 동일한 이름 및 **암호** 입력에 해당하는 암호로 SQL Server 마스터 인스턴스에 프로비저닝됩니다. 컨트롤러 관리 사용자를 프로비저닝하는 데 동일한 **사용자 이름** 및 **암호** 값이 사용됩니다. SQL Server 2019 CU5 이전에 배포된 클러스터에서 게이트웨이(Knox)에 지원되는 유일한 사용자는 **루트**이며 암호는 위와 동일합니다.
