...
## <a id="docker"></a> 기본 Docker 레지스트리, 리포지토리 및 이미지 태그 변경
...
## <a id="jsonpatch"></a> JSON 패치 파일을 사용한 기타 사용자 지정
