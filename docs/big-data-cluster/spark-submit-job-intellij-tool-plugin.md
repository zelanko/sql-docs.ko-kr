...
5. 에 **새 프로젝트** 창에서 다음 단계를 수행 합니다.  
   ...
   c. 에 **Spark 버전** 드롭 다운 목록에서 Scala 프로젝트 생성 마법사는 Spark SDK 및 Scala SDK에 대 한 적절 한 버전을 통합 합니다. Spark 클러스터 버전을 2.0 보다 이전 이면 선택 **Spark 1.x**합니다. 선택이 고, 그렇지 **spark 2.x**합니다. 이 예제에서는 **Spark (Scala 2.11.8) 2.0.2**합니다.
