...
description: OLE DB Driver for SQL Server의 ITableDefinition::CreateTable 함수에서 소비자가 SQL Server 테이블을 만들 수 있도록 하는 방법을 알아봅니다.
ms.custom: ''
ms.date: 06/14/2018
ms.prod: sql
...
## <a name="dbcolumndesc-usage"></a>DBCOLUMNDESC 사용  
 소비자는 *pwszTypeName* 멤버나 *wType* 멤버를 사용하여 열 데이터 형식을 나타낼 수 있습니다. 소비자가 *pwszTypeName*에 데이터 형식을 지정하면 OLE DB Driver for SQL Server가 *wType* 값을 무시합니다.  
 ...
 *pwszTypeName* 멤버를 사용하는 경우 소비자는 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 데이터 형식 이름을 사용하여 데이터 형식을 지정합니다. 올바른 데이터 형식 이름은 PROVIDER_TYPES 스키마 행 집합의 TYPE_NAME 열에 반환되는 값입니다.  
 ...
 OLE DB Driver for SQL Server는 *wType* 멤버에 있는 OLE DB가 열거하는 DBTYPE 값의 하위 집합을 인식합니다. 자세한 내용은 [ITableDefinition의 데이터 형식 매핑](../../oledb/ole-db-data-types/data-type-mapping-in-itabledefinition.md)을 참조하세요.  
