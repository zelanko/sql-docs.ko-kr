...
|속성 ID|설명|  
|-----------------|-----------------|  
|DBPROP_AUTH_CACHE_AUTHINFO|OLE DB Driver for SQL Server 인증 정보를 캐시 하지 않습니다.<br /><br /> OLE DB Driver for SQL Server 속성 값을 설정 하려고 할 때가 DB_S_ERRORSOCCURRED를 반환 합니다. DBPROP 구조의 *dwStatus* 멤버는 DBPROPSTATUS_NOTSUPPORTED를 나타냅니다.|  
|DBPROP_AUTH_ENCRYPT_PASSWORD|OLE DB Driver for SQL Server는 표준을 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 보안 메커니즘 암호를 숨깁니다.<br /><br /> OLE DB Driver for SQL Server 속성 값을 설정 하려고 할 때가 DB_S_ERRORSOCCURRED를 반환 합니다. DBPROP 구조의 *dwStatus* 멤버는 DBPROPSTATUS_NOTSUPPORTED를 나타냅니다.|  
...
|DBPROP_AUTH_MASK_PASSWORD|OLE DB Driver for SQL Server standard를 사용 하 여 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 보안 메커니즘 암호를 숨깁니다.<br /><br /> OLE DB Driver for SQL Server 속성 값을 설정 하려고 할 때가 DB_S_ERRORSOCCURRED를 반환 합니다. DBPROP 구조의 *dwStatus* 멤버는 DBPROPSTATUS_NOTSUPPORTED를 나타냅니다.|  
...
|DBPROP_AUTH_PERSIST_ENCRYPTED|OLE DB Driver for SQL Server에서 유지 하는 경우 인증 정보를 암호화 하지 않습니다.<br /><br /> OLE DB Driver for SQL Server 속성 값을 설정 하려고 할 때가 DB_S_ERRORSOCCURRED를 반환 합니다. DBPROP 구조의 *dwStatus* 멤버는 DBPROPSTATUS_NOTSUPPORTED를 나타냅니다.|  
...
|DBPROP_INIT_ASYNCH|OLE DB Driver for SQL Server는 비동기 시작을 지원합니다.<br /><br /> DBPROP_INIT_ASYNCH 속성에 DBPROPVAL_ASYNCH_INITIALIZE 비트를 설정하면 **IDBInitialize::Initialize**는 비블로킹 호출이 됩니다. 자세한 내용은 [비동기 작업 수행](../../oledb/features/performing-asynchronous-operations.md)합니다.|  
...
|DBPROP_INIT_IMPERSONATION_LEVEL|OLE DB Driver for SQL Server에는 가장 수준 조정을 지원 하지 않습니다.<br /><br /> OLE DB Driver for SQL Server 속성 값을 설정 하려고 할 때가 DB_S_ERRORSOCCURRED를 반환 합니다. DBPROP 구조의 *dwStatus* 멤버는 DBPROPSTATUS_NOTSUPPORTED를 나타냅니다.|  
|DBPROP_INIT_LCID|SQL Server용 OLE DB 드라이버는 로캘 ID의 유효성을 확인하고 로캘 ID가 지원되지 않거나 클라이언트에 설치되어 있지 않은 경우 오류를 반환합니다.|  
|DBPROP_INIT_LOCATION|OLE DB Driver for SQL Server 속성 값을 설정 하려고 할 때가 DB_S_ERRORSOCCURRED를 반환 합니다. DBPROP 구조의 *dwStatus* 멤버는 DBPROPSTATUS_NOTSUPPORTED를 나타냅니다.|  
|DBPROP_INIT_MODE|OLE DB Driver for SQL Server 속성 값을 설정 하려고 할 때가 DB_S_ERRORSOCCURRED를 반환 합니다. DBPROP 구조의 *dwStatus* 멤버는 DBPROPSTATUS_NOTSUPPORTED를 나타냅니다.|  
|DBPROP_INIT_PROMPT|OLE DB Driver for SQL Server 데이터 원본 초기화에 대 한 모든 확인 모드를 지원합니다. OLE DB Driver for SQL Server 속성에 대 한 기본 설정으로 DBPROMPT_NOPROMPT를 사용합니다.|  
|DBPROP_INIT_PROTECTION_LEVEL|OLE DB Driver for SQL Server 인스턴스의 연결에 대 한 보호 수준을 지원 하지 않습니다 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]합니다.<br /><br /> OLE DB Driver for SQL Server 속성 값을 설정 하려고 할 때가 DB_S_ERRORSOCCURRED를 반환 합니다. DBPROP 구조의 *dwStatus* 멤버는 DBPROPSTATUS_NOTSUPPORTED를 나타냅니다.|  
|DBPROP_INIT_PROVIDERSTRING|OLE DB 드라이버를이 항목의 뒷부분에 나오는 SQL Server 문자열을 참조 하세요.|  
...
```  
 ...
 OLE DB 드라이버와 SQL Server에 대 한 소비자는 공급자 문자열 속성을 사용 하려면 필요 하지 않습니다. 소비자는 OLE DB 또는 SQL Server용 OLE DB 드라이버별 초기화 속성을 사용하여 공급자 문자열에 반영된 모든 초기화 속성을 설정할 수 있습니다.  
  
 OLE DB 드라이버에서 SQL Server에 사용할 수 있는 키워드 목록은 참조 하세요 [OLE DB Driver for SQL Server를 사용 하 여 연결 문자열 키워드를 사용 하 여](../../oledb/applications/using-connection-string-keywords-with-oledb-driver-for-sql-server.md)입니다.  
