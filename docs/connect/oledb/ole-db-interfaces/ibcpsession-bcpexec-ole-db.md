---
title: IBCPSession::BCPExec(OLE DB 드라이버) | Microsoft Docs
description: OLE DB Driver for SQL Server에서 IBCPSession::BCPExec 메서드는 데이터를 사용자 파일과 데이터베이스 간에 복사합니다.
ms.custom: ''
ms.date: 06/14/2018
ms.prod: sql
...
## <a name="remarks"></a>설명  
 **BCPExec** 메서드는 [IBCPSession::BCPInit](../../oledb/ole-db-interfaces/ibcpsession-bcpinit-ole-db.md) 메서드에 사용된 *eDirection* 매개 변수 값에 따라 데이터를 사용자 파일에서 데이터베이스 테이블로 복사하거나 그 반대로 복사합니다.  
