---
title: ISSAsynchStatus::WaitForAsynchCompletion(OLE DB 드라이버) | Microsoft Docs
description: OLE DB Driver for SQL Server에서 ISSAsynchStatus::WaitForAsynchCompletion 메서드가 비동기 작업이 완료되거나 시간 초과될 때까지 대기하는 방법을 알아봅니다.
ms.custom: ''
ms.date: 06/14/2018
ms.prod: sql
...
> [!NOTE]  
>  OLE DB Service Component를 사용하는 경우 DB_S_ASYNCHRONOUS 대신 S_OK가 반환될 수 있으므로 S_OK 또는 DB_S_ASYNCHRONOUS가 반환되는 경우 애플리케이션은 [ISSAsynchStatus::GetStatus](../../oledb/ole-db-interfaces/issasynchstatus-getstatus-ole-db.md) 를 호출하여 완료 여부를 확인해야 합니다.  
