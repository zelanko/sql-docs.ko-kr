...
description: SQL Server 용 OLE DB 드라이버와 비동기 작업을 수행합니다.
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
## <a name="opening-and-closing-a-database-connection"></a>데이터베이스 연결 열기 및 닫기  
 OLE DB Driver for SQL Server를 사용할 때 데이터 원본 개체를 비동기적으로 초기화 하도록 설계 된 응용 프로그램 호출 하기 전에 DBPROP_INIT_ASYNCH 속성의 DBPROPVAL_ASYNCH_INITIALIZE 비트를 설정할 수 **idbinitialize:: Initialize** . 공급자를 호출 하 여 즉시 반환 하는이 속성이 설정 된 경우 **초기화** 작업이 즉시 완료 하면 s_ok이 고 또는 DB_S_ASYNCHRONOUS를 초기화 하는 비동기식으로 계속 됩니다. 응용 프로그램에 대해 쿼리할 수는 **IDBAsynchStatus** 또는 [ISSAsynchStatus](../../oledb/ole-db-interfaces/issasynchstatus-ole-db.md) 인터페이스 데이터 원본 개체에 대 한 다음 호출 **idbasynchstatus:: Getstatus** 또는 [ Issasynchstatus:: Waitforasynchcompletion](../../oledb/ole-db-interfaces/issasynchstatus-waitforasynchcompletion-ole-db.md) 초기화 상태를 가져올 수 있습니다.  
  
 또한 SSPROP_ISSAsynchStatus 속성이 DBPROPSET_SQLSERVERROWSET 속성 집합에 추가되었습니다. 지 원하는 공급자는 **ISSAsynchStatus** 인터페이스 variant_true 값으로이 속성을 구현 해야 합니다.  
  
 **Idbasynchstatus:: Abort** 또는 [issasynchstatus:: Abort](../../oledb/ole-db-interfaces/issasynchstatus-abort-ole-db.md) 비동기 취소 하기 위해 호출할 수 **초기화** 호출 합니다. 소비자는 비동기 데이터 원본 초기화를 명시적으로 요청해야 합니다. 그렇지 않으면 **idbinitialize:: Initialize** 데이터 원본 개체가 완전히 초기화 될 때까지 반환 하지 않습니다.  
  
> [!NOTE]  
>  호출할 수 없습니다. 연결 풀링에 사용 되는 데이터 원본 개체는 **ISSAsynchStatus** OLE DB 드라이버에서 SQL Server에 대 한 인터페이스입니다. **ISSAsynchStatus** 풀된에 있는 데이터 원본 개체에 대 한 인터페이스를 노출 되지 않습니다.  
>   
>  명시적으로 응용 프로그램 사용 하면 커서 엔진을 사용 하는 경우 **iopenrowset:: Openrowset** 및 **imultipleresults:: Getresult** 비동기 처리를 지원 하지 것입니다.  
>   
>  원격 프록시/스텁 dll (MDAC 2.8)에서 호출할 수 없습니다 또한는 **ISSAsynchStatus** OLE DB 드라이버에서 SQL Server에 대 한 인터페이스입니다. **ISSAsynchStatus** 인터페이스 원격을 통해 노출 되지 않습니다.  
>   
...
## <a name="execution-and-rowset-initialization"></a>실행 및 행 집합 초기화  
 ...
 메서드가 즉시 반환 하면 S_OK와 함께 행 집합을 비동기식으로 계속 되 면 즉시 또는 DB_S_ASYNCHRONOUS와 행 집합 초기화 완료 하는 경우와 *ppRowset* 에 요청된 된 인터페이스에 설정 된 행 집합입니다. 이 인터페이스는 OLE DB Driver for SQL Server에 대 한만 수 **IDBAsynchStatus** 또는 **ISSAsynchStatus**합니다. 이 인터페이스 동작을 마치는 일시 중단 된 상태 및 호출에는 행 집합이 완전히 초기화 될 때까지 **QueryInterface** 이외의 인터페이스에 대 한 **IID_IDBAsynchStatus** 또는 **IID_ ISSAsynchStatus** 하면 e_nointerface가 반환 될 수 있습니다. 소비자가 비동기 처리를 명시적으로 요청하지 않을 경우 행 집합은 동기식으로 초기화됩니다. 요청 된 모든 인터페이스를 사용할 수 **idbasynchstaus:: Getstatus** 또는 **issasynchstatus:: Waitforasynchcompletion** 비동기 작업이 완료 되었음을 나타내는 값을 반환 합니다. 이는 행 집합이 모두 채워지지는 않았지만 초기화가 완료되어 모든 기능이 올바로 작동함을 의미합니다.  
