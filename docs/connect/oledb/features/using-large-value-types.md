...
description: OLE DB Driver for SQL Server가 varchar(max), varbinary(max) 및 nvarchar(max) 형식을 사용하는 큰 값 데이터 형식을 지원하는 방법을 알아봅니다.
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
  ...
  [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] 이전에 큰 값 데이터 형식으로 작업하려면 특별한 처리가 필요했습니다. 큰 값 데이터 형식은 최대 행 크기가 8KB를 초과하는 데이터 형식입니다. [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)]에서는 **varchar**, **nvarchar** 및 **varbinary** 데이터 형식에 대해 **max** 지정자를 제공하여 값을 2^31 -1바이트로 스토리지할 수 있도록 합니다. 테이블 열과 [!INCLUDE[tsql](../../../includes/tsql-md.md)] 변수에서 **varchar(max)** , **nvarchar(max)** 또는 **varbinary(max)** 데이터 형식을 지정할 수 있습니다.  
...
-   DBTYPE_IUNKNOWN으로 바인딩하고 스트리밍을 사용합니다.  
 ...
 열의 최대 크기를 보고할 때 OLE DB Driver for SQL Server는 다음 중 하나를 보고합니다.  
...
```  
 ...
 OLE DB Driver for SQL Server서버가 큰 값 데이터 형식을 노출하는 방법에 관한 자세한 내용은 [BLOBs and OLE Objects](../../oledb/ole-db-blobs/blobs-and-ole-objects.md)를 참조하세요.  
