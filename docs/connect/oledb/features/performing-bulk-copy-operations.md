---
title: 대량 복사 작업 수행
description: OLE DB Driver for SQL Server에서 대량 복사 작업을 실행하여 데이터를 데이터베이스에 빠르게 전송할 수 있는 방법을 알아봅니다.
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
 ...
 이 기능을 보여 주는 샘플 애플리케이션은 [IRowsetFastLoad를 사용하여 데이터 대량 복사&#40;OLE DB&#41;](../../oledb/ole-db-how-to/bulk-copy-data-using-irowsetfastload-ole-db.md)를 참조하세요.  
...
## <a name="ole-db-driver-for-sql-server"></a>SQL Server용 OLE DB 드라이버 
 OLE DB Driver for SQL Server는 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 데이터베이스를 사용하여 대량 복사 작업을 수행하는 두 가지 방법을 구현합니다. 첫 번째 방법은 메모리 기반 대량 복사 작업에 [IRowsetFastLoad](../../oledb/ole-db-interfaces/irowsetfastload-ole-db.md) 인터페이스를 사용하고 두 번째 방법은 파일 기반 대량 복사 작업에 [IBCPSession](../../oledb/ole-db-interfaces/ibcpsession-ole-db.md) 인터페이스를 사용합니다.  
...
#### <a name="enabling-a-session-for-irowsetfastload"></a>IRowsetFastLoad에 세션 사용  
 소비자는 OLE DB Driver for SQL Server 관련 데이터 원본 속성 SSPROP_ENABLEFASTLOAD를 VARIANT_TRUE로 설정하여 대량 복사가 필요하다는 것을 OLE DB Driver for SQL Server에 알립니다. 소비자는 데이터 원본의 속성 집합을 사용하여 OLE DB Driver for SQL Server 세션을 만듭니다. 새 세션을 사용하여 소비자는 **IRowsetFastLoad** 인터페이스에 액세스할 수 있습니다.  
...
#### <a name="irowsetfastload-rowsets"></a>IRowsetFastLoad 행 집합  
 SQL Server용 OLE DB 드라이버 대량 복사 행 집합은 쓰기 전용이지만 소비자가 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 테이블의 구조를 결정할 수 있도록 하는 인터페이스를 노출합니다. 다음 인터페이스는 대량 복사 사용 OLE DB Driver for SQL Server 행 집합에 노출됩니다.  
