...
description: SQL Server에 대 한 OLE DB 드라이버를 사용 하 여 대량 복사 작업 수행
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
## <a name="ole-db-driver-for-sql-server"></a>SQL Server용 OLE DB 드라이버 
 SQL Server에 대 한 OLE DB 드라이버는 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 데이터베이스를 사용 하 여 대량 복사 작업을 수행 하는 두 가지 방법을 구현 합니다. 첫 번째 방법은 메모리 기반 대량 복사 작업에 [IRowsetFastLoad](../../oledb/ole-db-interfaces/irowsetfastload-ole-db.md) 인터페이스를 사용하고 두 번째 방법은 파일 기반 대량 복사 작업에 [IBCPSession](../../oledb/ole-db-interfaces/ibcpsession-ole-db.md) 인터페이스를 사용합니다.  
...
#### <a name="enabling-a-session-for-irowsetfastload"></a>IRowsetFastLoad에 세션 사용  
 소비자는 SQL Server 관련 데이터 원본 속성 SSPROP_ENABLEFASTLOAD에 대 한 OLE DB 드라이버를 VARIANT_TRUE로 설정 하 여 대량 복사에 대 한 요구를 SQL Server OLE DB 드라이버에 알립니다. 소비자는 데이터 원본에 설정 된 속성을 사용 하 여 SQL Server 세션의 OLE DB 드라이버를 만듭니다. 새 세션을 사용하여 소비자는 **IRowsetFastLoad** 인터페이스에 액세스할 수 있습니다.  
...
>  **IDataInitialize** 인터페이스가 데이터 원본 초기화에 사용되는 경우 **IOpenRowset::OpenRowset** 메서드의 *rgPropertySets* 매개 변수에 있는 SSPROP_IRowsetFastLoad 속성을 설정해야 합니다. 그렇지 않으면 **OpenRowset** 메서드를 호출할 때 E_NOINTERFACE가 반환됩니다.  
 ...
 대량 복사에 대해 세션을 사용 하도록 설정 하면 세션의 인터페이스에 대 한 SQL Server 지원 OLE DB 드라이버가 제한 됩니다. 대량 복사 사용 세션은 다음과 같은 인터페이스만 노출합니다.  
...
#### <a name="irowsetfastload-rowsets"></a>IRowsetFastLoad 행 집합  
 SQL Server용 OLE DB 드라이버 대량 복사 행 집합은 쓰기 전용이지만 소비자가 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 테이블의 구조를 결정할 수 있도록 하는 인터페이스를 노출합니다. 다음 인터페이스는 SQL Server 행 집합에 대 한 대량 복사 사용 OLE DB 드라이버에서 노출 됩니다.  
