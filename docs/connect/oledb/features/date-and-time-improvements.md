...
description: SQL Server에 대 한 OLE DB 드라이버의 날짜 및 시간 기능 향상
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
  ...
  이 항목에서는에 [!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]추가 된 날짜 및 시간 데이터 형식에 대 한 SQL Server 지원 OLE DB 드라이버에 대해 설명 합니다.  
  
 날짜/시간 기능 향상에 대 한 자세한 내용은 [OLE DB&#41;날짜 및 시간 &#40;향상](../../oledb/ole-db-date-time/date-and-time-improvements-ole-db.md)을 참조 하세요.  
...
### <a name="use-date-as-a-distinct-data-type"></a>고유 데이터 형식으로 Date 사용  
 [!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]부터 날짜/시간 형식에 대 한 향상 된 지원을 통해 DBTYPE_DBDATE OLE DB 형식을 보다 효율적으로 사용할 수 있습니다.  
  
### <a name="use-time-as-a-distinct-data-type"></a>고유 데이터 형식으로 Time 사용  
 OLE DB에는 정밀도가 1초인 시간(DBTYPE_DBTIME)만 포함하는 데이터 형식이 이미 있습니다.
  
 새 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] time 데이터 형식은 100나노초까지 정확한 소수 자릿수 초를 사용합니다. 이렇게 하려면 SQL Server에 대 한 OLE DB 드라이버에 새 형식이 필요 합니다. DBTYPE_DBTIME2. 소수 자릿수 초가 없는 시간을 사용하도록 작성된 기존 애플리케이션은 time(0) 열을 사용할 수 있습니다. 애플리케이션에서 메타데이터로 반환된 형식을 사용하지 않는 한 기존 OLE DB DBTYPE_TIME 형식과 해당 구조체는 올바로 작동해야 합니다.  
...
### <a name="use-time-as-a-distinct-data-type-with-extended-fractional-seconds-precision"></a>확장된 초 소수 부분 자릿수의 고유 데이터 형식으로 Time 사용  
 프로세스 제어 및 제조 애플리케이션과 같은 일부 애플리케이션에는 정밀도가 최대 100나노초인 시간 데이터를 처리하는 기능이 필요합니다. OLE DB에서이 용도의 새 형식은 DBTYPE_DBTIME2입니다.  
...
### <a name="use-datetime-with-extended-fractional-seconds-precision-and-timezone"></a>확장된 초 소수 부분 자릿수 및 표준 시간대의 Datetime 사용  
 일부 애플리케이션에는 표준 시간대 정보가 포함된 datetime 값이 필요합니다. 새 DBTYPE_DBTIMESTAMPOFFSET 형식에서 지원 됩니다.
