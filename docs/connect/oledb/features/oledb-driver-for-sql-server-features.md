...
보다 안전한 기본 설정이 있으며 페더레이션 id를 사용 하 여 Azure SQL Database 인스턴스에 대 한 연결을 허용 하는 OLE DB driver 18.2.1에 도입 된 새로운 인증 방법에 대해 설명 합니다.

 [MARS&#40;Multiple Active Result Sets&#41; 사용](../../oledb/features/using-multiple-active-result-sets-mars.md)  
 SQL Server 드라이버를 OLE DB 하 여 MARS (multiple active result sets)를 지 원하는 방법에 대해 설명 합니다. MARS를 사용하면 단일 데이터베이스 연결을 통해 여러 결과 집합을 실행 및 검색할 수 있습니다.  
 ...
 OLE DB Driver for SQL Server에서 LOB (large object data types) 인 큰 값 데이터 형식을 지 원하는 방법에 대해 설명 합니다.  
 ...
 OLE DB Driver for SQL Server가 행 집합 수정에 대 한 소비자 알림을 지 원하는 방법에 대해 설명 합니다.  
 ...
 SQL Server에 대 한 OLE DB 드라이버를 사용 하 여 인증서의 유효성을 검사 하지 않고 서버로 전송 된 데이터를 암호화 하는 방법을 설명 합니다.  
  
 [테이블 반환 매개 변수&#40;SQL Server용 OLE DB 드라이버&#41;](../../oledb/features/table-valued-parameters-oledb-driver-for-sql-server.md)  
 테이블 반환 매개 변수를 SQL Server 지원 하기 위한 OLE DB 드라이버에 대해 설명 합니다.  
 ...
 향상 된 FILESTREAM 기능에 대 한 SQL Server 지원 OLE DB 드라이버에 대해 설명 합니다.  
 ...
 스파스 열에 대 한 SQL Server 지원 OLE DB 드라이버에 대해 설명 합니다.  
  
 [날짜 및 시간 기능 향상](../../oledb/features/date-and-time-improvements.md)  
 날짜 및 시간 데이터 형식에 대 한 SQL Server에 대 한 OLE DB 드라이버에 추가 된 지원을 설명 합니다.  
 ...
 U t f-8로 인코딩된 데이터로 작업할 때 사용자가 수행 해야 하는 UTF-8 서버 인코딩 및 구성 주의 사항에 대해 설명 합니다.
 ...
 LocalDB 기능에 대 한 SQL Server 지원 OLE DB 드라이버에 대해 설명 합니다.  
