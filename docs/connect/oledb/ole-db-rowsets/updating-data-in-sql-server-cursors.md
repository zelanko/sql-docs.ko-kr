...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
 ...
 트랜잭션이 오랫동안 열려 있을 수 있도록 설계된 클라이언트 응용 프로그램의 경우 트랜잭션 격리 수준 때문에 행 위치 지정에 상당한 시간 지연이 발생할 수 있습니다. 기본적으로 SQL Server용 OLE DB 드라이버는 DBPROPVAL_TI_READCOMMITTED로 지정되는 커밋된 읽기 격리 수준을 사용합니다. SQL Server에 대 한 OLE DB 드라이버는 행 집합 동시성이 읽기 전용일 때 커밋되지 않은 읽기 격리를 지원 합니다. 따라서 소비자는 수정이 가능한 행 집합에 더 높은 수준의 격리를 요청할 수 있지만 더 낮은 수준은 요청할 수 없습니다.  
 ...
 **IRowsetUpdate:: Update**를 사용 하는 경우 SQL Server의 OLE DB 드라이버는 표시 된 각 행을 처리 하려고 합니다. 행에서 잘못된 데이터, 길이 또는 상태 값 때문에 오류가 발생하더라도 SQL Server용 OLE DB 드라이버는 처리를 중지하지 않습니다. 업데이트에 참여하는 행은 모두 수정되거나 전혀 수정되지 않습니다. SQL Server용 OLE DB 드라이버가 DB_S_ERRORSOCCURRED를 반환하면 소비자는 반환된 *prgRowStatus* 배열을 조사하여 문제가 발생한 행이 있는지 확인해야 합니다.  
