...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
 ...
 트랜잭션이 오랫동안 열려 있을 수 있도록 설계된 클라이언트 애플리케이션의 경우 트랜잭션 격리 수준 때문에 행 위치 지정에 상당한 시간 지연이 발생할 수 있습니다. 기본적으로 SQL Server용 OLE DB 드라이버는 DBPROPVAL_TI_READCOMMITTED로 지정되는 커밋된 읽기 격리 수준을 사용합니다. SQL Server에 대 한 OLE DB 드라이버는 행 집합 동시성이 읽기 전용일 때 커밋되지 않은 읽기 격리를 지원 합니다. 따라서 소비자는 수정이 가능한 행 집합에 더 높은 수준의 격리를 요청할 수 있지만 더 낮은 수준은 요청할 수 없습니다.  
