...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
  ...
  이 항목에서는 클라이언트 응용 프로그램의 SPN(서비스 사용자 이름)을 지원하는 OLE DB 속성 및 멤버 함수에 대해 설명합니다. SPN 및 Kerberos 인증에 대한 자세한 내용은 [클라이언트 연결의 서비스 보안 주체 이름&#40;SPN&#41; 지원](../../oledb/features/service-principal-name-spn-support-in-client-connections.md)을 참조하십시오. 샘플을 보려면 [통합 Kerberos 인증 &#40;OLE DB&#41;](../../oledb/ole-db-how-to/integrated-kerberos-authentication-ole-db.md)합니다.  
...
|키워드|설명|값|  
|-------------|-----------------|-----------|  
|ServerSPN|서버 SPN|서버의 SPN입니다. 기본값은 SPN 그러면 OLE DB Driver for SQL Server 기본값을 사용 하려면 빈 문자열인 경우 공급자가 생성 합니다.|  
|FailoverPartnerSPN|장애 조치(Failover) 파트너 SPN|장애 조치(failover) 파트너의 SPN입니다. 기본값은 SPN 그러면 OLE DB Driver for SQL Server 기본값을 사용 하려면 빈 문자열인 경우 공급자가 생성 합니다.|  
...
|속성|형식|사용법|  
|----------|----------|-----------|  
|SSPROP_INIT_SERVERSPN|VT_BSTR, 읽기/쓰기|서버의 SPN을 지정합니다. 기본값은 SPN 그러면 OLE DB Driver for SQL Server 기본값을 사용 하려면 빈 문자열인 경우 공급자가 생성 합니다.|  
|SSPROP_INIT_FAILOVERPARTNERSPN|VT_BSTR, 읽기/쓰기|장애 조치(Failover) 파트너의 SPN을 지정합니다. 기본값은 SPN 그러면 OLE DB Driver for SQL Server 기본값을 사용 하려면 빈 문자열인 경우 공급자가 생성 합니다.|  
...
|속성|형식|사용법|  
|----------|----------|-----------|  
|SSPROP_INTEGRATEDAUTHENTICATIONMETHOD|VT_BSTR, 읽기 전용|연결에 사용된 인증 방법을 반환합니다. 응용 프로그램에 반환 되는 값에는 Windows SQL Server 용 OLE DB 드라이버에 반환 하는 값이입니다. 다음은 가능한 값입니다. <br />NTLM 인증을 사용하여 연결을 열 때 반환되는 "NTLM"<br />Kerberos 인증을 사용하여 연결을 열 때 반환되는 "Kerberos"<br /><br /> 연결이 열려 있지만 인증 방법을 확인할 수 없는 경우에는 VT_EMPTY가 반환됩니다.<br /><br /> 이 속성은 데이터 원본이 초기화된 경우에만 읽을 수 있습니다. 데이터 원본이 초기화되기 전에 이 속성을 읽으려고 하면 IDBProperties::GetProperies에서 DB_S_ERRORSOCCURRED 또는 DB_E_ERRORSOCCURRED가 반환되고 이 속성의 DBPROPSET_PROPERTIESINERROR에 DBPROPSTATUS_NOTSUPPORTED가 설정됩니다. 이 동작은 OLE DB 핵심 사양을 따르는 것입니다.|  
