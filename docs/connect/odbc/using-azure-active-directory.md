...
Microsoft ODBC Driver for SQL Server 버전 13.1 이상에서는 ODBC 애플리케이션이 Active Directory의 페더레이션된 ID로 사용자 이름/암호, Azure Active Directory 액세스 토큰, Azure Active Directory 관리 ID(17.3+) 또는 Windows 통합 인증(Linux/macOS에서 17.6+)을 사용하여 Azure SQL Database 인스턴스에 연결할 수 있습니다. ODBC 드라이버 버전 13.1의 경우 Azure Active Directory 액세스 토큰 인증은 _Windows 전용_입니다. ODBC 드라이버 버전 17 이상에서는 모든 플랫폼(Windows, Linux 및 macOS)에서 이 인증을 지원합니다. 로그인 ID를 사용하는 새로운 Azure Active Directory 대화형 인증은 Windows용 ODBC 드라이버 버전 17.1에서 도입되었습니다. 시스템 할당 ID와 사용자 할당 ID 모두에 대해 새로운 Azure Active Directory 관리 ID 인증 방법이 ODBC 드라이버 버전 17.3.1.1에서 추가되었습니다. 이러한 기능은 모두 새 DSN 및 연결 문자열 키워드를 사용하고 연결 특성을 사용하여 수행됩니다.

> [!NOTE]
> 버전 17.6보다 이전의 Linux 및 macOS용 ODBC 드라이버는 Azure Active Directory에 대해 직접 Azure Active Directory 인증만을 지원합니다. Linux 또는 macOS 클라이언트에서 Azure Active Directory 사용자 이름/암호 인증을 사용하고 Active Directory 구성에서 Active Directory Federation Services 엔드포인트에 대해 클라이언트를 인증해야 하는 경우 인증이 실패할 수 있습니다. 드라이버 버전 17.6부터 이 제한이 제거되었습니다.
...
`Authentication=ActiveDirectoryIntegrated` - Azure SQL Database에 대한 Azure Active Directory 통합 인증 지원

![Azure Active Directory 통합 인증이 선택된 DSN 만들기 및 편집 화면.](windows/create-dsn-ad-integrated.png)

`Authentication=ActiveDirectoryPassword` - Azure SQL Database에 대한 Azure Active Directory 사용자 이름/암호 인증 지원

![Azure Active Directory 암호 인증이 선택된 DSN 만들기 및 편집 화면.](windows/create-dsn-ad-password.png)

`Authentication=ActiveDirectoryInteractive` - Azure SQL Database에 대한 Azure Active Directory 대화형 인증 지원

![Azure Active Directory 대화형 인증이 선택된 DSN 만들기 및 편집 화면.](windows/create-dsn-ad-interactive.png)

`Authentication=SqlPassword` SQL Server에 대한 사용자 이름/암호 인증(Azure 또는 기타)

![SQL Server 인증이 선택된 DSN 만들기 및 편집 화면.](windows/create-dsn-ad-sql-server.png)

`Trusted_Connection=Yes` Windows 레거시 SSPI 통합 인증

![Windows 통합 인증이 선택된 DSN 만들기 및 편집 화면.](windows/create-dsn-win-sspi.png)

`Authentication=ActiveDirectoryMsi` - Azure Active Directory 관리 ID 인증 지원

![관리 서비스 ID 인증이 선택된 DSN 만들기 및 편집 화면.](windows/create-dsn-ad-msi.png)

이 6가지 옵션은 `Trusted_Connection=Yes`(기존 레거시 Windows SSPI 전용 통합 인증) 및 `Authentication=` `ActiveDirectoryIntegrated`, `SqlPassword`, `ActiveDirectoryPassword`, `ActiveDirectoryInteractive` 및 `ActiveDirectoryMsi`에 각각 해당합니다.

### <a name="sqldriverconnect-prompt-windows-driver-only"></a>SQLDriverConnect 프롬프트(Windows 드라이버만 해당)

SQLDriverConnect에서 연결을 완료하는 데 필요한 정보를 요청할 때 표시되는 프롬프트 대화 상자에는 Azure AD 인증을 위한 네 가지 새로운 옵션이 포함되어 있습니다.

![SQLDriverConnect에 의해 표시되는 SQL Server 로그인 대화 상자입니다.](windows/server-login.png)

이러한 옵션은 위의 DSN 설정 UI에서 사용할 수 있는 것과 동일한 6가지 옵션에 해당합니다.
...
5. Azure Active Directory 사용자 이름/암호 인증(대상 데이터베이스가 Azure SQL Database에 있는 경우). 암호화 설정에 관계없이 서버 인증서의 유효성을 검사합니다(`TrustServerCertificate`를 `true`로 설정하지 않은 경우). 사용자 이름/암호가 연결 문자열에 전달됩니다. 
`server=Server;database=Database;UID=UserName;PWD=Password;Authentication=ActiveDirectoryPassword;`
6. (Windows 드라이버 및 Linux/macOS 17.6+ 드라이버만 해당.) Windows 계정 자격 증명을 Azure AD 발급 액세스 토큰으로 교환하는 ADAL 또는 Kerberos를 사용하는 Windows 통합 인증(대상 데이터베이스가 Azure SQL Database에 있는 것으로 가정). 암호화 설정에 관계없이 서버 인증서의 유효성을 검사합니다(`TrustServerCertificate`를 `true`로 설정하지 않은 경우). Linux/macOS에서는 적절한 Kerberos 티켓을 사용할 수 있어야 합니다. 자세한 내용은 아래의 페더레이션 계정에 관한 섹션 및 [통합 인증 사용](linux-mac/using-integrated-authentication.md)을 참조하세요.
`server=Server;database=Database;Authentication=ActiveDirectoryIntegrated;`
7. (_Windows 드라이버만 해당_.) Azure AD 대화형 인증은 Azure Multi-factor Authentication 기술을 사용하여 연결을 설정합니다. 이 모드에서 로그인 ID를 제공하면 Azure 인증 대화 상자가 트리거되고 사용자가 암호를 입력하여 연결을 완료할 수 있습니다. 사용자 이름은 연결 문자열에서 전달됩니다.
`server=Server;database=Database;UID=UserName;Authentication=ActiveDirectoryInteractive;`

![Active Directory 대화형 인증을 사용하는 경우 Windows Azure 인증 UI.](windows/WindowsAzureAuth.png)

8. Azure Active Directory 관리 ID 인증은 인증에 시스템 할당 또는 사용자 할당 ID를 사용하여 연결을 설정합니다. 사용자 할당 ID의 경우 UID를 사용자 ID의 개체 ID로 설정합니다.<br>
시스템 할당 ID의 경우<br>
...
>- Azure Active Directory 구성에 조건부 액세스 정책이 포함되어 있고 클라이언트가 Windows 10 또는 Server 2016 이상인 경우에는 통합 또는 사용자 이름/암호를 통한 인증이 실패할 수 있습니다. 조건부 액세스 정책에는 Windows용 드라이버 버전 17.6 이상에서 지원되는 WAM(Windows 계정 관리자)를 사용해야 합니다. WAM을 사용하려면 전역, 사용자 DSN 또는 시스템 DSN 범위 구성 각각에 대해 `HKLM\Software\ODBC\ODBCINST.INI\ODBC Driver 17 for SQL Server`, `HKCU\Software\ODBC\ODBC.INI\<your-user-DSN-name>` 또는 `HKLM\Software\ODBC\ODBC.INI\<your-system-DSN-name>`에 `ADALuseWAM`라는 새 문자열 또는 DWORD 값을 만들고 값을 1로 설정합니다. WAM을 사용하는 인증은 `runas`를 사용하여 다른 사용자로 애플리케이션을 실행하는 것을 지원하지 않습니다. Linux 또는 macOS에서는 조건부 액세스 정책이 필요한 시나리오가 지원되지 않습니다.
>- SQL Server 계정 사용자 이름 및 암호를 사용하여 연결하려면 이제 새 `SqlPassword` 옵션을 사용할 수 있습니다. 이 옵션을 통해 보다 안전한 연결 기본값을 사용할 수 있으므로 특히 Azure SQL에서 이 옵션을 사용하는 것이 좋습니다.
...
>- Windows 통합 인증 또는 Active Directory 통합 인증(Windows 드라이버 및 Linux/macOS 17.6+ 드라이버만 해당)을 사용하여 연결하려면 연결 문자열에 `Authentication=ActiveDirectoryIntegrated`를 지정합니다. 드라이버에서 자동으로 올바른 인증 모드를 선택합니다. `UID` 및 `PWD`는 지정하지 않아야 합니다.
...
다음 샘플에서는 연결 키워드와 Azure Active Directory를 사용하여 SQL Server에 연결하는 데 필요한 코드를 보여 줍니다. 애플리케이션 코드 자체를 변경할 필요는 없습니다. 연결 문자열 또는 DSN을 사용하는 경우 인증을 위해 Azure AD를 사용하는 데 필요한 유일한 수정 사항은 다음과 같습니다.
~~~
    ...
...
~~~
다음은 Azure Active Directory 관리 ID 인증에 사용할 수 있는 샘플 연결 문자열입니다. 사용자 할당 ID의 경우 UID를 사용자 ID의 개체 ID로 설정합니다.
~~~
...
## <a name="considerations-for-using-adfs-federated-accounts-on-linuxmacos"></a>Linux/macOS에서 ADFS 페더레이션 계정을 사용하는 경우의 고려 사항

버전 17.6부터 Linux 및 macOS용 드라이버는 사용자 이름/암호(`ActiveDirectoryPassword`) 또는 Kerberos(`ActiveDirectoryIntegrated`)를 사용하여 Azure Active Directory ADFS 페더레이션 계정을 통한 인증을 지원합니다. 통합 모드를 사용하는 경우 플랫폼에 따라 몇 가지 제한 사항이 있습니다.

UPN 접미사가 Kerberos 영역과 다른 사용자(즉, 대체 UPN 접미사를 사용하는 경우)를 사용하여 인증하는 경우 Kerberos 티켓을 가져올 때 엔터프라이즈 보안 주체 옵션을 사용해야 합니다(`kinit`와 함께 `-E` 옵션을 사용하고 `user@federated-domain` 형식으로 보안 주체 이름을 제공). 이렇게 하면 드라이버가 페더레이션된 도메인과 Kerberos 영역을 모두 올바르게 확인할 수 있습니다.

`klist` 명령의 출력을 검사하여 적절한 Kerberos 티켓을 사용할 수 있는지 확인할 수 있습니다. 페더레이션된 도메인이 Kerberos 영역 및 UPN 접미사와 같으면 사용자 이름은 `user@realm` 형식이 됩니다. 다른 경우에는 보안 주체 이름이 `user@federated-domain@realm` 형식이어야 합니다.

### <a name="linux"></a>Linux

SuSE 11에서 1.6.x의 기본 Kerberos 라이브러리 버전은 대체 UPN 접미사를 사용하는 데 필요한 엔터프라이즈 보안 주체 옵션을 지원하지 않습니다. Azure AD 통합 인증에 대체 UPN 접미사를 사용하려면 Kerberos 라이브러리를 1.7 이상으로 업그레이드합니다.

Alpine Linux에서 기본 `libcurl`은 Azure AD 통합 인증에 필요한 SPNEGO/Kerberos 인증을 지원하지 않습니다.

### <a name="macos"></a>macOS

시스템 Kerberos 라이브러리 `kinit`는 `--enterprise` 옵션으로 엔터프라이즈 보안 주체를 지원하지만 이름 정규화를 암시적으로 수행하므로 대체 UPN 접미사를 사용할 수 없습니다. Azure AD 통합 인증에 대체 UPN 접미사를 사용하려면 `brew install krb5`를 통해 최신 Kerberos 라이브러리를 설치하고 위에 설명된 대로 `-E` 옵션으로 `kinit`를 사용합니다.

## <a name="see-also"></a>참고 항목

[Azure AD 인증을 사용하는 Azure SQL Database에 대한 토큰 기반 인증 지원](/archive/blogs/sqlsecurity/token-based-authentication-support-for-azure-sql-db-using-azure-ad-auth)

[통합 인증 사용](linux-mac/using-integrated-authentication.md)
