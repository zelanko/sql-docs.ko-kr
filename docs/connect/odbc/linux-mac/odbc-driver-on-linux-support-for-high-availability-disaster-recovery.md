...
DNS 서버의 첫 번째 반환 된 IP 주소 연결 가능 하지 않은, 경우에 이러한 반복 시간이 걸릴 수 있습니다. 가용성 그룹 수신기에 연결할 경우 드라이버가 모든 IP 주소에 대해 병렬 연결을 설정하려고 합니다. 연결 시도에 성공하면 드라이버가 모든 보류 중인 연결 시도를 삭제합니다.
...
-   지정할 **MultiSubnetFailover = Yes** 단일 서브넷 또는 다중 서브넷 가용성 그룹에 연결할 때 성능을 향상 시키기 위해.
...
[!INCLUDE[ssHADR](../../../includes/sshadr_md.md)]을 사용하는 ODBC 응용 프로그램은 연결을 만들 때 두 가지 함수 중 하나를 사용할 수 있습니다.  
