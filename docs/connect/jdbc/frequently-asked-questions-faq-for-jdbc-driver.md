...
ms.locfileid: "35452163"
---
# <a name="frequently-asked-questions-faq-for-jdbc-driver"></a>질문과 대답 (FAQ) JDBC 드라이버에 대 한
[!INCLUDE[Driver_JDBC_Download](../../includes/driver_jdbc_download.md)]

  이 페이지에서는 SQL Server 용 Microsoft JDBC Driver에 대 한 자주 묻는 질문에 대 한 답을 제공 합니다.  
...
**Microsoft 다운로드 센터에서 사용할 수 있는 JDBC 드라이버 패키지와 GitHub에서 사용할 수 있는 JDBC 드라이버의 차이 무엇입니까?**  
 JDBC 드라이버에 있는 파일 GitHub 리포지토리 Microsoft JDBC driver에 대 한 JDBC 드라이버의 핵심 이므로 저장소에 나열 된 오픈 소스 라이선스에 따라 있습니다. 드라이버 패키지는 Microsoft 다운로드 센터에 Windows 통합 인증 및 JDBC 드라이버와 함께 사용할 수 있도록 XA 트랜잭션에 대해 추가 라이브러리를 포함 합니다. 다운로드 가능 패키지에 포함 된 라이선스에 따라 이러한 추가 있는지 확인 합니다.
 ...
 **Linux 컴퓨터에서 Microsoft SQL Server에 액세스할 수는 드라이버를 사용할 수 있습니까?** 예 이 드라이버를 사용하여 Linux, Unix 및 기타 Windows 이외의 플랫폼에서 SQL Server에 액세스할 수 있습니다. 자세한 내용은 참조 [Microsoft JDBC Driver for SQL Server 지원 매트릭스](../../connect/jdbc/microsoft-jdbc-driver-for-sql-server-support-matrix.md)합니다.
 ...
 **SQL Server 용 Microsoft JDBC 드라이버에서 지원 되는 인증 유형은?**  
 다음 표에는서 사용할 수 있는 인증 옵션을 보여 줍니다. 순수한 Java Kerberos 인증 사용 가능한 드라이버의 4.0 버전부터 됩니다.  
...
**드라이버가 인터넷 프로토콜 버전 6 (IPv6)를 지원 하나요 주소?**  
 예 드라이버는 IPv6 주소 사용을 지원합니다. 연결 속성 컬렉션 및 serverName 연결 문자열 속성을 사용 합니다. 자세한 내용은 참조 [연결 URL 작성](../../connect/jdbc/building-the-connection-url.md)합니다.  
  
**선택 버퍼링 하는 것이 무엇입니까?**  
 선택 버퍼링 Microsoft SQL Server 2005 JDBC 드라이버 버전 1.2부터 도입 되었습니다. 모든 유형의 서버 커서 오버 헤드 없이 큰 값 데이터를 검색 하는 설계 되었습니다. Microsoft SQL Server JDBC Driver의 선택 버퍼링 기능은 "선택" 또는 "전체"로 설정될 수 있는 responseBuffering 연결 문자열 속성을 제공합니다. 버전 1.2 릴리스에서 버퍼링 모드가 기본적으로 "전체" 이며 응용 프로그램 선택 버퍼링 모드를 명시적으로 설정 해야 합니다. JDBC Driver 버전 2.0부터, 이 드라이버의 기본 동작은 "선택"입니다. 따라서 응용 프로그램에서 적응 버퍼링 동작을 명시적으로 선택 동작을 요청할 필요가 없습니다. 자세한 내용은 참조 [선택 버퍼링 사용 하 여](../../connect/jdbc/using-adaptive-buffering.md) 및 블로그 [어떤 버퍼링 이란 무엇 이며 왜 사용 해야 합니까?](http://go.microsoft.com/fwlink/?LinkId=111575)합니다.
