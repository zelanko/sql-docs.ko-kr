...
| JAR                       | JDBC 사양     | JDK 버전 |
| ------------------------- | ---------------------- | ----------- |
| mssql-jdbc-포함 되어 있습니다 | JDBC 4.2, 4.1 및 4.0 | JDK 8.0     |
| mssql-6.2.2.jre7.jar | JDBC 4.1 및 4.0       | JDK 7.0     |
...
**기존에 사용하던 SQL Server 버전에서 최신 드라이버를 사용하기 위해 내 응용 프로그램에서 코드를 변경해야 하나요?**  
일반적으로 드라이버는 이전 버전과 호환되도록 설계되므로 드라이버를 업그레이드할 때 기존 응용 프로그램을 변경할 필요는 없습니다. 새 드라이버 버전에는 주요 변경 사항이 도입 되었습니다 합니다 [JDBC 드라이버에 대 한 릴리스](../../connect/jdbc/release-notes-for-the-jdbc-driver.md) 섹션 변경 및 기존 응용 프로그램에 미치는 영향에 일반 세부 정보를 제공 합니다. 또한 드라이버에 포함된 릴리스 정보에서 해당 릴리스에서 수정된 버그 목록과 알려진 문제를 검토할 수 있습니다.
