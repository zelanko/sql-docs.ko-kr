...
|columnEncryptionSetting|문자열<br /><br /> ["Enabled"&#124;"Disabled"]|사용 안 함|SQL Server 용 Microsoft JDBC Driver 6.0 상시 암호화 (AE) 기능을 시작 하 여 사용 하도록 "사용"으로 설정 합니다. AE를 사용하도록 설정하면 JDBC 드라이버가 SQL Server의 암호화된 데이터베이스 열에 저장되어 있는 중요한 데이터를 투명하게 암호화하고 암호를 해독합니다.<br /><br /> 참조 [상시 암호화와 JDBC 드라이버를 사용 하 여](../../connect/jdbc/using-always-encrypted-with-the-jdbc-driver.md) 내용을 확인 합니다.<br /><br /> **참고:** 상시 암호화는 SQL Server 2016 이상을 사용할 수 있습니다.|  
|databaseName, database|문자열<br /><br /> [<=128자]|null|연결할 데이터베이스 이름입니다. 지정하지 않으면 기본 데이터베이스에 연결합니다.|  
|disableStatementPooling|boolean<br /><br /> ["true"&#124;"false"]|true|문 풀링을 사용 하는 경우를 나타내는 플래그입니다.|  
|enablePrepareOnFirstPreparedStatementCall|boolean<br /><br /> ["true"&#124;"false"]|false|이 구성은 false로 설정 되어 준비 된 문을 첫 번째 실행에서 sp_executesql을 호출 및 sp_prepexec 호출 발생 하는 두 번째로 실행 되 면 문을 준비 하지 않으며 실제로 준비 된 문 핸들을 설정 합니다.|
...
|failoverPartner|문자열|null|데이터베이스 미러링 구성에서 사용되는 장애 조치 서버의 이름입니다. 이 속성은 주 서버에 대한 초기 연결 실패 시 사용되며 초기 연결이 이루어진 후에는 무시됩니다. databaseName 속성과 함께 사용해야 합니다.<br /><br /> **참고:** 드라이버 연결 문자열에서 failoverPartner 속성의 일부로 장애 조치 파트너 인스턴스에 대 한 서버 인스턴스의 포트 번호를 지정할 수 없습니다. 그러나 파트너 인스턴스는 동일한 연결 문자열에서 장애 조치는 주 서버 인스턴스와 failoverPartner 속성의 serverName, instanceName 및 portNumber 속성 지정 지원 됩니다.<br /><br /> 에 가상 네트워크 이름을 지정 하는 경우는 **서버** 연결 속성을 데이터베이스 미러링을 사용할 수 없습니다. 참조 [고가용성, 재해 복구에 대 한 JDBC 드라이버 지원](../../connect/jdbc/jdbc-driver-support-for-high-availability-disaster-recovery.md) 자세한 정보에 대 한 합니다.|  
|fips|부울 값을 ["true / false"]|"false"|이 속성은 같아야 FIPS JVM을 사용 하도록 설정 된 **true**합니다.|
|fipsProvider|문자열|null|JVM에 구성 된 FIPS 공급자입니다. 예를 들어 BCFIPS 또는 SunPKCS11 NSS 합니다. 세부 정보를 보려면 6.4.0-버전에서 제거 [여기](https://github.com/Microsoft/mssql-jdbc/pull/460) |
...
|serverPreparedStatementDiscardThreshold|정수|10|개수 뛰어난 준비 동작 (sp_unprepare) 서버의 미처리 핸들을 정리에 대 한 호출을 실행 하기 전에 연결 별로 처리 중인 있을 수 있으므로 문을 삭제를 제어 합니다. 설정 되어 있으면 < = 1 준비 작업 준비 문에서 닫기 즉시 실행 됩니다. 로 설정 되어 있으면 > 이러한 호출은 sp_unprepare를 너무 자주 호출의 오버 헤드를 방지 하기 위해 함께 일괄 처리 되는 1입니다.|
...
|statementPoolingCacheSize|int|0|문 풀링에 대 한 캐시 크기를 정의합니다. 0 캐싱 안 함을 의미합니다.|
|socketTimeout|int|0|소켓에서 초과 하기 전에 기다릴 밀리초 수 읽기 하거나 적용 합니다. 기본값은 0으로, 제한 시간 무한대를 의미 합니다.|
|sslProtocol|문자열|TLS|6.4.0 릴리스에서 추가합니다. TLS 프로토콜 키워드를 지정 하려면이 값을 설정 합니다. 가능한 값은: **TLS**, **TLSv1**, **TLSv1.1**, 및 **TLSv1.2**합니다. 자세한 내용은 참조 [SSLProtocol](https://github.com/Microsoft/mssql-jdbc/wiki/SSLProtocol)|
...
|trustManagerClass|문자열|null|사용자 지정 javax.net.ssl.TrustManager의 정규화 된 클래스 이름|
|trustManagerConstructorArg|문자열|null|TrustManager 생성자에 전달할 선택적 인수입니다. TrustManagerClass 지정 하는 경우 암호화 된 연결 요청 된 사용자 지정 TrustManager 기본 시스템 JVM 키 저장소 기반 TrustManager 대신 사용 됩니다.|
...
|trustStore|문자열|null|인증서 trustStore 파일에 대한 경로(파일 이름 포함)입니다. trustStore 파일에는 클라이언트에서 신뢰하는 인증서 목록이 포함되어 있습니다.<br /><br /> 이 속성이 지정 되지 않은 경우 null로 설정, 인증서 저장소를 사용 하 여 결정 하기 위해 트러스트 관리자 팩터리의 조회 규칙 의존 드라이버 또는 합니다.<br /><br /> **기본 SunX509 TrustManagerFactory에서는 다음 검색 순서에 트러스트 자료를 찾으려고 시도:**<br /><br /> "javax.net.ssl.trustStore" JVM(Java Virtual Machine) 시스템 속성에 지정된 파일<br /><br /> "\<java-home>/lib/security/jssecacerts" file.<br /><br /> "\<java-home>/lib/security/cacerts" file.<br /><br /> <br /><br /> 자세한 내용은 Sun Microsystems 웹 사이트에서 SUNX509 TrustManager 인터페이스 설명서를 참조하십시오.<br /><br /> **참고:** 연결에서 SSL 암호화를 사용 하는 경우에만 인증서 trustStore 조회를 적용이 속성 및 **trustServerCertificate** 속성이 "false"로 설정 되어 있습니다.|  
...
|trustStoreType|문자열|JKS|FIPS 모드 집합 트러스트 저장소 형식에 대 한 PKCS12 또는 유형 중 하나에 정의 된 FIPS 공급자입니다.|
