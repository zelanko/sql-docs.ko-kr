...
> [!IMPORTANT]  
>  이 샘플을 만들지 않은 경우 작업 테이블에 설명 된 대로 실행 되지 것입니다 [테이블 설정](../../connect/jdbc/using-bulk-copy-with-the-jdbc-driver.md#BKMK_TableSetup)합니다. 또한 Production.Product 테이블에서 내보낸 데이터로 CSV 파일을 만들어야 하며 이 작업은 SSMS(SQL Server Management Studio)를 사용하여 수행할 수 있습니다.  방문할 수 있는 경우 SSMS가 없는 [SQL Server Management Studio 다운로드](https://msdn.microsoft.com/library/mt238290.aspx) 를 가져오려고 합니다.  
