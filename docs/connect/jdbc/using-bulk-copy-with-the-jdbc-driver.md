...
|Void setUseInternalTranscation(Boolean useInternalTransaction)|여부 트랜잭션 내에서 대량 복사 작업의 각 일괄 처리가 발생 하는지 여부를 설정 합니다.|  
