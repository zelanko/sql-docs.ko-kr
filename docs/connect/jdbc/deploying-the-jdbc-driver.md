...
애플리케이션과 함께 JDBC 드라이버를 배포하는 데에는 두 가지 접근 방식이 있습니다. 그 중 하나는 JDBC 드라이버 파일을 자체 사용자 지정 설치 패키지의 일부로 포함시키는 것입니다. 두 번째 방식은 Microsoft가 제공하는 JDBC 설치 패키지를 사용하는 것으로, 이 패키지는 [SQL Server용 Microsoft JDBC 드라이버 다운로드 페이지](download-microsoft-jdbc-driver-for-sql-server.md)에서 다운로드할 수 있습니다.  
