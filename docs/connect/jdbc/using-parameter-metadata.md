...
>
> **SQL Server용 Microsoft JDBC 드라이버 6.0 이상**: SQL Server 2008 또는 2008 R2를 사용하는 경우 SELECT, DELETE, INSERT 및 UPDATE 문에 하위 쿼리 및/또는 조인이 포함되지 않는 한 JDBC 드라이버에서 이러한 문을 지원합니다.
...
암호화된 열에 대한 매개 변수 메타데이터 검색은 지원되지 않습니다. **SQL Server용 Microsoft JDBC Driver 4.1 또는 4.2**: SELECT, DELETE, INSERT 및 UPDATE 문에 하위 쿼리 및/또는 조인이 포함되지 않는 한 JDBC 드라이버에서 이러한 문을 지원합니다. MERGE 쿼리는 SQLServerParameterMetaData 클래스에서 지원되지 않습니다.
