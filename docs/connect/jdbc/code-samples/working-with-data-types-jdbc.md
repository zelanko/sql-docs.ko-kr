...
| [공간 데이터 형식 샘플](../../../connect/jdbc/code-samples/spatial-data-types-sample.md) | SQL Server에 공간 데이터 형식을 저장하는 방법 및 이러한 형식을 SQL Server에서 다시 검색하는 방법에 대해 설명합니다. 또한 이러한 데이터 형식의 Java 참조를 관리하기 위해 드라이버에서 새로 정의된 **Geometry** 및 **Geography** 클래스를 사용하는 방법에 대해서도 설명합니다. |
