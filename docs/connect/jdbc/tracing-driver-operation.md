...
|속성|Description|  
|----------|-----------------|  
|AuthenticationJNI|Windows 통합 인증 문제에 관한 메시지를 로깅합니다(**authenticationScheme** 연결 속성이 암시적 또는 명시적으로 **NativeAuthentication**으로 설정된 경우).<br /><br /> 애플리케이션에서는 로깅 수준을 FINEST 및 FINE으로 설정할 수 있습니다.|  
