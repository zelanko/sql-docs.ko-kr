...
또한 애플리케이션 소스 코드에 암호와 연결 URL을 함께 저장하지 않는 것이 좋습니다. 대신 액세스가 제한된 별도의 파일에 암호를 저장하는 방법을 고려합니다. 이러한 파일에 대한 액세스 권한은 응용 프로그램에서 실행하는 컨텍스트에 부여될 수 있습니다.
