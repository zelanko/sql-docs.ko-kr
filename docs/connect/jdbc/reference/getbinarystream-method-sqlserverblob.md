---
description: getBinaryStream 메서드(SQLServerBlob)
title: getBinaryStream 메서드(SQLServerBlob) | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
ms.assetid: 4fb5a2d3-5db6-4e62-8000-d0f486c7cf46
author: David-Engel
ms.author: v-daenge
ms.openlocfilehash: 2f281a5d1f11ed98332151d1bbcc6dbb22b5d780
ms.sourcegitcommit: e700497f962e4c2274df16d9e651059b42ff1a10
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/17/2020
ms.locfileid: "88437195"
---
# <a name="getbinarystream-method-sqlserverblob"></a>getBinaryStream 메서드(SQLServerBlob)
[!INCLUDE[Driver_JDBC_Download](../../../includes/driver_jdbc_download.md)]

  BLOB에서 데이터를 읽기 위한 입력 스트림을 반환합니다.  
  
## <a name="overload-list"></a>오버로드 목록  
  
|이름|설명|  
|----------|-----------------|  
|[getBinaryStream 메서드 &#40;&#41;](../../../connect/jdbc/reference/getbinarystream-method.md)|BLOB에서 데이터를 읽기 위한 입력 스트림을 반환합니다.|  
|[getBinaryStream 메서드&#40;long, long&#41;](../../../connect/jdbc/reference/getbinarystream-method-long-long.md)|지정된 시작 위치와 길이를 사용하여 부분적 BLOB 값이 들어 있는 입력 스트림 개체를 반환합니다.|  
  
## <a name="see-also"></a>참고 항목  
 [SQLServerBlob 메서드](../../../connect/jdbc/reference/sqlserverblob-methods.md)   
 [SQLServerBlob 멤버](../../../connect/jdbc/reference/sqlserverblob-members.md)   
 [SQLServerBlob 클래스](../../../connect/jdbc/reference/sqlserverblob-class.md)  
  
  
