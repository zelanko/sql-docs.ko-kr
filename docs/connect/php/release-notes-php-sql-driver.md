...
| 새 인출 기능 | &bull; &nbsp; Datetime 개체로 반환할 pdo_sqlsrv에 대 한 새 PDO::SQLSRV_ATTR_FETCHES_DATETIME_TYPE 플래그입니다.<br/><br/>&bull; &nbsp; Sqlsrv 문 수준으로 ReturnDatesAsStrings 옵션을 추가 합니다.<br/><br/>&bull; &nbsp; 인출한 결과에 10 진수 값의 서식을 지정 하는 것에 대 한 두 드라이버에 대 한 연결 및 문 수준에서 새 옵션입니다. |
| 사용자가 원본에서 빌드를 선택 하는 경우 드라이버의 정적 컴파일 지원 합니다. | &nbsp; |
| 인출에서 메타 데이터 캐싱 및 유니코드 문자열 변환 속도에서 성능이 향상 되었습니다. | &nbsp; |
| &nbsp; | &nbsp; |
