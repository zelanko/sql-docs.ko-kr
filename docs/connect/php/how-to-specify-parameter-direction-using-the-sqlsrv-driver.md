---
title: '방법: SQLSRV 드라이버를 사용 하 여 매개 변수 방향 지정 | Microsoft Docs'
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
ms.locfileid: "35307842"
---
# <a name="how-to-specify-parameter-direction-using-the-sqlsrv-driver"></a>방법: SQLSRV 드라이버를 사용하여 매개 변수 방향 지정
[!INCLUDE[Driver_PHP_Download](../../includes/driver_php_download.md)]

이 항목에서는 SQLSRV 드라이버를 사용하여 저장 프로시저를 호출할 때 매개 변수 방향을 지정하는 방법을 설명합니다. 매개 변수 방향에 전달 되는 매개 변수 배열 (3 단계)를 생성할 때 지정 된 [sqlsrv_query](../../connect/php/sqlsrv-query.md) 또는 [sqlsrv_prepare](../../connect/php/sqlsrv-prepare.md)합니다.  
...
3.  2단계의 PHP 변수를 사용하여 Transact-SQL  문자열의 매개 변수 자리 표시자에 순서대로 해당 매개 변수 값의 배열을 만들거나 업데이트합니다. 배열에서 각 매개 변수의 방향을 지정합니다. 각 매개 변수의 방향은 두 가지 방법 중 하나로 결정 됩니다: 기본적으로 (에 대해 입력된 매개 변수) 또는 사용 하 여 **SQLSRV_PARAM_\***  (출력 및 양방향 매개 변수)에 대 한 상수입니다. 예를 들어 다음 코드는 입력 매개 변수로 *$employeeId* 매개 변수를 지정하고 양방향 매개 변수로 *$usedVacationHours* 매개 변수를 지정합니다.  
    ...
    -   입력된 매개 변수를 암시적으로 지정 하 고, 출력 매개 변수 및 양방향 매개 변수를 명시적으로 지정:  
    ...
    -   입력된 매개 변수를 명시적으로 지정 하 고, 출력 매개 변수 및 양방향 매개 변수를 명시적으로 지정:  
...
4.  쿼리를 실행할 [sqlsrv_query](../../connect/php/sqlsrv-query.md) 또는 [sqlsrv_prepare](../../connect/php/sqlsrv-prepare.md) 및 [sqlsrv_execute](../../connect/php/sqlsrv-execute.md)합니다. 예를 들어 다음 코드는 *$params* 에서 지정된 매개 변수 값으로 *$tsql* 쿼리를 실행하기 위해 *$conn*연결을 사용합니다.  
