...
|CEKeystoreProvider<br />CEKeystoreName<br />CEKeystoreEncryptKey|문자열|경로, 이름 및 사용자 지정 키 저장소 공급자에는 항상 암호화 기능에 대 한 암호화 키를 지정 합니다. 연결할 때 사용자 지정 키 저장소 공급자를 올바르게 구성 하기 위해 세 값이 모두 설정 되어야 합니다. 자세한 내용은 참조 [PHP 응용 프로그램에서 상시 암호화 설정](../../connect/php/enabling-always-encrypted-php-application.md)합니다.|설정된 값이 없습니다.|
...
|ColumnEncryption|**활성화** 또는 **사용 안 함**|항상 암호화 기능이 사용 되는지 여부를 지정 합니다. 자세한 내용은 참조 [PHP 응용 프로그램에서 상시 암호화 설정](../../connect/php/enabling-always-encrypted-php-application.md)합니다.|사용 안 함|  
