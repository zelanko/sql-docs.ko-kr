---
title: 커서 유형 지정 및 행 선택 | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.suite: sql
ms.technology: connectivity
ms.tgt_pltfrm: ''
ms.topic: conceptual
helpviewer_keywords:
- cursor types
- scrollable cursors
ms.assetid: 51147e4f-5f07-4ee6-8245-e937eefec71f
caps.latest.revision: 27
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: ca3edbf7831789209b21a39b20d428cc5afe94d9
ms.sourcegitcommit: f16003fd1ca28b5e06d5700e730f681720006816
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 06/11/2018
ms.locfileid: "35309062"
---
# <a name="specifying-a-cursor-type-and-selecting-rows"></a>커서 유형 지정 및 행 선택
[!INCLUDE[Driver_PHP_Download](../../includes/driver_php_download.md)]

커서 유형에 따라 어떤 순서로든지 액세스할 수 있는 행을 가진 결과 집합을 만들 수 있습니다.  이 섹션에서는 클라이언트 쪽 및 서버 쪽 커서를 설명합니다.  
  
-   [커서 유형&#40;SQLSRV 드라이버&#41;](../../connect/php/cursor-types-sqlsrv-driver.md)  
  
-   [커서 유형&#40;PDO_SQLSRV 드라이버&#41;](../../connect/php/cursor-types-pdo-sqlsrv-driver.md)  
  
## <a name="see-also"></a>관련 항목  
[데이터 검색](../../connect/php/retrieving-data.md)  
  
