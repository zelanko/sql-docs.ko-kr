...
사용자가 `sqlsrv_field_metadata`를 사용하여(Microsoft ODBC Driver 17.4.2 이상이 필요함) Microsoft SQL Server 2019에서 [민감도 데이터 분류 메타데이터](../../relational-databases/security/sql-data-discovery-and-classification.md?tabs=t-sql#subheading-4)에 액세스할 수 있도록 새 옵션인 `DataClassification`이 버전 5.8.0에 도입되었습니다.
