...
ms.locfileid: "36092014"
---
# <a name="manage-authentication-in-database-engine-powershell"></a>데이터베이스 엔진 PowerShell에서 인증 관리
  기본적으로 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] PowerShell 구성 요소는 Windows 인증을 사용하여 [!INCLUDE[ssDE](../includes/ssde-md.md)]인스턴스에 연결합니다. PowerShell 가상 드라이브를 정의 하거나 또는 지정 하 여 SQL Server 인증을 사용할 수 있습니다는 `–Username` 및 `–Password` 에 대 한 매개 변수 `Invoke-Sqlcmd`합니다.  
 ...
 **sqldrive** 함수는 로그인에 대한 암호를 입력하라는 메시지를 표시하고 사용자가 입력하는 암호를 마스킹합니다. 디렉터리 변경 명령을 사용 하 여 때마다 (`cd`)를 사용 하 여 모든 작업이 수행 됩니다을 가상 드라이브 이름을 사용 하 여 경로에 연결 하는 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 드라이브를 만들었을 때 제공한 인증 로그인 자격 증명.  
...
1.  사용 하 여는 `–Username` 로그인 ID를 지정 하려면 매개 변수 및 `–Password` 매개 변수를 연결된 된 암호를 지정 합니다.  
