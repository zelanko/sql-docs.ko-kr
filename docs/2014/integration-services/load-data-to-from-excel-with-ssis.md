...
3.  알려진 문제 및 제한 사항입니다.
    -   [데이터 형식](#issues-types)문제
    -   [가져오기](#issues-importing)문제
    -   [내보내기와](#issues-exporting)관련 된 문제

## <a name="get-the-files-you-need-to-connect-to-excel"></a><a name="files-you-need"></a>Excel에 연결 하는 데 필요한 파일 가져오기
...
Office 365 구독이 있는 경우 설치 관리자를 실행할 때 오류 메시지가 나타날 수 있습니다. 오류는 Office 간편 실행 구성 요소와 함께 다운로드를 설치할 수 없음을 나타냅니다. 이 오류 메시지를 무시하려면 명령 프롬프트 창을 열고 `/quiet` 스위치를 통해 다운로드한 .EXE 파일을 실행하여 자동 모드에서 설치를 실행합니다. 예를 들면 다음과 같습니다.
...
## <a name="specify-excel"></a><a name="specify-excel"></a> Excel 지정
...
-   네트워크 공유에 있는 파일의 경우 ** \\ \\ 판매 \\ 데이터가 \\TestData.xlsx**.
...
## <a name="first-row-has-column-names"></a><a name="first-row"></a>첫 번째 행에 열 이름 있음
...
[가져오기 및 내보내기 마법사의 간단한 예제 시작](/sql/integration-services/import-export-data/get-started-with-this-simple-example-of-the-import-and-export-wizard)
