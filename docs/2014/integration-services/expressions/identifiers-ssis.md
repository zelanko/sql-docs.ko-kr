...
-   후속 글자는 Unicode Standard 2.0 밑줄 (_)에 정의 된 문자 또는 숫자 수 \@, $, # 자입니다.  
...
## <a name="variables-in-expressions"></a>식의 변수  
 식에서 참조 된 변수를 포함 해야 합니다는 \@ 접두사입니다. 예를 들어 합니다 **카운터** 변수는 사용 하 여 참조 \@카운터입니다. \@ 문자 변수 이름의 일부가 아닙니다.;만 변수 식 계산기를 식별 합니다. 식을 작성 하면 대화 상자를 사용 하는 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 디자이너에서 제공 된 \@ 문자 변수 이름에 자동으로 추가 됩니다. 유효 하지 않은 사이 공백이 포함 된 \@ 문자와 변수 이름입니다.  
...
-   후속 글자는 Unicode Standard 2.0 밑줄 (_)에 정의 된 문자 또는 숫자 수 \@, $, # 자입니다.  
 ...
 목록에 없는 문자가 변수 이름에 포함되어 있으면 해당 변수를 대괄호로 묶어야 합니다. 예를 들어 공백이 포함된 변수 이름은 대괄호로 묶어야 합니다. 여는 괄호 뒤의 \@ 문자입니다. 예를 들어 합니다 **My Name** 변수가 참조로 \@[My Name]. 변수 이름과 대괄호 사이에는 공백을 넣을 수 없습니다.  
