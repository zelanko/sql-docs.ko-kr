...
# <a name="create-the-ssis-catalog"></a>SSIS 카탈로그 만들기
  ...
  [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] 서버에 프로젝트를 배포하려면 서버에 `SSISDB` 카탈로그가 있어야 합니다. 
  [!INCLUDE[ssSQL11](../includes/sssql11-md.md)] 설치 프로그램에서 카탈로그를 자동으로 만들지 않으므로 다음 지침에 따라 카탈로그를 수동으로 만들어야 합니다.  
...
2.  
  [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 데이터베이스 엔진에 연결합니다.  
...
5.  
  **SQL Server 시작 시 Integration Services 저장 프로시저 자동 실행** 을 클릭하여 [서버 인스턴스를 다시 시작할 때마다](/sql/integration-services/system-stored-procedures/catalog-startup) catalog.startup [!INCLUDE[ssIS](../includes/ssis-md.md)] 저장 프로시저를 실행하도록 지정합니다.  
  
     저장 프로시저에서는 SSISDB 카탈로그에 대한 작업의 상태를 유지 관리합니다. 
  [!INCLUDE[ssIS](../includes/ssis-md.md)] 서버 인스턴스가 다운될 때 실행 중이었던 패키지의 상태를 수정합니다.  
  
6.  암호를 입력 한 다음 **확인**을 클릭 합니다.  
