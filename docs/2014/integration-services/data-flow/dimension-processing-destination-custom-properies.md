...
|KeyErrorLimit|정수|UseDefaultConfiguration가 인 `False`경우 설정 된 키 오류의 상한값입니다.|  
...
|KeyNotFound|Integer(열거형)|UseDefaultConfiguration가 인 `False`경우 누락 된 키 오류를 처리 하는 방법을 나타내는 값입니다. 가능한 값은 `IgnoreError`(0), `ReportAndContinue`(1) 및 `ReportAndStop`(2)입니다. 이 속성의 기본값은 `IgnoreError`(0)입니다.|  
...
|NullKeyNotAllowed|Integer(열거형)|UseDefaultConfiguration가 인 `False`경우 허용 되지 않는 null을 처리 하는 방법을 나타내는 값입니다. 가능한 값은 `IgnoreError`(0), `ReportAndContinue`(1) 및 `ReportAndStop`(2)입니다. 이 속성의 기본값은 `IgnoreError`(0)입니다.|  
