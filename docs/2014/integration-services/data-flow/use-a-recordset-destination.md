...
2.  레코드 집합 대상이 메모리에 저장 한 레코드 집합을 포함 한 다음이 변수의 유형을 설정 변수를 만들고 `Object`합니다.  
...
3.  **변수** 창에서 다음과 같이 레코드 집합과 현재 행의 열 값을 보유할 변수를 만듭니다.  
    ...
    1.  변수를 만들고, `BonusRecordset`, 해당 형식으로 설정 하 고 `Object`합니다.  
  
         `BonusRecordset` 변수는 레코드 집합을 보유 합니다.  
  
    2.  변수를 만들고, `EmailAddress`, 해당 형식으로 설정 하 고 `String`합니다.  
  
         `EmailAddress` 변수는 판매 직원의 전자 메일 주소를 보유 합니다.  
  
    3.  변수를 만들고, `FirstName`, 해당 형식으로 설정 하 고 `String`합니다.  
  
         `FirstName` 변수는 판매 직원의 이름을 보유 합니다.  
  
    4.  변수를 만들고, `Bonus`, 해당 형식으로 설정 하 고 `Double`합니다.  
...
2.  **Foreach 루프 편집기**를 열고 다음과 같이 컨테이너를 구성합니다.  
    ...
    1.  에 **컬렉션** 페이지에 대 한 **열거자**선택, **Foreach ADO 열거자**, 및에 대 한 **ADO 개체 원본 변수**, 선택`User::BonusRecordset`.  
  
    2.  에 **변수 매핑** 페이지, 지도 `User::EmailAddress` 를 인덱스 0으로, `User::FirstName` 인덱스 1 및 `User::Bonus` 를 인덱스 2입니다.  
