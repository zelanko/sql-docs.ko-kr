...
## <a name="bulk-inserts"></a>대량 삽입  
 SQL Server 대상을 사용 하 여 데이터 대량 로드는 원격 SQL Server 데이터베이스로 하려고 하면 다음과 비슷한 오류 메시지가 표시 될 수 있습니다. "OLE DB 레코드를 사용할 수 있습니다. 원본: "Microsoft [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client" Hresult: 0x80040E14 설명: "SSIS 파일 매핑 개체 'Global\DTSQLIMPORT'를 열 수 없으므로 부하를 대량 수 없습니다. 운영 체제 오류 코드 2(시스템에서 지정한 파일을 찾을 수 없습니다.)입니다. Windows 보안을 통해 로컬 서버에 액세스하고 있는지 확인하십시오.""와 유사한 오류 메시지가 나타납니다.  
