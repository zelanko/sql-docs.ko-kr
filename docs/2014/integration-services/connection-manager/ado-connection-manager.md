...
# <a name="ado-connection-manager"></a>ADO 연결 관리자
 ...
 패키지에 ado 연결 관리자를 추가 하면에서 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 런타임에 ado 연결로 확인 되는 연결 관리자를 만들고, 연결 관리자 속성을 설정 하 고, 연결 관리자를 `Connections` 패키지의 컬렉션에 추가 합니다. 연결 관리자의 `ConnectionManagerType` 속성이 `ADO`로 설정됩니다.  
