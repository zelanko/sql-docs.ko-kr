...
6.  `Parameters`를 클릭 하 고 **쿼리 매개 변수 설정** 대화 상자에서 쿼리의 단일 입력 매개 변수인 0을 DtsClient:: Country 변수에 매핑합니다.  
...
4.  런타임에 패키지를 실행 하는 데 필요한 명령줄 매개 변수를 포함 하도록 **필요에 따라** `dtexecArgs` 변수의 값을 수정 합니다. 예제 코드는 파일 시스템에서 패키지를 로드합니다.  
  
5.  패키지에 있는 DataReader 대상의 이름을 포함 하도록 필요에 따라 `dataReaderName` 변수의 값을 수정 합니다.  
  
6.  폼에 단추와 입력란을 추가합니다. 이 샘플 코드는 `btnRun`를 단추의 이름으로 사용 하 고 `txtResults` 텍스트 상자의 이름으로 사용 합니다.  
