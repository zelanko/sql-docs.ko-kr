...
-   **패키지는 해당 패키지를 실행 하는 프로그램과 동일한 컴퓨터에서 실행**됩니다. 프로그램에서 다른 서버에 원격으로 저장된 패키지를 로드하더라도 해당 패키지는 로컬 컴퓨터에서 실행됩니다.  
  
-   **Integration Services 설치 된 컴퓨터의 개발 환경 외부 에서만 패키지를 실행할 수**있습니다. 
