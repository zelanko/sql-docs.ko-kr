...
|`ProtectionLevel`|패키지의 보호 수준입니다. 값은 `DontSaveSensitive` ,, `EncryptSensitiveWithUserKey` `EncryptSensitiveWithPassword` , `EncryptAllWithPassword` 및 **serverstorage**입니다. 이 속성의 기본값은 `EncryptSensitiveWithUserKey`입니다. 자세한 내용은 <xref:Microsoft.SqlServer.Dts.Runtime.DTSProtectionLevel>를 참조하세요.|  
