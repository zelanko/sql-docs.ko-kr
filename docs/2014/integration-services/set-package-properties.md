...
|`CheckpointUsage`|패키지를 다시 시작할 수 있는 시점을 지정합니다. 값은 `Never`하십시오 `IfExists`, 및 `Always`합니다. 이 속성의 기본값은 `Never`, 패키지를 다시 시작할 수 없습니다 나타냅니다. 자세한 내용은 <xref:Microsoft.SqlServer.Dts.Runtime.DTSCheckpointUsage>를 참조하세요.|  
...
|속성|Description|  
|--------------|-----------------|  
|`DelayValidation`|패키지가 실행될 때까지 패키지 유효성 검사를 지연할지 여부를 나타냅니다. 이 속성의 기본값은 `False`합니다.|  
...
|`PackagePriorityClass`|패키지 스레드의 Win32 스레드 우선 순위 클래스입니다. 값은 `Default`, `AboveNormal`를 `Normal`합니다 `BelowNormal`, `Idle`합니다. 이 속성의 기본값은 `Default`합니다. 자세한 내용은 <xref:Microsoft.SqlServer.Dts.Runtime.DTSPriorityClass>를 참조하세요.|  
...
|`Name`|패키지의 이름입니다.|  
|`PackageType`|패키지 유형입니다. 값은 `Default`, `DTSDesigner`, `DTSDesigner100`, `DTSWizard`합니다 `SQLDBMaint`, 및 `SQLReplication`합니다. 이 속성의 기본값은 `Default`합니다. 자세한 내용은 <xref:Microsoft.SqlServer.Dts.Runtime.DTSPackageType>를 참조하세요.|  
...
|`ForceExecutionResult`|패키지의 실행 결과입니다. 값은 `None`, `Success`를 `Failure`, 및 `Completion`합니다. 이 속성의 기본값은 `None`합니다. 자세한 내용은 T:Microsoft.SqlServer.Dts.Runtime.DTSForcedExecResult를 참조하세요.|  
|`LocaleId`|Microsoft Win32 로캘입니다. 이 속성의 기본값은 로컬 컴퓨터 운영 체제의 로캘입니다.|  
|`LoggingMode`|패키지의 로깅 동작을 지정하는 값입니다. 값은 `Disabled`하십시오 `Enabled`, 및 `UseParentSetting`합니다. 이 속성의 기본값은 `UseParentSetting`합니다. 자세한 내용은 <xref:Microsoft.SqlServer.Dts.Runtime.DTSLoggingMode>가 될 때까지 태스크를 반복합니다.|  
...
|`UpdateObjects`|새로운 버전을 사용할 수 있는 경우 패키지에 포함된 새로운 버전의 개체를 사용하도록 패키지가 업데이트되는지 여부를 나타냅니다. 예를 들어가 속성이 `True`, 대량 삽입 태스크를 포함 하는 패키지를 최신 버전의 대량 삽입을 사용 하도록 업데이트 됩니다 작업 [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] 제공 합니다. 이 속성의 기본값은 `False`합니다.|  
...
|`ProtectionLevel`|패키지의 보호 수준입니다. 값은 `DontSaveSensitive`, `EncryptSensitiveWithUserKey`, `EncryptSensitiveWithPassword`합니다 `EncryptAllWithPassword`, 및 **ServerStorage**합니다. 이 속성의 기본값은 `EncryptSensitiveWithUserKey`합니다. 자세한 내용은 <xref:Microsoft.SqlServer.Dts.Runtime.DTSProtectionLevel>를 참조하세요.|  
