...
# <a name="add-expressions-to-precedence-constraints"></a>선행 제약 조건에 식 추가
 ...
 식은 `True` 또는 `False`로 계산되어야 하며 유효한 [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] 식이어야 합니다. 식에서는 문자, 시스템 및 사용자 지정 변수와 [!INCLUDE[ssIS](../includes/ssis-md.md)] 식 문법에서 제공하는 함수와 연산자를 사용할 수 있습니다. 예를 들어 `@Count == SQRT(144) + 10` 식에서는 `Count` 변수, SQRT 함수 및 등호(==)와 더하기(+) 연산자가 사용되었습니다. 자세한 내용은 [Integration Services&#40;SSIS&#41; 식](expressions/integration-services-ssis-expressions.md)을 참조하세요.  
 ...
 실행 개체는 또한 여러 식이 포함된 여러 선행 제약 조건을 사용하여 연결될 수 있습니다. 예를 들어 다음 그림에서 태스크 B와 C는 실행 결과와 식을 사용하는 선행 제약 조건에 의해 태스크 A에 연결됩니다. 로 설정 된 제약 조건 값을 모두 `Success.` 하나의 선행 제약 조건에 식이 포함 `@X >== @Z`, 다른 선행 제약 조건 식을 포함 하 고 `@X < @Z`합니다. 변수 `X`와 변수 `Z`의 값에 따라 태스크 C 또는 태스크 B가 실행됩니다.  
