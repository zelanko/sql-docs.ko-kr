...
4.  되어 있는지 확인의 값은 `AllowConfigurationChanges` 특성은 **true** XML에 포함 되어는 `Package` 두 개의 패키지의 각 요소는 `MiscellaneousFile` 4 개의 패키지가 아닌 파일의 각 요소 및 `ConfigurationFile` 각 두 XML 구성 파일에 대 한 요소입니다.  
