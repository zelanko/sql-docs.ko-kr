...
# <a name="set-or-change-the-protection-level-of-packages"></a>패키지 보호 수준 설정 또는 변경
 ...
 이 항목의에서 절차 중 하나를 사용 하는 방법에 설명 [!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)] 또는 dtutil 명령 프롬프트 유틸리티를 변경 하 여 `ProtectionLevel` 속성입니다.  
  
> [!NOTE]  
>  이 항목의 절차를 하는 것 외에도 설정할 수 있습니다 일반적으로 변경 하거나는 `ProtectionLevel` 를 가져오거나 패키지를 내보낼 때 패키지의 속성입니다. 또한 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 가져오기 및 내보내기 마법사를 사용하여 패키지를 저장할 때도 패키지의 `ProtectionLevel` 속성을 변경할 수 있습니다.  
...
4.  명령 프롬프트에서 설정 하려는 패키지를 포함 하는 폴더로 이동 합니다 `ProtectionLevel` 속성입니다.  
  
     다음 단계에 나오는 구문 예에서는 이 폴더가 현재 폴더라고 가정합니다.  
  
5.  다음 예 중 하나와 비슷한 명령을 사용하여 패키지 보호 수준을 설정하거나 변경합니다.  
  
    -   다음 명령 집합을 `ProtectionLevel` 수준 2, "암호로 중요 한 정보 암호화" 암호로 "strongpassword"를 파일 시스템의 개별 패키지의 속성:  
