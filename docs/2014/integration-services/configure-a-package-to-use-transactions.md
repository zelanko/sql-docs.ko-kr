...
## <a name="configuring-a-single-transaction"></a>단일 트랜잭션 구성  
 ...
 그런 다음 이 단일 트랜잭션에 특정 태스크와 컨테이너를 등록합니다. 태스크 또는 컨테이너 트랜잭션에 등록할 태스크 또는 컨테이너의 TransactionOption 속성을 설정한 `Supported`합니다.  
...
5.  에 **속성** 창에서 TransactionOption 속성을 설정 `Required`합니다.  
  
6.  **제어 흐름** 탭의 디자인 화면에서 트랜잭션에 등록할 태스크 또는 컨테이너를 마우스 오른쪽 단추로 클릭한 후 **속성**을 클릭합니다.  
  
7.  에 **속성** 창에서 TransactionOption 속성을 설정 `Supported`합니다.  
...
## <a name="configuring-multiple-transactions"></a>여러 트랜잭션 구성  
 ...
 그런 다음 패키지에 포함된 태스크와 컨테이너 중에서 원하는 태스크와 컨테이너를 트랜잭션을 시작하거나 트랜잭션에 참여하도록 구성합니다. 태스크 또는 컨테이너는 트랜잭션을 시작 하도록을 구성 하려면 해당 태스크 또는 컨테이너의 TransactionOption 속성을 설정한 `Required`합니다.  
...
5.  에 **속성** 창에서 TransactionOption 속성을 설정 `Supported`합니다.  
...
7.  에 **속성** 창에서 TransactionOption 속성을 설정 `Required`합니다.  
  
8.  트랜잭션이 컨테이너에 의해 시작되는 경우 트랜잭션에 등록할 태스크 또는 컨테이너를 마우스 오른쪽 단추로 클릭한 후 **속성**을 클릭합니다.  
  
9. 에 **속성** 창에서 TransactionOption 속성을 설정 `Supported`합니다.  
