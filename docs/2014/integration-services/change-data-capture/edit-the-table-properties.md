...
2.  캡처 인스턴스를 다음 중 하나로 저장합니다.  
    ...
    -   **새 캡처 인스턴스**: 이 경우 새 캡처 인스턴스는 저장 하 고 이전 캡처 인스턴스는 삭제 되지 않습니다.  
  
         **참고**: 캡처 인스턴스는 테이블당 두 개 까지만 사용할 수 있습니다. 이미 두 개의 캡처 인스턴스가 있는 경우에는 이 옵션을 사용할 수 없습니다.  
  
    -   **기존 항목 바꾸기**: 이 경우 현재 캡처 인스턴스가 삭제 되 고 사용자가 만든 캡처 인스턴스로 대체 합니다. 이 테이블에 대해 두 개의 캡처 인스턴스가 정의되어 있는 경우 하나를 선택하여 바꾸어야 합니다.  
