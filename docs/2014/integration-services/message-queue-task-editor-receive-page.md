...
|**변수에 대 한 문자열 메시지**|메시지<br /><br /> 이 값을 선택하면 동적 옵션 **StringMessage**가 표시됩니다.|  
...
|**패키지에서**|메시지는 지정한 패키지의 메시지만 받습니다. 이 값을 선택하면 동적 옵션 **Identifier**가 표시됩니다.|  
...
|**하**|메시지가 **CompareString** 옵션의 문자열을 포함해야 합니다.|  
