...
6.  에 `Value` 드롭 다운 목록에서 선행 제약 조건의 실행 결과 선택 합니다.  
  
7.  계산 작업에 식이 사용 되는 경우는 `Expression` 상자에 식을 입력 하 고 클릭 **테스트** 식을 계산할 수 있습니다.  
  
    > [!NOTE]  
    >  변수 이름은 대소문자를 구분합니다.  
  
8.  제한 된 실행 파일에 여러 태스크 또는 컨테이너가 연결 되어 있으면 선택 **Logical AND** 모든 선행 실행 개체의 실행 결과를 계산 하는 되어야 하도록 지정 `true`합니다. 선택 **Logical OR** 하나의 식 결과만으로 계산 되어야 합니다는 되도록 `true`합니다.  
