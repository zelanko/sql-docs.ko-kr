 ...
 **SQL Server 인증 사용**  
 패키지에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인증을 사용하여 데이터베이스에 로그인할지 여부를 지정합니다. 
 ...
 **통합 보안**  
 Windows 통합 인증을 사용하여 연결(권장)하려면 `True`를 지정하고 `False` 인증을 사용하여 연결하려면 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]를 지정합니다. 
  `False`를 지정하면 사용자 ID와 암호를 입력해야 합니다. 기본값은 `False`입니다.  
