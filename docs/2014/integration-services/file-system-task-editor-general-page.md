...
> [!NOTE]  
>  및 속성에 대해 선택한 변수를 재정의 `SourceConnection` 하려면 `DestinationConnection` **Source** 및 **Destination** 속성에 대 한 식을 입력 합니다. **파일 시스템 태스크 편집기** 의 **식**페이지에 식을 입력합니다. 예를 들어 태스크에서 대상으로 사용하는 파일 경로를 설정하기 위해 특정 조건에서 변수 A를 사용하고 다른 조건에서는 변수 B를 사용할 수 있습니다.  
 ...
 **DestinationVariable**  
 목록에서 변수 이름을 선택 하거나를 클릭 하 여 \<**New variable...**> 새 변수를 만듭니다.  
