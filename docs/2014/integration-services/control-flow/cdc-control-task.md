...
|MarkProcessedRange|: 이 작업은 각 cdc 실행 (CDC 데이터 흐름이 성공적으로 완료 됨) 후 CDC 실행 중에 완전히 처리 된 마지막 LSN을 기록 하기 위해 실행 됩니다. 다음에 GetProcessingRange를 실행하면 이 위치가 처리 범위의 시작 부분이 됩니다.|  
...
-   수동 상태 지 속성: 이 경우 CDC 제어 태스크가 패키지 변수에 저장 된 상태를 관리 하지만 패키지 개발자 해야 변수 영구 저장소에서 CDC 제어를 호출 하기 전에 읽은 다음 CDC 제어가 마지막 되 면 해당 영구 저장소에 다시 쓰기 호출 고 CDC 실행이 완료 됩니다.  
  
-   자동 상태 지 속성: CDC 상태가 데이터베이스의 테이블에 저장 됩니다. 상태는 **상태를 저장하는 데 사용할 테이블** 속성(상태 저장을 위해 선택한 연결 관리자에 있음)으로 지정한 테이블의 **StateName** 속성에 제공된 이름으로 저장됩니다. 기본값은 원본 연결 관리자이지만 대상 연결 관리자를 사용하는 것이 일반적입니다. CDC 제어 태스크는 상태 테이블에서 상태 값을 업데이트하며 이는 앰비언트 트랜잭션의 일환으로 커밋됩니다.  
