...
|StateVariable|String|현재 CDC 컨텍스트의 CDC 상태를 저장하는 SSIS 패키지 변수입니다. CDC 제어 태스크가 상태를 읽고 **StateVariable** 에 쓰며, **AutomaticStatePersistence** 가 선택되지 않은 한 상태를 로드하거나 영구 저장소에 저장하지 않습니다. [상태 변수 정의](../data-flow/define-a-state-variable.md)|  
