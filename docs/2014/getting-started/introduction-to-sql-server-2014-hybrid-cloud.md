...
# <a name="introduction-to-sql-server-2014-hybrid-cloud"></a>SQL Server 2014 하이브리드 클라우드 소개
 ...
 비즈니스 필요에 따라 확장할 수 있는 유연한 IT 인프라에 필요한 경우에 데이터 센터에서 사설 클라우드 또는 Azure 글로벌 데이터 센터에서 공용 클라우드를 빌드할 수 있습니다. 퍼블릭 클라우드와 합쳐지도록 데이터 센터를 확장하면 하이브리드 클라우드 모델을 구축하게 됩니다. 
