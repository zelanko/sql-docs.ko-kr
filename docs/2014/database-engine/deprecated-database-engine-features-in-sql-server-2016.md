...
|호환성 수준|버전 90([!INCLUDE[ssDEversion2005](../includes/ssdeversion2005-md.md)])의 업그레이드|호환성 수준은 마지막 두 버전에서만 사용할 수 있습니다. 호환성 수준에 대한 자세한 내용은 [ALTER DATABASE 호환성 수준&#40;Transact-SQL&#41;](/sql/t-sql/statements/alter-database-transact-sql-compatibility-level)을 참조하세요.<br /><br /> [!INCLUDE[ssSQL14](../includes/sssql14-md.md)]에서 SQL Server 2005 데이터베이스를 업그레이드할 수는 있지만 업그레이드 작업 중 호환성 수준이 90에서 100으로 업데이트됩니다.|데이터베이스 호환성 수준 90|107|  
...
|도구|sqlmaint 유틸리티|[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 유지 관리 계획 기능을 사용합니다.|None|None|  
...
|[!INCLUDE[tsql](../includes/tsql-md.md)]|:: 함수 호출 시퀀스|Sys에서 *COLUMN_LIST* 선택으로 바꿉니다. \<*function_name*> ().<br /><br /> 예를 들어 `SELECT * FROM ::fn_virtualfilestats(2,1)`를 `SELECT * FROM sys.fn_virtualfilestats(2,1)`로 대체합니다.|'::' 함수 호출 구문|166|  
...
|[!INCLUDE[tsql](../includes/tsql-md.md)]|세미콜론이 있는 종료되지 않는 [!INCLUDE[tsql](../includes/tsql-md.md)] 문|세미콜론( ; )을 사용하여 [!INCLUDE[tsql](../includes/tsql-md.md)] 문을 종료합니다.|없음|None|  
