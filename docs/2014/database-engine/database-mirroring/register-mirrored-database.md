...
|열 이름|Description|  
|-----------------|-----------------|  
|**등록**|등록할 각 데이터베이스를 검사합니다. 데이터베이스가 현재 모니터링되고 있는 경우 해당 확인란은 선택된 상태로 비활성화되어 있습니다.<br /><br /> 참고: 데이터베이스의 등록을 취소 하려면 닫습니다 합니다 **미러된 데이터베이스 등록** 대화 상자의 탐색 트리에서 데이터베이스를 선택 하 고 선택 **등록 취소** 에서 합니다 **작업** 메뉴입니다.|  
