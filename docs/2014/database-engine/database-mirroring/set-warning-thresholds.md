...
## <a name="options"></a>옵션  
 *서버 인스턴스 및 해당 연결 상태*  
 _시스템_**\\**_INSTANCE_NAME_형식으로 된 파트너 서버 인스턴스의 이름입니다. 기본 서버 인스턴스의 경우 시스템 이름만 표시됩니다.  
  
 또한 이 필드는 모니터가 이 서버 인스턴스에 현재 연결되어 있는지 여부를 나타냅니다. 가능한 연결 상태는 다음과 같습니다.  
  
-   *Server_instance_name* **에 연결 되지 않음**    
  
-   *Server_instance_name* **에 연결 하는 중**    
  
-   *Server_instance_name* **에 연결 됨**    
 ...
 **연결**/**취소**  
 **연결**/**취소** 단추는 각 *서버 인스턴스와 연결 상태* 필드와 연결 되어 있습니다. 단추의 상태는 연결 상태에 따라 달라집니다.  
