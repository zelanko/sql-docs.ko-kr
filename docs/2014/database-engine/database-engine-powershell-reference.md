...
|Description|Cmdlet|  
|-----------------|------------|  
|사용 하 여 실행할 수 있는 TRANSACT-SQL 및 XQuery 스크립트를 실행 하는 `sqlcmd` 유틸리티입니다.|[Invoke-Sqlcmd cmdlet](../../2014/database-engine/invoke-sqlcmd-cmdlet.md)|  
...
### <a name="information-about-other-cmdlets"></a>다른 cmdlet에 대한 정보  
 `Encode-Sqlname` 및 `Decode-Sqlname` cmdlet PowerShell 경로에서 지원 되지 않는 문자를 포함 하는 SQL Server 식별자를 지정할 수 있습니다. 자세한 내용은 [SQL Server Identifiers in PowerShell](../powershell/sql-server-identifiers-in-powershell.md)을(를) 참조하세요.  
