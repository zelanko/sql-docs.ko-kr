---
title: 지원 되지 않는 SQL Server 2014에서에서 데이터베이스 엔진 기능 | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
|범주|지원되지 않는 기능|대체 기능|  
|--------------|--------------------------|-----------------|  
|Backup 및 Restore 메서드|**백업 {데이터베이스 &#124; 로그} WITH PASSWORD** 및 **백업 {데이터베이스 &#124; 로그} WITH MEDIAPASSWORD** 은 지원 되지 않습니다. **복원 {데이터베이스 &#124; 로그} [MEDIA] PASSWORD와**되지 계속 됩니다.|InclusionThresholdSetting|  
|Backup 및 Restore 메서드|**RESTORE {데이터베이스 &AMP;#124; 로그}... … WITH DBO_ONLY**|**RESTORE {데이터베이스 &AMP;#124; 로그}...... WITH RESTRICTED_USER**|  
|호환성 수준|호환성 수준 80|데이터베이스를 호환성 수준 90 이상으로 설정해야 합니다.|  
|구성 옵션|`sp_configure 'user instance timeout'` 및 `'user instances enabled'`|Local Database 기능을 사용합니다. 자세한 내용은 참조 [SqlLocalDB 유틸리티](../tools/sqllocaldb-utility.md)|  
...
|메일|SQL 메일|데이터베이스 메일을 사용합니다. 자세한 내용은 참조 [데이터베이스 메일](../relational-databases/database-mail/database-mail.md) 및 [Use Database Mail Instead of SQL Mail](../relational-databases/policy-based-management/use-database-mail-instead-of-sql-mail.md)합니다.|  
...
|보안|`sp_dropalias`|별칭을 사용자 계정 및 데이터베이스 역할의 조합으로 대체해야 합니다. 사용 하 여 `sp_dropalias` 업그레이드 된 데이터베이스에서 별칭을 제거 하려면.|  
|보안|version 매개 변수 **PWDCOMPARE** 로그인에서 값을 나타내는 보다 이전 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 2000는 중단 되었습니다.|InclusionThresholdSetting|  
...
|Transact-SQL 구문|사용 하 여 **\* =** 및 **=\***|ANSI 조인 구문을 사용합니다. 자세한 내용은 [FROM (Transact-SQL)](http://msdn.microsoft.com/library/ms177634\(SQL.105\).aspx)을 참조하십시오.|  
|XEvents|databases_data_file_size_changed, databases_log_file_size_changed<br /><br /> eventdatabases_log_file_used_size_changed<br /><br /> locks_lock_timeouts_greater_than_0<br /><br /> locks_lock_timeouts|Database_file_size_change 이벤트, database_file_size_change로 대체<br /><br /> database_file_size_change event<br /><br /> lock_timeout_greater_than_0<br /><br /> lock_timeout|  
