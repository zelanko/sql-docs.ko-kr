...
# <a name="exclude-files-from-source-control"></a>원본 제어에서 파일 제외
 ...
 생성 된 파일 에서만 **소스 제어에서 제외** 명령을 사용 해야 합니다. 생성 된 파일은 솔루션에 있는 다른 파일의 내용을 기반으로 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 하 여 완전히 다시 만들 수 있는 파일입니다.  
  
### <a name="to-exclude-a-file-from-source-control"></a>원본 제어에서 파일을 제외하려면  
  
1.  솔루션 탐색기에서 제외할 파일을 선택합니다.  
  
2.  **파일** 메뉴에서 **원본 제어**를 가리킨 다음 **소스 제어에서** * \<파일 이름>* **제외** 를 클릭 합니다.  
