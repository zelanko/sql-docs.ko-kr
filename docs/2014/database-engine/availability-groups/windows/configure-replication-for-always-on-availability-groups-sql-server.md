...
1.  원격 배포를 구성합니다. 게시자 구성에 저장 프로시저를 사용하는 경우 `sp_adddistributor`을 실행합니다. 배포를 설정 하기 위해 배포자에서 `sp_adddistrbutor`를 실행할 때 사용 되는 *@password* 에 대해 동일한 값을 지정 합니다.  
...
##  <a name="step2"></a>2. AlwaysOn 가용성 그룹 구성  
...
```  
 ...
 각 보조 복제본 호스트에서 배포를 구성합니다. 원래 게시자의 배포자를 원격 배포자로 식별합니다. 배포자에서 원래 `sp_adddistributor`를 실행했을 때와 동일한 암호를 사용합니다. 배포를 구성 하는 데 저장 프로시저를 사용 하는 경우 `sp_adddistributor`의 *@password* 매개 변수를 사용 하 여 암호를 지정 합니다.  
...
```  
 ...
 가용성 그룹에 대한 정보를 쿼리하려면 각 가용성 그룹 복제본 호스트에서 충분한 권한이 있는 로그인으로 `sp_validate_replica_hosts_as_publishers` 저장 프로시저를 실행해야 합니다. `sp_validate_redirected_publisher`와 달리이 메서드는 호출자의 자격 증명을 사용 하며, msdb. m a. m. m a. m. m a. m. m a. m.  
