...
|----------|--------------------------------------------------------------------|-------------------------------------------|  
|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 인스턴스당 하나의 데이터베이스 미러링 끝점 만들기|`New-SqlHadrEndPoint`|데이터베이스 미러링 끝점이 없는 각 서버 인스턴스에서 실행합니다.<br /><br /> 참고: 기존 데이터베이스 미러링 끝점을 변경 하려면 사용 하 여 `Set-SqlHadrEndpoint`합니다.|  
|가용성 그룹 만들기|먼저 `New-SqlAvailabilityReplica` cmdlet과 `-AsTemplate` 매개 변수를 사용하여 가용성 그룹에 포함할 두 개의 각 가용성 복제본에 대한 메모리 내 가용성 복제본 개체를 만듭니다.<br /><br /> 그런 다음 사용 하 여 가용성 그룹을 만듭니다는 `New-SqlAvailabilityGroup` cmdlet 하 여 가용성 복제본 개체를 참조 합니다.|초기 주 복제본을 호스트할 서버 인스턴스에서 실행합니다.|  
 ...
 **<sup>*</sup>**  지정된 된 작업을 수행 하려면 디렉터리를 변경 (`cd`) 표시 된 서버 인스턴스 또는 인스턴스에 있습니다.  
...
> [!NOTE]  
>  구문 및 특정된 cmdlet의 예를 보려면 사용 하 여는 `Get-Help` cmdlet에는 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] PowerShell 환경입니다. 자세한 내용은 [Get Help SQL Server PowerShell](../../../powershell/sql-server-powershell.md)을 참조하세요.  
  
1.  디렉터리를 변경 (`cd`)을 주 복제본을 호스팅하는 서버 인스턴스에 있습니다.  
...
8.  필요에 따라 Windows를 사용 하 여 `dir` 명령을 사용 하는 새 가용성 그룹의 내용을 확인 합니다.  
...
2.  복원 `MyDatabase` 및 해당 트랜잭션 로그를 사용 하는 `-NoRecovery` 옵션입니다.  
...
-   **블로그:**  
     ...
     [SQL Server PowerShell을 사용 하 여 AlwaysOn 구성](http://blogs.msdn.com/b/sqlalwayson/archive/2012/02/03/configuring-alwayson-with-sql-server-powershell.aspx)  
