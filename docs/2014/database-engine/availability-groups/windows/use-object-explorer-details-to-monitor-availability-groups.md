...
|**동기화됨**|데이터베이스가 현재 주 복제본 또는 마지막 주 복제본(있는 경우)의 주 데이터베이스와 동기화되어 있습니다.<br /><br /> 참고: 성능 모드에서는 데이터베이스가 Synchronized 상태에 있지 않도록 합니다.|  
...
|값|Description|  
|-----------|-----------------|  
|동기화 중|보조 데이터베이스가 디스크에 아직 기록(확정)되지 않은 주 데이터베이스에 대한 트랜잭션 로그 기록을 수신했습니다.<br /><br /> 참고: 비동기-커밋 모드 동기화 상태는 항상 **Synchronizing**합니다.|  
