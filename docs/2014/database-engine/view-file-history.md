---
title: 파일 기록 보기 | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.suite: ''
ms.technology:
- dbe-cross-instance
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- version control services [SQL Server], file history
- historical information [SQL Server], files
ms.assetid: 9d80d4ea-a0f6-4a79-885d-79143ca09c00
caps.latest.revision: 22
author: JennieHubbard
ms.author: jhubbard
manager: jhubbard
ms.openlocfilehash: f7ec533709d829807311f1c6aef4af1cbad006a6
ms.sourcegitcommit: 5dd5cad0c1bbd308471d6c885f516948ad67dfcf
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 06/19/2018
ms.locfileid: "36186364"
---
# <a name="view-file-history"></a>파일 기록 보기
  사용할 수는 **기록** 소스 제어 파일에 대 한 버전 기록을 보려면 명령입니다. 이 기록에는 원본 제어에서 파일이 추가 또는 체크 인된 날짜와 시간이 포함되어 있습니다.  
  
### <a name="to-view-the-history-of-a-file"></a>파일의 기록을 보려면  
  
1.  솔루션 탐색기에서 파일을 선택합니다.  
  
2.  에 **파일** 메뉴에서 **소스 제어** 클릭 **기록**합니다.  
  
3.  에 **파일의 기록** 대화 상자에서 다음 작업 중 필요한 작업을 수행 합니다.  
  
4.  선택한 버전을 봅니다.  
  
5.  선택한 버전에 대한 특정 세부 사항을 봅니다.  
  
6.  선택한 버전 복사본을 로컬 디스크로 읽어 들입니다.  
  
7.  선택한 버전과 로컬에 저장된 버전 간의 차이를 확인합니다.  
  
8.  특정 파일 버전을 현재 버전으로 식별합니다.  
  
9. 기록 보고서를 프린터, 파일 또는 [!INCLUDE[msCoName](../includes/msconame-md.md)] Windows 클립보드에 내보냅니다.  
  
 이러한 작업을 수행 하는 방법에 대 한 내용은 클릭는 **도움말** 단추는 **파일의 기록** 대화 상자.  
  
## <a name="see-also"></a>관련 항목  
 [설정 및 버전 정보를 검색 합니다.](../../2014/database-engine/set-and-retrieve-version-information.md)   
 [프로젝트 기록 보기](../../2014/database-engine/view-project-history.md)   
 [파일 상태 보기](../../2014/database-engine/view-file-status.md)   
 [파일 검색](../../2014/database-engine/retrieve-files.md)   
 [파일 비교](../../2014/database-engine/compare-files.md)  
  
  