 ...
 **옵션** 대화 상자를 사용 하 여 소스 제어 역할을 하나 이상 구성 하 여 원본 제어 옵션을 구성 합니다. 역할은 [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]에 적용되는 설정에 대한 일반적인 설명과 해당 설정에 연결된 원본 제어 옵션으로 구성됩니다.  
 ...
 예를 들어 독자적 데이터베이스 개발자인 경우 일반적으로 파일을 체크 인한 후에 파일을 체크 아웃된 상태로 유지함으로써 다른 사용자와의 충돌을 방지합니다. 결과적으로 [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]는 독자적 개발자 역할을 정의합니다. 이 역할의 경우 [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 는 **체크 인할 때 항목을 체크 아웃 상태로 유지** 옵션을 자동으로 선택 합니다.  
...
3.  사용하려는 각 원본 제어 역할에 대한 로그인 자격 증명을 설정합니다. 이 페이지는 원본 제어 플러그 인이 설치된 경우에만 사용할 수 있습니다.  
    ...
    |역할|Description|  
    |----------|-----------------|  
    |**Visual SourceSafe**|[!INCLUDE[msCoName](../includes/msconame-md.md)] Visual SourceSafe 사용자가 가장 일반적으로 사용 하는 설정을 사용 하도록 지정 합니다.|  
...
5.  **원본 제어 환경 설정** 상자에서 소스 제어 옵션을 설정 하려는 역할을 선택 합니다.  
     ...
     **체크 인된 항목**  
     체크 아웃 되지 않은 항목을 편집 하려고 [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 할 때 동작을 수행 하는 방법을 지정 하는 옵션 목록을 표시 합니다. 다음 표에서는 사용 가능한 옵션에 대해 설명 합니다.  
