...
ms.locfileid: "36185677"
---
# <a name="manage-checkouts"></a>체크 아웃 관리
  파일이 원본 제어에 추가된 후에 파일을 수정할 수 있으려면 먼저 체크 아웃해야 합니다. 파일을 원본 제어에서 체크 아웃할 경우 원본 제어 공급자는 로컬 디스크에서 최신 버전의 복사본을 만들고 파일의 읽기 전용 특성을 제거합니다. 경우에 따라서는 파일을 체크 아웃하지 않고 편집해야 할 수 있습니다. 체크 아웃 하지 않고 파일을 편집 하는 방법에 대 한 자세한 내용은 참조 [Checked-In 파일 편집](../../2014/database-engine/edit-checked-in-files.md)합니다.  
  
 사용할 수 있습니다 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 파일을 수동 또는 자동으로 체크 아웃 합니다. 사용자 파일을 수동으로 체크 아웃에 파일이 들어 있는 솔루션을 열어는 [!INCLUDE[ssManStudio](../includes/ssmanstudio-md.md)] 환경을 클릭 한 다음는 **체크 아웃** 명령입니다. 자동으로 체크 아웃하도록 [!INCLUDE[ssManStudio](../includes/ssmanstudio-md.md)] 환경을 구성한 경우 파일을 자동으로 체크 아웃할 수 있습니다.  
