...
4.  등록 작업 중의 인스턴스에 연결 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] "sa"를 사용 하 여 등록 합니다. "Domain_2"의 프록시 계정을 지정합니다.  
  
5.  유효성 검사는 성공하지만 등록이 실패합니다.  
  
 위 예제를 사용 하 여이 문제를 해결의 인스턴스에 연결 하는 것 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 에 등록 하는 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] "sa"를 사용 하 여 유틸리티 "Domain_1."에서 프록시 계정 제공  
...
```  
 ...
 WMI 문제 해결에 대 한 자세한 내용은 참조 하세요. [WMI 문제 해결](https://go.microsoft.com/fwlink/?LinkId=178250)합니다. 이러한 SQL Server 유틸리티 작업의 쿼리는 로컬로 실행할 수 있으므로 DCOM 및 원격 문제 해결 정보에는 영향을 주지 않습니다.  
...
-   UCP에서 SQL Server Browser 서비스가 시작되어야 하며 자동으로 시작하도록 구성되어 있어야 합니다. 조직에서 SQL Server Browser 서비스 사용을 금지하는 경우 다음 단계를 사용하여 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 의 관리되는 인스턴스가 UCP에 연결하는 것을 허용하세요.  
    ...
    4.  에 **SQL Server 클라이언트 네트워크 유틸리티** 대화 상자를 선택 합니다 **별칭** 탭을 클릭 한 다음 **추가...** .  
