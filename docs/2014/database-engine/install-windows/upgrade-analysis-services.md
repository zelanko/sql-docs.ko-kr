...
-   **전체 업그레이드** 는 기존 프로그램 파일을 대체 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 프로그램 파일입니다. 데이터베이스 위치는 동일하게 유지됩니다. 프로그램 폴더는 새 이름을 반영하여 업데이트됩니다.  
  
-   A **side-by-side-업그레이드** 는의 새로운 설치 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 기존 Analysis Services 인스턴스가 있는 동일한 컴퓨터에 있습니다. 같은 컴퓨터의 새 인스턴스로 데이터베이스를 이동한 후에 더 이상 사용하지 않는 이전 버전을 제거할 수 있습니다.  
