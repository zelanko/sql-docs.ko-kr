...
ms.locfileid: "36081911"
---
# <a name="program-exception-message-box"></a>프로그램 예외 메시지 상자
  제공 하는 것 보다 훨씬 더 많은 메시징 환경 제어할 수 있도록 응용 프로그램에서 예외 메시지 상자를 사용할 수는 <xref:System.Windows.Forms.MessageBox> 클래스입니다. 자세한 내용은 참조 [예외 메시지 상자 프로그래밍](../../../2014/database-engine/dev-guide/exception-message-box-programming.md)합니다. 예외 메시지 상자.dll 가져오기 및 배포 하는 방법에 대 한 정보를 참조 하십시오. [예외 메시지 상자 응용 프로그램 배포](../../../2014/database-engine/dev-guide/deploying-an-exception-message-box-application.md)합니다.  
...
2.  (선택 사항) 추가 `using` (C#) 또는 `Imports` ([!INCLUDE[msCoName](../../includes/msconame-md.md)] Visual Basic.NET) 지시어를 사용 하는 <xref:Microsoft.SqlServer.MessageBox> 네임 스페이스입니다.  
  
3.  예상되는 예외를 처리할 try-catch 블록을 만듭니다.  
  
4.  내에서 `catch` 블록을의 인스턴스를 만들고는 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox> 클래스입니다. 전달 된 <xref:System.Exception> 에서 처리 하는 개체는 `try` - `catch` 블록입니다.  
...
5.  (선택 사항) 다음 속성 중 하나 이상을 설정 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox>:  
    ...
    -   <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.DefaultButton%2A> - <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBoxDefaultButton> 예외 메시지 상자에 대 한 기본 단추를 지정 하는 열거형입니다.  
  
    -   <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.Options%2A> - <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBoxOptions> 예외 메시지 상자의 다른 동작을 제어 하는 사용 하는 열거형입니다.  
...
7.  (선택 사항) 값을 확인 반환 된 <xref:System.Windows.Forms.DialogResult> 열거형 어느 단추는 사용자를 결정 하는 경우 클릭 합니다.  
  
#### <a name="to-display-the-exception-message-box-without-an-exception"></a>예외 없이 예외 메시지 상자를 표시하려면  
  
1.  관리 코드 프로젝트의 참조를 Microsoft.ExceptionMessageBox.dll 어셈블리에 추가합니다.  
  
2.  (선택 사항) 추가 `using` (C#) 또는 `Imports` (Visual Basic.NET) 지시어를 사용 하 여 <xref:Microsoft.SqlServer.MessageBox> 네임 스페이스입니다.  
  
3.  <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox> 클래스의 인스턴스를 만듭니다. 메시지 텍스트로 전달 된 <xref:System.String> 값입니다.  
...
6.  (선택 사항) 반환된 된 값을 기록해 둡니다 <xref:System.Windows.Forms.DialogResult> 열거형 어느 단추는 사용자를 결정 하는 경우 클릭 합니다.  
...
3.  다음의 두 가지 방법 중 하나로 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox> 클래스의 인스턴스를 만듭니다.  
    ...
    -   전달 된 <xref:System.Exception> 에서 처리 하는 개체는 `try` - `catch` 블록입니다.  
  
    -   메시지 텍스트로 전달 된 <xref:System.String> 값입니다.  
  
4.  에 대 한 값은 다음 중 하나를 설정 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.Buttons%2A>:  
  
    -   <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBoxButtons.AbortRetryIgnore> -표시는 **중단**, **을 다시 시도**, 및 **무시** 단추입니다.  
    ...
    -   <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBoxButtons.OKCancel> -표시 된 **확인** 및 **취소** 단추입니다.  
  
    -   <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBoxButtons.RetryCancel> -표시는 **을 다시 시도** 및 **취소** 단추입니다.  
  
    -   <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBoxButtons.YesNo> -표시 **예** 및 **아니요** 단추입니다.  
  
    -   <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBoxButtons.YesNoCancel> -표시 **예**, **아니요**, 및 **취소** 단추입니다.  
  
5.  (선택 사항) 사용자 지정 단추를 사용 하는 경우의 오버 로드 중 하나를 호출는 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.SetButtonText%2A> 메서드를 최대 5 개의 사용자 지정 단추의 텍스트를 지정 합니다.  
  
6.  <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.Show%2A> 메서드를 호출합니다. 예외 메시지 상자가 속해 있는 부모 창을 전달합니다.  
  
7.  (선택 사항) 반환된 된 값을 기록해 둡니다 <xref:System.Windows.Forms.DialogResult> 열거형 어느 단추는 사용자를 결정 하는 경우 클릭 합니다. 사용자 지정 단추를 사용 하는 경우의 값을 확인 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBoxDialogResult> 에 대 한는 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.CustomDialogResult%2A> 속성 어느 사용자 지정 단추 사용자 확인을 클릭 합니다.  
...
4.  설정의 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.ShowCheckbox%2A> 속성을 `true`합니다.  
  
5.  (선택 사항) 사용자가 다시 예외 메시지 상자의 표시 여부를 결정 하는 텍스트를 지정 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.CheckboxText%2A>합니다. 기본 텍스트는 "이 메시지를 다시 표시 안 함"입니다.  
  
6.  응용 프로그램의 실행 기간 동안에만 사용자의 의사 결정을 유지 해야 하는 경우의 값을 설정 합니다. <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.IsCheckboxChecked%2A> 을 전역 <xref:System.Boolean> 변수입니다. 예외 메시지 상자의 인스턴스를 만들기 전에 이 값을 확인하십시오.  
  
7.  사용자가 선택한 내용을 영구적으로 저장하려면 다음과 같이 합니다.  
  
    1.  CreateSubKey 메서드를 호출 응용 프로그램에서 사용 하는 사용자 지정 레지스트리 키를 열고 및 설정 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.CheckboxRegistryKey%2A> 반환 되는 RegistryKey 개체에 있습니다.  
  
    2.  <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.CheckboxRegistryValue%2A>를 사용되는 레지스트리 값의 이름으로 설정합니다.  
  
    3.  설정 <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox.CheckboxRegistryMeansDoNotShowDialog%2A> 를 `true`합니다.  
