...
9. 명령 프롬프트에서 다음 줄을 실행하여 응용 프로그램을 테스트합니다.  
...
```  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] (`test.sql`) 저장된 프로시저와이 샘플에 정의 된 함수를 실행 하 여 예제를 테스트 합니다.  
...
```  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)]은 데이터베이스에서 어셈블리, 함수 및 저장 프로시저를 제거합니다.  
