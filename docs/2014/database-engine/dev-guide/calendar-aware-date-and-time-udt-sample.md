---
title: 달력 인식 날짜 및 시간 UDT 예제 | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
3.  c:\MySample에서 `CalendarAware.cs`를 만들고 C# 예제 코드(아래)를 파일에 복사합니다.  
  
4.  c:\MySample에서 `calendars.txt` 파일을 만들고 예제 코드를 파일에 복사합니다.  
  
5.  c:\MySample에서 `calendars.ar-SA.txt` 파일을 만들고 다음을 파일에 복사합니다.  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 1`  
  
6.  c:\MySample에서 `calendars.ja.txt` 파일을 만들고 다음을 파일에 복사합니다.  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 3`  
  
7.  c:\MySample에서 `calendars.``zh-CN.txt` 파일을 만들고 다음을 파일에 복사합니다.  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 8`  
  
8.  c:\MySample에서 `build.com` 파일을 만들고 다음을 파일에 복사합니다.  
...
9. 명령 프롬프트에서 파일 빌드를 실행하여 위성 어셈블리를 빌드합니다.  
  
10. 다음을 실행하여 명령줄 프롬프트에서 예제 코드를 컴파일합니다.  
...
11. [!INCLUDE[tsql](../../includes/tsql-md.md)] 설치 코드를 파일에 복사하고 해당 파일을 예제 디렉터리에 `Install.sql` 로 저장합니다.  
  
12. 예제가 `C:\MySample\`이외의 디렉터리에 설치된 경우 해당 위치를 가리키도록 표시된 대로 `Install.sql` 파일을 편집합니다.  
  
13. 다음을 실행하여 어셈블리 및 저장 프로시저를 배포합니다.  
  
    -   `sqlcmd -E -I -i install.sql`  
  
14. [!INCLUDE[tsql](../../includes/tsql-md.md)] 테스트 명령 스크립트를 파일에 복사하고 해당 파일을 예제 디렉터리에 `test.sql` 로 저장합니다.  
  
15. 다음 명령으로 테스트 스크립트를 실행합니다.  
  
    -   `sqlcmd -E -I -i test.sql`  
  
16. [!INCLUDE[tsql](../../includes/tsql-md.md)] 정리 스크립트를 파일에 복사하고 해당 파일을 예제 디렉터리에 `cleanup.sql` 로 저장합니다.  
  
17. 다음 명령으로 스크립트를 실행합니다.  
...
```  
 ...
 이는 어셈블리를 배포하고 데이터베이스 내에서 변환 함수를 만드는 [!INCLUDE[tsql](../../includes/tsql-md.md)] 설치 스크립트(`Install.sql`)입니다. 이 스크립트에서는 예제 데이터의 테이블도 만듭니다.  
...
```  
 ...
 이는 다양한 달력에서 함수를 실행하여 예제를 테스트하는 `test.sql`입니다.  
...
```  
 ...
 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)]은 데이터베이스에서 어셈블리, 함수 및 테이블을 제거합니다.  
