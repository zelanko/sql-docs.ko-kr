---
title: 기록 및 상태 보고서를 만들 | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.suite: ''
ms.technology:
- dbe-cross-instance
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- version control services [SQL Server], reports
- Visual SourceSafe, reports
ms.assetid: 3017f5d0-ef53-4522-b42f-bdf637e7d75e
caps.latest.revision: 23
author: JennieHubbard
ms.author: jhubbard
manager: jhubbard
ms.openlocfilehash: a2050b2ed38129566cefb702659a62b9285860f8
ms.sourcegitcommit: 5dd5cad0c1bbd308471d6c885f516948ad67dfcf
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 06/19/2018
ms.locfileid: "36183503"
---
# <a name="create-history-and-status-reports"></a>기록 및 상태 보고서 만들기
  현재 로드된 솔루션에서 파일의 기록 또는 현재 상태에 대한 보고서를 만들 수 있습니다. 또한 보고서를 미리 본 다음 프린터, 파일 또는 클립보드로 보고서를 보낼 수 있습니다. 이러한 보고서는 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]에 의해 생성되지 않습니다. 대신에 [!INCLUDE[msCoName](../includes/msconame-md.md)] Visual SourceSafe가 이 보고서를 생성합니다.  
  
### <a name="to-create-a-history-report"></a>기록 보고서를 만들려면  
  
1.  솔루션 탐색기에서 파일을 선택합니다.  
  
2.  에 **파일** 메뉴에서 **소스 제어**를 클릭 하 고 **기록**합니다.  
  
3.  에 **기록을** \<파일 또는 프로젝트 > 대화 상자를 클릭 **보고서**합니다.  
  
4.  에 **기록 보고서** 대화 상자에서 적절 한 옵션을 선택한 다음 **확인**합니다.  
  
 파일의 기록을 검색할 때 지정할 수 있는 옵션에 대 한 자세한 내용은 참조 [파일 기록 보기](../../2014/database-engine/view-file-history.md)합니다.  
  
#### <a name="to-create-a-status-report"></a>상태 보고서를 만들려면  
  
1.  솔루션 탐색기에서 파일을 선택합니다.  
  
2.  에 **파일** 메뉴에서 **소스 제어**를 클릭 하 고 **SourceSafe 속성**합니다.  
  
3.  에 **파일 상태** 대화 상자를 클릭 **보고서**합니다.  
  
4.  에 **파일 속성 보고서** 대화 상자에서 적절 한 옵션을 클릭 한 다음 클릭 **확인**합니다.  
  
 파일 상태를 검색할 때 지정할 수 있는 옵션에 대 한 자세한 내용은 참조 하십시오. [파일 상태 보기](../../2014/database-engine/view-file-status.md)합니다.  
  
## <a name="see-also"></a>관련 항목  
 [버전 정보 설정 및 검색](../../2014/database-engine/set-and-retrieve-version-information.md)  
  
  