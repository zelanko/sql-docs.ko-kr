...
-   [SharePoint 2013 예제 배포 토폴로지](#bkmk_example_deployments_2013)  
    ...
    -   [SharePoint 용 PowerPivot 2013 단일 서버 배포](#bkmk_powerpivot_sharepoint2013_1server)  
  
    -   [SharePoint 용 PowerPivot 2013 두 서버 배포](#bkmk_powerpivot_sharepoint2013_2server)  
  
    -   [SharePoint 용 PowerPivot 2013 세 서버 배포](#bkmk_powerpivot_sharepoint2013_3server)  
...
-   [SharePoint 2010 예제 배포 토폴로지](#bkmk_example_deployments_2010)  
    ...
    -   [3 계층 확장 배포](#bkmk_sharepoint2010_scaleserver)  
 ...
 **[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] SharePoint 모드** SharePoint 서비스 응용 프로그램 아키텍처 및 SharePoint server 2013가 필요 합니다.  
...
###  <a name="bkmk_bi_Sharepoint2013_3tier"></a> PowerPivot for SharePoint 2013 및 Reporting Services 세 서버 배포  
 다음 세 서버 배포에서 SQL Server 데이터베이스 엔진에서 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] SharePoint 모드 및 SharePoint에서 별도 서버에 각 실행에서 실행 중인 서버. [!INCLUDE[ssGeminiShortvnext](../../includes/ssgeminishortvnext-md.md)] 2013 설치 관리자 패키지 (**spPowerPivot.msi**) SharePoint 서버에서 실행 해야 합니다.  
...
|**(5)**|실행 된 **spPowerPivot.msi** 데이터 공급자를 설치는 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 구성 도구, [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 갤러리 및 데이터 새로 고침 예약 합니다.|  
...
###  <a name="bkmk_powerpivot_sharepoint2013_1server"></a> SharePoint 용 PowerPivot 2013 단일 서버 배포  
 단일 서버 배포는 테스트용으로는 유용하지만 프로덕션 배포에는 권장되지 않습니다.  
  
 다음 다이어그램은 단일 서버의 일부인 구성 요소를 보여 줍니다. [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 배포 합니다.  
...
###  <a name="bkmk_powerpivot_sharepoint2013_2server"></a> SharePoint 용 PowerPivot 2013 두 서버 배포  
 다음 두 서버 배포에서 SQL Server 데이터베이스 엔진 및 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] SharePoint 서버와 별도 서버에서 실행 되는 SharePoint 모드에 있습니다. SharePoint 2013 용는 [!INCLUDE[ssGeminiLongvnext](../../includes/ssgeminilongvnext-md.md)] 설치 관리자 패키지 (**spPowerPivot.msi**) SharePoint 서버에 설치 합니다.  
  
 [!INCLUDE[ssGeminiShortvnext](../../includes/ssgeminishortvnext-md.md)] SharePoint Server 2013 서버 쪽 데이터 새로 고침 처리, 추가 하려면 데이터 공급자 확장 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 갤러리, 및에 대 한 관리 지원 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 통합 문서 및 Excel 통합 문서와 고급 데이터 모델입니다.  
  
 설치 관리자 패키지는 [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 기능 팩의 일부로 사용할 수 있습니다. 기능 팩에서 다운로드할 수 있습니다는 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 다운로드 센터의 [Microsoft® SharePoint® 용 Microsoft® SQL Server® 2014 PowerPivot®](http://go.microsoft.com/fwlink/?LinkID=296473) (HYPERLINK "http://go.microsoft.com/fwlink/?LinkID=296473" \t "_blank" http://go.microsoft.com/fwlink/?LinkID=296473)합니다.  
...
|**(1)**|Excel Services 응용 프로그램. 서비스 응용 프로그램은 SharePoint 설치의 일부로 만들어집니다.|  
|**(2)**|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 서비스 응용 프로그램입니다. 기본 이름은 **기본 PowerPivot 서비스 응용 프로그램**입니다.|  
|**(3)**|실행 된 **spPowerPivot.msi** 데이터 공급자를 설치는 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 구성 도구, [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 갤러리 및 데이터 새로 고침 예약 합니다.|  
|**(4)**|SharePoint 모드의 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 서버. Excel Services 응용 프로그램 **데이터 모델 설정** 에서 이 서버를 사용하도록 구성합니다.|  
|**(5)**|SharePoint 콘텐츠, 구성 및 서비스 응용 프로그램 데이터베이스입니다.|  
  
###  <a name="bkmk_powerpivot_sharepoint2013_3server"></a> SharePoint 용 PowerPivot 2013 세 서버 배포  
 다음 세 서버 배포에서 SQL Server 데이터베이스 엔진에서 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] SharePoint 모드 및 SharePoint에서 별도 서버에 각 실행에서 실행 중인 서버. SharePoint 서버에 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] 2013 설치 관리자 패키지(spPowerPivot.msi)를 설치해야 합니다.  
  
 ![PowerPivot 모드 3 서버 배포로](../../../2014/sql-server/install/media/as-powerpivot-mode-3server-deployment.gif "PowerPivot 모드 3 서버 배포로")  
...
##  <a name="bkmk_example_deployments_2010"></a> SharePoint 2010 예제 배포 토폴로지  
 다음 다이어그램에서는 각 계층에서 실행되는 서비스 및 공급자를 보여 줍니다. 다이어그램에는 여러 기본 제공 서비스가 포함되어 있는데 이러한 서비스는 일부 SQL Server BI 시나리오에서 필요합니다. Excel 서비스, Secure Store Services 및 Windows 토큰 서비스에 대한 클레임은 PowerPivot for SharePoint 또는 SharePoint의 Reporting Services 배포를 위해 필요하거나 권장됩니다. 또한 일부 PowerPivot 데이터 액세스 시나리오의 경우 MSOLAP OLE DB 공급자 및 ADO.NET 서비스가 필요합니다. 필요에 따라 설치할 수 있습니다 Analysis Services를 데이터 계층 빌드 하려는 경우 [!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)] 보고서가 SharePoint 외부에서 호스팅되는 테이블 형식 모델 데이터베이스에 기반 합니다.  
...
##  <a name="bkmk_sharepoint2010_1server"></a> 단일 서버 배포  
 ...
 단일 서버에 모든 기능을 설치하기 위해 동일한 물리적 서버에 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 및 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]를 순서대로 설치합니다. 독립 실행형 서버 구성에 대 한 지침을 참조 하십시오. [배포 검사 목록: Reporting Services, Power View 및 PowerPivot for SharePoint](deployment-checklist-reporting-services-power-view-power-pivot-for-sharepoint.md)합니다.  
  
##  <a name="bkmk_sharepoint2010_2server"></a> 2 계층 배포  
 2계층 배포는 일반적으로 특정 컴퓨터에 SharePoint Server 2010을 배포하고 두 번째 컴퓨터에 SQL Server 데이터베이스 엔진을 배포하는 것입니다. 데이터 계층을 전용 서버로 이동하는 것이 2개 컴퓨터 팜에 대한 가장 일반적인 구성입니다. 2 계층 팜에서 설치한 둘 다 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 및 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] SharePoint 서버에 있습니다. 프런트 엔드의 모든 웹 서비스와 응용 프로그램 계층의 공유 서비스를 동일한 물리적 서버에서 실행합니다. 2계층 배포의 설치 단계는 독립 실행형 배포와 매우 유사하며 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 및 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]를 동일한 물리적 서버에 순서대로 설치합니다.  
  
##  <a name="bkmk_sharepoint2010_3server"></a> 3 계층 배포  
 3계층 배포는 일반적으로 웹 프런트 엔드 서비스를 처리 또는 메모리 집중 응용 프로그램과 구분합니다. 이 토폴로지에서 설치 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 및 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] 는 응용 프로그램 서버. 웹 프런트 엔드에서 실행되는 웹 서비스는 서버 구성 동안 팜에서 응용 프로그램에 배포되는 솔루션을 통해 사후 설치 태스크로 설치됩니다. 다음 다이어그램에서는 3계층 배포를 보여 줍니다.  
  
 ![3 서버 토폴로지](../../../2014/sql-server/install/media/sql11bisetup-3server.gif "3 서버 토폴로지")  
  
##  <a name="bkmk_sharepoint2010_scaleserver"></a> 3 계층 확장 배포  
