---
title: 업그레이드 관리자 오류 | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.technology: database-engine
ms.topic: conceptual
helpviewer_keywords:
- errors [Upgrade Advisor]
ms.assetid: a493a459-e7ea-4629-a14c-684d8c57e310
author: mashamsft
ms.author: mathoma
manager: craigg
ms.openlocfilehash: f6c9f133af7b8054414f6867f41cee3022d68925
ms.sourcegitcommit: 3026c22b7fba19059a769ea5f367c4f51efaf286
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 06/15/2019
ms.locfileid: "66091676"
---
# <a name="upgrade-advisor-errors"></a>업그레이드 관리자 오류
  보고서 뷰어에 업그레이드 관리자 오류가 표시될 수 있습니다. 이러한 오류는 이 도움말 항목에 링크됩니다.  
  
 보고서 뷰어에 표시되는 문제 중 이 항목에 링크되는 문제는 무시해도 됩니다.  
  
## <a name="see-also"></a>관련 항목  
 [업그레이드 문제 해결](../../../2014/sql-server/install/resolving-upgrade-issues.md)  
  
  
