...
> [!NOTE]  
>  A [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 로그인 있는 없기 해당 데이터베이스 사용자 하지 않는 한 데이터베이스에 액세스할 수 없습니다 **게스트** 해당 데이터베이스에서 사용 하도록 설정 합니다. 데이터베이스 사용자 계정을 만드는 방법은 참조 [CREATE USER &#40;TRANSACT-SQL&#41;](/sql/t-sql/statements/create-user-transact-sql)합니다.  
...
```  
 ...
 현재 데이터베이스에 있으며 어떤 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 로그인에도 연결되지 않은 사용자와 이에 해당되는 SID(보안 ID)가 나열됩니다. 자세한 내용은 참조 [sp_change_users_login &#40;TRANSACT-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-change-users-login-transact-sql)합니다.  
  
> [!NOTE]  
>  **sp_change_users_login** 함께 사용할 수 없습니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Windows에서 생성 된 로그인.  
  
## <a name="to-resolve-an-orphaned-user"></a>분리된 사용자를 확인하려면  
 분리된 사용자를 확인하려면 다음 절차를 수행합니다.  
  
1.  다음 명령은 지정 된 서버 로그인 계정을 다시 링크 *< login_name >* 하 여 지정 된 데이터베이스 사용자와 *< database_user >* 합니다.  
    ...
    ```  
     ...
     자세한 내용은 참조 [sp_change_users_login &#40;TRANSACT-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-change-users-login-transact-sql)합니다.  
  
2.  앞 단계에서 코드를 실행한 다음 사용자는 데이터베이스에 액세스할 수 있습니다. 사용자 다음 암호를 변경할 수의 *< login_name >* 로그인 계정을 사용 하 여는 **sp_password** 저장 프로시저를 다음과 같이 합니다.  
    ...
    >  **sp_password** 에 사용할 수 없습니다 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Windows 계정입니다. Windows 네트워크 계정을 통해 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스에 연결하는 사용자는 Windows에 의해 인증되므로 암호는 Windows에서만 변경할 수 있습니다.  
     ...
     자세한 내용은 참조 [sp_password &#40;TRANSACT-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-password-transact-sql)합니다.  
