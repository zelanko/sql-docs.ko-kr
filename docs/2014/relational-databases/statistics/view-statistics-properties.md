...
6.  **통계 속성 -** _statistics_name_ 대화 상자의 **페이지 선택** 창에서 **자세히**를 선택합니다.  
     ...
     **인덱스 statistics_name에 대한 통계**  
     이 텍스트 상자에는 통계 개체에서 반환되는 속성이 표시됩니다. 이 속성은 세 가지 섹션으로 구분 됩니다. 통계 헤더, 밀도 벡터 및 히스토그램입니다.  
     ...
     **모든 밀도**  
     밀도는 1/ *고유 값*입니다. 결과에는 통계 개체에 있는 각 열 접두사의 밀도가 한 행씩 표시됩니다. 고유 값은 행별 및 열 접두사별 열 값의 고유한 목록입니다. 예를 들어 통계 개체가 키 열 (A, B, C)를 포함 하는 경우 결과 값에서 이러한 각 열 접두사의 고유 목록의 밀도 보고: (A), (A, B) 및 (A, B, C). 접두사 (A, B, C)를 사용 하면 이러한 각 목록은 다음과 같습니다. 고유 값 목록 (3, 5, 6), (4, 4, 6), (4, 5, 6), (4, 5, 7). 접두사 (A, B)를 사용 하면 동일한 열 값은 고유 값 목록: (3, 5), (4, 4) 및 (4, 5).  
