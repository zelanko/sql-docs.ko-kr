...
# <a name="sqltriggercontext-object"></a>SqlTriggerContext 개체
  ...
  `SqlTriggerContext` 클래스는 트리거에 대한 컨텍스트 정보를 제공합니다. 이 컨텍스트 정보에는 트리거를 발생시킨 동작 유형, UPDATE 작업에서 수정된 열, DDL(데이터 정의 언어) 트리거의 경우 트리거 작업을 설명하는 `EventData` 구조가 포함됩니다. `SqlTriggerContext` 클래스를 사용 하는 방법에 대 한 자세한 내용 및 예제는 [CLR 트리거](../../database-engine/dev-guide/clr-triggers.md)를 참조 하세요.  
