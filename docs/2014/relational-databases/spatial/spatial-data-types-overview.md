...
-   합니다 `geography` 데이터 형식 메서드는 두 개의 입력이 있어야만 `geography` stintersection (), stunion (), stdifference () 및 stsymdifference (), 같은 인스턴스는 메서드의 결과가 단일 반구 내에 포함 되지 않을 경우 null을 반환 합니다. STBuffer()도 결과가 단일 반구를 초과할 경우 null을 반환합니다.  
...
### <a name="outer-and-inner-rings-not-important-in-geography-data-type"></a>geography 데이터 형식에서 중요하지 않은 외부 및 내부 링  
 OGC Simple Features for SQL Specification에서는 외부 링 및 내부 링에 설명 하지만 이러한 차이 대해 거의 의미가 합니다 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] `geography` 데이터 형식 다각형의 링 사용할 수는 외부 링으로 합니다.  
...
```  
 ...
 다음 그림에서는 각 형식이 저장 되는 방법을 보여 줍니다 (빨간색 선 `LineString``@g1`을 나타내고 파란색 선은 `CircularString``@g2`):  
