...
|**stream_id**|**[uniqueidentifier] rowguidcol**||`NEWID()` 함수에서 반환되는 값|FILESTREAM 데이터의 고유 ID입니다.|해당 없음|  
|**file_stream**|`varbinary(max)`<br /><br /> `filestream`|변수|NULL|FILESTREAM 데이터를 포함합니다.|해당 없음|  
