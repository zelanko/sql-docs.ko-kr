...
##  <a name="storage"></a> FILESTREAM 저장소  
 FILESTREAM 저장소는 데이터가 파일 시스템에 BLOB로 저장된 `varbinary(max)` 열로 구현됩니다. BLOB의 크기는 파일 시스템의 볼륨 크기로만 제한됩니다. 표준 `varbinary(max)` 제한은 2GB의 파일 크기로 파일 시스템에 저장된 BLOB에 적용되지 않습니다.  
