...
ms.locfileid: "36093742"
---
# <a name="using-data-files-and-format-files"></a>데이터 파일 및 서식 파일 사용
  가장 간단한 대량 복사 프로그램은 다음을 수행합니다.  
  
1.  호출 [bcp_init](../native-client-odbc-extensions-bulk-copy-functions/bcp-init.md) 지정 하려면 테이블에서을 대량 복사 (BCP_OUT 설정) 또는 데이터 파일에 보고 합니다.  
...
2.  호출 [bcp_control](../native-client-odbc-extensions-bulk-copy-functions/bcp-control.md) 와 *eOption* BCPHINTS로 설정 하 고 *iValue* TRANSACT-SQL 문이 포함 된 SQLTCHAR 문자열에 대 한 포인터를 설정 합니다.  
...
3.  호출 **bcp_exec** 대량 복사 작업을 실행 합니다.  
 ...
 호출 되는 열에 데이터 저장에 테이블의 다른 형식 보다 데이터 파일을 만들려면 [bcp_columns](../native-client-odbc-extensions-bulk-copy-functions/bcp-columns.md) 지정 하려면 개수 열은 변경, 호출 [bcp_colfmt](../native-client-odbc-extensions-bulk-copy-functions/bcp-colfmt.md) 각 열에 대 한 서식을 변경 하려고 합니다. 호출한 후 이렇게 **bcp_init** 호출 하기 전에 **bcp_exec**합니다. **bcp_colfmt** 데이터 파일에 저장 되어 있는 열의 데이터 형식을 지정 합니다. 대량 복사하거나 대량 복사해 올 때 사용될 수 있습니다. 사용할 수도 있습니다 **bcp_colfmt** 행 및 열 종결자를 설정할 수 있습니다. 예를 들어 탭 문자를 포함 하는 데이터를 만들면 탭으로 구분 된 파일 사용 하 여 **bcp_colfmt** 각 열의 종결자로는 탭 문자를 설정할 수 있습니다.  
  
 때 대량 복사할를 사용 하 여 **bcp_colfmt**를 호출 하 여 만든 데이터 파일을 설명 하는 서식 파일을 쉽게 만들 수 있습니다 [bcp_writefmt](../native-client-odbc-extensions-bulk-copy-functions/bcp-writefmt.md) 마지막 호출 후 **bcp_colfmt**.  
  
 대량 복사 서식 파일에서 설명 하는 데이터 파일에서 호출 하 여 서식 파일을 읽을 [bcp_readfmt](../native-client-odbc-extensions-bulk-copy-functions/bcp-readfmt.md) 후 **bcp_init** 하기 전에 **bcp_exec**합니다.  
