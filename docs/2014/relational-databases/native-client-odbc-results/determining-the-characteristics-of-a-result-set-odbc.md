...
# <a name="determining-the-characteristics-of-a-result-set-odbc"></a>결과 집합의 특징 확인(ODBC)
 ...
 ODBC는 카탈로그 API 함수를 통해 애플리케이션에 메타데이터를 제공합니다. 합니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client ODBC 드라이버는 많은 ODBC API 카탈로그 함수를 해당 호출으로 구현 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 카탈로그 프로시저입니다.  
...
-   [SQLColAttribute](../native-client-odbc-api/sqlcolattribute.md) 나 [SQLDescribeCol](../native-client-odbc-api/sqldescribecol.md) 를 결과 집합의 열을 설명 합니다.  
 ...
 잘 디자인된 애플리케이션은 결과 집합을 알 수 없고 결과 집합이 이러한 함수에서 반환하는 정보를 사용하여 해당 결과 집합의 열을 바인딩한다는 가정 하에 작성됩니다. 애플리케이션에서는 문이 준비되거나 실행된 후 언제든지 이러한 함수를 호출할 수 있습니다. 그러나 최적의 성능을 위해 응용 프로그램 호출 해야 **SQLColAttribute**를 **SQLDescribeCol**, 및 **SQLNumResultCols** 문이 실행 된 후입니다.  
