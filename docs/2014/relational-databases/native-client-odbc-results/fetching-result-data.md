...
# <a name="fetching-result-data"></a>결과 데이터 인출
 ...
 ODBC 함수는 데이터를 프로그램 변수에 안팎으로 같은 이동 다루는 **SQLGetData**를 **SQLBindCol**, 및 [SQLBindParameter](../native-client-odbc-api/sqlbindparameter.md), 암시적 데이터 형식 지원 변환 합니다. 예를 들어 응용 프로그램이 정수 열을 문자열 프로그램 변수에 바인딩할 경우 드라이버는 해당 데이터를 자동으로 정수에서 문자로 변환한 후 이를 프로그램 변수에 전달합니다.  
