...
-   배치 모드 실행이라는 새로운 쿼리 실행 메커니즘이 SQL Server에 추가되어 CPU 사용량을 크게 줄입니다. 배치 모드 실행은 columnstore 저장소 형식과 긴밀히 통합되고 그에 맞게 최적화되어 있습니다. 배치 모드 실행을 벡터 기반 또는 벡터화된 실행이라고도 합니다.  
