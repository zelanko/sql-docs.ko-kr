...
## <a name="non-contained-databases"></a>포함되지 않은 데이터베이스  
 모든 데이터베이스는 기본 데이터 정렬이 있습니다. 기본 데이터 정렬은 데이터베이스를 만들거나 변경할 때 설정할 수 있습니다. 이러한 데이터 정렬은 데이터베이스의 모든 메타데이터에 사용되며 데이터베이스 내 모든 문자열 열의 기본값으로 사용됩니다. 사용자가 사용 하 여 특정 열의 데이터 정렬을 다르게 선택할 수는 `COLLATE` 절.  
...
```  
 ...
 이러한 방법은 상당히 간단해 보이지만 몇 가지 문제가 있습니다. 열의 데이터 정렬은 테이블 생성 하는 데이터베이스에 의존 하므로 문제가 발생에 저장 되는 임시 테이블 사용 `tempdb`합니다. 데이터 정렬을 `tempdb` 일반적으로 데이터베이스 데이터 정렬과 일치 하지 않아도 인스턴스용 데이터 정렬과 일치 합니다.  
...
## <a name="contained-databases"></a>포함된 데이터베이스  
 ...
 데이터베이스 데이터 정렬은 유지되지만 사용자 데이터의 기본 데이터 정렬로만 사용됩니다. 기본적으로 데이터베이스 데이터 정렬을 모델 데이터베이스 데이터 정렬과 같지만 통해 사용자가 변경할 수는 `CREATE` 또는 `ALTER DATABASE` 명령으로 포함 되지 않은 데이터베이스에 있습니다.  
  
 새 키워드 `CATALOG_DEFAULT`를 `COLLATE` 절에서 사용할 수 있습니다. 이 키워드는 포함된 데이터베이스와 포함되지 않은 데이터베이스 모두에서 현재 메타데이터 데이터 정렬에 대한 바로 가기로 사용됩니다. 즉, 포함 되지 않은 데이터베이스에서 `CATALOG_DEFAULT` 메타 데이터는 데이터베이스 데이터 정렬에서 데이터 정렬 되어 있으므로 현재 데이터베이스 데이터 정렬을 반환 합니다. 포함된 데이터베이스에서는 사용자가 카탈로그 데이터 정렬과 일치하지 않도록 데이터베이스 데이터 정렬을 변경할 수 있으므로 이 두 값이 다를 수 있습니다.  
...
## <a name="crossing-between-contained-and-uncontained-contexts"></a>포함된 컨텍스트와 포함되지 않은 컨텍스트 간 교차  
 포함된 데이터베이스의 세션은 포함된 상태로 유지되는 한 연결된 데이터베이스 내부에 남아 있어야 합니다. 이 경우의 동작은 매우 명확합니다. 하지만 세션이 포함된 컨텍스트와 포함되지 않은 컨텍스트 사이를 교차하는 경우 두 규칙 집합이 중재되어야 하므로 동작은 더 복잡해집니다. 사용자가 부분적으로 포함 된 데이터베이스에서 이러한 `USE` 다른 데이터베이스에 있습니다. 이 경우 데이터 정렬 규칙의 차이는 다음 원칙에 따라 처리됩니다.  
  
-   일괄 처리의 데이터 정렬 동작은 일괄 처리가 시작되는 데이터베이스에 의해 결정됩니다.  
  
 어떤 명령도 실행, 초기 포함 되기 이전에 이러한 결정 이루어집니다은 `USE`합니다. 즉, 일괄 처리에 포함된 된 데이터베이스에서 첫 번째 명령은 시작 되는 한 `USE` 포함 되지 않은 데이터베이스에 포함 된 데이터 정렬 동작 여전히에 사용할 일괄 처리 합니다. 이 점을 고려하면 예를 들어 변수에 대한 참조가 다음과 같은 여러 가지 결과를 생성할 수 있습니다.  
...
### <a name="example-3"></a>예제 3  
 다음 예에서는 참조가 원래는 구별되었던 여러 개의 일치 항목을 찾는 경우를 보여 줍니다. 시작 먼저 `tempdb` (인스턴스 동일한 대/소문자 구분 데이터 정렬을 있는) 다음 문을 실행 합니다.  
