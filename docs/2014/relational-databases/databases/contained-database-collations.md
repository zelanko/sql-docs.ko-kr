...
```  
 ...
 이 함수는 조금 특이한 함수입니다. 대/소문자 구분 데이터 정렬에서 합니다 @i 에 바인딩할 수 없습니다. return 절의 @I 또는 @?? 합니다. 대/소문자를 구분하지 않는 Latin1_General 데이터 정렬에서 @i는 @I에 바인딩하고 함수는 1을 반환합니다. 하지만 대/소문자 터키어 데이터 정렬에서 @i 바인딩할 @??, 함수 2를 반환 합니다. 이런 동작은 데이터 정렬이 다른 인스턴스 간을 이동하는 데이터베이스에 혼란을 초래합니다.  
