...
ms.locfileid: "36184999"
---
# <a name="database-properties-options-page"></a>데이터베이스 속성(옵션 탭)
  이 페이지를 사용하여 선택한 데이터베이스의 옵션을 확인하거나 수정할 수 있습니다. 이 페이지에서 사용할 수 있는 옵션에 대 한 자세한 내용은 참조 하십시오. [ALTER DATABASE SET 옵션 &#40;TRANSACT-SQL&#41;](/sql/t-sql/statements/alter-database-transact-sql-set-options)합니다.  
 ...
 **통계를 비동기적으로 자동 업데이트**  
 ...
 때 `False`, 날짜가 지난 통계의 자동 업데이트를 시작 하는 쿼리가 업데이트 된 통계는 쿼리 최적화 계획에 사용할 수 있을 때까지 기다립니다.  
  
 이 옵션을 설정 `True` 효과가 **통계 자동 업데이트** 도로 설정 `True`합니다.  
 ...
 **ANSI Null 기본값**  
 모든 사용자 정의 데이터 형식 또는로 명시적으로 정의 되지 않은 열에 대 한 null 값이 허용 `NOT NULL` 중는 `CREATE TABLE` 또는 `ALTER TABLE` 문 (기본 상태). 자세한 내용은 [SET ANSI_NULL_DFLT_ON&#40;Transact-SQL&#41;](/sql/t-sql/statements/set-ansi-null-dflt-on-transact-sql) 및 [SET ANSI_NULL_DFLT_OFF&#40;Transact-SQL&#41;](/sql/t-sql/statements/set-ansi-null-dflt-off-transact-sql)를 참조하세요.  
  
 **ANSI Null 설정**  
 Null 값과 함께 사용할 때 같음(`=`)과 같지 않음(`<>`) 비교 연산자의 동작을 지정합니다. 가능한 값은 `True` (설정) 및 `False` (해제) 합니다. `True`로 설정된 경우 Null 값에 대한 모든 비교가 UNKNOWN으로 평가됩니다. 때 `False`, 유니코드가 아닌 값을 null 값 비교 결과 `True` 두 값이 NULL입니다. 자세한 내용은 [SET ANSI_NULLS&#40;Transact-SQL&#41;](/sql/t-sql/statements/set-ansi-nulls-transact-sql)를 참조하세요.  
  
 **ANSI 패딩 설정**  
 ANSI 패딩을 설정할 것인지 여부를 지정합니다. 허용 된 값은 `True` (설정) 및 `False` (해제) 합니다. 자세한 내용은 [SET ANSI_PADDING&#40;Transact-SQL&#41;](/sql/t-sql/statements/set-ansi-padding-transact-sql)을 참조하세요.  
  
 **ANSI 경고 설정**  
 여러 오류 조건에 대한 ISO 표준 동작을 지정합니다. 때 `True`, null 값 (예: SUM, AVG, MAX, MIN, STDEV, STDEVP, VAR, VARP 또는 COUNT) 집계 함수에 있으면 경고 메시지가 생성 됩니다. 때 `False`, 경고가 발생 하지 않습니다. 자세한 내용은 [SET ANSI_WARNINGS&#40;Transact-SQL&#41;](/sql/t-sql/statements/set-ansi-warnings-transact-sql)를 참조하세요.  
 ...
 **날짜 상관 관계 최적화 설정**  
 때 `True`, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] FOREIGN KEY 제약 조건으로 연결 되 고 있는 데이터베이스에 두 테이블 간의 상관 관계 통계를 유지 관리 `datetime` 열입니다.  
  
 때 `False`, 상관 관계 통계가 유지 되지 않습니다.  
  
 **숫자 반올림 시 중단**  
 데이터베이스에서 반올림 오류를 처리하는 방법을 지정합니다. 가능한 값은 `True` 및 `False`입니다. `True`로 설정된 경우 식에서 전체 자릿수가 손실되면 오류가 발생합니다. 때 `False`, 전체 자릿수 손실이 발생 해도 오류 메시지를 생성 하지 않는 및 결과가 열 또는 결과 저장 한 변수의 정밀도로 반올림 됩니다. 자세한 내용은 [SET NUMERIC_ROUNDABORT&#40;Transact-SQL&#41;](/sql/t-sql/statements/set-numeric-roundabort-transact-sql)를 참조하세요.  
 ...
 **재귀적 트리거 설정**  
 다른 트리거가 트리거를 발생시킬 수 있는지 여부를 지정합니다. 가능한 값은 `True` 및 `False`입니다. 로 설정 하면 `True`, 이렇게 하면 트리거가 재귀적으로 발생할 수 있습니다. 로 설정 하면 `False`만 직접 재귀 금지 됩니다. 간접 재귀를 해제하려면 sp_configure를 사용하여 nested triggers 서버 옵션을 0으로 설정합니다. 자세한 내용은 [중첩 트리거 만들기](../triggers/create-nested-triggers.md)를 참조하세요.  
  
 `Trustworthy`  
 표시할 때 `True`,이 읽기 전용 옵션이 나타내는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터베이스 내에서 설정 된 가장 컨텍스트로 데이터베이스 외부 리소스에 액세스할 수 있습니다. 가장 컨텍스트는 데이터베이스 모듈에 EXECUTE AS 사용자 문이나 EXECUTE AS 절을 사용하여 데이터베이스 내에 설정할 수 있습니다.  
 ...
 기본적으로 모든 사용자 데이터베이스와 모든 시스템 데이터베이스 (제외 **MSDB**)이 속성은 설정 `False`합니다. **model** 및 **tempdb** 데이터베이스에 대해서는 이 값을 변경할 수 없습니다.  
  
 신뢰는 데이터베이스가 서버에 연결될 때마다 `False`로 설정됩니다.  
  
 때는 인증서와 서명을 사용 하는 가장 컨텍스트로 데이터베이스 외부 리소스에 액세스 하기 위한 방법이 권장된는 `Trustworthy` 옵션입니다.  
  
 이 속성을 설정하려면 ALTER DATABASE 문을 사용합니다.  
  
 **VarDecimal 저장소 형식 사용**  
 이 옵션은 읽기 전용 부터는 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 이상 버전에서는 모든 데이터베이스에 vardecimal 저장소 형식을 사용할 수 있습니다. 이 옵션은 [sp_db_vardecimal_storage_format](/sql/relational-databases/system-stored-procedures/sp-db-vardecimal-storage-format-transact-sql)을 사용합니다.  
 ...
 **암호화 사용**  
 때 `True`,이 데이터베이스는 데이터베이스 암호화에 사용할 수 있습니다. 데이터베이스 암호화 키는 암호화에 필요합니다. 자세한 내용은 [TDE&#40;투명한 데이터 암호화&#41;](../security/encryption/transparent-data-encryption.md)를 참조하세요.  
