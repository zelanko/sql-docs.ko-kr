...
2.  데이터베이스를 복사 하려는 컴퓨터 ( *대상 컴퓨터*)에서 데이터베이스를 복원 하려는의 인스턴스에 연결 합니다 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] . 필요한 경우 대상 서버 인스턴스에 원본 데이터베이스의 백업에 사용된 것과 같은 백업 디바이스를 만듭니다.  
