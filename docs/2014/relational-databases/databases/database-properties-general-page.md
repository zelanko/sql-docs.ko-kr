---
title: 데이터베이스 속성(일반 페이지) | Microsoft 문서
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.suite: ''
ms.technology:
- database-engine
ms.tgt_pltfrm: ''
ms.topic: article
f1_keywords:
- sql12.swb.databaseproperties.general.f1
ms.assetid: 43080d43-8841-4807-baea-34a29b0fe663
caps.latest.revision: 25
author: craigg-msft
ms.author: craigg
manager: jhubbard
ms.openlocfilehash: 5269d8c0ce124384e2b0a5635b697ab260484a7b
ms.sourcegitcommit: 5dd5cad0c1bbd308471d6c885f516948ad67dfcf
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 06/19/2018
ms.locfileid: "36081355"
---
# <a name="database-properties-general-page"></a>데이터베이스 속성(일반 페이지)
  이 페이지를 사용하여 선택한 데이터베이스의 속성을 확인하거나 수정할 수 있습니다.  
  
## <a name="options"></a>변수  
 **마지막 데이터베이스 백업**  
 데이터베이스를 마지막으로 백업한 날짜를 표시합니다.  
  
 **마지막 데이터베이스 로그 백업**  
 데이터베이스 트랜잭션 로그를 마지막으로 백업한 날짜를 표시합니다.  
  
 **이름**  
 데이터베이스 이름을 표시합니다.  
  
 **상태**  
 데이터베이스 상태를 표시합니다. 자세한 내용은 [Database States](database-states.md)을 참조하세요.  
  
 **소유자**  
 데이터베이스 소유자 이름을 표시합니다. **파일** 페이지에서 소유자를 변경할 수 있습니다.  
  
 **만든 날짜**  
 데이터베이스를 만든 날짜와 시간을 표시합니다.  
  
 **크기**  
 데이터베이스 크기를 MB 단위로 표시합니다.  
  
 **사용 가능한 공간**  
 데이터베이스에서 사용 가능한 공간의 크기를 MB 단위로 표시합니다.  
  
 **사용자 수**  
 데이터베이스에 연결된 사용자 수를 표시합니다.  
  
 **데이터 정렬 이름**  
 데이터베이스에 사용된 데이터 정렬을 표시합니다. **옵션** 페이지에서 데이터 정렬을 변경할 수 있습니다.  
  
## <a name="see-also"></a>관련 항목  
 [ALTER DATABASE&#40;Transact-SQL&#41;](/sql/t-sql/statements/alter-database-transact-sql)   
 [sys.databases&#40;Transact-SQL&#41;](/sql/relational-databases/system-catalog-views/sys-databases-transact-sql)  
  
  
