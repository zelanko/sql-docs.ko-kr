...
|`Address`|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 인스턴스를 실행하는 서버의 네트워크 주소입니다. `Address`는 일반적으로 서버의 네트워크 이름이지만 파이프, IP 주소나 TCP/IP 포트 및 소켓 주소와 같은 다른 이름일 수도 있습니다.<br /><br /> IP 주소를 지정하는 경우 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 구성 관리자에서 TCP/IP 또는 명명된 파이프 프로토콜이 설정되어 있는지 확인합니다.<br /><br /> [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client를 사용할 때 ODBC 연결 문자열에서 `Address` 값이 `Server`에 전달된 값보다 우선합니다. 또한 `Address=;`이면 `Server` 키워드에 지정된 서버에 연결하지만 `Address= ;, Address=.;`, `Address=localhost;` 및 `Address=(local);`이면 항상 로컬 서버에 연결합니다.<br /><br /> `Address` 키워드에 대한 전체 구문은 다음과 같습니다.<br /><br /> [*프로토콜*`:`]*주소*[`,`*포트 &#124;\pipe\pipename*]<br /><br /> *프로토콜* 될 수 있습니다 `tcp` (TCP/IP), `lpc` (공유 메모리) 또는 `np` (명명 된 파이프). 프로토콜에 대 한 자세한 내용은 참조 하세요. [Configure Client Protocols](../../../database-engine/configure-windows/configure-client-protocols.md)합니다.<br /><br /> 모두 *프로토콜* 또는 `Network` 키워드가 지정 되지 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client에 지정 된 프로토콜 순서를 사용할지 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Configuration Manager입니다.<br /><br /> *포트* 에 지정된 된 서버에서 연결할 포트입니다. 기본적으로 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]는 포트 1433을 사용합니다.|  
|`AnsiNPW`|"yes"인 경우 드라이버는 NULL 비교, 문자 데이터 패딩, 경고 및 NULL 연결 처리에 ANSI 정의 동작을 사용합니다. "no"인 경우 ANSI 정의 동작이 표시되지 않습니다. ANSI NPW 동작에 대 한 자세한 내용은 참조 하세요. [ISO 옵션의 효과](../../native-client-odbc-queries/executing-statements/effects-of-iso-options.md)합니다.|  
|`APP`|응용 프로그램 호출의 이름을 [SQLDriverConnect](../../native-client-odbc-api/sqldriverconnect.md) (선택 사항). 를 지정 하는 경우이 값은 **master.dbo.sysprocesses** 열 **program_name** 에서 반환 되 고 [sp_who](/sql/relational-databases/system-stored-procedures/sp-who-transact-sql) 고 [APP_NAME](/sql/t-sql/functions/app-name-transact-sql)함수입니다.|  
...
-   **Idatainitialize:: Getdatasource**  
 ...
 첫 번째 경우에는 DBPROPSET_DBINIT 속성 집합의 DBPROP_INIT_PROVIDERSTRING 속성을 설정하는 방법을 통해 공급자 문자열을 사용하여 연결 속성을 초기화할 수 있습니다. 후자의 경우 초기화 문자열을 전달할 수 있습니다 **idatainitialize:: Getdatasource** 연결 속성을 초기화 하는 방법입니다. 두 방법 모두 동일한 OLE DB 연결 속성을 초기화하지만 서로 다른 키워드 집합을 사용합니다. 사용 하는 키워드 집합이 **idatainitialize:: Getdatasource** 은 적어도 초기화 속성 그룹 내는 속성에 대해 설명 합니다.  
 ...
 사용 하 여 응용 프로그램 **idatainitialize:: Getdatasource** 에서 사용 된 키워드를 사용할 수도 있습니다 **idbinitialize:: Initialize** 기본값이 없는 속성에 대해서만 합니다. 응용 프로그램 모두를 사용 합니다 **idatainitialize:: Getdatasource** 키워드와 **idbinitialize:: Initialize** 초기화 문자열 키워드는 **IDataInitialize:: GetDataSource** 키워드 설정이 사용 됩니다. 응용 프로그램 사용 하지 마십시오이 가장 좋습니다 **idbinitialize:: Initialize** 키워드 **idatainitialize: Getdatasource** 이 동작은 나중에 유지 되지 않을 때 연결 문자열 해제합니다.  
  
 **참고:** 연결 문자열을 전달 **idatainitialize:: Getdatasource** 속성으로 변환 되 고 통해 적용할 **idbproperties:: Setproperties**합니다. 구성 요소 서비스에서 속성 설명을 찾으면 **idbproperties:: Getpropertyinfo**,이 속성이 독립 실행형 속성으로 적용 됩니다. 그렇지 않으면 DBPROP_PROVIDERSTRING 속성을 통해 속성이 적용됩니다. 예를 들어, 연결 문자열을 지정 하는 경우 **데이터 원본 = server1; Server = server2**, `Data Source` 속성으로 설정 됩니다 있지만 `Server` 는 공급자 문자열로 전달 됩니다.  
  
 같은 공급자별 속성의 인스턴스를 여러 개 지정하면 첫 번째 속성의 첫 번째 값이 사용됩니다.  
  
 Dbprop_init_providerstring을 지정 하는 OLE DB 응용 프로그램에서 사용 되는 연결 문자열 **idbinitialize:: Initialize** 다음 구문을 사용 해야 합니다.  
...
|`WSID`|SSPROP_INIT_WSID|워크스테이션 식별자입니다.|  
 ...
 사용 하 여 OLE DB 응용 프로그램에서 사용 되는 연결 문자열 **idatainitialize:: Getdatasource** 다음 구문을 사용 해야 합니다.  
 ...
 다음 표에서 사용할 수 있는 키워드 **idatainitialize:: Getdatasource**:  
 ...
 **참고** 연결 문자열에서 "이전 암호" 속성 SSPROP_AUTH_OLD_PASSWORD를 설정 공급자 문자열 속성을 통해 사용할 수 없는 현재 (만료 된) 암호.  
  
## <a name="activex-data-objects-ado-connection-string-keywords"></a>ADO(ActiveX Data Objects) 연결 문자열 키워드  
 ADO 응용 프로그램 집합을 **ConnectionString** 의 속성 **ADODBConnection** 개체 또는 연결 문자열을 매개 변수로 지정 합니다 **열기** 메서드의**ADODBConnection** 개체입니다.  
  
 ADO 응용 프로그램 OLE DB에서 사용 된 키워드를 사용할 수도 있습니다 **idbinitialize:: Initialize** 기본값이 없는 속성의 경우에 메서드. 응용 프로그램에서 ADO 키워드를 사용 하는 경우 및 **idbinitialize:: Initialize** 는 ADO 키워드 설정이 초기화 문자열 키워드를 사용할 수는 있습니다. 응용 프로그램에서 ADO 연결 문자열 키워드만 사용하는 것이 좋습니다.  
