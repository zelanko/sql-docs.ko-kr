...
# <a name="updating-an-application-from-sql-server-2005-native-client"></a>SQL Server 2005 Native Client에서 응용 프로그램 업데이트
 ...
 MDAC(Microsoft Data Access Components)에서 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client으로 업그레이드하는 경우에도 몇 가지 동작 차이점이 표시될 수 있습니다. 자세한 내용은 참조 [MDAC에서 SQL Server Native Client로 응용 프로그램 업데이트](updating-an-application-to-sql-server-native-client-from-mdac.md)합니다.  
...
|`sp_columns` 저장 프로시저 반환 **"NO"** 대신 **"NO"** IS_NULLABLE 열에 대 한 합니다.|부터는 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 10.0 ([!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]), `sp_columns` 저장 프로시저 반환 **"NO"** 대신 **"NO"** 는 IS_NULLABLE 열에 대 한 합니다.|  
...
|트리거가 실행될 때 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]에서 다른 결과를 반환할 수 있습니다.|[!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]에서 변경된 사항으로 인해 `NOCOUNT OFF`가 적용될 때 트리거를 실행하는 문에서 다른 결과가 반환될 수도 있습니다. 이 경우 응용 프로그램에서 오류가 발생할 수 있습니다. 이 오류를 해결 하려면 설정 `NOCOUNT ON` 트리거 또는 SQLMoreResults 호출 하 여 다음 결과로 이동 합니다.|  
