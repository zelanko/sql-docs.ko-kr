...
## <a name="opening-and-closing-a-database-connection"></a>데이터베이스 연결 열기 및 닫기  
 사용 하는 경우는 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 공급자는 데이터 원본 개체를 비동기적으로 초기화 하도록 설계 된 응용 프로그램 호출 하기 전에 DBPROP_INIT_ASYNCH 속성의 DBPROPVAL_ASYNCH_INITIALIZE 비트를 설정할 수  **Idbinitialize:: Initialize**합니다. 공급자를 호출 하 여 즉시 반환 하는이 속성이 설정 된 경우 **초기화** 작업이 즉시 완료 하면 s_ok이 고 또는 DB_S_ASYNCHRONOUS를 초기화 하는 비동기식으로 계속 됩니다. 응용 프로그램에 대해 쿼리할 수는 **IDBAsynchStatus** 또는 [ISSAsynchStatus](../../native-client-ole-db-interfaces/issasynchstatus-ole-db.md)인터페이스 데이터 원본 개체에 대 한 다음 호출 **idbasynchstatus:: Getstatus** 또는[ Issasynchstatus:: Waitforasynchcompletion](../../native-client-ole-db-interfaces/issasynchstatus-waitforasynchcompletion-ole-db.md) 초기화 상태를 가져올 수 있습니다.  
 ...
 **Idbasynchstatus:: Abort** 또는 [issasynchstatus:: Abort](../../native-client-ole-db-interfaces/issasynchstatus-abort-ole-db.md) 비동기 취소 하기 위해 호출할 수 **초기화** 호출 합니다. 소비자는 비동기 데이터 원본 초기화를 명시적으로 요청해야 합니다. 그렇지 않으면 **idbinitialize:: Initialize** 데이터 원본 개체가 완전히 초기화 될 때까지 반환 하지 않습니다.  
