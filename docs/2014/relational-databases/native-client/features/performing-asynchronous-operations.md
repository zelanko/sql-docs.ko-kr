 ...
 **Idbasynchstatus:: abort** 또는 [ISSAsynchStatus:: abort](../../native-client-ole-db-interfaces/issasynchstatus-abort-ole-db.md) 를 호출 하 여 비동기 **Initialize** 호출을 취소할 수 있습니다. 소비자는 비동기 데이터 원본 초기화를 명시적으로 요청해야 합니다. 그렇지 않으면 데이터 원본 개체가 완전히 초기화될 때까지 **IDBInitialize::Initialize**가 반환되지 않습니다.  
