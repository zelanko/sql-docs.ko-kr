...
-   `AttachDBFilename` 연결 문자열 키워드를 사용하여 서버가 연결해야 하는 데이터베이스 파일을 지정할 수 있습니다. 사용 하는 경우 `AttachDBFilename`사용 하 여 데이터베이스의 이름을 지정 하지 않으면 경우는 **데이터베이스** 연결 문자열 키워드를 응용 프로그램을 닫으면 LocalDB 인스턴스에서 데이터베이스가 제거 됩니다.  
