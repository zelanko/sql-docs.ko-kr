...
# <a name="impersonation-and-credentials-for-connections"></a>연결에 대한 가장 및 자격 증명
 ...
 경우에 따라 서비스 계정으로 실행하는 대신 `SqlContext.WindowsIdentity` 속성을 사용하여 호출자를 가장할 수도 있습니다. 
  `WindowsIdentity` 인스턴스는 호출 코드를 호출한 클라이언트의 ID를 나타내며, 클라이언트에서 Windows 인증을 사용한 경우에만 사용할 수 있습니다. 
  `WindowsIdentity` 인스턴스를 얻은 후 `Impersonate`를 호출하여 스레드의 보안 토큰을 변경한 다음 클라이언트 대신 ADO.NET 연결을 열 수 있습니다.  
