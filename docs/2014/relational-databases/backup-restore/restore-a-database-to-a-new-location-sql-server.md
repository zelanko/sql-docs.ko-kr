...
> [!NOTE]  
>  지정 시간 복원을 비롯하여 트랜잭션 로그를 백업 및 복원하는 예에서는 다음 `MyAdvWorks_FullRM` 예와 마찬가지로 [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)]에서 만든 `MyAdvWorks` 데이터베이스를 사용합니다. 그러나 결과 `MyAdvWorks_FullRM` 데이터베이스는 다음을 사용 하 여 전체 복구 모델을 사용 하도록 변경 해야 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문: ALTER DATABASE < database_name > SET RECOVERY FULL로 지정 합니다.  
