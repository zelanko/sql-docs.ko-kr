...
-   **suspect_pages 테이블에 기록되는 오류**  
    ...
    |복구됨(DBCC, AlwaysOn, 또는 페이지를 미러링 복구함)|5|  
    |DBCC에 의해 할당 취소됨|7|  
...
-   **데이터베이스 관리자의 유지 관리 역할**  
     ...
     suspect_pages 테이블에서 작업을 모니터링하기 위해 [Database Suspect Data Page 이벤트 클래스](../event-classes/database-suspect-data-page-event-class.md)를 사용할 수 있습니다. 일시적인 오류로 인해 행이 **suspect_pages** 테이블에 추가되기도 합니다. 그러나 많은 행이 이 테이블에 추가되는 경우 I/O 하위 시스템에 문제가 있을 수도 있습니다. 이 테이블에 추가되는 행 수가 갑자기 증가하는 경우에는 I/O 하위 시스템에 발생한 문제가 있는지 조사해 보는 것이 좋습니다.  
