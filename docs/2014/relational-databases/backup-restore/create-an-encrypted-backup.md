...
### <a name="backup-to-azure-storage-with-encryption"></a>암호화를 사용 하 여 Azure Storage에 백업  
 **Url SQL Server 백업** 옵션을 사용 하 여 azure storage에 백업을 만드는 경우 암호화 단계는 동일 하지만 url을 대상으로 사용 하 고 SQL 자격 증명을 사용 하 여 azure storage에 인증 해야 합니다. 암호화 옵션을 사용 하 [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)] 여를 구성 하려면 [azure에 대 한 관리 되는 백업 SQL Server 설정](enable-sql-server-managed-backup-to-microsoft-azure.md) [SQL Server 및 가용성 그룹에 대 한 azure로의 관리 되는 백업](../../database-engine/setting-up-sql-server-managed-backup-to-windows-azure-for-availability-groups.md)설정을 참조 하세요.  
  
 **사전 요구 사항:**  
  
-   Windows 스토리지 계정 및 컨테이너. 자세한 내용은 [1단원: Azure Storage 개체](../../tutorials/lesson-1-create-windows-azure-storage-objects.md)를 만듭니다.  
