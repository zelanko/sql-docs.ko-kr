...
### <a name="enable-and-configure-includesssmartbackupincludesss-smartbackup-mdmd-for-a-database"></a>설정 및 구성 [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)] 데이터베이스  
...
1.  **Microsoft Azure 저장소 계정 만들기:** Microsoft Azure storage 서비스에 백업을 저장 됩니다. 수행 하지 이미 계정이 있는 경우 Microsoft Azure 저장소 계정을 만들어야 합니다.
...
3.  **SQL Server 에이전트 서비스가 시작 되 고 실행 되도록:** SQL Server 에이전트 시작 현재 실행 하지 않는 경우.  [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)] 을 실행하여 백업 작업을 수행하려면 SQL Server 에이전트가 필요합니다.  SQL Server 에이전트가 자동으로 실행되어 백업 작업이 정기적으로 발생하도록 설정할 수 있습니다.  
