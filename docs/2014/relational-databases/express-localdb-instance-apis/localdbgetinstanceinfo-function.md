...
# <a name="localdbgetinstanceinfo-function"></a>LocalDBGetInstanceInfo 함수
 ...
 반환 하는 정보는 `struct` 라는 **LocalDBInstanceInfo**, 있으며 그 다음 정의 합니다.  
...
## <a name="details"></a>설명  
 도입 도입한 이유는 `struct` 크기 인수 (*lpInstanceInfoSize*)의 서로 다른 버전을 반환 하는 API를 사용 하도록 설정 하는 것은 **LocalDBInstanceInfostruct**, 효과적으로 및 이전 버전과 호환성을 사용 하도록 설정 합니다.  
  
 경우는 `struct` 크기 인수 (*lpInstanceInfoSize*)가 알려진된 버전의 크기와 일치 하는 **LocalDBInstanceInfostruct**, 해당 버전의는 `struct` 반환 됩니다. 그렇지 않으면 LOCALDB_ERROR_INVALID_PARAMETER가 반환됩니다.  
