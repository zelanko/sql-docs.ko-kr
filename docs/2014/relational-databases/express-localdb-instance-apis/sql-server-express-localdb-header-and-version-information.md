...
```  
 ...
 64 비트 프로그램은 `Installed Versions` 키의 64 비트 버전을 가리키는 값이 표시 됩니다는 **SQLUserInstance** DLL의 경우 32 비트 프로그램 (WOW64 모드의 64 비트 Windows에서 실행)는 자동으로 리디렉션됩니다는 동안`Installed Versions` 키 아래에 `Wow6432Node` 하이브입니다. 이 키의 32 비트 버전을 가리키는 값이 포함 된 **SQLUserInstance** DLL입니다.  
