...
|**DatabaseID**|**int**|USE *database* 문에서 지정된 데이터베이스의 ID입니다. 지정된 인스턴스에 대해 USE *database* 문이 실행되지 않은 경우 기본 데이터베이스의 ID입니다. [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] ServerName **데이터 열이 추적에서 캡처되고 서버를 사용할 수 있으면** 에 데이터베이스 이름이 표시됩니다. DB_ID 함수를 사용하여 데이터베이스의 값을 확인할 수 있습니다.|3|사용자 계정 컨트롤|  
...
|**HonorBrokerPriority**|**정수**|데이터베이스 HONOR_BROKER_PRIORITY 옵션의 현재 값: 0 = 꺼짐, 1 = ON입니다.|32|사용자 계정 컨트롤|  
...
|**StarvationElevation**|**int**|대화에 대해 구성 된 우선 순위 보다 우선 순위가 높은 메시지를 보냈습니다. 0 = false, 1 = true입니다.|33|사용자 계정 컨트롤|  
