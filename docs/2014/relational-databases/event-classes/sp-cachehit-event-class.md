...
|EventSubClass|`int`|이벤트 하위 클래스의 유형입니다.<br /><br /> 1=Execution Context Hit: 계획 캐시에 사용 가능한 실행 계획이 있습니다.<br /><br /> 2=Compplan Hit: 계획 캐시에 컴파일된 계획이 있습니다.|21|yes|  
...
|ObjectID|`int`|캐시에서 발견된 시스템이 할당한 저장 프로시저의 ID입니다.|22|yes|  
|ObjectName|`nvarchar`|캐시에서 발견된 개체의 이름입니다. ObjectName이 채워지면 TextData는 채워지지 않습니다.|34|yes|  
...
|TextData|`ntext`|캐시에서 발견된 SQL 코드의 텍스트입니다. TextData가 채워지면 ObjectName은 채워지지 않습니다.|1|yes|  
