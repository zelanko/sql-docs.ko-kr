...
# <a name="showplan-statistics-profile-event-class"></a>Showplan Statistics Profile 이벤트 클래스
  실행 계획 Statistics Profile 이벤트 클래스는가 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SQL 문을 실행할 때 발생 합니다. 여기서 설명하는 내용은 Showplan XML Statistics Profile 이벤트 클래스에서 사용할 수 있는 정보의 일부입니다.  
