...
|CPU|`int`|이벤트에서 사용한 CPU 시간입니다. 
  [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]부터 마이크로초 단위입니다. 이전 버전에서는 밀리초 단위입니다.|18|yes|  
...
|Duration|`bigint`|이벤트에서 사용한 시간입니다. 
  [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)]부터 마이크로초 단위입니다. 이전 버전에서는 밀리초 단위입니다.|13|yes|  
|EndTime|`datetime`|원격 프로시저 호출이 종료된 시간입니다.|15|yes|  
...
|읽기|`bigint`|원격 프로시저 호출로 실행된 페이지 읽기 수입니다.|16|yes|  
|RequestID|`int`|문을 포함하는 요청의 ID입니다.|49|yes|  
|RowCounts|`bigint`|RPC 일괄 처리의 행 수입니다.|48|yes|  
...
|TextData|`ntext`|원격 프로시저 호출의 텍스트입니다.|1|yes|  
|TransactionID|`bigint`|시스템이 할당한 트랜잭션의 ID입니다.|4|yes|  
|쓰기|`bigint`|원격 프로시저 호출로 실행된 페이지 쓰기 수입니다.|17|yes|  
