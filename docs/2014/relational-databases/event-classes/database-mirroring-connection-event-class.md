...
|**DatabaseID**|`int`|USE *database* 문으로 지정한 데이터베이스 ID이거나 지정한 인스턴스에 대해 실행된 USE *database*문이 없는 경우 기본 데이터베이스 ID입니다. [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)]**ServerName** 데이터 열이 추적에서 캡처되고 서버를 사용할 수 있는 경우 데이터베이스의 이름을 표시 합니다. **DB_ID** 함수를 사용 하 여 데이터베이스에 대 한 값을 확인 합니다.|3|yes|  
|**오류**|`int`|이벤트의 텍스트에 대 한 **sys.** 메시지의 메시지 ID 번호입니다. 이 이벤트에서 오류를 보고할 경우 이 열의 내용은 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 오류 번호에 해당합니다.|31|예|  
|**EventClass**|`int`|캡처된 이벤트 클래스 유형입니다. 
  **데이터베이스 미러링 Connection** 용은 항상 **151**입니다.|27|예|  
|**EventSequence**|`int`|이 이벤트의 시퀀스 번호입니다.|51|예|  
|**EventSubClass**|`nvarchar`|이 연결의 연결 상태입니다. 이 이벤트의 경우 하위 클래스는 다음 값 중 하나입니다.<br /><br /> **연결 중**입니다. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가 전송 연결을 시작하는 중입니다.<br /><br /> **연결**되었습니다. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가 전송 연결을 설정했습니다.<br /><br /> **연결 하지 못했습니다**. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가 전송 연결을 설정하지 못했습니다.<br /><br /> **닫습니다**. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가 전송 연결을 닫는 중입니다.<br /><br /> **닫힘**. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가 전송 연결을 닫았습니다.<br /><br /> **수락**. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가 다른 인스턴스의 전송 연결을 허용했습니다.<br /><br /> **IO 전송 오류**입니다. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에서 전송 오류가 발생했습니다.<br /><br /> **수신 IO 오류**입니다. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 에서 전송 오류가 발생했습니다.|21|yes|  
|**EID**|`uniqueidentifier`|이 연결의 엔드포인트 ID입니다.|54|예|  
