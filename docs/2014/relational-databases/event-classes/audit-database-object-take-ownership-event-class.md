...
|**ObjectType**|**int**|이벤트와 관련된 개체 유형을 나타내는 값입니다. 이 열에 대해 반환되는 값은 **sys.objects** 카탈로그 뷰에 있는 **type** 열의 해당 값과 [ObjectType 추적 이벤트 열](objecttype-trace-event-column.md)에 나열된 값의 조합입니다. 예를 들어 8277-U가 반환되면 개체 유형은 사용자 정의 테이블입니다.|28|예|  
