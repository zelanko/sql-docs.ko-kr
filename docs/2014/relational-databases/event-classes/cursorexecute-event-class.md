...
# <a name="cursorexecute-event-class"></a>CursorExecute 이벤트 클래스
  **CursorExecute** 이벤트 클래스는 API (응용 프로그래밍 인터페이스) 커서에서 발생 하는 커서 실행 이벤트를 설명 합니다. 커서 실행 이벤트는에서 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] 커서 준비 이벤트에 의해 생성 된 실행 계획의 커서를 만들고 채울 때 발생 합니다.  
...
|**처리**|**int**|ODBC, OLE DB, DB-Library가 서버와의 공동 실행을 위해 사용하는 정수입니다.|33|yes|  
