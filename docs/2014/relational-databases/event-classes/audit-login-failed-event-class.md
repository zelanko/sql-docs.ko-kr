...
# <a name="audit-login-failed-event-class"></a>Audit Login Failed 이벤트 클래스
  **Audit Login Failed** 이벤트 클래스는 사용자가에 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 로그인 하려고 시도 했으나 실패 했음을 나타냅니다. 이 이벤트 클래스는 연결 풀에서 다시 사용된 연결 또는 새 연결에 의해 발생합니다.  
...
|**Success**|**int**|1 = 성공, 0 = 실패. 이 이벤트는 항상 실패를 표시합니다.|23|yes|  
