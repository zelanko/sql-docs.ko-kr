...
|DatabaseID|`int`|시간 초과가 발생한 데이터베이스의 ID입니다. [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] 데이터베이스의 이름이 표시는 `ServerName` 데이터 열이 추적에서 캡처되고 서버를 사용할 수 있습니다. DB_ID 함수를 사용하여 데이터베이스의 값을 확인할 수 있습니다.|3|예|  
