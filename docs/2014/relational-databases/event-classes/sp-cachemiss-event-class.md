...
# <a name="spcachemiss-event-class"></a>SP:CacheMiss 이벤트 클래스
  SP:CacheMiss 이벤트 클래스는 캐시에 프로시저가 없음을 나타냅니다. SP: cachemiss 이벤트 클래스가 자주 발생 하면에서 더 많은 메모리를 사용할 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]수 있도록 설정 하 여 프로시저 캐시의 크기를 늘려야 함을 나타낼 수 있습니다.  
...
|ObjectName|`nvarchar`|저장 프로시저의 이름입니다. ObjectName을 채우면 TextData가 채워지지 않습니다.|34|yes|  
...
|TextData|`ntext`|캐시된 SQL 코드 텍스트입니다. TextData를 채우면 ObjectName이 채워지지 않습니다.|1|yes|  
