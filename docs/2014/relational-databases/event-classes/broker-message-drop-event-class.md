...
|-----------------|----------|-----------------|-------------------|----------------|  
|**애플리케이션 이름**|`nvarchar`|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]인스턴스에 연결한 클라이언트 애플리케이션의 이름입니다. 이 열은 프로그램의 표시 이름이 아니라 애플리케이션에서 전달한 값으로 채워집니다.|10|yes|  
|**BigintData1**|`bigint`|배달할 수 없는 메시지의 시퀀스 번호입니다.|52|예|  
|**BigintData2**|`bigint`|마지막 메시지의 시퀀스 번호를 성공적으로 인식했습니다.|53|예|  
...
|**EventClass**|`int`|캡처된 이벤트 클래스 유형입니다. 
  **Broker:MessageUndeliverable**의 경우 항상 **160**입니다.|27|예|  
|**EventSequence**|`int`|이 이벤트의 시퀀스 번호입니다.|51|예|  
|**EventSubClass**|`nvarchar`|배달할 수 없는 메시지가 순차화된 메시지인지 여부를 나타냅니다. 다음 두 값 중 하나입니다.<br /><br /> **순차화 된 메시지**입니다. 배달할 수 없는 메시지가 순차화된 메시지입니다.<br /><br /> **순차화 되지 않은 메시지**입니다. 배달할 수 없는 메시지가 순차화된 메시지가 아닙니다.|21|yes|  
|**EID**|`uniqueidentifier`|배달할 수 없는 메시지가 속한 대화의 대화 ID입니다. 이 식별자는 메시지의 일부로 전송되며 양쪽 대화 상대 간에 공유합니다.|54|예|  
...
|**IntegerData2**|`int`|배달할 수 없는 메시지가 인식한 메시지 조각 번호입니다.|55|예|  
...
|**TextData**|`ntext`|
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]가 메시지를 배달할 수 없는 이유입니다.|1|yes|  
