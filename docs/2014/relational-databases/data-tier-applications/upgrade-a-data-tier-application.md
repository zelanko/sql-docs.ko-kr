...
-   **Ignore Data Loss** – 경우 `True`, 업그레이드 작업 중 일부 데이터 손실이 발생할 경우에 진행 됩니다. `False`인 경우 이러한 작업은 업그레이드를 종료합니다. 예를 들어 현재 데이터베이스의 테이블이 새로운 DAC의 스키마에 제공 되지 않는 경우 테이블 삭제할 경우 `True` 지정 됩니다. 기본 설정은 `True`합니다.  
  
-   **변경 내용에는 블록** – 경우 `True`, 데이터베이스 스키마가 이전 DAC에 정의 된 것과 다르면 업그레이드가 종료 됩니다. 경우 `False`, 업그레이드가 변경이 감지 되는 경우에 계속 합니다. 기본 설정은 `False`합니다.  
  
-   **실패 시 롤백** – 경우 `True`, 경우 업그레이드가 트랜잭션에 포함 오류가 발생 하는 경우에 롤백을 시도 합니다. `False`인 경우 변경이 되면 모든 변경 내용이 커밋되고 오류가 발생하면 데이터베이스의 이전 백업을 복원해야 할 수 있습니다. 기본 설정은 `False`합니다.  
  
-   **Skip Policy Validation** – 경우 `True`, DAC 서버 선택 정책이 평가 되지 않습니다. 경우 `False`정책을 평가 하 고 유효성 검사 오류가 있으면 업그레이드가 종료 됩니다. 기본 설정은 `False`합니다.  
...
4.  사용 하 여 `add_DacActionStarted` 및 `add_DacActionFinished` DAC 업그레이드 이벤트를 구독할 수 있습니다.  
  
5.  설정의 `DacUpgradeOptions`합니다.  
  
6.  사용 하 여는 `IncrementalUpgrade` 메서드를 DAC를 업그레이드 합니다.  
