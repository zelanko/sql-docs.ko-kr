...
### <a name="assigning-for-xml-query-results-to-an-xml-type-variable"></a>xml 유형 변수에 FOR XML 쿼리 결과 할당  
 다음 예에서는 FOR XML 결과가 `xml` 유형 변수 `@x`에 할당됩니다. 쿼리 `BusinessEntityID`는의 `FirstName` `LastName` `AdditionalContactInfo` 열에서,, 및 추가 전화 번호와 같은 연락처 정보를 검색 합니다. `xml``TYPE` `FOR XML` 절은 `TYPE` 지시어를 지정하므로 XML은 `xml` 유형으로 반환되며 변수에 할당됩니다.  
