...
-   가변 길이 저장소 형식을 사용 하 여 숫자 형식에 대 한 (예를 들어 `integer`, `decimal`, 및 `float`)과 숫자를 기반으로 하는 유형 (예를 들어 `datetime` 고 `money`).  
...
|`datetimeoffset`|예|유사한 `datetime2`는 2 바이트의 형식 (hh: mm)의 표준 시간대가 있다는 점을 제외 하 고 있습니다.<br /><br /> `datetime2`와 같이 압축을 통해 2바이트를 절약할 수 있습니다.<br /><br /> 표준 시간대 값에 대해 대부분의 경우 MM 값은 0일 수 있습니다. 따라서 압축을 통해 1바이트를 절약할 수도 있습니다.<br /><br /> 행 압축에 대해 저장소의 변경 내용이 없습니다.|  
