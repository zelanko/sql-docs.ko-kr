...
### <a name="recursive-elements"></a>재귀적 요소  
 재귀 관계에서 부모 요소와 자식 요소 모두에 `sql:max-depth`를 지정하면 부모에 지정된 `sql:max-depth` 주석이 우선합니다. 예를 들어 다음 스키마에서는 부모 및 자식 직원 요소 모두에 대해 `sql:max-depth` 주석이 지정되었습니다. 이 경우 `sql:max-depth=4` ** \< Emp>** 부모 요소 (감독자 역할 재생)에 지정 된가 우선적으로 적용 됩니다. `sql:max-depth`자식 ** \< Emp>** 요소 (supervisee의 역할 재생)에 지정 된은 무시 됩니다.  
...
### <a name="nonrecursive-elements"></a>비재귀 요소  
 스키마에서 재귀가 발생하지 않는 요소에 대해 지정한 `sql:max-depth` 주석은 무시됩니다. 다음 스키마에서 ** \< emp>** 요소는 ** \<>** 자식 요소로 구성 되며,이 요소로 인해 ** \< emp>** 자식 요소가 있습니다.  
  
 이 스키마에서 `sql:max-depth` ** \< 상수>** 요소에 지정 된 주석은 ** \< Emp>** 부모와 ** \< 상수>** 자식 요소 사이에 재귀가 없기 때문에 무시 됩니다. 그러나 ** \< emp>** 상위 및 ** \< emp>** 자식 사이에 재귀가 있습니다. 이 스키마에서는 두 요소 모두에 대해 `sql:max-depth` 주석을 지정합니다. 따라서 `sql:max-depth` 상위 항목 (감독자 역할의** \< Emp>** )에 지정 된 주석이 우선 적용 됩니다.  
 ...
 ** \< 제한>** 의 복합 형식 파생이 있는 경우 해당 하는 기본 복합 형식의 요소는 주석을 지정할 수 없습니다 `sql:max-depth` . 이 경우 파생된 유형의 요소에 `sql:max-depth` 주석을 추가할 수 있습니다.  
  
 반면에 ** \< 확장>** 에의 한 복합 유형 파생이 있는 경우 해당 하는 기본 복합 유형의 요소에서 주석을 지정할 수 있습니다 `sql:max-depth` .  
  
 예를 들어 다음 XSD 스키마에서는 기본 유형에 대해 `sql:max-depth` 주석을 지정했으므로 오류가 생성됩니다. 이 주석은 다른 형식에서 ** \<>제한** 에 의해 파생 된 형식에서 지원 되지 않습니다. 이 문제를 해결하려면 스키마를 변경하고 파생된 유형의 요소에 대해 `sql:max-depth` 주석을 지정해야 합니다.  
...
```  
 ...
 이 스키마에서는 `sql:max-depth` 복합 유형에 대해 `CustomerBaseType`를 지정합니다. 또한 스키마는에서 파생 되는 형식의 ** \< Customer>** 요소를 지정 합니다 `CustomerType` `CustomerBaseType` . 제한 기본 유형으로 정의된 요소에서는 `sql:max-depth`가 지원되지 않으므로 이러한 스키마에서 지정한 XPath 쿼리는 오류를 생성합니다.  
