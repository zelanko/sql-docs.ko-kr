...
### <a name="nonrecursive-elements"></a>비재귀 요소  
 ...
 이 스키마 `sql:max-depth` 에서 ** \<상수>** 요소에 지정 된 주석은 ** \<Emp>** 부모와 ** \<상수>** 자식 요소 사이에 재귀가 없기 때문에 무시 됩니다. 그러나 ** \<emp>** 상위 및 ** \<emp>** 자식 사이에 재귀가 있습니다. 이 스키마에서는 두 요소 모두에 대해 `sql:max-depth` 주석을 지정합니다. 따라서 상위 항목 `sql:max-depth` (**\<** 감독자 역할의 Emp>)에 지정 된 주석이 우선 적용 됩니다.  
...
## <a name="complex-types-derived-by-restriction"></a>제한에 의해 파생되는 복합 유형  
 제한>의 복합 형식 파생이 있는 경우 해당 하는 기본 복합 형식의 요소는 `sql:max-depth` 주석을 지정할 수 없습니다. ** \< ** 이 경우 파생된 유형의 요소에 `sql:max-depth` 주석을 추가할 수 있습니다.  
  
 반면에 ** \<확장>** 에의 한 복합 유형 파생이 있는 경우 해당 하는 기본 복합 유형의 요소에서 `sql:max-depth` 주석을 지정할 수 있습니다.  
...
```  
 ...
 이 스키마에서는 `sql:max-depth` 복합 유형에 대해 `CustomerBaseType`를 지정합니다. 또한 스키마는에서 `CustomerType` `CustomerBaseType`파생 되는 형식의 ** \<Customer>** 요소를 지정 합니다. 제한 기본 유형으로 정의된 요소에서는 `sql:max-depth`가 지원되지 않으므로 이러한 스키마에서 지정한 XPath 쿼리는 오류를 생성합니다.  
