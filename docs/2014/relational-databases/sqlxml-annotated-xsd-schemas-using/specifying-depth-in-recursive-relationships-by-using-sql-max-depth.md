...
5.  SQLXML 4.0 테스트 스크립트(Sqlxml4test.vbs)를 만든 다음 이 스크립트를 사용하여 템플릿을 실행합니다. 자세한 내용은 참조 [SQLXML 4.0 쿼리 실행을 사용 하 여 ADO](../sqlxml/using-ado-to-execute-sqlxml-4-0-queries.md)합니다.  
...
## <a name="sqlmax-depth-annotation"></a>sql:max-depth 주석  
 ...
 스키마에서 `sql:max-depth` 주석을 사용하여 스키마에 설명된 재귀 관계의 재귀 깊이를 지정할 수 있습니다. 값은 `sql:max-depth` 주석은 재귀 횟수를 나타내는 양의 정수 (1-50): 값 1 요소에서 재귀가 중단는 `sql:max-depth` 주석이 지정 되지 않으면 값이 2의 다음 수준에서 재귀가 중단 에 있는 요소에서 `sql:max-depth` 지정 되어 있으면 등에입니다.  
...
### <a name="recursive-elements"></a>재귀적 요소  
 재귀 관계에서 부모 요소와 자식 요소 모두에 `sql:max-depth`를 지정하면 부모에 지정된 `sql:max-depth` 주석이 우선합니다. 예를 들어 다음 스키마에서는 부모 및 자식 직원 요소 모두에 대해 `sql:max-depth` 주석이 지정되었습니다. 이 경우 `sql:max-depth=4`에 지정 된 된  **\<Emp >** (supervisor 역할을 재생) 부모 요소에 우선 합니다. `sql:max-depth` 자식 요소에서 지정 된  **\<Emp >** (supervisee 역할을 재생) 요소는 무시 됩니다.  
...
### <a name="nonrecursive-elements"></a>비재귀 요소  
 스키마에서 재귀가 발생하지 않는 요소에 대해 지정한 `sql:max-depth` 주석은 무시됩니다. 다음 스키마에는  **\<Emp >** 요소는  **\<상수 >** 는 대 한 자식 요소는  **\<Emp >** 자식 요소입니다.  
  
 이 스키마는 `sql:max-depth` 에 지정 된 주석을  **\<상수 >** 사이 재귀 관계가 없으므로 요소는 무시 됩니다는  **\<Emp >** 부모 및  **\<상수 >** 자식 요소입니다. 간에 재귀가 발생 하지만  **\<Emp >** 상위 항목 및  **\<Emp >** 자식입니다. 이 스키마에서는 두 요소 모두에 대해 `sql:max-depth` 주석을 지정합니다. 따라서는 `sql:max-depth` 상위 항목에서 지정 된 주석이 (**\<Emp >** supervisor 역할에서) 우선적으로 적용 합니다.  
...
## <a name="complex-types-derived-by-restriction"></a>제한에 의해 파생되는 복합 유형  
 복합 유형이 파생 되는 경우  **\<제한 >**, 해당 기본 복합 유형의 요소를 지정할 수 없습니다는 `sql:max-depth` 주석입니다. 이 경우 파생된 유형의 요소에 `sql:max-depth` 주석을 추가할 수 있습니다.  
  
 반면, 의해 복합 유형이 파생 되는 경우  **\<확장 >**, 해당 기본 복합 유형의 요소를 지정할 수는 `sql:max-depth` 주석입니다.  
  
 예를 들어 다음 XSD 스키마에서는 기본 유형에 대해 `sql:max-depth` 주석을 지정했으므로 오류가 생성됩니다. 이 주석이 하 여 파생 형식에서 지원 되지 않습니다  **\<제한 >** 다른 형식에서 합니다. 이 문제를 해결하려면 스키마를 변경하고 파생된 유형의 요소에 대해 `sql:max-depth` 주석을 지정해야 합니다.  
...
```  
 ...
 이 스키마에서는 `sql:max-depth` 복합 유형에 대해 `CustomerBaseType`를 지정합니다. 스키마도 지정 된  **\<고객 >** 형식의 요소 `CustomerType`에서 파생 된 `CustomerBaseType`합니다. 제한 기본 유형으로 정의된 요소에서는 `sql:max-depth`가 지원되지 않으므로 이러한 스키마에서 지정한 XPath 쿼리는 오류를 생성합니다.  
