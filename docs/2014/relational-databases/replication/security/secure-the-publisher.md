...
-   병합 에이전트  
 ...
 이러한 에이전트에 적절한 로그인을 제공하고 필요한 최소 권한만 부여하는 원칙을 따르며 모든 암호 스토리지를 보호하는 것이 좋습니다. 각 에이전트에 필요한 사용 권한에 대한 자세한 내용은 [Replication Agent Security Model](replication-agent-security-model.md)을 참조하십시오.  
...
> [!NOTE]  
>  복제는 각 게시의 게시자에 PAL 멤버 자격을 적용할 역할을 만듭니다. 역할은 병합 복제에 대 한 **Msmerge_** 형식으로 이름이 있고 _\<PublicationID>_ **MSReplPAL_** _\<PublicationDatabaseID>_ **_** _\<PublicationID>_ 트랜잭션 및 스냅숏 복제의 경우 MSReplPAL_ _입니다.  
