...
>  에이전트 매개 변수에 대한 변경 사항은 다음에 에이전트가 시작될 때 적용됩니다. 에이전트가 연속적으로 실행되는 경우에는 에이전트를 중단했다가 다시 시작해야 합니다.  
 ...
 매개 변수를 직접 수정할 수도 있지만 에이전트 프로필을 통해 수정하는 것이 일반적입니다. 자세한 내용은 [Replication Agent Profiles](replication-agent-profiles.md)을 참조하세요.  
...
|병합 게시 파티션에 대한 스냅샷 에이전트|**Dyn_\<게시자>-\<publicationdatabase>>-\<게시>-\<GUID>**|[Replication Snapshot Agent](replication-snapshot-agent.md)|  
...
|밀어넣기 구독에 대한 배포 에이전트|**\<\<\<게시자>-publicationdatabase>>-게시>-구독자>-\<정수>1 \<** <sup></sup>|[Replication Distribution Agent](replication-distribution-agent.md)|  
|끌어오기 구독에 대한 배포 에이전트|**\<\<\<\<게시자>-publicationdatabase>>-게시>-구독자>-subscriptiondatabase>-\<GUID>2 \<** <sup></sup>|[Replication Distribution Agent](replication-distribution-agent.md)|  
|SQL Server 이외 구독자의 밀어넣기 구독에 대한 배포 에이전트|**\<게시자>-\<publicationdatabase>>-\<게시>-\<구독자>-\<정수>**|[Replication Distribution Agent](replication-distribution-agent.md)|  
|큐 판독기 에이전트|**[\<배포자>]. \<정수>**|[Replication Queue Reader Agent](replication-queue-reader-agent.md)|  
  
 <sup>1</sup> Oracle 게시에 대 한 밀어넣기 구독의 경우 게시자 ** \<>-\<publicationdatabase>** 이 ** \<\<** 아닌 게시자>>>  
  
 <sup>2</sup> Oracle 게시에 대 한 끌어오기 구독의 경우 게시자 ** \<>-\<publicationdatabase>** 가 ** \<아닌 게시자>\<DistributionDatabase**>>  
...
1.  
  [!INCLUDE[ssManStudio](../../../includes/ssmanstudio-md.md)]에서 적절한 컴퓨터에 연결한 다음 해당 서버 노드를 확장합니다.  
...
4.  
  **작업 속성 - **Job>** 대화 상자의 \<단계** 페이지에서 **에이전트 실행** 단계를 선택한 다음 **편집**을 클릭합니다.  
  
5.  
  **작업 단계 속성 - 에이전트를 실행합니다** 대화 상자에서 **명령** 필드를 편집합니다.  
