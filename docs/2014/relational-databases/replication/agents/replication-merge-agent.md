 ...
 **-ExchangeType** [ **1**| **2**| **3**]  
 > [!WARNING]  
>  [!INCLUDE[ssNoteDepFutureDontUse](../../../includes/ssnotedepfuturedontuse-md.md)] 업로드를 제한 하려면 사용 하 여는 `@subscriber_upload_options` 의 `sp_addmergearticle` 대신 합니다.  
...
> [!NOTE]  
>  구독자에서의 데이터 형식과 게시자에서의 데이터 형식이 다른 경우 이진 체크섬 또는 체크섬 유효성 검사가 올바르지 않을 수 있습니다. 자세한 내용은 [복제된 데이터의 유효성 검사](../validate-replicated-data.md)의 "데이터 유효성 검사에 대한 고려 사항" 섹션을 참조하세요.  
