 ...
 **새 피어 연결 추가**  
 두 노드 사이에 연결을 추가합니다. 예를 들어 노드 A와 노드 B 사이의 연결을 추가 하는 경우 복제는 두 개의 구독이 추가: 첫 번째 노드는 노드 b에서 게시에서 변경 내용을 받는 데 사용 하도록 설정 하 고, 두 번째는 노드 B에서 노드 A의 게시 변경 내용을 받았습니다.  
