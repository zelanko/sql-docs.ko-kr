...
-   "Oracle 서버 인스턴스 ' \<*OraclePublisherName*> '이 (가) 이전에 ' '을 (를) 배포자로 사용 하도록 구성 되었습니다 \<*SQLServerDistributorName*> . ' '을 (를) \<*NewSQLServerDistributorName*> 배포자로 사용 하려면 Oracle 서버 인스턴스의 현재 복제 구성을 제거 하 여 해당 서버 인스턴스의 모든 게시를 삭제 해야 합니다. "  
  
-   "Oracle 서버 ' \<*OracleServerName*> '은 (는) \<*OraclePublisherName*> 배포자 ' \<*SQLServerDistributorName*> . *\<DistributionDatabaseName>* '에서 이미 게시자 ' ' (으)로 정의 되었습니다. 게시자를 삭제 하거나 공용 동의어 ' *\<SynonymName>* '을 (를) 삭제 하 여 다시 만드십시오. "  
...
## <a name="sql-server-error-21663-is-raised-regarding-the-lack-of-a-primary-key"></a>PRIMARY KEY 부재와 관련된 SQL Server 오류 21663이 발생했습니다.  
 ...
 "원본 테이블 []. []에 대해 올바른 기본 키를 찾을 수 없습니다 \<*TableOwner*> . \<*TableName*> "  
...
## <a name="sql-server-error-21642-is-raised-regarding-a-duplicate-linked-server-login"></a>연결된 서버로의 중복 로그인과 관련된 SQL Server 오류 21642가 발생했습니다.  
 ...
 "유형이 다른 게시자에는 연결된 서버가 필요합니다. ' ' (이) 라는 연결 된 서버가 *\<LinkedServerName>* 이미 있습니다. 연결된 서버를 제거하거나 다른 게시자 이름을 선택하십시오."  
