...
####  <a name="Permissions"></a> Permissions  
 비관리자의 복제 모니터의 멤버 사용을 허용 하는 **sysadmin** 고정된 서버 역할에서 배포 데이터베이스에 사용자를 추가 하 고 해당 사용자를 할당 해야는 `replmonitor` 역할입니다.  
...
6.  선택 된 `replmonitor` 확인란에는 **데이터베이스 역할 멤버 자격** 눈금.  
...
1.  배포 데이터베이스의 배포자에서 [sp_helpuser&#40;Transact-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-helpuser-transact-sql)를 실행합니다. 사용자에 나열 되지 않은 경우 `UserName` 결과 집합에는 사용자에 게 있어야 사용 하 여 배포 데이터베이스에 대 한 액세스는 [CREATE USER &#40;TRANSACT-SQL&#41; ](/sql/t-sql/statements/create-user-transact-sql) 문입니다.  
  
2.  배포 데이터베이스의 배포자에서 실행 [sp_helprolemember &#40;TRANSACT-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-helprolemember-transact-sql), 값을 지정 `replmonitor` 에 대 한는 **@rolename** 매개 변수입니다. 사용자에 표시 된 경우 `MemberName` 결과 집합에 사용자 이미이 역할에 속해 있습니다.  
  
3.  사용자에 속하지 않은 경우는 `replmonitor` 역할을 실행 [sp_addrolemember &#40;TRANSACT-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql) 배포 데이터베이스의 배포자에서 합니다. 값을 지정 `replmonitor` 에 대 한 **@rolename** 및 데이터베이스 사용자의 이름 또는 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] Windows 로그인에는 추가할 **@membername**합니다.  
...
2.  사용자가에 속해는 `replmonitor` 역할을 실행 [sp_droprolemember &#40;TRANSACT-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sp-droprolemember-transact-sql) 배포 데이터베이스의 배포자에서 합니다. 값을 지정 `replmonitor` 에 대 한 **@rolename** 및 데이터베이스 사용자 또는 제거할 Windows 로그인의 이름을 **@membername**합니다.  
