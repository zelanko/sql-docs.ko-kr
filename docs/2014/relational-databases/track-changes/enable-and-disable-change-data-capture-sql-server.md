...
## <a name="enable-change-data-capture-for-a-database"></a>데이터베이스에 변경 데이터 캡처 설정  
 ...
 데이터베이스 변경 데이터 캡처를 사용 하는 경우는 `cdc` 스키마 `cdc` 데이터베이스에 대 한 사용자, 메타 데이터 테이블 및 다른 시스템 개체가 만들어집니다. `cdc` 스키마는 변경 데이터 캡처 메타 데이터 테이블이 포함 되며 개별 변경 테이블이 변경 데이터에 대 한 리포지토리로 사용 될 원본 테이블은 변경 데이터 캡처를 사용 하도록 설정 합니다. `cdc` 스키마에 사용 되는 관련된 시스템 함수도 포함 변경 데이터에 대 한 쿼리 합니다.  
...
## <a name="enable-change-data-capture-for-a-table"></a>테이블의 변경 데이터 캡처 설정  
 변경 데이터 캡처의 멤버에 대 한 데이터베이스에 사용 하도록 설정한 후 합니다 `db_owner` 고정된 데이터베이스 역할 저장된 프로시저를 사용 하 여 개별 원본 테이블의 캡처 인스턴스를 만들 수 있습니다 `sys.sp_cdc_enable_table`합니다. 원본 테이블에 변경 데이터 캡처가 이미 설정되었는지 확인하려면 `sys.tables` 카탈로그 뷰의 is_tracked_by_cdc 열을 살펴봅니다.  
...
## <a name="disable-change-data-capture-for-a-table"></a>테이블의 변경 데이터 캡처 해제  
 멤버는 `db_owner` 고정된 데이터베이스 역할 저장된 프로시저를 사용 하 여 개별 원본 테이블의 캡처 인스턴스를 제거할 수 `sys.sp_cdc_disable_table`입니다. 원본 테이블에 현재 변경 데이터 캡처가 설정되었는지 확인하려면 `is_tracked_by_cdc` 카탈로그 뷰의 `sys.tables` 열을 살펴봅니다. 해제를 수행한 후 데이터베이스에 변경 데이터 캡처 기능이 설정된 테이블이 없으면 변경 데이터 캡처 작업도 제거됩니다.  
