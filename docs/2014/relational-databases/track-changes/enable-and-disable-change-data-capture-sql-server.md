...
## <a name="enable-change-data-capture-for-a-database"></a>데이터베이스에 변경 데이터 캡처 설정  
 ...
 데이터베이스가 변경 데이터 캡처를 사용 하는 경우는 `cdc` 스키마 `cdc` 데이터베이스에 대 한 사용자, 메타 데이터 테이블 및 다른 시스템 개체가 만들어집니다. `cdc` 스키마 변경 데이터 캡처 메타 데이터 테이블을 포함 하 고 원본 테이블에 변경 데이터 캡처를 활성화 한 후 개별 변경 테이블 저장소의 역할을 변경 데이터에 대 한 합니다. `cdc` 스키마에 사용 되는 관련된 시스템 함수도 포함 변경 데이터에 대 한 쿼리입니다.  
...
## <a name="disable-change-data-capture-for-a-database"></a>데이터베이스의 변경 데이터 캡처 해제  
 멤버는 `sysadmin` 고정된 서버 역할에서 저장된 프로시저를 실행할 수 [sys.sp_cdc_disable_db &#40;TRANSACT-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sys-sp-cdc-disable-db-transact-sql) 에 데이터베이스 컨텍스트를 데이터베이스에 대 한 변경 데이터 캡처를 사용 하지 않도록 설정 합니다. 개별 테이블에서 먼저 이 기능을 해제한 후 데이터베이스에서 이 기능을 해제할 필요는 없습니다. 데이터베이스를 사용 하지 않으면 모든 연결 된 변경 데이터 캡처 메타 데이터를 제거 등의 `cdc` 사용자 및 스키마와 변경 데이터 캡처 작업 합니다. 하지만 변경 데이터 캡처로 생성된 모든 제어 역할은 자동으로 제거되지 않으며 명시적으로 삭제해야 합니다. 데이터베이스에 이 기능이 설정되었는지 확인하려면 sys.databases 카탈로그 뷰의 `is_cdc_enabled` 열을 쿼리합니다.  
...
## <a name="enable-change-data-capture-for-a-table"></a>테이블의 변경 데이터 캡처 설정  
 데이터베이스에 변경 데이터 캡처의 멤버에 대해 설정한 후는 `db_owner` 고정된 데이터베이스 역할 저장된 프로시저를 사용 하 여 개별 원본 테이블에 대 한 캡처 인스턴스를 만들 수 `sys.sp_cdc_enable_table`합니다. 원본 테이블에 변경 데이터 캡처가 이미 설정되었는지 확인하려면 `sys.tables` 카탈로그 뷰의 is_tracked_by_cdc 열을 살펴봅니다.  
 ...
 기본적으로 변경 테이블은 데이터베이스의 기본 파일 그룹에 있습니다. 개별 변경 테이블의 배치를 제어하려는 데이터베이스 소유자는 *@filegroup_name* 매개 변수를 사용하여 캡처 인스턴스와 연결된 변경 테이블에 대해 특정 파일 그룹을 지정할 수 있습니다. 명명된 파일 그룹은 이미 존재해야 합니다. 일반적으로 변경 테이블은 원본 테이블과 별도의 파일 그룹에 배치하는 것이 좋습니다. 참조는 `Enable a Table Specifying Filegroup Option` 서식 파일의 사용을 보여 주는 예제는 *@filegroup_name* 매개 변수입니다.  
 ...
 *@supports_net_changes* 매개 변수가 1로 설정된 경우 캡처 인스턴스에 순 변경 함수도 생성됩니다. 이 함수는 호출에 지정된 간격 내에 변경된 각 개별 행에 대해 하나의 변경만 반환합니다. 자세한 내용은 [cdc.fn_cdc_get_net_changes_&#60;capture_instance&#62;&#40;Transact-SQL&#41;](/sql/relational-databases/system-functions/cdc-fn-cdc-get-net-changes-capture-instance-transact-sql)를 참조하세요.  
  
 순 변경 쿼리를 지원하기 위해 원본 테이블에는 행을 고유하게 식별하는 기본 키 또는 고유 인덱스가 있어야 합니다. 고유 인덱스가 사용되는 경우 *@index_name* 템플릿을 참조하세요. 기본 키 또는 고유 인덱스에 정의된 열은 캡처할 원본 열 목록에 포함되어야 합니다.  
...
## <a name="disable-change-data-capture-for-a-table"></a>테이블의 변경 데이터 캡처 해제  
 멤버는 `db_owner` 고정된 데이터베이스 역할 저장된 프로시저를 사용 하 여 개별 원본 테이블에 대 한 캡처 인스턴스를 제거할 수 `sys.sp_cdc_disable_table`합니다. 원본 테이블에 현재 변경 데이터 캡처가 설정되었는지 확인하려면 `is_tracked_by_cdc` 카탈로그 뷰의 `sys.tables` 열을 살펴봅니다. 해제를 수행한 후 데이터베이스에 변경 데이터 캡처 기능이 설정된 테이블이 없으면 변경 데이터 캡처 작업도 제거됩니다.  
