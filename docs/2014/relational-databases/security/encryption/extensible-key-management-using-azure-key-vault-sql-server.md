...
2.  **자격 증명 모음에서 비대칭 키를 생성 합니다.** Key vault의 비대칭 키를 보호 하는 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 암호화 키입니다. 비대칭 키의 퍼블릭 부분만 자격 증명 모음을 떠나고 프라이빗 부분은 자격 증명 모음에서 내보내지 않습니다. 비대칭 키를 사용하는 모든 암호화 작업은 Azure 키 자격 증명 모음에 위임되며, 키 자격 증명 모음 보안에 의해 보호됩니다.  
