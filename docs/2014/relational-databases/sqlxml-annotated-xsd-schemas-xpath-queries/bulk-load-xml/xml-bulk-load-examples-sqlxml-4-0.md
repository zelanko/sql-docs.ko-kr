...
```  
 ...
 스키마 지정는  **\<순서 >** 사용 하 여 요소를  **\<제품 >** 자식 요소입니다. 합니다  **\<순서 >** 요소는 Ord 테이블에 매핑합니다 및  **\<제품 >** 요소는 데이터베이스의 Product 테이블에 매핑됩니다. 에 지정 된 체인 관계는  **\<제품 >** 요소 OrderDetail 테이블이 나타내는 M:N 관계를 식별 합니다. 하나의 주문이 여러 제품을 포함할 수 있으며, 하나의 제품은 여러 주문에 포함될 수 있습니다.  
...
5.  VBScript 코드를 실행합니다.  
     ...
     해당 스키마를 지정 해야 합니다 `sql:datatype` 에 대 한 합니다 **CustomerID** 때 특성 값 **CustomerID** 으로 중괄호 ({및}) 등을 포함 하는 GUID로 지정 됩니다.  
    ...
    ```  
     ...
     때 `sql:datatype` 열 형식으로 식별 된 `uniqueidentifier`, 중괄호를 제거 하는 대량 로드 작업 ({및})에서 **CustomerID** 값 열에 삽입 하기 전에 합니다.  
...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>10. xml 데이터 형식 열에 대량 로드  
 매핑 스키마 지정 하는 경우는 [xml 데이터 형식](/sql/t-sql/xml/xml-transact-sql) 를 사용 하 여 열을 `sql:datatype="xml"` 주석, XML 대량 로드는 원본 문서에서를이 열에 매핑된 필드에 대 한 XML 자식 요소를 복사할 수 있습니다.  
  
 AdventureWorks 예제 데이터베이스에 있는 Production.ProductModel 테이블의 뷰를 매핑하는 다음 XSD 스키마를 고려해 보십시오. 이 테이블에서의 CatalogDescription 필드 `xml` 데이터 형식에 매핑되는  **\<Desc >** 요소를 사용 하 여는 `sql:field` 및 `sql:datatype="xml"` 주석.  
