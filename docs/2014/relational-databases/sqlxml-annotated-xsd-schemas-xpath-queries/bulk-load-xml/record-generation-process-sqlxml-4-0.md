...
> [!NOTE]  
>  여기에서는 주석이 추가된 XSD 또는 XDR 매핑 스키마를 잘 알고 있다고 가정합니다. 스키마에 대 한 자세한 내용은 참조 하세요. [주석이 추가 된 XSD 스키마 소개 &#40;SQLXML 4.0&#41; ](../../sqlxml/annotated-xsd-schemas/introduction-to-annotated-xsd-schemas-sqlxml-4-0.md) 하거나 [주석이 추가 된 XDR 스키마 &#40;SQLXML 4.0에서 더 이상 사용 되지&#41;](../../sqlxml/annotated-xsd-schemas/annotated-xdr-schemas-deprecated-in-sqlxml-4-0.md).  
...
## <a name="scope-of-a-node"></a>노드 범위  
 XML 문서의 노드 (요소 또는 특성)가 *범위로* XML 대량 로드가 XML 입력된 데이터 스트림의 것으로 발견 되는 경우. 요소 노드의 경우 요소의 시작 태그로 요소의 범위가 시작됩니다. 특성 노드의 경우 특성 이름으로 특성의 범위가 시작됩니다.  
...
```  
 ...
 스키마 지정는  **\<고객 >** 요소 **CustomerID** 하 고 **CompanyName** 특성입니다. `sql:relation` 주석 맵 합니다  **\<고객 >** Customers 테이블에는 요소입니다.  
...
-   XML 대량 로드 끝 태그에 도달 하는 경우는  **\<고객 >** 요소는 요소 범위를 벗어납니다. 그러면 XML 대량 로드는 레코드가 완료된 것으로 간주하고 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]로 해당 레코드를 보냅니다.  
  
 XML 대량 로드는 각각에 대해이 프로세스를 따릅니다 후속  **\<고객 >** 요소입니다.  
...
-   경우는  **\<고객 >** XML 데이터 파일의 요소 노드의 범위가 시작 될, XML 대량 로드가 Cust 테이블에 대 한 레코드를 생성 합니다. 그런 다음 XML 대량 로드에서 필요한 열 값 (CustomerID, CompanyName 및 City)을 복사 합니다  **\<CustomerID >** 합니다  **\<CompanyName >**, 및  **\<도시 >** 범위로 이러한 요소와 자식 요소를 입력 합니다.  
  
-   경우는  **\<순서 >** 요소 노드의 범위가 시작 될, XML 대량 로드는 CustOrder 테이블에 대 한 레코드를 생성 합니다. XML 대량 로드의 값을 복사 합니다 **OrderID** 이 레코드에 대 한 특성입니다. CustomerID 열에서 가져온에 필요한 값을  **\<CustomerID >** 자식 요소는  **\<고객 >** 요소입니다. 에 지정 된 정보를 사용 하는 XML 대량 로드 `<sql:relationship>` 경우가 아니면이 레코드에 대 한 CustomerID 외래 키 값을 가져오려고 합니다 **CustomerID** 특성에 지정 된를  **\<순서 >** 요소입니다. 일반 규칙은 자식 요소가 외래 키 특성 값을 명시적으로 지정하는 경우 XML 대량 로드가 해당 값을 사용하고 지정된 `<sql:relationship>`을 사용하여 부모 요소에서 값을 가져오지 않는 것입니다. 이  **\<순서 >** 요소 노드가 범위를 벗어나는 XML 대량 로드에 레코드를 보냅니다 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 한 다음 모든 후속 처리  **\<순서 >** 요소 노드 동일한 방식으로 합니다.  
  
-   마지막으로, 합니다  **\<고객 >** 요소 노드가 범위를 벗어납니다. 이 시점에서 XML 대량 로드가 고객 레코드를 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]로 보냅니다. XML 대량 로드는 XML 데이터 스트림의 모든 후속 고객에 대해 이 프로세스를 따릅니다.  
  
 여기서 매핑 스키마에 대한 두 가지 사항을 확인할 수 있습니다.  
  
-   스키마 "포함" 규칙을 충족 하는 경우 (연결 된 범위 내에 고객 및 주문과 연관 된 모든 데이터는 정의 하는 예를 들어  **\<고객 >** 고  **\<순서 >** 요소 노드)에서 대량 로드가 성공 합니다.  
  
-   설명 하는 합니다  **\<고객 >** 요소, 해당 자식 요소는 적절 한 순서로 지정 됩니다. 이 경우는  **\<CustomerID >** 하기 전에 지정 된 자식 요소가 합니다  **\<순서 >** 자식 요소입니다. 입력된 XML 데이터 파일, 즉를  **\<CustomerID >** 요소 값이 외래 키로 사용할 수 있는 경우이 값을  **\<순서 >** 요소의 범위가 시작 합니다. 키 특성이 먼저 지정되므로 이것은 "키 순서 지정 규칙"입니다.  
  
     지정 하는 경우는  **\<CustomerID >** 자식 요소 뒤의  **\<순서 >** 자식 요소에 값을 사용할 수 없는 경우 합니다  **\< 순서 >** 요소의 범위가 시작 됩니다. 경우는  **\</>** 끝 태그를 읽고, CustOrder 테이블에 대 한 레코드가 완료 된 것으로 간주 됩니다 및 원하는 결과가 아닌 경우는 CustomerID 열에 NULL 값이 있는 CustOrder 테이블에 삽입 됩니다.  
...
## <a name="exceptions-to-the-record-generation-rule"></a>레코드 생성 규칙의 예외  
 ...
 예를 들어 다음 XSD 스키마를 설명 하는 것이 좋습니다  **\<고객 >** 하 고  **\<순서 >** 요소입니다. **\<고객 >** 요소를 포함 한 **OrderList** IDREFS 유형의 특성입니다. `<sql:relationship>` 태그는 고객과 주문 목록 간의 일 대 다 관계를 지정합니다.  
...
```  
 ...
 대량 로드가 IDREFS 형식의 노드를 무시 하기 때문에 있는 레코드가 생성 되지 않습니다 경우는 **OrderList** 특성 노드의 범위가 시작 합니다. 따라서 Orders 테이블에 주문 레코드를 추가하려면 스키마의 특정 위치에서 해당 주문을 설명해야 합니다. 이 스키마에서 지정 하는  **\<순서 >** 요소를 사용 하면 XML 대량 로드는 Orders 테이블에 주문 레코드에 추가 하는 합니다. 합니다  **\<순서 >** 요소는 CustOrder 테이블에 레코드를 입력 하는 데 필요한 모든 특성을 설명 합니다.  
  
 되도록 해야 합니다는 **CustomerID** 하 고 **OrderID** 값을  **\<고객 >** 요소 값과 일치를  **\<순서 >** 요소입니다. 참조 무결성을 유지하는 책임은 사용자에게 있습니다.  
