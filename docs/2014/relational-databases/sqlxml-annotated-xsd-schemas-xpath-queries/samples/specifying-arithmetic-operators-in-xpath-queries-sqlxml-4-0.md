---
title: (SQLXML 4.0) XPath 쿼리에 산술 연산자 지정 | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
```  
 ...
 쿼리에서 `child` 는 축이고 및 `OrderDetail` 는 노드 테스트 (TRUE 이면 **OrderDetail** 되는  **\<요소 노드 >** 이므로  **\< 요소 >** 노드는 주 노드는 `child` 축). 모든 합니다  **\<OrderDetail >** 요소 노드, 조건자의 테스트가 적용 되 고 조건을 충족 하는 노드만 반환 됩니다.  
