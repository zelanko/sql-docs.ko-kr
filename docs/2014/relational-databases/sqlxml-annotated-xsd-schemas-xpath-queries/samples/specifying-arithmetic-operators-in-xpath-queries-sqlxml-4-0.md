...
```  
 ...
 쿼리에서는 `child` 축 이며 `OrderDetail` 노드 테스트 ( **orderdetail** 이 **\<element node>** **\<element>** 축에 대 한 주 노드인지 때문에 orderdetail이 인 경우 TRUE `child` )입니다. 모든 **\<OrderDetail>** 요소 노드에 대해 조건자의 테스트가 적용 되며 조건을 충족 하는 노드만 반환 됩니다.  
