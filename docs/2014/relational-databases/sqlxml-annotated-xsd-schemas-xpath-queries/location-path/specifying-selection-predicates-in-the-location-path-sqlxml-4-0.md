...
> [!NOTE]  
>  XPath의이 XPath 구현 제한 사항에 대 한 정보 및 해당 사항과 W3C 사양과 차이점에 대 한 참조 [XPath 쿼리를 사용 하 여 소개 &#40;SQLXML 4.0&#41;](../introduction-to-using-xpath-queries-sqlxml-4-0.md)합니다.  
...
```  
 ...
 이 XPath 쿼리에서 `child` 및 `attribute`는 축 이름이고, `Customer` 노드 테스트 (TRUE 이면 `Customer` 는  **\<요소 노드 >** 때문에,  **\<요소 >** 에 대 한 주 노드 유형는 `child` 축). `attribute::CustomerID="ALFKI"`는 조건자입니다. 조건자에서 `attribute` 는 축이고와 `CustomerID` 는 노드 테스트 (TRUE 이면 **CustomerID** 는 컨텍스트 노드의 특성 때문에  **\<특성 >** 보안 주체 노드 유형이 `attribute` 축).  
...
## <a name="see-also"></a>관련 항목  
 ...
 [클라이언트 쪽 XML 서식을 &#40;SQLXML 4.0&#41;](../../sqlxml/formatting/client-side-xml-formatting-sqlxml-4-0.md)  
