...
```  
 ...
 이 XPath 쿼리에서 `child` 및 `attribute`는 축 이름이고, `Customer`는 노드 테스트 ( `Customer` 는 요소가 축에 대 한 주 노드 유형 ** \<>** 이므로가 ** \<>요소 노드인 **경우 TRUE)입니다 `child` . `attribute::CustomerID="ALFKI"`는 조건자입니다. 조건자에서 `attribute` 는 축이 고 `CustomerID` 는 노드 테스트입니다. **CustomerID** 가 컨텍스트 노드의 특성인 경우 TRUE이 고, ** \< 특성>** 축의 주 노드 유형인 경우 TRUE입니다 `attribute` .  
...
```  
 ...
 이 XPath 식에서 `child`는 축 이름이고, `Customer`는 노드 테스트 ( `Customer` 가 요소 ** \<>** 노드인 경우 TRUE)입니다. ** \< 요소>** 는 축에 대 한 주 노드 형식 이기 때문입니다 `child` . `child::ContactName`는 조건자입니다. 조건자에서는 `child` 축이 고 `ContactName` 는 노드 테스트 ( `ContactName` 가 ** \< 요소>** 노드) 인 경우 TRUE입니다.  
...
```  
 ...
 이 예에서 `child`는 축이고, `Customer`노드 테스트 ( `Customer` 가 \< 요소> 노드) 인 경우 TRUE입니다. `not(child::ContactName)`는 조건자입니다. 조건자에서는 `child` 축이 고 `ContactName` 는 노드 테스트 ( `ContactName` 가 \< 요소> 노드) 인 경우 TRUE입니다.  
...
```  
 ...
 이 예제에서는 `child` 축과 `Customer` 노드 테스트 ( `Customer` 가 \< 요소> 노드) 인 경우 TRUE입니다. `attribute::CustomerID`는 조건자입니다. 조건자에서는 `attribute` 축이 고 `CustomerID` 는 조건자 ( `CustomerID` 가 ** \< 특성>** 노드) 인 경우 TRUE입니다.  
