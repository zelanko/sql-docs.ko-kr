...
```  
 ...
 이 XPath 쿼리에서 `child` 및 `attribute`는 축 이름이고, `Customer``Customer` 는 노드 테스트 (는 요소가 `child` 축에 대 한 주 노드 유형 ** \<>** 이므로가 ** \<>요소 노드인 **경우 TRUE)입니다. 
  `attribute::CustomerID="ALFKI"`는 조건자입니다. 조건자 `attribute` 에서는 축이 고는 노드 `CustomerID` 테스트입니다. **CustomerID** 가 컨텍스트 노드의 특성인 경우 TRUE이 고, ** \<특성>** `attribute` 축의 주 노드 유형인 경우 TRUE입니다.  
