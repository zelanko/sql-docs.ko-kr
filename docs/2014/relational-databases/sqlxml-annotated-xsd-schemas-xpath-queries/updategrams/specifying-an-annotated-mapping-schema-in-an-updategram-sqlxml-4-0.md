...
```  
 ...
 업데이트 작업 중에 부모-자식 관계를 updategram 논리에 지시 하려면 `inverse` ** \< relationship>** 요소에 주석을 지정 합니다. 그 결과, 레코드가 먼저 Cust 테이블에 추가된 다음 Ord 테이블에 추가되어 작업이 성공합니다.  
