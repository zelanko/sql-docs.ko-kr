...
## <a name="lowest-level-of-protection"></a>가장 낮은 수준의 보호  
 이 보호 수준에서는 데이터베이스를 마지막으로 읽은 후 적용된 다른 업데이트에 대한 참조 없이 업데이트가 처리됩니다. 에 기본 키 열만 지정 하면 이러한 경우는  **\<하기 전에 >** 블록의 레코드를 식별 하 고에 업데이트 된 정보를 지정 된  **\<후 >** 블록입니다.  
...
-   테이블에 추가 열을 지정 된  **\<하기 전에 >** 블록입니다.  
  
     추가 열을 지정 하는 경우는  **\<하기 전에 >** 블록에 updategram에 이러한 열이 업데이트를 적용 하기 전에 데이터베이스에 있는 값으로 지정 된 값을 비교 합니다. 트랜잭션에서 레코드를 읽은 후 레코드 열 중 하나라도 변경되었으면 updategram은 업데이트를 수행하지 않습니다.  
  
     예를 들어 다음 updategram 근무 시간 이름을 업데이트 하지만의 추가 열 (StartTime, EndTime)을 지정 된  **\<하기 전에 >** 더 높은 수준의 동시에 대 한 보호를 요청 하 여 블록 업데이트합니다.  
    ...
    ```  
     ...
     레코드에 대 한 모든 열 값을 지정 하 여 가장 높은 수준의 보호를 지정 하는이 예제는  **\<하기 전에 >** 블록입니다.  
  
-   타임 스탬프 열 (있는 경우)을 지정 된  **\<하기 전에 >** 블록입니다.  
  
     모든 레코드 열을 지정 하는 대신는 `<before`> 블록 지정 해도 타임 스탬프 열 (테이블에 있는 경우 하나)에 기본 키 열과 함께  **\<하기 전에 >** 블록입니다. 데이터베이스는 레코드가 업데이트될 때마다 타임스탬프 열을 고유한 값으로 업데이트합니다. 이 경우 updategram은 타임스탬프 값을 데이터베이스의 해당 값과 비교합니다. 타임스탬프 값은 데이터베이스에 이진 값으로 저장되므로 스키마에서 타임스탬프 열은 `dt:type="bin.hex"`, `dt:type="bin.base64"` 또는 `sql:datatype="timestamp"`로 지정해야 합니다. (하나를 지정할 수는 `xml` 데이터 형식 또는 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 데이터 형식입니다.)  
