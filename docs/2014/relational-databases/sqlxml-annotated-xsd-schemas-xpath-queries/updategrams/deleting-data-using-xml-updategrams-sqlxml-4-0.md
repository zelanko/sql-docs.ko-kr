...
```  
 ...
 생략할 수 있습니다 합니다  **\<후 >** updategram 삭제 작업만 수행 하는 경우 태그를 지정 합니다. 선택적 지정 하지 않은 경우 `mapping-schema` 특성을  **\<ElementName >** 데이터베이스 테이블에 updategram 맵과 자식 요소 또는 특성에 매핑할 테이블의 열에 지정 된 합니다.  
...
## <a name="examples"></a>예  
 이 섹션의 예에서는 기본 매핑을 사용합니다. 즉, Updategram에 매핑 스키마가 지정되어 있지 않습니다. 매핑 스키마를 사용 하는 updategram에 대 한 더 많은 예제를 참조 하세요 [Updategram에 주석이 추가 된 매핑 스키마 지정 &#40;SQLXML 4.0&#41;](specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-4-0.md)합니다.  
...
1.  전체 예제 (여러 레코드 삽입 updategram을 사용 하 여")에서 B [XML Updategram를 사용 하 여 데이터 삽입 &#40;SQLXML 4.0&#41;](inserting-data-using-xml-updategrams-sqlxml-4-0.md)합니다.  
  
2.  위의 updategram을 메모장에 복사 하 고에서 ("여러 레코드 삽입 updategram을 사용 하 여") 하는 데 사용 된 동일한 폴더에 Updategram-removeshifts.xml로 저장 [XML Updategram를 사용 하 여 데이터 삽입 &#40;SQLXML 4.0&#41; ](inserting-data-using-xml-updategrams-sqlxml-4-0.md).  
