...
# <a name="creating-a-memory-optimized-table-and-a-natively-compiled-stored-procedure"></a>메모리 액세스에 최적화된 테이블 및 고유하게 컴파일된 저장 프로시저 만들기
 ...
 이 예제에서는 애플리케이션에서 메모리 내 OLTP를 사용할 수 있도록 하는 방법을 보여 줍니다. 이 과정에서는 데이터베이스를 메모리 내 OLTP용으로 설정하고, DURABILITY 옵션을 보여 주는 메모리에 최적화된 테이블을 만든 다음, 고유하게 컴파일된 저장 프로시저를 만들어 메모리 최적화 테이블의 데이터에 액세스합니다. 아래의 [!INCLUDE[tsql](../../includes/tsql-md.md)] 에는 다음 단계가 자세히 설명되어 있습니다.  
