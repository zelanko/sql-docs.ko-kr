...
|기능|계산 열|계산된 열은 메모리 최적화 테이블에서 지원되지 않습니다. 계산된 열을 제거 합니다 `CREATE TABLE` 문입니다.|  
|기능|복제|복제는 메모리 최적화 테이블에서 지원되지 않습니다.|  
|기능|FILESTREAM|FILESTREAM 저장소는 메모리 최적화 테이블의 열에 지원되지 않습니다. 제거 된 `FILESTREAM` 열 정의에서 키워드입니다.|  
|기능|SPARSE|메모리 최적화 테이블의 열을 SPARSE로 정의할 수 없습니다. 제거 된 `SPARSE` 열 정의에서 키워드입니다.|  
|기능|ROWGUIDCOL|ROWGUIDCOL 옵션은 메모리 최적화 테이블의 열에서 지원되지 않습니다. 제거 된 `ROWGUIDCOL` 열 정의에서 키워드입니다.|  
...
|기능|CHECK|CHECK 제약 조건은 메모리 최적화 테이블에서 지원되지 않습니다. 테이블 정의에서 제약 조건을 제거합니다.<br /><br /> 제약 조건에 대 한 지원 부족을 완화 하는 방법에 대 한 자세한 내용은 [마이그레이션 확인 및 Foreign Key 제약 조건](../../database-engine/migrating-check-and-foreign-key-constraints.md)합니다.|  
|기능|UNIQUE|UNIQUE 제약 조건은 메모리 최적화 테이블에서 지원되지 않습니다. 테이블 정의에서 제약 조건을 제거합니다.<br /><br /> 제약 조건에 대 한 지원 부족을 완화 하는 방법에 대 한 자세한 내용은 [마이그레이션 확인 및 Foreign Key 제약 조건](../../database-engine/migrating-check-and-foreign-key-constraints.md)합니다.|  
...
|연산|TRUNCATE TABLE|TRUNCATE 작업은 메모리 최적화 테이블에서 지원되지 않습니다. 테이블에서 모든 행을 제거 하려면 사용 하 여 모든 행을 삭제 `DELETE FROM` *테이블* 하거나 삭제 하 고 테이블을 다시 만듭니다.|  
...
|기능|ANSI_PADDING OFF|세션 옵션 `ANSI_PADDING` 테이블이 나 고유 하 게 컴파일된 저장된 프로시저를 메모리 최적화를 만들 때 ON 이어야 합니다. CREATE 문을 실행하기 전에 `SET ANSI_PADDING ON`을 실행합니다.|  
...
|형식|기능|해결 방법|  
|----------|-------------|----------------|  
|기능|인라인 테이블 변수|변수 선언을 사용하여 테이블 형식을 인라인으로 선언할 수 없습니다. 테이블 형식을 사용 하 여 명시적으로 선언 되어야 합니다는 `CREATE TYPE` 문입니다.|  
|기능|커서|커서는 고유하게 컴파일된 저장 프로시저 위나 안에서 지원되지 않습니다.<br /><br /> -을 클라이언트에서 프로시저를 실행 하는 경우 커서 API 대신 RPC를 사용 합니다. ODBC가 있는 경우 [!INCLUDE[tsql](../../includes/tsql-md.md)] 문 `EXECUTE`를 사용하지 말고 그 대신 프로시저 이름을 직접 지정합니다.<br /><br /> -에서 프로시저를 실행 하는 경우는 [!INCLUDE[tsql](../../includes/tsql-md.md)] 일괄 처리 또는 다른 저장된 프로시저를 고유 하 게 컴파일된 저장된 프로시저를 사용 하 여 커서를 사용 하지 마십시오.<br /><br /> 집합 기반 논리를 사용 하 여 커서를 사용 하는 것이 아니라 고유 하 게 컴파일된 저장된 프로시저를 만들 때 또는 `WHILE` 루프입니다.|  
...
|기능|번호가 매겨진 저장 프로시저|고유하게 컴파일된 저장 프로시저에 번호를 매길 수 없습니다. 제거 합니다 `;` *번호* 에서 `CREATE PROCEDURE` 문입니다.|  
...
|기능|SELECT INTO|`INTO` 절은 `SELECT` 문에 지원되지 않습니다. 로 쿼리를 다시 작성 `INSERT INTO` *표*`SELECT`합니다.|  
|기능|OUTPUT|`OUTPUT` 절은 지원되지 않습니다. 쿼리에서 이 절을 제거합니다.|  
|기능|불완전한 삽입 열 목록|`INSERT` 문에서는 테이블에 잇는 모든 열에 대해 값을 지정해야 합니다.|  
|기능|*함수*|기본 제공 함수는 고유하게 컴파일된 저장 프로시저에서 지원되지 않습니다. 저장 프로시저에서 함수를 제거합니다. 지원 되는 기본 제공 함수에 대 한 자세한 내용은 참조 하세요. [Natively Compiled Stored Procedures](../in-memory-oltp/natively-compiled-stored-procedures.md)합니다.|  
|기능|CASE|`CASE` 문은 고유하게 컴파일된 저장 프로시저 내부의 쿼리에서 지원되지 않습니다. 각 사례에 대해 쿼리를 만듭니다. 자세한 내용은 [는 CASE 문 구현](implementing-a-case-expression-in-a-natively-compiled-stored-procedure.md)합니다.|  
...
|연산자|CONTAINS|이 연산자는 지원되지 않습니다. 제거 `CONTAINS` 고유 하 게 컴파일된 저장 프로시저에서 합니다.|  
