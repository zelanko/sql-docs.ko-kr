...
-   데이터베이스 엔진 튜닝 관리자는 다음 상황에서 권장을 수행할 수 없습니다.  
    ...
    3.  데이터베이스 엔진 튜닝 관리자를 실행 하는 사용자의 구성원이 아닙니다.는 `db_owner` 데이터베이스 역할 또는 `sysadmin` 고정된 서버 역할입니다. 작업의 쿼리는 데이터베이스 엔진 튜닝 관리자를 실행하는 사용자의 보안 컨텍스트에서 분석됩니다. 사용자는 `db_owner` 데이터베이스 역할의 멤버여야 합니다.  
