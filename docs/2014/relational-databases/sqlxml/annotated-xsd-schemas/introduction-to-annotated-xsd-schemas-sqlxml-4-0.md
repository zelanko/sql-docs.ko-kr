...
# <a name="introduction-to-annotated-xsd-schemas-sqlxml-40"></a>주석이 추가된 XSD 스키마 소개(SQLXML 4.0)
 ...
 XSD 스키마에서는  **\<xsd: schema >** 전체 스키마를 포함 하는 요소, 모든 요소 선언에 포함 되어야 합니다는  **\<xsd: schema >** 요소입니다. 네임 스페이스를 정의 하는 특성을 설명할 수 있는 스키마는 서버와의 속성으로 스키마에 사용 되는 네임 스페이스는  **\<xsd: schema >** 요소입니다.  
  
 유효한 XSD 스키마를 포함 해야 합니다는  **\<xsd: schema >** 요소가 다음과 같이 정의 합니다.  
...
>  [!INCLUDE[msCoName](../../../includes/msconame-md.md)] SQLXML 4.0의 XSD 스키마 언어는 [!INCLUDE[ssVersion2000](../../../includes/ssversion2000-md.md)]에서 주석이 추가된 XDR(XML-Data Reduced) 스키마 언어에 도입된 주석을 지원합니다. 주석이 추가된 XDR은 SQLXML 4.0에서 더 이상 사용되지 않습니다.  
 ...
 관계형 데이터베이스 컨텍스트에서는 임의의 XSD 스키마를 관계형 저장소에 매핑하는 것이 유용합니다. 이를 수행하는 한 가지 방법은 XSD 스키마에 주석을 추가하는 것입니다. 주석이 추가 된 XSD 스키마 라고는 *매핑 스키마*, XML 데이터가 관계형 저장소에 매핑되는 방법에 대 한 정보를 제공 하는 합니다. 매핑 스키마는 궁극적으로 관계형 데이터에 대한 XML 뷰로 생각할 수 있습니다. 이러한 매핑을 사용하여 관계형 데이터를 XML 문서로 검색할 수 있습니다.  
  
## <a name="namespace-for-annotations"></a>주석에 대한 네임스페이스  
 네임 스페이스를 사용 하 여 지정 된 주석은 XSD 스키마에서 **:-microsoft-com:mapping-스키마**합니다. 네임 스페이스를 지정 하는 가장 쉬운 방법은 지정 하는 것은 다음 예에서 같이  **\<xsd: schema >** 태그입니다.  
...
```  
 ...
 매핑 스키마에는  **\<연락처 >** 요소는 사용 하 여 샘플 AdventureWorks 데이터베이스의 Person.Contact 테이블에 매핑됩니다는 `sql:relation` 주석입니다. ConID, FName 및 LName 특성은 `sql:field` 주석을 사용하여 Person.Contact 테이블의 ContactID, FirstName 및 LastName 열에 매핑됩니다.  
...
## <a name="see-also"></a>관련 항목  
 ...
 [주석이 추가 된 XDR 스키마 &#40;SQLXML 4.0에서에서 사용 되지 않습니다&#41;](annotated-xdr-schemas-deprecated-in-sqlxml-4-0.md)  
