---
title: MSSQLSERVER_7914 | Microsoft 문서
ms.custom: ''
ms.date: 03/06/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.technology: supportability
ms.topic: conceptual
helpviewer_keywords:
- 7914 (Database Engine error)
ms.assetid: d32a81ce-4ca7-4b33-b536-c7ea0ed6f226
author: MashaMSFT
ms.author: mathoma
manager: craigg
ms.openlocfilehash: 098fc44f4a46a821c3e0f6d17d150d34f3d0d967
ms.sourcegitcommit: 3026c22b7fba19059a769ea5f367c4f51efaf286
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 06/15/2019
ms.locfileid: "62913145"
---
# <a name="mssqlserver7914"></a>MSSQLSERVER_7914
    
## <a name="details"></a>설명  
  
|||  
|-|-|  
|제품 이름|SQL Server|  
|이벤트 ID|7914|  
|이벤트 원본|MSSQLSERVER|  
|구성 요소|SQLEngine|  
|심볼 이름|DBCC2_REPAIR_ALLOCATION_PAGE_REBUILT|  
|메시지 텍스트|복구: P_ID에서 PAGE_TYPE 페이지가 다시 작성 되었습니다.|  
  
## <a name="explanation"></a>설명  
 GAM 또는 SGAM 페이지가 PFS 페이지 데이터를 사용하여 다시 작성되었음을 나타내는 REPAIR의 정보 페이지입니다.  
  
## <a name="user-action"></a>사용자 동작  
 없음  
  
  
