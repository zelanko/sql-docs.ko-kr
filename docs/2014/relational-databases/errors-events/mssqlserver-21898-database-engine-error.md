...
## <a name="explanation"></a>설명  
 `sp_validate_redirected_publisher` 새 게시자에서 사용 하는 배포 데이터베이스에서 원래 게시자에서 사용 하는 배포 데이터베이스와 동일 하 게 되었는지 확인 하는 로컬 배포자에서 msdb.dbo.MSdistpublishers를 쿼리 합니다. 이 오류는 이 두 데이터베이스가 서로 달라 게시자가 게시자 데이터베이스에 대한 적합한 호스트가 될 수 없는 경우에 반환됩니다.  
