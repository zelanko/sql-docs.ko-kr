...
```  
 ...
 SET STATISTICS IO의 출력은 결과 집합의 끝에 도달할 때까지 표시되지 않습니다. STATISTICS IO 출력을 호출 하 여 응용 프로그램을 가져올 **SQLGetDiagRec** 당시 **SQLFetch** 또는 [SQLFetchScroll](../native-client-odbc-api/sqlfetchscroll.md) SQL_NO_DATA를 반환 합니다. STATISTICS IO 출력은 다음과 같은 형식으로 반환됩니다.  
...
```  
 ...
 PRINT 또는 RAISERROR 문 (예: SELECT 문), SQL 문 다음에 오는 경우 PRINT 또는 RAISERROR 정보 때 반환 [SQLMoreResults](../native-client-odbc-api/sqlmoreresults.md)결과에 위치 오류를 포함 하는 설정입니다. **SQLMoreResults** 메시지의 심각도 따라 SQL_SUCCESS_WITH_INFO 또는 SQL_ERROR를 반환 합니다. 메시지를 호출 하 여 검색 **SQLGetDiagRec** sql_no_data가 반환 될 때까지 합니다.  
