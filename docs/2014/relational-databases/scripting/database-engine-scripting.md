...
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 명령을 구현하는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] cmdlet 집합. **Invoke-Sqlcmd**는 이러한 cmdlet 중 하나로, 이 실행 하는 데 사용 됩니다 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 쿼리를 실행 하는 스크립트는 `sqlcmd` 유틸리티입니다.  
...
-   `sqlcmd` 명령 프롬프트 유틸리티를 사용 하면 대화형으로 실행할 수 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 쿼리 및 실행 기존 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 쿼리 스크립트 파일도 있습니다.  
...
-   사용 하는 **-i * * * input_file* 매개 변수를 사용 하 여 파일을 실행은 `sqlcmd` 유틸리티입니다.  
...
|사용 하는 방법에 설명 된 `sqlcmd` 유틸리티가 실행 되도록 [!INCLUDE[tsql](../../includes/tsql-md.md)] 스크립트를 대화식으로 개발 하는 기능을 포함 하 여 명령 프롬프트에서 스크립트입니다.|[sqlcmd 방법 도움말 항목](../../database-engine/sqlcmd-how-to-topics.md)|  
