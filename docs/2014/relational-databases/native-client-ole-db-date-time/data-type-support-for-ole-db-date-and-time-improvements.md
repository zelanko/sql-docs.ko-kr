...
```  
 ...
 `sql_variant`를 사용하고 `datetime`의 제한된 정밀도에 의존하는 응용 프로그램을 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client로 마이그레이션하는 경우 `datetime2` 대신 `datetime`를 사용하도록 기본 스키마가 업데이트되면 해당 응용 프로그램도 이에 맞게 업데이트되어야 합니다.  
