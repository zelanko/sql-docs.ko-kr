...
-   **대상 복구 시간을 변경 하려면:**[SQL Server Management Studio](#SSMSProcedure) 또는 [transact-sql](#TsqlProcedure) 을 사용 합니다.    
...
3.  
  **데이터베이스 속성** 대화 상자에서 **옵션** 페이지를 클릭합니다.  
  
4.  
  **복구** 패널의 **대상 복구 시간(초)** 필드에서 이 데이터베이스에 대한 복구 시간의 상한으로 사용할 시간(초)을 지정합니다.  
...
2.  다음 [ALTER DATABASE](/sql/t-sql/statements/alter-database-transact-sql-set-options)문을 다음과 같이 사용합니다.  
     ...
     TARGET_RECOVERY_TIME **=** _target_recovery_time_ {SECONDS | 내  
