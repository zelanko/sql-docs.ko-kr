...
# <a name="sqlvariant-support-for-date-and-time-types"></a>sql_variant 날짜 및 시간 형식에 대 한 지원
 ...
 결과 열의 경우 드라이버에서는 variant를 날짜/시간 형식으로 변환합니다. 자세한 내용은 참조 [SQL에서 C로 변환을](datetime-data-type-conversions-from-sql-to-c.md)합니다. SQL_C_BINARY에 바인딩할 때 버퍼 길이가 SQL 형식에 해당하는 구조체를 받을 수 있을 만큼 커야 합니다.  
...
|SQL_C_BINARY|Time|SQL_CA_SS_VARIANT_SQL_TYPE = SQL_SS_TIME2<br /><br /> 배율 SQL_DESC_PRECISION로 설정 됩니다 (의 *DecimalDigits* 의 매개 변수 `SQLBindParameter`).|  
|SQL_C_BINARY|datetimeoffset|SQL_CA_SS_VARIANT_SQL_TYPE= SQL_SS_TIMESTAMPOFFSET<br /><br /> 배율 SQL_DESC_PRECISION로 설정 됩니다 (의 *DecimalDigits* 의 매개 변수 `SQLBindParameter`).|  
|SQL_C_TYPE_DATE|날짜|SQL_CA_SS_VARIANT_SQL_TYPE이 무시됩니다.|  
|SQL_C_TYPE_TIME|time(0)|SQL_CA_SS_VARIANT_SQL_TYPE이 무시됩니다.|  
|SQL_C_TYPE_TIMESTAMP|Datetime2|배율 SQL_DESC_PRECISION로 설정 됩니다 (의 *DecimalDigits* 의 매개 변수 `SQLBindParameter`).|  
|SQL_C_NUMERIC|Decimal|전체 자릿수가 SQL_DESC_PRECISION로 설정 됩니다 (의 *ColumnSize* 의 매개 변수 `SQLBindParameter`).<br /><br /> 소수 자릿수가 SQL_DESC_SCALE(SQLBindParameter의 *DecimalDigits* 매개 변수)로 설정됩니다.|  
