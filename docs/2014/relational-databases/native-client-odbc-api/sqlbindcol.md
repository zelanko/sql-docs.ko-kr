...
# <a name="sqlbindcol"></a>SQLBindCol
 ...
 큰 값 데이터 형식의 경우 사용자가 제공한 버퍼 열의 전체 값을 보유할 충분히 크지 않은 경우 `SQL_SUCCESS_WITH_INFO` 반환 되 고 "문자열 데이터 오른쪽 잘림"경고가 발생 합니다. `StrLen_or_IndPtr` 인수에는 버퍼에 저장된 문자/바이트의 수가 포함됩니다.  
