...
-   참조 된 매개 변수 수를 *식* UPDATE 문의 SET 절에서.  
 ...
 블록을 실행 하는 서버에서 데이터를 제공 하는 SQLPutData 호출 시퀀스로 취소 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 6.5 또는 이전 버전을 사용 하는 경우 열 값의 부분 업데이트를 사용 하면 됩니다. 합니다 `text`, `ntext`, 또는 `image` SQLCancel가 호출 될 때 참조 된 열은 중간 자리 표시자 값으로 설정 합니다.  
...
## <a name="sqlputdata-support-for-enhanced-date-and-time-features"></a>향상된 날짜 및 시간 기능에 대한 SQLPutData 지원  
 에 설명 된 대로 날짜/시간 형식의 매개 변수 값을 변환할지 [C에서 SQL로 변환을](../native-client-odbc-date-time/datetime-data-type-conversions-from-c-to-sql.md)입니다.  
