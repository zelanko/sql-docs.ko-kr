...
> [!NOTE]  
>  [ibcpsession:: Bcpcolumns](ibcpsession-bcpcolumns-ole-db.md) 메서드를 호출 하기 전에 호출 해야 **BCPColFmt**합니다. 사용자 파일에 있는 각 열에 대해 **BCPColFmt** 를 한 번씩 호출해야 합니다. 사용자 파일의 열에 대해 **BCPColFmt** 를 두 번 이상 호출하면 오류가 발생합니다.  
 ...
 **참고** 는 [ibcpsession:: Bcpwritefmt](ibcpsession-bcpwritefmt-ole-db.md) 함수를 통해 제공 된 형식 지정에 사용할 수 있습니다 **BCPColFmt**합니다.  
 ...
 *cbUserData*[in]  
 ...
 설정 `cbUserData` 를 bcp_length_null로 모든 값의 데이터 파일 필드를 나타내는 이거나 NULL로 설정 해야 합니다. 설정 `cbUserData` 를 bcp_length_variable로 시스템이 각 필드에 대 한 데이터의 길이 확인 함을 나타냅니다. 일부 필드의 경우 이 설정은 길이 또는 Null 표시기가 생성되어 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에서 복사되는 복사본의 데이터 앞에 추가됨을 의미하거나 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]로 복사되는 데이터에 표시기가 필요함을 나타냅니다.  
  
 에 대 한 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 문자 및 이진 데이터 형식의 경우 `cbUserData` BCP_LENGTH_VARIABLE, BCP_LENGTH_NULL, 0 또는 양수 값일 수 있습니다. 경우 `cbUserData` 가 BCP_LENGTH_VARIABLE 이면 시스템 있는 경우, 길이 표시기 나 종결자 시퀀스를 사용 하 여 데이터의 길이 확인 하려면. 길이 표시기와 종결자 시퀀스를 모두 지정하는 경우 대량 복사에는 복사되는 데이터 크기가 가장 작은 방법이 사용됩니다. 경우 `cbUserData` 가 BCP_LENGTH_VARIABLE 이면 데이터 형식이 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 문자 또는 이진 형식이 며 길이 표시기 나 종결자 시퀀스를 지정 하는 경우 시스템 오류 메시지를 반환 합니다.  
  
 경우 `cbUserData` 가 0 이거나 양수 값 이면 시스템은 `cbUserData` 최대 데이터 길이로 합니다. 그러나는 양수 `cbUserData`, 길이 표시자 또는 종결자 시퀀스도 제공, 시스템 복사 되는 데이터 크기가 가장 발생 하는 메서드를 사용 하 여 데이터 길이 확인 합니다.  
  
 `cbUserData` 값은 데이터의 바이트 수를 나타냅니다. 문자 데이터가 유니코드 와이드 문자로 다음 양수 표현 되는 경우 `cbUserData` 매개 변수 값의 각 문자를 바이트 단위로 크기와 곱하면 문자의 수를 나타냅니다.  
...
## <a name="return-code-values"></a>반환 코드 값  
 ...
 E_FAIL  
 자세한 내용은 사용에 대 한 공급자 관련 오류가 발생 한는 [ISQLServerErrorInfo](../../database-engine/dev-guide/isqlservererrorinfo-ole-db.md) 인터페이스입니다.  
