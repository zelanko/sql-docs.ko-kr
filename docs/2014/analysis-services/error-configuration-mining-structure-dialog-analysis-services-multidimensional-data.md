...
-   **알 수 없음 상태로 변환** 은 레코드 정보를 알 수 없는 멤버로 집계 합니다.  
  
-   **레코드 삭제** 는 레코드에 대 한 정보가 개체와 함께 처리 되지 않도록 제외 합니다.  
...
-   **처리 중지** 는 처리 작업을 종료 합니다.  
  
-   **로깅 중지** 는 오류 기록을 중지 하지만 처리 작업은 계속 합니다.  
...
-   **보고 하 고 중지** 는 오류를 보고 하 고 처리 작업을 중지 합니다.  
