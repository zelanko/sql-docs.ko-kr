...
|**타임 스탬프 만들기**|테이블을 만든 날짜와 시간을 표시합니다.|  
|**최종 스키마 업데이트**|테이블에 대한 메타데이터를 마지막으로 업데이트한 날짜와 시간을 표시합니다.|  
|**시스템 상태**|테이블의 처리 상태를 표시합니다. 이 속성 값에 대한 자세한 내용은 <xref:Microsoft.AnalysisServices.ProcessableMajorObject.State%2A>를 참조하십시오.|  
