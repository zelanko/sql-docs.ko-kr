...
5.  에 **서버 이름** 텍스트 상자에서 `localhost`합니다.  
  
     로컬 컴퓨터에 명명된 된 인스턴스를 연결 하려면 입력 **localhost\\< 인스턴스 이름\>** 합니다. 로컬 컴퓨터 대신 특정 컴퓨터에 연결하려면 컴퓨터 이름 또는 IP 주소를 입력합니다.  
