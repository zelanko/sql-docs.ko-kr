 ...
 **참고** **부모 계층이** 측정값이 아닌 계층을 지정 하는 경우이 옵션을 사용할 수 있습니다.  
  
 **변경**  
 
  **부모 멤버 선택** 대화 상자를 표시하여 **부모 멤버**에 대한 멤버를 선택하려면 이 옵션을 선택합니다. 
  **부모 멤버 선택** 대화 상자에 대한 자세한 내용은 [부모 멤버 선택 대화 상자&#40;Analysis Services - 다차원 데이터&#41;](select-parent-member-dialog-box-analysis-services-multidimensional-data.md)를 참조하세요.  
 ...
 **비어 있지 않은 동작**  
 계산 멤버에 대해 MDX로 NON EMPTY 쿼리를 해결하는 데 사용하는 측정값의 이름을 선택합니다. 
  **비어 있지 않은 동작** 속성이 비어 있는 경우 계산 멤버를 반복적으로 계산하여 멤버가 비어 있는지 확인해야 합니다. 
  **비어 있지 않은 동작** 속성에 측정값의 이름이 있는 경우 지정한 측정값이 비어 있으면 계산 멤버도 비어 있는 것으로 처리됩니다.  
