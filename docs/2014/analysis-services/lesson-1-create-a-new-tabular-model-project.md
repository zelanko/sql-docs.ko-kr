...
# <a name="lesson-1-create-a-new-tabular-model-project"></a>1단원: 새 테이블 형식 모델 프로젝트를 만들기
 ...
 다양한 유형의 테이블 형식 모델 프로젝트에 대한 자세한 내용은 [테이블 형식 모델 프로젝트&#40;SSAS 테이블 형식&#41;](tabular-models/tabular-model-projects-ssas-tabular.md)를 참조하세요. 테이블 형식 모델 제작 환경에 대 한 자세한 참조 [테이블 형식 모델 디자이너 &#40;SSAS 테이블 형식&#41;](tabular-model-designer-ssas-tabular.md)합니다.  
...
2.  에 **새 프로젝트** 대화 상자의 **설치 된 템플릿**, 클릭 **Business Intelligence**, 클릭 **Analysis Services**, 및 클릭 **Analysis Services 테이블 형식 프로젝트**합니다.  
  
3.  **이름**, 형식 `AW Internet Sales Tabular Model`, 프로젝트 파일에 대 한 위치를 지정 합니다.  
...
## <a name="understanding-the-sql-server-data-tools-tabular-model-authoring-environment"></a>SQL Server Data Tools 테이블 형식 모델 제작 환경 이해  
 ...
 프로젝트를 만들면 [!INCLUDE[ssBIDevStudio](../includes/ssbidevstudio-md.md)]에서 열립니다. 모델 디자이너에 빈 모델이 표시되고 **솔루션 탐색기** 창에서 **Model.bim** 파일이 선택됩니다. 데이터를 추가하면 테이블과 열이 디자이너에 표시됩니다. (Model.bim 탭 된 빈 창) 디자이너에 표시 되지 않으면 **솔루션 탐색기**아래 `AW Internet Sales Tabular Model`, 두 번 클릭은 **Model.bim** 파일입니다.  
 ...
 **솔루션 탐색기**를 마우스 오른쪽 단추로 클릭는 `AW Internet Sales Tabular Model` 프로젝트를 마우스 클릭 **속성**합니다. **AW Internet Sales Tabular Model 속성 페이지** 대화 상자가 나타납니다. 이러한 속성은 고급 프로젝트 속성입니다. 나중에 모델을 배포할 준비가 되었을 때 이러한 속성 중 일부를 설정합니다.  
