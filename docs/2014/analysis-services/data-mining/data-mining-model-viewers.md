...
1.  먼저 Microsoft Time Series 알고리즘을 기반으로 하는 마이닝 모델을 선택할 때 표시되는 사용자 지정 뷰어를 보여 줍니다.  
