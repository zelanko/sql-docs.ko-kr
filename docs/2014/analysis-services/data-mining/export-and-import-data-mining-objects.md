...
# <a name="export-and-import-data-mining-objects"></a>데이터 마이닝 개체 내보내기 및 가져오기
 ...
 다차원 데이터베이스 대신 관계형 데이터가 사용 하는 데이터 마이닝 솔루션을 하는 경우 사용 하 여 모델을 전송 `EXPORT` 및 `IMPORT` 훨씬 쉽고 빠르게 보다 데이터베이스 복원을 사용 하거나 전체 솔루션을 배포 합니다.  
...
## <a name="exporting-data-mining-structures"></a>데이터 마이닝 구조 내보내기  
 ...
 마이닝 구조를 내보낼 때 기본 동작에 따라 마이닝 구조가 처리되어 결과가 캐시된 경우, 구조의 기반이 되는 데이터에 대한 요약이 정의에 포함됩니다. 이러한 요약을 제거 하려면 수행 하 여 마이닝 구조와 연결 된 캐시를 선택을 취소 해야는 `Process Clear Structure` 작업 합니다. 자세한 내용은 [Process a Mining Structure](process-a-mining-structure.md)를 참조하세요.  
  
### <a name="exporting-data-mining-models"></a>데이터 마이닝 모델 내보내기  
 사용할 수는 `WITH DEPENDENCIES` 키워드를 데이터 원본 및 마이닝 모델 및 구조와 함께 데이터 원본 뷰 정의 내보냅니다.  
