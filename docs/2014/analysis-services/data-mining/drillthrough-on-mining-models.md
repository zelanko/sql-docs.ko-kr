...
|Microsoft 선형 회귀 알고리즘|지원됩니다.<br /><br /> 그러나 모델은 단일 노드인 만들기 때문에 `All`, 드릴스루는 모델에 대 한 모든 학습 사례가 반환 합니다. 학습 집합이 큰 경우 결과를 로드하는 데 시간이 많이 소요될 수 있습니다.|  
