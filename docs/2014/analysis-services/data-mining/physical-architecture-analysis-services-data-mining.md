...
## <a name="server-configuration"></a>서버 구성
 ...
 테이블 형식 모델과 데이터 마이닝 및/또는 다차원 모델을 사용하려면 별도의 인스턴스를 설치해야 합니다. [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]는 테이블 형식 모드에서 실행 되는 인스턴스 (VertiPaq (메모리 내 분석 엔진) 저장소 엔진) 및 기본 OLAP, MOLAP 또는 ROLAP 구성 중 하나에서 실행 되는 인스턴스를 함께 설치할 수 있도록 지원 합니다. 자세한 내용은 [Analysis Services 인스턴스의 서버 모드 확인](../instances/determine-the-server-mode-of-an-analysis-services-instance.md)을 참조하세요.
