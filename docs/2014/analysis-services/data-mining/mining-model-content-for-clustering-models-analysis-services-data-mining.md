...
## <a name="understanding-the-structure-of-a-clustering-model"></a>클러스터링 모델 구조에 대한 이해  
 ...
 ![클러스터링에 대한 모델 콘텐츠 구조](../media/modelcontentstructure-clust.gif "클러스터링에 대한 모델 콘텐츠 구조")  
...
|ATTRIBUTE_NAME|ATRIBUTE_VALUE|지원|PROBABILITY|분산|VALUE_TYPE|  
|---------------------|---------------------|-------------|-----------------|--------------|-----------------|  
|Age|Missing|0|0|0|1(누락)|  
|Age|44.9016152716593|12939|1|125.663453102554|3(연속)|  
|성별|Missing|0|0|0|1(누락)|  
|성별|F|6350|0.490764355823479|0|4 (Discrete)|  
|성별|M|6589|0.509235644176521|0|4 (Discrete)|  
...
>  분산은 클러스터에 대한 총 분산을 나타냅니다. 분산 값이 작은 경우 이는 대부분의 열 값이 평균에 매우 근접해 있음을 의미합니다. 표준 편차를 구하려면 분산의 제곱근을 계산하십시오.  
 ...
 각 특성마다 해당 특성에 대한 데이터가 없는 사례 수를 보여 주는 `Missing` 값 유형이 있습니다. 누락 데이터는 중요하며 데이터 형식에 따라 여러 가지 방식으로 계산에 영향을 미칩니다. 자세한 내용은 [누락 값&#40;Analysis Services - 데이터 마이닝&#41;](missing-values-analysis-services-data-mining.md)에서 미리 정의한 모델링 플래그 외에 다른 모델링 플래그를 사용할 수 있습니다.  
 ...
 **클러스터 노드** 클러스터를 다른 클러스터와 구별 하는 쉼표로 구분 된 기본 특성 목록입니다.  
 ...
 **클러스터 노드** 확률은 특성의 복합 확률을 나타내며 클러스터링 모델을 만드는 데 사용 된 알고리즘에 따라 몇 가지 조정이 있습니다.  
 ...
 **부모 노드** 이 항목에 대 한 소개를 참조 하세요.  
  
 **클러스터 노드** 이 클러스터에 포함 된 사례에 대 한 특성 및 값의 분포를 나타냅니다.  
  
 NODE_SUPPORT  
 이 노드를 지지하는 사례 수입니다. **부모 노드** 전체 모델의 학습 사례 수를 나타냅니다.  
  
 **클러스터 노드** 클러스터의 크기를 사례 수로 나타냅니다.  
  
 **참고** 모델에서 K 의미 클러스터링을 사용 하는 경우 각 사례는 하나의 클러스터에만 속할 수 있습니다. 그러나 모델이 EM 클러스터링을 사용하는 경우 각각의 사례가 서로 다른 클러스터에 속할 수 있으며 사례가 속해 있는 각 클러스터에 대해 가중치가 적용된 거리가 사례에 할당됩니다. 따라서 EM 모델의 경우 개별 클러스터에 대한 지지도 합계가 전체 모델에 대한 지지도보다 큽니다.  
 ...
 **부모 노드** 클러스터링 모델에 대 한 BIC (Bayesian Information 조건문) 점수입니다.  
