...
## <a name="build-a-dependency-network-graph"></a>종속성 네트워크 그래프 작성  
 시작 하려면 Visio 페이지에 셰이프를 삭제 합니다 **종속성 네트워크 Visio 셰이프 마법사** 고 다이어그램 옵션을 설정 합니다.  
...
2.  끌어서 합니다 **종속성 네트워크** 셰이프 마법사를 시작 페이지로 끌어다 놓습니다. **다음**을 클릭합니다.  
  
3.  시작 페이지에는 **종속성 네트워크 Visio 셰이프 마법사**, 클릭 **다음**합니다.  
  
4.  에 **데이터 원본 선택** 페이지를 **종속성 네트워크 Visio 셰이프 마법사**에 대 한 연결을 선택는 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 시각화 하려는 모델이 있는 서버.  
  
5.  적절 한 마이닝 모델을 선택 하 고 클릭 **다음**합니다.  
  
     적합 한 모델을 선택 하려면 이름, 설명, 열 및 데이터 형식을 검토할 수 있습니다 합니다 **속성** 창입니다.  
...
6.  페이지의 **렌더링할 노드 선택**그래프의 크기를 사용자 지정 하 고 특정 노드만 포함 하도록 선택적으로 필터링 합니다.  
  
     큰 데이터 집합을 사용 하 여 종속성 그래프를 될 하 라고 하는 여러 관련된 개체를 포함 *노드*합니다. 이 대화 상자를 사용하여 관심 있는 노드만 포함된 사용자 지정 네트워크 그래프를 만들 수 있습니다.  
...
8.  클릭 **닫기** 종료 합니다 **고급** 옵션 대화 상자 및 클릭 **마침** 그래프를 만듭니다.  
...
1.  사용 하 여는 **이동 및 확대/축소** 컨트롤을 **작업창** Visio의 영역 **보기** 리본의 그래프의 섹션에 집중 하 고 다이어그램을 이동 합니다.  
  
2.  Visio에서 제공 하는 다른 그래프 레이아웃을 사용 하 여 실험 **페이지 레이아웃 재지정** 옵션입니다.  
  
3.  클릭 합니다 **추가 기능** 리본을 한 후 데이터 마이닝 다이어그램을 사용 하 여 작업에 사용 되는 사용자 지정 도구 모음 중 하나를 표시 합니다.  
     ...
     **슬라이더**  
     합니다 **슬라이더** 통해 종속성 네트워크 다이어그램에 표시 되는 관계의 강도 제어할 수 있습니다.  
