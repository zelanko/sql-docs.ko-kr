...
# <a name="analysis-services-ole-db-provider-analysis-services---multidimensional-data"></a>Analysis Services OLE DB 공급자(Analysis Services - 다차원 데이터)
 ...
 독립 실행형 공급자인 Analysis Services OLE DB 공급자는 관계형/다차원 원본에서 로컬 큐브 파일과 마이닝 모델을 만들 수 있는 클라이언트 응용 프로그램을 제공합니다. 클라이언트 응용 프로그램은 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 인스턴스를 실행하는 대규모 실제 서버와 상호 작용 없이도 MDX(Multidimensional Expressions)를 사용하여 로컬 큐브에 연결하고 쿼리를 실행할 수 있습니다.  
