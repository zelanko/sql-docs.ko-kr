 ...
 **EqualAreas**  
 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]에서는 차원 멤버의 전체 채우기가 그룹 전체에 동일 하 게 분산 되도록 그룹 범위를 만듭니다.  
  
 **클러스터**  
 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]는 가우스 분포를 사용 하 여 K-수단 클러스터링 메서드를 사용 하 여 입력 값에 단일 차원 클러스터링을 수행 하는 방식으로 그룹을 만듭니다. 이 옵션은 숫자 열에만 유효합니다.  
