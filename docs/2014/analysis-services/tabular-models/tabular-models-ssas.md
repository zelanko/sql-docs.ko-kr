...
# <a name="tabular-modeling-ssas-tabular"></a>테이블 형식 모델링(SSAS 테이블 형식)
 ...
 테이블 형식 모델 두 가지 모드를 통해 데이터 액세스를 지원합니다. 캐시 된 모드와 DirectQuery 모드입니다. 캐시된 모드에서는 관계형 데이터베이스, 데이터 피드 및 일반 텍스트 파일을 포함하여 여러 원본의 데이터를 통합할 수 있습니다. DirectQuery 모드에서는 인 메모리 모델을 무시하고 클라이언트 애플리케이션에서 (SQL Server 관계형) 원본에 데이터를 직접 쿼리할 수 있도록 허용합니다.  
