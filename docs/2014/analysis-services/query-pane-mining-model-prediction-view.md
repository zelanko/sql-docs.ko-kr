 ...
 **자세한 내용은** 데이터 [마이닝 확장 &#40;Dmx&#41; 데이터 조작 문](/sql/dmx/dmx-statements-data-manipulation)을 [SQL Server Management Studio에서 dmx 쿼리를 만듭니다](data-mining/create-a-dmx-query-in-sql-server-management-studio.md) .  
  
## <a name="options"></a>옵션  
 **쿼리 결과 뷰로 전환**  
 
  **디자인**, **쿼리**및 **결과** 창 사이를 전환하려면 클릭합니다. 
  **결과** 창으로 전환하면 쿼리가 실행됩니다.  
 ...
 **단일 쿼리**  
 입력 값의 테이블에 대한 참조를 제공하는 대신 쿼리에 직접 입력 데이터를 제공하는 단일 쿼리를 만들 수 있습니다. 
  **입력 테이블 선택** 테이블이 **단일 쿼리 입력** 테이블로 바뀝니다. 단일 쿼리로 전환하면 현재 예측 쿼리가 손실됩니다.  
