---
title: Analysis Services 다차원에 대 한 세계화 시나리오 | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "36187310"
---
# <a name="globalization-scenarios-for-analysis-services-multiidimensional"></a>Analysis Services 다차원에 대한 세계화 시나리오
  [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 저장 하 고 다국어 데이터와 메타 데이터를 테이블 형식 및 다차원 모델 모두에서 데이터를 조작 합니다. 데이터 저장소는 유니코드 인코딩을 사용하는 문자 집합으로 된 유니코드(UTF-16)입니다. 데이터 모델에 ANSI 데이터를 로드하는 경우 문자는 유니코드 해당 코드 포인트를 사용하여 저장됩니다.  
...
-   오류, 경고 및 정보 메시지에서 반환 되는 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 엔진 (msmdsrv)는 Office와 Office 365에서 지 원하는 43 개의 언어로 지역화 됩니다. 메시지를 특정 언어로 가져오는 데에는 구성이 필요하지 않습니다. 클라이언트 응용 프로그램의 로캘은 반환되는 문자열을 결정합니다.  
...
-   [!INCLUDE[ssManStudio](../includes/ssmanstudio-md.md)] 및 [!INCLUDE[ss_dtbi](../includes/ss-dtbi-md.md)]와 같은 설명서와 도구는 한국어, 중국어 간체, 중국어 번체, 프랑스어, 독일어, 이탈리아어, 일본어, 포르투갈어(브라질), 러시아어 및 스페인어로 지역화되어 있습니다. 해당 언어 버전의 도구를 사용 하려면 해당 언어 버전의 SQL Server 설치 (예를 들어 독일어 버전의 SQL Server Management Studio 독일어로 가져오려는 설치)에 대 한 대상 언어로 독립 실행형 설치 프로그램을 실행 하거나 [!INCLUDE[ss_dtbi](../includes/ss-dtbi-md.md)]합니다.  
...
##  <a name="bkmk_features"></a> 세계화 된 다차원 솔루션을 구축 하기 위한 기능  
 ...
 클라이언트 수준에서 사용 하거나 조작 하는 응용 프로그램 전역화 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 다차원 데이터의 다국어 및 다문화 기능을 사용할 수 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]:  
  
-   [번역 &#40;Analysis Services&#41; ](translations-analysis-services.md) 번역 된 각 문자열이 다른 번역과 함께 존재할 수 있는 단일 개체에 대 한 여러 캡션을 포함 하 사용 됩니다. [!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)]을 사용하여 큐브 및 측정값, 차원 및 특성의 캡션, 설명 및 계정 유형에 대한 번역을 정의할 수 있습니다. [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 인스턴스에 연결할 때 로캘 ID를 제공하여 번역을 자동으로 정의한 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 개체에서 데이터 및 메타데이터를 검색할 수 있습니다.  
...
-   [통화 변환 &#40;Analysis Services&#41; ](currency-conversions-analysis-services.md) 는 통화 데이터를 포함 하는 측정값을 변환 하는 특수화 된 MDX 스크립트를 통해 수행 됩니다. [!INCLUDE[ss_dtbi](../includes/ss-dtbi-md.md)] 의 비즈니스 인텔리전스 마법사를 사용하면 통화 데이터를 포함하는 측정값을 변환하기 위해 차원, 특성 및 측정값 그룹의 데이터 및 메타데이터를 조합하여 사용하는 MDX 스크립트를 생성할 수 있습니다.  
...
|[번역 &#40;Analysis Services&#41;](translations-analysis-services.md)|에 대 한 번역을 정의 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 데이터베이스와 데이터베이스에 포함 된 개체입니다. 이 항목에서는 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]이 클라이언트 응용 프로그램의 번역된 데이터와 메타데이터에 대한 요청을 해결하는 방법에 대해 설명합니다.|  
...
## <a name="see-also"></a>관련 항목  
 ...
 [로캘 기반 적응형 디자인을 사용 하 여 쓰기 Windows 스토어 앱](http://blogs.windows.com/buildingapps/2014/03/06/writing-windows-store-apps-with-locale-based-adaptive-design/)   
