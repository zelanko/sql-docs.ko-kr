...
|**열 이름**|**유형 표시기**|**제한**|**설명**|  
|---------------------|------------------------|---------------------|---------------------|  
|`CATALOG_NAME`|`DBTYPE_WSTR`|사용자 계정 컨트롤|CSDLBI 설명이 요청된 데이터베이스의 이름을 지정합니다. 생략하는 경우 현재 데이터베이스가 사용됩니다.<br /><br /> 이 제한은 모든 모델 유형에 필수입니다.|  
|`PERSPECTIVE_ID`|`DBTYPE_WSTR`|사용자 계정 컨트롤|CATALOG_NAME으로 지정한 모델에 정의되어 있는 큐브 뷰 ID를 지정합니다.<br /><br /> 선택적 제한입니다. 모든 모델 유형에 적용됩니다.|  
|`PERSPECTIVE_NAME`|`DBTYPE_WSTR`|사용자 계정 컨트롤|CATALOG_NAME에 의해 지정된 모델에 정의되어 있는 큐브 뷰 이름을 지정합니다.<br /><br /> 이 제한은 테이블 형식 모델에 큐브 뷰가 포함되거나 다차원 솔루션에 큐브 또는 큐브 뷰가 여러 개 포함된 경우에 필요합니다.|  
|`METADATA`|`DBTYPE_WSTR`|아니요|CSDLBI 스키마에 따라 데이터 원본 및 그 속성의 XML 정의를 포함하는 문자열입니다.|  
|`CUBE_ID`|`DBTYPE_WSTR`|사용자 계정 컨트롤|문자열 식별자입니다.<br /><br /> 이 제한은 다차원 데이터베이스에 대한 선택 사항입니다. 큐브를 여러 개 사용할 수 있고 제한을 생략한 경우 기본 큐브가 반환됩니다.|  
