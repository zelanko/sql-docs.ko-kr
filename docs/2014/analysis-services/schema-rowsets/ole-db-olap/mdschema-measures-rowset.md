...
|`MEASURE_AGGREGATOR`|`DBTYPE_I4`||측정값이 파생된 방식을 식별하는 열거형입니다. 다음 값 중 하나를 사용할 수 있습니다.<br /><br /> -   `MDMEASURE_AGGR_SUM` (`1`)는 측정값에서 집계 됨을 식별 `SUM`합니다.<br />-   `MDMEASURE_AGGR_COUNT` (`2`)는 측정값에서 집계 됨을 식별 `COUNT`합니다.<br />-   **MDMEASURE_AGGR_MIN** (`3`)는 측정값에서 집계 됨을 식별 `MIN`합니다.<br />-   **MDMEASURE_AGGR_MAX** (`4`)는 측정값에서 집계 됨을 식별 `MAX`합니다.<br />-   `MDMEASURE_AGGR_AVG` (`5`)는 측정값에서 집계 됨을 식별 `AVG`합니다.<br />-   `MDMEASURE_AGGR_VAR` (`6`)는 측정값에서 집계 됨을 식별 `VAR`합니다.<br />-   `MDMEASURE_AGGR_STD` (`7`)는 측정값에서 집계 됨을 식별 `STDEV`합니다.<br />-   `MDMEASURE_AGGR_DST` (`8`)는 측정값에서 집계 됨을 식별 `DISTINCT COUNT`합니다.<br />-   `MDMEASURE_AGGR_NONE` (`9`)는 측정값에서 집계 됨을 식별 `NONE`합니다.<br />-   `MDMEASURE_AGGR_AVGCHILDREN` (`10`)는 측정값에서 집계 됨을 식별 `AVERAGEOFCHILDREN`합니다.<br />-   `MDMEASURE_AGGR_FIRSTCHILD` (`11`)는 측정값에서 집계 됨을 식별 `FIRSTCHILD`합니다.<br />-   `MDMEASURE_AGGR_LASTCHILD` (`12`)는 측정값에서 집계 됨을 식별 `LASTCHILD`합니다.<br />-   `MDMEASURE_AGGR_FIRSTNONEMPTY` (`13`)는 측정값에서 집계 됨을 식별 `FIRSTNONEMPTY`,<br />-   `MDMEASURE_AGGR_LASTNONEMPTY` (`14`)는 측정값에서 집계 됨을 식별 `LASTNONEMPTY`합니다.<br />-   `MDMEASURE_AGGR_BYACCOUNT` (`15`)는 측정값에서 집계 됨을 식별 `BYACCOUNT`합니다.<br />-   `MDMEASURE_AGGR_CALCULATED` (`127`) 측정값이 위의 단일 함수가 없는 수식에서 파생 된 식별 합니다.<br />-   `MDMEASURE_AGGR_UNKNOWN` (`0`)-측정값을 알 수 없는 집계 함수나 수식에서 파생 된 식별 합니다.|  
...
|`MEASURE_DISPLAY_FOLDER`|`DBTYPE_WSTR`||사용자 인터페이스에 측정값을 표시할 때 사용할 경로입니다. 폴더 이름은 세미콜론으로 구분됩니다. 중첩 된 폴더는 백슬래시로 표시 됩니다 (\\).|  
...
|`MEASURE_VISIBILITY`|`DBTYPE_UI2`|다음 유효 값 중 하나가 포함된 비트맵입니다(선택 사항).<br /><br /> -1 표시<br />-2는 보이지 않음<br /><br /> 기본 제한 값은 1입니다.|  
