...
|`MINIMUM_SCALE`|`DBTYPE_I2`||유형 표시기가 `DBTYPE_VARNUMERIC`, `DBTYPE_DECIMAL` 또는 `DBTYPE_NUMERIC`인 경우 소수점 오른쪽에 허용되는 최소 자릿수입니다. 그렇지 않으면 `NULL`합니다.|  
...
|`GUID`|`DBTYPE_GUID`||(나중에 사용할 계획) 형식이 형식 라이브러리에 설명되어 있는 경우 형식의 `GUID`입니다. 그렇지 않으면 `NULL`합니다.|  
|`TYPELIB`|`DBTYPE_WSTR`||(나중에 사용할 계획) 형식이 형식 라이브러리에 설명되어 있는 경우 형식 설명을 포함하는 형식 라이브러리입니다. 그렇지 않으면 NULL입니다.|  
