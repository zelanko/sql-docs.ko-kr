...
1.  시작 메뉴에서 클릭 **모든 프로그램**, Microsoft SharePoint 2010 제품을 연 클릭 **SharePoint 2010 중앙 관리**합니다.  
...
8.  클릭 **기본 PowerPivot 서비스 응용 프로그램** 를이 응용 프로그램에 대 한 PowerPivot 관리 대시보드를 엽니다. 처음 사용하는 경우 대시보드는 로드하는 데 몇 분 정도 걸립니다.  
  
     또는 클릭 하 여 빈 공간 옆에 **기본 PowerPivot 서비스 응용 프로그램** 하 여 행을 선택 하 고 클릭 **속성** 이 서비스 응용 프로그램에 대 한 구성 설정을 볼 수 있습니다. 구성 설정과 응용 프로그램 속성을 모두 수정하여 서버 구성을 변경할 수 있습니다. 이러한 설정에 대 한 자세한 내용은 참조 [만들고 중앙 관리에서 PowerPivot 서비스 응용 프로그램 구성](../../power-pivot-sharepoint/create-and-configure-power-pivot-service-application-in-ca.md)합니다.  
...
8.  서버 이름에 입력  **\<서버 이름 > \powerpivot**여기서  **\<서버 이름 >** PowerPivot for SharePoint가 설치 된 컴퓨터의 이름입니다.  
  
9. **연결**을 클릭합니다. Analysis Services 서버를 사용할 수 있는지 확인합니다.  
  
10. 개체 탐색기에서 클릭할 수 있는 **데이터베이스** 로드 되는 PowerPivot 데이터 파일의 목록을 볼 수 있습니다.  
  
11. 컴퓨터 파일 시스템의 다음 폴더에서 파일이 디스크로 캐시되었는지 확인합니다. 배포가 작동하는지 확인하려면 캐시된 파일이 있는지도 확인해야 합니다. 파일 캐시를 보려면로 이동는 \<드라이브 >: files\microsoft SQL Server\MSAS11 합니다. POWERPIVOT\OLAP\Backup\Sandboxes\Default PowerPivot 서비스 응용 프로그램 폴더입니다. 캐시된 각 데이터베이스는 고유 이름을 사용하도록 GUID 기반 명명 규칙을 사용하여 고유 폴더에 저장됩니다.  
