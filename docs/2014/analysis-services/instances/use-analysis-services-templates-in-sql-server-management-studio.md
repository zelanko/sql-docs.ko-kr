...
> [!WARNING]  
>  직접 쿼리 모드에서 배포된 테이블 형식 모델에 대해서는 MDX 쿼리를 사용할 수 없습니다. 그러나 EVALUATE 명령이 포함된 DAX 테이블 쿼리를 사용하여 동일한 쿼리를 보낼 수 있습니다. 자세한 내용은 참조 [DAX 쿼리 매개 변수](https://msdn.microsoft.com/library/gg492200(v=sql.120).aspx)합니다.  
...
2.  끌어서는 `Backup` 템플릿을 비어 있는 쿼리 창으로 합니다.  
  
3.  내에서 텍스트를 두 번 클릭은 \<DatabaseID > 요소입니다.  
  
4.  개체 탐색기에서 백업할 데이터베이스를 선택한 후 DatabaseID 요소를 묶고 있는 대괄호 사이에 끌어다 놓습니다.  
  
5.  내에서 텍스트를 두 번 클릭은 \<파일 > 요소입니다. .abf 파일 확장명을 포함하여 백업 파일의 이름을 입력합니다. 기본 백업 위치를 사용하지 않는 경우 전체 파일 경로를 지정합니다. 자세한 내용은 [데이터베이스 백업, 복원 및 동기화&#40;XMLA&#41;](../multidimensional-models-scripting-language-assl-xmla/backing-up-restoring-and-synchronizing-databases-xmla.md)를 참조하세요.  
...
3.  서식 파일을 바꾸고는 [RequestType 요소가 &#40;XMLA&#41; ](../xmla/xml-elements-properties/type-element-xmla.md) 다음 텍스트를 포함 하는 요소: `<RequestType>MDSCHEMA_INPUT_DATASOURCES</RequestType>`  
...
||Continuous Column Values|DMX SELECT DISTINCT FROM 사용 하는 방법을 보여 줍니다.  *\<모델 >* DMX 사용 하 여 문을 `RangeMin` 및 `RangeMax` 의 연속 열에서 지정된 된 범위에 있는 값의 집합을 검색 하는 함수는 지정한 마이닝 모델입니다.|  
...
||Nested Prediction|DMX SELECT FROM 결합 하는 방법을 보여 줍니다.  *\<모델 >* PREDICTION JOIN 문을 SHAPE 및 OPENQUERY 원본 데이터 쿼리와 중첩 포함 된 데이터를 사용 하 여 마이닝 모델에 대해 예측 쿼리를 실행 하려면 테이블 쿼리를 사용 하 여 기존 데이터 원본에서 검색 합니다.|  
||Nested Singleton Prediction|DMX SELECT FROM 사용 하는 방법을 보여 줍니다.  *\<모델 >* 열에서 예측 쿼리에 명시적으로 지정 된 단일 값을 사용 하 여 마이닝 모델에 대해 예측 쿼리를 실행 하려면 NATURAL PREDICTION JOIN 절 와 마이닝 모델의 열 이름이 마이닝 모델의 중첩 열 이름과 동일 UNION 문을 사용 하 여 만든 중첩된 테이블에 있는 값의 집합을 포함 하 고 합니다.|  
||Singleton Prediction|DMX SELECT FROM 사용 하는 방법을 보여 줍니다. \<모델 > NATURAL PREDICTION JOIN 문을 이름이 열에 일치 하는 열에서 예측 쿼리에 명시적으로 지정 된 단일 값을 사용 하 여 마이닝 모델에 대해 예측 쿼리를 실행 하려면 마이닝 모델입니다.|  
||Stored Procedure Call|DMX CALL 문을 사용하여 저장 프로시저를 호출하는 방법을 보여 줍니다.|  
|MDX\Expressions|Moving Average-Fixed|MDX를 사용 하는 방법을 보여 줍니다. `ParallelPeriod` 및 `CurrentMember` 고정 시간 차원의 계층에 포함 된 기간 동안의 측정값 이동 평균을 제공 하는 계산된 측정값을 만드는 일반적인 순서로 정렬 된 집합을 사용 하는 함수입니다.|  
||Moving Average-Variable|MDX를 사용 하는 방법을 보여 줍니다. `CASE` 내 문의 `Avg` 여러 시간 차원의 계층 구조에 포함 된 기간에 대 한 측정값의 이동 평균을 제공 하는 계산된 측정값을 만들 함수입니다.|  
||Periods to Date|계산 멤버에서 MDX `PeriodsToDate` 함수를 사용하는 방법을 보여 줍니다.|  
||Ratio to Parent|MDX를 사용 하는 방법을 보여 줍니다. `Parent` 지정한 계층에 있는 부모 멤버의 각 자식에 대 한 측정값의 비율을 나타내는 계산된 측정값을 만드는 함수입니다.|  
...
||KPI Query|MDX를 사용 하는 방법을 보여 줍니다. `KPIValue` 및 `KPIGoal` MDX 쿼리에서 핵심 성과 지표 (KPI) 정보를 검색 하는 함수입니다.|  
...
||취소|XMLA를 사용 하는 방법을 보여 줍니다. `Cancel` 명령을 (관리자 또는 서버 관리자 이외의 사용자)에 대 한 현재 세션에서 실행 중인 모든 작업 취소, 데이터베이스 (관리자 인 경우) 또는 인스턴스 (서버 관리자 용.)|  
...
||Process Partition|XMLA를 사용 하는 방법을 보여 줍니다. `Batch` 함께 명령은 `Parallel` 요소 및 `Process` 병렬 일괄 처리 작업을 사용 하 여 한 파티션을 완전히 처리 명령입니다.|  
||복원|XMLA를 사용 하는 방법을 보여 줍니다. `Restore` 복원 하려면 명령을 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 기존 백업 파일에서 데이터베이스입니다.|  
||동기화|XMLA를 사용 하는 방법을 보여 줍니다. `Synchronize` 다른 동기화 하는 명령을 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 현재의 데이터베이스 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] SkipMembership 옵션을 사용 하 여 SynchronizeSecurity 태그에 대 한 데이터베이스입니다.|  
|XMLA\Schema Rowsets|스키마 행 집합 검색|XMLA `Discover` 메서드를 사용하여 DISCOVER_SCHEMA_ROWSETS 스키마 행 집합의 콘텐츠를 검색하는 방법을 보여 줍니다.|  
|XMLA\Server Status|Connections|XMLA를 사용 하는 방법을 보여 줍니다. `Discover` DISCOVER_CONNECTIONS 스키마 행 집합의 콘텐츠를 검색 하는 메서드입니다.|  
||에서|XMLA를 사용 하는 방법을 보여 줍니다. `Discover` DISCOVER_JOBS 스키마 행 집합의 콘텐츠를 검색 하는 메서드입니다.|  
||위치|XMLA를 사용 하는 방법을 보여 줍니다. `Discover` 위치 백업 파일의 경로 지정 하며 DISCOVER_LOCATIONS 스키마 행 집합을의 콘텐츠를 검색 하는 메서드.|  
...
||성능 카운터|XMLA를 사용 하는 방법을 보여 줍니다. `Discover` DISCOVER_PERFORMANCE_COUNTERS 스키마 행 집합의 콘텐츠를 검색 하는 메서드입니다.|  
||세션|XMLA `Discover` 메서드를 사용하여 DISCOVER_SESSIONS 스키마 행 집합의 콘텐츠를 검색하는 방법을 보여 줍니다.|  
||Traces|XMLA를 사용 하는 방법을 보여 줍니다. `Discover` DISCOVER_TRACES 스키마 행 집합의 콘텐츠를 검색 하는 메서드입니다.|  
||트랜잭션|XMLA를 사용 하는 방법을 보여 줍니다. `Discover` DISCOVER_TRANSACTIONS 스키마 행 집합의 콘텐츠를 검색 하는 메서드입니다.|  
