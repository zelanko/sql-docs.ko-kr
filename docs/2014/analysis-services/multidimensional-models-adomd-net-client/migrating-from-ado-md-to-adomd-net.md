...
-   `Cellset` 개체를 사용하여 MDX(Multidimensional Expressions) 문을 실행합니다.  
  
-   `Position` 개체에서 검색된 `Cellset` 개체에서 메타데이터와 데이터를 검색합니다.  
...
-   `AdomdConnection` 개체를 사용하여 연결을 만듭니다.  
  
-   `AdomdCommand` 개체를 사용하여 MDX 문을 실행합니다.  
  
-   `Set` 개체에서 검색된 `Cellset` 개체에서 메타데이터와 데이터를 검색합니다.  
