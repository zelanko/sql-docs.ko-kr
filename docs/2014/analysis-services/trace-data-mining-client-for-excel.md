...
ms.locfileid: "36089994"
---
# <a name="trace-data-mining-client-for-excel"></a>추적(Excel용 데이터 마이닝 클라이언트)
  ![추적 단추](media/misc-trace.gif "추적 단추")  
  
 **추적 프로그램** 대화 상자를 사용 하면 인스턴스에 보내는 문을 모니터링할 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 데이터 마이닝에 사용 중인 합니다. 인스턴스에 대 한 연결을 만든 후 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)], 클라이언트와 서버 간의 모든 상호 작용에 기록 되는 **추적 프로그램** 구조를 만들고, 마이닝 모델을 추가 및 확인 하는 문을 포함 하는 창 예측을 뿐만 아니라 서버에서 반환 되는 메시지입니다.  
 ...
 **추적** 의 내용과 현재 연결만 모니터링는 **추적 프로그램** 대화 상자는 저장 되지 않습니다.  
 ...
 **참고** 임시 개체를 사용 하는 기능은 Excel 용 테이블 분석 도구를 사용 하는 경우에 사용할 수 있습니다. Visio용 데이터 마이닝 템플릿 및 Excel용 데이터 마이닝 클라이언트에서는 구조와 모델을 서버에 저장해야 합니다.  
...
## <a name="tracing-temporary-structures-and-models"></a>임시 구조 및 모델 추적  
 ...
 옵션을 선택을 취소할 수 있는 테이블 분석 도구 중 하나를 사용 하는 경우 작업 한 내용을 유지 하려는 경우 **세션 모델 사용**서버에서 영구적으로 저장 하 여 모델을 취소 하 합니다. 복사할 수도 있습니다는 **추적 프로그램** 파일로 창 작업을 나중에 만들 다시 수 있도록 합니다.  
...
## <a name="changing-connections"></a>연결 변경  
 ...
 연결을 변경 하 여 Excel 통합 문서에서 작업 하는 동안 연결을 변경 하는 경우에 기록 되지 않습니다는 **추적 프로그램** 창. 이름 및 현재 연결의 상태를 명시적으로 표시 하려면 클릭 하면 **현재 연결**합니다.  
...
## <a name="understanding-statements-in-the-tracer"></a>Tracer의 문 이해  
 ...
 DMX 문 및 해당 구문에 대한 자세한 설명은 이 항목에서 다루지 않습니다. 그러나의 정보를 사용할 수는 **추적 프로그램** DMX 문의 동작에 대 한 자세한 정보를 찾는 창. 또한 Excel용 데이터 마이닝 추가 기능을 사용하면 손쉽게 복잡한 DMX 문을 작성하고 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 서버와 상호 작용할 수 있습니다. 자세한 내용은 참조 [쿼리 &#40;SQL Server 데이터 마이닝 추가 기능&#41;](query-sql-server-data-mining-add-ins.md)합니다.  
