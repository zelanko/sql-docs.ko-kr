...
2.  아무 곳 이나 마우스 오른쪽 단추로 클릭는 **다이어그램 구성 도우미** 창에서 클릭 **새 다이어그램**를 선택한 다음 다이어그램의 이름을 `Sales Quotas`합니다.  
  
3.  끌어서는 **직원**, **Sales Territory**, 및 `Date` 에서 테이블의 **테이블** 창에는 **다이어그램** 창.  
...
6.  데이터 원본 뷰 디자이너에서 변경 된 **FriendlyName** 속성은 **FactSalesQuota** 테이블 `SalesQuotas`합니다.  
  
7.  [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] Tutorial 큐브로 전환한 다음 **큐브 구조** 탭을 클릭합니다.  
  
8.  아무 곳 이나 마우스 오른쪽 단추로 클릭는 **측정값** 창에서 클릭 **새 측정값 그룹**, 클릭 `SalesQuotas` 에 **새 측정값 그룹** 대화 상자를 선택한 다음 클릭**확인**합니다.  
  
     `Sales Quotas` 측정값 그룹에 나타나지는 **측정값** 창. 에 **차원** 창에서 다음에 유의 새 `Date` 큐브 차원 에서도 정의에 따라는 `Date` 데이터베이스 차원입니다. [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 에서는 Sales Quotas 측정값 그룹의 기반이 되는 **FactSalesQuota** 팩트 테이블의 **DateKey** 열과 관련된 기존 시간 관련 큐브 차원을 인식하지 못하므로 새 시간 관련 큐브 차원이 정의됩니다. 이 항목의 다른 태스크에서 이 큐브 차원을 변경합니다.  
  
9. 확장 된 `Sales Quotas` 측정값 그룹입니다.  
  
10. **측정값** 창에서 **Sales Amount Quota**를 선택한 다음 속성 창에서 **FormatString** 속성 값을 **Currency** 로 설정합니다.  
  
11. 선택 된 **Sales Quotas Count** 측정값을 선택한 다음 입력 `#,#` 에 대 한 값으로는 **FormatString** 속성 창에서 속성.  
  
12. 삭제는 **Calendar Quarter** 에서 측정는 `Sales Quotas` 측정값 그룹입니다.  
...
13. 에 **측정값** 창에서 마우스 오른쪽 단추로 클릭는 `Sales Quotas` 측정값 그룹을 마우스 클릭 **새 측정값**합니다.  
  
     **합계** 사용 유형을 사용하는 측정값에 사용 가능한 원본 열이 포함되어 있는 **새 측정값**대화 상자가 열립니다.  
  
14. 에 **새 측정값** 대화 상자에서 **고유 카운트** 에 **사용** 나열 되어 있는지 확인 합니다. `SalesQuotas` 에서 선택한는 **원본테이블** 목록에서 **EmployeeKey** 에 **원본 열** 목록으로 이동한 다음 클릭 **확인**합니다.  
...
15. 에 대 한 값을 변경는 **이름** 속성에 대 한는 **Employee Key Distinct Count** 측정값을 `Sales Person Count`, 한 다음 입력 `#,#` 에 대 한 값으로는 **FormatString** 속성입니다.  
...
4.  피벗 테이블 필드 목록에서 확장 된 `Sales Quotas` 측정값 그룹, 한 다음 끌어는 **Sales Amount Quota** 측정값을 값 영역입니다.  
...
11. 피벗 테이블에서 행 레이블 옆에 있는 아래쪽 화살표를 클릭합니다. **FY 2008**을 제외한 모든 연도를 선택 취소합니다.  
     ...
     공지만는 **2007 년 7 월** 의 멤버는 **월** 수준이 나타납니다 대신는 **2007 년 7 월 일**, **2007 년 8 월**, 및 **2007 년 9 월 일** 의 멤버 **월** 수준 및만 하는 **2007 년 7 월 1 일** 의 멤버는 `Date` 모든 31 일 대신 수준이 나타납니다. 이 동작은 팩트 테이블의 데이터 수준이 분기 수준과 수준은 때문에 발생는 `Date` 차원은 매일입니다. 이 항목의 다음 태스크에서는 이 동작을 변경하는 방법에 대해 설명합니다.  
     ...
     ![Sales에 대 한 값 해석 할당량](../../2014/tutorials/media/l5-granularity-3.png "판매에 대 한 값 해석 할당량")  
...
2.  **특성** 창에서 **SalesTerritoryKey**를 선택한 후 속성 창에서 **AttributeHierarchyVisible** 속성을 **False** 로, **AttributeHierarchyOptimizedState** 속성을 **NotOptimized**로, **AttributeHierarchyOrdered** 속성을 **False**로 설정합니다.  
     ...
     연결 하려면이 특성이 필요는 **Sales Territory** 차원는 `Sales Quotas` 및 **Sales Quotas 1** 참조 차원으로 측정값 그룹입니다.  
  
3.  에 대 한 큐브 디자이너의 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] Tutorial 큐브에 클릭는 **차원 용도** 탭을 클릭 한 다음 내에서 차원 용도 검토는 `Sales Quotas` 및 **Sales Quotas 1** 측정값 그룹입니다.  
  
     에 **직원** 및 `Date` 큐브 차원에 연결 된는 **Quotasand Sales Quotas 1<** 일반 관계를 통해 측정값 그룹과 합니다. **Sales Territory** 큐브 차원은 이러한 측정값 그룹에 연결되어 있지 않습니다.  
  
4.  교집합에서 셀을 클릭 하 고 **Sales Territory** 차원 및 `Sales Quotas` 다음 찾아보기 단추를 클릭 하 고 측정값 그룹 (**...** ). **관계 정의** 대화 상자가 열립니다.  
...
18. 삭제 된 `Date` 큐브 차원입니다.  
     ...
     4 개의 시간 관련 큐브 차원이 아니라 사용 합니다는 **Order Date** 큐브 차원에는 `Sales Quotas` 측정값 그룹을 보고 있는 판매 할당량을 차원이 결정 됩니다. 또한 이 큐브 차원을 큐브의 주 날짜 차원으로 사용합니다.  
  
19. 에 **차원** 목록, 이름 바꾸기는 **Order Date** 큐브 차원의 `Date`합니다.  
  
     이름 바꾸기는 **Order Date** 큐브 차원의 `Date` 사용자가 역할을이 큐브의 주 날짜 차원으로 이해 하기 쉬워집니다.  
...
1.  로 전환 **차원 디자이너** 에 대 한는 `Date` 차원을 한 다음 클릭는 **특성 관계** 탭 합니다.  
  
     되었지만 **역 년** 에 연결 된 **r t e r** 통해는 **Calendar Semester** 회계 달력 특성 하나에 연결 된 특성 다른; 에 연결 되지 않은 **Calendar Quarter** 특성을 따라서 됩니다에서 제대로 집계 되지는 `Sales Quotas` 측정값 그룹입니다.  
...
4.  **확인**을 클릭합니다.  
     ...
     않는다는 경고 메시지가 표시는 `Date` 차원 데이터가 키가 아닌 특성이 세분성 특성으로 사용 될 때 집계 되지 못할 수도 있다는 하나 이상의 중복 특성 관계가 포함 되어 있습니다.  
...
## <a name="next-lesson"></a>다음 단원  
 [6 단원: 계산 정의] ((단원-6-정의-calculations.md)  
