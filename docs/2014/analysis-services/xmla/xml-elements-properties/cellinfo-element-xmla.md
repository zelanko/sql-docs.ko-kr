...
ms.locfileid: "36094044"
---
# <a name="cellinfo-element-xmla"></a>CellInfo 요소(XMLA)
  부모에 포함 된 셀 메타 데이터를 나타내는 [OlapInfo](olapinfo-element-xmla.md) 요소입니다.  
...
```  
 ...
 공급자는 필요에 따라 `AxisInfo` 또는 `CellInfo` 섹션의 개별 멤버나 셀 속성에 대해 기본값을 지정할 수 있습니다. 속성에 항상 또는 거의 항상 같은 값이 지정되는 경우 기본값은 보다 작은 결과를 제공할 수 있습니다. 속성에 대 한 기본값을 나타내기 위해는`Default` 셀 속성 정의 요소 중 하나의 자식 요소로 요소를 지정할 수도 있습니다. 따라서 결과에 멤버나 셀 속성이 없으면 지정된 기본값이 셀 속성의 값이 됩니다.  
