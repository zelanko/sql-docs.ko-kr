...
5.  에 대 한 행에는 **캡션** 개체에 대 한는 **날짜** 차원에서 입력 `Fecha` 에 **스페인어 (스페인)** 번역 열 및 `Temps` 는 에서 **프랑스어 (프랑스)** 번역 열입니다.  
  
6.  에 대 한 행에는 **캡션** 개체에 대 한는 **Month Name** 특성을 입력 `Mes del Año` 에 **스페인어 (스페인)** 번역 열 및 `Mois d'Année` 에 **프랑스어 (프랑스)** 번역 열입니다.  
...
1.  [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] Tutorial 큐브에 대한 큐브 디자이너로 전환한 후 **번역** 탭으로 전환합니다.  
     ...
     ![기본 언어 번역 탭에서](../../2014/tutorials/media/l9-translations-5.gif "기본 언어 번역 탭에서")  
...
6.  에 대 한 행에는 **캡션** 개체에 대 한는 **인터넷 판매** 측정값 그룹을 입력 `Ventas del lnternet` 에 **스페인어 (스페인)** 번역 열 및 `Ventes D'Internet` 에서 **프랑스어 (프랑스)** 번역 열입니다.  
  
7.  에 대 한 행에는 **캡션** Internet Sales-sales Amount 측정값을 형식에 대 한 개체 `Cantidad de las Ventas del Internet` 에 **스페인어 (스페인)** 번역 열 및 `Quantité de Ventes d'Internet` 에 **(프랑스어 프랑스)** 번역 열입니다.  
...
5.  도구 모음의 **언어** 목록에서 **스페인어(스페인)** 를 선택합니다.  
     ...
     메타데이터 창의 항목이 다시 채워집니다. 메타데이터 창의 항목이 다시 채워지면 인터넷 판매 표시 폴더에 Internet Sales-Sales Amount 측정값이 더 이상 나타나지 않습니다. 대신, 라는 새 표시 폴더에 스페인어로 표시 됩니다 `Ventas del lnternet`다음 그림에 표시 된 것 처럼 합니다.  
...
7.  메타 데이터 창에서 확장 `Fecha`, 확장 **Fecha.Calendar Date**를 마우스 오른쪽 단추로 클릭 **Fecha.Calendar Date**를 선택한 후 **필터에 추가**합니다.  
...
9. 메타데이터 창에서 **Mes del Ano** 를 마우스 오른쪽 단추로 클릭하고 **쿼리에 추가**를 선택합니다.  
     ...
     ![데이터 창에 스페인어에서 월 이름](../../2014/tutorials/media/l9-translations-7.gif "데이터 창에 스페인어에서 월 이름")  
