...
## <a name="cell-properties-keyword-syntax"></a>CELL PROPERTIES 키워드 구문  
 에 대해 다음 구문을 사용 합니다 `CELL PROPERTIES` mdx 키워드 `SELECT` 문:  
...
|**FONT_NAME**|표시 하는 데 사용할 글꼴을 `VALUE` 또는 `FORMATTED_VALUE` 속성입니다.|  
...
|`FORMATTED_VALUE`|형식이 지정 된 표시를 나타내는 문자열을 `VALUE` 속성입니다.|  
|`LANGUAGE`|`FORMAT_STRING`이 적용될 로캘입니다. `LANGUAGE` 통화 변환에 대 한 일반적으로 사용 됩니다.|  
...
|`VALUE`|형식이 지정되지 않은 셀 값입니다.|  
 ...
 만 `CELL_ORDINAL`, `FORMATTED_VALUE`, 및 `VALUE` 셀 속성은 필수입니다. 모든 셀 속성(고유 또는 공급자별)은 해당 데이터 형식 및 공급자 지원을 포함해 `PROPERTIES` 스키마 행 집합에 정의되어 있습니다. 에 대 한 자세한 내용은 합니다 `PROPERTIES` 스키마 행 집합 참조 [MDSCHEMA_PROPERTIES 행 집합](../../schema-rowsets/ole-db-olap/mdschema-properties-rowset.md)합니다.  
  
 기본적으로 하는 경우는 `CELL PROPERTIES` 키워드가 사용 되지 않습니다, 셀 속성이 반환 됩니다 `VALUE`를 `FORMATTED_VALUE`, 및 `CELL_ORDINAL` (해당 순서 대로). 경우는 `CELL PROPERTIES` 키워드를 사용 하는 키워드를 사용 하 여 명시적으로 지정한 셀 속성만 반환 됩니다.  
  
 다음 예제에서는 사용 된 `CELL PROPERTIES` MDX 쿼리에서 키워드:  
...
```  
 ...
 일반된 행 집합;를 반환 하는 MDX 쿼리에 대 한 셀 속성이 반환 되지 않습니다. 처럼 각 셀은 표시 하는 경우에 `FORMATTED_VALUE` 셀 속성만 반환 된 것입니다.  
