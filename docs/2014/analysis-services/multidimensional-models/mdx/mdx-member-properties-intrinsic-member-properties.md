...
# <a name="intrinsic-member-properties-mdx"></a>기본 멤버 속성(MDX)
 ...
 기본 속성으로는 모든 멤버가 모든 수준에서 표시하는 `ID`, `KEY`, `KEYx` 및 `NAME`이 있습니다. 위치 정보 등 반환할 수도 `LEVEL_NUMBER` 또는 `PARENT_UNIQUE_NAME`, 등입니다.  
...
## <a name="types-of-member-properties"></a>멤버 속성의 유형  
 ...
 다음 예제에 대 한 경로 포함 하는 방법을 확인할 수는 `KEY` 속성: `MEMBER [Measures].[Parent Member Key] AS [Product].[Product Categories].CurrentMember.Parent.PROPERTIES("KEY")`합니다.  
  
 상황에 맞지 않는 멤버 속성  
 이 유형의 멤버 속성은 특정 차원 또는 수준의 컨텍스트에서 사용할 수 없고 한 축의 모든 멤버에 대한 값을 반환합니다.  
  
 상황에 맞지 않는 속성은 독립적이며 경로 정보를 포함하지 않습니다. 차원 또는 수준에 대해 지정 된 없습니다 되어 `PARENT_UNIQUE_NAME` 다음 예제에서: `DIMENSION PROPERTIES PARENT_UNIQUE_NAME ON COLUMNS`  
...
> [!NOTE]  
>  MEMBERS 스키마 행 집합의 열은 다음 표에 나열된 기본 멤버 속성을 지원합니다. 에 대 한 자세한 내용은 `MEMBERS` 스키마 행 집합 참조 [MDSCHEMA_MEMBERS 행 집합](../../schema-rowsets/ole-db-olap/mdschema-members-rowset.md)합니다.  
...
|`MEMBER_TYPE`|멤버의 유형입니다. 이 속성 값은 다음 중 하나일 수 있습니다. <br />**MDMEMBER_TYPE_REGULAR**<br />**MDMEMBER_TYPE_ALL**<br />**MDMEMBER_TYPE_FORMULA**<br />**MDMEMBER_TYPE_MEASURE**<br />**MDMEMBER_TYPE_UNKNOWN**<br /><br /> <br /><br /> MDMEMBER_TYPE_FORMULA가 MDMEMBER_TYPE_MEASURE보다 우선합니다. 따라서 Measures 차원에 수식 (계산된) 멤버가 없는 경우, 고 `MEMBER_TYPE` 계산된 멤버에 대 한 속성은 MDMEMBER_TYPE_FORMULA입니다.|  
...
### <a name="properties-syntax-for-non-context-sensitive-properties"></a>상황에 맞지 않는 속성에 대한 PROPERTIES 구문  
 다음 구문을 사용 하 여 사용 하 여 내장 비 컨텍스트 기본 멤버 속성을 지정 하는 `PROPERTIES` 키워드:  
  
 `DIMENSION PROPERTIES Property`  
  
 이 구문에서는 차원이나 수준별로 속성을 정규화할 수 없습니다. 상황에 맞지 않는 기본 멤버 속성이 축의 모든 멤버에 적용되므로 속성을 정규화할 수 없습니다.  
  
 예를 들어 지정 하는 MDX 문의 `DESCRIPTION` 내장 멤버 속성에는 다음 구문이 있을 것:  
