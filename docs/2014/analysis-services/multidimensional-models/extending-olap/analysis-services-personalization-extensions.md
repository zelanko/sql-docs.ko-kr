...
### <a name="mdx-extensions-and-enhancements"></a>MDX 확장 및 향상된 기능  
 ...
 CREATE SET 명령은 `caption` 속성, `display_folder` 속성 및 새로운 `STATIC | DYNAMIC` 키워드를 통해 향상되었습니다. *정적* 집합이 생성 시에만 계산 되는 것을 의미 합니다. *동적* 집합은 쿼리에서 사용 될 때마다 계산을 의미 합니다. 키워드가 생략된 경우 기본값은 `STATIC`입니다.  
