...
##  <a name="measures"></a><a name="bkmk_measure"></a>측정값  
 ...
 측정값은 컨텍스트를 구분하며, 쿼리에 포함되는 모든 차원 멤버에 의해 결정되는 컨텍스트에서 숫자 데이터에 대해 작동합니다. 예를 들어 **대리점 판매** 를 계산 하는 측정값은 운영자에 의해 지원 `Sum` 되며 쿼리에 포함 된 각 차원 멤버의 판매액을 추가 합니다. 쿼리가 개별 제품을 지정하든 범주로 롤업하든 시간 또는 지리로 분할되든 관계없이 측정값은 쿼리에 포함된 차원에 대해 유효한 연산을 생성해야 합니다.  
