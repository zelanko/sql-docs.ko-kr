...
|속성|정의|  
|--------------|----------------|  
|`AggregateFunction`|필수 요소. 측정값이 집계되는 방법을 결정합니다. `Sum`이 기본 집계입니다. 자세한 내용은 각 함수 설명의 [Use Aggregate Functions](use-aggregate-functions.md) 을 참조하세요.|  
|`DataType`|필수 요소. 측정값이 바인딩되는 기본 팩트 테이블 열의 데이터 형식을 지정합니다. 이 값은 기본적으로 원본 열에서 상속됩니다.|  
...
|`ID`|필수 요소. 측정값의 고유 ID를 표시합니다. 이 속성은 읽기 전용입니다.|  
...
|`Name`|필수 요소. 측정값의 이름을 지정합니다.|  
|`Source`|필수 요소. 측정값이 바인딩되는 데이터 원본 뷰의 열을 지정합니다. [데이터 원본 및 바인딩&#40;SSAS 다차원&#41;](data-sources-and-bindings-ssas-multidimensional.md)을 참조하세요.|  
