...
## <a name="modifying-the-input-files"></a>입력 파일 수정  
 전체를 편집 하지 않고 배포 옵션 및 배포 대상, 구성 설정을 변경할 수 있도록 입력된 파일에서 검색 된 값 이나 입력된 파일에서 값을 수정 \< *프로젝트 이름*>.asdatabase 파일 (또는 기존 스크립트를 생성 하는 경우 전체 XMLA 스크립트 파일 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 데이터베이스). 개별 파일을 수정할 수 있으면 다른 용도의 다른 배포 스크립트를 쉽게 만들 수 있습니다.  
