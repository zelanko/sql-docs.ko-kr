...
# <a name="multidimensional-model-solution-deployment"></a>다차원 모델 솔루션 배포
  ...
  [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 프로젝트 개발을 완료했으면 Analysis Services 서버에 데이터베이스를 배포할 수 있습니다. Analysis Services는 데이터베이스를 테스트 서버나 프로덕션 서버로 이동하는 데 사용할 수 있는 6가지의 배포 방법을 제공합니다. 메소드를 이점이 많은 순서에 따라 나열하면 AMO Automation, XMLA, 배포 마법사, 배포 유틸리티, 동기화 마법사, 백업 및 복원과 같습니다.  
...
|**Backup 및 복원**|백업은 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 데이터베이스를 이동하는 가장 단순한 방법입니다. 
  **백업** 대화 상자에서 옵션을 구성한 다음 대화 상자에서 직접 백업을 실행하거나 스크립트를 만들고 저장하여 필요할 때마다 실행할 수 있습니다.<br /><br /> 백업 및 복원은 다른 배포 방법만큼 자주 사용하지는 않지만 최소한의 인프라 요구 사항으로 배포를 빠르게 완료할 수 있습니다.|[Analysis Services 데이터베이스 백업 및 복원](backup-and-restore-of-analysis-services-databases.md)|  
  
##  <a name="bkmk_considerations"></a>배포 고려 사항  
...
|배포된 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 데이터베이스에서 데이터를 어떻게 로드하고 업데이트합니까?<br /><br /> 배포된 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 데이터베이스에서 메타데이터(예: 계산)를 어떻게 업데이트하나요?|[배포 방법](#bkmk_meth) 항목을 참조 하십시오.|  
|인터넷을 통해 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 데이터에 대한 액세스를 사용자에게 제공하기를 원합니까?|[&#40;IIS&#41; 8.0 인터넷 정보 서비스에서 Analysis Services에 대 한 HTTP 액세스 구성](../instances/configure-http-access-to-analysis-services-on-iis-8-0.md)|  
|
...
|연결된 개체 또는 원격 파티션을 사용하여 분산 환경에 개체를 배포하기를 원합니까?|[Analysis Services&#41;&#40;로컬 파티션 만들기 및 관리 ](create-and-manage-a-local-partition-analysis-services.md), Analysis Services&#41;및 [연결 된 측정값 그룹](linked-measure-groups.md) [&#40;원격 파티션 만들기 및 관리](create-and-manage-a-remote-partition-analysis-services.md)|  
|
  [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 데이터를 어떻게 보호합니까?|[Analysis Services&#41;&#40;개체 및 작업에 대 한 액세스 권한 부여](authorizing-access-to-objects-and-operations-analysis-services.md)|  
