 ...
 **차원 보안에 대 한 장애물 확인: 기본 측정값, 기본 멤버 및 측정값 식에 사용 되는 측정값**  
  
1.  SQL Server Management Studio에서 큐브를 마우스 오른쪽 단추로 클릭 하 고 **큐브** | 스크립팅**을 선택 하 여** | **새 쿼리 편집기 창**으로 변경 합니다.  
  
2.  
  `DefaultMeasure`를 검색합니다. 큐브에 대해 하나, 각 큐브 뷰에 대해 하나를 찾아야 합니다. 차원 보안을 정의할 때 기본 측정값에 대한 액세스를 제한하지 않습니다.  
...
3.  특성 계층을 선택합니다. 일부 특성은 사용할 수 없습니다. 
  **AttributeHierarchyEnabled** 가 있는 특성만 **특성 계층** 목록에 표시됩니다.  
  
4.  액세스를 허용하거나 거부할 구성원을 선택합니다. 
  **멤버 모두 선택** 옵션에서 액세스 허용이 기본값입니다. 이 기본값을 유지한 다음 이 역할을 통해 **멤버 자격** 창의 Windows 사용자 및 그룹 계정에 표시하지 않을 각 구성원을 선택 해제하는 것이 좋습니다. 향후 처리 작업에서 추가되는 새 구성원이 이 역할을 통해 연결하는 사람들에게 자동으로 표시되는 이점이 있습니다.  
...
5.  필요에 따라 **고급** 을 클릭 `Visual Totals` 하 여이 특성 계층에 대해를 사용 하도록 설정 합니다. 이 옵션은 역할을 통해 사용 가능한 구성원을 기준으로 집계를 다시 계산합니다.  
  
    > [!NOTE]  
    >  차원 구성원을 지우는 권한을 적용할 때, 집계된 합계가 자동으로 다시 계산되지 않습니다. 사용 권한이 `All` 적용 되기 전에 특성 계층의 멤버가 200의 수를 반환 한다고 가정 합니다. 일부 멤버에 대 한 액세스를 거부 하는 `All` 권한을 적용 한 후에도 사용자에 게 표시 되는 멤버 값이 훨씬 더 적으면에서 200을 반환 합니다. 큐브의 소비자를 혼동 하지 않으려면 특성 계층의 모든 멤버를 집계 `All` 하는 것이 아니라 멤버를 역할 멤버에 대 한 멤버의 집계로 구성할 수 있습니다. 이 동작을 호출 하기 위해 차원 보안 `Visual Totals` 을 구성할 때 **고급** 탭에서를 사용 하도록 설정할 수 있습니다. 이 기능을 설정하면 미리 계산된 집계에서 가져오지 않고 쿼리 시 집계가 계산됩니다. 이 기능은 쿼리 성능에 많은 영향을 미칠 수 있으므로 필요한 경우에만 사용하세요.  
...
## <a name="advanced-dimension-security"></a>고급 차원 보안  
 ...
 MDX 작성기를 사용하여 MDX 문을 쓸 수 있습니다. 자세한 내용은 [MDX 작성기&#40;Analysis Services - 다차원 데이터&#41;](../mdx-builder-analysis-services-multidimensional-data.md)를 참조하세요. 
  **고급** 탭에는 다음 옵션이 있습니다.  
...
> [!NOTE]  
>  특성 멤버의 빈 집합 ({})을 정의 하는 경우에는 해당 특성의 멤버가 데이터베이스 역할에 표시 되지 않습니다. 허용 집합이 없는 경우가 빈 집합으로 해석되지는 않습니다.  
 ...
 **보이는 합계 사용**  
 ...
 기본적으로 VisualTotals 속성은 사용 하지 않도록 설정 됩니다 ( `False`로 설정 됨). 이 기본 설정을 사용하면 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 가 계산할 셀 값을 선택하는 데 시간을 들이는 대신 모든 셀 값의 합계를 빠르게 계산할 수 있기 때문에 성능이 향상됩니다.  
 ...
 VisualTotals 속성을로 `True` 설정 하면 이러한 위험을 없앨 수 있습니다. VisualTotals 속성을 사용할 경우 데이터베이스 역할은 해당 역할이 사용 권한을 갖는 차원 멤버에 대해 집계된 합계만 볼 수 있습니다.  
...
## <a name="see-also"></a>참고 항목  
 ...
 [Analysis Services&#41;&#40;데이터 원본 개체에 대 한 사용 권한 부여](grant-permissions-on-a-data-source-object-analysis-services.md)  
