...
-   필요에 따라 `Type` 속성을 ExchangeRate로 설정합니다.  비즈니스 인텔리전스 마법사는 통화 차원과 시간 차원으로 관계를 사용 하 여 가능성이 있는 요율 측정값 그룹을 식별, 하는 동안 설정 된 `Type` 속성을 exchangerate로 클라이언트 응용 프로그램 속도 측정값을 보다 쉽게 식별할 수 있습니다 그룹입니다.  
