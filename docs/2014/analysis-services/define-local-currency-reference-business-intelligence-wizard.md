 ...
 **차원 테이블의 특성**  
 ...
 트랜잭션과 다른 비즈니스 엔터티 간의 관계(위치 등)가 해당 트랜잭션에 대한 현지 통화를 결정하는 경우 이 옵션을 사용합니다. 예를 들어 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 예제 데이터베이스[!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)]에서 Financial Reporting 측정값 그룹은 Organization 차원을 통해 Currency 차원에 대한 참조된 차원 관계를 포함합니다. 즉, Financial Reporting 측정값 그룹의 팩트 테이블에는 Organization 차원의 차원 테이블에 있는 멤버를 참조하는 외래 키 열이 있습니다. Organization 차원의 차원 테이블에는 Currency 차원의 차원 테이블에 있는 통화 식별자를 참조하는 외래 키 열이 있습니다.  
