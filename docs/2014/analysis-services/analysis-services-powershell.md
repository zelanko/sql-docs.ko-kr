...
|테이블 형식 인스턴스 및 데이터베이스|로컬 및 원격 관리에 지원됩니다.<br /><br /> 에 대 한 자세한 내용은 참조는 2011 년 8 월 블로그 [테이블 형식 모델 사용 하 여 PowerShell 관리](http://go.microsoft.com/fwlink/?linkID=227685)합니다.|  
...
|SharePoint의 BI 의미 체계 모델 연결 파일(.bism)에 대한 HTTP 연결<br /><br /> "데이터 원본 =http://server/shared_docs/name.bism"|지원되지 않습니다.|  
...
##  <a name="bkmk_auth"></a> 인증 요구 사항 및 보안 고려 사항  
 ...
 HTTP 연결은 IIS를 통해 간접적으로 이루어지며 기본 인증 등의 추가 인증 옵션을 사용하여 Analysis Services 인스턴스에 연결할 수 있습니다. IIS는 가장을 지원하므로 IIS가 연결 시 가장에 사용할 자격 증명을 포함하는 연결 문자열을 제공할 수 있습니다. 자격 증명을 제공하려면 –Credential 매개 변수를 사용합니다.  
  
 **사용 하 여 PowerShell에서 – Credential 매개 변수**  
  
 –Credential 매개 변수는 사용자 이름 및 암호를 지정하는 PSCredential 개체를 사용합니다. Analysis Services PowerShell에서 –Credential 매개 변수는 기존 연결의 컨텍스트 내에서 실행되는 cmdlet이 아니라 Analysis Services에 대한 연결 요청을 수행하는 cmdlet에서 사용할 수 있습니다. 연결 요청을 수행하는 cmdlet에는 Invoke-ASCmd, Backup-ASDatabase, Restore-ASDatabase 등이 있습니다. 이러한 cmdlet의 경우 다음 조건이 충족되면 –Credential 매개 변수를 사용할 수 있습니다.  
...
```  
 ...
 기본 인증을 사용하는 경우 사용자 이름과 암호가 암호화된 연결을 통해 전송되도록 언제나 HTTPS와 SSL을 사용해야 합니다. 자세한 내용은 [IIS 7.0에서 Secure Sockets Layer 구성](http://go.microsoft.com/fwlink/?linkID=184299) 하 고 [기본 인증 구성 (IIS 7)](http://go.microsoft.com/fwlink/?LinkId=230776)합니다.  
...
6.  두 번 클릭 **Windows 방화벽: 인바운드 원격 관리 예외 허용**를 선택 **Enabled**를 클릭 하 고 **확인**합니다.  
  
7.  두 번 클릭 **Windows 방화벽: 인바운드 파일 및 프린터 공유 예외 허용**를 선택 **Enabled**를 클릭 하 고 **확인**합니다.  
...
```  
 ...
 다음 컬렉션을 표시 합니다: 어셈블리, 데이터베이스, 역할 및 추적 합니다. 계속하여 `cd` 및 `dir`을 사용하여 각 컬렉션의 내용을 볼 수 있습니다.  
 ...
 **Analysis Services에 대 한 HTTP 연결**  
 ...
 HTTP 연결은이 문서의 지침을 사용 하 여 HTTP 액세스를 위해 서버를 구성 하는 경우에 유용 합니다. [인터넷 정보 서비스에서 Analysis Services에 HTTP 액세스 구성 &#40;IIS&#41; 8.0](instances/configure-http-access-to-analysis-services-on-iis-8-0.md)  
...
```  
 ...
 다음 컬렉션을 표시 합니다: 어셈블리, 데이터베이스, 역할 및 추적 합니다. 이 컬렉션의 내용을 볼 수 없는 경우 OLAP 가상 디렉터리의 인증 설정을 확인하십시오. 익명 액세스가 비활성화되어 있는지 확인합니다. Windows 인증을 사용하는 경우 Windows 사용자 계정에 Analysis Services 인스턴스에 대한 관리 권한이 있어야 합니다.  
