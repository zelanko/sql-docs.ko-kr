...
## <a name="in-this-article"></a>이 문서의 내용  
 ...
 [PowerShell 작업 Analysis Services](#bkmk_tasks)  
...
-   [Analysis Services 공급자 및 Cmdlet을 로드 합니다.](#bkmk_load)  
  
-   [원격 관리 사용](#bkmk_remote)  
...
###  <a name="bkmk_remote"></a>원격 관리 사용  
 원격 Analysis Services 인스턴스에서 Analysis Services PowerShell을 사용하려면 먼저 원격 관리 및 파일 공유를 사용하도록 설정해야 합니다. 다음 오류는 방화벽 구성 문제를 나타냅니다. "RPC 서버를 사용할 수 없습니다. (예외가 발생한 HRESULT: 0x800706BA)".  
...
3.  원격 서버에서 다음 서비스가 시작 되었는지 확인 합니다.  RPC (원격 프로시저 호출) 서비스, TCP/IP NetBIOS 도우미 서비스, WMI(Windows Management Instrumentation) (WMI) 서비스, Windows 원격 관리 (WS-MANAGEMENT) 서비스.  
  
4.  원격 서버에서 그룹 정책 개체 편집기 스냅인(gpedit.msc)을 시작합니다.  
  
5.  컴퓨터 구성에서 관리 템플릿, 네트워크, 네트워크 구성 요소, Windows 방화벽을 차례로 연 다음 도메인 프로파일을 엽니다.  
  
6.  Windows 방화벽을 **두 번 클릭 합니다. 인바운드 원격 관리 예외**허용에서 **사용**을 선택한 다음 **확인**을 클릭 합니다.  
  
7.  Windows 방화벽을 **두 번 클릭 합니다. 인바운드 파일 및 프린터 공유 예외**허용에서 **사용**을 선택한 다음 **확인**을 클릭 합니다.  
...
```  
 ...
 다음과 같은 컬렉션이 표시 됩니다. 어셈블리, 데이터베이스, 역할 및 추적입니다. 계속하여 `cd` 및 `dir`을 사용하여 각 컬렉션의 내용을 볼 수 있습니다.  
 ...
 **Analysis Services에 대 한 HTTP 연결**  
 ...
 HTTP 연결은이 항목의 지침을 사용 하 여 HTTP 액세스를 위해 서버를 구성한 경우에 유용 합니다. [IIS&#40;인터넷 정보 서비스&#41; 8.0에서 Analysis Services에 대한 HTTP 액세스 구성](instances/configure-http-access-to-analysis-services-on-iis-8-0.md)  
  
 의 http://localhost/olap/msmdpump.dll 서버 URL을 가정 하면 연결이 다음과 같이 표시 될 수 있습니다.  
...
```  
 ...
 다음과 같은 컬렉션이 표시 됩니다. 어셈블리, 데이터베이스, 역할 및 추적입니다. 이 컬렉션의 내용을 볼 수 없는 경우 OLAP 가상 디렉터리의 인증 설정을 확인하십시오. 익명 액세스가 비활성화되어 있는지 확인합니다. Windows 인증을 사용하는 경우 Windows 사용자 계정에 Analysis Services 인스턴스에 대한 관리 권한이 있어야 합니다.  
