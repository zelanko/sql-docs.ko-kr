---
title: 모델 유효성 검사 및 예측 (데이터 마이닝 추가 기능 Excel 용)에 대 한 모델을 사용 하 여 | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
 ...
 **정확도 차트** 마법사를 통해 예측 쿼리를 만들고 리프트 차트 또는 산 점도 차트를 만들어 데이터 마이닝 모델의 성능을 평가할 수 있습니다. 리프트 차트는 한 구조에서 거의 같은 모델을 구별하여 가장 적합한 예측을 제공하는 모델을 판별하는 데 유용합니다.  
  
 [정확도 차트 &#40;SQL Server 데이터 마이닝 추가 기능&#41;](accuracy-chart-sql-server-data-mining-add-ins.md)  
  
## <a name="classification-matrix"></a>분류표  
 **분류 행렬** 마법사 분류 모델의 성능을 평가 하는 예측 쿼리를 만들 수 있습니다. 해당 모델로 산출한 결과는 정확한 예측과 정확하지 않은 예측을 모두 요약하는 차트로 출력됩니다. 행렬은 모델이 값을 정확하게 예측한 빈도뿐만 아니라 모델이 자주 잘못 예측한 값도 표시하기 때문에 매우 유용한 도구입니다.  
  
 [분류 행렬 &#40;SQL Server 데이터 마이닝 추가 기능&#41;](classification-matrix-sql-server-data-mining-add-ins.md)  
  
## <a name="profit-chart"></a>수익 차트  
 **수익 차트** 마법사를 사용 하는 데이터 마이닝 모델을 사용 하 여 가치가 거짓 긍정 및 거짓 부 정의 비용을 평가할 수  
 ...
 **교차 유효성 검사** 마법사 하면 데이터를 분할할 접기 수를 지정할 수 있도록 하 고 이러한 교집합 영역 간의 차이 통계학적으로 설명 하는 교차 유효성 검사 보고서를 제공 합니다. 여기에서 모델이 모든 학습 데이터에 대해 잘 수행되고 있는지 또는 특정 하위 집합에 대해 비대칭인지 확인할 수 있습니다.  
 ...
 **쿼리** 마법사는 예측 쿼리를 작성 하는 데 도움이 되는 대화형 도구입니다. 쿼리는 권장 사항, 미래 예측 등을 생성하는 방법입니다.  
 ...
 에 **쿼리** 마법사는 마법사를 통해 출력할 열을 선택할 수 및 모델을 선택 하 고 단일 값 또는 테이블 또는 범위에서 다음 입력된 데이터를 제공 합니다. 쿼리에 기능을 추가하여 확률 점수 및 기타 유용한 통계를 생성할 수도 있습니다.  
  
 [쿼리 &#40;SQL Server 데이터 마이닝 추가 기능&#41;](query-sql-server-data-mining-add-ins.md)  
  
## <a name="advanced-query-editor"></a>고급 쿼리 편집기  
 **고급 쿼리 편집기** 때 도움이 되는 대화형 대화 상자 집합이 모든 종류의 만들기를 사용자 지정 쿼리 실행부터 DMX 문 작성 하 고 모델을 삭제 하는 새 모델을 학습 또는 새 데이터 집합 만들기.  
