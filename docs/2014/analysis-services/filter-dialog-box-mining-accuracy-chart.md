...
-   
 ...
 단일 테이블에 적용되는 조건 집합을 만든 후 [!INCLUDE[clickOK](../includes/clickok-md.md)] 
  **필터** 대화 상자에서 적용한 변경 내용은 부모 대화 상자인 **데이터 집합 필터** 또는 **모델 필터**의 필터 식에 자동으로 추가됩니다.  
...
|**마이닝 구조 열**|
  **데이터 집합 필터** 대화 상자의 데이터 원본에서 선택한 테이블에 포함된 열 목록에서 열을 선택하려면 클릭합니다.|  
