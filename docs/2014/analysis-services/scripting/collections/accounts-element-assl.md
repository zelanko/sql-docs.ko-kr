...
## <a name="remarks"></a>Remarks  
 차원, 해당 [형식](../properties/type-element-dimension-assl.md) 로 설정 된 *계정*, Income, Expense 등 계정 유형을 지정 하는 특성을 보유할 수 있으며 등의 차원 멤버에 의해 표현 합니다. 계정 유형을 사용 합니다 [측정값](../objects/measure-element-assl.md) 요소를 가진 [AggregationFunction](../properties/aggregatefunction-element-assl.md) 로 설정 된 *ByAccount*, 집계 함수를 사용 하는 경우를 확인 하려면 해당 차원의 멤버를 집계 합니다. `Accounts` 요소는 계정 유형 및 각 계정 유형에 사용해야 하는 집계 함수를 나타내는 `Account` 요소의 컬렉션을 보유합니다.  
