 ...
 **예측** 도구를 사용 하면 Excel 데이터 테이블 또는 다른 데이터 원본에 데이터를 기반으로 예측을 수행 하 고 필요에 따라 각 예측된 값과 관련 된 확률을 볼 수 있습니다. 예를 들어 데이터에 날짜 열 및 월의 각 날짜에 해당하는 총 판매를 표시하는 열이 포함된 경우에는 이후 날짜의 판매를 예측할 수 있습니다. 수행할 예측 수를 지정할 수도 있습니다. 예를 들어 5일 또는 30일간 예측할 수 있습니다.  
...
4.  열에 날짜, 시간 또는 기타 계열 식별자가 포함되도록 지정합니다. 옵션을 선택 하는 경우  **\<타임 스탬프 없음 >** 도구는 원본 데이터의 행 순서를 기준으로 계열을 만듭니다.  
  
5.  수행할 예측 수를 지정합니다.  
  
6.  선택적으로 알고리즘에 대한 힌트, 즉 데이터 반복 주기가 매주인지, 매월인지 또는 다른 기간인지를 제공합니다. 데이터 맞지 않습니다. 지정 된 패턴과 않거나 어떠한 패턴도 인식할 수 없는 경우 선택  **\<자동으로 검색 >** 하면이 도구는 반복 되는 기간을 찾습니다.  
...
### <a name="requirements"></a>요구 사항  
 ...
 가능한 경우 사용자 데이터에 시간 또는 날짜 계열을 포함하는 열이 있으면 좋습니다. 날짜 및 시간 데이터 대신 숫자 시리즈(1,2,3….)를 사용할 수 있습니다. 그러나 계열 열의 값은 고유해야 합니다. 오류가 발생 하는 경우는 **예측** 도구가 계열 열에서 중복 값을 검색 합니다.  
...
> [!NOTE]  
>  그래프에서 예측된 값에 대 한 시간 축에 레이블을 표시 되지 않으면 예측된 값이 포함 된 워크시트를 열고 사용는 **채우기, 연속** 는 예측을 포함 하도록 타임 스탬프 열을 확장 하는 Excel의 함수 값입니다.  
...
## <a name="related-tools"></a>관련 도구  
 ...
 두는 **예측** (Excel 용 테이블 분석 도구)의 도구와 **예측** 마법사 (Excel 용 데이터 마이닝 클라이언트)에서 사용 하 여는 [!INCLUDE[msCoName](../includes/msconame-md.md)] 시계열 알고리즘입니다.  
  
-   **예측** 도구는 자동으로 데이터에 가장 적합 한 설정을 사용 하는 알고리즘을 구성 하기 때문에 사용 하기 쉽습니다.  
  
-   **예측** Excel 매개 변수를 사용자 지정 하는 기능을 제공에 대 한 데이터 마이닝 클라이언트에서 마법사.  
  
 에 대 한 자세한 내용은 **예측** 마법사 참조 [예측 마법사 &#40;Excel 용 데이터 마이닝 추가 기능&#41;](forecast-wizard-data-mining-add-ins-for-excel.md)합니다. 예측에 사용되는 알고리즘에 대한 자세한 내용은 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 온라인 설명서의 "Microsoft 시계열 알고리즘" 항목을 참조하십시오.  
