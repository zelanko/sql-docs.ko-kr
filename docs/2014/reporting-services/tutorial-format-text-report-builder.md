...
### <a name="set-up-the-report"></a>보고서 설정  
 1. [데이터를 사용 하 여 빈 보고서를 만들 원본 및 데이터 집합](#CreateReport)  
...
##  <a name="CreateReport"></a> 데이터를 사용 하 여 빈 보고서를 만들 원본 및 데이터 집합  
...
1.  보고서 데이터 창의 **FullName** 필드를 디자인 화면으로 끌어옵니다.  
     ...
     보고서 작성기로 표시 된 식이 있는 입력란이 만들어집니다 \<Expr >.  
  
2.  **실행**을 클릭합니다.  
  
     레코드 하나만 있다는 점에 주의 **Fernando Ross**, 즉 첫 번째 레코드는 쿼리에서 사전순으로 합니다. 즉, 이 필드가 반복되어 해당 필드의 다른 레코드가 표시되지 않습니다.  
  
3.  **디자인** 을 클릭하여 디자인 뷰로 돌아갑니다.  
  
4.  식을 선택 \<Expr > 입력란에 있습니다.  
...
1.  에 **삽입** 메뉴에서 클릭 **테이블**를 클릭 하 고 **테이블 마법사**합니다.  
  
2.  에 **데이터 집합 선택** 페이지의 새 테이블 또는 행렬 마법사를 클릭 **이 보고서 또는 공유 데이터 집합에서 기존 데이터 집합을 선택**를 클릭 하 고 **TextDataset (이 보고서)** 를 클릭 하 고 **다음**합니다.  
  
3.  에 **필드 정렬** 페이지에서 합니다 **Territory**를 **LinkText**, 및 **제품** 필드를 **행 그룹**를 끌어 합니다 **Sales** 필드를 **값**를 클릭 하 고 **다음**.  
  
4.  에 **레이아웃 선택** 페이지의 선택을 취소 합니다 **그룹 확장/축소** 전체 테이블을 마우스 클릭을 볼 수 있도록 확인란 **다음**합니다.  
  
5.  에 **스타일을 선택 합니다** 페이지에서 **슬레이트**를 클릭 하 고 **마침**합니다.  
  
6.  테이블을 끌어 제목 블록 아래에 배치합니다.  
  
7.  **실행**을 클릭합니다.  
  
     테이블에 이상이 없어 보이지만 두 개의 합계 행이 있습니다. 합니다 **LinkText** 필드에는 합계 행이 필요 하지 않습니다.  
  
8.  **디자인** 을 클릭하여 디자인 뷰로 돌아갑니다.  
  
9. 포함 된 입력란을 마우스 오른쪽 단추로 클릭 `[LinkText]`, 클릭 **셀 분할**합니다.  
  
10. 아래의 빈 셀을 선택 합니다 `[LinkText]` 셀에서 SHIFT 키를 누른을 선택한 다음 오른쪽에 있는 두 셀: 합니다 **총** 셀에 **제품** 열 및 `[Sum(Sales)]` 셀에는  **Sales** 열입니다.  
  
11. 세 개의 셀 선택에서 사용 하 여이 셀 중 하나를 마우스 오른쪽 단추로 클릭 하 고 클릭 **행 삭제**합니다.  
...
3.  에 **텍스트 상자 속성** 상자를 클릭 합니다 **동작**합니다.  
  
4.  클릭 **URL로 이동**합니다.  
  
5.  에 **URL 선택** 상자를 클릭 합니다 **[URL]** 를 클릭 하 고 **확인**합니다.  
  
6.  텍스트가 다르게 표시되지 않습니다. 텍스트가 링크 텍스트처럼 표시되도록 해야 합니다.  
  
7.  `[LinkText]`을(를) 선택합니다.  
  
8.  **글꼴** 부분을 **홈** 탭을 클릭를 **Underline** 단추를 선택한 다음 옆에 드롭다운 화살표를 클릭 합니다 **색** 단추를 클릭 **파란색**합니다.  
...
6.  에 **일반적인** 페이지의 **태그 형식**, 클릭 **HTML-HTML 태그 스타일으로 해석**합니다.  
...
4.  (선택 사항) 에 **홈** 탭의 **수** 그룹에서 클릭는 **자리 표시자 스타일** 단추를 클릭 **샘플 값** 숫자는 어떻게 볼 수 포맷 합니다.  
...
## <a name="next-steps"></a>다음 단계  
 보고서 작성기에서 텍스트의 서식을 지정 하는 방법은 여러 가지 [자습서: 자유 형식 보고서 만들기 &#40;보고서 작성기&#41; ](../reporting-services/tutorial-creating-a-free-form-report-report-builder.md) 더 많은 예제가 포함 되어 있습니다.  
