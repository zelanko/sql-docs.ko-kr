 ...
 **임시 보고 사용**  
 ...
 이 옵션 설정에 따라 보고서 서버의 `EnableLoadReportDefinition` 속성이 `True` 또는 `False`로 지정됩니다. 이 옵션을 선택 취소 하면 속성 설정 됩니다 `False` 및 보고서 서버는 데이터 탐색 중 작성 된 클릭 광고 보고서를 생성 하지 것입니다. `LoadReportDefinition` 메서드에 대한 모든 호출은 차단됩니다.  
  
 이 옵션을 끄면 악의적인 사용자가 사용 하 여 보고서 서버를 오버 로드 하 여 한 서비스 거부 공격을가 하는 그에 따라 위협을 완화 `LoadReportDefinition` 요청 합니다.  
...
## <a name="see-also"></a>관련 항목  
 ...
 [자격 증명 및 보고서 데이터 원본에 대 한 연결 정보 지정] (.. /report-data/specify-credential-and-connection-information-for-report-data-sources.md [보고서 서버에서 Management Studio F1 도움말](report-server-in-management-studio-f1-help.md)  
