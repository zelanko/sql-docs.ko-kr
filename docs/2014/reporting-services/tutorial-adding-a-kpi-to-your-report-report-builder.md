...
3.  [배경색을 사용 하 여 KPI를 표시 하려면](#BackgroundColors)  
 ...
 **시작** 대화 상자에서 공유 데이터 원본을 선택 포함된 된 데이터 집합을 만들거나 테이블에 데이터를 표시 합니다.  
...
1.  **시작**을 클릭하고 **프로그램**, **Microsoft SQL Server 2012 보고서 작성기**를 차례로 가리킨 다음 **보고서 작성기**를 클릭합니다.  
    ...
    > [!NOTE]  
    >  경우는 **시작** 대화 상자가 나타나지 않으면 보고서 작성기 단추에서 클릭 **새로**합니다.  
  
2.  왼쪽 창에 **새 보고서** 가 선택되어 있는지 확인합니다.  
  
3.  오른쪽 창에서 **테이블 또는 행렬 마법사**를 클릭합니다.  
  
4.  데이터 집합 페이지 선택에서 클릭 **데이터 집합을 만들**합니다.  
...
1.  테이블을 마우스 오른쪽 단추로 클릭 두 셀 아래로는 `[Sum(Sales)]` (되는 부분합 행 하위 범주에 대 한 판매량 표시), 셀을 클릭 한 다음 **텍스트 상자 속성**합니다.  
  
2.  **채우기**, 클릭는 **fx** 단추 옆에 **채우기 색** 및 옵션에 다음 식을 입력는 **대 한 식 설정: BackgroundColor** 필드:  
...
##  <a name="Gauge"></a> 4. 계기를 사용하여 KPI 표시  
 계기는 데이터 집합의 단일 값을 표시합니다. 이 자습서에서는 가로 선형 계기를 사용합니다. 이는 이 계기가 작은 크기로 테이블 셀 안에 사용될 경우에도 모양이 읽기 쉽고 단순하기 때문입니다. 자세한 내용은 [계기&#40;보고서 작성기 및 SSRS&#41;](report-design/gauges-report-builder-and-ssrs.md)를 참조하세요.  
  
#### <a name="to-display-the-present-state-of-a-kpi-using-a-gauge"></a>계기를 사용하여 KPI의 현재 상태를 표시하려면  
  
1.  디자인 뷰로 전환합니다.  
  
2.  테이블에서의 열 머리글을 마우스 오른쪽 단추로 이전 절차에서 변경 된 셀에 대 한 가리킨 **열 삽입**, 클릭 하 고 **오른쪽**합니다. 테이블에 새 열이 추가됩니다.  
  
3.  형식 **KPI** 열 머리글에 있습니다.  
  
4.  에 **삽입** 탭에 **데이터 영역** 그룹을 클릭 합니다 **계기**, 다음 테이블 외부 디자인 화면을 클릭 합니다. **계기 유형 선택** 대화 상자가 나타납니다.  
  
5.  클릭 **선형**합니다. 첫 번째 선형 계기 유형인 **가로**을 선택 합니다.  
...
8.  에 Sales를 놓습니다는 **값** 목록입니다.  
  
     필드를 계기에 놓으면 기본 제공 Sum 함수를 사용하여 필드가 집계됩니다.  
  
9. 계기에서 포인터를 마우스 오른쪽 단추로 클릭 하 고 클릭 **포인터 속성**합니다.  
  
10. **포인터 형식**선택, **모음**합니다. 이렇게 하면 포인터가 표식에서 막대로 변경되어 테이블에 계기를 추가할 때 더 잘 보입니다.  
  
11. 클릭 **포인터 채우기**합니다. **보조 색** 선택 **노란색**합니다. 그라데이션 채우기 패턴이 흰색에서 노란색으로 변경됩니다.  
  
12. 계기 눈금을 마우스 오른쪽 단추로 클릭하고 **눈금 속성**을 클릭합니다.  
  
13. 설정의 **최대** 옵션을 25000 합니다.  
...
16. (선택 사항) 최대값 핀을 추가하여 범위 최대값을 초과하는 값이 항상 최대값 핀을 가리키도록 함으로써 오버플로를 해결합니다.  
    ...
    3.  에 **눈금 핀** 범주를 확장 하 고는 **MaximumPin** 노드.  
  
    4.  설정의 **사용** 속성을 `True`합니다. 눈금의 최대값 뒤에 핀이 나타납니다.  
  
    5.  설정 **BorderColor** 를 `Lime`합니다.  
...
5.  에 **삽입** 탭에 **데이터 영역** 그룹에서 두 번 클릭 **표시기입니다.**  
  
     **표시기 유형 선택** 대화 상자가 열립니다.  
  
6.  클릭 **셰이프**합니다. 첫 번째 셰이프 유형인 **3 색 신호등 (테두리 없음)** 을 선택 합니다.  
...
10. 값 드롭다운 목록에서 선택 **[sum (sales)]**, 있지만 다른 옵션을 변경할 수 없습니다.  
...
## <a name="next-steps"></a>다음 단계  
 보고서에 KPI 추가 자습서를 성공적으로 완료했습니다. 자세한 내용은 계기 (보고서 작성기)를 참조 하십시오. [표시기 &#40;보고서 작성기 및 SSRS&#41;](report-design/indicators-report-builder-and-ssrs.md)합니다.  
