...
1.  텍스트 편집기에서 RSReportServer.config를 엽니다.  
     ...
     파일은 * \<드라이브>:* 파일 \ Files\Microsoft SQL Server\MSRS12.에 있습니다. MSSQLSERVER\Reporting Services\reportserver  
...
-   필요에 따라 최소 권한 계정을 지정하여 보고서 서버와 다른 보안 컨텍스트에서 요청을 처리할 수 있습니다. 이 계정은 보고서 서버의 보고서 작성기 파일에 액세스하기 위한 익명 계정이 됩니다. 이 계정은 ASP.NET 작업자 프로세스에 있는 스레드의 ID를 설정합니다. 이 스레드에서 실행되는 요청은 인증 검사 없이 보고서 서버로 전달됩니다. 이 계정은 익명 액세스 및 가장이\<사용 되는 경우 ASP.NET worker 프로세스에 대 한 보안 컨텍스트를 설정 하는 데 사용 되는 인터넷 정보 서비스 (IIS)의 IUSR_ machine> 계정에 해당 합니다. 계정을 지정하려면 보고서 작성기 Web.config 파일에 계정을 추가합니다.  
