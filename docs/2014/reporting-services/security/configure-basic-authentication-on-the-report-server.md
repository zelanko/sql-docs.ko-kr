...
1.  텍스트 편집기에서 RSReportServer.config를 엽니다.  
     ...
     파일은  *\<드라이브 >:* files\microsoft SQL Server\MSRS12 합니다. MSSQLSERVER\Reporting Services\ReportServer입니다.  
...
4.  에 대 한 기존 항목을 통해 붙여 <`Authentication`> 합니다.  
     ...
     여러 인증 유형을 사용 하는 경우 추가 테이블만 `RSWindowsBasic` 요소에 대 한 항목을 삭제 하지 않고 `RSWindowsNegotiate`, `RSWindowsNTLM`, 또는 `RSWindowsKerberos`합니다.  
...
|요소|필수|유효한 값|  
|-------------|--------------|------------------|  
|LogonMethod|예<br /><br /> 값을 지정하지 않으면 3이 사용됩니다.|`2` = 일반 텍스트 암호를 인증 하는 고성능 서버를 위한 네트워크 로그온을 제공 합니다.<br /><br /> `3` = 일반 텍스트 로그온 각 HTTP 요청과 함께 전송 되는 인증 패키지에 로그온 자격 증명, 보존 하는 서버를 네트워크에 있는 다른 서버에 연결할 때 사용자를 가장할 수 있습니다. (기본값)<br /><br /> 참고: 값 0(대화형 로그온) 및 1(일괄 처리 로그온)은 [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]에서 지원되지 않습니다.|  
...
## <a name="enabling-anonymous-access-to-report-builder-application-files"></a>보고서 작성기 응용 프로그램 파일에 대한 익명 액세스 사용  
 보고서 작성기는 ClickOnce 기술을 사용하여 해당 응용 프로그램 파일을 클라이언트 컴퓨터로 다운로드하고 설치합니다. 클라이언트 컴퓨터에서 시작된 ClickOnce 응용 프로그램 실행 프로그램은 보고서 서버 컴퓨터의 추가 응용 프로그램 파일을 요청합니다. 보고서 서버가 기본 인증용으로 구성된 경우 ClickOnce 응용 프로그램 실행 프로그램은 기본 인증을 지원하지 않으므로 인증 검사에 실패하게 됩니다.  
...
-   필요에 따라 최소 권한 계정을 지정하여 보고서 서버와 다른 보안 컨텍스트에서 요청을 처리할 수 있습니다. 이 계정은 보고서 서버의 보고서 작성기 파일에 액세스하기 위한 익명 계정이 됩니다. 이 계정은 ASP.NET 작업자 프로세스에 있는 스레드의 ID를 설정합니다. 이 스레드에서 실행되는 요청은 인증 검사 없이 보고서 서버로 전달됩니다. 이 계정에서 s r _과 같습니다\<컴퓨터 > 익명 액세스 및 가장 사용할 경우를 처리 하는 계정에 인터넷 정보 서비스 (IIS), ASP.NET 작업자에 대 한 보안 컨텍스트를 설정 하는 데 사용 됩니다. 계정을 지정하려면 보고서 작성기 Web.config 파일에 계정을 추가합니다.  
...
4.  필요에 따라 익명 계정에서 보고서 작성기 요청을 처리하기 위한 Web.config 파일을 만듭니다.  
    ...
    ```  
     ...
     `Identity impersonate` 수 `True` 또는 `False`합니다.  
  
    -   로 설정 `False` ASP.NET 보안 토큰을 읽지 않도록 하려는 경우. 요청은 보고서 서버 서비스의 보안 컨텍스트에서 실행됩니다.  
...
6.  찾을 서비스 섹션의 RSReportServer.config 파일을 열어 `IsReportManagerEnabled` 그 아래의 다음 설정을 추가 합니다.  
