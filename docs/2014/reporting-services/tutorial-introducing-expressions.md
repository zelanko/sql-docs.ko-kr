...
2.  [데이터의 기본 이름 업데이트 원본 및 데이터 집합](#UpdateNames)  
  
3.  [표시 이름, 이니셜 및 성 이름](#Concatenate)  
  
4.  [이미지를 사용 하 여 성별 표시](#Gender)  
...
##  <a name="Gender"></a> 4. 이미지를 사용하여 성별 표시  
...
7.  에 **값** 목록에서 두 번 클릭 `CountryRegionID`합니다.  
...
9. 오른쪽 괄호를 삭제한 다음 **,Fields!ID.value, Fields!CountryRegion.value, "CountryRegion")** 을 입력합니다.  
     ...
     **Lookup** 함수의 구문에서는 CountryRegion 데이터 집합에서 CountryRegionID 및 ID 간의 조회를 지정합니다. 여기에서는 CountryRegion 값을 반환하며 이 값은 CountryRegion 데이터 집합에도 있습니다.  
...
35. 에 **식** 대화 상자에서 식을 만듭니다. `=Round(Fields!YTDPurchase.Value - Avg(Fields!YTDPurchase.Value, "Expressions")) >0`  
...
##  <a name="DateFormat"></a> (선택 사항) 날짜 열 서식 지정  
