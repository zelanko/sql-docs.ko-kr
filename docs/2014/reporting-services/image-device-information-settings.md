...
|`DpiX`|출력 이미지의 수평 해상도입니다. 기본값은 **96**입니다. , `BMP` `GIF`, `PNG`및 `TIFF` 출력 형식에 적용 됩니다.|  
|`DpiY`|출력 이미지의 수직 해상도입니다. 기본값은 **96**입니다. , `BMP` `GIF`, `PNG`및 `TIFF` 출력 형식에 적용 됩니다.|  
...
|**OutputFormat**|
  [!INCLUDE[ndptecgdiexpanded](../includes/ndptecgdiexpanded-md.md)]([!INCLUDE[ndptecgdi](../includes/ndptecgdi-md.md)]) 지원되는 출력 형식 중 하나: `BMP`, `EMF`, `GIF`, `JPEG`, `PNG` 또는 `TIFF`.|  
...
|**PrintDpiX**|출력 이미지의 수평 해상도입니다. 기본값은 `300`입니다. 확장 메타 파일 (`EMF`) 출력 형식에 적용 됩니다.|  
|**PrintDpiY**|출력 이미지의 수직 해상도입니다. 기본값은 `300`입니다. 확장 메타 파일 (`EMF`) 출력 형식에 적용 됩니다.|  
|`StartPage`|렌더링할 보고서의 첫 페이지입니다. 
  `0` 값은 모든 페이지가 렌더링됨을 나타냅니다. 기본값은 `1`입니다.|  
