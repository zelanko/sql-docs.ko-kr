...
> [!IMPORTANT]  
>  보고서 서버 웹 서비스와 보고서 관리자는 있지만 [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] 응용 프로그램을 실행 하지 않아서 아래는 [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] 계정. 단일 서비스 아키텍처는 동일한 보고서 서버 프로세스 ID 내에서 두 ASP.NET 애플리케이션을 모두 실행합니다. 이것은 이전 버전에서의 중요한 변경 사항으로 보고서 서버 웹 서비스 및 보고서 관리자는 IIS에서 지정된 [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] 작업자 프로세스 ID에서 실행됩니다.  
