...
-   `URLReservations` 섹션의 각 요소에 대 한 설명을 보려면 온라인 설명서의 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [rsreportserver.config 구성 파일](../report-server/rsreportserver-config-configuration-file.md) 을 참조 하십시오.  
  
-   `UrlString` 요소의 구문에 대 한 자세한 내용은 [URL 예약 구문 &#40;SSRS Configuration Manager&#41;](url-reservation-syntax-ssrs-configuration-manager.md)를 참조 하세요.  
...
#### <a name="urls-for-report-access"></a>보고서 액세스용 URL  
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 에는 보고서 링크나 첨부 파일을 보낼 때 사용할 수 있는 보고서 서버 전자 메일 배달 확장 프로그램이 포함되어 있습니다. 보고서 링크는 보고서가 배달될 때 생성됩니다. 보고서 서버 전자 메일 배달 확장 프로그램은 구성 파일의 `UrlRoot` 설정을 사용하여 링크를 만듭니다. 
  `UrlRoot`는 무인 보고서 처리를 통해 생성된 렌더링된 보고서의 링크를 확인하는 데에도 사용됩니다.  
 ...
 대부분의 경우 `UrlRoot`를 수정할 필요는 없습니다. 그러나 정규화 된 URL을 통해 보고서 서버에 액세스 하 고 호스트 헤더를 사용 하는 URL을 정규화 된 사이트 이름으로 구성 하지 않은 경우 Rsreportserver.config을 수동으로 편집 하 여 보고서를 렌더링 하는 데 사용 되는 정규화 `UrlRoot` 된 보고서 서버 url (예:)으로 설정 해야 합니다 https://www.adventure-works.com/mywebapp/reportserver).  
