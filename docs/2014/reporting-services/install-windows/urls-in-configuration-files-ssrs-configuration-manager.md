...
-   섹션에서 각 요소에 대한 설명을 보려면 온라인 도서의 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [RSReportServer 구성 파일을](../report-server/rsreportserver-config-configuration-file.md) 참조하십시오. `URLReservations`  
  
-   요소의 구문에 대한 자세한 내용은 [URL 예약 구문 &#40;SSRS 구성 관리자&#41;](url-reservation-syntax-ssrs-configuration-manager.md)참조하세요. `UrlString`  
...
#### <a name="urls-for-report-access"></a>보고서 액세스용 URL  
 ...
 대부분의 경우 `UrlRoot`를 수정할 필요는 없습니다. 그러나 보고서 서버가 정규화된 URL을 통해 액세스되고 호스트 헤더를 사용하는 URL을 완전히 정규화된 사이트 이름으로 구성하지 않은 경우 RSReportServer.config를 `UrlRoot` 수동으로 편집하여 보고서를 `https://www.adventure-works.com/mywebapp/reportserver`렌더링하는 데 사용할 정규화된 보고서 서버 URL로 설정해야 합니다.  
