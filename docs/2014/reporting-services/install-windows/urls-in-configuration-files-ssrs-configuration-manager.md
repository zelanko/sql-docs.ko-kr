...
#### <a name="urls-for-report-access"></a>보고서 액세스용 URL  
 ...
 대부분의 경우 `UrlRoot`를 수정할 필요는 없습니다. 그러나 정규화 된 URL을 통해 보고서 서버에 액세스 하 고 호스트 헤더를 사용 하는 URL을 정규화 된 사이트 이름으로 구성 하지 않은 경우 Rsreportserver.config을 수동으로 편집 하 여 보고서를 렌더링 하는 데 사용 되는 정규화 `UrlRoot` 된 보고서 서버 URL로를 설정 해야 합니다 (예: `https://www.adventure-works.com/mywebapp/reportserver`).  
