...
보고서 도구 모음  
 ...
 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]에서는 요청 시 실행되거나 보고서 실행 스냅숏에서 실행되도록 보고서를 구성할 수 있습니다. 보고서가 요청 시 실행되는 경우 보고서를 열 때마다 모든 데이터 및 보고서가 처리됩니다. 보고서 실행 스냅샷으로 실행되도록 구성된 보고서를 보는 경우 스냅샷을 만들 때 데이터가 처리됩니다.  
