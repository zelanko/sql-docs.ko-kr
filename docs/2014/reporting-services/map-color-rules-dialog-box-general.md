 ...
 **데이터 필드**  
 ...
 드롭다운 목록에서 사용할 분석 데이터 필드를 선택합니다. 공간 데이터의 원본에 따라 목록에는 공간 데이터 원본의 필드와 공간 데이터와 분석 데이터 간의 관계가 있는 보고서 데이터 집합의 필드가 표시됩니다. 이러한 관계를 만들려면 선택된 지도 계층에 대한 분석 데이터 페이지에서 데이터 옵션을 설정합니다.  
