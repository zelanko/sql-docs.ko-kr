...
## <a name="setting-delivery-options"></a>배달 옵션 설정  
 ...
 경로  
 SharePoint 웹 애플리케이션 및 사이트를 포함하는 SharePoint 라이브러리에 대한 정규화된 URL을 지정합니다. 예: <http://mySharePointWeb/MySite/MyDocLib>; 여기서 "<http://mySharePointWeb>"는 웹 응용 프로그램을 나타내고 "내 사이트"는 sharepoint 사이트 이며 "MyDocLib"는 보고서가 배달 되는 sharepoint 라이브러리입니다.  
