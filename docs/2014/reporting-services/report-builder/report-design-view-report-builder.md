...
##  <a name="Ribbon"></a> 리본  
 ...
 실행된 모드에서 리본 메뉴에 탭 하나만 **실행**합니다.  
  
### <a name="home-tab"></a>홈 탭  
 홈 탭은 보고서 내 항목의 모양을 다루는 데 많이 사용되는 명령의 모음입니다. 홈 탭에서는 실행, 글꼴, 단락, 테두리, 숫자 및 레이아웃 명령에 액세스할 수 있습니다. 탭의 항목을 클릭하면 디자인 화면의 선택된 항목이 변경됩니다. 클릭할 때 **실행**보고서가 HTML 렌더링에 게시 될 때 보고서 내용이 어떻게 표시를 볼 수 있도록 하 고 홈 탭 대신 실행 탭이 표시 됩니다. 보고서를 처음 만들면 기본적으로 홈 탭이 표시됩니다.  
...
### <a name="insert-tab"></a>삽입 탭  
 ...
 클릭 하면 **보고서 파트** 탭 삽입 시 보고서 파트 갤러리를 엽니다. 보고서 서버에 저장된 보고서 파트를 검색할 수 있습니다. 자세한 내용은 [보고서 파트&#40;보고서 작성기 및 SSRS&#41;](../report-parts-report-builder-and-ssrs.md)를 참조하세요.  
...
### <a name="run-tab"></a>실행 탭  
 클릭할 때 **실행** 홈 탭에서 HTML 뷰어에서 보고서 미리 보기는 실행 하 고 홈 탭 대신 실행 탭이 표시 됩니다.  
  
 실행 탭에는 보고서 렌더링 후 사용할 수 있는 명령 모음이 들어 있습니다. 보고서를 인쇄하고, 보고서 페이지를 탐색하고, 보고서를 다른 파일 형식으로 내보내고, 문서 구조를 보고, 매개 변수(보고서에 있는 경우)를 보고, 보고서 내에서 항목을 찾을 수 있습니다. 자세한 내용은 참조 [실행 모드에서 보고서 미리 보기](#RunMode)합니다.  
  
 보고서 디자인 뷰로 돌아가려면는 **실행** 탭을 클릭 **디자인**합니다.  
...
-   보고서 자체 페이지 설정과 같은 디자인 화면 주변의 파란색 영역을 마우스 오른쪽 단추로 클릭 하 고 클릭 하 여 속성 **보고서 속성**합니다.  
...
##  <a name="ReptDataPane"></a> The Report Data Pane  
 보고서 데이터 창에서는 보고서 레이아웃을 디자인하기 전에 보고서에 필요한 보고서 데이터와 보고서 리소스를 정의할 수 있습니다. 예를 들어 데이터 원본, 데이터 집합, 계산 필드, 보고서 매개 변수 및 이미지를 보고서 데이터 창에 추가할 수 있습니다.  
 ...
 보고서 디자인 화면에 추가할 때 일부 항목은 보고서 데이터 창에 자동으로 추가됩니다. 예를 들어 보고서 파트 갤러리에서 보고서 파트를 추가할 때 보고서 파트가 데이터 영역인 경우 데이터 집합이 보고서 데이터 창에 자동으로 추가됩니다. 자세한 내용은 [보고서 작성기의 보고서 파트 및 데이터 집합](../report-data/report-parts-and-datasets-in-report-builder.md)을 참조하세요. 또한 보고서에 이미지를 포함한 경우 보고서 데이터 창의 이미지 폴더에 이미지가 추가됩니다.  
...
##  <a name="RunMode"></a> 실행 모드에서 보고서 미리 보기  
 보고서 디자인 뷰에서는 실제 데이터로 작업하는 것이 아니라 필드 이름이나 식으로 나타나는 데이터 표현으로 작업합니다. 디자인한 보고서의 문맥에 표시되는 실제 데이터를 확인하려는 경우에는 보고서를 실행하여 보고서 레이아웃에 표시되는 기본 데이터베이스의 데이터를 미리 볼 수 있습니다. 보고서 디자인과 실행 작업 간을 전환하면서 보고서의 디자인을 조정하고 결과를 즉시 볼 수 있습니다. 보고서를 미리 보려면 클릭 **실행** 에 **뷰** 리본 메뉴에 그룹화 합니다.  
