...
3.  [보고서 매개 변수를 만들려면 쿼리 매개 변수를 추가 합니다.](#Query)  
  
4.  [보고서 매개 변수의 기본 데이터 형식 및 기타 속성 변경](#ChangeDefaultProperties)  
  
    1.  [사용 가능한 값을 제공 하는 데이터 집합을 추가 및 표시 이름](#AddDataset)  
  
    2.  [값의 드롭다운 목록을 만들려면 사용 가능한 값 지정](#AvailableValues)  
...
5.  에 **레이아웃 선택** 페이지의 **옵션**, 확인 **부분합 및 총합계 표시** 선택 합니다.  
...
3.  다음을 추가 합니다 [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE` 절을 쿼리의 마지막 줄으로:  
  
    ```  
    WHERE StoreID = (@StoreID)  
    ```  
  
     합니다 `WHERE` 쿼리 매개 변수에 의해 지정 된 상점 식별자를 검색된 된 데이터를 제한 하는 절 *@StoreID* 합니다.  
...
8.  보고서 데이터 창에서 **매개 변수** 폴더를 확장합니다.  
 ...
 이제는 라는 보고서 매개 변수가 *@StoreID* 합니다. 기본적으로 매개 변수는 데이터 형식이 **텍스트**합니다. 상점 식별자가 정수이므로 다음 절차에서 데이터 형식을 Integer로 변경합니다.  
...
1.  보고서 데이터 창의 합니다 **매개 변수** 노드를 마우스 오른쪽 단추로 클릭 *@StoreID* 를 클릭 하 고 **매개 변수 속성**합니다.  
  
2.  프롬프트에서 입력 **Store identifier?** 보고서를 실행할 때 이 텍스트가 보고서 뷰어 도구 모음에 표시됩니다.  
...
6.  보고서를 미리 봅니다. 에 대 한 프롬프트를 표시 하는 보고서 뷰어에 *@StoreID* 합니다.  
...
5.  **데이터 원본**, 드롭 다운 목록에서 첫 번째 절차에서 만든 데이터 원본을 선택 합니다.  
...
1.  보고서 데이터 창에서 매개 변수를 마우스 오른쪽 단추로 *@StoreID* 를 클릭 하 고 **매개 변수 속성**합니다.  
...
9. 보고서를 미리 봅니다.  
     ...
     보고서 뷰어 도구 모음에서 매개 변수 입력란은 이제 표시 하는 드롭다운 목록을  **\<값을 선택 >** 합니다.  
  
10. 드롭다운 목록에서 Contoso Catalog Store를 선택한 다음 클릭 **보고서 보기**합니다.  
...
3.  클릭 **기본값**를 클릭 하 고 **쿼리에서 값 가져오기**합니다.  
...
7.  보고서를 미리 봅니다.  
 ...
 에 대 한 *@StoreID* , 보고서 뷰어에 "Contoso North America Online Store" 값을 표시 합니다. 결과 데이터 집합에 대 한 집합에서 첫 번째 값인 **저장소**합니다. 보고서에는 상점 식별자 **199**에 대한 Digital Cameras의 판매 수량이 표시됩니다.  
...
3.  클릭 **기본값**, 클릭 **값을 지정**를 클릭 하 고 **추가**합니다. 새 값 행이 추가됩니다.  
...
##  <a name="NameValue"></a> 4d. 이름/값 쌍이 있는 데이터 세트에서 값 조회  
...
2.  페이지 바닥글을 마우스 오른쪽 **삽입**를 클릭 하 고 **텍스트 상자**합니다. 입력란을 타임스탬프가 있는 입력란 옆으로 끌어옵니다. 입력란의 측면 핸들을 잡고 너비를 확장합니다  
...
4.  **식**의 드롭다운 목록에서 데이터 집합 필드 StoreID를 선택합니다. 데이터 형식에 **Integer**가 표시됩니다. 식 값이 데이터 세트 필드이면 데이터 형식이 자동으로 설정됩니다.  
  
5.  **연산자**, 되어 있는지 확인 `equals` (=)가 선택 합니다.  
...
6.  변경 `equals` (=)를 `IN` 에 [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE` 쿼리의 마지막 줄에서 절:  
...
8.  보고서 뷰어 도구 모음에서 옆 **Show selections**, 클릭 `True`합니다.  
