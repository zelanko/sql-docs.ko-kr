 ...
 *DatabaseName* 이 비어 있으면 *IsRemote* 는 무시되고 데이터베이스 이름에 보고서 서버 구성 파일 값이 사용됩니다.  
 ...
 경우 *IsWindowsUser* 로 설정 되어 `true`, *UserName* 형식에 있어야 \<도메인 >\\< 사용자 이름\>합니다.  
  
 때 *IsWindowsUser* 로 설정 되어 `true`, 생성된 된 스크립트에 대 한 사용자에 대 한 로그인 권한을 부여는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], 보고서 서버 데이터베이스의 기본 데이터베이스와 권한 부여 설정는 **RSExec** 보고서 서버 데이터베이스, 보고서 서버 임시 데이터베이스, master 데이터베이스 및 MSDB 시스템 데이터베이스에는 역할입니다.  
  
 때 *IsWindowsUser* 로 설정 된 `true`, 메서드에 표준 Windows Sid를 입력 합니다. 입력된 표준 Windows SID 또는 서비스 계정 이름은 사용자 이름 문자열로 변환됩니다. 데이터베이스가 로컬 데이터베이스인 경우 계정은 계정의 지역화된 올바른 표현으로 변환되고 원격 데이터베이스인 경우 계정은 컴퓨터 계정으로 표현됩니다.  
...
|NT AUTHORITY\LocalService|로컬 서비스|오류 - 아래를 참조하십시오.|  
 ...
 경우는 `LocalService` 기본 제공 계정을 지정 하 고 보고서 서버 데이터베이스가 원격 오류가 반환 됩니다.  
...
|\<IPAddress>|180.012.345,678|  
 ...
 때 *IsWindowsUser* 로 설정 된 `true`, WMI 공급자는 계정에 대 한 SID를 가져오려는 LookupAccountName을 호출 하 고 다음에 배치 하 여 이름을 가져올 LookupAccountSID 호출에서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 스크립트입니다. 이렇게 하면 사용되는 계정 이름이 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 유효성 검사를 통과합니다.  
  
 때 *IsWindowsUser* 로 설정 되어 `false`, 스크립트 부여 생성는 **RSExec** 보고서 서버 데이터베이스, 보고서 서버 임시 데이터베이스 및 MSDB 데이터베이스에는 역할입니다.  
  
 때 *IsWindowsUser* 로 설정 된 `false`, SQL Server 사용자에 이미 있어야는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 성공적으로 실행 하려면 스크립트에 대 한 합니다.  
