...
# <a name="powershell-cmdlets-for-reporting-services-sharepoint-mode"></a>Reporting Services SharePoint 모드용 PowerShell cmdlet
  Sharepoint 모드를 [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 설치 하는 경우 sharepoint 모드에서 보고서 서버를 지원 하기 위해 PowerShell cmdlet이 설치 됩니다. cmdlet은 세 가지 범주의 기능을 포함합니다.  
...
-   [사용자 지정 기능 Cmdlet Reporting Services](#bkmk_ssrsfeatures_cmdlets)  
...
-   [자세한 샘플](#bkmk_detailedsamples)  
    ...
    -   [Reporting Services 애플리케이션 데이터베이스의 속성 가져오기 및 설정(예: 데이터베이스 시간 제한)](#bkmk_example_db_properties)  
  
    -   [Reporting services 데이터 확장 프로그램 나열-SharePoint 모드](#bkmk_example_list_data_extensions)  
  
    -   [구독 소유자 변경 및 나열](#bkmk_change_subscription_owner)  
  
##  <a name="cmdlet-summary"></a><a name="bkmk_cmdlet_sum"></a>Cmdlet 요약  

 cmdlet을 실행하려면 SharePoint 관리 셸을 열어야 합니다. Microsoft Windows에 포함된 그래픽 사용자 인터페이스 편집기인 **Windows PowerShell ISE(통합 스크립팅 환경)** 를 사용할 수도 있습니다. 자세한 내용은 [Windows Server에서 Windows PowerShell 시작](https://docs.microsoft.com/powershell/scripting/getting-started/starting-windows-powershell)를 사용할 수도 있습니다. 다음 cmdlet 요약에서 서비스 응용 프로그램 ' 데이터베이스 '에 대 한 참조는 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 서비스 응용 프로그램에서 만들고 사용 하는 모든 데이터베이스를 참조 합니다. 여기에는 구성, 경고 및 임시 데이터베이스가 포함됩니다.  
...
###  <a name="service-application-and-proxy-cmdlets"></a><a name="bkmk_serviceapp_cmdlets"></a> 서비스 애플리케이션 및 프록시 Cmdlet  
...
|Dismount-SPRSDatabase|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 서비스 애플리케이션에 대한 서비스 애플리케이션 데이터베이스를 분리합니다.|  
|Remove-SPRSDatabase|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 서비스 애플리케이션에 대한 서비스 애플리케이션 데이터베이스를 제거합니다.|  
|Set-SPRSDatabase|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 서비스 애플리케이션에 연결된 데이터베이스의 속성을 설정합니다.|  
...
|Get-SPRSDatabaseCreationScript|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 서비스 애플리케이션 데이터베이스 화면에 데이터베이스 생성 스크립트를 출력합니다. 그런 다음 SQL Server Management Studio에서 스크립트를 실행할 수 있습니다.|  
|Get-SPRSDatabase|하나 이상의 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 서비스 애플리케이션 데이터베이스를 가져옵니다. 이 명령을 사용하여 서비스 애플리케이션 데이터베이스의 ID를 가져오므로, Set-SPRSDatabase comdlet을 사용하여 속성(예: `querytimeout`)을 수정할 수 있습니다. 이 항목의 예제를 참조 하십시오. [Reporting Servicea 응용 프로그램 데이터베이스의 속성 가져오기 및 설정 (예: 데이터베이스 시간 제한](#bkmk_example_db_properties)).|  
...
###  <a name="reporting-services-custom-functionality-cmdlets"></a><a name="bkmk_ssrsfeatures_cmdlets"></a> Reporting Services 사용자 지정 기능 Cmdlet  
...
|Get-SPRSExtension|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 서비스 애플리케이션에 대해 하나 이상의 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 확장을 가져옵니다.<br /><br /> 유효한 값은 다음과 같습니다.<br /><br /> **배달**<br /><br /> **DeliveryUI**<br /><br /> **Render**<br /><br /> **Data**<br /><br /> **보안**<br /><br /> **인증**<br /><br /> **EventProcessing**<br /><br /> **ReportItems**<br /><br /> **Designer**<br /><br /> **ReportItemDesigner**<br /><br /> **ReportItemConverter**<br /><br /> **ReportDefinitionCustomization**|  
|Get-SPRSSite|"ReportingService" 기능 사용 여부에 따라 SharePoint 사이트를 가져옵니다. 기본적으로 "ReportingService" 기능이 설정되어 있는 사이트가 반환됩니다.|  
  
##  <a name="basic-samples"></a><a name="bkmk_basic_samples"></a>기본 샘플  
...
##  <a name="detailed-samples"></a><a name="bkmk_detailedsamples"></a>자세한 샘플  
 다음 샘플 외에도 [Windows powershell 스크립트](../../2014/sql-server/install/install-reporting-services-sharepoint-mode-for-sharepoint-2013.md#bkmk_full_script)항목의 "Windows powershell 스크립트" 섹션에서 1-4 단계를 참조 하세요.  
  
###  <a name="create-a-reporting-services-service-application-and-proxy"></a><a name="bkmk_example_create_service_application"></a>Reporting Services 서비스 응용 프로그램 및 프록시 만들기  
...
3.  포트 80 웹앱의 콘텐츠 데이터베이스에 대한 서비스 앱 액세스를 허용합니다. 이 스크립트는 "http://sitename" 사이트가 이미 있다고 가정 합니다.  
...
###  <a name="review-and-update-a-reporting-services-delivery-extension"></a><a name="bkmk_example_delivery_extension"></a>Reporting Services 배달 확장 프로그램 검토 및 업데이트  
...
###  <a name="get-and-set-properties-of-the-reporting-servicea-application-database-for-example-database-timeout"></a><a name="bkmk_example_db_properties"></a>Reporting Servicea 응용 프로그램 데이터베이스의 속성 가져오기 및 설정 (예: 데이터베이스 시간 제한)  
...
###  <a name="list-reporting-services-data-extensions---sharepoint-mode"></a><a name="bkmk_example_list_data_extensions"></a>Reporting services 데이터 확장 프로그램 나열-SharePoint 모드  
...
###  <a name="change-and-list-subscription-owners"></a><a name="bkmk_change_subscription_owner"></a>구독 소유자 변경 및 나열  
...
## <a name="see-also"></a>참고 항목  
 [PowerShell을 사용 하 여 Reporting Services 구독 소유자 변경 및 나열 및 구독 실행](subscriptions/manage-subscription-owners-and-run-subscription-powershell.md)   
 [검사 목록: PowerShell을 사용 하 여 SharePoint용 PowerPivot 확인](https://docs.microsoft.com/analysis-services/instances/install-windows/checklist-use-powershell-to-verify-power-pivot-for-sharepoint)   
 [CodePlex SharePoint 관리 PowerShell 스크립트](https://sharepointpsscripts.codeplex.com/)   
