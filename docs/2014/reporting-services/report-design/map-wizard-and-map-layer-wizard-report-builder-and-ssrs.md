...
-   **공간 데이터 원본입니다.** 공간 데이터를 포함하는 데이터베이스 및 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 인스턴스의 이름 또는 ESRI(Environmental Systems Research Institute, Inc.) 셰이프 파일의 이름과 같은 공간 데이터를 제공하는 원본에 대한 위치 또는 연결입니다.  
  
-   **공간 데이터.** 공간 데이터 원본에 있는 필드로, 위치를 지정하는 좌표 집합을 포함합니다.  
...
##  <a name="BackToTop"></a>지도 마법사 및 지도 계층 마법사 페이지  
...
2.  [공간 데이터 및 지도 보기 옵션을 선택](#MapView)합니다. 지도 보기와 지도 해상도를 설정하고, 공간 데이터를 보고서에 포함할지 여부를 지정하고, [!INCLUDE[msCoName](../../../includes/msconame-md.md)] Bing 지도 타일의 타일 배경을 포함할지 여부를 지정합니다.  
...
4.  **데이터 원본에 대 한 연결을 선택**합니다. 데이터 원본 연결을 선택하거나 지도에 표시할 분석 데이터가 포함된 외부 데이터 원본에 대한 연결을 만듭니다.  
...
7.  [공간 및 분석 데이터의 일치 필드를 지정](#SpecifyMatchFields)합니다. 지도 요소의 모양이 데이터에 따라 달라질 수 있도록 공간 데이터와 분석 데이터 간의 관계를 만듭니다.  
  
    -   [일치 필드란?](#MatchFields)  
  
8.  [색 테마 및 데이터 시각화를 선택](#ThemeandVisualization)합니다. 지도 배경에 대해 데이터를 시각화하는 방법을 지정하려면 지도 테마, 시각화할 필드를 지정하고 색, 크기 및/또는 표식 유형 중 변경할 항목을 지정합니다.  
 ...
 ![맨 위로 이동 링크와 함께 사용 되는 화살표 아이콘](../../2014-toc/media/uparrow16x16.gif "맨 위로 이동 링크와 함께 사용되는 화살표 아이콘") [](#BackToTop)  
...
###  <a name="GetShapefiles"></a>ESRI 셰이프 파일는 어디에서 얻을 수 있나요?  
...
###  <a name="SqlServerSpatial"></a>SQL Server 공간 쿼리 란?  
...
###  <a name="Viewport"></a>지도 보기 또는 뷰포트 란?  
...
###  <a name="Resolution"></a>지도 해상도 및 최적화 란?  
...
###  <a name="Embed"></a>포함 공간 데이터는 어떤 역할을 하나요?  
...
###  <a name="Tiles"></a>Bing 지도 타일 배경 이란?  
...
###  <a name="MapType"></a>기본 지도, 거품형 지도 및 분석 지도의 차이는 무엇 인가요?  
...
###  <a name="Diff"></a>공간 데이터와 분석 데이터의 차이는 무엇 인가요?  
...
###  <a name="Theme"></a>테마는 어떤 역할을 하나요?  
 선택하는 테마에 따라 색, 테두리 및 글꼴의 기본값이 설정됩니다. 마법사를 완료한 후 이러한 옵션을 변경할 수 있습니다.  
  
###  <a name="Legends"></a>지도 미리 보기의 범례와 눈금은 무엇 인가요?  
