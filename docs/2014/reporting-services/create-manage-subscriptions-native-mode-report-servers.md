...
## <a name="disabling-subscriptions"></a>구독 해제  
 ...
 보고서 서버에서 구독 처리를 사용 하지 않도록 설정 하려면 정책 기반 `ScheduleEventsAndReportDeliveryEnabled` 관리의 `False` [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Reporting Services 패싯에 **대 한 노출 영역 구성** 에서 속성을로 설정 하면 됩니다. 이렇게 하면 예약된 모든 작업이 실행되지 않습니다. 보고서 서버에서 처리 중인 구독만 해제할 수는 없습니다.  
