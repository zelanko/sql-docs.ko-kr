...
ms.locfileid: "36181501"
---
# <a name="enable-and-disable-client-side-printing-for-reporting-services"></a>Reporting Services에 대한 클라이언트 쪽 인쇄 기능 설정 및 해제
  [!INCLUDE[msCoName](../../includes/msconame-md.md)] ActiveX 컨트롤 **RSClientPrint**, 브라우저에 표시 되는 보고서에 대 한 클라이언트 쪽 인쇄를 제공 합니다. 이 컨트롤은 다른 인쇄 대화 상자에 공통적인 기능을 지원하는 사용자 지정 인쇄 대화 상자를 표시합니다. 인쇄 미리 보기, 특정 페이지와 범위 지정을 위한 페이지 선택, 페이지 여백 및 방향이 여기에 포함됩니다. 클라이언트 쪽 인쇄 기능은 기본적으로 설정되어 있지만 이 기능을 사용하지 않으려면 해제할 수 있습니다.  
  
> [!NOTE]  
>  ActiveX 컨트롤을 다운로드하려면 관리자 권한이 있어야 합니다.  
  
## <a name="downloading-the-activex-control"></a>AcitveX 컨트롤 다운로드  
 인쇄 기능을 사용하려는 사용자는 클라이언트 인쇄 기능을 제공하는 ActiveX 컨트롤을 다운로드하여 설치해야 합니다. 처음으로 사용자가 클릭는 **프린터** 아이콘이 보고서 도구 모음, Microsoft ActiveX 컨트롤이 컴퓨터에 다운로드 됩니다. 컨트롤이 다운로드 되는 **인쇄** 클릭할 때마다 대화 상자에 표시 됩니다는 **프린터** 아이콘입니다.  
  
 브라우저 설정에 따라 컨트롤을 설치하라는 메시지가 표시되거나 컨트롤을 설치할 수 없거나 컨트롤이 백그라운드에 이미 설치되어 있을 수 있습니다.  
  
 에 대 한 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Internet Explorer ActiveX 컨트롤 다운로드 및 설치에 영향을 주는 설정을 통해 지정 되며는 **ActiveX 컨트롤 및 플러그 인** 에서 노드는 **보안 설정** 에 대 한 페이지 웹 콘텐츠 영역입니다. 웹 영역 보안 기본 설정을 기반으로 사용자가 인쇄 컨트롤을 다운로드하고 실행할 수 있는지 여부는 다음 설정에 따라 결정됩니다.  
...
-   ActiveX 컨트롤 및 플러그 인 실행  
 ...
 사용 하려는 사용자는 **RSClientPrint** 클라이언트 쪽 인쇄를 수행 하려면 다음을 설정 해야 합니다.  
  
-   **서명 된 ActiveX 컨트롤 다운로드** 및 **ActiveX 컨트롤 스크립트 스크립팅에 안전한 것으로 표시** 설치에 필요 합니다.  
  
-   **ActiveX 컨트롤 및 플러그 인 실행** 인쇄 작업 진행에 필요 합니다.  
  
 **RSClientPrint** 에서 유효한 디지털 인증서를 포함 되어 있는 것 ActiveX 컨트롤이 서명 되어 [!INCLUDE[msCoName](../../includes/msconame-md.md)]합니다.  
  
## <a name="enabling-and-disabling-client-side-printing"></a>클라이언트 쪽 인쇄 기능 설정 및 해제  
 보고서 서버 관리자는 보고서 서버 시스템 속성을 설정 하 여 인쇄 기능을 해제할 수 **EnableClientPrinting** 를 `false`합니다. 이렇게 하면 해당 서버에서 관리하는 모든 보고서에 대한 클라이언트 쪽 인쇄 기능이 해제됩니다. 기본적으로 **EnableClientPrinting** 로 설정 된 `true`합니다. 다음과 같은 방법으로 클라이언트 쪽 인쇄 기능을 해제할 수 있습니다.  
