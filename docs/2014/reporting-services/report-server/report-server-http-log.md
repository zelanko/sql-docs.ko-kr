...
## <a name="log-file-fields"></a>로그 파일 필드  
 다음 표에서는 로그에서 사용 가능한 필드를 설명합니다.  필드 목록은 구성 가능하므로 `HTTPTraceSwitches` 구성 설정을 통해 포함할 필드를 지정할 수 있습니다. **기본** 열은를 지정 `HTTPTraceSwitches`하지 않을 경우 자동으로 로그 파일에 필드를 포함할지 여부를 지정 합니다.  
