...
-   [기본 URL을 사용하도록 보고서 관리자 구성](#ConfigureRMURL)  
     ...
     보고서 관리자는 웹 브라우저에서 사용자가 액세스하는 웹 애플리케이션입니다. 적어도 브라우저 창에서 이 애플리케이션을 여는 데 사용되는 URL은 정의해야 합니다. URL은 호스트 이름, 포트 및 가상 디렉터리로 구성됩니다. 이 URL에 대한 기본값에는 보고서 서버 웹 서비스 URL에 대해 정의된 호스트 이름 및 포트 값과 **reports** 가상 디렉터리 이름이 포함됩니다. 명명 된 인스턴스가 있는 경우 가상 디렉터리는 **reports_instance**됩니다. 여기서 **instance** 는 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 인스턴스의 이름입니다.  
...
##  <a name="configure-report-manager-to-use-the-default-url"></a><a name="ConfigureRMURL"></a>기본 URL을 사용 하도록 보고서 관리자 구성  
...
5.  [!INCLUDE[wiprlhlong](../../includes/wiprlhlong-md.md)] 또는 Windows Server 2008을 사용하는 경우 보고서 관리자를 사용하기 위해서는 추가 단계를 수행해야 할 수 있습니다. 자세한 내용은 [로컬 관리에 대해 기본 모드 보고서 서버 구성&#40;SSRS&#41;](configure-a-native-mode-report-server-for-local-administration-ssrs.md)를 참조하세요.  
  
##  <a name="configure-report-manager-to-use-a-specific-report-server-url"></a><a name="ConfigureSpecificURL"></a>특정 보고서 서버 URL을 사용 하도록 보고서 관리자 구성  
...
1.  텍스트 편집기에서 **rsreportserver.config** 파일을 엽니다. 기본적으로이 파일은 Files\Microsoft SQL Server\MSRS12.에 있습니다. \< *instancename*> \reporting services\reportserver  
...
##  <a name="configure-report-manager-to-use-a-remote-report-server"></a><a name="ConfigureRemoteRS"></a>원격 보고서 서버를 사용 하도록 보고서 관리자 구성  
...
##  <a name="customize-styles-or-application-title"></a><a name="ModifyTitle"></a>스타일 또는 응용 프로그램 제목 사용자 지정  
...
3.  보고서 관리자 URL을 입력합니다. 기본적으로\<**서버 이름**>/reports를 http://하지만 Reporting Services를 명명 된 인스턴스로 설치한\<**경우에는** 기본 URL이>\<http://**_instancename**>.  
...
##  <a name="turn-off-report-manager"></a><a name="DisableRM"></a>보고서 관리자 해제  
