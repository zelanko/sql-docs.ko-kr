...
|----------|-----------------|----------------------|  
|1|NLB 클러스터의 서버 노드에서 Reporting Services를 설치하기 전에 스케일 아웃 배포를 위한 요구 사항을 확인합니다.|[기본 모드 보고서 서버 확장 배포 구성 &#40;SSRS 구성 관리자&#41; ](../install-windows/configure-a-native-mode-report-server-scale-out-deployment.md) [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 온라인 설명서|  
...
|6|구성 `Hostname` 및 `UrlRoot` NLB 클러스터의 가상 서버 IP를 사용 합니다.|이 항목의[Hostname 및 UrlRoot 구성 방법](#SpecifyingVirtualServerName) 을 참조하십시오.|  
...
2.  보고서 관리자에 대 한 및 Web.config 파일을 열고는 <`system.web`> 섹션에 붙여 넣습니다는 <`machineKey`> 요소를 생성 합니다. 기본적으로 보고서 관리자 Web.config 파일은 \Program Files\Microsoft SQL Server\MSRS10_50.MSSQLSERVER\Reporting Services\ReportManager\Web.config에 있습니다.  
...
##  <a name="SpecifyingVirtualServerName"></a> Hostname 및 UrlRoot 구성 방법  
 ...
 구성의 `Hostname` 보고 환경에서 와일드 카드 URL 예약을 사용 하는 경우이 속성입니다. NLB 서버의 가상 서버 이름으로 사용할 `Hostname` 속성을 지정하면 보고 환경의 네트워크 트래픽이 NLB 서버로 전송됩니다. 그러면 NLB가 보고서 서버 노드에 요청을 배포합니다.  
...
2.  찾을  **\<서비스 >** 섹션을 구성 파일에 다음 정보를 추가 합니다. 대체에서 `Hostname` NLB 서버의 가상 서버 이름 사용 하 여 값:  
  
    ```  
    <Hostname>virtual_server</Hostname>  
    ```  
  
3.  찾을 `UrlRoot`합니다. 요소는 구성 파일에 지정 되지 않은 사용 되는 기본 값이 형식의 URL: http:// 또는 https://\<*computername*>/\<*reportserver*> 여기서 \< *reportserver*>는 보고서 서버 웹 서비스의 가상 디렉터리 이름입니다.  
...
##  <a name="Verify"></a> 보고서 서버 액세스 권한 확인  
 확장 배포는 가상 서버 이름을 통해 액세스할 수 있는지 확인 하십시오 (예를 들어 https://MyVirtualServerName/reportserver 및 https://MyVirtualServerName/reports)합니다.  
  
 보고서 서버 로그 파일 또는 RS 실행 로그를 검토하여 실제로 보고서를 처리하는 노드를 확인할 수 있습니다. 실행 로그 테이블에는 특정 요청을 처리한 인스턴스를 나타내는 **InstanceName** 열이 포함되어 있습니다. 자세한 내용은 참조 [Reporting Services 로그 파일 및 소스](../report-server/reporting-services-log-files-and-sources.md) 에 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 온라인 설명서.  
