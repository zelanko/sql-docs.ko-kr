...
1.  [지도 마법사에서 다각형 계층에 맵 만들기](#Map)  
  
2.  [상점 위치 표시 하려면 지도 점 계층 추가](#PointLayer)  
  
3.  [경로 표시 하려면 지도 선 계층 추가](#LineLayer)  
  
4.  [Bing Maps 타일 배경을 추가 합니다.](#TileLayer)  
  
5.  [레이어를 투명 하 게 만들기](#Transparent)  
  
6.  [판매액을 기준으로 군 색 변경](#Vary)  
  
    1.  [공간 및 분석 데이터 간의 관계를 만듭니다](#Relationship)  
  
    2.  [다각형에 대 한 색 규칙을 지정 합니다.](#ColorRules)  
...
6.  지도 갤러리 창의 확장 **States by County** 아래 **USA**를 클릭 하 고 **뉴욕**합니다.  
...
13. **데이터 필드** 드롭 다운 목록에서 #COUNTYNAME를 클릭 합니다. 마법사의 지도 미리 보기 창에 다음 항목이 표시됩니다.  
...
15. 클릭 하 여 선택 하 고 표시 된 **지도 계층 창**합니다. **지도 계층 창** 계층 유형의 다각형 계층이 하나 표시 **Embedded**합니다. 각 군은 이 계층의 포함된 지도 요소입니다.  
  
    > [!NOTE]  
    >  표시 되지 않으면는 **지도 계층** 창에서 보기의 외부에 현재 표시 될 수 있습니다. 디자인 뷰 창의 아래쪽에 있는 스크롤 막대를 사용하여 뷰를 변경합니다. 또는에서 **보기** 탭을 선택 취소 된 **속성** 또는 **보고서 데이터** 자세한 디자인 화면 영역을 제공할 수 있습니다.  
  
16. 지도 제목을 마우스 오른쪽 단추로 누른 **제목 속성**합니다.  
  
17. 제목 텍스트와 **스토어를 통해 판매**합니다.  
...
4.  에 **SQL Server 공간 데이터가 있는 데이터 집합 선택** 페이지 **SQL Server 공간 데이터가 있는 새 데이터 집합 추가**, 클릭 하 고 **다음**합니다.  
  
5.  **SQL Server 공간 데이터 원본에 대한 연결을 선택하십시오.** 페이지에서 기존 데이터 원본을 선택하거나 보고서 서버로 이동한 후 데이터 원본을 선택합니다.  
  
6.  **다음**을 클릭합니다.  
  
7.  쿼리 디자인 페이지를 클릭 **텍스트로 편집**합니다.  
...
9. 쿼리 디자이너 도구 모음에서 **실행** (**!**)을 클릭합니다.  
     ...
     결과 집합 열이 7 개 표시 됩니다: StoreKey, StoreName, SellingArea, City, County, Sales 및 SpatialLocation 합니다. 이 데이터는 소비재를 판매하는 뉴욕 주의 상점 집합을 나타냅니다. 결과 집합의 각 행에는 상점 식별자, 상점 이름, 제품 전시 구역, 상점이 있는 도시와 군, 판매량 합계, 경도와 위도로 나타낸 공간 위치 등이 포함됩니다. 전시 구역의 범위는 455제곱피트(42제곱미터)부터 1125제곱피트(105제곱미터)까지입니다.  
  
10. **다음**을 클릭합니다.  
  
     DataSet1이라는 보고서 데이터 집합이 만들어집니다. 마법사를 완료한 후 보고서 데이터를 사용하여 해당 필드 컬렉션을 볼 수 있습니다.  
  
11. 에 **공간 데이터를 선택 하 고 지도 보기 옵션** 페이지에서 **공간 필드** 은 `SpatialLocation` 하 고는 **계층 유형에** 은 **지점**. 이 페이지의 다른 기본값을 적용합니다.  
  
     지도 보기에 각 상점의 위치를 표시하는 원이 표시됩니다.  
  
12. **다음**을 클릭합니다.  
  
13. 분석 데이터에 따라 달라지는 표식을 표시하는 지도 유형을 지정합니다. 지도 시각화 선택 페이지에서 클릭 **분석 표식 지도**, 클릭 하 고 **다음**합니다.  
  
14. 에 **분석 데이터 집합 선택** 페이지 DataSet1를 클릭 합니다. 이 데이터 집합에는 새 점 계층에 표시될 분석 데이터와 공간 데이터가 모두 들어 있습니다.  
  
15. **다음**을 클릭합니다.  
  
16. 에 **색 테마 및 데이터 시각화 선택** 선택을 취소 페이지는 **표식 색을 사용 하 여 데이터를 시각화 하려면** 옵션을 선택한 다음 옵션을 선택 **표식 유형을 사용 하 여 시각화할 데이터**.  
  
17. **데이터 필드**을 선택 `[Sum(SellingArea)]` 하는 제품을 전시 하기 따로 설정한 영역의 크기에 따라 표식 유형을 변경 합니다.  
...
20. 제목 삭제 하 고 형식 **Display Area (Square Feet)** 합니다.  
  
21. [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
22. 마법사를 통해 설정된 기본값을 확인합니다. 에 **지도 계층 창**에서 점 계층을 마우스 오른쪽 단추로 클릭 한 다음 클릭 **표식 유형 규칙**합니다.  
  
     에 **일반** 탭 표식이 범례에 표시 되는 순서로 나열 됩니다. 에 **배포** 하위 범위의 수가 탭은 5입니다. 에 **범례** 탭을 범례 텍스트는 각 범위의 시작 및 끝 값을 표시 하도록 설정 되어 있습니다.  
...
5.  에 **SQL Server 공간 데이터 원본에 대 한 연결 선택**, DataSource1, 첫 번째 절차에서 만든 데이터 원본을 선택 합니다.  
...
14. **마침**을 클릭합니다.  
 ...
 지도의 공간 데이터 원본 유형이 인 새 선 계층이 표시 **DataSet**합니다. 이 예제에서 공간 데이터는 데이터 집합에서 제공되지만 분석 데이터가 선과 연결되어 있지 않습니다.  
...
4.  TileLayer1에 있는 아래쪽 화살표를 클릭 한 다음 클릭 **타일 속성이**합니다.  
  
5.  **형식**선택, **항공**합니다. 항공 보기에는 텍스트가 포함되지 않습니다.  
...
3.  PolygonLayer1에 있는 아래쪽 화살표를 클릭 한 다음 클릭 **계층 데이터**합니다. **지도 다각형 계층 속성** 대화 상자가 열립니다.  
...
2.  PolygonLayer1에 있는 아래쪽 화살표를 클릭 한 다음 클릭 **다각형 색 규칙**합니다. **지도 색 규칙 속성** 대화 상자가 열립니다. 색 규칙 옵션 **색상표를 사용하여 데이터 시각화** 가 선택되어 있습니다. 이 옵션은 마법사에서 설정된 것입니다.  
...
###  <a name="ColorScale"></a> 6 c 합니다. 색 눈금의 데이터 형식을 통화로 지정  
...
2.  뷰포트 외부의 지도 마우스 오른쪽 단추로 누른 **범례 추가**합니다. 새 범례가 지도의 기본 위치에 추가됩니다.  
  
3.  범례를 마우스 오른쪽 단추로 누른 **범례 속성**합니다.  
  
4.  **위치 옵션**, 범례를 뷰포트에 상대적인 위치를 지정 하는 위치를 클릭 합니다. 디자인 화면의 지도가 변경되어 선택 항목의 효과를 표시합니다.  
  
5.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
6.  클릭 **제목** 범례의 범례 제목을 선택 합니다.  
  
7.  클릭 **제목** 다시 하 텍스트 삽입 모드로 전환 합니다. 대체 **제목** 여 **Sales (Thousands)**, 다음 텍스트 외부를 클릭 합니다.  
...
2.  PolygonLayer1에 있는 아래쪽 화살표를 클릭 한 다음 클릭 **다각형 색 규칙**합니다. **지도 색 규칙 속성** 대화 상자가 열립니다.  
  
3.  **범례**를 클릭합니다.  
  
4.  **색 눈금 옵션**일반 **색 눈금에 표시**합니다.  
  
5.  **범례 옵션**, 드롭 다운 목록에서 범례 2를 선택 합니다. 범례 텍스트 옵션이 나타납니다. 기본적으로 범례 텍스트는 일반 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 형식 문자열을 사용하여 형식이 지정됩니다. N0의 0은 소수 자릿수가 표시되지 않도록 지정합니다.  
  
6.  **범례 텍스트**, 다음 형식을 사용 하 여 통화를 소수 자릿수 없이 지정: `#FROMVALUE {C0} - #TOVALUE {C0}`  
...
5.  채우기 스타일 인지 확인 **실선입니다.** 인지 확인합니다. 그라데이션과 패턴이 모든 색에 적용됩니다.  
  
6.  **색**, 아래쪽 화살표를 클릭 한 다음 클릭 **연한 강철색**합니다.  
  
7.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
8.  보고서를 미리 봅니다.  
  
 연결된 데이터가 없는 군은 파란색으로 표시되고 에 연결 된 분석 데이터가 있는 군만 표시는 **빨간색** 통해 **녹색** 지정한 색 규칙의 색상입니다.  
  
##  <a name="CustomPoint"></a> 7. 사용자 지정 점 추가  
 아직 빌드되지 않은 새 저장소를 나타내기 위한 지정 하 고 사용 된 **압정** 표식 유형입니다.  
  
#### <a name="to-add-a-custom-point"></a>사용자 지정 점을 추가하려면  
  
1.  디자인 뷰로 전환합니다.  
  
2.  지도를 두 번 클릭하여 **지도 계층** 창을 표시합니다. 도구 모음에서 클릭 **계층 추가**, 클릭 하 고 **점 계층**합니다.  
  
     새 점 계층이 지도에 추가됩니다. 기본적으로 점 계층의 공간 데이터 형식은 **포함**입니다.  
  
3.  PointLayer2에 있는 아래쪽 화살표를 클릭 한 다음 클릭 **지점 추가**합니다.  
...
9. 에 대 한 **표식 유형을**선택, **별**합니다.  
  
10. [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
11. 보고서를 미리 봅니다.  
  
 추가한 새 점이로 표시 한 **별**합니다.  
...
## <a name="next-steps"></a>다음 단계  
 ...
 자세한 내용은 참조 [지도 &#40;보고서 작성기 및 SSRS&#41; ](report-design/maps-report-builder-and-ssrs.md) 및 블로그 항목 [지도 조정의 공간 데이터를 SQL Server Reporting Services](http://go.microsoft.com/fwlink/?LinkId=152771) blogs.msdn.com 합니다.  
  
 다른 자습서를 참조 하십시오. [자습서 &#40;보고서 작성기&#41;](report-builder-tutorials.md)합니다.  
