...
14. 에 **분석 데이터 집합 선택** 페이지에서 DataSet1을 클릭 합니다. 이 데이터 세트에는 새 점 계층에 표시될 분석 데이터와 공간 데이터가 모두 들어 있습니다.  
...
14. **마침**을 클릭합니다.  
 ...
 지도 공간 데이터 원본 유형 사용 하 여 새 선 계층이 표시 됩니다 **데이터 집합**합니다. 이 예제에서 공간 데이터는 데이터 세트에서 제공되지만 분석 데이터가 선과 연결되어 있지 않습니다.  
