---
title: (중급 데이터 마이닝 자습서) 콜 센터 구조에 로지스틱 회귀 모델 추가 | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
2.  데이터 마이닝 디자이너에서 클릭 하 고 **마이닝 모델** 탭 합니다.  
  
3.  클릭 **관련된 마이닝 모델을 만들**합니다.  
  
4.  에 **새 마이닝 모델** 대화 상자에 대 한 **모델 이름**, 형식 `Call Center - LR`합니다.  에 대 한 **알고리즘 이름**선택, **Microsoft 로지스틱 회귀**합니다.  
  
5.  **확인**을 클릭합니다.  
  
     새 마이닝 모델에 표시 됩니다는 **마이닝 모델** 탭 합니다.  
...
2.  ServiceGrade 및 Level Two Operators에 값을 변경 **Predict**합니다.  
     ...
     이러한 열은 입력 및 예측에 모두 사용됩니다. 기본적으로 모델을 만드는 두 개의 별도 동일한 데이터에서: 전화 상담원 수를 예측 하 고 다른 하나는 서비스 등급을 예측 합니다.  
...
2.  HOLDOUT_SEED 매개 변수에 대 한 행에서 빈 셀을 클릭 **값**, 유형과 `1`합니다. **확인**을 클릭합니다.  
