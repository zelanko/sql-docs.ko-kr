...
2.  데이터 마이닝 디자이너에서 클릭 하 고 **마이닝 모델 예측** 탭 합니다.  
  
3.  에 **마이닝 모델** 창에서 클릭 **모델 선택**합니다.  
...
6.  **마이닝 모델** 메뉴에서 **단일 쿼리**를 선택합니다.  
     ...
     **단일 쿼리 입력** 마이닝 모델의 열에 매핑된 열이 있는 대화 상자가 나타납니다.  
  
7.  에 **단일 쿼리 입력** 대화 상자에서 교대조에 대 한 행을 클릭 한 다음 선택 *자정*합니다.  
  
8.  Lvl 2 Operators 및 유형에 대 한 행을 클릭 합니다. `6`합니다.  
  
9. 아래에 있는 절반는 **마이닝 모델 예측** 탭에서 표의 첫 번째 행을 클릭 합니다.  
  
10. 에 **소스** 열에서 아래쪽 화살표를 클릭 하 고 선택 **예측 함수**합니다. 에 **필드** 열에서 선택 **PredictHistogram**합니다.  
  
     자동으로이 예측 함수와 함께 사용할 수 있는 인수 목록에 표시 된 **조건/인수** 상자입니다.  
  
11. 열 목록에서 ServiceGrade 열을 끌어는 **마이닝 모델** 창에는 **조건/인수** 상자입니다.  
  
     열 이름이 자동으로 인수로 삽입됩니다. 이 입력란으로 끌어다 놓을 임의의 예측 가능한 특성 열을 선택할 수 있습니다.  
  
12. 단추 클릭 **스위치를 쿼리 결과 보기로**, 예측 쿼리 작성기의 위쪽 모퉁이에 있습니다.  
...
## <a name="creating-predictions-by-using-a-logistic-regression-model"></a>로지스틱 회귀 모델을 사용하여 예측 만들기  
 비즈니스 문제 관련 특성을 이미 알고 있는 경우 로지스틱 회귀 모델을 사용하여 일부 특성을 변경했을 때의 효과를 예측할 수 있습니다. 로지스틱 회귀는 일반적으로 독립 변수의 변경 내용을 기반으로 예측을 만드는 사용 되는 통계 방법: 예를 들어, 해당 하는 데 재무 상태 평가에 고객 인구 통계를 기반으로 고객 동작을 예측 합니다.  
...
### <a name="generating-data-used-for-bulk-prediction"></a>대량 예측에 사용할 데이터 생성  
 입력된 데이터를 제공 하는 방법은 여러 가지가: 스프레드시트에서 직원 수준을 가져올 하 고 다음 달에 대 한 서비스 품질을 예측 하는 모델을 통해 해당 데이터를 실행할 수 예를 들어 있습니다.  
...
2.  데이터 원본 뷰 마법사에서 선택 [!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)] 클릭 한 다음 확인 하 고 데이터 원본으로 **다음**합니다.  
  
3.  에 **테이블 및 뷰 선택** 페이지 **다음** 어떤 테이블도 선택 하지 않고 있습니다.  
  
4.  에 **마법사 완료** 페이지에서 이름을 입력 `Shifts`합니다.  
...
8.  디자인 창에서 Shifts for Call Center 테이블을 마우스 오른쪽 단추로 누르고 **데이터 탐색** T-SQL 쿼리에 의해 반환 된 데이터를 미리 봅니다.  
  
9. 탭을 마우스 오른쪽 단추로 클릭 **Shifts.dsv (디자인)** 클릭 하 고 **저장** 새 데이터 원본 뷰 정의 저장 합니다.  
...
1.  데이터 마이닝 디자이너에서 클릭 하 고 **마이닝 모델 예측** 탭 합니다.  
  
2.  에 **마이닝 모델** 창에서 클릭 **모델 선택**, Call Center-LR 사용 가능한 모델 목록에서 선택 합니다.  
  
3.  **마이닝 모델** 메뉴 옵션의 선택을 취소 **단일 쿼리**합니다. 단일 쿼리 입력이 손실된다는 경고가 표시됩니다. **확인**을 클릭합니다.  
  
     **단일 쿼리 입력** 대화 상자 아래 템플릿으로 바뀝니다는 **입력 테이블 선택** 대화 상자.  
  
4.  **사례 테이블 선택**을 클릭합니다.  
  
5.  에 **테이블 선택** 대화 상자, 데이터 원본 목록에서 selectShifts 합니다. 에 **테이블/뷰 이름** 목록에서 Shifts for Call Center (있습니다 수 자동으로 선택) 하 고 클릭 한 다음 선택 **확인 합니다.**  
  
     **마이닝 모델 예측** 디자인 화면이 입력 데이터에서 및 모델에서의 열 이름과 데이터 형식을 기반으로 생성 된 매핑을 표시 하도록 업데이트 됩니다.  
  
6.  조인 선을 마우스 오른쪽 단추로 클릭 한 다음 선택 **연결 수정**합니다.  
...
8.  호출의 경우 선택 옆에 있는 빈 셀을 클릭 **Shifts for Call Center.AvgCalls**합니다. 클릭 하 고 **확인**합니다.  
  
##### <a name="to-create-the-predictions-for-each-shift"></a>각 교대조에 대한 예측을 만들려면  
  
1.  아래쪽에 있는 표에서 절반는 **예측 쿼리 작성기**에서 빈 셀을 클릭 **소스** 한 다음 Shifts for Call Center를 선택 합니다.  
  
2.  아래의 빈 셀에서 **필드**, Shift를 선택 합니다.  
  
3.  표의 다음 빈 줄을 클릭하고 위에 설명한 절차를 반복하여 WageType에 대한 다른 행을 추가합니다.  
  
4.  표에서 다음 빈 줄을 클릭합니다. 에 **소스** 열에서 선택 **예측 함수**합니다. 에 **필드** 열에서 선택 **Predict**합니다.  
  
5.  ServiceGrade 열을 끌어에서 **마이닝 모델** 창에서 표 아래로 하며이 **조건/인수** 셀입니다. 에 **별칭** 필드를 입력 **Predicted Service Grade**합니다.  
  
6.  표에서 다음 빈 줄을 클릭합니다. 에 **소스** 열에서 선택 **예측 함수**합니다. 에 **필드** 열에서 선택 **PredictProbability**합니다.  
  
7.  ServiceGrade 열을 끌어에서 **마이닝 모델** 창에서 표 아래로 하며이 **조건/인수** 셀입니다. 에 **별칭** 필드를 입력 **확률**합니다.  
...
|교대조, 임금 및 요일|현재 평균 응답 시간으로 예측되는 서비스 품질|응답 시간을 90퍼센트 줄여 예측되는 서비스 품질|예상 응답 시간 80% 감소를 사용 하 여 서비스 품질|  
|--------------------------|------------------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|  
