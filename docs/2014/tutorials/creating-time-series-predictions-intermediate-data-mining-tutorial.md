...
2.  에 **마이닝 모델 선택** 대화 상자를 Forecasting 구조를 확장, 선택는 **Forecasting** 클릭 하 고 목록에서 모델 **확인**합니다.  
...
4.  에 **소스** 열에 있는 표에서 **마이닝 모델 예측** 탭, 첫 번째 빈 행의 셀을 클릭 한 다음 선택 **Forecasting 마이닝 모델**합니다.  
  
5.  에 **필드** 열에서 선택 **Model Region**합니다.  
  
     이렇게 하면 예측이 적용되는 모델 및 지역 조합을 나타내기 위해 예측 쿼리에 계열 식별자가 추가됩니다.  
  
6.  다음 빈 행을 클릭는 **소스** 열을 선택한 후 **예측 함수**합니다.  
  
7.  에 **필드** 열에서 선택 **PredictTimeSeries**합니다.  
  
    > [!NOTE]  
    >  `Predict` 함수도 시계열 모델에 사용할 수 있습니다. 그러나 기본적으로 Predict 함수는 각 계열에 대해 하나의 예측만 만듭니다. 따라서 여러 예측 단계를 지정 하려면 사용 해야는 **PredictTimeSeries** 함수입니다.  
  
8.  에 **마이닝 모델** 창에서 마이닝 모델 열 선택 **양입니다.** 금액을 끌어는 **조건/인수** 상자에 **PredictTimeSeries** 이전에 추가한 함수.  
  
9. 클릭는 **조건/인수** 쉼표와 입력 **5**, 필드 이름입니다.  
...
11. 다음 빈 행을 클릭는 **소스** 열을 선택한 후 **예측 함수** 다시 합니다.  
  
12. 에 **필드** 열에서 선택 **PredictTimeSeries**합니다.  
  
13. 에 **마이닝 모델** 창에서 Quantity 열을 선택한 다음으로 끌어 놓습니다는 **조건/인수** 두 번째에 대 한 상자 **PredictTimeSeries** 함수입니다.  
  
14. 클릭는 **조건/인수** 쉼표와 입력 **5**, 필드 이름입니다.  
...
|$TIME|Amount|  
|-----------|------------|  
|2008 년 7 월 25 일|99978.00|  
|2008 년 8 월 25 일|145575.07|  
|9/25/2008|116835.19|  
|2008 년 10 월 25 일|116537.38|  
...
|$TIME|Amount|  
|-----------|------------|  
|2008 년 7 월 25 일|348533.93|  
|2008 년 8 월 25 일|340097.98|  
|9/25/2008|257986.19|  
|2008 년 10 월 25 일|374658.24|  
...
2.  에 **데이터 마이닝 쿼리 결과 저장** 대화 상자에 대 한 **데이터 소스**선택, [!INCLUDE[ssSampleDBDWobject](../includes/sssampledbdwobject-md.md)]합니다. 데이터를 다른 관계형 데이터베이스에 저장하려는 경우 데이터 원본을 만들 수도 있습니다.  
  
3.  에 **테이블 이름** 와 같은 새 임시 테이블 이름, 유형, 열 **Test Predictions**합니다.  
