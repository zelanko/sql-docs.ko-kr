...
# <a name="creating-a-neural-network-structure-and-model-intermediate-data-mining-tutorial"></a>신경망 구조 및 모델 만들기(중급 데이터 마이닝 자습서)
 ...
 신경망은 매우 유연하고 많은 입력 및 출력 조합을 분석할 수 있기 때문에 최상의 결과를 얻기 위해 여러 가지 데이터 처리 방법을 시험해야 합니다. 예를 들어, 서비스 품질에 대 한 수치 목표 되는 방식을 사용자 지정할 수도 *범주화*, 나 그룹화 된 특정 비즈니스 요구 사항을 대상으로 합니다. 이렇게 하려면 숫자 데이터를 다른 방식으로 그룹화하는 새 열을 마이닝 구조에 추가한 다음 새 열을 사용하는 모델을 만듭니다. 이러한 마이닝 모델을 사용하여 탐색을 수행합니다.  
 ...
 **단계**  
 ...
 [모델을 비교할 수 있도록 예측 가능한 열에 대 한 별칭 만들기](#bkmk_Alias2)  
...
4.  에 **데이터 마이닝 구조 만들기** 페이지에서 옵션 **마이닝 모델이 포함 된 마이닝 구조를 만들기** 을 선택 합니다.  
...
7.  아래 **사용 가능한 데이터 원본 뷰**선택, `Call Center`를 클릭 하 고 **다음**합니다.  
  
8.  에 **테이블 유형 지정** 선택 페이지는 **대/소문자** 옆에 확인란은 **FactCallCenter** 테이블입니다. 에 대해 아무 것도 선택 하지 않으면 **DimDate**합니다. **다음**을 클릭합니다.  
  
9. 에 **학습 데이터 지정** 페이지에서 **키** 열 옆에 **FactCallCenterID 합니다.**  
  
10. 선택 된 `Predict` 및 **입력** 확인란 합니다.  
  
11. 선택 된 **키**, **입력**, 및 `Predict` 다음 표에 나와 있는 것 처럼 확인란:  
...
12. 에 **지정 열 내용 및 데이터 형식** 페이지, 모눈 열, 콘텐츠 형식 및 데이터 형식이 다음 표에 나와 있는 것 처럼이 포함 되어 있는지 확인 한 다음 클릭 **다음**합니다.  
...
13. 에 **테스트 설정 만들기** 페이지, 옵션, 텍스트 상자의 선택을 취소 **테스트용 데이터 비율**합니다. **다음**을 클릭합니다.  
  
14. 에 **마법사 완료** 페이지에 대 한는 **마이닝 구조 이름**, 형식 `Call Center`합니다.  
  
15. 에 대 한는 **마이닝 모델 이름**, 형식 `Call Center Default NN`, 클릭 하 고 **마침**합니다.  
  
     **드릴스루 허용** 상자 사용할 수 없습니다. 신경망 모델을 사용 하 여 데이터를 드릴스루할 수 없습니다.  
  
16. 솔루션 탐색기에서 마우스 오른쪽 단추로 클릭 방금 데이터 마이닝 구조의 이름, 만든 선택 **프로세스**합니다.  
...
## <a name="use-discretization-to-bin-the-target-column"></a>분할을 사용하여 대상 열 범주화  
 ...
 ![서비스 등급 값의 분포](../../2014/tutorials/media/skt-service-grade-valuesc.gif "서비스 등급 값 분포")  
  
 따라서 모델을 처리할 때 출력이 예상과 다르게 그룹화될 수 있습니다. 예를 들어 최적 값 그룹을 식별 하려면 클러스터링을 사용 하는 경우 알고리즘의에서 값을 0.0748051948 ServiceGrade이 샘플과 같이 범위로: 0.0748051948-0.09716216215와 합니다. 이 그룹화가 수학적으로 정확하기는 하지만 비즈니스 사용자에게는 이러한 범위가 의미가 없을 수 있습니다.  
...
> [!NOTE]  
>  이러한 통계는 모든 데이터를 처리한 후 모델의 한계 통계 노드에서 가져올 수 있습니다. 한계 통계 노드에 대 한 자세한 내용은 참조 하십시오. [마이닝 모델 콘텐츠 신경망 모델에 대 한 &#40;Analysis Services-데이터 마이닝&#41;](../../2014/analysis-services/data-mining/mining-model-content-for-neural-network-models-analysis-services-data-mining.md)합니다.  
...
-   **Equal areas에의 한 범주화**  
 ...
 범주화 옵션에 대 한 자세한 내용은 참조 [분할 방법 &#40;데이터 마이닝&#41;](../../2014/analysis-services/data-mining/discretization-methods-data-mining.md)합니다.  
  
 또는 숫자 값을 사용 하는 대신 추가할 수 있습니다와 같은 미리 정의 된 대상 범위로 서비스 등급을 분류 하는 별도 파생된 열 **최상의** (ServiceGrade \<= 0.05),  **허용 가능한** (0.10 > ServiceGrade > 0.05), 및 **저하** (ServiceGrade > = 0.10).  
  
###  <a name="bkmk_newColumn"></a> 열의 복사본을 만들고 분할 메서드 변경  
 대상 특성인 ServiceGrade 포함 된 마이닝 열의 복사본을 확인 하 고 숫자의 그룹화 방식을 변경 합니다. 예측 가능한 특성을 비롯하여 임의의 열의 여러 복사본을 마이닝 구조에서 만들 수 있습니다.  
  
 이 자습서에서는 Equal Areas 분할 메서드를 사용하고 4개의 버킷을 지정합니다. 이 메서드에서 생성되는 그룹화는 비즈니스 사용자가 관심을 가지는 대상 값에 매우 근접해 있습니다.  
  
####  <a name="bkmk_ColumnCopy"></a> 마이닝 구조에 열의 사용자 지정 된 복사본을 만들려면  
  
1.  솔루션 탐색기에서 앞에서 만든 마이닝 구조를 두 번 클릭합니다.  
  
2.  마이닝 구조 탭에서를 클릭 **마이닝 구조 열 추가**합니다.  
...
4.  ServiceGrade 1을 마우스 오른쪽 단추로 클릭 하 고 선택 **속성**합니다.  
  
5.  에 **속성** 창 찾습니다는 **이름** 속성 하도록 이름을 변경 하 고 **Service Grade Binned** 합니다.  
  
6.  관련된 모든 마이닝 모델 열의 이름을 동일하게 변경할 것인지 묻는 대화 상자가 나타납니다. **아니요**를 클릭합니다.  
  
7.  에 **속성** 창에서 섹션을 찾습니다 **데이터 형식** 필요한 경우이 확장 합니다.  
...
9. 데이터 마이닝 디자이너에서 클릭 하 고 **마이닝 모델** 탭 합니다.  
...
1.  솔루션 탐색기에서 마우스 오른쪽 단추로 클릭 하는 마이닝 구조를 생성 하 고 선택 **열려**합니다.  
  
2.  **마이닝 모델** 탭을 클릭합니다.  
  
3.  클릭 **관련된 마이닝 모델을 만들**합니다.  
  
4.  에 **새 마이닝 모델** 대화 상자에 대 한 **모델 이름**, 형식 `Call Center Binned NN`합니다. 에 **알고리즘 이름** 드롭다운 목록에서 선택 **Microsoft 신경망**합니다.  
...
##  <a name="bkmk_Alias2"></a> 대상 열의 별칭 만들기  
...
1.  에 **마이닝 모델** 탭의 **구조**, ServiceGrade Binned를 선택 합니다.  
     ...
     **속성** ScalarMiningStructure 열 개체의 속성 창에 표시 됩니다.  
  
2.  ServiceGrade Binned NN 마이닝 모델의 열 아래에서 ServiceGrade Binned 열에 해당하는 셀을 클릭합니다.  
  
     참고 하 여 **속성** MiningModelColumn 개체에 대 한 속성 창에 표시 됩니다.  
  
3.  찾을 **이름** 속성 값을 변경 하 고 `ServiceGrade`합니다.  
  
4.  찾을 **설명** 속성 유형과 **임시 열 별칭**합니다.  
  
     **속성** 창에서 다음 정보를 포함 해야 합니다.  
...
5.  아무 곳 이나 클릭는 **마이닝 모델** 탭 합니다.  
  
     표의 업데이트 되어 새 임시 열 별칭인 표시 `ServiceGrade`, 열 사용법 옆에 있는 합니다. 마이닝 구조와 두 마이닝 모델이 포함된 표는 다음과 같습니다.  
...
1.  에 **마이닝 모델** 탭 명명 된 Call Center-LR을 모델에 대 한 열을 마우스 오른쪽 단추로 클릭 하 고 선택 **알고리즘 매개 변수 설정**합니다.  
  
2.  HOLDOUT_SEED 매개 변수에 대 한 행에서 빈 셀을 클릭 **값**, 유형과 `1`합니다. **확인**을 클릭합니다. 구조와 연관된 각 모델에 대해 이 단계를 반복합니다.  
  
    > [!NOTE]  
    >  모든 관련 모델에 대해 동일한 초기값을 사용하는 경우에는 초기값으로 선택한 값이 중요하지 않습니다.  
  
3.  에 **마이닝 모델** 메뉴 선택 **마이닝 구조 및 모든 모델 처리**합니다. 클릭 **예** 서버에 업데이트 된 데이터 마이닝 프로젝트를 배포 해야 합니다.  
  
4.  에 **마이닝 모델 처리** 대화 상자를 클릭 **실행**합니다.  
  
5.  클릭 **닫습니다** 를 닫으려면는 **처리 진행률** 대화 상자를 닫은 다음 **닫습니다** 에서 다시는 **마이닝 모델 처리** 대화 상자.  
