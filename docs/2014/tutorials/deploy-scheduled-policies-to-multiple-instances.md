...
-   예약된 된 정책을 배포 하려는 서버 중 하나에 등록 된 서버에 등록 되어야 합니다는 **로컬 서버 그룹** 또는 **중앙 관리 서버** 노드. 자세한 내용은 다음 항목을 참조하십시오.  
...
1.  이전 작업에서 예약 된 정책의 구성한 서버에서 **관리**를 확장 하 고 **정책 관리**, 클릭 하 고 **정책**합니다.  
  
2.  **보기** 메뉴에서 **개체 탐색기 정보**를 클릭합니다.  
  
3.  에 **개체 탐색기 정보** 창 모두에서 예약 된 최선의 구현 방법 정책을 등록 된 서버를 통해 다른 서버에 배포 하려는 선택 합니다.  
  
    > [!NOTE]  
    >  클릭할 수는 **범주** 머리글을 범주별으로 정책을 정렬할 합니다.  
  
4.  선택 영역을 마우스 오른쪽 단추로 누른 **정책 내보내기**합니다.  
  
5.  내보낼 여러 정책을 선택 하는 경우는 **폴더 찾아보기** 대화 상자에서 대상 폴더를 선택 하거나 새 폴더를 만듭니다. 이 단원에 대 한 경로와 새 폴더를 만든 **C:\Scheduled_BP_Policies**, 클릭 하 고 **확인**합니다.  
  
     내보낼 정책을 하나만 선택 하는 경우는 **정책 내보내기** 대화 상자에서 새 폴더의 경로를 만들 **C:\Scheduled_BP_Policies**, 클릭 **열려**, 클릭 하 고 **저장**합니다.  
...
2.  확장 **데이터베이스 엔진**를 확장 한 다음 **로컬 서버 그룹** 또는 **중앙 관리 서버**를, 정책을 배포 하려는 노드를 마우스 오른쪽 단추로 클릭 한 다음 클릭 **정책 가져오기**합니다.  
  
    > [!NOTE]  
    >  마우스 오른쪽 단추로 **로컬 서버 그룹** 또는 중앙 관리 서버 자체를 정책이 모든 관리 서버에 배포 됩니다. 특정 서버 그룹을 마우스 오른쪽 단추로 클릭하면 해당 그룹의 서버에만 정책이 배포됩니다. 등록된 특정 서버를 마우스 오른쪽 단추로 클릭하면 해당 서버에만 정책이 배포됩니다.  
  
3.  옆에 **가져올 파일을**, 줄임표 단추를 클릭 (**...** ).  
  
4.  에 **정책 선택** 대화 상자에서 예약된 된 정책을 저장 한 폴더 위치를 찾습니다. 이 예제에 대 한 위치를 찾아 **C:\Scheduled_BP_Policies**합니다.  
  
5.  대상 인스턴스를 가져오고 클릭 한 다음 원하는 정책을 선택 **열려**합니다.  
  
6.  에 **가져오기** 대화 상자는 **정책 상태** 목록에서 원하는 정책 상태를 선택 합니다. 가져올 때 정책 상태를 유지하거나 정책을 설정 또는 해제하도록 선택할 수 있습니다. 정책은 예약 실행하도록 설정해야 합니다.  
  
7.  클릭 **확인** 을 모든 대상 인스턴스로 정책 가져오기.  
  
    > [!NOTE]  
    >  오류가 발생 하는 경우는 **가져오기** 대화 상자가 사라지지 않습니다. 클릭는 **로그** 페이지 여 메시지를 검토 합니다. 클릭 **취소** 대화 상자를 닫습니다.  
  
8.  대상 인스턴스의 정책을 보려면, 인스턴스에 연결할를 열고 개체 탐색기, **관리**을 펼친 다음 **정책**합니다. 가져온된 정책에 따라 표시 됩니다는 **정책** 노드. 각 정책을 두 번 클릭하면 일정을 보거나 설정을 변경할 수 있습니다.  
...
## <a name="next"></a>다음  
 이 자습서를 마칩니다. 시작 부분으로 돌아가려면 참조 [자습서: 최선의 구현 방법 정책 기반 관리를 여 평가](../../2014/tutorials/tutorial-evaluating-best-practices-by-using-policy-based-management.md)합니다.  
