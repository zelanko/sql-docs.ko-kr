---
title: 시계열 예측 (중급 데이터 마이닝 자습서) 대체 데이터를 사용 하 여 | Microsoft Docs
ms.custom: ''
ms.date: 04/27/2017
...
## <a name="performing-cross-prediction-with-a-time-series-model"></a>시계열 모델에 교차 예측 수행  
 다른 계열의 추세를 예측하기 위해 계열 하나의 데이터를 사용하는 프로세스를 교차 예측이라고 합니다. 대부분의 시나리오에서 교차 예측을 사용할 수 있습니다: 예를 들어 텔레비전 sales 전반적인 경제 활동의 좋은 지표가 된다고을 일반 경제 데이터에 텔레비전 sales에 학습 된 모델을 적용을 결정할 수 있습니다.  
  
 교차 예측 함수에 대 한 인수 내의 매개 변수 REPLACE_MODEL_CASES를 사용 하 여 수행 하면 SQL Server 데이터 마이닝에 [PredictTimeSeries &#40;DMX&#41;](/sql/dmx/predicttimeseries-dmx)합니다.  
...
1.  **솔루션 탐색기**를 마우스 오른쪽 단추로 클릭 **Mining Structures**를 선택한 후 **새 마이닝 구조** 데이터 마이닝 마법사를 시작 합니다.  
...
1.  모델이 아직 열려 있지 않은 경우 AllRegions 구조를 두 번 클릭 하 고 데이터 마이닝 디자이너에서 클릭 된 **마이닝 모델 예측** 탭 합니다.  
  
2.  에 **마이닝 모델** 창, 모델이 이미 AllRegions를 선택 해야 합니다. 선택 하지 않으면 클릭 **모델 선택**, 한 다음 AllRegions 모델을 선택 합니다.  
  
3.  에 **입력 테이블 선택** 창에서 클릭 **사례 테이블 선택**합니다.  
  
4.  에 **테이블 선택** 대화 상자에서 변경 된 데이터 원본을 T1000 Pacific Region 및 클릭 **확인**합니다.  
  
5.  마이닝 모델 및 입력된 데이터 간의 조인 선을 마우스 오른쪽 단추로 클릭 하 고 선택 **연결 수정**합니다. 데이터 원본 뷰에 있는 데이터를 다음과 같이 모델에 매핑합니다.  
  
    1.  마이닝 모델의 ReportingDate 열 입력된 데이터의 ReportingDate 열에 매핑되어 있는지 확인 합니다.  
  
    2.  에 **매핑 수정** 에서 대화 상자의 AvgQty, 모델 열에 대 한 행에서 클릭 **테이블 열** 한 다음 T1000 Pacific.Quantity를 선택 합니다. **확인**을 클릭합니다.  
  
         이 단계에서는 모델에서 평균 수량 예측을 위해 만든 열을 판매 수량에 대한 T1000 계열의 실제 데이터로 매핑합니다.  
  
    3.  모든 입력된 열에는 모델의 열 영역에에서 매핑되지 않습니다.  
...
6.  이제 예측 쿼리를 작성합니다.  
    ...
    1.  표에서 첫 번째 빈 행을 아래에서 클릭 **소스**, 한 다음 AllRegions 마이닝 모델을 선택 합니다.  
  
    2.  에 대 한 **필드**, 지역을 선택 합니다.  
  
    3.  에 대 한 **별칭**, 형식 **모델 사용**합니다.  
  
7.  다음으로 예측이 계열용임을 알 수 있도록 다른 레이블을 결과에 추가합니다.  
  
    1.  빈 행을 클릭 하 고 **소스**을 선택 **사용자 지정 식**합니다.  
...
8.  이제 교차 예측 함수를 설정합니다.  
    ...
    2.  에 **필드** 열에서 선택 **PredictTimeSeries**합니다.  
  
    3.  에 대 한 **별칭**, 형식 **Predicted Values**합니다.  
  
    4.  AvgQty 필드를 끌어에서 **마이닝 모델** 창에는 **조건/인수** 끌어서 놓기 작업을 사용 하 여 열입니다.  
  
    5.  에 **조건/인수** 열, 필드 이름 뒤에 오는 다음 텍스트를 입력 합니다. `,5, REPLACE_MODEL_CASES`  
  
         전체 텍스트는 **조건/인수** 입력란 다음과 같아야 합니다. `[AllRegions].[AvgQty],5,REPLACE_MODEL_CASES`  
  
9. 클릭 **결과**합니다.  
  
## <a name="creating-the-cross-prediction-query-in-dmx"></a>DMX에서 교차 예측 쿼리 만들기  
 교차 예측을 함께 사용 문제를 발견할 수: 즉, 북미 지역의 T1000 제품 모델과 같이 다른 데이터 계열에 일반 모델을 적용할 만들어야 합니다 각 계열에 대해 서로 다른 쿼리는 각 매핑할 수 있도록의 설정 모델에 넣습니다.  
