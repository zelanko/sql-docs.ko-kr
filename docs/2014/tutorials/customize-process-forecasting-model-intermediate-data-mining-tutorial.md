---
title: 사용자 지정 및 예측 (중급 데이터 마이닝 자습서) 모델은 처리 | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
2.  에 대 한 두 개의 다른 중요 한 매개 변수는 Microsoft 시계열 알고리즘에 대 한에 대해 설명 합니다: 예측에 사용 되는 방식을 제어할 수 있습니다, FORECAST_METHOD 및 수 있는 PREDICTION_SMOOTHING 장기 및 단기 보호의 혼합을 사용자 지정 예측 합니다.  
 ...
 *PERIODICITY_HINT* 매개 변수 데이터에서 참조 되는 추가 기간에 대 한 정보를 알고리즘에 제공 합니다. 기본적으로 시계열 모델은 자동으로 데이터에서 패턴을 감지하려고 합니다. 그러나 예측 시간 주기를 이미 알고 있는 경우 주기성 힌트를 제공하여 잠재적으로 모델의 정확도를 개선할 수 있습니다. 반면 잘못된 주기성 힌트를 제공한 경우 정확도를 떨어뜨릴 수 있으므로, 사용해야 할 값에 확신이 없는 경우 기본값을 사용하는 것이 최선입니다.  
  
 예를 들어, 이 모델에 사용된 뷰에서는 [!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)]로부터 판매 데이터를 월별로 집계합니다. 따라서 모델에 사용된 각 시간 조각은 한 달을 나타내며 또한 모든 예측은 개월 수 기준입니다. 1 년에는 12 개월이 있으므로 예상는 매출 패턴 반복 1 년 단위로 설정 합니다는 *PERIODICITY_HINT* 매개 변수를 `12`에 12 시간 조각 (개월) 하나를 구성 합니다. 판매 주기를 완료 합니다.  
  
 **예측 메서드**  
  
 *FORECAST_METHOD* 매개 변수는 시계열 알고리즘이 단기 또는 장기 예측에 최적화 되었는지 여부를 제어 합니다. 기본적으로는 *FORECAST_METHOD* 매개 변수 즉, 두 개의 서로 다른 알고리즘이 단기 및 장기 예측에 대 한 좋은 결과 제공 하도록 균등 혼합를 MIXED로 설정 됩니다.  
...
2.  `PERIODICITY_HINT` 의 행은 **알고리즘 매개 변수** 대화 상자에서 클릭는 **값** 열을 입력 한 다음 `{12}`, 중괄호 포함 합니다.  
  
     기본적으로 알고리즘에서도 값 {1}을 추가합니다.  
  
3.  에 `FORECAST_METHOD` 행에서 되어 있는지 확인는 **값** 텍스트 상자는 비어 있거나 설정에 `MIXED`합니다. 다른 값을 입력 하는 경우 입력 `MIXED` 기본 값으로 다시 매개 변수를 변경 합니다.  
  
4.  **PREDICTION_SMOOTHING** 행에서 확인 하는 **값** 입력란이 비어 있거나 설정 되어 0.5로 합니다. 다른 값을 입력 하는 경우 클릭 **값** 유형과 `0.5` 기본 값으로 다시 매개 변수를 변경 합니다.  
...
## <a name="handling-missing-data-optional"></a>누락된 데이터 처리(선택 사항)  
 ...
 "오류 (데이터 마이닝): 동기화 되지 않은 시작으로 타임 스탬프가 계열 \<계열 이름 >, 마이닝 모델의 \<모델 이름 >. 모든 시계열은 같은 시간 표식에서 끝나야 하며 임의의 누락 데이터 요소가 있으면 안 됩니다. MISSING_VALUE_SUBSTITUTION 매개 변수를 Previous 또는 숫자 상수로 설정하면 누락 데이터 요소가 자동으로 패치됩니다."  
...
1.  에 **마이닝 모델** 탭을 마우스 오른쪽 단추로 클릭는 **Forecasting** 열 및 선택 **알고리즘 매개 변수 설정**합니다.  
  
2.  에 **알고리즘 매개 변수** 대화 상자는 **MISSING_VALUE_SUBSTITUTION** 행을 클릭는 **값** 열 및 형식을 `Mean`합니다.  
...
1.  에 **마이닝 모델** 의 메뉴 [!INCLUDE[ssBIDevStudio](../includes/ssbidevstudio-md.md)]선택, **마이닝 구조 및 모든 모델 처리**합니다.  
  
2.  프로젝트를 빌드하여을 묻는 경고에서 클릭 **예**합니다.  
  
3.  에 **마이닝 구조 처리-예측** 대화 상자를 클릭 **실행**합니다.  
  
     **처리 진행률** 대화 상자가 열리고 모델 처리에 대 한 정보를 표시 합니다. 모델 처리는 시간이 걸릴 수 있습니다.  
  
4.  처리가 완료 되 면 클릭 **닫기** 종료 하 고 **처리 진행률** 대화 상자.  
  
5.  클릭 **닫기** 를 닫습니다는 **마이닝 구조 처리-예측** 대화 상자.  
