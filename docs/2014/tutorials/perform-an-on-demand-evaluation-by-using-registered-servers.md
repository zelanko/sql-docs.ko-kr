---
title: 등록 된 서버를 사용 하 여 요청 시 평가 수행 합니다. | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
3.  다음 중 하나를 수행합니다.  
    ...
    -   특정 서버 또는 서버 그룹에 대해 정책을 평가 수행 하려면 확장 **로컬 서버 그룹** 또는 중앙 관리 서버 이름, 서버 또는에 대해 정책을 평가 하 고 클릭 하려는 서버 그룹을 마우스 오른쪽 단추로 클릭 **정책을 평가**합니다.  
  
4.  에 **정책 평가** 옆에 대화 상자는 **소스** 상자에서 줄임표를 클릭 (**...** ) 단추입니다.  
  
5.  에 **원본 선택** 대화 상자에서 하나를 선택할 수 **파일** 또는 **서버** 를 평가할 정책 파일의 원본으로 합니다. 클릭 하면 **서버**, 로컬 또는 원격 서버에서 정책 기반 관리로 이전에 가져온 모든 최선의 구현 방법 정책의 요청 시 평가 수행할 수 있습니다. 이 자습서에서는 클릭 **파일**, 한 다음 평가할 개별 정책 파일을 선택 합니다. 이렇게 하려면 다음 단계를 수행합니다.  
  
    1.  클릭 **파일**합니다.  
  
    2.  옆에 **파일**, 줄임표 (**...** ) 단추입니다.  
  
    3.  하나 이상의.xml 정책 파일을 평가 하 고 클릭 한 다음 선택 **열려**합니다.  
  
         선택한 파일의 목록에 표시 된 **파일** 상자입니다.  
  
    4.  에 **원본 선택** 대화 상자를 클릭 **확인**합니다.  
  
    5.  경우는 **로드 정책** 대화 상자가 나타나면 클릭 **닫기**합니다.  
  
     선택한 정책에 나열 되는 **정책 선택** 페이지. 정책 옆의 경고 아이콘은 정책에 스크립트가 포함되어 있다는 것을 나타냅니다.  
  
6.  클릭 **평가** 정책을 평가 하 합니다.  
  
7.  일부 정책 실패의 경우 정책 기반 관리를 사용하면 대상에서 정책 준수를 즉시 강제할 수 있습니다. 이러한 실패의 경우 실패한 정책 옆에 확인란이 나타납니다. 확인란에 표시 확인란을 선택 하거나 실패 한 정책이 있는 행을 클릭는 **Target details** 는 평가 실패 한 대상 인스턴스 옆에 있는 창입니다. 확인란을 선택 하는 경우는 **적용** 버튼을 사용할 수 있습니다. 클릭할 때 **적용**, 선택한 대상 인스턴스에서 비호환 설정이 자동 업데이트 됩니다.  
...
8.  정책에 대 한 자세한 결과 보려면에서 정책을 클릭는 **결과** 테이블입니다. **Target details** 테이블 각 인스턴스에 대 한 세부 정보를 표시 합니다.  
  
## <a name="next-lesson"></a>다음 단원  
 [2단원: 일정에 따라 최선의 구현 방법 정책 평가](../../2014/tutorials/lesson-2-evaluate-best-practices-policies-on-a-scheduled-basis.md)  
  
## <a name="see-also"></a>관련 항목  
 [모니터링 하 고 정책 기반 관리를 사용 하 여 모범 사례 적용](../relational-databases/policy-based-management/monitor-and-enforce-best-practices-by-using-policy-based-management.md)   
