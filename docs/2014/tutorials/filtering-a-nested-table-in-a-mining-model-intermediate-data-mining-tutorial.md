...
# <a name="filtering-a-nested-table-in-a-mining-model-intermediate-data-mining-tutorial"></a>마이닝 모델의 중첩 테이블 필터링(중급 데이터 마이닝 자습서)
 ...
 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]에서는 마이닝 모델에 사용되는 데이터를 필터링하는 기능을 제공합니다. 이 기능은 새 데이터 원본 뷰를 서로 다른 데이터를 사용 하도록 설정 해야 하기 때문에 유용 합니다. 기본 데이터 마이닝 자습서에서는 사례 테이블에 조건을 적용하여 플랫 테이블의 데이터를 필터링하는 방법을 배웠습니다. 이 태스크에서는 중첩 테이블에 적용되는 필터를 만듭니다.  
...
1.  에 **마이닝 모델** 탭 [!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)]를 마우스 오른쪽 단추로 클릭는 `Association` 모델을 마우스 선택 **새 마이닝 모델**합니다.  
  
2.  에 대 한 **모델 이름**, 형식 `Association Filtered`합니다. 에 대 한 **알고리즘 이름**선택, **Microsoft 연결 규칙**합니다. **확인**을 클릭합니다.  
  
3.  Association Filtered 모델에 대 한 열에서 IncomeGroup 행을 클릭 하 고 값에서 변경 **무시** 를 **입력**합니다.  
...
1.  에 **마이닝 모델** 탭에서 모델 Association Filtered를 마우스 오른쪽 단추로 클릭 하 고 선택 **모델 필터 설정**합니다.  
  
2.  **모델 필터** 대화 상자의 **마이닝 구조 열** 입력란에서 표의 첫 행을 클릭합니다.  
  
3.  에 **마이닝 구조 열** 텍스트 상자에서 IncomeGroup 선택 합니다.  
  
     입력란 왼쪽에 있는 아이콘이 변경되어 선택한 항목이 열임을 나타냅니다.  
  
4.  클릭는 **연산자** 텍스트 상자와 선택은 **=** 목록에서 연산자.  
  
5.  클릭는 **값** 텍스트 상자 `High` 상자에서입니다.  
  
6.  표에서 다음 행을 클릭합니다.  
  
7.  클릭는 **및/또는** 표 형태 창 및 선택의 다음 행의 입력란 **또는**합니다.  
  
8.  에 **마이닝 구조 열** 텍스트 상자에서 IncomeGroup 선택 합니다. 에 **값** 텍스트 상자에서 `Moderate`합니다.  
  
     이전에 만든 필터 조건이 자동으로 추가 됩니다는 **식** 텍스트 상자 및 다음과 같이 나타납니다.  
  
     `[IncomeGroup] = 'High' OR [IncomeGroup] = 'Moderate'`  
  
9. 기본적으로 연산자를 그대로 두고 표에서 다음 행을 클릭 **AND**합니다.  
  
10. 에 대 한 **연산자**, 기본값을 그대로 두고 **Contains**합니다. 클릭는 **값** 입력란.  
  
11. 에 **필터** 대화 상자의 첫 번째 행에서 **마이닝 구조 열**선택, `Model`합니다.  
  
12. 에 대 한 **연산자**선택, **IS NOT NULL**합니다. 유지 된 **값** 입력란을 비워 합니다. **확인**을 클릭합니다.  
  
     필터 조건에는 **식** 텍스트 상자에는 **모델 필터** 대화 상자의 중첩된 테이블에 새 조건을 포함 하도록 자동으로 업데이트 됩니다. 전체 식은  
...
1.  에 **마이닝 모델** 탭을 마우스 오른쪽 단추로 클릭는 `Association Filtered` 모델을 마우스 선택 **속성**합니다.  
  
2.  변경 된 **AllowDrillThrough** 속성을 **True**합니다.  
  
3.  마우스 오른쪽 단추로 클릭는 `Association Filtered` 마이닝 모델을 마우스 선택 **프로세스 모델**합니다.  
  
4.  클릭 **예** 새 모델을 배포 하는 오류 메시지에는 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 데이터베이스입니다.  
  
5.  에 **마이닝 구조 처리** 대화 상자를 클릭 **실행**합니다.  
  
6.  처리가 완료 되 면 클릭 **닫기** 종료 하는 **처리 진행률** 대화 상자와 클릭 **닫기** 를 닫습니다는 **마이닝 구조 처리**  대화 상자.  
...
```  
 ...
 클릭할 수 있는 **쿼리 편집** 에 필터 식을 수동으로 변경 합니다. 그러나 필터 식의 임의 부분을 수동으로 변경하면 표가 비활성화되어 텍스트 편집 모드에서만 필터 식 작업을 수행할 수 있습니다. 표 편집 모드를 복원하려면 해당 필터 식을 지우고 다시 시작해야 합니다.  
