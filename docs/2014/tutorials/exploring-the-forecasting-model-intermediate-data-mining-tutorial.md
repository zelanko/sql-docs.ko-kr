...
ms.locfileid: "36313041"
---
# <a name="exploring-the-forecasting-model-intermediate-data-mining-tutorial"></a>예측 모델 탐색(중급 데이터 마이닝 자습서)
  사용 하 여 결과 탐색할 수 예측 마이닝 모델을 작성 했으므로 했으므로 **마이닝 모델 뷰어** 데이터 마이닝 디자이너의 탭 합니다. [!INCLUDE[msCoName](../includes/msconame-md.md)] 에 두 개의 탭이 시계열 뷰어: **차트** 및 **모델**합니다.  
 ...
 **차트** 탭은 [!INCLUDE[msCoName](../includes/msconame-md.md)] 시계열 뷰어를 그래픽으로 보여 주는 하면 기록 데이터 및 예측을 포함 하 여 계열의 각 합니다. 시계열 그래프의 각 선은 제품, 지역 및 예측 가능한 특성의 고유한 조합을 나타냅니다.  
...
1.  클릭는 **차트** 탭은 **마이닝 모델 뷰어** 보이지 않으면 탭 합니다.  
...
1.  에 대 한 값을 바꾸려고 **예측 단계** 에서 점차적으로 증가 **5** 를 **10**, 다음 다시 **6**합니다.  
...
3.  X축 눈금을 확인합니다. 기록 데이터 및 예측 데이터의 변화는 모두 항상 백분율로 표시되지만 실제 값은 그래프 상의 모든 값에 맞춰 자동으로 조정됩니다. 따라서 모델을 비교할 때 시각적인 것에만 의존하지 않도록 주의해야 합니다. 정확한 값 또는 백분율 증가 예측 값 점선 또는 실선 위에 마우스를 놓으면 누르거나 줄에서 값을 볼 수는 **마이닝 범례**합니다.  
  
     **팁**: 경우는 **마이닝 범례** 표시 되지 않으면 전환할 **모델** 볼 모든 노드를 마우스 오른쪽 단추로 클릭 하 고 선택 **범례 표시**합니다.  
 ...
 **모델** 탭은 [!INCLUDE[msCoName](../includes/msconame-md.md)] 시계열 뷰어 데이터 마이닝 디자이너에서 트리 그래프 형식의 예측 모델을 볼 수 있습니다.  
 ...
 제품 라인, 지역 및 판매 메트릭을에서 계열을 선택 하 여 보려는의 조합을 선택할 수 있습니다는 **트리** 드롭다운 목록에는 **모델** 탭 합니다.  
...
-   시계열의 트리 그래프에 여러 분기가 있는 경우 검색된 시계열이 너무 복잡하여 단일 수식으로 나타낼 수 없음을 의미합니다. 여러 분기가, 트리를 발생 시킨 조건으로 레이블이 지정 된 각 분기 트리 그래프 포함 될 수 있습니다는 대신, *분할*합니다. 트리가 분할될 때 각 분기는 서로 다른 시간 세그먼트를 나타내며 내부의 추세는 단일 수식으로 표현될 수 있습니다.  
  
     예를 들어 차트 그래프를 확인 하 고 9 월에서 어느 및 연말 휴가 시작 하는 판매량에서 급등 전환할 수 있습니다는 **모델** 여 추세가 변경 된 정확한 날짜 표시 되도록 보기. "이전 년 9 월" 및 "9 월 이후"를 나타내는 트리의 분기는 서로 다른 수식을 포함 됩니다: 분할 까지의 판매 추세를 수학적으로 설명 하는 하나의 수식 및 9 월에 대 한 판매 추세를 설명 하는 다른 수식 연말 휴가 기간 동안의 합니다.  
  
#### <a name="to-explore-the-decision-tree-for-a-time-series-model"></a>시계열 모델의 의사 결정 트리를 탐색하려면  
  
1.  에 **트리** 목록에서 **모델** 뷰어의 탭은 **T1000 Europe: 양** 계열입니다.  
  
     레이블이 지정 된 노드를 클릭 **모든**합니다.  
  
     에 대 한 프로그램 **모든** 전체 계열의 사례 수와 같은 정보를 포함 하는 노드를 표시 된 도구 설명 및에서 파생 된 시계열 수식이 데이터를 분석 합니다.  
  
2.  경우는 **마이닝 범례** 표시 되지 않는 노드를 마우스 오른쪽 단추로 클릭 하 고 선택 **범례 표시**합니다.  
  
     **마이닝 범례** 뷰어의 동일한 정보 도구 설명에 있는 제공 합니다. 독립 변수 중 불연속인 변수가 있는 경우 노드의 변수 분포를 보여 주는 히스토그램도 표시됩니다.  
  
3.  이제 표시할 다른 시계열을 선택합니다. 사용 하 여는 **트리** 목록에서 **모델** 뷰어의 탭은 **M200 North America: Amount** 계열입니다.  
  
     이제 트리 그래프에 포함 되어 있는 **모든** 노드와 두 개의 자식 노드가 있습니다. 자식 노드의 레이블을 확인하여 추세 선의 어느 지점이 변경되었는지 알 수 있습니다.  
  
     각 자식 노드에 설명에 대 한는 **마이닝 범례** 사례의 수는 트리의 각 분기에 포함 되어 있습니다.  
  
 다음 목록에서는 트리 뷰어의 일부 추가 기능을 설명합니다.  
  
-   사용 하 여 차트에 표시 된 변수를 변경할 수는 **배경** 제어 합니다. 기본적으로 짙을 수록 노드에 사례를 추가로 때문에 포함 값 **배경** 로 설정 된 **채우기**합니다. 노드에 있는 사례 수는 방금를 노드 위에 마우스를 놓으면을 보려면 확인 표시 되 면 또는 노드를 클릭 하 고에서 개수를 확인 하는 도구 설명의 **노드 범례** 창.  
...
##  <a name="bkmk_Content"></a> (선택 사항) 일반 콘텐츠 트리 뷰어  
 시계열에 대 한 사용자 지정 뷰어 외에도 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 제공는 **MicrosoftGeneric 콘텐츠 트리 뷰어** 모든 데이터 마이닝 모델을 사용 합니다. 이 뷰어는 다음과 같은 이점을 제공합니다.  
...
-   **일반 콘텐츠 트리 뷰어**: 탐색 하 고 모든 데이터 계열을 사용 된 모델에 한 번에 볼 수 있습니다 및 혼합 만든 경우 모델을 두는 ARIMA 및 ARTXP 트리가 같은 그래프에 표시 됩니다.  
...
1.  에 **마이닝 모델 뷰어** 탭에서 **Microsoft 일반 콘텐츠 트리 뷰어** 에서 **뷰어** 드롭 다운 목록입니다.  
  
2.  에 **노드 캡션** 창에서 최상위 (All) 노드를 클릭 합니다.  
  
3.  에 **노드 자세히 보기** 창에서 ATTRIBUTE_NAME에 대 한 값을 확인 합니다.  
  
     이 값은 이 노드에 포함된 제품 및 지역의 결합 또는 계열을 알려 줍니다. AdventureWorks 예제에서 최상위 노드는 M200 Europe 계열에 대한 것입니다.  
  
4.  에 **노드 캡션** 창에서 자식 노드가 있는 첫 번째 노드를 찾습니다.  
...
7.  에 **노드 자세히 보기** 창에서 ATTRIBUTE_NAME에 대 한 값을 확인 합니다.  
