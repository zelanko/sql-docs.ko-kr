 ...
 *데이터베이스 메일 프로필* 은 데이터베이스 메일 계정 컬렉션입니다. *데이터베이스 메일 계정은* 에서 전자 메일 메시지를 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] SMTP 서버로 보내기 위해 사용 하는 정보를 포함 합니다. [!INCLUDE[ssMDScfgmgr](../includes/ssmdscfgmgr-md.md)]에서 프로필 및 계정을 만들면 계정이 프로필에 자동으로 추가되고 해당 계정 정보가 전자 메일을 보내는 데 사용됩니다.  
