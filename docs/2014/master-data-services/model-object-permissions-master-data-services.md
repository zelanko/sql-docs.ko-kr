...
# <a name="model-object-permissions-master-data-services"></a>모델 개체 권한(Master Data Services)
 ...
 이외의 다른 기능 영역에 대 한 사용자 액세스를 부여 하려면 **탐색기**, 사용자는 모델 개체 사용 권한 할당 작업도 모델 관리자 여야 합니다. 자세한 내용은 [관리자&#40;Master Data Services&#41;](administrators-master-data-services.md)를 참조하세요.  
 ...
 할당할 수 **읽기 전용**, **업데이트**, 또는 **거부** 모델 개체 사용 권한. **모델** 탭에서 사용 권한을 할당하지 않은 경우 사용자는 [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)]에서 모델이나 데이터를 볼 수 없습니다.  
  
## <a name="best-practice"></a>최선의 구현 방법  
 일반적으로 요구 사항을 배정 해야 **업데이트** 모델 개체에 대 한 사용 권한을 다음 그 아래 개체에 권한을 명시적으로 할당 합니다. 그 아래 개체에 대한 사용 권한을 할당하지 않는 경우 사용 권한이 상속되고 사용자는 관리자가 됩니다.  
