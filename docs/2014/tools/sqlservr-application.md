 ...
 **-e** _error_log_path_  
 오류 로그 파일의 정규화된 경로를 나타냅니다. 지정 하지 않은 경우 기본 위치는 * \<드라이브>*:: server\mssql\log\errorlog이 고, 명명 된 인스턴스에 대 한 * \<드라이브>*: Server\MSSQL $*instance_name*\Log\Errorlog입니다. **-e** 와 *error_log_path*사이에 공백이 없어야 합니다.  
