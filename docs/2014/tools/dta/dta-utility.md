...
```  
 ...
 XML 입력 파일을 사용 하는 경우 요소 아래에 있는 요소를 사용 하 여 **dta** 가 연결 하는 첫 번째 데이터베이스를 지정할 수 있습니다 `DatabaseToConnect` `TuningOptions` . 자세한 내용은 [Database Engine Tuning Advisor](../../relational-databases/performance/database-engine-tuning-advisor.md)을 참조하세요.  
 ...
 **dta** 가 권장 구성에 포함해야 할 실제 디자인 구조 유형을 지정합니다. 다음 표에서는 이 인수에 지정할 수 있는 값을 나열하고 설명합니다. 값을 지정 하지 않으면 **dta** 는 기본 **-fa**를 사용 `IDX` 합니다.  
