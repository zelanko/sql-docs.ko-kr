...
|**데이터 형식 및 길이**|없음|  
|**기본값**|없음|  
|**발생 빈도**|다른 작업 유형이 지정되지 않은 경우 한 번만 지정해야 합니다. 지정 해야 합니다는 `EventString`, `File`, 또는 `Database` 자식 요소는 `Workload` 부모 하지만 한 유형만 사용할 수 있습니다. 예를 들어 `Database` 요소로 작업을 지정할 경우 동일한 XML 입력 파일에서 `File` 요소로 작업을 지정할 수 없습니다.|  
...
## <a name="example"></a>예제  
 이 사용 예 `Database` 요소에 코드 예제를 참조 하세요 [Workload 요소 &#40;DTA&#41;](workload-element-dta.md)합니다.  
