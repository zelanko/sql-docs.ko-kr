 ...
 **-OutPutFormat** { **Text** | **XML** }  
 유틸리티 출력 형식이 텍스트 문자열 (**text**) 또는 직렬화 된 export-clixml 형식 (XML)이 되도록 지정 합니다.**** `sqlps`  
  
 **-InPutFormat** { **Text** | **XML** }  
 유틸리티에 대 한 입력 형식이 텍스트 문자열 (**text**) 또는 직렬화 된 export-clixml 형식 (XML)이 되도록 지정 합니다.**** `sqlps`  
  
 **-Command**  
 
  `sqlps` 유틸리티에 대한 명령이 실행되도록 지정합니다. 유틸리티 `sqlps` 는 **-noexit** 가 지정 되지 않은 경우 명령을 실행 한 다음 종료 됩니다. **-Command**뒤에는 다른 스위치를 지정하지 마세요. 이 경우 스위치가 명령 매개 변수로 읽힙니다.  
  
 **-**  
 **-Command-** 유틸리티에서 `sqlps` 표준 입력의 입력을 읽도록 지정 합니다.  
  
 *script_block* [ **-args**_argument_array_ ]  
 실행할 PowerShell 명령 블록을 지정합니다. 명령 블록은 중괄호 {}로 묶어야 합니다. 유틸리티를 **PowerShell** 또는 다른 `sqlps` 유틸리티 세션 `sqlps` 에서 호출 하는 경우에만 *Script_block* 지정할 수 있습니다. *argument_array* 는 *script_block*의 PowerShell 명령에 대한 인수를 포함하는 PowerShell 변수 배열입니다.  
...
## <a name="remarks"></a>설명  
 유틸리티 `sqlps` 는 powershell 환경 (powershell .exe)을 시작 하 고 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] powershell 모듈을 로드 합니다. 또한 라는 `sqlps`모듈은 이러한 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] PowerShell 스냅인을 로드 하 고 등록 합니다.  
