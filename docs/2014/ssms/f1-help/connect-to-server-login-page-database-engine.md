...
# <a name="connect-to-server-login-page-database-engine"></a>서버에 연결(로그인 페이지) 데이터베이스 엔진
  에 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]연결할 때이 탭을 사용 하 여 옵션을 확인 하거나 지정할 수 있습니다.  
 ...
 **인증**  
 ...
 를 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 통해 [!INCLUDE[ssSDS](../../includes/sssds-md.md)]데이터베이스 엔진 인스턴스에 연결할 때 인증을 사용 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 하 고 **서버에 연결** 대화 상자의 **연결 속성** 탭에서 데이터베이스를 지정 해야 합니다. **연결 암호화** 확인란을 선택 했는지 확인 합니다.  
