...
1.  에 **시작** 메뉴에서 **모든 프로그램**, 가리킨 [!INCLUDE[ssCurrentUI](../../includes/sscurrentui-md.md)], 클릭 하 고 **SQL Server Management Studio**합니다.  
  
    > [!NOTE]  
    >  기본적으로 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]는 설치되지 않습니다. [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]를 사용할 수 없으면 설치 프로그램을 실행하여 설치합니다. [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]는 [!INCLUDE[ssExpress](../../includes/ssexpress-md.md)]에서 사용할 수 없습니다. [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] Express는에서 무료 다운로드로 사용할 수는 [Microsoft 다운로드 센터](http://go.microsoft.com/fwlink/?LinkID=37075&clcid=0x409), 있지만이 자습서에 설명 된 것과 다른 사용자 인터페이스에 있습니다.  
  
2.  **서버에 연결** 대화 상자에서 기본 설정을 확인한 다음 **연결**을 클릭합니다. 에 연결 하려면는 **서버 이름** 상자에는 컴퓨터의 이름이 포함 되어야 합니다 여기서 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가 설치 되어 있습니다. 경우는 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 명명 된 인스턴스는 **서버 이름** 상자도 형식으로 인스턴스 이름이 있어야 \< *t e r _* > \\ < *instance_name*> 합니다.  
...
2.  서버에 표시 되지 않으면 등록 된 서버에서 마우스 오른쪽 단추로 클릭 **데이터베이스 엔진**, 클릭 하 고 **로컬 서버 등록 업데이트**합니다.  
