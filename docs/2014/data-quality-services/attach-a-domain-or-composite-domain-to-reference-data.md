...
>  도메인을 참조 데이터 서비스 스키마의 열에 매핑하는 동안 참조 데이터 서비스에 연결된 복합 도메인을 도메인 드롭다운 목록에서 사용할 수 있습니다. 복합 도메인을 참조 데이터 서비스 스키마의 열에 매핑하지 마세요. 복합 도메인 내의 개별 도메인만 참조 데이터 서비스 스키마의 적절한 열에 매핑해야 합니다. 그렇지 않으면 오류가 발생합니다.  
 ...
 이 항목에서는 네 개의 도메인을 만듭니다. **Address Line**, **City**, **State** 및 **Zip**과 복합 도메인 내의 **Address Verification**에서는 복합 도메인을 **Melissa Data - Address Check** 참조 데이터 서비스에 연결한 다음, 복합 도메인 내의 개별 도메인을 참조 데이터 서비스 스키마의 적절한 열에 매핑합니다.  
...
4.  **도메인 관리** 화면에서 **도메인 만들기** 아이콘을 클릭하여 도메인을 만듭니다. 다음 네 개의 도메인을 만듭니다. **Address Line**, **City**, **State** 및 **Zip**를 만듭니다.  
...
9. **참조 데이터** 탭으로 돌아갑니다. **공급자 설정** 영역에서 필요에 따라 다음 입력란의 값을 변경합니다.  
    ...
    -   **제안된 후보**: 참조 데이터 서비스에서 표시할 제안 된 후보 수입니다.  
...
##  <a name="FollowUp"></a> 후속 작업: 도메인을 참조 데이터에 매핑한 후  
