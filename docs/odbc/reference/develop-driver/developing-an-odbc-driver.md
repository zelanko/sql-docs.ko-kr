---
title: ODBC 드라이버 개발 | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
ms.assetid: 3225a011-5605-46ba-bb74-1ca6106a5271
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: 6c4affd835931c00fd5f8ad2514070527ce1b80f
ms.sourcegitcommit: f7fced330b64d6616aeb8766747295807c92dd41
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 04/23/2019
ms.locfileid: "62636141"
---
# <a name="developing-an-odbc-driver"></a>ODBC 드라이버 개발
이 섹션에서는 ODBC 드라이버를 개발 하는 방법을 설명 하는 항목을 포함 합니다.  
  
 이 섹션에는 다음 항목이 포함 되어 있습니다.  
  
-   [ODBC 드라이버 아키텍처](../../../odbc/reference/develop-driver/odbc-driver-architecture.md)  
  
-   [3.5 드라이버를 3.8 드라이버로 업그레이드](../../../odbc/reference/develop-driver/upgrading-a-3-5-driver-to-a-3-8-driver.md)  
  
-   [ODBC 드라이버에서 연결 풀 인식 개발](../../../odbc/reference/develop-driver/developing-connection-pool-awareness-in-an-odbc-driver.md)  
  
-   [비동기 함수 완료 알림](../../../odbc/reference/develop-driver/notification-of-asynchronous-function-completion.md)
