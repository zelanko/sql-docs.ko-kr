...
|SQL 유형 식별자|길이|  
|-------------------------|------------|  
|모든 문자 형식 [a]|열의 정의 된 또는 최대값 (변수 형식의 경우) 길이 (바이트)입니다. 이 값은 설명자 필드 SQL_DESC_OCTET_LENGTH와 동일 합니다.|  
...
| &nbsp; | &nbsp; |
 ...
 [a] 드라이버에서 변수 형식의 열 또는 매개 변수 길이를 확인할 수 없는 경우 SQL_NO_TOTAL를 반환 합니다.
