...
|**CONCAT (** _string_exp1_하십시오_string_exp2_**)** (ODBC 1.0)|연결한 결과인 문자열을 반환 *string_exp2* 하 *string_exp1*합니다. 결과 문자열은 DBMS에 종속됩니다. 예를 들어 열을 나타내는 *string_exp1* 반환 NULL이 아니지만 SQL Server는 반환 된 NULL이 아닌 문자열을 NULL 값을 DB2에 포함 되어 있습니다.|  
|**차이 (** _string_exp1_하십시오_string_exp2_**)** (ODBC 2.0)|에 대해 SOUNDEX 함수에서 반환 되는 값 사이의 차이 나타내는 정수 값을 반환 *string_exp1* 하 고 *string_exp2*합니다.|  
|**삽입 (** _string_exp1_를 *시작*를 *길이*하십시오 _string_exp2_**)** (ODBC 1.0)|문자를 반환 합니다. 문자열 위치 *길이* 문자에서 삭제 되었습니다 *string_exp1*부터 *시작*, 및 위치 *string_exp2* 에 삽입 되었음을 *string_exp를* 부터 *시작*합니다.|  
...
|**대체 (** _string_exp1_를 *string_exp2*하십시오 _string_exp3_**)** (ODBC 1.0)|검색 *string_exp1* 의 foroccurrences *string_exp2*를 바꾸고 *string_exp3*합니다.|  
