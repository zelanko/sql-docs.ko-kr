...
|**ATAN2 (** *float_exp1*하십시오 *float_exp2 * * *)** (ODBC 2.0)|아크탄젠트를 반환 합니다 *x* 하 고 *y* 으로 지정 된 좌표 *float_exp1* 및 *float_exp2*각각 각도 라디안 단위로 표현 됩니다.|  
...
|**MOD (** *integer_exp1*하십시오 *integer_exp2 * * *)** (ODBC 1.0)|나머지를 반환 합니다 (모듈러스) *integer_exp1* 나눈 *integer_exp2*합니다.|  
|**PI ()** (ODBC 1.0)|부동 소수점 값으로 pi의 상수 값을 반환합니다.|  
|**POWER (** *numeric_exp*하십시오 *integer_exp * * *)** (ODBC 2.0)|값을 반환 *numeric_exp* 활용할 *integer_exp*합니다.|  
...
|**ROUND (** *numeric_exp*하십시오 *integer_exp * * *)** (ODBC 2.0)|반환 *numeric_exp* 반올림 *integer_exp* 소수점이 하 자릿수를 배치 합니다. 하는 경우 *integer_exp* 가 음수 이면 *numeric_exp* 반올림 됩니다 &#124; *integer_exp* &#124; 소수점의 왼쪽에 배치 합니다.|  
|**기호 (** *numeric_exp* **)** (ODBC 1.0)|부호를 나타내는 표시기 반환 *numeric_exp*합니다. 하는 경우 *numeric_exp* 은 0,-1 보다 작거나이 반환 됩니다. 하는 경우 *numeric_exp* 0, 0이 반환 됩니다. 하는 경우 *numeric_exp* 0 보다 크면 1이 반환 됩니다.|  
...
|**TRUNCATE (** *numeric_exp*하십시오 *integer_exp * * *)** (ODBC 2.0)|반환 *numeric_exp* 잘립니다 *integer_exp* 소수점이 하 자릿수를 배치 합니다. 하는 경우 *integer_exp* 가 음수 이면 *numeric_exp* 잘립니다 &#124; *integer_exp* &#124; 소수점의 왼쪽에 배치 합니다.|
