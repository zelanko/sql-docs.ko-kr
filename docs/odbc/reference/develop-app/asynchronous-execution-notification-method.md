...
```  
 ...
 ODBC 연결 핸들에 설정 된 마지막 SQL_ATTR_ASYNC_DBC_EVENT 값이 NULL이 아니고 응용 프로그램에서 특성 SQL_ATTR_ASYNC_DBC_FUNCTION_ENABLE를 SQL_ASYNC_DBC_ENABLE_ON로 설정 하 여 비동기 모드를 사용 하도록 설정한 경우에는 ODBC 연결을 호출 합니다. 비동기 모드를 지 원하는 함수는 완료 알림을 받게 됩니다. ODBC 연결 핸들에 설정 된 마지막 SQL_ATTR_ASYNC_DBC_EVENT 값이 NULL 인 경우 ODBC는 비동기 모드의 설정 여부에 관계 없이 응용 프로그램을 보내지 않습니다.  
