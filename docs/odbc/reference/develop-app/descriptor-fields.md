...
설명자 포함 *헤더* 및 *레코드* 완전히 열 또는 매개 변수를 설명 하는 필드입니다.  
 ...
 설명자 헤더 필드의 단일 복사본을 포함합니다. 헤더 필드를 변경 합니다. 모든 열 또는 매개 변수의 영향을 줍니다.  
...
|SQL_DESC_BIND_OFFSET_PTR||  
 ...
 설명자를 0 개 이상의 설명자 레코드를 포함합니다. 열 또는 매개 변수 설명자의 유형에 따라 각 레코드에 설명합니다. 새 열 또는 매개 변수는 바인딩되는 경우 새 레코드 설명자에 추가 됩니다. 열 또는 매개 변수 바인딩 해제 하는 설명자에서 레코드가 제거 됩니다. 각 레코드는 다음 필드의 복사본이 포함 되어 있습니다.  
...
|SQL_DESC_LITERAL_SUFFIX|SQL_DESC_UPDATABLE|  
 ...
 여러 문 특성 설명자의 헤더 필드에 해당합니다. 호출을 통해 이러한 특성을 설정 **SQLSetStmtAttr** 호출 하 여 해당 되는 설명자 헤더 필드를 설정 하 고 **SQLSetDescField** 동일한 효과가 있습니다. 에 마찬가지입니다 **SQLGetStmtAttr** 및 **SQLGetDescField**, 둘 다 동일한 정보를 검색 합니다. 설명자 함수 대신 문 함수를 호출 하는 것에 설명자 핸들을 검색할 수 없는 장점이 있습니다.  
