---
title: SQL 문 | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
ms.assetid: 78133ee0-1e2f-4dc8-81f6-3bac8eb21332
author: David-Engel
ms.author: v-daenge
ms.openlocfilehash: bda24e8d22bda3a692237215b9d5f1904f5903a4
ms.sourcegitcommit: e042272a38fb646df05152c676e5cbeae3f9cd13
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 04/27/2020
ms.locfileid: "81299783"
---
# <a name="sql-statements"></a>SQL 문
ODBC 응용 프로그램은 SQL 문을 실행 하 여 거의 모든 데이터베이스 액세스를 수행 합니다. 이러한 문의 형태는 응용 프로그램의 요구 사항에 따라 달라 지 며 런타임에 하드 코딩 되거나 런타임에 생성 됩니다.  
  
 이 섹션에서는 다음 항목을 다룹니다.  
  
-   [SQL 문 생성](../../../odbc/reference/develop-app/constructing-sql-statements.md)  
  
-   [SQL 문의 상호 운용성](../../../odbc/reference/develop-app/interoperability-of-sql-statements.md)  
  
-   [ODBC의 이스케이프 시퀀스](../../../odbc/reference/develop-app/escape-sequences-in-odbc.md)
