...
|변경 유형|설명|  
|--------------------|-----------------|  
|새로운 기능|이는 인라인 바인딩 또는 설명자와 같이 *ODBC 3.x*의 새로운 기능입니다. 이러한 작업은 응용 프로그램 및 드라이버와 드라이버 관리자 *의 버전이 2.x 인 경우*에만 구현 되므로 이러한 이전 버전과의 호환성을 위해 시도 하지 않습니다.|  
