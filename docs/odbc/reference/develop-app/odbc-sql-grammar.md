---
title: ODBC SQL 문법 | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
helpviewer_keywords:
- interoperability [ODBC], SQL grammar
ms.assetid: 301817a7-d656-4078-b95f-e5b8099d4be4
author: David-Engel
ms.author: v-daenge
ms.openlocfilehash: 251560ec5772e76f2b3758dbd15ef8d6b87ff392
ms.sourcegitcommit: e042272a38fb646df05152c676e5cbeae3f9cd13
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 04/27/2020
ms.locfileid: "81282521"
---
# <a name="odbc-sql-grammar"></a>ODBC SQL 문법
상호 운용 가능한 응용 프로그램은 항상 SQL 문에서 ODBC SQL 문법을 사용 해야 합니다. 그러나이 문법 내 에서도 상당한 변형이 가능 합니다. 자세한 내용은 [SQL 문의 상호 운용성](../../../odbc/reference/develop-app/interoperability-of-sql-statements.md)을 참조 하세요.
