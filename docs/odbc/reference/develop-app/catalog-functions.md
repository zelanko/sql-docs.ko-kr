...
모든 데이터베이스에는 데이터베이스에 데이터를 저장 하는 방법에 대해 간략하게 설명 하는 구조체입니다. 예를 들어, 간단한 판매 주문 데이터베이스 테이블에 연결할 사용 되는 ID 열, 다음 그림에 표시 되는 구조가 있을 수 있습니다.  
 ...
 응용 프로그램에 대 한 호출을 통해이 구조를 검색할 수는 *카탈로그 함수*합니다. 카탈로그 함수 결과 집합에는 정보를 통해 일반적으로 구현 반환 **선택** 카탈로그의 테이블에 대해 문의 합니다. 예를 들어 애플리케이션은 시스템의 모든 테이블이나 특정 테이블의 모든 열에 대한 정보를 포함하는 결과 집합을 요청할 수 있습니다.  
