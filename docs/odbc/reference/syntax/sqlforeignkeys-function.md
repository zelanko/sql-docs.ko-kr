...
|UPDATE_RULE (ODBC 1.0)|10|Smallint|SQL 작업이 일 때 외래 키에 적용할 동작 **업데이트**합니다. 다음 값 중 하나일 수 있습니다. (참조 테이블은 기본 키가 있는 테이블; 참조 테이블은 외래 키가 있는 테이블입니다.)<br /><br /> SQL_CASCADE: 참조 된 테이블의 기본 키 업데이트 되 면 참조 테이블의 외래 키도 업데이트 됩니다.<br /><br /> SQL_NO_ACTION:의 기본 키를 업데이트 하는 경우 참조 되는 테이블으로 인해 "현 수 참조"에서 참조 하는 테이블 (즉, 참조 테이블의 행은가 없는 해당 참조 되는 테이블에서) 업데이트가 거부 되었습니다. 참조 하는 테이블의 외래 키 업데이트는 참조 되는 테이블의 기본 키의 값으로 존재 하지 않는 값에 도입 업데이트 거부 됩니다. (이 작업은 ODBC 2 SQL_RESTRICT 동작 동일 *.x*.)<br /><br /> SQL_SET_NULL: 참조 된 테이블의 행을 하나 이상의 기본 키의 하나 이상의 구성 요소가 변경 되는 방식으로 업데이트 되는 구성 요소 참조 하는 테이블의 외래 키의 기본 키의 변경 된 구성 요소에 해당 하는 설정 됩니다. 참조 테이블의 모든 일치 하는 행의 NULL로 합니다.<br /><br /> SQL_SET_DEFAULT: 참조 테이블의 행을 하나 이상의 기본 키의 하나 이상의 구성 요소가 변경 되는 방식으로 업데이트 되는 기본 키의 변경 된 구성 요소에 해당 하는 참조 테이블의 외래 키의 구성 요소는 참조 테이블의 모든 일치 하는 행의 해당 기본값으로 설정 합니다.<br /><br /> 데이터 원본에 해당 하지 않는 경우 NULL입니다.|  
|DELETE_RULE (ODBC 1.0)|11|Smallint|SQL 작업이 일 때 외래 키에 적용할 동작 **삭제**합니다. 다음 값 중 하나일 수 있습니다. (참조 테이블은 기본 키가 있는 테이블; 참조 테이블은 외래 키가 있는 테이블입니다.)<br /><br /> SQL_CASCADE: 참조 된 테이블의 행이 삭제 되 면 참조 테이블에 일치 하는 모든 행도 삭제 됩니다.<br /><br /> SQL_NO_ACTION:의 행을 삭제 하는 경우 참조 되는 테이블으로 인해 "현 수 참조"에서 참조 하는 테이블 (즉, 참조 테이블의 행은가 없는 해당 참조 되는 테이블에서) 업데이트가 거부 되었습니다. (이 작업은 ODBC 2 SQL_RESTRICT 동작 동일 *.x*.)<br /><br /> SQL_SET_NULL: 참조 된 테이블에서 하나 이상의 행이 삭제 되는 경우 참조 하는 테이블의 외래 키의 각 구성 요소 참조 테이블의 모든 일치 하는 행의 NULL로 설정 됩니다.<br /><br /> SQL_SET_DEFAULT: 참조 되는 테이블에서 하나 이상의 행이 삭제 되는 경우 참조 하는 테이블의 외래 키의 각 구성 요소 참조 테이블의 모든 일치 하는 행의 해당 기본값으로 설정 됩니다.<br /><br /> 데이터 원본에 해당 하지 않는 경우 NULL입니다.|  
