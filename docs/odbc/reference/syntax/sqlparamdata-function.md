 ...
 **SQLParamData** 두 가지 용도 대 한 데이터 실행 시 데이터를 제공 하기 위해 호출할 수:에 대 한 호출에서 사용할 수 있는 매개 변수 데이터 **SQLExecute** 또는 **SQLExecDirect**, 또는 열 데이터를 사용 하는 경우 행이 업데이트 되거나를 호출 하 여 추가 **SQLBulkOperations** 에 대 한 호출에 의해 업데이트 **SQLSetPos**합니다. 실행 시 **SQLParamData** 표시기는 데이터의 드라이버 필요한 응용 프로그램에 반환 합니다.  
