 ...
 **요약**  
 **SQLPrepare** SQL 문자열 실행을 위해 준비 합니다.  
...
|22018|캐스트 사양의 문자 값|**StatementText* 리터럴 또는 매개 변수를 포함 하는 SQL 문을 포함 하 고 값이 연결 된 테이블 열의 데이터 형식과 호환 되지 않습니다.|  
|22019|잘못 된 이스케이프 문자|인수 *StatementText* 포함 된 한 **같은** 조건자와 함께 **이스케이프** 에 **여기서** 절과 이스케이프의 길이 문자 다음 **이스케이프** 1과 같을 수 없습니다.|  
|22025|잘못 된 이스케이프 시퀀스|인수 *StatementText* 포함 된 "**같은** *패턴 값* **이스케이프** *이스케이프 문자*"에 **여기서** 절 및 패턴 값의 이스케이프 문자 뒤의 문자를"%"또는"_"아닙니다.|  
...
|34000|잘못된 커서 이름|\**StatementText* 를 위치로 지정 된 포함 된 **삭제** 또는 위치를 지정 **업데이트**, 및 준비 중인 문에서 참조 하는 커서를 열 수 없습니다.|  
|3D000|잘못 된 카탈로그 이름|에 지정 된 카탈로그 이름을 *StatementText* 올바르지 않습니다.|  
|3F000|잘못 된 스키마 이름|지정 된 스키마 이름이 *StatementText* 올바르지 않습니다.|  
|42000|구문 오류 또는 액세스 위반|\**StatementText* 준비할 수 있는 되었거나 구문 오류를 포함 하는 SQL 문을 포함 합니다.<br /><br /> **StatementText* 를 사용자는 필요한 권한이 없는 않았습니다 문이 포함 되었습니다.|  
...
|HY010|함수 시퀀스 오류입니다.|DM ()는 비동기적으로 실행 중인 함수를 호출한 연관 된 연결 핸들에 대 한는 *StatementHandle*합니다. 이 비동기 함수 계속 실행 될 때는 **SQLPrepare** 함수를 호출 했습니다.<br /><br /> (DM) **SQLExecute**, **SQLExecDirect**, 또는 **SQLMoreResults** 에 대 한 호출 된는 *StatementHandle* SQL_PARAM_DATA_ 반환 사용할 수 있습니다. 모든 스트리밍된 매개 변수에 대 한 데이터를 검색 하기 전에이 함수가 호출 되었습니다.<br /><br /> DM ()를 비동기적으로 실행 중인 함수 (하지이 하나)에 대해 호출 되었습니다는 *StatementHandle* 호출 되었을 때 계속 실행 하 고 있습니다.<br /><br /> (DM) **SQLExecute**, **SQLExecDirect**, **SQLBulkOperations**, 또는 **SQLSetPos** 가 대 한 호출에서  *StatementHandle* SQL_NEED_DATA를 반환 합니다. 이 함수는 모든 실행 시 데이터 매개 변수 또는 열에 대 한 데이터를 보내기 전에 호출 되었습니다.|  
|HY013|메모리 관리 오류입니다.|기본 메모리 개체에 액세스할 수 없습니다, 가능한 메모리 부족 때문에 함수 호출을 처리할 수 없습니다.|  
|HY090|문자열 또는 버퍼 길이가 잘못 되었습니다.|(DM) 인수 *TextLength* 가 보다 작거나 같으면 0 하지만 SQL_NTS 같지는있지 않습니다.|  
|HY117|연결 알 수 없는 트랜잭션 상태로 인해 일시 중단 됩니다. 만 연결을 끊고 읽기 전용 함수를 사용할 수 있습니다.|(DM) 일시 중단 된 상태에 대 한 자세한 내용은 참조 [SQLEndTran 함수](../../../odbc/reference/syntax/sqlendtran-function.md)합니다.|  
|HYC00|선택적 기능이 구현 되지 않았습니다|동시성 설정을 정의 하는 커서의 종류에 올바르지 않습니다.<br /><br /> SQL_ATTR_USE_BOOKMARKS 문 특성이 SQL_UB_VARIABLE로 설정 된 및 SQL_ATTR_CURSOR_TYPE 문 특성 드라이버 책갈피에 지원 하지 않으면 커서 유형으로 설정 되었습니다.|  
...
>  응용 프로그램에서 사용 **SQLPrepare** 준비 하 고 **SQLExecute** 제출 하는 **커밋** 또는 **롤백** 문, 되지 것입니다 DBMS 제품 간의 상호 운용 가능 합니다. 커밋하거나 트랜잭션을 롤백하려면 호출 **SQLEndTran**합니다.  
 ...
 드라이버는 데이터 원본에서 사용 하는 SQL 형식을 사용 하 여 한 다음 준비에 대 한 데이터 소스에 제출 하는 문을 수정할 수 있습니다. 특히, 드라이버는 특정 기능에 SQL 구문을 정의 하는 데 이스케이프 시퀀스를 수정 합니다. (SQL 문 문법에 대 한 참조 [odbc에서 이스케이프 시퀀스](../../../odbc/reference/develop-app/escape-sequences-in-odbc.md) 및 [부록 c: SQL 문법을](../../../odbc/reference/appendixes/appendix-c-sql-grammar.md).) 드라이버를 문 핸들은 포함 된 SQL 코드에서 문 식별자 비슷합니다. 문 식별자를 지 원하는 데이터 원본 드라이버는 데이터 원본에 문 식별자 및 매개 변수 값 보낼 수 있습니다.  
  
 문이 준비 된 후 응용 프로그램은 문 핸들을 사용 하 여 이후 함수 호출에서 방침을 참조 합니다. 문 핸들에 연결 된 준비 된 문을 호출 하 여 다시 실행할 수 있습니다 **SQLExecute** 응용 프로그램에 대 한 호출을 사용 하 여 문을 해제 될 때까지 **SQLFreeStmt** SQL_DROP 옵션 또는 문 핸들에 대 한 호출에 사용 될 때까지 **SQLPrepare**, **SQLExecDirect**, 또는 카탈로그 함수 중 하나 (**SQLColumns**,  **SQLTables**등). 문을 준비 하는 응용 프로그램, 되 면 결과 집합의 형식에 대 한 정보를 요청할 수 있습니다. 일부 구현에서는 호출에 대 한 **SQLDescribeCol** 또는 **SQLDescribeParam** 후 **SQLPrepare** 효율적으로 후함수를호출하지못할**SQLExecute** 또는 **SQLExecDirect**합니다.  
  
 일부 드라이버 구문 오류를 반환할 수 없거나 또는 액세스 위반 하면 응용 프로그램 호출 때 **SQLPrepare**합니다. 드라이버에서 구문 오류를 처리 하 고 액세스 위반 수만 구문 오류 또는 구문 오류도 아니고 액세스 위반입니다. 응용 프로그램 관련 함수와 같은 이후의 호출 하는 경우 이러한 상태를 처리 할 수 있어야 따라서 **SQLNumResultCols**, **SQLDescribeCol**, **SQLColAttribute**, 및 **SQLExecute**합니다.  
  
 데이터 원본 및 드라이버의 기능에 따라 (모든 매개 변수가 바인딩되면) 하는 경우 문을 준비할 때 매개 변수 정보 (예: 데이터 형식)를 검사할 수 있습니다 또는 (모든 매개 변수에 바인딩되어 있지) 하는 경우 실행 될 때입니다. 상호 운용성을 극대화 응용 프로그램 같은 문에서 새 SQL 문을 준비 하기 전에 SQL 문을 이전에 적용 하는 모든 매개 변수 바인딩 해제 해야 합니다. 이 새 문에 적용 되 고 이전 매개 변수 정보 때문에 발생 하는 오류를 방지할 수 있습니다.  
  
> [!IMPORTANT]  
>  명시적으로 호출 하 여 트랜잭션 커밋 **SQLEndTran** 하거나 자동 커밋 모드에서 작업 하 여 연결에서 모든 문에 대 한 액세스 계획을 삭제할 데이터 원본을 발생할 수 있습니다. 자세한 내용은 참조에서 SQL_CURSOR_COMMIT_BEHAVIOR 및 SQL_CURSOR_ROLLBACK_BEHAVIOR 정보 유형이 [SQLGetInfo](../../../odbc/reference/syntax/sqlgetinfo-function.md) 및 [커서 및 준비 된 문에서 영향의 트랜잭션을](../../../odbc/reference/develop-app/effect-of-transactions-on-cursors-and-prepared-statements.md)합니다.  
  
## <a name="code-example"></a>코드 예  
 참조 [SQLBindParameter](../../../odbc/reference/syntax/sqlbindparameter-function.md), [SQLPutData](../../../odbc/reference/syntax/sqlputdata-function.md), 및 [SQLSetPos](../../../odbc/reference/syntax/sqlsetpos-function.md)합니다.  
...
|문에 의해 영향을 받는 행 수 반환|[SQLRowCount 함수](../../../odbc/reference/syntax/sqlrowcount-function.md)|  
