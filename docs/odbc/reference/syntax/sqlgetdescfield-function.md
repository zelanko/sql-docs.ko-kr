 ...
 *BufferLength*  
 입력 *FieldIdentifier* 가 ODBC에 정의 된 필드이 고 *요소 문자열이* 나 이진 버퍼를 가리키는 경우이 인수는이 인수의 길이 \*여야 *합니다.* *FieldIdentifier* 가 ODBC에 정의 된 필드이 고 \*이상 값 *eptr* 이 정수 이면 *bufferlength* 는 무시 됩니다. **SQLGetDescFieldW**를 호출할 때 값 * \*eptr* 의 값이 유니코드 데이터 형식이 면 *bufferlength* 인수는 짝수 여야 합니다.  
