 ...
 *StringLengthPtr*  
 출력 Null 종료 문자를 제외 하 고 \* *이름* 버퍼에서 반환할 수 있는 데이터의 문자 수를 반환할 버퍼에 대 한 포인터입니다. 문자 수가 *bufferlength*보다 크거나 같은 경우 \* *이름의* 데이터는 *bufferlength* 에서 null 종료 문자의 길이를 뺀 값으로 잘리고 드라이버에 의해 null로 종료 됩니다.  
