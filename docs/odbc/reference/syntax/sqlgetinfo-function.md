 ...
 *InfoValuePtr*  
 [출력] 정보를 반환 하는 버퍼에 대 한 포인터입니다. 에 따라는 *정보 항목* 요청 반환 되는 정보 중 하나가 됩니다 다음: null로 끝나는 문자열 "," SQLUSMALLINT 값 ","는 SQLUINTEGER 비트 마스크 "," SQLUINTEGER 플래그 "," SQLUINTEGER 이진 값 또는 SQLULEN 값입니다.  
  
 경우는 *정보 항목* 인수가 SQL_DRIVER_HDESC 또는 SQL_DRIVER_HSTMT는 *InfoValuePtr* 인수는 모두 입력 및 출력 합니다. (자세한 내용은이 함수 설명의 뒷부분에 나오는 SQL_DRIVER_HDESC 또는 SQL_DRIVER_HSTMT 설명자 참조).  
...
-   **SQL_ASYNC_NOTIFICATION_NOT_CAPABLE** 비동기 실행 알림이 드라이버에서 지원 되지 않습니다.  
 ...
 SQL_BRC_ROLLED_UP = 연속 된 INSERT, DELETE 또는 UPDATE 문 하나에 롤업 됩니다에 대 한 행 개수입니다. 이 비트가 설정 되지 않으면, 행 개수를 각 문에 대해 사용할 수 있습니다.  
 ...
 이 *정보 항목* ODBC 2.0에서 ODBC 3.0에 대 한 이름을 바꾼 *정보 항목* SQL_QUALIFIER_NAME_SEPARATOR 합니다.  
 ...
 다음 비트 마스크는 변환을 지원 되는지 확인 하는 데 사용 됩니다.  
 ...
 **팬텀** 트랜잭션 1 몇 가지 검색 기준을 만족 하는 행 집합을 읽습니다. 트랜잭션 2 검색 조건과 일치 하는 삽입 또는 업데이트) (통해 하나 이상의 행을 생성 합니다. 트랜잭션 1에 행을 읽는 문을 reexecutes, 다른 행 집합을 받습니다.  
 ...
 SQL 92 전체 수준 – 준수 드라이버에서는 지원 됩니다 "Y"를 반환 일반적으로 **입력에 설명** 문. 그러나이 직접을 지정 하지 않으면 기본적으로 SQL 지원 하기 때문에 매개 변수를 설명 하는 지원 되지 않는, 수준 –와 호환 되 하는 SQL 92 전체 드라이버에도 합니다.  
 ...
 ODBC 드라이버 관리자는 기존 응용 프로그램에 대 한 이전 버전과 호환성을 유지 하기 위해 SQLGetInfo(SQL_DRIVER_ODBC_VER)의 반환 값을 수정 하지 않습니다. 드라이버는 값이 반환 될 지정 합니다. 하지만 C 데이터 형식 확장성을 지 원하는 드라이버로 3.8 (또는 이상) 경우에 반환 해야 응용 프로그램이 호출 **SQLSetEnvAttr** 3.8를 sql_attr_odbc_version으로 설정 합니다. 자세한 내용은 참조 [odbc에서 C 데이터 형식을](../../../odbc/reference/develop-app/c-data-types-in-odbc.md)합니다.  
 ...
 SQL_CA1_BULK_ADD =는 *작업* SQL_ADD의 인수에 대 한 호출에서 지원 됩니다 **SQLBulkOperations** 커서가 있는 경우 동적 커서.  
 ...
 SQL_CA2_MAX_ROWS_AFFECTS_ALL The SQL_ATTR_MAX_ROWS 문 특성에 영향을 미칩니다 = **선택**, **삽입**, **삭제**, 및 **업데이트** 문 및 **카탈로그** 동적 커서는 커서가 있는 경우 결과 집합을 합니다.  
  
 SQL_CA2_CRC_EXACT 정확한 = 커서 동적 커서를 가져갈 때 SQL_DIAG_CURSOR_ROW_COUNT 진단 필드 행 개수는 사용할 수 있습니다.  
  
 SQL_CA2_CRC_APPROXIMATE 대략적인 = 커서 동적 커서를 가져갈 때 SQL_DIAG_CURSOR_ROW_COUNT 진단 필드 행 개수는 사용할 수 있습니다.  
  
 SQL_CA2_SIMULATE_NON_UNIQUE 드라이버 = 시뮬레이션 보장 하지 업데이트 배치 또는 동적 커서; 때 delete 문은 한 행만가 적용 것은이 응용 프로그램의 책임입니다. (문이 둘 이상의 행에 영향을 **SQLExecute** 또는 **SQLExecDirect** SQLSTATE 01001 [커서 작업이 충돌]를 반환 합니다.) 이 동작을 호출 하 여 응용 프로그램을 설정 하려면 **SQLSetStmtAttr** 는 SQL_ATTR_SIMULATE_CURSOR와 특성이 SQL_SC_NON_UNIQUE로 설정 합니다.  
  
 SQL_CA2_SIMULATE_TRY_UNIQUE = 하 시뮬레이션 된 위치 지정된 update 또는 delete 문의 영향을 줍니다 하나의 행만 동적 커서는 커서가 있을 때 드라이버 하려고 합니다. 예를 들어 둘 이상의 행에 영향을 줄 수 있는 경우에 항상 드라이버 이러한 문의 실행 고유 키가 있습니다. (문이 둘 이상의 행에 영향을 **SQLExecute** 또는 **SQLExecDirect** SQLSTATE 01001 [커서 작업이 충돌]를 반환 합니다.) 이 동작을 호출 하 여 응용 프로그램을 설정 하려면 **SQLSetStmtAttr** 는 SQL_ATTR_SIMULATE_CURSOR와 특성이 SQL_SC_TRY_UNIQUE로 설정 합니다.  
  
 SQL_CA2_SIMULATE_UNIQUE 드라이버 = 시뮬레이션 보장 배치 업데이트 또는 동적 커서는 커서가 있는 경우 delete 문이 하나의 행에 영향을 줍니다. 드라이버는이 지정 된 문에서 보장할 수 없습니다 경우 **SQLExecDirect** 또는 **SQLPrepare** SQLSTATE 01001 (커서 작업 충돌)를 반환 합니다. 이 동작을 호출 하 여 응용 프로그램을 설정 하려면 **SQLSetStmtAttr** 는 SQL_ATTR_SIMULATE_CURSOR와 특성이 SQL_SC_UNIQUE로 설정 합니다.  
  
 SQL_EXPRESSIONS_IN_ORDERBY (ODBC 1.0)  
 문자열: "Y" 데이터 소스 지원 식에는 **ORDER BY** ; 나열 "N" 그렇지 않은 경우입니다.  
  
 SQL_FILE_USAGE (ODBC 2.0)  
 단일 계층 드라이버 파일을 데이터 원본에서 직접 처리 하는 방법을 나타내는 SQLUSMALLINT 값:  
  
 SQL_FILE_NOT_SUPPORTED = 드라이버는 단일 계층 드라이버가 아닙니다. 예를 들어 ORACLE 드라이버는 2 계층 드라이버.  
  
 SQL_FILE_TABLE = 테이블로 데이터 원본에는 단일 계층 드라이버 처리 파일. 예를 들어 Xbase 드라이버 각 Xbase 파일을 테이블로 처리합니다.  
  
 SQL_FILE_CATALOG = 카탈로그로 데이터 원본에서 단일 계층 드라이버 처리 파일. 예를 들어 Microsoft Access 드라이버는 전체 데이터베이스를 각 Microsoft Access 파일을 처리합니다.  
 ...
 이러한 비트 마스크의 설명에 대 한 SQL_DYNAMIC_CURSOR_ATTRIBUTES1 (및 참조 설명에 "동적 커서"에 대 한 "정방향 전용 커서" 대신).  
  
 SQL_FORWARD_ONLY_CURSOR_ATTRIBUTES2(ODBC 3.0)  
 드라이버에서 지원 되는 정방향 전용 커서의 특성을 설명 하는 SQLUINTEGER 비트 마스크입니다. 특성의 두 번째 하위 집합을 포함 하는이 비트 마스크 첫 번째 하위 집합 SQL_FORWARD_ONLY_CURSOR_ATTRIBUTES1를 참조 하십시오.  
 ...
 이러한 비트 마스크의 설명에 대 한 SQL_DYNAMIC_CURSOR_ATTRIBUTES2 (및 참조 설명에 "동적 커서"에 대 한 "정방향 전용 커서" 대신).  
  
 SQL_GETDATA_EXTENSIONS (ODBC 2.0)  
 에 대 한 확장을 열거 하는 SQLUINTEGER 비트 마스크 **SQLGetData**합니다.  
  
 드라이버 지원에 대 한 일반적인 확장 기능을 확인 하려면 다음 비트 마스크 플래그와 함께 사용 됩니다 **SQLGetData**:  
  
 SQL_GD_ANY_COLUMN = **SQLGetData** 바인딩된 마지막 열 전에 포함 하 여 모든 바인딩되지 않은 열에 대해 호출할 수 있습니다. SQL_GD_ANY_ORDER도 반환 하지 않는 한 열 번호를 오름차순의 순서로 열을 호출 해야 하는 참고 합니다.  
  
 SQL_GD_ANY_ORDER = **SQLGetData** 에 바인딩되지 않은 열 순서에 관계 없이 호출할 수 있습니다. **SQLGetData** 바인딩된 마지막 열 SQL_GD_ANY_COLUMN도 반환 하지 않는 한 후 열에 대해서만 호출할 수 있습니다.  
  
 SQL_GD_BLOCK = **SQLGetData** 후 해당 행에 위치 데이터 (여기서 행 집합 크기가 1 보다 크면) 블록의 모든 행에 바인딩되지 않은 열에 대해 호출할 수 있습니다 **SQLSetPos**합니다.  
  
 SQL_GD_BOUND = **SQLGetData** 바인딩되지 않은 열과 함께 바인딩된 열에 대해 호출할 수 있습니다. 드라이버는 SQL_GD_ANY_COLUMN도 반환 하지 않는 한이 값을 반환할 수 없습니다.  
 ...
 **SQLGetData** 바인딩된 마지막 열을 한 후에 발생 하는 바인딩되지 않은 열 에서만에서 데이터의 열 번호를 오름차순 라고 하며의 경우 행 블록의 행에 없는 반환 하는 데 필요 합니다.  
  
 호출을 지원 해야 하는 드라이버 책갈피 (고정 길이 또는 가변 길이)을 지 원하는 경우 **SQLGetData** 열 0에 있습니다. 이 지원은에 대 한 호출에 대 한 드라이버 반환 하는 것에 관계 없이 필요 **SQLGetInfo** 는 SQL_GETDATA_EXTENSIONS와 *정보 항목*합니다.  
  
 SQL_GROUP_BY (ODBC 2.0)  
 열 간의 관계를 지정 하는 SQLUSMALLINT 값의 **GROUP BY** 절과 선택 목록의 집계 되지 않은 원시 열:  
  
 SQL_GB_COLLATE = A **COLLATE** 각 그룹화 열의 끝에 절을 지정할 수 있습니다. (ODBC 3.0)  
  
 SQL_GB_NOT_SUPPORTED = **GROUP BY** 절이 지원 되지 않습니다. (ODBC 2.0)  
  
 SQL_GB_GROUP_BY_EQUALS_SELECT =는 **GROUP BY** 절 select 목록의 집계 되지 않은 원시 모든 열이 포함 되어야 합니다. 다른 열을 포함할 수 없습니다. 예를 들어 **DEPT, MAX(SALARY)에서 직원 그룹 선택 BY DEPT**합니다. (ODBC 2.0)  
  
 SQL_GB_GROUP_BY_CONTAINS_SELECT =는 **GROUP BY** 절 select 목록의 집계 되지 않은 원시 모든 열이 포함 되어야 합니다. Select 목록에 없는 열을 포함할 수 있습니다. 예를 들어 **DEPT, MAX(SALARY)에서 직원 그룹 선택 BY 부서, 나이**합니다. (ODBC 2.0)  
  
 SQL_GB_NO_RELATION = 열에는 **GROUP BY** 절과 select 목록 관련이 없습니다. Select 목록의 집계 되지 않은 원시, 그룹화 되지 않은 열의 의미는 데이터 소스에 따라 다릅니다. 예를 들어 **DEPT, 급여에서 직원 그룹 선택 BY 부서, 나이**합니다. (ODBC 2.0)  
  
 S Q l 92 항목 수준 – 준수 드라이버 지원 SQL_GB_GROUP_BY_EQUALS_SELECT 옵션을 항상 반환 됩니다. SQL 92 전체 수준 – 준수 드라이버를 지 원하는 대로 SQL_GB_COLLATE 옵션을 항상 반환 됩니다. 옵션은 지원 되는 경우는 **GROUP BY** 절은 데이터 원본에서 지원 되지 않습니다.  
  
 SQL_IDENTIFIER_CASE (ODBC 1.0)  
 SQLUSMALLINT 값 다음과 같습니다.  
  
 SQL_IC_UPPER = sql에서 식별자 대/소문자 구분 하지 않으며에 저장 된 시스템 카탈로그에 대문자입니다.  
  
 SQL_IC_LOWER = sql에서 식별자 대/소문자 구분 하지 않으며 시스템 카탈로그에서 소문자에 저장 됩니다.  
  
 Sql_ic_sensitive입니다 = sql에서 식별자 대/소문자 구분 되며 혼합 된 경우 시스템 카탈로그에 저장 됩니다.  
  
 Sql_ic_mixed입니다 = sql에서 식별자 대/소문자 구분 하지 않으며 시스템 카탈로그에서 대/소문자를 혼합된에 저장 됩니다.  
  
 S Q l 92 식별자는 대/소문자 구분 하지 않습니다, 때문에 지원 sql-92 (수준)를 엄격 하 게 준수 하는 드라이버 sql_ic_sensitive입니다 옵션을 반환 하지 않습니다.  
  
 SQL_IDENTIFIER_QUOTE_CHAR (ODBC 1.0)  
 따옴표 붙은의 시작 및 끝 구분 기호로 사용 되는 문자열 (분리) SQL 문에서 식별자입니다. (ODBC 함수에 인수로 전달 된 식별자 필요가 없습니다 따옴표로 묶을 수 있습니다.) 데이터 원본에서 따옴표 붙은 식별자를 지원 하지는 빈 값이 반환 됩니다.  
  
 이 문자열을 연결 특성 SQL_ATTR_METADATA_ID SQL_TRUE로 설정 된 경우 카탈로그 함수 인수를 따옴표로 사용할 수도 있습니다.  
  
 S Q l-92에 식별자 인용 문자 큰따옴표 (") 이기 때문에 맞는 드라이버 엄격 하 게 sql-92에 항상 반환 됩니다 큰따옴표 문자.  
  
 SQL_INDEX_KEYWORDS (ODBC 3.0)  
 CREATE INDEX 문은에서 드라이버에서 지원 되는 키워드를 열거 하는 SQLUINTEGER 비트 마스크:  
  
 SQL_IK_NONE = 지원 되지 않습니다는 키워드입니다.  
 ...
 CREATE INDEX 문을 사용할 수 있는지를 확인 하려면 응용 프로그램이 호출 **SQLGetInfo** SQL_DLL_INDEX 정보 유형을 사용 합니다.  
  
 SQL_INFO_SCHEMA_VIEWS (ODBC 3.0)  
 드라이버에서 지원 되는 뷰는 INFORMATION_SCHEMA에 열거 SQLUINTEGER 비트 마스크입니다. 뷰와의 내용은 INFORMATION_SCHEMA s Q l-92에 정의 되어 있습니다.  
  
 이 기능을 지원 해야 하는 SQL 92 또는 FIPS 규격 수준 각 비트 마스크 옆 괄호 안에 표시 됩니다.  
  
 다음 비트 마스크는 뷰가 지원 되는지 확인 하는 데 사용 됩니다.  
  
 SQL_ISV_ASSERTIONS = 지정된 된 사용자가 소유 하 고 있는 카탈로그의 어설션을 식별 합니다. (전체 수준)  
  
 SQL_ISV_CHARACTER_SETS = 지정된 된 사용자가 액세스할 수 있는 카탈로그의 문자 집합을 식별 합니다. (중간 수준)  
  
 SQL_ISV_CHECK_CONSTRAINTS 식별 검사 = 지정된 된 사용자가 소유 하는 제약 조건입니다. (중간 수준)  
  
 SQL_ISV_COLLATIONS = 지정된 된 사용자가 액세스할 수 있는 카탈로그에 대 한 문자 데이터 정렬을 식별 합니다. (전체 수준)  
  
 SQL_ISV_COLUMN_DOMAIN_USAGE = 카탈로그에 정의 된 도메인에 따라 달라 집니다는 지정된 된 사용자가 소유 하는 카탈로그에 대 한 열을 식별 합니다. (중간 수준)  
  
 SQL_ISV_COLUMN_PRIVILEGES = 사용할 수 있는 권한을 부여 또는 지정된 된 사용자가 영구 테이블의 열에 대 한 권한을 식별 합니다. (FIPS 전환 수준)  
  
 SQL_ISV_COLUMNS = 지정된 된 사용자가 액세스할 수 있는 영구 테이블의 열을 식별 합니다. (FIPS 전환 수준)  
  
 SQL_ISV_CONSTRAINT_COLUMN_USAGE 근사치 = CONSTRAINT_TABLE_USAGE 뷰 열이 지정된 된 사용자가 소유 하는 다양 한 제약 조건에 대 한 식별 됩니다. (중간 수준)  
  
 SQL_ISV_CONSTRAINT_TABLE_USAGE = 제약 조건에 의해 사용 되는 테이블을 식별 (참조, 고유, 어설션), 지정된 된 사용자가 소유 하 고 있습니다. (중간 수준)  
  
 SQL_ISV_DOMAIN_CONSTRAINTS = 지정된 된 사용자가 액세스할 수 있는 카탈로그에 있는 도메인) (의 도메인 제약 조건을 식별 합니다. (중간 수준)  
  
 SQL_ISV_DOMAINS = 사용자가 액세스할 수 있는 카탈로그에 정의 된 도메인을 식별 합니다. (중간 수준)  
  
 SQL_ISV_KEY_COLUMN_USAGE = 지정된 된 사용자가 키로 제약 하는 카탈로그에 정의 된 식별 열입니다. (중간 수준)  
  
 SQL_ISV_REFERENTIAL_CONSTRAINTS = 지정된 된 사용자가 소유 하 고 있는 참조 제약 조건을 식별 합니다. (중간 수준)  
  
 SQL_ISV_SCHEMATA = 지정된 된 사용자가 소유 하는 스키마를 식별 합니다. (중간 수준)  
  
 SQL_ISV_SQL_LANGUAGES = SQL 구현에서 지 원하는 SQL 받는 규칙 수준, 옵션 및 언어 식별 합니다. (중간 수준)  
  
 SQL_ISV_TABLE_CONSTRAINTS = 지정된 된 사용자가 소유 하는 테이블 제약 조건을 식별 합니다. (중간 수준)  
  
 SQL_ISV_TABLE_PRIVILEGES = 영구는 테이블에는 사용할 수 있거나 부여한 지정된 된 사용자가 권한을 식별 합니다. (FIPS 전환 수준)  
  
 SQL_ISV_TABLES = 지정된 된 사용자가 액세스할 수 있는 카탈로그에 정의 된 영구 테이블 식별 합니다. (FIPS 전환 수준)  
  
 SQL_ISV_TRANSLATIONS = 지정된 된 사용자가 액세스할 수 있는 카탈로그에 대 한 문자 변환을 식별 합니다. (전체 수준)  
  
 SQL_ISV_USAGE_PRIVILEGES = 사용할 수 있거나 지정된 된 사용자가 소유 하는 카탈로그 개체에 사용 권한을 식별 합니다. (FIPS 전환 수준)  
  
 SQL_ISV_VIEW_COLUMN_USAGE의 카탈로그 뷰는 기반이 되는 열은 지정된 된 사용자가 소유 하는 식별 = 따라 달라 집니다. (중간 수준)  
  
 SQL_ISV_VIEW_TABLE_USAGE 테이블의 카탈로그 뷰는에 지정된 된 사용자가 소유 하는 식별 = 따라 달라 집니다. (중간 수준)  
  
 SQL_ISV_VIEWS = 지정된 된 사용자가 액세스할 수 있는이 카탈로그에 정의 된 표시 테이블 식별 합니다. (FIPS 전환 수준)  
  
 SQL_INSERT_STATEMENT (ODBC 3.0)  
 에 대 한 지원을 지정 하는 SQLUINTEGER 비트 마스크 **삽입** 문:  
 ...
 문자열: "Y" 데이터 소스 지원 무결성 향상 기능입니다. "N" 그렇지 않은 경우입니다.  
  
 이 *정보 항목* ODBC 2.0에서 ODBC 3.0에 대 한 이름을 바꾼 *정보 항목* SQL_ODBC_SQL_OPT_IEF 합니다.  
  
 SQL_KEYSET_CURSOR_ATTRIBUTES1(ODBC 3.0)  
 드라이버에서 지원 되는 키 집합 커서의 특성을 설명 하는 SQLUINTEGER 비트 마스크입니다. 특성의 첫 번째 하위 집합을 포함 하는이 비트 마스크 두 번째 하위 집합 SQL_KEYSET_CURSOR_ATTRIBUTES2를 참조 하십시오.  
 ...
 이러한 비트 마스크의 설명에 대 한 SQL_DYNAMIC_CURSOR_ATTRIBUTES1 (및 참조 "키 집합 커서" 설명에서 "동적 커서"에 대 한 대체).  
  
 S Q l 92 중간 수준 – 준수 드라이버 일반적으로 지원 반환 됩니다 SQL_CA1_NEXT, SQL_CA1_ABSOLUTE, 및 SQL_CA1_RELATIVE 옵션, 드라이버가 포함 된 SQL FETCH 문을 통해 스크롤 가능 커서를 지원 하기 때문에 있습니다. 하지만이 확인 되지 않으므로 직접 기본 SQL 지원, 스크롤 가능 커서 수 지원 되지 SQL 92 중간 수준 – 준수 드라이버에 대해서도 합니다.  
  
 SQL_KEYSET_CURSOR_ATTRIBUTES2(ODBC 3.0)  
 드라이버에서 지원 되는 키 집합 커서의 특성을 설명 하는 SQLUINTEGER 비트 마스크입니다. 특성의 두 번째 하위 집합을 포함 하는이 비트 마스크 첫 번째 하위 집합 SQL_KEYSET_CURSOR_ATTRIBUTES1를 참조 하십시오.  
 ...
 모든 데이터 원본에 따른 특정 키워드의 쉼표로 구분 된 목록이 포함 된 문자열입니다. 이 목록에 특정 odbc 키워드 또는 데이터 원본 및 ODBC 모두에서 사용 하는 키워드 이 목록은 모든 예약 된 키워드입니다. 상호 운용 가능한 응용 프로그램 개체 이름에이 예약어를 사용 해야 합니다.  
  
 목록이 ODBC 키워드에 대 한 참조 [예약 된 키워드](../../../odbc/reference/appendixes/reserved-keywords.md) 에 [부록 c: SQL 문법을](../../../odbc/reference/appendixes/appendix-c-sql-grammar.md)합니다. **#define** 값 SQL_ODBC_KEYWORDS ODBC 키워드의 쉼표로 구분 된 목록을 포함 합니다.  
  
 부록 c: SQL 문법  
  
 SQL_LIKE_ESCAPE_CLAUSE (ODBC 2.0)  
 문자열: "Y" 백분율 문자 (%)에 대 한 데이터 원본에서 이스케이프 문자를 지원 및 밑줄 (_)의 문자는 **같은** 조건자와 드라이버를 정의 하기 위한 ODBC 구문을 지원함는 **같은** 이스케이프 문자; 조건자 "N" 그렇지 않으면입니다.  
  
 SQL_MAX_ASYNC_CONCURRENT_STATEMENTS (ODBC 3.0)  
 드라이버는 지정된 된 연결을 지원할 수 있는 비동기 모드로 최대 동시 활성 문 수를 지정 하는 SQLUINTEGER 값입니다. 특정 제한이 알 수 없는 경우이 값은 0입니다.  
  
 SQL_MAX_BINARY_LITERAL_LEN (ODBC 2.0)  
 최대 길이 지정 하는 SQLUINTEGER 값 (리터럴 접두사와 접미사 반환한 제외 하 고 16 진수 문자의 숫자 **SQLGetTypeInfo**)는 SQL 문의 리터럴 이진 파일의 합니다. 예를 들어 리터럴 0xFFAA 이진 파일의 4의 길이입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 SQL_MAX_CATALOG_NAME_LEN (ODBC 1.0)  
 데이터 원본의 카탈로그 이름의 최대 길이 지정 하는 SQLUSMALLINT 값입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 전체 수준 – 준수 드라이버를 사용 하는 최소한 128을 반환 합니다.  
  
 이 *정보 항목* ODBC 2.0에서 ODBC 3.0에 대 한 이름을 바꾼 *정보 항목* SQL_MAX_QUALIFIER_NAME_LEN 합니다.  
  
 SQL_MAX_CHAR_LITERAL_LEN (ODBC 2.0)  
 최대 길이 지정 하는 SQLUINTEGER 값 (리터럴 접두사와 접미사 반환한 제외 하 고 문자 수 **SQLGetTypeInfo**)는 SQL 문의 리터럴 문자. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 SQL_MAX_COLUMN_NAME_LEN (ODBC 1.0)  
 데이터 원본에서 열 이름의 최대 길이 지정 하는 SQLUSMALLINT 값입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버 18 이상 반환 합니다. FIPS 중간 수준 – 준수 드라이버 최소한 128을 반환 합니다.  
  
 SQL_MAX_COLUMNS_IN_GROUP_BY (ODBC 2.0)  
 사용할 수 있는 열의 최대 수를 지정 하는 SQLUSMALLINT 값은 **GROUP BY** 절. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버 6 이상 반환 합니다. FIPS 중간 수준 – 준수 드라이버 적어도 15를 반환 합니다.  
  
 SQL_MAX_COLUMNS_IN_INDEX (ODBC 2.0)  
 인덱스에 사용할 수 있는 열의 최대 수를 지정 하는 SQLUSMALLINT 값입니다. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 SQL_MAX_COLUMNS_IN_ORDER_BY (ODBC 2.0)  
 사용할 수 있는 열의 최대 수를 지정 하는 SQLUSMALLINT 값은 **ORDER BY** 절. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버 6 이상 반환 합니다. FIPS 중간 수준 – 준수 드라이버 적어도 15를 반환 합니다.  
  
 SQL_MAX_COLUMNS_IN_SELECT (ODBC 2.0)  
 Select 목록에서 사용할 수 있는 열의 최대 수를 지정 하는 SQLUSMALLINT 값입니다. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버에서 적어도 100 개를 반환 합니다. FIPS 중간 수준 – 준수 드라이버 250 이상 반환 합니다.  
  
 SQL_MAX_COLUMNS_IN_TABLE (ODBC 2.0)  
 테이블에 사용할 수 있는 열의 최대 수를 지정 하는 SQLUSMALLINT 값입니다. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버에서 적어도 100 개를 반환 합니다. FIPS 중간 수준 – 준수 드라이버 250 이상 반환 합니다.  
  
 SQL_MAX_CONCURRENT_ACTIVITIES (ODBC 1.0)  
 활성 문 드라이버에 대 한 연결을 지원할 수 있는 최대 수를 지정 하는 SQLUSMALLINT 값입니다. 행이 포함 된 용어 "결과" 의미를 보류 중인 결과가 있으면 문을 활성으로 정의 된 **선택** 작업이 나 영향을 받는 행은 **삽입**, **업데이트**, 또는 **삭제** 작업 (예: 행 수) NEED_DATA 상태 이면 또는 합니다. 이 값은 드라이버 또는 데이터 원본 제한을 반영할 수 있습니다. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 이 *정보 항목* ODBC 2.0에서 ODBC 3.0에 대 한 이름을 바꾼 *정보 항목* SQL_ACTIVE_STATEMENTS 합니다.  
  
 SQL_MAX_CURSOR_NAME_LEN (ODBC 1.0)  
 데이터 원본에는 커서 이름의 최대 길이 지정 하는 SQLUSMALLINT 값입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버 18 이상 반환 합니다. FIPS 중간 수준 – 준수 드라이버 최소한 128을 반환 합니다.  
  
 SQL_MAX_DRIVER_CONNECTIONS (ODBC 1.0)  
 환경에 대 한 드라이버를 지원할 수 있는 활성 연결의 최대 수를 지정 하는 SQLUSMALLINT 값입니다. 이 값은 드라이버 또는 데이터 원본 제한을 반영할 수 있습니다. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 이 *정보 항목* ODBC 2.0에서 ODBC 3.0에 대 한 이름을 바꾼 *정보 항목* SQL_ACTIVE_CONNECTIONS 합니다.  
  
 SQL_MAX_IDENTIFIER_LEN (ODBC 3.0)  
 사용자 정의 이름에 대 한 데이터 소스에서 지 원하는 문자 최대 크기를 나타내는 SQLUSMALLINT 합니다.  
  
 FIPS 항목 수준 – 준수 드라이버 18 이상 반환 합니다. FIPS 중간 수준 – 준수 드라이버 최소한 128을 반환 합니다.  
  
 SQL_MAX_INDEX_SIZE (ODBC 2.0)  
 인덱스의 결합 된 필드에 허용 되는 바이트의 최대 수를 지정 하는 SQLUINTEGER 값입니다. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 SQL_MAX_PROCEDURE_NAME_LEN (ODBC 1.0)  
 데이터 원본에 프로시저 이름의 최대 길이 지정 하는 SQLUSMALLINT 값입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 SQL_MAX_ROW_SIZE (ODBC 2.0)  
 테이블의 단일 행의 최대 길이 지정 하는 SQLUINTEGER 값입니다. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버 적어도 2, 000를 반환 합니다. FIPS 중간 수준 – 준수 드라이버 이상 8000을 반환 합니다.  
  
 SQL_MAX_ROW_SIZE_INCLUDES_LONG (ODBC 3.0)  
 문자열: 최대 행 크기는 행에서 모든 SQL_LONGVARBINARY 및 SQL_LONGVARCHAR 열 길이 포함 하는 SQL_MAX_ROW_SIZE 정보 형식에 대 한 반환 되 면 "Y" "N" 그렇지 않으면입니다.  
  
 SQL_MAX_SCHEMA_NAME_LEN (ODBC 1.0)  
 데이터 원본의 스키마 이름의 최대 길이 지정 하는 SQLUSMALLINT 값입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버 18 이상 반환 합니다. FIPS 중간 수준 – 준수 드라이버 최소한 128을 반환 합니다.  
  
 이 *정보 항목* ODBC 2.0에서 ODBC 3.0에 대 한 이름을 바꾼 *정보 항목* SQL_MAX_OWNER_NAME_LEN 합니다.  
  
 SQL_MAX_STATEMENT_LEN (ODBC 2.0)  
 SQL 문의 최대 길이 (문자를 포함 하 여 공백 수)를 지정 하는 SQLUINTEGER 값입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 SQL_MAX_TABLE_NAME_LEN (ODBC 1.0)  
 데이터 원본에 테이블 이름의 최대 길이 지정 하는 SQLUSMALLINT 값입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버 18 이상 반환 합니다. FIPS 중간 수준 – 준수 드라이버 최소한 128을 반환 합니다.  
  
 SQL_MAX_TABLES_IN_SELECT (ODBC 2.0)  
 에 허용 되는 테이블의 최대 수를 지정 하는 SQLUSMALLINT 값의 **FROM** 절은 **선택** 문. 지정 된 제한이 없습니다도 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 FIPS 항목 수준 – 준수 드라이버 적어도 15를 반환 합니다. FIPS 중간 수준 – 준수 드라이버 50 이상 반환 합니다.  
  
 SQL_MAX_USER_NAME_LEN (ODBC 2.0)  
 데이터 원본에 사용자 이름의 최대 길이 지정 하는 SQLUSMALLINT 값입니다. 최대 길이가 없음을 있거나 길이 알 수 없는 경우이 값은 0으로 설정 됩니다.  
  
 SQL_MULT_RESULT_SETS (ODBC 1.0)  
 문자열: "Y" 그렇지 않은 경우 데이터 원본에서 여러 결과 집합을 "N"를 지원 합니다.  
  
 여러 결과 집합에 대 한 자세한 내용은 참조 [여러 결과](../../../odbc/reference/develop-app/multiple-results.md)합니다.  
  
 SQL_MULTIPLE_ACTIVE_TXN (ODBC 1.0)  
 문자열: "Y" 드라이버 언제 든 지 하나의 트랜잭션만 활성화할 수만 동시 "N"에 둘 이상의 활성 트랜잭션을 지원 합니다.  
  
 이 정보 유형에 대해 반환 되는 정보는 분산된 트랜잭션을 경우 적용 되지 않습니다.  
  
 SQL_NEED_LONG_DATA_LEN (ODBC 2.0)  
 문자열: "Y" 데이터 원본에 해당 값 앞 (데이터 형식이 SQL_LONGVARCHAR, SQL_LONGVARBINARY, 또는 긴 데이터 원본에 따른 특정 데이터 형식)는 긴 데이터 값의 길이 있어야 하는 경우는 그렇지 않은 경우 "N" 데이터 원본에 전송 됩니다. 자세한 내용은 참조 [SQLBindParameter 함수](../../../odbc/reference/syntax/sqlbindparameter-function.md) 및 [SQLSetPos 함수](../../../odbc/reference/syntax/sqlsetpos-function.md)합니다.  
  
 SQL_NON_NULLABLE_COLUMNS (ODBC 1.0)  
 데이터 소스 열에 NOT NULL을 지원 하는지 여부를 지정 하는 SQLUSMALLINT 값:  
  
 SQL_NNC_NULL = 모든 열이 null을 허용 해야 합니다.  
  
 SQL_NNC_NON_NULL = 열 null 허용 될 수 없습니다. (데이터 소스가 지원은 **NOT NULL** 열 제약 조건에서 **CREATE TABLE** 문.)  
  
 S Q l 92 항목 수준 – 준수 드라이버 SQL_NNC_NON_NULL 반환 됩니다.  
 ...
 SQL_NC_HIGH = Null ASC 또는 DESC 키워드에 따라 결과 집합의 높은 끝에 정렬 됩니다.  
  
 SQL_NC_LOW = Null ASC 또는 DESC 키워드에 따라 결과 집합의 하위 끝에 정렬 됩니다.  
  
 SQL_NC_START = Null ASC 또는 DESC 키워드에 관계 없이 결과 집합의 시작 부분에 정렬 됩니다.  
  
 SQL_NUMERIC_FUNCTIONS (ODBC 1.0)  
 참고: 정보 유형은; ODBC 1.0에 도입 된 각 비트 마스크 도입 된 버전으로 표시 됩니다.  
  
 드라이버와 관련 된 데이터 원본에서 지 원하는 숫자 스칼라 함수를 열거 SQLUINTEGER 비트 마스크입니다.  
 ...
 SQL_OIC_LEVEL1: 핵심 스크롤 가능 커서를 포함 하는 표준 준수 수준 기능, 책갈피, 배치를 포함 하 여 수준 업데이트 및 삭제, 등에입니다.  
  
 SQL_OIC_LEVEL2: 수준을 수준 1 표준 준수 수준 기능을 포함 한 중요 한 커서; 등의 고급 기능을 포함 하 여 업데이트, 삭제 및 책갈피; 하 여 새로 고침 저장된 프로시저 지원 합니다. 기본 및 외래 키;에 대 한 카탈로그 함수 다중 카탈로그 지원 합니다. 등에입니다.  
  
 자세한 내용은 참조 [인터페이스 받는 규칙 수준](../../../odbc/reference/develop-app/interface-conformance-levels.md)합니다.  
  
 SQL_ODBC_VER (ODBC 1.0)  
 버전의 ODBC 드라이버 관리자는 준수 하는 문자열입니다. 폼의 버전은 # #. # #. 0000, 여기에서 처음 두 자리는 주 버전 하 고 다음 두 자리는 부 버전. 드라이버 관리자에만 구현 되었습니다.  
  
 SQL_OJ_CAPABILITIES (ODBC 2.01)  
 드라이버 및 데이터 원본에서 지 원하는 외부 조인의 형식 열거 SQLUINTEGER 비트 마스크입니다. 다음 비트 마스크는 지원 되는 유형을 결정 하는 데 사용 됩니다.  
  
 SQL_OJ_LEFT = 왼쪽 우선 외부 조인은 지원 됩니다.  
  
 SQL_OJ_RIGHT = 오른쪽 우선 외부 조인은 지원 됩니다.  
  
 SQL_OJ_FULL = Full outer 조인을 사용할 수 있습니다.  
  
 SQL_OJ_NESTED Nested = 외부 조인은 지원 됩니다.  
  
 SQL_OJ_NOT_ORDERED = outer join의 ON 절에는 이름에 해당 테이블 이름으로 같은 순서로 배열 될 필요가 없습니다 열은 **외부 조인** 절.  
  
 SQL_OJ_INNER = 내부 테이블 (왼쪽된 우선 외부 조인 오른쪽 테이블) 또는 오른쪽 우선 외부 조인은 왼쪽된 테이블 inner join에 사용할 수도 있습니다. 내부 테이블 없으면 완전 외부 조인에는 적용 되지 않습니다.  
  
 SQL_OJ_ALL_COMPARISON_OPS 비교 = ON 절에서 연산자는 ODBC 비교 연산자 중 하나일 수 있습니다. 이 비트가 설정 되지 않으면 경우 equals (=) 비교 연산자만 우선 외부 조인에서 사용할 수 있습니다.  
  
 이러한 옵션을 모두 반환 되 면 지원, 외부 조인 절 없이 사용할 수 있습니다.  
  
 SELECT 문에서 관계형 join 연산자는 지원에 대 한 정보에 대 한 SQL 92에 정의 된 대로 SQL_SQL92_RELATIONAL_JOIN_OPERATORS 참조 합니다.  
  
 SQL_ORDER_BY_COLUMNS_IN_SELECT (ODBC 2.0)  
 문자열: "Y" 하는 경우의 열은 **ORDER BY** 절; select 목록에 있어야 합니다. 그렇지 않은 경우 "N"입니다.  
  
 SQL_PARAM_ARRAY_ROW_COUNTS (ODBC 3.0)  
 행의 가용성에 대 한 드라이버의 속성을 열거 하는 SQLUINTEGER 매개 변수가 있는 실행에 계산 합니다. 값은 다음과 같습니다.  
  
 SQL_PARC_BATCH = 개인 행 개수는 각 매개 변수 집합에 사용할 수 있습니다. 배열에 설정 하는 각 매개 변수에 대해 하나씩, SQL 문 일괄 처리를 생성 하는 드라이버를 개념적으로 같습니다. SQL_PARAM_STATUS_PTR 설명자 필드를 사용 하 여 확장된 오류 정보를 검색할 수 있습니다.  
  
 SQL_PARC_NO_BATCH = 매개 변수의 전체 배열에 대 한 문 실행에서으로 인해 발생 된 누적 행 수는 하나의 행 개수를 사용할 수 있습니다. 이 원자 단위로 전체 매개 변수 배열 함께 문을 처리 하는 것과 개념상 동일 합니다. 오류는 하나의 문을 실행 하는 경우 동일한 처리 됩니다.  
  
 SQL_PARAM_ARRAY_SELECTS (ODBC 3.0)  
 결과의 가용성에 대 한 드라이버의 속성을 열거 하는 SQLUINTEGER 매개 변수가 있는 실행에 설정 합니다. 값은 다음과 같습니다.  
  
 SQL_PAS_BATCH = 하나의 결과 집합 매개 변수 집합을 사용할 수 있습니다. 배열에 설정 하는 각 매개 변수에 대해 하나씩, SQL 문 일괄 처리를 생성 하는 드라이버를 개념적으로 같습니다.  
  
 SQL_PAS_NO_BATCH = 누적 결과 나타내는 하나의 결과 집합을 사용할 수 있는 집합이 매개 변수의 전체 배열에 대 한 문 실행에서으로 인해 발생 합니다. 이 원자 단위로 전체 매개 변수 배열 함께 문을 처리 하는 것과 개념상 동일 합니다.  
  
 SQL_PAS_NO_SELECT = A 드라이버는 결과 집합 생성 문을 매개 변수 배열을 사용 하 여 실행 하도록 허용 하지 않습니다.  
  
 SQL_PROCEDURE_TERM (ODBC 1.0)  
 프로시저;에 대 한 데이터 원본 공급 업체의 이름 가진 문자열 예를 들어 "데이터베이스 프로시저", "저장된 프로시저", "프로시저", "패키지" 또는 "저장된 쿼리"입니다.  
  
 SQL_PROCEDURES (ODBC 1.0)  
 문자열: "Y" 데이터 소스에서 프로시저를 지원 하 고 드라이버는 ODBC 프로시저 호출 구문; 지원 "N" 그렇지 않으면입니다.  
  
 SQL_POS_OPERATIONS (ODBC 2.0)  
 지원 작업을 열거 하는 SQLINTEGER 비트 마스크 **SQLSetPos**합니다.  
  
 다음 비트 마스크 옵션이 지원 되는지 확인 하는 플래그와 함께 사용 됩니다.  
 ...
 SQL_IC_UPPER = 따옴표 붙은 식별자 SQL에서 대/소문자 구분 하지 않으며에 저장 된 시스템 카탈로그에 대문자 합니다.  
  
 SQL_IC_LOWER 따옴표 붙은 = sql에서 식별자 대/소문자 구분 하지 않으며 시스템 카탈로그에서 소문자에 저장 됩니다.  
  
 Sql_ic_sensitive입니다 = 따옴표 붙은 식별자 SQL에서 대/소문자 구분 되며 시스템 카탈로그에서 혼합 된 경우에 저장 됩니다. (SQL 92 – 호환 데이터베이스에 따옴표 붙은 식별자는 항상 대/소문자 구분입니다.)  
  
 Sql_ic_mixed입니다 = 따옴표 붙은 식별자 SQL에서 대/소문자 구분 하지 않으며 시스템 카탈로그에서 대/소문자를 혼합된에 저장 됩니다.  
  
 S Q l 92 항목 수준 – 준수 드라이버 sql_ic_sensitive입니다 항상 반환 됩니다.  
  
 SQL_ROW_UPDATES (ODBC 1.0)  
 문자열: "Y" 행 버전을 유지 관리 또는 혼합 키 집합 커서 또는 모든 값을 행을 인출 하 고 따라서 행이 인출 된 마지막 이후 모든 사용자가 행에 수행 된 모든 업데이트를 검색할 수 있습니다. (이에 적용 됩니다 업데이트, 삭제 또는 삽입에 없습니다.) 드라이버 행 상태에 SQL_ROW_UPDATED 플래그를 반환할 수 있습니다 때 배열 **SQLFetchScroll** 호출 됩니다. 그렇지 않은 경우 "N"입니다.  
  
 SQL_SCHEMA_TERM (ODBC 1.0)  
 스키마;에 대 한 데이터 원본 공급 업체의 이름 가진 문자열 예를 들어 "소유자", "권한 부여 ID" 또는 "스키마"입니다.  
  
 위쪽, 아래쪽, 또는 혼합의 경우 문자열을 반환할 수 있습니다.  
  
 S Q l 92 항목 수준 – 준수 드라이버 항상 "스키마"를 반환 합니다.  
  
 이 *정보 항목* ODBC 2.0에서 ODBC 3.0에 대 한 이름을 바꾼 *정보 항목* SQL_OWNER_TERM 합니다.  
  
 SQL_SCHEMA_USAGE (ODBC 2.0)  
 스키마를 사용할 수 있는 문을 열거 SQLUINTEGER 비트 마스크:  
  
 SQL_SU_DML_STATEMENTS = 스키마는 모든 데이터 조작 언어 문에서 지원: **선택**, **삽입**, **업데이트**, **DELETE**, 지원 및 **업데이트에 대 한 선택** 및 배치 update 및 delete 문입니다.  
 ...
 SQL_SU_PRIVILEGE_DEFINITION = 모든 권한 정의 문에 스키마는 지원: **GRANT** 및 **해지**합니다.  
  
 S Q l 92 항목 수준 – 준수 드라이버 지원 SQL_SU_DML_STATEMENTS, SQL_SU_TABLE_DEFINITION, 및 SQL_SU_PRIVILEGE_DEFINITION 옵션을 항상 반환 됩니다.  
  
 이 *정보 항목* ODBC 2.0에서 ODBC 3.0에 대 한 이름을 바꾼 *정보 항목* SQL_OWNER_USAGE 합니다.  
 ...
 다음 비트 마스크 옵션이 지원 되는지 확인 하는 데 사용 됩니다.  
  
 SQL_SO_FORWARD_ONLY = 앞으로 커서만 스크롤합니다. (ODBC 1.0)  
  
 SQL_SO_STATIC 데이터 = 결과 집합은 정적입니다. (ODBC 2.0)  
  
 SQL_SO_KEYSET_DRIVEN = 드라이버 저장 하 고 결과 집합의 모든 행에 대 한 키를 사용 하 여 합니다. (ODBC 1.0)  
  
 SQL_SO_DYNAMIC = 드라이버 유지 (키 집합 크기는 행 집합 크기와 동일) 행 집합의 모든 행에 대 한 키입니다. (ODBC 1.0)  
  
 SQL_SO_MIXED = 드라이버 유지 키, 키 집합 및 키 집합 크기의 모든 행이 행 집합 크기 보다 큰에 대 한 합니다. 키 집합 내 키 집합 커서와 키 집합 외부 동적 커서가 있습니다. (ODBC 1.0)  
  
 스크롤 가능 커서에 대 한 정보를 참조 하십시오. [스크롤 가능 커서](../../../odbc/reference/develop-app/scrollable-cursors.md)합니다.  
  
 SQL_SEARCH_PATTERN_ESCAPE (ODBC 1.0)  
 검색 패턴에서 유효한 문자로 패턴 일치 메타 밑줄 (_) 및 백분율 기호 (%)의 사용을 허용 하는 이스케이프 문자로 드라이버 지원 지정 하는 문자열입니다. 이 이스케이프 문자 검색 문자열을 지 원하는 해당 카탈로그 함수 인수에만 적용 됩니다. 이 문자열이 비어 있으면 드라이버는 검색 패턴 이스케이프 문자를 지원 하지 않습니다.  
  
 이 정보 유형을 일반 지원에서 이스케이프 문자를 인식 하지 못하기 때문에 **같은** 조건자 SQL 92는이 문자열에 대 한 요구 사항입니다.  
  
 이 *정보 항목* 카탈로그 함수로 제한 됩니다. 검색 패턴 문자열에는 이스케이프 문자 사용에 대 한 참조 [패턴 값 인수](../../../odbc/reference/develop-app/pattern-value-arguments.md)합니다.  
  
 SQL_SERVER_NAME (ODBC 1.0)  
 에 실제 데이터 원본에 따른 특정 서버 이름, 문자열 데이터 원본 이름을 사용 하는 동안 때 유용한 **SQLConnect**, **SQLDriverConnect**, 및 **SQLBrowseConnect**합니다.  
  
 SQL_SPECIAL_CHARACTERS (ODBC 2.0)  
 테이블 이름, 열 이름 또는 인덱스 이름, 데이터 원본에 같은 식별자 이름에 사용할 수 있는 모든 특수 문자 (즉, a ~ z, A ~ Z, 0 ~ 9 및 밑줄을 제외한 모든 문자)를 포함 하는 문자열입니다. 예를 들어 "#$^"입니다. 식별자에 이러한 문자가 하나 이상 있으면, 식별자에 구분된 식별자 여야 합니다.  
  
 SQL_SQL_CONFORMANCE (ODBC 3.0)  
 SQL 92 드라이버에서 지원 수준을 표시 하는 SQLUINTEGER 값:  
  
 SQL_SC_SQL92_ENTRY = 항목 수준 sql-92 규정을 준수 합니다.  
  
 SQL_SC_FIPS127_2_TRANSITIONAL = FIPS 127-2 전환 수준 규정을 준수 합니다.  
  
 SQL_SC_SQL92_FULL = 전체 수준 sql-92 규정을 준수 합니다.  
  
 SQL_SC_ SQL92_INTERMEDIATE = 중간 수준 sql-92 규정을 준수 합니다.  
  
 SQL_SQL92_DATETIME_FUNCTIONS(ODBC 3.0)  
 S Q l-92에 정의 된 대로 드라이버와 관련 된 데이터 원본에서 지원 되는 날짜/시간 스칼라 함수를 열거 SQLUINTEGER 비트 마스크입니다.  
 ...
 외래 키에 대 한 지원 되는 규칙을 열거 하는 SQLUINTEGER 비트 마스크는 **삭제** 문, s Q l-92에 정의 된 대로 합니다.  
  
 다음 비트 마스크는 어떤 절은 데이터 원본에 의해 지원 되는지 확인 하는 데 사용 됩니다.  
 ...
 외래 키에 대 한 지원 되는 규칙을 열거 하는 SQLUINTEGER 비트 마스크는 **업데이트** 문, s Q l-92에 정의 된 대로 합니다.  
 ...
 지원 되는 절을 열거 하는 SQLUINTEGER 비트 마스크는 **GRANT** 문, s Q l-92에 정의 된 대로 합니다.  
  
 이 기능을 지원 해야 하는 SQL 92 또는 FIPS 규격 수준 각 비트 마스크 옆 괄호 안에 표시 됩니다.  
  
 다음 비트 마스크는 어떤 절은 데이터 원본에 의해 지원 되는지 확인 하는 데 사용 됩니다.  
  
 (항목 수준) SQL_SG_DELETE_TABLE SQL_SG_INSERT_COLUMN (중간 수준) (항목 수준) SQL_SG_INSERT_TABLE SQL_SG_REFERENCES_TABLE (항목 수준) (항목 수준) SQL_SG_REFERENCES_COLUMN SQL_SG_SELECT_TABLE (항목 수준) SQL_SG_UPDATE_COLUMN 드 ( 항목 수준) (항목 수준) SQL_SG_UPDATE_TABLE SQL_SG_USAGE_ON_DOMAIN (FIPS 전환 수준) (FIPS 전환 수준) SQL_SG_USAGE_ON_CHARACTER_SET SQL_SG_USAGE_ON_COLLATION (FIPS 전환 수준) SQL_SG_USAGE_ON_TRANSLATION FIPS ( 전환 수준) SQL_SG_WITH_GRANT_OPTION (항목 수준)  
  
 SQL_SQL92_NUMERIC_VALUE_FUNCTIONS(ODBC 3.0)  
 S Q l-92에 정의 된 대로 드라이버와 관련 된 데이터 원본에서 지원 되는 숫자 값 스칼라 함수를 열거 SQLUINTEGER 비트 마스크입니다.  
 ...
 지원 되는 조건자를 열거 하는 SQLUINTEGER 비트 마스크는 **선택** 문, s Q l-92에 정의 되어 있습니다.  
  
 이 기능을 지원 해야 하는 SQL 92 또는 FIPS 규격 수준 각 비트 마스크 옆 괄호 안에 표시 됩니다.  
  
 다음 비트 마스크 옵션은 데이터 원본에 의해 지원 되는지 확인 하는 데 사용 됩니다.  
  
 (항목 수준) SQL_SP_BETWEEN SQL_SP_COMPARISON (항목 수준) SQL_SP_EXISTS (항목 수준) SQL_SP_IN (항목 수준) SQL_SP_ISNOTNULL (항목 수준) SQL_SP_ISNULL (항목 수준) SQL_SP_LIKE (항목 수준) SQL_SP_MATCH_FULL (전체 수준) SQL_SP_MATCH_PARTIAL (전체 수준) (전체 수준) SQL_SP_MATCH_UNIQUE_FULL SQL_SP_MATCH_UNIQUE_PARTIAL (전체 수준) SQL_SP_OVERLAPS (FIPS 전환 수준) (항목 수준) SQL_SP_QUANTIFIED_COMPARISON SQL_SP_UNIQUE (항목 수준)  
  
 SQL_SQL92_RELATIONAL_JOIN_OPERATORS(ODBC 3.0)  
 지원 되는 관계형 조인 연산자를 열거 하는 SQLUINTEGER 비트 마스크는 **선택** 문, s Q l-92에 정의 된 대로 합니다.  
  
 이 기능을 지원 해야 하는 SQL 92 또는 FIPS 규격 수준 각 비트 마스크 옆 괄호 안에 표시 됩니다.  
  
 다음 비트 마스크 옵션은 데이터 원본에 의해 지원 되는지 확인 하는 데 사용 됩니다.  
  
 (전체 수준) SQL_SRJO_CROSS_JOIN SQL_SRJO_EXCEPT_JOIN (중간 수준) SQL_SRJO_FULL_OUTER_JOIN (중간 수준) SQL_SRJO_INNER_JOIN (FIPS 전환 수준) SQL_SRJO_INTERSECT_JOIN SQL_SRJO_CORRESPONDING_CLAUSE (중간 수준) (중간 수준) SQL_SRJO_LEFT_OUTER_JOIN (FIPS 전환 수준) (FIPS 전환 수준) SQL_SRJO_NATURAL_JOIN SQL_SRJO_RIGHT_OUTER_JOIN (FIPS 전환 수준) SQL_SRJO_UNION_JOIN (전체 수준)  
  
 SQL_SRJO_INNER_JOIN에 대 한 지원을 나타냅니다는 **내부 조인** 구문에는 내부 조인 기능이 없습니다. 에 대 한 지원의 **내부 조인** 반면의 구문은 FIPS 전환 내부 조인 기능에 대 한 지원은 **항목**합니다.  
  
 SQL_SQL92_REVOKE(ODBC 3.0)  
 지원 되는 절을 열거 하는 SQLUINTEGER 비트 마스크는 **해지** 문, 데이터 원본에서 지 원하는 SQL-92에 정의 되어 있습니다.  
 ...
 지원 되는 행 값 생성자 식을 열거 하는 SQLUINTEGER 비트 마스크는 **선택** 문, s Q l-92에 정의 되어 있습니다. 다음 비트 마스크 옵션은 데이터 원본에 의해 지원 되는지 확인 하는 데 사용 됩니다.  
  
 SQL_SRVC_VALUE_EXPRESSION SQL_SRVC_NULL SQL_SRVC_DEFAULT SQL_SRVC_ROW_SUBQUERY  
  
 SQL_SQL92_STRING_FUNCTIONS(ODBC 3.0)  
 S Q l-92에 정의 된 대로 드라이버와 관련 된 데이터 원본에서 지 원하는 문자열 스칼라 함수를 열거 SQLUINTEGER 비트 마스크입니다.  
  
 다음 비트 마스크는 지원 되는 문자열 함수를 결정 하는 데 사용 됩니다.  
 ...
 S Q l-92에 정의 된 대로 지원 값 식 열거 SQLUINTEGER 비트 마스크입니다.  
 ...
 드라이버 준수 하는 CLI 표준 또는 표준 열거 SQLUINTEGER 비트 마스크입니다. 다음 비트 마스크는 드라이버 준수 하는 수준을 결정 하기 위해 사용 됩니다.  
  
 SQL_SCC_XOPEN_CLI_VERSION1: 드라이버 Open 그룹 CLI 버전 1 준수합니다.  
  
 SQL_SCC_ISO92_CLI: 드라이버 ISO 92 CLI 준수합니다.  
  
 SQL_STATIC_CURSOR_ATTRIBUTES1(ODBC 3.0)  
 드라이버에서 지원 되는 정적 커서의 특성을 설명 하는 SQLUINTEGER 비트 마스크입니다. 특성의 첫 번째 하위 집합을 포함 하는이 비트 마스크 두 번째 하위 집합 SQL_STATIC_CURSOR_ATTRIBUTES2를 참조 하십시오.  
 ...
 이러한 비트 마스크의 설명에 대 한 SQL_DYNAMIC_CURSOR_ATTRIBUTES1 (및 참조 설명에 "동적 커서"에 대 한 "정적 커서" 대신).  
  
 S Q l 92 중간 수준 – 준수 드라이버 일반적으로 지원 반환 됩니다 SQL_CA1_NEXT, SQL_CA1_ABSOLUTE, 및 SQL_CA1_RELATIVE 옵션, 드라이버가 포함 된 SQL FETCH 문을 통해 스크롤 가능 커서를 지원 하기 때문에 있습니다. 하지만이 확인 되지 않으므로 직접 기본 SQL 지원, 스크롤 가능 커서 수 지원 되지 SQL 92 중간 수준 – 준수 드라이버에 대해서도 합니다.  
  
 SQL_STATIC_CURSOR_ATTRIBUTES2(ODBC 3.0)  
 드라이버에서 지원 되는 정적 커서의 특성을 설명 하는 SQLUINTEGER 비트 마스크입니다. 특성의 두 번째 하위 집합을 포함 하는이 비트 마스크 첫 번째 하위 집합 SQL_STATIC_CURSOR_ATTRIBUTES1를 참조 하십시오.  
  
 다음 비트 마스크는 특성에서 지원 되는지 확인 하는 데 사용 됩니다.  
  
 SQL_CA2_READ_ONLY_CONCURRENCYSQL_CA2_LOCK_CONCURRENCYSQL_CA2_OPT_ROWVER_CONCURRENCYSQL_CA2_OPT_VALUES_CONCURRENCYSQL_CA2_SENSITIVITY_ADDITIONSSQL_CA2_SENSITIVITY_DELETIONSSQL_CA2_SENSITIVITY_UPDATESSQL_CA2_MAX_ROWS_SELECTSQL_ CA2_MAX_ROWS_INSERTSQL_CA2_MAX_ROWS_DELETESQL_CA2_MAX_ROWS_UPDATESQL_CA2_MAX_ROWS_CATALOGSQL_CA2_MAX_ROWS_AFFECTS_ALLSQL_CA2_CRC_EXACTSQL_CA2_CRC_APPROXIMATESQL_CA2_SIMULATE_NON_UNIQUESQL_CA2_SIMULATE_TRY_UNIQUESQL_CA2_ SIMULATE_UNIQUE  
  
 이러한 비트 마스크의 설명에 대 한 SQL_DYNAMIC_CURSOR_ATTRIBUTES2 (및 참조 설명에 "동적 커서"에 대 한 "정적 커서" 대신).  
  
 SQL_STRING_FUNCTIONS (ODBC 1.0)  
 참고: 정보 유형은; ODBC 1.0에 도입 된 각 비트 마스크 도입 된 버전으로 표시 됩니다.  
  
 드라이버와 관련 된 데이터 원본에서 지 원하는 스칼라 문자열 함수를 열거 SQLUINTEGER 비트 마스크입니다.  
  
 다음 비트 마스크는 지원 되는 문자열 함수를 결정 하는 데 사용 됩니다.  
  
 SQL_FN_STR_ASCII (ODBC 1.0) SQL_FN_STR_BIT_LENGTH (ODBC 3.0) (ODBC 1.0) SQL_FN_STR_CHAR SQL_FN_STR_CHAR_LENGTH (ODBC 3.0) (ODBC 3.0) SQL_FN_STR_CHARACTER_LENGTH SQL_FN_STR_CONCAT (ODBC 1.0) (ODBC 2.0) SQL_FN_STR_DIFFERENCE SQL_FN_STR_INSERT (ODBC 1.0) SQL_FN_STR_LCASE (ODBC 1.0) SQL_FN_STR_LEFT (ODBC 1.0) SQL_FN_STR_LENGTH (ODBC 1.0) (ODBC 1.0) SQL_FN_STR_LOCATE SQL_FN_STR_LTRIM (ODBC 1.0) (ODBC 3.0) SQL_FN_STR_OCTET_LENGTH SQL_FN_STR_POSITION (ODBC 3.0) (ODBC 1.0) SQL_FN_STR_REPEAT SQL_FN_ STR_REPLACE (ODBC 1.0) (ODBC 1.0) SQL_FN_STR_RIGHT SQL_FN_STR_RTRIM (ODBC 1.0) (ODBC 2.0) SQL_FN_STR_SOUNDEX SQL_FN_STR_SPACE (ODBC 2.0) (ODBC 1.0) SQL_FN_STR_SUBSTRING SQL_FN_STR_UCASE (ODBC 1.0)  
  
 응용 프로그램에서 호출할 수 있는 경우는 **찾기** 와 스칼라 함수는 *string_exp1*, *string_exp2*, 및 *시작* 인수, 드라이버 SQL_FN_STR_LOCATE 비트 마스크를 반환합니다. 응용 프로그램에 사용 하 여 찾기 스칼라 함수 호출 수는 *string_exp1* 및 *string_exp2* 인수, 드라이버 SQL_FN_STR_LOCATE_2 비트 마스크를 반환 합니다. 완전 하 게 지원 드라이버는 **찾기** 스칼라 함수는 모두를 나타내는 비트 마스크를 반환 합니다.  
  
 (자세한 내용은 참조 [문자열 함수](../../../odbc/reference/appendixes/string-functions.md) 부록 E "스칼라 작동 합니다.")  
  
 SQL_SUBQUERIES (ODBC 2.0)  
 열거 하는 하위 쿼리를 지원 하는 조건자 SQLUINTEGER 비트 마스크:  
  
 SQL_SQ_CORRELATED_SUBQUERIESSQL_SQ_COMPARISONSQL_SQ_EXISTSSQL_SQ_INSQL_SQ_QUANTIFIED  
  
 SQL_SQ_CORRELATED_SUBQUERIES 비트 마스크 하위 쿼리를 지 원하는 모든 조건부 상관된 하위 쿼리를 지원 하는지 나타냅니다.  
  
 S Q l 92 항목 수준 – 준수 드라이버 항상 모든 이러한 비트가 설정 되는 비트 마스크를 반환 합니다.  
  
 SQL_SYSTEM_FUNCTIONS (ODBC 1.0)  
 드라이버와 관련 된 데이터 원본에서 지 원하는 스칼라 시스템 함수를 열거 SQLUINTEGER 비트 마스크입니다.  
 ...
 테이블에 대 한 데이터 원본 공급 업체의 이름 가진 문자열 예를 들어 "table" 또는 "file"입니다.  
  
 이 문자열은 위쪽, 아래쪽, 또는 혼합 대 소문자 될 수 있습니다.  
  
 S Q l 92 항목 수준 – 준수 드라이버에는 "table" 항상 반환 됩니다.  
  
 SQL_TIMEDATE_ADD_INTERVALS (ODBC 2.0)  
 드라이버와 TIMESTAMPADD 스칼라 함수에 대 한 연결 된 데이터 원본에서 지원 되는 타임 스탬프 간격 열거 SQLUINTEGER 비트 마스크입니다.  
  
 다음 비트 마스크는 간격으로 지원 되는지 확인 하는 데 사용 됩니다.  
 ...
 FIPS 전환 수준 – 준수 드라이버는 항상 모든 이러한 비트가 설정 되는 비트 마스크를 반환 합니다.  
  
 SQL_TIMEDATE_DIFF_INTERVALS (ODBC 2.0)  
 드라이버와 TIMESTAMPDIFF 스칼라 함수에 대 한 연결 된 데이터 원본에서 지원 되는 타임 스탬프 간격 열거 SQLUINTEGER 비트 마스크입니다.  
 ...
 스칼라 날짜 및 시간 함수 드라이버와 관련 된 데이터 원본에서 지 원하는 열거 SQLUINTEGER 비트 마스크입니다.  
  
 다음 비트 마스크는 지원 되는 날짜 및 시간 함수를 결정 하는 데 사용 됩니다.  
  
 SQL_FN_TD_CURRENT_DATE ODBC 3.0) (ODBC 3.0) SQL_FN_TD_CURRENT_TIME SQL_FN_TD_CURRENT_TIMESTAMP (ODBC 3.0) (ODBC 1.0) SQL_FN_TD_CURDATE SQL_FN_TD_CURTIME (ODBC 1.0) (ODBC 2.0) SQL_FN_TD_DAYNAME SQL_FN_TD_DAYOFMONTH (ODBC 1.0) SQL_FN_TD_DAYOFWEEK 드 ( ODBC 1.0) SQL_FN_TD_DAYOFYEAR (ODBC 1.0) SQL_FN_TD_EXTRACT (ODBC 3.0) (ODBC 1.0) SQL_FN_TD_HOUR SQL_FN_TD_MINUTE (ODBC 1.0) (ODBC 1.0) SQL_FN_TD_MONTH SQL_FN_TD_MONTHNAME (ODBC 2.0) SQL_FN_TD_NOW (ODBC 1.0) (ODBC 1.0) SQL_FN_TD_QUARTER SQL_FN_TD_ 두 번째 (ODBC 1.0) (ODBC 2.0) SQL_FN_TD_TIMESTAMPADD SQL_FN_TD_TIMESTAMPDIFF (ODBC 2.0) (ODBC 1.0) SQL_FN_TD_WEEK SQL_FN_TD_YEAR (ODBC 1.0)  
  
 SQL_TXN_CAPABLE (ODBC 1.0)  
 참고: 정보 유형은; ODBC 1.0에 도입 된 각 반환 값은 이전에 도입 된 버전으로 레이블이 표시 됩니다.  
  
 트랜잭션 지원 드라이버 또는 데이터 원본에 설명 하는 SQLUSMALLINT 값:  
  
 SQL_TC_NONE = 트랜잭션이 지원 되지 않습니다. (ODBC 1.0)  
  
 SQL_TC_DML = 트랜잭션을 조작 DML (데이터 언어) 문만 포함 될 수 있습니다 (**선택**, **삽입**, **업데이트**, **삭제** ). 트랜잭션 원인에 오류가 발생 하는 데이터 정의 언어 (DDL) 문을. (ODBC 1.0)  
  
 SQL_TC_DDL_COMMIT = 트랜잭션을 DML 문으로 포함 될 수 있습니다. DDL 문 (**CREATE TABLE**, **DROP INDEX**등) 트랜잭션을 커밋할 수 트랜잭션 원인에 발생 합니다. (ODBC 2.0)  
  
 SQL_TC_DDL_IGNORE = 트랜잭션을 DML 문으로 포함 될 수 있습니다. 트랜잭션에서 발생 하는 DDL 문이 무시 됩니다. (ODBC 2.0)  
  
 SQL_TC_ALL = DDL 문과 DML 문은 순서에 관계 없이 트랜잭션을 포함할 수 있습니다. (ODBC 1.0)  
  
 (트랜잭션 지원 SQL 92에서는 필수 이기 때문에 s Q l 92와 호환 되 드라이버 [모든 수준] 반환 하지 않습니다 SQL_TC_NONE.)  
  
 SQL_TXN_ISOLATION_OPTION (ODBC 1.0)  
 드라이버 또는 데이터 원본에서 사용할 수 있는 트랜잭션 격리 수준 열거 SQLUINTEGER 비트 마스크입니다.  
  
 다음 비트 마스크 옵션이 지원 되는지 확인 하는 플래그와 함께 사용 됩니다.  
  
 SQL_TXN_READ_UNCOMMITTEDSQL_TXN_READ_COMMITTEDSQL_TXN_REPEATABLE_READSQL_TXN_SERIALIZABLE  
  
 격리 수준 설명은 SQL_DEFAULT_TXN_ISOLATION에 대 한 설명을 참조 하십시오.  
  
 트랜잭션 격리 수준의 설정 하려면 응용 프로그램이 호출 **SQLSetConnectAttr** SQL_ATTR_TXN_ISOLATION 특성을 설정 합니다. 자세한 내용은 참조 [SQLSetConnectAttr 함수](../../../odbc/reference/syntax/sqlsetconnectattr-function.md)합니다.  
  
 S Q l 92 항목 수준 – 준수 드라이버 지원 SQL_TXN_SERIALIZABLE을 항상 반환 됩니다. FIPS 전환 수준 – 준수 드라이버는 항상 반환 이러한 모든 옵션 지원.  
  
 SQL_UNION (ODBC 2.0)  
 에 대 한 지원을 열거 하는 SQLUINTEGER 비트 마스크는 **UNION** 절:  
  
 SQL_U_UNION 데이터 원본에서 지 원하는 =는 **UNION** 절.  
  
 SQL_U_UNION_ALL 데이터 원본에서 지 원하는 =는 **모든** 키워드는 **UNION** 절. (**SQLGetInfo** SQL_U_UNION와 SQL_U_UNION_ALL이 경우에 반환 합니다.)  
  
 SQL 92 항목 수준 – 준수 드라이버는 항상 반환 두이 옵션 모두 지원.  
  
 SQL_USER_NAME (ODBC 1.0)  
 로그인 이름과 다를 수 있는 특정 데이터베이스에 사용 된 문자 문자열입니다.  
  
 SQL_XOPEN_CLI_YEAR (ODBC 3.0)  
 게시 된 버전의 ODBC 드라이버 관리자 완벽 하 게 준수 하는 Open Group 사양의 연도 나타내는 문자열입니다.  
 ...
 SQL_ALTER_DOMAIN (ODBC 3.0)  
 절을 열거 하는 SQLUINTEGER 비트 마스크는 **ALTER 도메인** 문, 데이터 원본에서 지 원하는 SQL-92에 정의 된 대로 합니다. Sql-92 Full 수준 – 규격 드라이버는 비트 마스크의 모든 항상 반환 됩니다. 반환 값이 "0"을 의미 하는 **ALTER 도메인** 문이 지원 되지 않습니다.  
...
## <a name="example"></a>예제  
 **SQLGetInfo** 는 SQLUINTEGER 비트 마스크에 지원 되는 옵션의 목록을 반환 **InfoValuePtr*합니다. 각 옵션에 대 한 비트 마스크 옵션이 지원 되는지 여부를 결정 하는 플래그와 함께 사용 됩니다.  
  
 예를 들어 응용 프로그램 SUBSTRING 스칼라 함수는 연결과 관련 된 드라이버에 의해 지원 되는지 여부를 결정 하 다음 코드를 사용할 수 없습니다.  
  
 사용 하 여의 또 다른 예에 대 한 **SQLGetInfo**, 참조 [SQLTables 함수](../../../odbc/reference/syntax/sqltables-function.md)합니다.  
