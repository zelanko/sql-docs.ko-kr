---
description: SQLWritePrivateProfileString 함수
 ...
 **요약**  
 **SQLWritePrivateProfileString** 는 시스템 정보의 Odbc.ini 하위 키에 값 이름과 데이터를 씁니다.  
 ...
 **SQLWritePrivateProfileString** 는 Microsoft® windows®에서 MICROSOFT windows NT®/windows 2000으로 드라이버 및 드라이버 설치 dll을 이식 하는 간단한 방법으로 제공 됩니다. Odbc.ini 파일에 프로필 문자열을 기록 하는 **WritePrivateProfileString** 에 대 한 호출은 **SQLWritePrivateProfileString**에 대 한 호출로 바꾸어야 합니다. **SQLWritePrivateProfileString** 는 WIN32® API의 함수를 호출 하 여 지정 된 값 이름과 데이터를 시스템 정보의 Odbc.ini 하위 키에 추가 합니다.  
