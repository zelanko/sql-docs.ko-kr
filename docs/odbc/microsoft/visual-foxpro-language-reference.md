---
description: Visual FoxPro 언어 참조
title: Visual FoxPro 언어 참조 | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
helpviewer_keywords:
- commands for FoxPro ODBC driver [ODBC]
- Visual FoxPro ODBC driver [ODBC], commands
ms.assetid: 8e052213-ca6f-44ca-917e-054ecb3eb0e5
author: David-Engel
ms.author: v-daenge
ms.openlocfilehash: d5420e0b5f91324efdd0746c6da22c3a4e768d7b
ms.sourcegitcommit: e700497f962e4c2274df16d9e651059b42ff1a10
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/17/2020
ms.locfileid: "88421537"
---
# <a name="visual-foxpro-language-reference"></a>Visual FoxPro 언어 참조
이 섹션에서는 다음 항목을 다룹니다.  
  
-   [ALTER TABLE - SQL 명령](../../odbc/microsoft/alter-table-sql-command.md)  
  
-   [CREATE TABLE - SQL 명령](../../odbc/microsoft/create-table-sql-command.md)  
  
-   [DELETE - SQL 명령](../../odbc/microsoft/delete-sql-command.md)  
  
-   [DELETE TAG 명령](../../odbc/microsoft/delete-tag-command.md)  
  
-   [DROP TABLE 명령](../../odbc/microsoft/drop-table-command.md)  
  
-   [INDEX 명령](../../odbc/microsoft/index-command.md)  
  
-   [INSERT - SQL 명령](../../odbc/microsoft/insert-sql-command.md)  
  
-   [SELECT - SQL 명령](../../odbc/microsoft/select-sql-command.md)  
  
-   [SET ANSI 명령](../../odbc/microsoft/set-ansi-command.md)  
  
-   [SET BLOCKSIZE 명령](../../odbc/microsoft/set-blocksize-command.md)  
  
-   [SET COLLATE 명령](../../odbc/microsoft/set-collate-command.md)  
  
-   [SET DELETED 명령](../../odbc/microsoft/set-deleted-command.md)  
  
-   [SET EXACT 명령](../../odbc/microsoft/set-exact-command.md)  
  
-   [SET EXCLUSIVE 명령](../../odbc/microsoft/set-exclusive-command.md)  
  
-   [SET NULL 명령](../../odbc/microsoft/set-null-command.md)  
  
-   [SET PATH 명령](../../odbc/microsoft/set-path-command.md)  
  
-   [SET REPROCESS 명령](../../odbc/microsoft/set-reprocess-command.md)  
  
-   [SET UNIQUE 명령](../../odbc/microsoft/set-unique-command.md)  
  
-   [UPDATE - SQL 명령](../../odbc/microsoft/update-sql-command.md)  
  
-   [Visual FoxPro 필드 데이터 형식](../../odbc/microsoft/visual-foxpro-field-data-types.md)
