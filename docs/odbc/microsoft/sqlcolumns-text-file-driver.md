---
description: SQLColumns(텍스트 파일 드라이버)
title: SQLColumns (텍스트 파일 드라이버) | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
ms.locfileid: "88412029"
---
# <a name="sqlcolumns-text-file-driver"></a>SQLColumns(텍스트 파일 드라이버)
> [!NOTE]  
...
|열|주석|  
|------------|--------------|  
|TABLE_QUALIFIER|디렉터리에 대 한 경로가 반환 됩니다.|  
|TABLE_OWNER|소유자 이름이 지원 되지 않으므로이 열에 NULL이 반환 됩니다.|  
|NULLABLE|기본 키 또는 고유 인덱스에 참여 하는 열에 대 한 SQL_NO_NULLS 반환 됩니다.|
