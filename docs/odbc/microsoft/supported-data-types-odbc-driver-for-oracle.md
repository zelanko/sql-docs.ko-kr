---
title: 지원되는 데이터 유형(오라클용 ODBC 드라이버) | 마이크로 소프트 문서
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
>  이 기능은 이후 버전의 Windows에서 제거됩니다. 새 개발 작업에서는 이 기능을 사용하지 않도록 하고, 현재 이 기능을 사용하는 애플리케이션은 수정하세요. 대신 오라클에서 제공하는 ODBC 드라이버를 사용합니다.  
 ...
 오라클용 ODBC 드라이버는 모든 Oracle 7.3 데이터 유형을 지원합니다. 그러나 여기에 나열된 새로운 Oracle8 데이터 형식은 지원하지 않습니다.  
...
|NCLOB|해당 없음|지원 안 함|  
|NUMBER|지원됨|지원됨|  
|NVARCHAR2|해당 없음|지원 안 함|  
|RAW|지원됨|지원됨|  
|VARCHAR2|지원됨|지원됨|  
|MLS라벨|지원되지 않습니다.|지원되지 않습니다.|  
