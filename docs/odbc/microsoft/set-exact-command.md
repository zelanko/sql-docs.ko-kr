...
## <a name="string-comparisons"></a>문자열 비교  
 ...
 그러나 문자 식을 = 연산자와 비교할 때 결과가 정확 하지 않을 수 있습니다. 식의 오른쪽에 있는 식의 끝에 도달할 때까지 (정확히 OFF 설정) 또는 두 식의 끝에 도달할 때까지 문자 식은 왼쪽에서 오른쪽으로 문자를 비교 합니다. (정확 하 게 설정)에 도달 했습니다.  
...
|----------------|------------------|-----------------|--------------------------|  
|"abc" = "abc"|일치|일치|일치|  
|"ab" = "abc"|일치 하지 않음|일치 하지 않음|일치 하지 않음|  
|"abc" = "ab"|일치|일치 하지 않음|일치 하지 않음|  
|"abc" = "ab_"|일치 하지 않음|일치 하지 않음|일치 하지 않음|  
|"ab" = "ab_"|일치 하지 않음|일치|일치 하지 않음|  
|"ab_" = "ab"|일치|일치|일치 하지 않음|  
|"" = "ab"|일치 하지 않음|일치 하지 않음|일치 하지 않음|  
|"ab" = ""|일치|일치 하지 않음|일치 하지 않음|  
|"__" = ""|일치|일치|일치 하지 않음|  
|"" = "___"|일치 하지 않음|일치|일치 하지 않음|  
|TRIM ("___") = ""|일치|일치|일치|  
|"" = TRIM ("___")|일치|일치|일치|  
