...
테이블 이름에는 모든 유효한 문자 (예: 공백)를 사용할 수 있습니다. 문자, 숫자 및 밑줄을 제외한 모든 문자를 포함 하는 테이블 이름에는 백슬래시 (')로 구분 하 여 구분 해야 합니다.  
 ...
 에서 참조 하는 Microsoft Excel 테이블 이름은 \<filename> Microsoft excel 3.0 및 4.0 파일에 대해 지원 됩니다. 에서 참조 하는 Microsoft Excel 테이블 이름은 \<workbook-name> Microsoft excel 5.0, 7.0 또는 97 파일에 대해 지원 됩니다.  
