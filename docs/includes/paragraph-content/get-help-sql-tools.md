---
ms.date: 05/21/2019
ms.topic: include
ms.prod: sql
author: rothja
ms.author: jroth
ms.openlocfilehash: 7502743d17abbc24bdc23d29e6948a93f20b527f
ms.sourcegitcommit: c7f40918dc3ecdb0ed2ef5c237a3996cb4cd268d
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 10/05/2020
ms.locfileid: "91745526"
---
## <a name="image-typeicon-sourcemediainfo-tippng-borderfalse-get-help-for-sql-tools"></a>:::image type="icon" source="../media/info-tip.png" border="false"::: SQL 도구 도움말 보기

- [도움말을 사용하는 모든 방법](../../sql-server/sql-server-get-help.md)
- [Azure Data Studio Git 문제 제출](https://github.com/Microsoft/azuredatastudio/issues)
- [Azure Data Studio에 참여](https://github.com/Microsoft/azuredatastudio)
- [SQL 클라이언트 도구 포럼](https://social.msdn.microsoft.com/Forums/en-US/home?forum=sqltools&filter=alltypes&sort=lastpostdesc)
- [SQL Server Data Tools - MSDN 포럼](https://social.msdn.microsoft.com/Forums/en-US/home?forum=ssdt&filter=alltypes&sort=lastpostdesc)
- [비즈니스 사용자에 대한 지원 옵션](https://support.microsoft.com/gp/support-options-for-business)