...
1. 클러스터를 만듭니다. 클러스터를 만들려면 다음 명령을 실행합니다.
   ...
   **RHEL8**
   ...
   >[!NOTE]
   >이전에 같은 노드에서 클러스터를 구성한 경우 `--force`을 실행할 때 `pcs cluster setup` 옵션을 사용해야 합니다. 이 옵션은 `pcs cluster destroy`를 실행하는 것과 동일합니다. Pacemaker를 다시 사용하도록 설정하려면 `sudo systemctl enable pacemaker`를 실행합니다.
