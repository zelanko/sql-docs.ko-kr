...
description: SQL Server 구성 관리자에서 기본 파이프에 대한 설명을 보거나 수정하는 방법을 알아봅니다. 다른 파이프에 연결하는 방법을 알아봅니다.
