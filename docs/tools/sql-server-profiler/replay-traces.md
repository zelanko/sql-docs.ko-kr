...
ms.topic: "article"
helpviewer_keywords: 
  - "SQL Server 프로파일러, 추적 재생"
  - "커서까지 실행 옵션"
  - "중단점 설정/해제 옵션"
  - "추적 [SQL Server], 재생"
  - "추적 재생"
  - "재생 엔진 [SQL Server Profiler]"
  - "이벤트 [SQL Server], 추적 재생"
  - "프로파일러 [SQL Server 프로파일러], 추적 재생"
...
# 추적 재생
 ...
 추적 재생은 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] **재생** 메뉴의 **중단점 설정/해제** 및 **커서까지 실행** 옵션을 사용한 디버깅을 지원합니다. 이 옵션을 사용하면 추적 재생을 짧은 조각으로 분리하고 점차 큰 부분으로 분석 범위를 늘릴 수 있어 긴 스크립트를 분석할 때 유용합니다.  
