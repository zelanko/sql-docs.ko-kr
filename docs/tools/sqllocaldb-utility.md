...
ms.topic: "article"
helpviewer_keywords: 
  - "SqlLocalDB 유틸리티 [SQL Server]"
  - "로컬 데이터베이스 런타임 유틸리티"
  - "LocalDB, SqlLocalDB 유틸리티"
...
### 1. LocalDB의 인스턴스 만들기  
 다음 예에서는 [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] 바이너리를 사용하여 `DEPARTMENT`라는 [!INCLUDE[ssExpress](../includes/ssexpress-md.md)]**LocalDB**의 인스턴스를 만든 다음 인스턴스를 시작합니다.  
  
```  
SqlLocalDB.exe create "DEPARTMENT" 12.0 -s  
```  
  
### 2. LocalDB의 공유 인스턴스 작업  
...
[명령줄 관리 도구: SqlLocalDB.exe](Command-Line%20Management%20Tool:%20SqlLocalDB.exe.md)  
