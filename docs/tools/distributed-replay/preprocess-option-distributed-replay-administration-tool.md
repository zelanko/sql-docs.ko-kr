...
---
# 전처리 옵션(Distributed Replay Utility Administration Tool)
 ...
 **-i** *input_trace_file*  
 ...
 같은 디렉터리에 롤오버 파일이 있으면 자동으로 로드되어 사용됩니다. 파일은 파일 롤오버 명명 규칙을 따라야 합니다. 예를 들면 `Mytrace.trc`, `Mytrace_1.trc`, `Mytrace_2.trc`, `Mytrace_3.trc`…과 같습니다. `Mytrace_n.trc`.  
