...
# Save and Run Package (SQL Server Import and Export Wizard)
  데이터 원본 및 대상을 지정하고 구성하고 나면 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가져오기 및 내보내기 마법사에서 **패키지 저장 및 실행**을 표시합니다. 이 페이지에서는 복사 작업을 즉시 실행할지 여부를 지정합니다. 구성에 따라 마법사에서 만든 SSIS([!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]) 패키지를 저장하여 사용자 지정하고 나중에 다시 사용할 수도 있습니다.
 ...
 **SSIS 패키지 저장**  
 패키지를 저장합니다. 나중에 필요에 따라 패키지를 사용자 지정하고 다시 실행할 수 있습니다. 패키지 저장을 선택하는 경우 다음 페이지 **SSIS 패키지 저장**에 추가 옵션이 있습니다. 패키지를 저장하는 옵션은 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Standard Edition 이상이 설치된 경우에만 사용할 수 있습니다.   
  
> [!NOTE] 마법사를 종료하고 패키지를 실행한 다음 패키지 실행이 완료되기 전에 패키지를 중지하면 이 페이지에서 **저장** 확인란을 선택했어도 패키지가 저장되지 않습니다.  
  
## <a name="specify-options-for-saving-the-package"></a>패키지 저장 옵션 지정
**SQL Server**  
 **sysssispackages** 테이블의 **msdb** 데이터베이스에 패키지를 저장하려면 이 옵션을 선택합니다.
 
> [!NOTE] 이 옵션은 SSIS 카탈로그 데이터베이스(SSISDB)에 패키지를 저장하지 않습니다.  
 ...
 **파일 시스템**  
 패키지를 확장자가 **.dtsx**인 파일로 저장하려면 이 옵션을 선택합니다.  
 ...
 **패키지 보호 수준**  
 ...
 보호 수준은 보호 방법(암호 또는 사용자 키)과 패키지 보호의 범위를 결정합니다. 보호에는 모든 데이터를 포함시키거나 중요한 데이터만 포함시킬 수 있습니다. 사용 가능한 옵션에 대한 자세한 내용은 [패키지의 중요한 데이터에 대한 액세스 제어](../../integration-services/packages/access-control-for-sensitive-data-in-packages.md)를 참조하세요.  
...
> [!NOTE] 암호 옵션은 암호가 필요한 **패키지 보호 수준** 즉, **암호로 중요한 데이터 암호화** 또는 **암호로 모든 데이터 암호화**를 지정한 경우에만 사용할 수 있습니다.  

## <a name="can-i-run-and-save-the-package"></a>패키지를 실행하고 저장할 수 있나요?
 마법사를 시작하는 데 사용하는 방법이 마법사에서 만든 패키지를 실행하고 저장할 수 있는지 여부를 결정합니다. 경우에 따라 마법사의 이 페이지에서 **실행** 옵션 또는 **저장** 옵션을 사용할 수 없습니다.
 
|마법사를 시작하는 방법|패키지를 실행할 수 있나요?|패키지를 저장할 수 있나요?|
|------------------------|------------------------|-------------------------|
|시작 메뉴, 명령 프롬프트 또는 SQL Server Management Studio에서 마법사를 시작합니다.|마법사의 마지막 단계에서 **즉시 실행** 확인란을 선택하여 패키지를 즉시 실행할 수 있습니다. 기본적으로 이 확인란이 선택되어 있으므로 패키지가 즉시 실행됩니다.|SQL Server Standard Edition 이상이 설치된 경우 필요에 따라 패키지를 저장할 수 있습니다.|
|SSDT(SQL Server Data Tools)의 Integration Services 프로젝트에서 마법사를 시작합니다.|마법사가 종료될 때까지는 패키지를 실행할 수 없습니다. 그런 다음 SQL Server Data Tools에서 패키지를 실행할 수 있습니다.|마법사는 마법사를 시작한 Integration Services 프로젝트에서 패키지를 저장합니다.|
