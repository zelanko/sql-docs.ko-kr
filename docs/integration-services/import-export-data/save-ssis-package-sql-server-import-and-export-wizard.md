...
---
# SSIS 패키지 저장(SQL Server 가져오기 및 내보내기 마법사)
  **패키지 저장 및 실행** 페이지에서 마법사가 만든 SSIS 패키지를 저장하도록 지정한 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 가져오기 및 내보내기 마법사에서 **SSIS 패키지 저장**을 표시합니다. 이 페이지에서는 패키지 저장 옵션을 추가로 지정할 수 있습니다.  
...
다음 스크린샷은 **패키지 저장 및 실행** 페이지에서 **파일 시스템** 옵션을 선택한 후 마법사의 **SSIS 패키지 저장** 페이지를 보여 줍니다. 
...
## <a name="screen-shot---save-the-package-in-sql-server"></a>스크린샷 - SQL Server에 패키지 저장
 ...
 다음 스크린샷은 **패키지 저장 및 실행** 페이지에서 **SQL Server** 옵션을 선택한 후 마법사의 **SSIS 패키지 저장** 페이지를 보여 줍니다. 
...
## <a name="save-the-package-in-the-file-system"></a>파일 시스템에 패키지 저장 
파일 시스템 대상을 선택했으면 다음 필드에 정보를 입력합니다.

 **파일 이름**  
 대상 파일의 경로 및 파일 이름을 입력하거나 **찾아보기** 단추를 사용하여 대상을 선택합니다.  
  
> [!TIP] 입력하거나 검색하여 대상 폴더를 지정해야 합니다. 경로 없이 파일 이름만 입력하는 경우 마법사에서 패키지를 저장하는 위치를 알 수 없습니다. 또한 마법사에서 파일을 저장할 수 있는 권한이 없는 위치에 패키지를 저장하려고 시도하고 오류가 발생할 수도 있습니다.  
>   
>  패키지 파일의 저장 위치를 기억해 두세요.  
  
 **찾아보기**  
 필요에 따라 **패키지 저장** 대화 상자를 사용하여 대상 파일의 경로를 찾아서 선택합니다.  

## <a name="save-the-package-in-sql-server"></a>SQL Server에 패키지 저장 
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 대상을 선택했으면 다음 필드에 정보를 입력합니다.

마법사는 **sysssispackages** 테이블의 **msdb** 데이터베이스에 패키지를 저장합니다.
 ...
 **사용자 이름**  
SQL Server 인증을 지정한 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 사용자 이름을 입력합니다.  
  
 **암호**  
SQL Server 인증을 지정한 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 암호를 입력합니다.  
...
-   파일 시스템에 패키지를 저장한 경우 개발 환경에서 패키지를 실행하려면 [SQL Server Data Tools에서 패키지 실행](../../integration-services/packages/run-a-package-in-sql-server-data-tools.md)을 참조하세요. 먼저 Integration Services 프로젝트에 패키지를 추가해야 열어서 실행할 수 있습니다.  
