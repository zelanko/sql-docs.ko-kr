...
> [!IMPORTANT]
> 이 항목에서는 마법사를 **시작** 하는 방법만 설명합니다. 다른 내용을 찾으려면 [관련 태스크 및 콘텐츠](#related-tasks-and-content)를 참조하세요.

다음과 같은 방법으로 마법사를 시작할 수 있습니다.

- 지원되는 데이터 원본을 가져오거나 내보내려면 [시작 메뉴](#start-menu)를 참조하세요.
- 지원되는 데이터 원본을 가져오거나 내보내려면 [명령 프롬프트](#command-prompt)를 참조하세요.
- SQL Server로 가져오거나 SQL Server에서 내보내는 경우 [SSMS(SQL Server Management Studio)](#sql-server-management-studio-ssms)를 참조하세요.
- SQL Server로 가져오거나 SQL Server에서 내보내는 경우 [SSDT(SQL Server Data Tools)가 포함된 Visual Studio](#visual-studio)를 참조하세요.
...
1. **시작** 메뉴에서 **Microsoft SQL Server 20xx**를 찾아서 확장합니다.
2. 다음 옵션 중 하나를 클릭합니다.
    - **SQL Server 20xx 데이터 가져오기 및 내보내기(64비트)**
    - **SQL Server 20xx 데이터 가져오기 및 내보내기(32비트)**

    데이터 원본에 32비트 데이터 공급자가 필요한 것을 알고 있는 경우가 아니면 64비트 버전의 마법사를 실행합니다.

    ![마법사 시작 시작](../../integration-services/import-export-data/media/start-wizard-start-64.png)

## <a name="command-prompt"></a>명령 프롬프트
...
- **C:\Program Files\Microsoft SQL Server\140\DTS\Binn**(64비트 버전).
  - *140* = SQL Server 2017.  이 값은 사용하는 SQL Server 버전에 따라 달라집니다.

- **C:\Program Files (x86)\Microsoft SQL Server\140\DTS\Binn**(32비트 버전).
  - *140* = SQL Server 2017.  이 값은 사용하는 SQL Server 버전에 따라 달라집니다.

데이터 원본에 32비트 데이터 공급자가 필요한 것을 알고 있는 경우가 아니면 64비트 버전의 마법사를 실행합니다.

![마법사 시작 cmd](../../integration-services/import-export-data/media/start-wizard-cmd.png)
...
- **프로젝트** 메뉴에서 **SSIS가져오기 및 내보내기 마법사**를 클릭합니다.
   ...
   ![마법사 시작 프로젝트](../../integration-services/import-export-data/media/start-wizard-project.png)

   \- 또는-

- 솔루션 탐색기에서 **SSIS 패키지** 폴더를 마우스 오른쪽 단추로 클릭한 다음 **SSIS 가져오기 및 내보내기 마법사**를 클릭합니다.

    ![마법사 시작 패키지](../../integration-services/import-export-data/media/start-wizard-packages.png)
...
## <a name="related-tasks-and-content"></a>관련 작업 및 콘텐츠

다음은 몇 가지 다른 기본 작업입니다.

- **마법사의 작동 원리에 대한 빠른 예제를 참조하세요.**

  - **스크린샷을 보려면.** 단일 페이지([가져오기 및 내보내기 마법사의 간단한 예제 시작](../../integration-services/import-export-data/get-started-with-this-simple-example-of-the-import-and-export-wizard.md))에서 간단한 예제를 살펴봅니다.
