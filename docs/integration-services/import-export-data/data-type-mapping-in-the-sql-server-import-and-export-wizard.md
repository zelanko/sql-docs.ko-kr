...
비즈니스에서 데이터 형식 간에 다른 매핑을 필요로 하는 경우 매핑 파일을 업데이트하여 마법사가 사용하는 매핑을 변경할 수 있습니다. 예를 들어 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **** nchar **데이터 형식이 DB2** 데이터 형식 대신 DB2 **VAR데이터 형식이 DB2** GRAPHIC [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터 형식에 매핑되도록 하려면 **** 매핑 파일에서 **nchar** 매핑을 변경하여 **데이터 형식이 DB2** 대신 **VAR데이터 형식이 DB2.**에서 DB2로 데이터를 전송할 때  
