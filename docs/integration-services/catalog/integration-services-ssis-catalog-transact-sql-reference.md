---
description: Integration Service(SSIS) 카탈로그 Transact-SQL 참조
title: Integration Services 카탈로그 Transact-SQL 참조 | Microsoft Docs
ms.custom: ''
ms.date: 03/03/2017
ms.prod: sql
ms.prod_service: integration-services
ms.reviewer: ''
ms.technology: integration-services
ms.topic: conceptual
ms.assetid: 9553cef4-36aa-4088-a9db-eee45a4502df
author: chugugrace
ms.author: chugu
ms.openlocfilehash: 4334a18c00eb040307b5fa2116c9a3edb4a011d4
ms.sourcegitcommit: e700497f962e4c2274df16d9e651059b42ff1a10
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/17/2020
ms.locfileid: "88351989"
---
# <a name="integration-services-ssis-catalog-transact-sql-reference"></a>Integration Service(SSIS) 카탈로그 Transact-SQL 참조

[!INCLUDE[sqlserver-ssis](../../includes/applies-to-version/sqlserver-ssis.md)]


[보기&#40;Integration Services 카탈로그&#41;](../../integration-services/system-views/views-integration-services-catalog.md)  
  
 [저장 프로시저&#40;Integration Services 카탈로그&#41;](../../integration-services/system-stored-procedures/stored-procedures-integration-services-catalog.md)  
  
 [함수&#40;Integration Services 카탈로그&#41;](https://msdn.microsoft.com/library/9f2aec85-3d4c-415f-b1f8-8328a60b1c7f) 
