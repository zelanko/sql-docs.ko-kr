...
[!INCLUDE[ssis-appliesto](../../includes/ssis-appliesto-ssvrpluslinux-asdb-asdw-xxx.md)]
 ...
 디자인 타임에, [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)]는 패키지 실행을 일시 중지하는 중단점, 진행률 창 및 데이터 뷰어를 제공합니다. 중단점을 사용하면 데이터가 데이터 흐름을 통과하는 과정을 모니터링할 수 있습니다. 그러나 배포된 패키지를 실행할 때는 이러한 기능을 사용할 수 없습니다. 배포된 패키지의 문제를 해결하는 주요 방법은 다음과 같습니다.  
...
## <a name="catch-and-handle-package-errors-by-using-event-handlers"></a>이벤트 처리기를 사용하여 패키지 오류 파악 및 처리  
 이벤트 처리기를 사용하여, 패키지 및 패키지 개체에 의해 발생된 이벤트에 대응합니다.  
  
-   **OnError 이벤트의 이벤트 처리기 만들기** 이벤트 처리기에서 메일 보내기 작업을 사용하여 관리자에게 실패 사실을 알릴 수 있습니다. 스크립트 작업 및 사용자 지정 논리를 사용하여 문제 해결을 위한 시스템 정보를 가져오거나 임시 리소스와 완전하지 않은 출력을 정리합니다. 자세한 내용은 [Integration Services&#40;SSIS&#41; 이벤트 처리기](../../integration-services/integration-services-ssis-event-handlers.md)를 참조하세요.  
...
-   **오류 출력에 정보 추가** 오류 출력의 명확성을 개선하려면 오류 출력에서 제공하는 두 개의 숫자 식별자에 더해 오류 메시지와 열 이름을 추가합니다. 스크립트를 사용하여 이러한 두 개의 추가 열을 추가하는 방법에 대한 예는 [스크립트 구성 요소를 사용하여 오류 출력 향상](../../integration-services/extending-packages-scripting-data-flow-script-component-examples/enhancing-an-error-output-with-the-script-component.md)을 참조하세요.  
...
## <a name="troubleshoot-package-execution-by-using-logging"></a>로깅을 사용하여 패키지 실행 문제 해결  
 로깅을 사용하여, 실행 중인 패키지에서 발생하는 문제를 대부분 추적할 수 있습니다. 로그 공급자는 나중에 분석할 수 있도록 지정된 이벤트에 대한 정보를 캡처하고 해당 정보를 몇 가지 지원되는 형식으로 저장합니다.  지원되는 로그 공급자 형식에는 데이터베이스 테이블, 플랫 파일, XML 파일 등이 있습니다.  
...
-   **기본 로깅 출력 향상**. 로깅 기능은 일반적으로 패키지가 실행될 때마다 로깅 대상에 행을 추가합니다. 로깅 출력의 각 행이 패키지를 이름과 고유 식별자로 식별하고 패키지 실행을 고유 ExecutionID로 식별하더라도 단일 목록에 많은 양의 로깅 출력이 있으면 분석하기가 어려울 수 있습니다.  
    ...
    3.  **행 개수 데이터 캡처** 행 개수 정보에 대한 별도의 테이블을 만듭니다. 이 테이블에서 패키지 실행의 각 인스턴스는 ExecutionID로 식별됩니다. 행 개수 변환을 사용하여 데이터 흐름의 중요 지점에서 일련의 변수에 행 개수를 저장합니다. 나중에 분석 및 보고할 수 있도록 SQL 실행 작업을 추가하여 일련의 값을 이 테이블의 행에 삽입합니다.  
...
-   **알려진 잘못된 패키지 요소에 대해 DelayValidation 속성을 구성합니다**. 알려진 구성 문제가 있는 패키지 요소에서 유효성 검사 오류가 발생하지 않도록 하려면 **DelayValidation**을 **True**로 설정합니다. 예를 들어, 패키지는 런타임에 SQL 실행 작업을 통해 테이블이 만들어지기 전까지는 존재하지 않는 대상 테이블을 사용하는 데이터 흐름 태스크를 포함합니다. **DelayValidation** 속성은 패키지 수준 또는 패키지에 포함된 개별 태스크 및 컨테이너 수준에서 설정할 수 있습니다.  
...
## <a name="troubleshoot-errors-without-a-description"></a>설명이 없는 오류 문제 해결  
 설명이 없는 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 오류가 발생할 경우 [Integration Services Error and Message Reference](../../integration-services/integration-services-error-and-message-reference.md)(Integration Services 오류 및 메시지 참조)에서 해당 오류에 대한 설명을 찾을 수 있습니다. 이 목록에는 현재 문제 해결 정보는 들어 있지 않습니다.  
