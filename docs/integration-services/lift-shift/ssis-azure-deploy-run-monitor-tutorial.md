...
2. **원본 선택** 페이지에서 배포할 기존 SSIS 프로젝트를 선택합니다.
    ...
    -   SSIS 카탈로그에 있는 프로젝트를 배포하려면 **Integration Services 카탈로그** 를 선택한 다음 카탈로그에 있는 프로젝트의 서버 이름과 경로를 입력합니다. SQL Server에서 호스트하는 SSISDB에 상주하는 프로젝트만 이 단계에서 재배포할 수 있습니다.
