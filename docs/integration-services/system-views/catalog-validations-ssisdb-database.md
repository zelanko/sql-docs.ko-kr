...
|설명이 포함 되어|**nvarchar (128)**|프로젝트의 이름입니다.|  
|project_lsn|**bigint**|유효성을 검사할 프로젝트의 버전입니다.|  
|use32bitruntime|**bit**|64비트 운영 체제에서 32비트 런타임을 사용하여 패키지를 실행해야 하는지 여부를 나타냅니다. 값이 `1`, 32 비트 런타임 실행이 수행 됩니다. 값이 `0`이면 64비트 런타임으로 실행이 수행됩니다.|  
