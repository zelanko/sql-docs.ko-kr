 ...
 **연결 암호화**  
 암호화된 연결을 사용하는 대상 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스와 Oracle CDC Service 사이의 통신을 위해 **연결 암호화**를 선택합니다.**고급**: 필요한 경우 **고급**을 클릭하고 고급 연결 속성 대화 상자에 추가 연결 속성을 입력합니다.  
