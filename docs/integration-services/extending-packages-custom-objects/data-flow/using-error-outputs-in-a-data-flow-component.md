---
title: "데이터 흐름 구성 요소의 오류 출력을 사용 하 여 | Microsoft Docs"
ms.custom: 
ms.date: 03/06/2017
...
# <a name="using-error-outputs-in-a-data-flow-component"></a>데이터 흐름 구성 요소에서 오류 출력 사용
 ...
 구현 하 고 구성 요소에서 오류 출력을 지원 하려면 먼저 설정 해야는 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSComponentMetaData100.UsesDispositions%2A> 하는 구성 요소의 속성 **true**합니다. 다음에 있는 구성 요소에 출력 추가 해야 해당 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSOutput100.IsErrorOut%2A> 속성이로 설정 **true**합니다. 마지막으로, 오류 또는 잘림이 발생할 경우 오류 출력으로 행을 리디렉션하는 코드를 구성 요소에 포함해야 합니다. 이 항목에서는 이러한 세 단계를 설명하고 동기 오류 출력과 비동기 오류 출력의 차이점에 대해 설명합니다.  
  
## <a name="creating-an-error-output"></a>오류 출력 만들기  
 호출 하 여 오류 출력을 만듭니다는 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSOutputCollection100.New%2A> 의 메서드는 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSComponentMetaData100.OutputCollection%2A>를 설정한 다음는 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSOutput100.IsErrorOut%2A> 속성을 새 출력의 **true**합니다. 출력이 비동기적인 경우에는 출력에 대해 아무 작업도 수행하지 않아야 합니다. 출력이 동기적이고 동일한 입력에 대해 동기적인 다른 출력이 더 있는 경우에는 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSOutput100.ExclusionGroup%2A> 및 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSOutput100.SynchronousInputID%2A> 속성도 설정해야 합니다. 두 속성은 모두 동일한 입력에 대해 동기적인 다른 출력과 동일한 값으로 설정되어야 합니다. 이러한 속성이 0이 아닌 값으로 설정되어 있는 경우 입력에서 제공된 행은 해당 입력에 동기적인 두 출력 모두로 보내집니다.  
...
### <a name="redirecting-a-row-with-asynchronous-outputs"></a>비동기 출력을 사용하는 경우 행 리디렉션  
 비동기 출력을 사용하는 구성 요소에서는 동기 오류 출력을 사용할 때와 같이 행을 출력으로 전송하는 대신 출력 <xref:Microsoft.SqlServer.Dts.Pipeline.PipelineBuffer>에 행을 명시적으로 추가하여 행을 오류 출력으로 보냅니다. 비동기 오류 출력을 사용하는 구성 요소를 구현하려면 다운스트림 구성 요소에 제공된 오류 출력에 열을 추가하고 <xref:Microsoft.SqlServer.Dts.Pipeline.PipelineComponent.PrimeOutput%2A> 메서드 실행 중 구성 요소에 제공된 오류 출력에 대한 출력 버퍼를 캐시해야 합니다. 비동기 출력을 사용 하는 구성 요소를 구현 하는 세부 정보 항목에서 자세히 설명 [와 비동기 출력을 사용자 지정 변환 구성 요소 개발](../../../integration-services/extending-packages-custom-objects-data-flow-types/developing-a-custom-transformation-component-with-asynchronous-outputs.md)합니다. 오류 출력에 열이 명시적으로 추가되지 않을 경우 출력 버퍼에 추가된 버퍼 행에는 두 개의 오류 열만 있습니다.  
...
## <a name="see-also"></a>관련 항목:  
 ...
 [오류 출력을 사용 하 여](../../../integration-services/extending-packages-custom-objects/data-flow/using-error-outputs-in-a-data-flow-component.md)  
