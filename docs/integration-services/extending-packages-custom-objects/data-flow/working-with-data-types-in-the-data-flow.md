...
## <a name="inserting-data-into-the-data-flow"></a>데이터 흐름에 데이터 삽입  
 <xref:Microsoft.SqlServer.Dts.Pipeline.PipelineBuffer> 클래스는 버퍼 열에 데이터를 복사하는 일련의 **Set** 메서드 및 해당 버퍼 열에서 데이터를 검색하는 일련의 **Get** 메서드를 제공합니다. 다음 표에서는 각 [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] 데이터 형식으로 사용할 메서드를 보여 줍니다.  
  
### <a name="set-methods-to-use-with-data-types"></a>데이터 형식에 사용할 수 있는 Set 메서드  
 다음 표에서는 첫 번째 열에 데이터 형식을 나열한 다음 해당 **Set** 및 **Get** 메서드를 나열합니다.  
...
|<xref:Microsoft.SqlServer.Dts.Runtime.Wrapper.DataType.DT_NULL>|<xref:Microsoft.SqlServer.Dts.Pipeline.PipelineBuffer.SetNull%2A>|이 데이터 형식에 적용할 수 있는 **Get** 메서드가 없습니다.|  
