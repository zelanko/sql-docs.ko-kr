---
title: "이벤트 발생 및 정의 데이터에서 흐름 구성 요소 | Microsoft Docs"
ms.custom: 
ms.date: 03/14/2017
...
## <a name="raising-events"></a>이벤트 발생  
 구성 요소를 사용 하 여 이벤트를 발생 시킵니다.는 **화재\<X >** 의 메서드는 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSComponentMetaData100> 인터페이스입니다. 구성 요소를 디자인하거나 실행할 때 이벤트를 발생시킬 수 있습니다. 일반적으로 구성 요소를 디자인할 때는 유효성 검사 과정에서 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSComponentMetaData100.FireError%2A> 및 <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSComponentMetaData100.FireWarning%2A> 메서드가 호출됩니다. 메시지를 표시 하는 이러한 이벤트는 **오류 목록** 의 창 [!INCLUDE[ssBIDevStudioFull](../../../includes/ssbidevstudiofull-md.md)] 구성 요소가 잘못 구성 된 구성 요소의 사용자에 게 피드백을 제공 합니다.  
 ...
 *allowEventHandlers* 의 매개 변수는 <xref:Microsoft.SqlServer.Dts.Runtime.Wrapper.IDTSEventInfos100.Add%2A> 메서드는 구성 요소를 허용 하는지 여부를 지정 <xref:Microsoft.SqlServer.Dts.Runtime.DtsEventHandler> 개체를 이벤트에 대해 만들 수 있습니다. <xref:Microsoft.SqlServer.Dts.Runtime.DtsEventHandlers>는 동기 개체이므로 구성 요소에서는 사용자 지정 이벤트에 연결된 <xref:Microsoft.SqlServer.Dts.Runtime.DtsEventHandler>가 실행을 마칠 때까지 실행을 다시 시작하지 않습니다. 경우는 *allowEventHandlers* 매개 변수는 **true**, 이벤트의 각 매개 변수를 자동으로 제공할 모든 <xref:Microsoft.SqlServer.Dts.Runtime.DtsEventHandler> 만들어지고 자동으로 채워진 되는 변수를 통해 개체는 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] 런타임 합니다.  
