...
helpviewer_keywords: 
  - "원본 [Integration Services], 플랫 파일"
  - "텍스트 파일 읽기 [Integration Services]"
  - "플랫 파일"
  - "플랫 파일 원본"
...
-   플랫 파일 원본이 길이가 0인 열의 문자열을 Null 값으로 해석할지를 지정합니다.  
 ...
 플랫 파일 원본 출력의 출력 열에는 FastParse 속성이 포함됩니다. FastParse는 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]에서 제공하는 더 빠르지만 로캘을 구분하지 않는 빠른 구문 분석 루틴을 사용할지 또는 로캘을 구분하는 표준 구문 분석 루틴을 사용할지를 나타냅니다. 자세한 내용은 [Fast Parse](../Topic/Fast%20Parse.md) 및 [Standard Parse](../Topic/Standard%20Parse.md)를 참조하세요.  
