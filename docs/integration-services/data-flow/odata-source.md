...
-   OData V3 프로토콜의 경우 구성 요소는 ATOM 및 JSON 데이터 형식을 지원 합니다.  
  
-   OData V4 프로토콜의 경우 구성 요소는 JSON 데이터 형식을 지원 합니다.  

OData 원본에는 다음 데이터 원본에 대 한 지원이 포함 됩니다.
-   Microsoft Dynamics AX 온라인 및 Microsoft Dynamics CRM Online
-   SharePoint 목록입니다. 를 SharePoint 서버에 모든 목록을 보려면 다음 URL을 사용: http://\<서버 > / _vti_bin/ListData.svc 합니다. SharePoint URL 규칙에 대한 자세한 내용은 [SharePoint Foundation REST 인터페이스](http://msdn.microsoft.com/library/ff521587.aspx)를 참조하십시오.
  
## <a name="odata-format-and-performance"></a>OData 형식 및 성능
 대부분의 OData 서비스는 여러 형식의 결과 반환할 수 있습니다. 사용 하 여 결과 집합의 형식을 지정할 수 있습니다는 `$format` 쿼리 옵션입니다. JSON 및 JSON Light와 같은 형식은 ATOM 또는 XML보다 효율적이며 많은 양의 데이터를 전송하는 경우 더 나은 성능을 제공할 수 있습니다. 다음 표에서는 샘플 테스트의 결과를 제공합니다. 표에 나와 있듯이 ATOM에서 JSON으로 전환하는 경우 성능이 30-53% 향상되었고, ATOM에서 새로운 JSON Light 형식(WCF Data Services 5.1에서 사용 가능)으로 전환하는 경우에는 성능이 67% 향상되었습니다.  
...
## <a name="related-topics-in-this-section"></a>이 섹션의 관련된 항목  
 ...
 **쿼리 옵션**  
 쿼리에 대한 옵션을 지정합니다. 예를 들어 `$top=5` 
  
 **피드 URL**  
 표시의 읽기 전용 피드 URL이 대화 상자에서 선택한 옵션에 따라 합니다.  
...
### <a name="options"></a>옵션  
 **사용 가능한 외부 열**  
 데이터 원본에서 사용 가능한 원본 열의 목록을 표시합니다. 목록의 확인란을 사용하여 페이지의 아래쪽에 있는 테이블에 열을 추가하거나 제거할 수 있습니다. 선택한 열은 출력에 추가 됩니다.  
