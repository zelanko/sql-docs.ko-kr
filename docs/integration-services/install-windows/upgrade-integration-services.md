...
helpviewer_keywords: 
  ...
  - "SSIS, 업그레이드"
  - "SQL Server Integration Services, 업그레이드"
  - "Integration Services 업그레이드"
...
# Integration Services 업그레이드
 ...
 함께 설치하면 여러 버전의 dtexec 유틸리티가 설치됩니다. 올바른 버전의 유틸리티가 실행되도록 하려면 명령 프롬프트에서 전체 경로(\<드라이브>:\Program Files\Microsoft SQL Server\\<버전\>\DTS\Binn)를 입력하는 방식으로 유틸리티를 실행합니다. dtexec에 대한 자세한 내용은 [dtexec Utility](../../integration-services/packages/dtexec-utility.md)를 참조하십시오.  
...
>   
>  지원되지 않는 다른 기능에 대한 자세한 내용은 [SQL Server 2016에서 지원되지 않는 Integration Services 기능](../Topic/Discontinued%20Integration%20Services%20Functionality%20in%20SQL%20Server%202016.md)을 참조하세요.  
...
## Integration Services와 데이터베이스 엔진 모두 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]  
...
## 데이터베이스 엔진만 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]  
 이 섹션에서는 다음 조건에 해당하는 업그레이드를 수행할 때 나타나는 결과에 대해 설명합니다.  
