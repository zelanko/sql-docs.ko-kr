...
-   데이터 탭 도중에 캡처되는 행 수를 제한하여 대용량 데이터 파일이 생성되지 않도록 하는 것이 좋습니다. 저장 프로시저가 실행되는 컴퓨터에 데이터 파일을 위한 스토리지 공간이 부족한 경우 패키지 실행이 중지되고 오류 메시지가 로그에 기록됩니다.  
