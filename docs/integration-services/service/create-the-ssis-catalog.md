...
5.  **SQL Server 시작 시 Integration Services 저장 프로시저 자동 실행**을 클릭하여 [!INCLUDE[ssIS](../../includes/ssis-md.md)]서버 인스턴스를 다시 시작할 때마다 [catalog.startup](../../integration-services/system-stored-procedures/catalog-startup.md) 저장 프로시저를 실행하도록 지정합니다.  
...
1.  다음 PowerShell 스크립트를 실행합니다.  
    ...
    ```  
     ...
     Windows PowerShell 및 <xref:Microsoft.SqlServer.Management.IntegrationServices> 네임스페이스를 사용하는 방법의 예를 더 보려면 blogs.msdn.com에서 [SSIS and PowerShell in SQL Server 2012](http://go.microsoft.com/fwlink/?LinkId=242539)(SQL Server 2012의 SSIS 및 PowerShell) 블로그 항목을 참조하세요. 네임스페이스 및 코드 예제에 대한 개요는 blogs.msdn.com에서 [SSIS 카탈로그 관리 개체 모델에 대한 이해](http://go.microsoft.com/fwlink/?LinkId=254267)블로그 항목을 참조하십시오.  
  
## 관련 항목:  
 [SSIS 카탈로그](../../integration-services/service/ssis-catalog.md)   
