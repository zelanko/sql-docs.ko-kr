 ...
 **Azure HDInsight 하이브 태스크**는 SQL Server 2016에 대해 제공되는 Azure용 SSIS(SQL Server Integration Services) 기능 팩의 구성 요소입니다. [여기](http://go.microsoft.com/fwlink/?LinkID=626967)서 기능 팩을 다운로드하세요.  
...
2.  **HDInsightClusterName** 필드에서는 하이브 스크립트를 실행할 HDInsight 클러스터의 이름을 선택합니다.  
  
3.  **LocalLogFolder** 필드에서는 **...(줄임표)**를 클릭하고 HDInsight 클러스터에서 하이브 처리 로그를 다운로드할 폴더를 선택합니다.  
  
4.  두 가지 방법으로 하이브 스크립트를 지정할 수 있습니다.  
