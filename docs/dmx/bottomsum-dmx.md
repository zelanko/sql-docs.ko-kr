...
## <a name="remarks"></a>설명  
 **BottomSum** 함수는 최하위 행을 차수 보다 오름차순으로 반환 합니다. Rank는 차수 > 식의 \<계산 된 값을 기준으로 하 여 각 행에 대 한 값 > 값 \<의 합계가 적어도 \<sum > 인수로 지정 된 지정 된 합계 이상이 되도록 합니다. **BottomSum** 는 지정 된 sum 값을 충족 하는 동안 가능한 가장 작은 수의 요소를 반환 합니다.  
