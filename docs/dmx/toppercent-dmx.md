...
## <a name="remarks"></a>설명  
 **TopPercent** 함수는 차수 > 식의 \<계산 된 값을 기준으로 최상위 행을 내림차순으로 반환 합니다 .이 값은 각 행에 대 한 인수를 계산 하 여 값 > 값의 \<합계가 적어도 \<percent > 인수로 지정 된 백분율입니다. **TopPercent** 는 지정 된 백분율 값을 충족 하는 동안 가능한 가장 작은 수의 요소를 반환 합니다.  
