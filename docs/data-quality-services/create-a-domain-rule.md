...
|값이 패턴과 일치하지 않음|피연산자의 문자, 숫자 또는 특수 문자 패턴과 일치하지 않는 값만 유효합니다.|피연산자 예: A1(값은 *임의의* 문자 하나 뒤에 *임의의* 숫자 하나가 오는 패턴과 일치하지 않아야 합니다.)<br /><br /> 유효한 값: AB1, A, A:5<br /><br /> 유효하지 않은 값: B7, c9|  
|값이 패턴 포함|피연산자의 문자, 숫자 또는 특수 문자 패턴을 포함하는 값만 유효합니다.|피연산자 예: AA-12(값은 *임의의* 문자 두 개 뒤에 하이픈(-)이 오고, 그 뒤에 *임의의* 숫자 두 개가 오는 패턴을 포함해야 합니다.)<br /><br /> 유효한 값: AAA-01, ab-975<br /><br /> 유효하지 않은 값: A7, AA-6, C-45, aa;98|  
|값이 패턴 포함 안 함|피연산자의 문자 패턴을 포함하지 않는 값만 유효합니다.|피연산자 예: AB-12(값은 *임의의* 문자 두 개 뒤에 하이픈(-)이 오고, 그 뒤에 *임의의* 숫자 두 개가 오는 패턴을 포함하지 않아야 합니다.)<br /><br /> 유효한 값: A7, AA-6, C-45, aa;98<br /><br /> 유효하지 않은 값: AAA-01, ab-975|  
