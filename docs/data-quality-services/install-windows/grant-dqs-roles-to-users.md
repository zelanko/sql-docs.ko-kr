...
6.  오른쪽 창에서 **DQS_MAIN** 데이터베이스의 **Map** 열 아래의 확인란을 선택한 다음, 사용자에게 필요한 액세스 수준에 따라 **dqs_administrator**, **dqs_kb_editor** 또는 **dqs_kb_operator**(**데이터베이스 역할 멤버 자격: DQS_MAIN** 창) 확인란을 선택합니다. 세 가지 DQS 역할에 대한 자세한 내용은 [DQS 보안](../../data-quality-services/dqs-security.md)을 참조하십시오.  
