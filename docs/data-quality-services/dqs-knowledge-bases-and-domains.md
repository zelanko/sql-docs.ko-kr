...
### DQS의 대/소문자 구분 안 함  
 ...
 그러나 결과를 정리할 때 내보내는 값에서는 대/소문자를 제어할 수 있습니다. 이렇게 하면 설정 하 여는 **출력 형식** 도메인 속성 (참조 [도메인 속성 설정](../data-quality-services/set-domain-properties.md))를 사용 하 여는 **출력 표준화** 정리 결과 내보낼 때 확인란 (참조 [데이터를 사용 하 여 DQS 정리 & #40; 내부 & #41; 기술](../data-quality-services/cleanse-data-using-dqs-internal-knowledge.md)).  
...
-   복합 도메인의 데이터는 참조 데이터 원본에 연결할 수 있으며, 이 경우 복합 도메인이 참조 데이터 공급자에 전송됩니다. 이러한 작업은 종종 주소 데이터에서 수행됩니다.  
 ...
 복합 도메인으로 표현되는 데이터를 구문 분석하는 방법은 복합 도메인 속성에 의해 결정됩니다. 데이터를 구분 기호나 도메인, 순서에 의해 구문 분석 또는 복합 도메인에 연결 된 도메인의 지식을 기반으로 수 있습니다 (선택 하 여는 **기술 자료 구문 분석 사용** 복합 도메인의 속성). 자세한 내용은 [Set Composite Domain Properties](../data-quality-services/create-a-composite-domain.md#CompositeDomainProperties)을 참조하세요.  
...
##  <a name="Matching"></a> 데이터 일치  
 ...
 기술 자료 및 데이터 일치 하는 방법에 대 한 자세한 내용은 참조 [일치 하는 데이터](../data-quality-services/data-matching.md)합니다.  
