...
> [!IMPORTANT]  
>  내보낼 수 *모든* 기술 자료에 [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)] 를 명령 프롬프트에서 DQSInstaller.exe 파일을 실행 하 여 한 번에 DQS 백업 파일 (.dqsb). 마찬가지로, 가져올 수 있습니다 *모든* 기술 자료를 DQS 백업 파일 (.dqsb) 하 여 [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)] 명령 프롬프트에서 DQSInstaller.exe 파일을 실행 하 여 한 번에 합니다. 방법은 DQS 설치 설명서에서 [Export and Import DQS Knowledge Bases Using DQSInstaller.exe](../data-quality-services/install-windows/export-and-import-dqs-knowledge-bases-using-dqsinstaller-exe.md) 를 참조하십시오.  
