...
-    **도메인 값** 페이지 또는 **용어 기반 관계** 의 페이지는 **도메인 관리** 활동  
...
2.  사용할 도메인 속성에서 맞춤법 검사기 사용 하는 경우는 **도메인 값 관리** 페이지는 **도메인 값** 페이지 또는 **용어 기반 관계** 페이지 및 **결과 관리 및 보기** 페이지입니다. 이러한 페이지에서 맞춤법 검사기를 사용 하지 않으려면 클릭는 **활성화/비활성화 맞춤법 검사기** 아이콘입니다. 이 아이콘을 클릭하면 페이지에서 맞춤법 검사기의 상태가 변경됩니다. 마찬가지로, 경우는 **맞춤법 검사기 사용** 도메인에 대 한 속성은 해제를 클릭 하 고 **활성화/비활성화 맞춤법 검사기** 아이콘을 사용 하 여 페이지에서 맞춤법 검사기. 페이지를 나갔다가 다시 돌아오면 버튼 상태가 **맞춤법 검사기 사용** 도메인 속성에 의해 다시 결정됩니다.  
...
##  <a name="How"></a> 맞춤법 검사기 작동 방식  
 ...
 DQS는 **값** 테이블의 **값** 열과 **다음으로 수정** 열 모두에 값 제안을 제공합니다. **값** 열에서 제안을 선택하면 값 유형이 **오류**로 설정되고 사용자가 수동으로 삽입한 것처럼 **다음으로 수정** 열에 제안이 복사됩니다. 기존 수정 사항이 있는 경우 제안이 됩니다. **정리** 작업에서 **결과 관리 및 보기** 페이지의 **다음으로 수정** 열에 있는 제안을 선택하면 DQS가 현재 선택된 값을 선택 항목으로 바꾸고 현재 선택된 값이 제안이 됩니다. 에 **결과 관리 및 보기** 의 페이지는 **정리** 활동, 제안 사항 없음 레코드 수준 (하단 표)에 적용 합니다.  
