...
3.  **데이터 품질 프로젝트 열기** 를 클릭하여 기존 데이터 품질 프로젝트에서 일치를 수행합니다. 프로젝트를 선택하고 **다음**을 클릭합니다. (아래에서 프로젝트를 클릭할 수 또는 **최근 데이터 품질 프로젝트**.) 닫힌 일치 프로젝트를 열면 해당 일치 프로젝트 작업이 닫힌 단계로 이동 합니다 (표시 된 대로 **상태** 아래의 프로젝트 이름 또는 프로젝트 테이블의 열 **최근 데이터 품질 프로젝트**). 완료 된 일치 프로젝트를 열면 이동 하 게는 **내보내기** 페이지 (및 이전 화면으로 돌아갈 수 없습니다).  
...
4.  클릭 된 **더하기 (+)** 매핑 테이블에 행을 추가 하려면 컨트롤 또는 **빼기 (-)** 행을 삭제 하는 컨트롤입니다.  
  
5.  **데이터 원본 미리 보기** 를 클릭하여 선택한 SQL Server 테이블이나 뷰 또는 선택한 Excel 워크시트에서 데이터를 확인합니다.  
  
6.  클릭 **복합 도메인 보기/선택** 기술 자료에서 사용할 수 있는 복합 도메인 목록을 확인 하 고 매핑에 적절 하 게 선택 합니다.  
...
1.  에 **일치 하는 페이지**, 선택, **겹치는 클러스터** 피벗 레코드와 레코드를 표시 다음 모든 클러스터에 대 한 일치가 실행 될 때 클러스터 그룹에 공통 레코드를 포함 하는 경우에 드롭다운 목록에서. 일치가 실행될 때 공통적인 레코드가 있는 클러스터를 단일 클러스터로 표시하려면 **겹치지 않는 클러스터** 를 선택합니다.  
  
2.  클릭 **원본에서 데이터를 다시 로드** (기본값) 데이터 원본에서 준비 테이블로 데이터를 복사 하 고 다시 일치 프로젝트를 실행할 때 인덱스에 있습니다. 클릭 **이전 데이터에 대해 Execute** 준비 테이블로 데이터를 복사 하 고 데이터 인덱싱을 다시 수행 하지 않고 일치 프로젝트를 실행 합니다. 일치 프로젝트를 처음 실행하거나**맵** 페이지에서 매핑을 변경한 후 팝업에서 **예** 를 클릭한 경우에는 **이전 데이터에 대해 실행** 이 비활성화됩니다. 두 경우 모두 인덱스를 다시 작성해야 합니다. 일치 프로젝트가 변경되지 않은 경우에는 인덱스를 다시 작성할 필요가 없습니다. 이전 데이터에 대해 실행하면 성능이 향상됩니다.  
...
7.  테이블에는 비 피벗 레코드를 선택 하 고 클릭 하는 경우는 **세부 정보 보기** 아이콘 (또는 레코드를 두 번 클릭) 표시 됩니다는 **일치 점수 정보** 레코드를 두 번 클릭 하 고 해당 피벗 레코드 (및 모든 필드의 값)을 표시 하는 팝업,와 일치 하는 드릴 다운 사이의 점수 점수 각 필드의 작성 글입니다. 피벗 레코드를 두 번 클릭한 경우에는 팝업이 표시되지 않습니다.  
...
10. 최소 일치 레코드를 표시 하는 데 필요한 일치 수준을 결정 하는 점수를 변경 하려면 선택은 **분입니다. 일치 점수** 테이블의 오른쪽 위에 있는 아이콘 더 높은 숫자를 입력 합니다. 최소 일치 점수는 기본적으로 80%로 설정됩니다. **새로 고침** 을 클릭하여 테이블 내용을 변경합니다.  
...
3.  에 대 한 확인란을 선택 **일치 결과** 를 SQL Server 데이터베이스에 지정 된 테이블이 나 지정 된.csv 또는 Excel 파일로 일치 하는 결과 (위 설명 참조)를 내보냅니다. 에 대 한 확인란을 선택 **Survivorship 결과** 를 SQL Server 데이터베이스의 지정 된 테이블이 나 지정 된.csv 또는 Excel 파일로 survivorship 결과 (위 설명 참조)를 내보냅니다.  
...
7.  다음과 같이 Survivorship 규칙을 선택합니다.  
    ...
    -   선택 **피벗 레코드** (기본값)을 DQS에서 임의로 선택한 초기 피벗 레코드와 레코드를 존속 레코드로 식별 합니다.  
