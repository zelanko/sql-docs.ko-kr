...
-   동일한 일괄 처리 내에서 둘 이상의 임시 테이블에 동일한 이름을 사용 합니다. 일괄 처리 내에서 둘 이상의 로컬 임시 테이블을 사용 하는 경우 각 고유 이름이 있어야 합니다. 여러 세션 동일한 일괄 처리를 실행 하 고 동일한 로컬 임시 테이블을 만들지, 하는 경우 SQL Server PDW 각 로컬 임시 테이블에 대 한 고유한 이름을 유지 하기 위해 로컬 임시 테이블 이름에 숫자 접미사를 내부적으로 추가 합니다.  
