---
title: "병렬 데이터 웨어하우스에 대 한 투명 한 데이터 암호화"
author: barbkess
ms.author: barbkess
manager: jhubbard
...
description: "투명 한 데이터 암호화 (TDE) 실시간 I/O 암호화 및 데이터 및 트랜잭션 로그 파일 및 특수 PDW 로그 파일의 암호 해독을 수행합니다."
ms.date: 10/20/2016
ms.topic: article
...
![이 항목에서 설명 하는 계층 구조를 표시 합니다. ] (media/tde-architecture.png "TDE_Architecture")  
...
3.  사용 하 여 [sp_pdw_add_network_credentials](../relational-databases/system-stored-procedures/sp-pdw-add-network-credentials-sql-data-warehouse.md) 인증 하 고 인증서의 백업을 저장할 공유에 쓸 수 있는 자격 증명을 만들어야 합니다. 의도 한 저장소 서버에 대 한 자격 증명이 이미 있는 경우에 기존 자격 증명을 사용할 수 있습니다.  
...
**첫 번째: SQL Server PDW에서 TDE를 사용 하도록 합니다.** 이것은 한 번 필요 합니다.  
...
**두 번째: 만들기 및 master 데이터베이스에 인증서를 백업 합니다.** 이만 번만 지정 해야 합니다. (권장) 각 데이터베이스에 대 한 별도 인증서가 없거나 하나의 인증서로 여러 개의 데이터베이스를 보호할 수 있습니다.  
...
**마지막: DEK 만들고 ALTER DATABASE를 사용 하 여 사용자 데이터베이스를 암호화 합니다.** 이 TDE로 보호 되는 각 데이터베이스에 대해 반복 됩니다.  
...
PDW에서 수동으로 이러한 문을 실행 하 고 DMK를 복구 하려면 그 후 기기를 다시 시작 하십시오.  
...
다음 링크는 SQL Server 암호화를 관리 하는 방법에 대 한 일반 정보를 포함 합니다. 이러한 항목에는 SQL Server 암호화 하기는 하지만 이러한 항목 SQL Server PDW에 정보만 없고 SQL Server PDW에 없는 기능에 이야기를 이해 하는 데 수 있습니다.  
