---
title: "SQL Server vNext 릴리스 정보 | Microsoft 문서"
ms.custom: 
ms.date: 03/12/2017
ms.prod: sql-vnext
ms.reviewer: 
...
---
# <a name="sql-server-vnext-release-notes"></a>SQL Server vNext 릴리스 정보
...
- [SQL Server Reporting Services 릴리스 정보](../reporting-services/reporting-services-release-notes.md) 는 Reporting Services 섹션 내에서 게시됩니다.
...
## <a name="sql-server-vnext-ctp-14-march--2017"></a>SQL Server vNext CTP 1.4(2017년 3월)
### <a name="supported-installation-scenarios-ctp-14"></a>지원되는 설치 시나리오(CTP 1.4)
...
특히 다음과 같은 시나리오가 사용될 수 있지만 철저히 테스트되지 않았으며 **에서 지원되지** 않습니다 [!INCLUDE[ssSQLv14_md](../includes/sssqlv14-md.md)].
- CTP 1.1 제거
...
**문제 및 고객에게 미치는 영향:** [!INCLUDE[ssIS_md](../includes/ssis-md.md)] 또는 catalog.stop_operation 저장 프로시저를 사용하여 [!INCLUDE[ssManStudio_md](../includes/ssmanstudio-md.md)] 에서 작업을 중지하면 "사용자 정의 루틴 또는 집계 'stop_operation_internal'을(를) 실행하는 동안 .NET Framework 오류가 발생했습니다."라는 오류가 발생하며 실패할 수 있습니다.
...
**문제 및 고객에게 미치는 영향:** [!INCLUDE[ssIS_md](../includes/ssis-md.md)] 규모 확장 기능이 컴퓨터에 설치되어 있는 경우 **SSISDB** 카탈로그를 삭제하면 "사용자가 현재 '...'(으)로 로그인되어 있기 때문에 로그인을 삭제할 수 없습니다."라는 오류가 발생하고 SSISDB 카탈로그 데이터베이스가 삭제되지 않을 수 있습니다. 
