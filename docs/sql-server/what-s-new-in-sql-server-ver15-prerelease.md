...
이 문서에서는 CTP([!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)]) 릴리스의 기능 알림 보관 파일을 제공합니다. 이 파일은 기록 목적으로 제공되지만 프로덕션 또는 최신 시험판 버전 [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)]에는 적용되지 않습니다.
...
|:---|:---|
| `mssqlctl` 명령 변경 내용 | `mssqlctl cluster` 명령이 `mssqlctl bdc`로 변경되었습니다. 자세한 내용은 [`mssqlctl`참조](../big-data-cluster/reference-azdata.md)를 참조하세요. |
...
| **mssqlctl** 도구 명령 사용법이 변경되었습니다. | 자세한 내용은 [mssqlctl의 알려진 문제](../big-data-cluster/release-notes-big-data-cluster.md )를 참조하세요. |
...
#### <a name="new-query_post_execution_plan_profile-extended-event-ctp-24"></a>새 query_post_execution_plan_profile 확장 이벤트(CTP 2.4)
...
#### <a name="new-dmf-sysdm_exec_query_plan_stats-ctp-24"></a>새 DMF sys.dm_exec_query_plan_stats(CTP 2.4) 
...
#### <a name="new-options-added-to-sp_estimate_data_compression_savings-ctp-20"></a>sp_estimate_data_compression_savings에 새로 추가된 옵션(CTP 2.0)
...
#### <a name="new-sysdm_db_page_info-system-function-returns-page-information-ctp-20"></a>페이지 정보를 반환하는 새 sys.dm_db_page_info 시스템 함수(CTP 2.0)
...
#### <a name="online-attach-ctp32"></a>온라인 연결(CTP 3.2)

CTP 3.2에는 테이블 형식 모델을 온라인 작업으로 연결하는 기능이 도입되었습니다. 온라인 연결을 사용하여 온-프레미스 쿼리 스케일 아웃 환경에서 읽기 전용 복제본을 동기화할 수 있습니다. 온라인 연결 작업을 수행하려면 XMLA 연결 명령의 **AllowOverwrite** 옵션을 사용합니다. 
...
이 작업을 수행하려면 새 버전을 로드하는 동안 이전 버전을 온라인 상태로 유지하기 위해 *2배의 모델 메모리*가 필요할 수 있습니다. 
...
4. AllowOverwrite=True인 <Attach> 명령이 DB1(버전2)의 새 위치를 사용하여 서버 B에서 실행됩니다. 
