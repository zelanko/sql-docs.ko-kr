...
helpviewer_keywords: 
  - "자동 페이지 복구"
  - "가용성 그룹 [SQL Server], 자동 페이지 복구"
  - "데이터베이스 미러링 [SQL Server], 자동 페이지 복구"
...
##  <a name="ErrorTypes"></a> 자동 페이지 복구 시도가 발생되는 오류 유형  
 데이터베이스 미러링 자동 페이지 복구는 다음 표에 나열된 오류 중 하나로 인해 실패한 작업이 있는 데이터 파일의 페이지만 복구하도록 시도합니다.  
  
|오류 번호|설명|자동 페이지 복구 시도가 발생되는 인스턴스|  
|------------------|-----------------|---------------------------------------------------------|  
|[823](../Topic/MSSQLSERVER_823.md)|운영 체제가 데이터에서 실패한 CRC(순환 중복 검사)를 수행한 경우에만 동작이 수행됩니다.|ERROR_CRC이며 이 오류에 대한 운영 체제 값은 23입니다.|  
|[824](../Topic/MSSQLSERVER_824.md)|논리 오류입니다.|조각난 쓰기 오류 또는 잘못된 페이지 체크섬과 같은 논리적 데이터 오류입니다.|  
|829|페이지가 복원 보류로 표시되었습니다.|모두|  
  
 최근에 발생한 823 CRC 오류 및 824 오류를 보려면 [msdb](../../relational-databases/databases/msdb-database.md) 데이터베이스의 [suspect_pages](../../relational-databases/system-tables/suspect-pages-transact-sql.md) 테이블을 참조하세요.  
...
##  <a name="DevBP"></a> 개발자를 위한 최선의 구현 방법  
