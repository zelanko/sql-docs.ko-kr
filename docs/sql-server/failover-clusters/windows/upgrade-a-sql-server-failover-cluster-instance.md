...
description: 설치 미디어를 사용하여 SQL Server 장애 조치(failover) 클러스터 인스턴스를 업그레이드하는 방법을 알아봅니다. 롤링 업그레이드 및 다중 서브넷 클러스터 업그레이드에 대해 알아봅니다.
...
## <a name="upgrade-a-multi-subnet-failover-cluster"></a>다중 서브넷 장애 조치(failover) 클러스터 업그레이드  

다중 서브넷 환경에서 SQL Server 장애 조치(failover) 클러스터 인스턴스를 업그레이드하려면 다음 단계를 수행합니다. 
