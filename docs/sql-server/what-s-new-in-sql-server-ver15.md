...
**SQL Server 2019를 통해 최상의 환경에 맞는 [최신 도구](#tools)를 사용해 보세요.**

## <a name="ctp-23"></a>CTP 2.3

CTP(Community Technology Preview) 2.3은 [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)]의 최신 공개 릴리스입니다. 이 릴리스에는 버그를 수정하고, 보안을 개선하고, 성능을 최적화하는 이전 CTP 릴리스의 개선 사항이 포함됩니다. 또한 [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)] CTP 2.3에서 추가되었거나 향상된 기능은 다음과 같습니다.

- [빅 데이터 클러스터](#bigdatacluster)
  - IntelliJ의 SQL Server 2019 빅 데이터 클러스터에 Spark 작업 제출
  - R 및 Python을 통한 기계 학습 모델 운영, SSIS 작업 실행 등 다양한 데이터 관련 앱에 대한 애플리케이션 배포 및 관리 환경
  - SQL Server 2019 빅 데이터 클러스터에서 Sparklyr 사용
  - HDFS 계층화를 통해 빅 데이터 클러스터에 외장 HDFS 호환 스토리지 탑재

- [데이터베이스 엔진](#databaseengine)
  - 가속 데이터베이스 복구
  - 여러 범위에서 임시 테이블을 사용하여 워크로드에 대한 재컴파일 감소
  - 향상된 간접 검사점 확장성
  - 쿼리 저장소 계획에서 빠른 전달 및 정적 커서에 대한 강제 적용 지원
  - SQL 그래프를 통한 노드 삭제 시의 계단식 에지 삭제 지원
  - Windows에서 Java 및 Python용 외부 라이브러리 지원

- [SSAS([!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Analysis Services)](#ssas)
  - 테이블 형식 모델의 계산 그룹은 계산 논리를 다시 사용하여 측정값의 수를 줄입니다.
...
- [빅 데이터 클러스터](#bigdatacluster) 
  ...
  - 빅 데이터 클러스터의 Azure Data Studio에서 SparkR 사용(CTP 2.2)
  - Python 및 R 앱 배포(CTP 2.2)
...
- [IntelliJ의 SQL Server 2019 빅 데이터 클러스터에 Spark 작업 제출](../big-data-cluster/spark-submit-job-intellij-tool-plugin.md)(CTP 2.3)
- [R 및 Python을 통한 기계 학습 모델 운영, SSIS 작업 실행 등 다양한 데이터 관련 앱에 대한 애플리케이션 배포 및 관리 환경](../big-data-cluster/big-data-cluster-create-apps.md)(CTP 2.3)
- [SQL Server 2019 빅 데이터 클러스터에서 Sparklyr 사용](../big-data-cluster/sparklyr-from-RStudio.md)(CTP 2.3)
- [HDFS 계층화](../big-data-cluster/hdfs-tiering.md)를 통해 빅 데이터 클러스터에 외장 HDFS 호환 스토리지 탑재(CTP 2.3)
- 빅 데이터 클러스터의 Azure Data Studio에서 SparkR 사용(CTP 2.2)
...
- Spark에서 고급 분석 및 Machine Learning 실행(CTP 2.0)
- Spark Streaming을 사용하여 SQL 데이터 풀에 데이터 스트리밍(CTP 2.0)
- [**Azure Data Studio**](../sql-operations-studio/what-is.md)에서 Notebook 환경을 제공하는 쿼리 세트 실행(CTP 2.0)
...
이 기능을 사용하기 전에 DML 명령문(`SELECT`, `INSERT`, `UPDATE`, `DELETE`)을 사용하여 임시 테이블을 참조할 때 임시 테이블이 외부 범위 일괄 처리에서 만들어지면 DML 문이 실행될 때마다 다시 컴파일됩니다. 이 향상된 기능을 사용하면 SQL Server에서 불필요한 재컴파일을 방지하기 위해 다음과 같은 간단한 검사를 추가로 수행합니다.
...
- [**Azure Data Studio**](../azure-data-studio/what-is.md): 이전에 미리 보기 이름 SQL Operations Studio로 릴리스되었던 Azure Data Studio는 데이터 개발 및 관리 측면에서 가장 일반적인 작업을 수행하기 위한 경량의 최신 오픈 소스, 플랫폼 간 데스크톱 도구입니다. Azure Data Studio 및 [SQL Server 2019 미리 보기 확장](../azure-data-studio/sql-server-2019-extension.md)을 사용하면 Windows, macOS 및 Linux에서 온-프레미스 및 클라우드의 SQL Server에 연결할 수 있습니다. Azure Data Studio를 사용하여 다음을 수행할 수 있습니다.

  - 이제 AAD를 지원합니다. (CTP 2.3)
  - Notebook 보기 UI가 Azure Data Studio 코어로 이동되었습니다. (CTP 2.3)
  - 외부 데이터 원본을 HDFS에서 SQL Server 빅 데이터 클러스터로 만드는 마법사가 새로 추가되었습니다. (CTP 2.3)
  - Notebook 뷰어 UI가 향상되었습니다. (CTP 2.3)
  - 새 Notebook API가 추가되었습니다. (CTP 2.3)
  - Python 패키지 업데이트를 지원하는 "Notebook 종속성 재설치" 명령이 추가되었습니다. (CTP 2.3)
  - SQL Server 2019 빅 데이터 클러스터를 연결하고 관리합니다. (CTP 2.1)
...
- [**SQL Server PowerShell 모듈**](https://www.powershellgallery.com/packages/SqlServer/21.1.18080): SqlServer PowerShell 모듈을 사용하면 SQL Server 개발자, 관리자 및 BI 전문가가 데이터베이스 배포 및 서버 관리를 자동화할 수 있습니다.
...
이 릴리스의 계산 그룹에는 다음과 같은 몇 가지 제한 사항이 있습니다.

- `ALLSELECTED DAX` 함수는 아직 지원되지 않습니다.
- 계산 그룹 테이블에 정의된 행 수준 보안은 아직 지원되지 않습니다.
- 계산 그룹 테이블에 정의된 개체 수준 보안은 아직 지원되지 않습니다.
- DetailsRows 계산 항목을 참조하는 식은 아직 지원되지 않습니다.
- MDX는 아직 지원되지 않습니다.

계산 그룹에는 현재 SQL Server 2019 CTP 2.3 이상에서만 지원되는 1470 호환성 수준의 모델이 필요합니다. 계산 그룹은 현재 TOM(테이블 형식 개체 모델) API, TMSL( 테이블 형식 모델 스크립팅 언어) 및 오픈 소스 테이블 형식 편집기 도구를 사용하여 만들 수 있습니다. SSDT(SQL Server Data Tools) 지원은 설명서와 함께 이후 릴리스에 포함됩니다. 이 CTP 및 다른 CTP 기능 릴리스에 대한 추가 정보는 Analysis Services 블로그에서 제공됩니다.

## <a name="other-services"></a>기타 서비스

CTP 2.3부터 [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)]에서는 다음 서비스의 새로운 기능을 도입하지 않습니다.
