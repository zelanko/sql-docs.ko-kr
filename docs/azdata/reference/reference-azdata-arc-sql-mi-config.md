---
title: azdata arc sql mi config 참조
titleSuffix: SQL Server big data clusters
description: azdata arc sql mi config 명령에 대한 참조 문서입니다.
author: MikeRayMSFT
ms.author: mikeray
ms.reviewer: seanw
ms.date: 09/22/2020
...
|명령|설명|
| --- | --- |
[azdata arc sql mi config init](#azdata-arc-sql-mi-config-init) | SQL 관리형 인스턴스의 CRD 및 사양 파일을 초기화합니다.
[azdata arc sql mi config add](#azdata-arc-sql-mi-config-add) | 구성 파일에서 json 경로의 값을 추가합니다.
[azdata arc sql mi config remove](#azdata-arc-sql-mi-config-remove) | 구성 파일에서 json 경로의 값을 제거합니다.
[azdata arc sql mi config replace](#azdata-arc-sql-mi-config-replace) | 구성 파일에서 json 경로의 값을 바꿉니다.
[azdata arc sql mi config patch](#azdata-arc-sql-mi-config-patch) | json 패치 파일을 기준으로 구성 파일을 패치합니다.
