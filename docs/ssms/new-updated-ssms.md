...
- 최소한의 사용자 간섭 또는 특수화된 도메인 지식을 요구하는 지능형 프레임워크로 CSV 파일의 가져오기 작업을 단순화하기 위해 새 "플랫 파일 가져오기" 마법사가 추가되었습니다. 자세한 내용은 [Import Flat File to SQL Wizard--../relational-databases/import-export/import-flat-file-wizard.md)를 참조하세요.
- 개체 탐색기에 "XEvent Profiler" 노드가 추가되었습니다. 자세한 내용은 [Use the SSMS XEvent Profiler--../relational-databases/extended-events/use-the-ssms-xe-profiler.md)를 참조하세요.
...
- Always On: 가용성 그룹에 대해 [read-only routing support--../database-engine/availability-groups/windows/configure-read-only-routing-for-an-availability-group-sql-server.md)을 추가했습니다.
