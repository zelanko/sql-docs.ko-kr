...
SSMS(SQL Server Management Studio)는 SQL Server에서 SQL Database까지 모든 SQL 인프라를 관리하기 위한 통합 환경입니다. SSMS는 어디에 배포하든 배포 위치에서 SQL의 인스턴스를 구성, 모니터링 및 관리하는 도구를 제공합니다. SSMS는 데이터 계층 구성 요소(예: 응용 프로그램에 사용되는 데이터 웨어하우스 및 데이터베이스)를 배포, 모니터링 및 업그레이드하는 도구와 쿼리 및 스크립트를 작성하는 도구를 제공합니다. 

이 릴리스는 이전 버전의 SQL Server와의 향상된 호환성, 독립 실행형 웹 설치 관리자 및 새 릴리스를 사용할 수 있는 경우 SSMS 내 알림 메시지 기능을 제공합니다.  

SSMS는 무료입니다. 아래에서 다운로드하세요!
    
![다운로드](../ssdt/media/download.png) **[SQL Server Management Studio 17.1 다운로드](https://go.microsoft.com/fwlink/?linkid=849819)** 
...
* 이 버전의 SSMS는 [지원되는 모든 SQL Server 버전(SQL Server 2008 - SQL Server 2017)](https://support.microsoft.com/en-us/lifecycle?C2=1044)과 작동하며 Azure SQL Database와 Azure SQL Data Warehouse의 최신 클라우드 기능 사용에 대해 최고 수준의 지원을 제공합니다.  
...
Windows 10, Windows 8, Windows 8.1, Windows 7(SP1), Windows Server 2016, Windows Server 2012(64비트), Windows Server 2012 R2(64비트), Windows Server 2008 R2(64비트)  

>[!NOTE]
>SSMS 17.X는 Windows Server 2016 이전에 출시된 Visual Studio 2015 격리 셸을 기반으로 합니다. Microsoft는 앱 호환성을 매우 중요하게 생각하며 이미 제공된 응용 프로그램이 최신 Windows 버전에서 계속 실행되는지 확인합니다. 이 때문에 Windows Server 2016에서 실행할 때 모든 최신 업데이트가 적용된 SSMS에서 문제가 발생할 것이라 예상하지 않습니다. Windows Server 2016에서 SSMS에 대한 문제가 발생할 경우 지원 담당자에게 문의해 주세요. 그러면 지원 담당자가 고객과 협력하여 문제가 SSMS 또는 Visual Studio에 대한 것인지, 아니면 Windows 호환성에 대한 것인지 여부를 확인하고 문제를 적절히 전달합니다.
...
알려진 문제 및 해결 방법 목록은   
                [SQL Server Management Studio - 릴리스 정보](../ssms/sql-server-management-studio-release-notes.md)를 참조하세요.  
...
![needhelp_person_icon](../ssms/media/needhelp_person_icon.png) [SQL 클라이언트 도구 포럼](https://social.msdn.microsoft.com/Forums/en-US/home?forum=sqltools) |  [Microsoft Connect에 문제 또는 제안 기록하기](https://connect.microsoft.com/SQLServer/Feedback)  
