---
title: "SQL Server 2016에서 지원되지 않는 데이터베이스 엔진 기능 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "02/02/2017"
ms.prod: "sql-server-2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "database-engine"
ms.tgt_pltfrm: ""
ms.topic: "article"
helpviewer_keywords: 
  - "VIA 프로토콜"
  - "지원되지 않는 기능 [SQL Server]"
  - "SQL 메일"
  - "지원되지 않는 기능 [SQL Server]"
  - "RESTORE WITH DBO_ONLY"
  - "BACKUP WITH PASSWORD"
  - "사용자 인스턴스 활성화"
  - "BACKUP WITH MEDIAPASSWORD"
  - "AWE"
  - "SQL-DMO"
  - "*= 및 =*"
  - "80 호환성 수준"
  - "COMPUTE BY"
  - "사용자 인스턴스 제한 시간"
  - "sp_dropalias"
  - "COMPUTE"
  - "WITH APPEND"
  - "sys.database_principal_aliases"
  - "sp_dboption"
  - "DATABASEPROPERTY"
  - "FASTFIRSTROW 힌트"
  - "SET DISABLE_DEF_CNST_CHK"
ms.assetid: d686cdf0-d11d-4dba-9ec8-de1a5f189f25
caps.latest.revision: 100
author: "BYHAM"
ms.author: "rickbyh"
manager: "jhubbard"
caps.handback.revision: 100
---
# SQL Server 2016에서 지원되지 않는 데이터베이스 엔진 기능
[!INCLUDE[tsql-appliesto-ss2016-xxxx-xxxx-xxx_md](../includes/tsql-appliesto-ss2016-xxxx-xxxx-xxx-md.md)]

  이 항목에서는 [!INCLUDE[ssDE](../includes/ssde-md.md)] 에서 더 이상 사용할 수 없는 [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)]기능에 대해 설명합니다.  
  
## <a name="discontinued-features-in-includesssql15tokensssql15mdmd"></a>[!INCLUDE[ssSQL15](../includes/sssql15-md.md)]에서 지원되지 않는 기능  
  
-   [!INCLUDE[ssSQL15](../includes/sssql15-md.md)]는 64비트 응용 프로그램입니다. 일부 요소는 32비트 구성 요소로 실행되지만 32비트 설치는 중단되었습니다.  
  
-   호환성 수준 90이 중단되었습니다. 자세한 내용은 [ALTER DATABASE 호환성 수준&#40;Transact-SQL&#41;](../Topic/ALTER%20DATABASE%20Compatibility%20Level%20\(Transact-SQL\).md)을 참조하세요.  

-   ActiveX 하위 시스템이 중단되었습니다. 명령줄 또는 PowerShell 스크립트를 대신 사용 합니다.
  
## <a name="previous-versions"></a>이전 버전  
  
-   [SQL Server 2014에서 지원되지 않는 데이터베이스 엔진 기능](https://msdn.microsoft.com/library/ms144262\(v=sql.120\))  
  
-   [SQL Server 2012에서 지원되지 않는 데이터베이스 엔진 기능](https://msdn.microsoft.com/library/ms144262\(v=sql.110\))  
  
-   [SQL Server 2008에서 지원되지 않는 데이터베이스 엔진 기능](https://msdn.microsoft.com/library/ms144262\(v=sql.100\))  
  
## <a name="see-also"></a>관련 항목:  
 [SQL Server 2016 이후에는 지원되지 않는 데이터베이스 엔진 기능](../database-engine/deprecated-database-engine-features-in-sql-server-2016.md)   
 [SQL Server 복제에서 사용되지 않는 기능](../relational-databases/replication/deprecated-features-in-sql-server-replication.md)  
  
 