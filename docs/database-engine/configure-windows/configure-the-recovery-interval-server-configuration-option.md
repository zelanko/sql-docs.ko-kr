...
helpviewer_keywords: 
  ...
  - "기본 recovery interval 옵션"
  - "시간 [SQL Server], 복구 간격"
  - "복구 간격 [SQL Server]"
  - "데이터베이스 복구당 최대 시간(분)"
  - "복구 [SQL Server], recovery interval 옵션"
...
-   복구 간격은 기본 대상 복구 시간(0)을 사용하는 데이터베이스에만 영향을 줍니다. 데이터베이스에 대한 서버 복구 간격을 재정의하려면 데이터베이스에 기본이 아닌 대상 복구 시간을 구성합니다. 자세한 내용은 [데이터베이스의 대상 복구 시간 변경&#40;SQL Server&#41;](../../relational-databases/logs/change-the-target-recovery-time-of-a-database-sql-server.md)을 참조하세요.  
...
3.  다음 예를 복사하여 쿼리 창에 붙여 넣고 **실행**을 클릭합니다. 이 예에서는 [sp_configure](../../relational-databases/system-stored-procedures/sp-configure-transact-sql.md)를 사용하여 `recovery interval` 옵션 값을 `3`분으로 설정하는 방법을 보여 줍니다.  
...
```  
 ...
 자세한 내용은 [서버 구성 옵션&#40;SQL Server&#41;](../../database-engine/configure-windows/server-configuration-options-sql-server.md)을 참조하세요.  
...
## 참고 항목  
 ...
 [show advanced options 서버 구성 옵션](../../database-engine/configure-windows/show-advanced-options-server-configuration-option.md)   
