...
helpviewer_keywords: 
  - "기본 프로토콜"
  - "네트워크 프로토콜 [SQL Server], 클라이언트 구성"
  - "TCP/IP [SQL Server], 클라이언트 프로토콜"
  - "클라이언트 프로토콜 해제"
  - "프로토콜 정렬 [SQL Server]"
  - "프로토콜 [SQL Server], 클라이언트 컴퓨터 순서"
  - "클라이언트 프로토콜 구성"
  - "클라이언트 프로토콜 [SQL Server]"
  - "프로토콜 [SQL Server], 클라이언트 구성"
  - "기본 프로토콜, 클라이언트"
...
-   sqlcmd.exe 같은 일부 클라이언트 응용 프로그램은 연결 문자열의 일부로 프로토콜을 지정할 수 있습니다. 자세한 내용은 [sqlcmd를 사용하여 데이터베이스 엔진에 연결](../../relational-databases/scripting/connect-to-the-database-engine-with-sqlcmd.md)을 참조하세요.  
...
###  <a name="Configure"></a> 클라이언트가 TCP/IP를 사용하도록 구성하려면  
...
## 참고 항목  
 [remote login timeout 서버 구성 옵션 구성](../../database-engine/configure-windows/configure-the-remote-login-timeout-server-configuration-option.md)  
