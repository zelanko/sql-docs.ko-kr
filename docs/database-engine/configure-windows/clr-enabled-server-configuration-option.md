...
>  **경량 풀링에서는 CLR(공용 언어 런타임) 실행이 지원되지 않습니다.** "clr enabled" 또는 "lightweight pooling" 옵션 중 하나를 해제하세요. CLR에 의존하며 파이버 모드에서 제대로 작동하지 않는 기능에는 **hierarchy** 데이터 형식, 복제, 정책 기반 관리 등이 있습니다.  
> 
> [!WARNING]
