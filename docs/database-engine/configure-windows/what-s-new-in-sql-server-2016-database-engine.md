---
title: "SQL Server 2016(데이터베이스 엔진)의 새로운 기능 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
  - "SQL2016_rc1"
ms.date: "01/13/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "새로운 기능 [SQL Server 데이터베이스 엔진]"
  - "데이터베이스 엔진 [SQL Server], 새로운 기능"
...
---
# SQL Server 2016(데이터베이스 엔진)의 새로운 기능
...
- [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]을 다운로드하려면 **[평가 센터](https://www.microsoft.com/en-us/evalcenter/evaluate-sql-server-2016)**![download](../../analysis-services/media/download.png "download")로 이동하세요.

- Azure 계정이 있으세요?  계정이 있는 경우 **[여기](https://azure.microsoft.com/en-us/marketplace/partners/microsoft/sqlserver2016rc0evaluationwindowsserver2012r2/?wt.mc_id=sqL16_vm)**로 이동하여 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]이 이미 설치된 가상 컴퓨터를 실행해 보세요.
...
-     `OPTION (USE HINT('<hint1>', '<hint2>'))` 같은 보다 일반적인 쿼리 힌트 모델에 대한 지원이 추가되었습니다. 자세한 내용은 [쿼리 힌트(Transact-SQL)](Query%20Hints%20\(Transact-SQL\).md)를 참조하세요.  
...
-  더 많은 버전에서 다양한 기능을 지원합니다. 예를 들면 행 수준 보안, Always Encrypted, 동적 데이터 마스킹, 데이터베이스 감사, 메모리 내 OLTP 등 기타 몇몇 다른 기능이 모든 버전에 추가되었습니다. 자세한 내용은 [SQL Server 2016의 버전과 지원하는 기능](../../sql-server/sql-server-2016의-버전과-지원하는-기능.md)을 참조하세요.   
...
####  <a name="a-namecolumnstorea-columnstore-indexes"></a><a name="columnstore"></a> Columnstore 인덱스
...
자세한 내용은 온라인 설명서의 [Columnstore 인덱스 가이드](../Topic/Columnstore%20Indexes%20Guide.md) 섹션을 참조하세요.

- [Columnstore 인덱스 버전형 기능 요약](../Topic/Columnstore%20Indexes%20Versioned%20Feature%20Summary.md) – 새로운 기능을 포함합니다.
...
####  <a name="a-namescopedconfigurationa-database-scoped-configurations"></a><a name="scopedconfiguration"></a> 데이터베이스 범위 구성
...
####  <a name="a-nameinmemorya-in-memory-oltp"></a><a name="InMemory"></a> 메모리 내 OLTP
...
- [메모리 내 OLTP에 대해 지원되지 않는 SQL Server 기능](Unsupported%20SQL%20Server%20Features%20for%20In-Memory OLTP.md)
...
- 더 이상 데이터 크기 제한이 없습니다. [메모리 액세스에 최적화된 테이블에 필요한 메모리 예측](Estimate%20Memory%20Requirements%20for%20Memory-Optimized Tables.md)을 참조하세요.
...
- [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 데이터베이스에서 여러 개체의 마이그레이션 적합도를 평가하기 위한 [마이그레이션 평가용 PowerShell Cmdlet](../../relational-databases/in-memory-oltp/powershell-cmdlet-for-migration-evaluation.md)이 향상되었습니다.
...
####  <a name="a-namequeryoptimizera-query-optimizer"></a><a name="QueryOptimizer"></a> 쿼리 최적화 프로그램
...
####  <a name="a-namelivestatsa-live-query-statistics"></a><a name="LiveStats"></a> 활성 쿼리 통계
...
####  <a name="a-namequerystorea-query-store"></a><a name="QueryStore"></a> 쿼리 저장소
...
####  <a name="a-nametta-temporal-tables"></a><a name="TT"></a> 임시 테이블
...
####  <a name="a-namestripedbackuptoazurea-striped-backups-to-microsoft-azure-blob-storage"></a><a name="StripedBackupToAzure"></a> Microsoft Azure Blob Storage에 대한 스트라이프 백업
...
####  <a name="a-namefilesnapshotbackupa-file-snapshot-backups-to-microsoft-azure-blob-storage"></a><a name="FileSnapshotBackup"></a> Microsoft Azure Blob Storage에 대한 파일-스냅숏 백업
...
####  <a name="a-namemanagedbackupa-managed-backup"></a><a name="ManagedBackup"></a> Managed Backup
...
####  <a name="a-namemultipletempdba-tempdb-database"></a><a name="multipleTempDB"></a> TempDB 데이터베이스
...
####  <a name="a-nameforjsona-built-in-json-support"></a><a name="ForJson"></a> 기본 제공 JSON 지원
...
####  <a name="a-nameutf8a-support-for-utf-8"></a><a name="UTF8"></a> UTF-8에 대한 지원
...
####  <a name="a-namedefaultdba-new-default-database-size-and-autogrow-values"></a><a name="DefaultDB"></a> 새로운 기본 데이터베이스 크기 및 자동 증가 값
model 데이터베이스의 새 값 및 모델을 기반으로 하는 새 데이터베이스의 기본값이 추가되었습니다. 이제 데이터 및 로그 파일의 초기 크기는 8MB입니다. 이제 데이터 및 로그 파일의 기본 자동 증가는 64MB입니다.


###  <a name="a-nametsqla-transact-sql-enhancements"></a><a name="TSQL"></a> 향상된 Transact-SQL 기능
...
- 새 쿼리 힌트 **NO_PERFORMANCE_SPOOL**은 스풀 연산자가 쿼리 계획에 추가되지 않도록 합니다. 그 결과, 스풀 작업에서 많은 동시 쿼리가 실행 중일 때 성능이 향상될 수 있습니다. 자세한 내용은 [쿼리 힌트&#40;Transact-SQL&#41;](../Topic/Query%20Hints%20\(Transact-SQL\).md)를 참조하세요.
...
- InstanceDefaultDataPath, InstanceDefaultLogPath, ProductBuild, ProductBuildType, ProductMajorVersion, ProductMinorVersion, ProductUpdateLevel 및 ProductUpdateReference라는&8;개의 새로운 속성이 [SERVERPROPERTY&#40;Transact-SQL&#41;](../../t-sql/functions/serverproperty-transact-sql.md)에 추가되었습니다.
...
- 새로운&7;개의 뷰가 쿼리 저장소 기능을 지원합니다. 자세한 내용은 [쿼리 저장소 카탈로그 뷰&#40;Transact-SQL&#41;](../../relational-databases/system-catalog-views/query-store-catalog-views-transact-sql.md)를 참조하세요.
- 메모리 부여에 대한 정보를 제공하는 새로운&24;개의 열이 [sys.dm_exec_query_stats&#40;Transact-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-exec-query-stats-transact-sql.md)에 추가되었습니다.
- 메모리 부여를 지정하도록 새로운 두 개의 쿼리 힌트(MIN_GRANT_PERCENT 및 MAX_GRANT_PERCENT)가 추가되었습니다. [쿼리 힌트&#40;Transact-SQL&#41;](../Topic/Query%20Hints%20\(Transact-SQL\).md)를 참조하세요.
...
###  <a name="a-namesecuritya-security-enhancements"></a><a name="Security"></a> 향상된 보안 기능

####  <a name="a-namerlsa-row-level-security"></a><a name="RLS"></a> 행 수준 보안
...
####  <a name="a-nametcea-always-encrypted"></a><a name="TCE"></a> 상시 암호화
...
####  <a name="a-namemaskinga-dynamic-data-masking"></a><a name="Masking"></a> 동적 데이터 마스킹
...
####  <a name="a-namepermsa-new-permissions"></a><a name="Perms"></a> 새 사용 권한
...
####  <a name="a-nametdea-transparent-data-encryption"></a><a name="TDE"></a> 투명한 데이터 암호화
- 암호화의 Intel AES-NI 하드웨어 가속이 지원되어 투명한 데이터 암호화가 향상되었습니다. 그 결과, 투명한 데이터 암호화를 켜는 경우 CPU 오버헤드가 감소합니다.

###  <a name="a-nameaesa-aes-encryption-for-endpoints"></a><a name="AES"></a> 끝점에 대한 AES 암호화
- 끝점에 대한 기본 암호화가 RC4에서 AES로 변경되었습니다.

####  <a name="a-namenewcredentialtypea-new-credential-type"></a><a name="newcredentialtype"></a> 새 자격 증명 유형
- 이전에 제공된 서버 수준 자격 증명 이외에 이제 데이터베이스 수준에서 자격 증명을 만들 수 있습니다. 자세한 내용은 [CREATE DATABASE SCOPED CREDENTIAL&#40;Transact-SQL&#41;](../../t-sql/statements/create-database-scoped-credential-transact-sql.md)을 참조하세요.


###  <a name="a-namehighavailabilitya-high-availability-enhancements"></a><a name="HighAvailability"></a> 향상된 고가용성 기능
...
**성능 향상** – 가용성 그룹의 동기화 처리량은 주 복제본에서 로그 블록의 병렬 및 빠른 압축, 최적화된 동기화 프로토콜, 보조 복제본에서 로그 레코드 다시 실행 및 병렬 압축 풀기를 통해&10;배까지 증가되었습니다. 따라서 읽기 가능한 보조 복제본의 새로 고침이 증가되고 장애 조치(failover) 시 데이터베이스 복구 시간이 줄어듭니다. 메모리 액세스에 최적화된 테이블에 대한 다시 실행은 SQL Server 2016에서 아직 병렬로 수행되지 않습니다.

###  <a name="a-namerepla-replication-enhancements"></a><a name="Repl"></a> 향상된 복제 기능
...
###  <a name="a-nametoolsa-tools-enhancements"></a><a name="Tools"></a> 향상된 도구 기능
...
- SQL Server 관리 도구는 더 이상 주 기능 트리에서 설치되지 않습니다. 자세한 내용은 [SSMS로 SQL Server 관리 도구 설치](Install%20SQL%20Server%20Management%20Tools%20\(SSMS\).md)를 참조하세요.
...
####  <a name="a-nameuaa-upgrade-advisor"></a><a name="UA"></a> 업그레이드 관리자
...
[SSMS로 SQL Server 관리 도구 설치](Install%20SQL%20Server%20Management%20Tools%20\(SSMS\).md)
