...
description: SQL Server에서 CLR(공용 언어 런타임) 엄격한 보안을 구성하는 방법을 알아봅니다. SAFE, EXTERNAL ACCESS 및 UNSAFE 권한의 해석을 제어합니다.
ms.custom: ''
ms.date: 06/20/2017
ms.prod: sql
...
|1 |Enabled - [!INCLUDE[ssde-md](../../includes/ssde-md.md)]에서 어셈블리에 대한 `PERMISSION_SET` 정보를 무시하도록 하고 항상 `UNSAFE`로 해석합니다.  [!INCLUDE[sssqlv14](../../includes/sssqlv14-md.md)]부터 `Enabled`가 기본값입니다. | 
