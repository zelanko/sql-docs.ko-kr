...
-   SQL Server 2016 Express를 다운로드하고 설치하려면 **[다운로드 센터](http://www.microsoft.com/download/details.aspx?id=52679)**로 이동하세요. LocalDB는 설치 중 선택하는 기능입니다. **다운로드 센터**에서 LocalDB는 **사용자 지정 설치** 또는 미디어를 다운로드할 때 이용할 수 있습니다. 미디어를 다운로드하려면 **Express Advanced** 또는 **LocalDB** 패키지를 선택합니다. 
  
-   Azure 계정이 있으세요?  계정이 있는 경우 **[여기](https://azure.microsoft.com/en-us/marketplace/partners/microsoft/sqlserver2016ctp33evaluationwindowsserver2012r2/?wt.mc_id=sqL16_vm)** 로 이동하여 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 이 이미 설치된 가상 컴퓨터를 실행해 보세요.  
...
## LocalDB의 공유 인스턴스  
 ...
 컴퓨터의 관리자만 **LocalDB**의 공유 인스턴스를 만들 수 있습니다. **LocalDB** 의 공유 인스턴스는 관리자 또는 **LocalDB**의 공유 인스턴스 소유자가 공유를 해제할 수 있습니다. **LocalDB** 인스턴스를 공유 및 공유 해제하려면 **LocalDB** API의 `LocalDBShareInstance` 및 `LocalDBUnShareInstance` 메서드를 사용하거나 SqlLocalDb 유틸리티의 공유 및 공유 해제 옵션을 사용합니다.  
 ...
 **LocalDB**의 공유 인스턴스에 연결하려면 공유 인스턴스에 대해 예약된 네임스페이스를 참조하기 위해 연결 문자열에 **.\\** (점과 백슬래시)를 추가합니다. 예를 들어 **라는** LocalDB `AppData` 의 공유 인스턴스에 연결하려면 연결 문자열의 일부로 `(localdb)\.\AppData` 와 같은 연결 문자열을 사용합니다. 자신이 소유하고 있지 않은 **LocalDB** 공유 인스턴스에 연결하는 사용자는 Windows 인증 또는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인증 로그인을 갖고 있어야 합니다.  
