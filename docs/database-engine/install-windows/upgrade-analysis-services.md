...
helpviewer_keywords: 
  - "데이터베이스 업그레이드"
  - "데이터베이스 [Analysis Services], 업그레이드"
  - "Analysis Services 설치, 함께 설치"
  - "Analysis Services 업그레이드"
  - "Analysis Services 업그레이드, Analysis Services 업그레이드 정보"
  - "SSAS, 데이터베이스 마이그레이션"
  - "Analysis Services 업그레이드"
  - "Analysis Services 설치, 업그레이드"
  - "SSAS, 업그레이드"
...
---
# Analysis Services 업그레이드
  [What's New in Analysis Services](../../analysis-services/what-s-new-in-analysis-services.md)에 설명된 대로 현재 릴리스에 도입된 기능을 활용하기 위해 동일한 서버 모드의 SQL Server 2016 버전으로 Analysis Services 인스턴스를 업그레이드할 수 있습니다.  
...
-   [SQL Server 2016 릴리스 정보](http://go.microsoft.com/fwlink/?LinkID=398124) 는 알려진 문제 및 해결 방법을 설명합니다.  
...
## 1200 CompatibilityLevel에서 테이블 형식 모델에 대한 업그레이드 후  
...
## DirectQuery 모드에서 테이블 형식 모델 업그레이드  
...
4.  **DirectQuery** 을(를) 다시 켭니다. 자세한 지침은 [DirectQuery for Tabular 1200 models](../Topic/DirectQuery%20for%20Tabular%201200%20models.md) 을(를) 참조하세요.  
