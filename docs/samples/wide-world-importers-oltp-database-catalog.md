...
|Purchasing|공급 업체의 재고 항목 구매 및 공급 업체에 대 한 세부 정보입니다.|  
...
|스키마|Description|
|-----------------------------|---------------------|
|웹 사이트|회사 웹 사이트의 데이터베이스에 대 한 모든 액세스는이 스키마를 통해 실행 됩니다.|
...
|Countries|주 또는도는 국가의 일부입니다. 이 표에는 각 국가의 경계를 설명 하는 공간 데이터를 비롯 하 여이에 대 한 세부 정보가 있습니다.|
...
- 다음 테이블에는 단일 시퀀스 (TransactionID)가 사용 됩니다. CustomerTransactions, SupplierTransactions 및 StockItemTransactions입니다. 테이블 집합에서 단일 시퀀스를 사용할 수 있는 방법을 보여 줍니다.
...
|Configuration_ApplyFullTextIndexing|`Application.People`, ,`Sales.Customers`및 에전체텍스트인덱스`Warehouse.StockItems`를 적용 합니다. `Purchasing.Suppliers` `Website.SearchForPeople` ,`Website.SearchForSuppliers`,, 를`Website.SearchForStockItemsByTags`전체 텍스트 인덱싱을 사용 하는 대체 프로시저로 바꿉니다. `Website.SearchForStockItems` `Website.SearchForCustomers`|
...
|Configuration_EnableInMemory|메모리 최적화 파일 그룹 (Azure에서 작업 하지 않는 경우)을 추가 하 `Warehouse.ColdRoomTemperatures`고 `Warehouse.VehicleTemperatures` , 메모리 내 항목으로 대체 `Website.OrderList`하 고, 데이터를 마이그레이션하고, `Website.OrderIDList`,, `Website.OrderLineList`, `Website.SensorDataList` 테이블 형식을로 다시 만듭니다. 메모리 액세스에 최적화 된 해당 테이블 형식을 사용 하 `Website.InvoiceCustomerOrders`는 `Website.InsertCustomerOrders`, 및 `Website.RecordColdRoomTemperatures` 프로시저를 삭제 하 고 다시 만듭니다.|
