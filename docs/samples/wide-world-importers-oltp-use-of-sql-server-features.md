...
|JSON에 대 한 AJAX 호출|응용 프로그램은 종종 AJAX 호출을 사용 하 여 다음 테이블을 쿼리 합니다. 사람, 고객, 공급 업체 및 StockItems. 호출은 JSON 형식으로 데이터를 반환 합니다. 예를 들어 저장 프로시저 `Website.SearchForCustomers`를 참조 하세요.|
|JSON 속성/값 모음|테이블의 관계형 데이터를 확장 하기 위해 JSON 데이터를 포함 하는 열이 있는 테이블이 여러 개 있습니다. 예 `Application.SystemParameters` 를 들어에는 응용 프로그램 설정에 대 `Application.People` 한 열이 있고 사용자 기본 설정을 기록 하는 열이 있습니다. 이러한 테이블은 열 `nvarchar(max)` 값이 유효한 json 인지 확인 하기 위해 기본 제공 함수 `ISJSON` 를 사용 하 여 CHECK 제약 조건과 함께 json 데이터를 기록 하는 열을 사용 합니다.|
...
|항상 암호화|Always Encrypted에 대 한 데모는 [샘플 릴리스의](https://go.microsoft.com/fwlink/?LinkID=800630)일부인 다운로드 가능한 샘플 .zip에 포함 되어 있습니다. 데모에서는 암호화 키, 중요 한 데이터에 대 한 암호화를 사용 하는 테이블 및 테이블에 데이터를 삽입 하는 작은 샘플 응용 프로그램을 만듭니다.|
...
|테이블 분할|(데이터베이스의 전체 버전) 및 `Sales.CustomerTransactions` `PF_TransactionDate` `PS_TransactionDate`테이블은 모두 파티션 함수 및 파티션 구성표를 사용 하 여 연도별로 분할 됩니다. `Purchasing.SupplierTransactions` 분할은 많은 테이블의 관리 효율성을 향상 시키는 데 사용 됩니다.|
|목록 처리|예제 테이블 형식이 `Website.OrderIDList` 제공 됩니다. 예제 프로시저 `Website.InvoiceCustomerOrders`에서 사용 됩니다. 이 절차에서는 Cte (공통 테이블 식), TRY/CATCH, JSON_MODIFY, XACT_ABORT, NOCOUNT, THROW 및 XACT_STATE를 사용 하 여 응용 프로그램에서로의 왕복을 최소화 하기 위해 단일 순서만이 아닌 주문 목록을 처리 하는 기능을 보여 줍니다. 데이터베이스 엔진.|
...
|쿼리 저장소|데이터베이스에서 쿼리 저장소를 사용할 수 있습니다. 몇 가지 쿼리를 실행 한 후에는 다음 단계를 수행 합니다.<br/><br/>1. Management Studio에서 데이터베이스를 엽니다.<br/>2. 데이터베이스 아래에 있는 노드 쿼리 저장소를 엽니다.<br/>3. 보고서 *상위 리소스 소비 쿼리*를 엽니다. 쿼리 실행을 참조 하 고 방금 실행 한 쿼리에 대 한 계획을 참조 하세요.|
|STRING_SPLIT|`DeliveryInstructions` 테이블`Sales.Invoices` 의 열은 STRING_SPLIT를 설명 하는 데 사용할 수 있는 쉼표로 구분 된 값입니다.|
